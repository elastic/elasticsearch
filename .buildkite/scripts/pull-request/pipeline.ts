...
export const generatePipelines = (
  ...
  if (!changedFiles?.length) {
    const mergeBase = execSync(`git fetch origin ${process.env["GITHUB_PR_TARGET_BRANCH"]}; git merge-base origin/${process.env["GITHUB_PR_TARGET_BRANCH"]} HEAD`,, { cwd: PROJECT_ROOT })
      .toString()
      .trim();

...
- ES|QL CSV tests: `./gradlew ":x-pack:plugin:esql:csvInternalClusterTest" --tests "org.elasticsearch.xpack.esql.Csv<PascalCaseName>IT"` (e.g. `--tests "...CsvStatsFirstLastIT"` for `stats_first_last.csv-spec`); append `.*<test-name>*` to target a single test within the file.

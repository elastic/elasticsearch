...
To quickly start a homogeneous multi-node cluster without editing any files, use `-Drun.nodes=N`:
-------------------------------------
./gradlew run -Drun.nodes=3
-------------------------------------
...
The same flag works for `internalClusterTest` tasks (embedded cluster, same JVM as Gradle).
For `yamlRestTest` and `javaRestTest`, the test process is a separate JVM and does not
inherit the Gradle property.  Without it, `LocalDistributionResolver` defaults
`build.snapshot` to `true` and tries to locate a `*-SNAPSHOT` distribution that was never
built, causing a `NoSuchFileException` before any test runs.  Pass the flag explicitly:

------------------------------
./gradlew :some:project:yamlRestTest \

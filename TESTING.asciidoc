...
To test you can instruct gradle build that BWC version from a local branch instead of pulling the
release branch from GitHub. You do so using the `tests.bwc.branch` system property:

-------------------------------------------------
gradle check -Dtests.bwc.branch=origin/index_req_bwc_5.x
-------------------------------------------------

The branch needs to be available on local clone that the BWC makes of the repository you run the
...
. Run the tests with `gradle check -Dtests.bwc.branch=origin/index_req_bwc_5.x`

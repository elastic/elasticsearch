...
It is possible to provide a test version name that allows to adapt the tests
behaviour to older features or bugs that have been changed or fixed in the
meantime. The following command disables the randomization of number of
shards and replicas:

-----------------------------------------
mvn test -Dtests.compatibility=default_num_shards_replicas test
-----------------------------------------

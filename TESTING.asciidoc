...
However, this requires modifying a source controlled file and local customizations should not be committed.
To avoid accidentally committing your local customizations you can you a Gradle init script with the -I flag
to configure this task locally.

For example:

To enable HTTPS for use with ./gradlew run a extraConfigFile is needed to be added to the cluster configuration.
...
Now when the http.p12 file will be placed in the config directory of the running cluster and available for use for use.

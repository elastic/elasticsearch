...
nodes. Since we are changing the default master node configuration we must also update the initial_master_nodes.
Finally, we need to "restart" the cluster to pick up the changes. However, note it does not actually start/stop/start

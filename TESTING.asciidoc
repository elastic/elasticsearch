...
It will then run the tests on every available system. This will take a very 
long time.
+
...
`DistroTestPlugin` plugin; we don't have a single command that will spin up and
prepare a Vagrant VM but stop just short of running the tests.

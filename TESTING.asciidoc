...
./gradlew :distribution:archives:integ-test-zip:integTestRunner \
 --tests "org.elasticsearch.test.rest.IntegTestZipTestRestApiSpec"
---------------------------------------------------------------------------
...
./gradlew :x-pack:qa:rolling-upgrade:v7.7.0#bwcTest \
 -Dtests.class=org.elasticsearch.upgrades.UpgradeClusterTestRestApiSpec\
...
* **`ESTestCase`**: The base class of all tests. It is typically extended
  directly by unit tests. Tests that extend this should have a name that ends
  with `Tests`
* **`ESSingleNodeTestCase`**: This test case sets up a cluster that has a
  single node. Tests that extend this should have a name that ends
  with `Tests`
* **`ESIntegTestCase`**: An integration test case that creates a cluster that
  might have multiple nodes.  Tests that extend this should have a name that ends
  with `IT`
* **`ESRestTestCase`**: An integration tests that interacts with an external
  cluster via the REST API. This is used for Java based REST tests.  Tests that
  extend this should have a name that ends with `IT`
* **`ESClientYamlSuiteTestCase` **: A subclass of `ESRestTestCase` used to run
  YAML based REST tests. Tests that extend this should have a name that ends
  with `TestRestApiSpec`

...
/**
 * Adapts an {@code org.apache.logging.log4j.Logger} to the {@link Logger} SPI used by
 * benchmarked production code. Mirrors server's {@code LoggerImpl} but lives here so
 * {@code benchmarks/common} does not have to depend on {@code :server}. When server's
 * log4j impl classes move into a shared library, this class can be dropped.
 */
final class BenchmarkLogger implements Logger {

    private final org.apache.logging.log4j.Logger log4jLogger;

    BenchmarkLogger(org.apache.logging.log4j.Logger log4jLogger) {
    ...
    public void log(Level level, String message) {
        log4jLogger.log(org.apache.logging.log4j.Level.valueOf(level.name()), message);
    }

    @Override
    public void log(Level level, Supplier<String> messageSupplier, Throwable throwable) {
        log4jLogger.log(org.apache.logging.log4j.Level.valueOf(level.name()), mapSupplier(messageSupplier), throwable);
    }
    ...
    public boolean isEnabled(Level level) {
        return log4jLogger.isEnabled(org.apache.logging.log4j.Level.valueOf(level.name()));
    }

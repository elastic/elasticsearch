...
// NOTE: Encoding benchmark using NumericDataGenerators double data sources.
// Doubles are converted to sortable longs via NumericUtils.doubleToSortableLong
// during setup, matching Lucene's doc-values storage format.
@Fork(value = 1)
@Warmup(iterations = 3)
...
public class EncodeDoubleGeneratorsBenchmark {

    // NOTE: 21 native double patterns + 9 long-as-double (all longs except boundary).
    // If NumericDataGenerators adds/removes sources, update this list and the sanity check will catch drift.
    @Param(
        {
            "constant-double",
            "percentage-double",

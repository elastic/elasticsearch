...
/**
 * Small segment write benchmark: indexes a fixed number of documents with one sorted
 * numeric doc values field and commits every {@code docsPerSegment} documents, producing
 * many small segments per measurement. Runs entirely in memory on a
 * {@code ByteBuffersDirectory} with merging disabled, so the measurement isolates
 * the per segment fixed overhead of each TSDB doc values format (codec init, per field
 ...
 *
 * <pre>{@code
 * ./gradlew :benchmarks:run --args="TSDBDocValuesSmallSegmentWriteBenchmark \
...
public class TSDBDocValuesSmallSegmentWriteBenchmark {
    ...
    @Benchmark
    public void smallSegmentWriteES819DocValuesFormat() throws IOException {
        writeInSmallSegments(es819Writer);
    }

    @Benchmark
    public void smallSegmentWriteES95DocValuesFormat() throws IOException {
        writeInSmallSegments(es95Writer);
    }

    @Benchmark
    public void smallSegmentWriteES95UncachedDocValuesFormat() throws IOException {
        writeInSmallSegments(es95UncachedWriter);
    }

    private void writeInSmallSegments(final IndexWriter writer) throws IOException {

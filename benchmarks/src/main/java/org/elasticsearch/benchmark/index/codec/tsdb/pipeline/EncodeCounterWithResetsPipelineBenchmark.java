...
public class EncodeCounterWithResetsPipelineBenchmark {
    private static final int SEED = 17;

    @Param({ "0.0", "0.02", "0.05", "0.1" })
    private double resetProbability;

    @Param({ "10" })
    private int blocksPerInvocation;

    // NOTE: "full" is ES87-compatible; "counterWithResets" is optimal for this data pattern (skips gcd)
    @Param({ "full", "counterWithResets" })
    private String pipeline;

    private AbstractPipelineBenchmark encode;
    private CounterWithResetsSupplier supplier;

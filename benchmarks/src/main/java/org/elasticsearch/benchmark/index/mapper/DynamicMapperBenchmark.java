...
public class DynamicMapperBenchmark {
    ...
    private SourceToParse generateRandomDocument() {
        ...
        for (int i = 0; i < intFields; i++) {
            if (random.nextBoolean()) {
                int fieldValueCount = random.nextInt(fieldValueCountMax);
                builder.append("\"int_field_").append(i).append("\":").append(Arrays.toString(IntStream.generate(() -> random.nextInt()).limit(fieldValueCount).toArray())).append(",");
            }
        }
        for (int i = 0; i < floatFields; i++) {
            if (random.nextBoolean()) {
                int fieldValueCount = random.nextInt(fieldValueCountMax);
                builder.append("\"float_field_").append(i).append("\":").append(Arrays.toString(DoubleStream.generate(() -> random.nextFloat()).limit(fieldValueCount).toArray())).append(",");
            }
        }
        for (int i = 0; i < objFields; i++) {
            ...
            for (int j = 0; j < textFields; j++) {
                if (random.nextBoolean()) {
                    ...
                    StringBuilder fieldValueBuilder = new StringBuilder();
                    fieldValueBuilder.append("[");
                    for (int k = 0; k < fieldValueCount - 1; k++) {
                        fieldValueBuilder.append("\"").append(randomString(6)).append("\"").append(",");
                    }
                    fieldValueBuilder.append("\"").append(randomString(6)).append("\"").append("]");
                    builder.append("\"" + objFieldPrefix +".text_field_").append(j).append("\":").append(fieldValueBuilder).append(",");
                }
            }
            for (int j = 0; j < intFields; j++) {
                if (random.nextBoolean()) {
                    int fieldValueCount = random.nextInt(fieldValueCountMax);
                    builder.append("\"" + objFieldPrefix +".int_field_").append(j).append("\":").append(Arrays.toString(IntStream.generate(() -> random.nextInt()).limit(fieldValueCount).toArray())).append(",");
                }
            }
            for (int j = 0; j < floatFields; j++) {
                if (random.nextBoolean()) {
                    int fieldValueCount = random.nextInt(fieldValueCountMax);
                    builder.append("\"" + objFieldPrefix +".float_field_").append(j).append("\":").append(Arrays.toString(DoubleStream.generate(() -> random.nextFloat()).limit(fieldValueCount).toArray())).append(",");
                }
            }
        }
    ...
    @Benchmark
    public List<LuceneDocument> benchmarkParseKeywordFields() throws Exception {
        for (int i = 0; i < 25; i++) {

...
/**
 * Kernel-direct benchmark for the symmetric 2-bit-doc / 2-bit-query striped (bit-plane) BBQ
 * dot product, dispatching straight to {@code vec_dotd2q2} via {@link VectorSimilarityFunctions}
 ...
 * </ul>
 * Sparse is intentionally skipped: OSQ's {@code NativeMemorySegmentScorer} does not use it for D2Q2.
 * <p>
 * Run with: {@code ./gradlew -p benchmarks run --args 'VectorScorerD2Q2StripedOperationBenchmark'}
 */
...
@State(Scope.Thread)
public class VectorScorerD2Q2StripedOperationBenchmark {
    ...
    // query: queryBytes (2 bit-planes back-to-back, packed striped)
    ...
    static final class VectorData extends VectorScorerBulkBenchmark.VectorData {
        ...
        VectorData(int dims, int numVectors, int numVectorsToScore, Random random) {
            ...
            randomBytes(random, unpackedQuery, 4);
            packedQuery = BBQTestUtils.packStriped(unpackedQuery, 2);
        }
    ...
    void setup(VectorData vectorData) {
        assert dims % 8 == 0 : "D2Q2 striped requires dims divisible by 8: " + dims;
    ...
    public float[] scoreSingle() {
        ...
        while (v < numVectorsToScore) {
            for (int i = 0; i < bulkSize && v < numVectorsToScore; i++, v++) {
                ...
                scores[i] = vectorSimilarityFunctions.dotProductD2Q2(vec, query, docBytes);
            }
        }
        return scores;
    }
    ...
    public float[] scoreBulk() {
        for (int i = 0; i < numVectorsToScore; i += bulkSize) {
            ...
            vectorSimilarityFunctions.dotProductD2Q2Bulk(slice, query, docBytes, count, resultsSeg);
        }
    ...
    public float[] scoreBulkOffsets() {
        for (int i = 0; i < numVectorsToScore; i += bulkSize) {
            ...
            vectorSimilarityFunctions.dotProductD2Q2BulkWithOffsets(dataset, query, docBytes, docBytes, ordinalsSeg, count, resultsSeg);
        }
        MemorySegment.copy(resultsSeg, ValueLayout.JAVA_FLOAT, 0L, scores, 0, scores.length);
        return scores;
    }

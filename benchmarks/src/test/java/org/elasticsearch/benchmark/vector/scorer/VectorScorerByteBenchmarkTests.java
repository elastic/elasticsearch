...
public class VectorScorerByteBenchmarkTests extends ESTestCase {

    private final double delta = 1e-3;
    private final VectorSimilarityType function;
    private final int dims;

    public VectorScorerByteBenchmarkTests(VectorSimilarityType function, int dims) {
    ...
    public void testScores() throws Exception {
        for (int i = 0; i < 100; i++) {
            VectorScorerByteBenchmark.VectorData data = new VectorScorerByteBenchmark.VectorData(dims);
            Float expected = null;
            for (var impl : VectorImplementation.values()) {
                var bench = new VectorScorerByteBenchmark();
    ...
    public static Iterable<Object[]> parametersFactory() {
        try {
            String[] dims = VectorScorerByteBenchmark.class.getField("dims").getAnnotationsByType(Param.class)[0].value();
            String[] functions = VectorScorerByteBenchmark.class.getField("function").getAnnotationsByType(Param.class)[0].value();

 ...
 * Registers a {@code generateEsqlSpecTests} task that produces:
 * <ul>
 *   <li>one JUnit test class per spec file for each declared variant, and</li>
 *   <li>one Suite class per variant ({@code <prefix>SuiteIT}) that lists all per-file classes
 *       in category-execution order.</li>
 * </ul>
 * The generated classes live in their own {@code csvSpecTest} source set so the standard
 * {@code javaRestTest} task never sees them.
 *
 ...
 *   forkEvery       = 0   // keep all classes in one JVM for shared static loadedCategory
 *   filter { includeTestsMatching '*.*SuiteIT' }   // run only Suite classes
 * }</pre>
 *
 ...
 *     classpath       = sourceSets.csvSpecTest.runtimeClasspath
 *     forkEvery       = 0
 *     filter { includeTestsMatching '*.*SuiteIT' }
 * }
 * }</pre>
 */
    ...
    public void apply(Project project) {
        ...
        var generateEsqlSpecTestsTaskTaskProvider = project.getTasks()
            .register("generateEsqlSpecTests", GenerateEsqlSpecTestsTask.class, task -> {
                ...
                task.getSpecDataYml().set(extension.getSpecDataYml());
                ...
                task.setDescription("Generates per-csv-spec-file IT classes and Suite classes for each declared variant.");

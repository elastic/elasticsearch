...
public record FlakinessPlan(
    ...
    public FlakinessPlan withCommands(List<PlanCommand> commands) {
        return new FlakinessPlan(buildFailed, reason, entries, expansions, unresolved, commands);
    }

    /**
     * One concrete target. {@code disposition} is {@code "run"} or {@code "skip"}; a {@code skip} carries a
     * {@code reason} (e.g. {@code "bwc"}, which the analyze step folds into {@code not_applicable}). An
     * entry produced by flattening an abstract base carries {@code expandedFrom} = the abstract FQCN.
     */
    @JsonInclude(JsonInclude.Include.NON_NULL)
    ...
    public static FlakinessPlan buildFailed(String reason) {
        return new FlakinessPlan(true, reason, List.of(), List.of(), List.of(), List.of());
    }
}

...
public class ClusterFormationTaskExecutionListener implements TaskExecutionListener, TaskActionListener {
    ...
    public void afterExecute(Task task, TaskState state) {
        ...
        // cluster to start.
        ClusterformationPlugin.getTaskExtension(task).getClaimedClusters().forEach(ElasticsearchCluster::unClaimAndStop);
    }

    @Override
    public void beforeActions(Task task) {
        // we only start the cluster before the actions, so we'll not start it if the task is up-to-date
        ClusterformationPlugin.getTaskExtension(task).getClaimedClusters().forEach(ElasticsearchCluster::start);
    }

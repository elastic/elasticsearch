...
public class JdkDownloadPlugin implements Plugin<Project> {
        ...
        project.afterEvaluate(
            p -> {
                for (Jdk jdk : jdksContainer) {
                    ...
                    depConfig.put(
                        "configuration",
                        configName("extracted_jdk", vendor, version, platform)
                    );
                    dependencies.add(
                        jdk.getConfiguration().getName(),
    ...
    private static void setupRootJdkDownload(
        Project rootProject, String platform, String vendor, String version
    ) {
            ...
            if (vendor.equals("adoptopenjdk")) {
                if (hash != null) {
                    throw new IllegalArgumentException(
                        "adoptopenjdk versions do not have hashes but was [" + version + "]"
                    );
                }
                repositories.ivy(
                    ivyRepo -> {
                        ...
                        ivyRepo.setUrl(
                            "https://artifactory.elstc.co/artifactory/oss-jdk-local/"
                        );
                ...
                if (hash != null) {
                    ...
                    // simpler legacy pattern from JDK 9 to JDK 12 that we are advocating to Oracle
                    // to bring back
                    repositories.ivy(
                        ivyRepo -> {
                            ivyRepo.setName(repoName);
        ...
        String platformDep = platform.equals("darwin")
        ...
        final Provider<Directory> extractPath = rootProject
            .getLayout()
            .getBuildDirectory()
        ...
        if (extension.equals("zip")) {
            ...
            Action<CopySpec> removeRootDir = copy -> {
                ...
                copy.eachFile(
                    details -> {
                        ...
                        String[] newPathSegments = Arrays.copyOfRange(
                            pathSegments,
                            index + 1,
                            pathSegments.length
                        );
                        details.setRelativePath(
                            new RelativePath(true, newPathSegments)
                        );
                    }
                );
                copy.setIncludeEmptyDirs(false);
            };
            extractTask = rootProject
                .getTasks()
                .register(
                    extractTaskName,
                    Copy.class,
            ...
            extractTask = rootProject
                ...
                .register(
                    extractTaskName,
                    SymbolicLinkPreservingUntarTask.class,
                    task -> {
                        ...
                        task.setTransform(
                            name -> {
                                ...
                                if (index + 1 >= entryName.getNameCount()) {
                                    // this happens on the top-level directories
                                    // in the archive, which we are removing
                                    return null;
                                }
                                // finally remove the top-level directories from
                                // the output path
                                return entryName.subpath(
                                    index + 1,
                                    entryName.getNameCount()
                                );
                            }
                        );
                    }
        ...
        rootProject
            .getArtifacts()
            .add(localConfigName, extractPath, artifact -> artifact.builtBy(extractTask));
    }
    ...
    private static String configName(
        String prefix, String vendor, String version, String platform
    ) {

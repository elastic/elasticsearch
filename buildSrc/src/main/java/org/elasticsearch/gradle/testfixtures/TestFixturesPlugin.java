...
public class TestFixturesPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        TestFixtureExtension extension =
                project.getExtensions().create("testFixtures", TestFixtureExtension.class, project);
        ...
        if (project.file(DOCKER_COMPOSE_YML).exists()) {
            ...
            if (dockerComposeSupported() == false) {
                ...
                ComposeExtension composeExtension =
                        project.getExtensions().getByType(ComposeExtension.class);
            ...
            project.afterEvaluate(
                    spec -> {
                        if (extension.fixtures.isEmpty()) {
                            // if only one fixture is used, that's this one, but without a compose
                            // file that's not a valid configuration
        ...
        conditionTaskByType(
                tasks, extension, getTaskClass("org.elasticsearch.gradle.test.RestIntegTestTask"));
        ...
        tasks.withType(
                Test.class,
                task ->
                        extension.fixtures.all(
                                fixtureProject -> {
                                    fixtureProject
                                            .getTasks()
                                            .matching(it -> it.getName().equals("buildFixture"))
                                            .all(task::dependsOn);
                                    fixtureProject
                                            .getTasks()
                                            .matching(it -> it.getName().equals("composeDown"))
                                            .all(task::finalizedBy);
                                    configureServiceInfoForTask(
                                            task,
                                            fixtureProject,
                                            (name, host) ->
                                    ...
                                    task.dependsOn(
                                            fixtureProject
                                                    .getTasks()
                                                    .getByName("postProcessFixture"));
                                }));
    }

    private void conditionTaskByType(
            TaskContainer tasks,
            TestFixtureExtension extension,
    ...
    private void configureServiceInfoForTask(
            Task task, Project fixtureProject, BiConsumer<String, Integer> consumer) {
    ...
    public static boolean dockerComposeSupported() {
        ...
        final boolean hasDockerCompose =
                (new File("/usr/local/bin/docker-compose")).exists()
                        || (new File("/usr/bin/docker-compose").exists());
        return hasDockerCompose
                && Boolean.parseBoolean(System.getProperty("tests.fixture.enabled", "true"));
    }

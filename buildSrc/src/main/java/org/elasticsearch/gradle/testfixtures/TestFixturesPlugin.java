...
public class TestFixturesPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        TestFixtureExtension extension = project.getExtensions()
            .create(
                "testFixtures",
            ...
            project.afterEvaluate(spec -> {
                if (extension.fixtures.isEmpty()) {
                    ...
                    throw new IllegalStateException(
                        "No " + DOCKER_COMPOSE_YML + " found for " + project.getPath() +
        ...
        tasks.withType(
            ...
            task -> task.onlyIf(
                spec -> extension.fixtures.stream()
                    .anyMatch(
                        fixtureProject -> fixtureProject.getTasks().getByName("buildFixture").getEnabled() == false
                    ) == false
            )
        );
    }

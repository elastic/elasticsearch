...
public class TestingConventionsTasksIT extends GradleIntegrationTestCase {
    ...
    public void testInnerClasses() {
        GradleRunner runner =
                getGradleRunner("testingConventions")
                        .withArguments(
                                "clean",
    ...
    public void testAllTestTasksIncluded() {
        GradleRunner runner =
                getGradleRunner("testingConventions")
                        .withArguments(
                                "clean", ":all_classes_in_tasks:testingConventions", "-i", "-s");
        BuildResult result = runner.buildAndFail();
        assertOutputContains(
                result.getOutput(),
    ...
    public void testTaskNotImplementBaseClass() {
        GradleRunner runner =
                getGradleRunner("testingConventions")
                        .withArguments(
                                "clean", ":not_implementing_base:testingConventions", "-i", "-s");
        BuildResult result = runner.buildAndFail();
        assertOutputContains(
                result.getOutput(),
    ...
    public void testValidSetupWithoutBaseClass() {
        GradleRunner runner =
                getGradleRunner("testingConventions")
                        .withArguments(
                                "clean", ":valid_setup_no_base:testingConventions", "-i", "-s");
    ...
    public void testValidSetupWithBaseClass() {
        GradleRunner runner =
                getGradleRunner("testingConventions")
                        .withArguments(
                                "clean", ":valid_setup_with_base:testingConventions", "-i", "-s");
        BuildResult result = runner.build();

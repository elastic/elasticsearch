...
public class IndicesClientIT extends ESRestHighLevelClientTestCase {
    ...
    public static final RequestOptions LEGACY_TEMPLATE_OPTIONS = RequestOptions.DEFAULT.toBuilder()
        .setWarningsHandler(warnings -> List.of(RestPutIndexTemplateAction.DEPRECATION_WARNING).equals(warnings) == false)
        .build();
    ...
    public void testOpenNonExistentIndex() throws IOException {
        ...
        ElasticsearchException strictException = expectThrows(
            ElasticsearchException.class,
            () -> execute(
                openIndexRequest,
                highLevelClient().indices()::open,
    ...
    public void testCloseEmptyOrNullIndex() {
        ...
        CloseIndexRequest closeIndexRequest = new CloseIndexRequest(indices);
        org.elasticsearch.client.ValidationException exception = expectThrows(
            org.elasticsearch.client.ValidationException.class,
            () -> execute(closeIndexRequest, highLevelClient().indices()::close, highLevelClient().indices()::closeAsync)
        );
    ...
    public void testShrink() throws IOException {
        ...
        resizeRequest.setTargetIndex(
            new org.elasticsearch.action.admin.indices.create.CreateIndexRequest("target").settings(targetSettings)
    ...
    public void testPutTemplateWithTypesUsingUntypedAPI() throws Exception {
        ...
        ElasticsearchStatusException badMappingError = expectThrows(
            ElasticsearchStatusException.class,
    ...
    public void testPutTemplateBadRequests() throws Exception {
        ...
        IllegalArgumentException withoutPatternError = expectThrows(
        ...
        ElasticsearchException alreadyExistsError = expectThrows(
            ElasticsearchException.class,
            () -> execute(goodTemplate, client.indices()::putTemplate, client.indices()::putTemplateAsync, LEGACY_TEMPLATE_OPTIONS)
        );
        assertThat(
        ...
        PutIndexTemplateRequest unknownSettingTemplate = new PutIndexTemplateRequest("t3", List.of("any")).settings(
            Settings.builder().put("this-setting-does-not-exist", 100)
        );
        ElasticsearchStatusException unknownSettingError = expectThrows(
            ElasticsearchStatusException.class,
            () -> execute(
                unknownSettingTemplate,
    ...
    public void testCRUDIndexTemplate() throws Exception {
        ...
        PutIndexTemplateRequest putTemplate1 = new PutIndexTemplateRequest("template-1", List.of("pattern-1", "name-1")).alias(
            new Alias("alias-1")
        );
        assertThat(
            execute(putTemplate1, client.indices()::putTemplate, client.indices()::putTemplateAsync, LEGACY_TEMPLATE_OPTIONS)
                .isAcknowledged(),
            equalTo(true)
        );
        PutIndexTemplateRequest putTemplate2 = new PutIndexTemplateRequest("template-2", List.of("pattern-2", "name-2")).mapping(
            "{\"properties\": { \"name\": { \"type\": \"text\" }}}",
            XContentType.JSON
        ).settings(Settings.builder().put("number_of_shards", "2").put("number_of_replicas", "0"));
        assertThat(
            execute(putTemplate2, client.indices()::putTemplate, client.indices()::putTemplateAsync, LEGACY_TEMPLATE_OPTIONS)
                .isAcknowledged(),
            equalTo(true)
        );
    ...
    public void testIndexTemplatesExist() throws Exception {
...

        {
            for (String suffix : Arrays.asList("1", "2")) {
                ...
                assertTrue(
                    execute(putRequest, client.indices()::putTemplate, client.indices()::putTemplateAsync, LEGACY_TEMPLATE_OPTIONS)
                        .isAcknowledged()
                );
    ...
    public void testReloadAnalyzer() throws IOException {
        ...
        ReloadAnalyzersResponse reloadResponse = execute(
            new ReloadAnalyzersRequest("test"),
            client.indices()::reloadAnalyzers,

...
public class PreviewDatafeedResponseTests extends ESTestCase {
    ...
    public void testGetDataList() throws IOException {
        ...
        assertThat(
            response.getDataList().stream().map(map -> (String) map.get("airline")).collect(Collectors.toList()),
            containsInAnyOrder("JZA", "JBU", "KLM")
        );
        ...
        assertThat(
            response.getDataList().stream().map(map -> (String) map.get("key")).collect(Collectors.toList()),
            containsInAnyOrder("my_value")
        );

...
public class CreateServiceAccountTokenRequestTests extends ESTestCase {
    ...
    public void testNewInstance() {
        ...
        final CreateServiceAccountTokenRequest request1 = new CreateServiceAccountTokenRequest(namespace, service);
        assertThat(request1.getNamespace(), equalTo(namespace));
        assertThat(request1.getServiceName(), equalTo(service));
        ...
        final CreateServiceAccountTokenRequest request2 = new CreateServiceAccountTokenRequest(namespace, service, tokenName);
        assertThat(request2.getNamespace(), equalTo(namespace));
        assertThat(request2.getServiceName(), equalTo(service));
        assertThat(request2.getTokenName(), equalTo(tokenName));
        assertNull(request2.getRefreshPolicy());

        final RefreshPolicy refreshPolicy = randomFrom(RefreshPolicy.values());
        final CreateServiceAccountTokenRequest request3 = new CreateServiceAccountTokenRequest(namespace, service, tokenName, refreshPolicy);
        assertThat(request3.getNamespace(), equalTo(namespace));
        assertThat(request3.getServiceName(), equalTo(service));

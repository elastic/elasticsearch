...
public class InvalidateTokenResponseTests extends ESTestCase {
    ...
    public void testFromXContent() throws IOException {
        ...
        try (XContentParser parser = createParser(xContentType.xContent(), xContent)) {
            ...
            assertThat(response.getResult().getInvalidatedTokens(), Matchers.equalTo(invalidatedTokens));
            assertThat(response.getResult().getPreviouslyInvalidatedTokens(), Matchers.equalTo(previouslyInvalidatedTokens));
            assertThat(response.getResult().getErrors().size(), Matchers.equalTo(0));
        }
    }
    ...
    public void testFromXContentWithErrors() throws IOException {
        ...
        try (XContentParser parser = createParser(xContentType.xContent(), xContent)) {
            ...
            assertThat(response.getResult().getInvalidatedTokens(), Matchers.equalTo(invalidatedTokens));
            assertThat(response.getResult().getPreviouslyInvalidatedTokens(), Matchers.equalTo(previouslyInvalidatedTokens));
            assertThat(response.getResult().getErrors().size(), Matchers.equalTo(2));
            assertThat(response.getResult().getErrors().get(0).toString(), containsString("type=exception, reason=foo"));
            assertThat(response.getResult().getErrors().get(0).toString(), containsString("type=illegal_argument_exception, reason=bar"));
            assertThat(response.getResult().getErrors().get(1).toString(), containsString("type=exception, reason=boo"));
            assertThat(response.getResult().getErrors().get(1).toString(), containsString("type=illegal_argument_exception, reason=far"));
        }
    }
}

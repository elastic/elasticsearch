...
public class ElasticsearchExceptionTests extends AbstractResponseTestCase<
    ...
    protected void assertInstances(org.elasticsearch.ElasticsearchException serverTestInstance, ElasticsearchException clientInstance) {
        ...
        ElasticsearchException caused1 = new ElasticsearchException(
            "Elasticsearch exception [type=illegal_argument_exception, reason=argument]",
            cause
        );
        ElasticsearchException caused2 = new ElasticsearchException(
            "Elasticsearch exception [type=exception, reason=elastic_exception]",
            caused1
        );

        caused2.addHeader("key", List.of("value"));
        ElasticsearchException supp = new ElasticsearchException("Elasticsearch exception [type=number_format_exception, reason=3/0]");

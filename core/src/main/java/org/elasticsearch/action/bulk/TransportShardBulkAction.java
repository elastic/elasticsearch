...
public class TransportShardBulkAction extends TransportWriteAction<BulkShardRequest, BulkShardRequest, BulkShardResponse> {
    ...
    public WriteReplicaResult<BulkShardRequest> shardOperationOnReplica(BulkShardRequest request, IndexShard replica) throws Exception {
        ...
        for (int i = 0; i < request.items().length; i++) {
            ...
            if (item.isIgnoreOnReplica() == false && item.getPrimaryResponse().isFailed() == false) {
                DocWriteRequest docWriteRequest = item.request();
                DocWriteResponse docWriteResponse = item.getPrimaryResponse().getResponse();
                final Engine.Result operationResult;
                try {
                    switch (docWriteRequest.opType()) {
                        case CREATE:
                        case INDEX:
                            operationResult = executeIndexRequestOnReplica(docWriteResponse, (IndexRequest) docWriteRequest, replica);
                            break;
                        case DELETE:
                            operationResult = executeDeleteRequestOnReplica(docWriteResponse, (DeleteRequest) docWriteRequest, replica);
                            break;
                        default:
    ...
    public static Engine.IndexResult executeIndexRequestOnReplica(DocWriteResponse primaryResponse, IndexRequest request, IndexShard replica) throws IOException {
        ...
        assert request.versionType().validateVersionForWrites(version);

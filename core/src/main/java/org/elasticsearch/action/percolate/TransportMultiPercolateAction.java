            ...
            for (int slot = 0;  slot < percolateRequests.size(); slot++) {
                ...
                if (element instanceof PercolateRequest) {
                    ...
                    for (ShardIterator shard : shards) {
                        ...
                        if (requests == null) {
                            requestsByShard.put(shardId, requests = new TransportShardMultiPercolateAction.Request(multiPercolateRequest, shardId.getIndexName(), shardId.getId(), percolateRequest.preference()));
                        }

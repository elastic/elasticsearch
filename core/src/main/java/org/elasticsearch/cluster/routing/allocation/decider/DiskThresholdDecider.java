...
public class DiskThresholdDecider extends AllocationDecider {
    ...
    public Decision decideOutgoingMovePerNode(RoutingNode node, RoutingAllocation allocation, RoutingNodes routingNodes) {
        ...
        // subtractLeavingShards is passed as true here, since this is only for shards
        // remaining, we will *eventually* have enough disk
        // since shards are moving away. No new shards will be incoming since in
        // canAllocate we pass false for this check.
        final DiskUsage usage = getDiskUsage(node, allocation, usages, true);
        ...
        if (freeBytes < diskThresholdSettings.getFreeBytesThresholdHigh().getBytes()) {
            logger.info("less than the required {} free bytes threshold ({} bytes free) on node {}, shard cannot remain",
                    diskThresholdSettings.getFreeBytesThresholdHigh(), freeBytes, node.nodeId());
        ...
        return allocation.decision(Decision.YES, NAME, "there is enough disk on this node for the shard to remain, free: [%s]",
                new ByteSizeValue(freeBytes));
    }

...
public class VersionsResolver {
    ...
    private static PerThreadIDAndVersionLookup getLookupState(LeafReader reader)
        ...
        if (cacheHelper == null) {
            throw new IllegalArgumentException("Reader " + reader + " does not support core caching");
        }
        CloseableThreadLocal<PerThreadIDAndVersionLookup> ctl = lookupStates.get(cacheHelper.getKey());
        if (ctl == null) {
            // First time we are seeing this reader's core; make a
            // new CTL:
            ctl = new CloseableThreadLocal<>();
            CloseableThreadLocal<PerThreadIDAndVersionLookup> other =
                lookupStates.putIfAbsent(cacheHelper.getKey(), ctl);
            if (other == null) {
                    ...
                    if (termsEnum.seekExact(id)) {
                        ...
                        if (docID != DocIdSetIterator.NO_MORE_DOCS) {
                            ...
                            if (dvField.advanceExact(docID) == false) {
                                ...
                                actualCount = dvField.docValueCount();
                            }
                            if (actualCount != 1) {
                                throw new AssertionError("Expected a single value for _seq_no, but got " + actualCount
                                        + " values for document " + docID + " of " + reader);
                            }
            ...
            if (fields != null) {
                ...
                if (terms != null) {
                    ...
                    if (termsEnum.seekExact(id)) {
                        ...
                        if (docID != DocIdSetIterator.NO_MORE_DOCS) {
                            if (dvField.advanceExact(docID) == false) {
                                throw new AssertionError("Document " + docID + " does not have a value for "
                                        + SeqNoFieldMapper.PRIMARY_TERM_NAME + " in redaer " + reader);
                            }

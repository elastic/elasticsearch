...
public final class EngineConfig {
    ...
    /**
     * Configures an index to allow or disallow document get calls with <code>realtime=true</code>. This is enabled by default but
     * can be disabled when an index is created. This setting can't be changed at runtime but only if the index is closed. If this is
     * set to <code>false</code> all calls to get with <code>realtime=true</code> will fail.
     */
    public static final Setting<Boolean> INDEX_ENABLE_REALTIME_GET = Setting.boolSetting("index.enable_realtime_get", true,
        Property.IndexScope);
    ...
    public boolean isRealtimeGet() {
        return indexSettings.getValue(INDEX_ENABLE_REALTIME_GET);
    }
}

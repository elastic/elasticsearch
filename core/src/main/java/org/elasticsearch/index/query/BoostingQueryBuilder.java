    ...
    public static BoostingQueryBuilder fromXContent(XContentParser parser) throws IOException {
        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if (POSITIVE_FIELD.match(currentFieldName, parser.deprecationHandler())) {
                    positiveQuery = parseInnerQueryBuilder(parser);
                    positiveQueryFound = true;
                } else if (NEGATIVE_FIELD.match(currentFieldName, parser.deprecationHandler())) {
                ...
                if (NEGATIVE_BOOST_FIELD.match(currentFieldName, parser.deprecationHandler())) {
                    negativeBoost = parser.floatValue();
                } else if (NAME_FIELD.match(currentFieldName, parser.deprecationHandler())) {
                    queryName = parser.text();
                } else if (BOOST_FIELD.match(currentFieldName, parser.deprecationHandler())) {

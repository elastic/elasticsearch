        ...
        try {
            switch (operation.opType()) {
                case INDEX:
                    ...
                    final boolean canHaveDuplicates = index.isAutoGeneratedID();
                    Engine.Index engineIndex = IndexShard.prepareIndex(docMapper(index.type()), source(shardId.getIndexName(), index.type(), index.id(), index.source())
                        ...
                        index.version(), index.versionType().versionTypeForReplicationAndRecovery(), origin, index.isAutoGeneratedID(), canHaveDuplicates);
                    maybeAddMappingUpdate(engineIndex.type(), engineIndex.parsedDoc().dynamicMappingsUpdate(), index.id(), allowMappingUpdates);

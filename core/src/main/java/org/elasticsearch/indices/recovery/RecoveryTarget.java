...
public class RecoveryTarget extends AbstractComponent implements IndexEventListener {
    ...
    private void doRecovery(final RecoveryStatus recoveryStatus) {
        ...
        try {
            logger.trace("[{}][{}] starting recovery from {}", request.shardId().index().getName(), request.shardId().id(), request.sourceNode());
            ...
            if (logger.isTraceEnabled()) {
                ...
                sb.append('[').append(request.shardId().index().getName()).append(']').append('[').append(request.shardId().id()).append("] ");
            ...
            if (logger.isTraceEnabled()) {
                logger.trace("[{}][{}] Got exception on recovery", e, request.shardId().index().getName(), request.shardId().id());
            }

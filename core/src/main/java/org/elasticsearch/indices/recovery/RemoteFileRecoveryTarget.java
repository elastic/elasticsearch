...
public class RemoteFileRecoveryTarget extends RemoteOpsRecoveryTarget
    ...
    public void prepareForTranslogOperations(int totalTranslogOps, long maxUnsafeAutoIdTimestamp)
        ...
        transportService.submitRequest(targetNode,
            PeerRecoveryTargetService.Actions.PREPARE_TRANSLOG,
                new RecoveryPrepareForTranslogOperationsRequest(recoveryId, shardId,
                    totalTranslogOps, maxUnsafeAutoIdTimestamp),
    ...
    public void receiveFileInfo(List<String> phase1FileNames, List<Long> phase1FileSizes,
        ...
        RecoveryFilesInfoRequest recoveryInfoFilesRequest =
            new RecoveryFilesInfoRequest(recoveryId, shardId,
                phase1FileNames, phase1FileSizes, phase1ExistingFileNames, phase1ExistingFileSizes,
                totalTranslogOps);
        transportService.submitRequest(targetNode, PeerRecoveryTargetService.Actions.FILES_INFO,
    ...
    public void writeFileChunk(StoreFileMetaData fileMetaData, long position,
        ...
        transportService.submitRequest(targetNode, PeerRecoveryTargetService.Actions.FILE_CHUNK,
            new RecoveryFileChunkRequest(recoveryId, shardId, fileMetaData, position, content,
                lastChunk,
                totalTranslogOps,
                // we send totalOperations with every request since we collect stats on the target
                // and that way we can see how many translog ops we accumulate while copying
                // files across the network. A future optimization would be in to restart file
                // copy again (new deltas) if we have too many translog ops are piling up.
                throttleTimeInNanos),
            fileChunkRequestOptions, EmptyTransportResponseHandler.INSTANCE_SAME).txGet();
    }
}

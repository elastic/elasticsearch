...
public class ScriptPermits {
    ...
    public ScriptPermits(Settings settings, ScriptSettings scriptSettings,
            ScriptContextRegistry scriptContextRegistry) {
        this.scriptModes = new ScriptModes(scriptSettings, settings);
        this.scriptContextRegistry = scriptContextRegistry;
        this.lastInlineCompileTime = System.nanoTime();
        this.setMaxCompilationsPerMinute(
                ScriptService.SCRIPT_MAX_COMPILATIONS_PER_MINUTE.get(settings));
    }

    void registerClusterSettingsListeners(ClusterSettings clusterSettings) {
        clusterSettings.addSettingsUpdateConsumer(ScriptService.SCRIPT_MAX_COMPILATIONS_PER_MINUTE,
                this::setMaxCompilationsPerMinute);
    }
    ...
    void setMaxCompilationsPerMinute(Integer newMaxPerMinute) {
        ...
        this.compilesAllowedPerNano = ((double) totalCompilesPerMinute)
                / TimeValue.timeValueMinutes(1).nanos();
    }

    /**
     * Check whether there have been too many compilations within the last minute, throwing a
     * circuit breaking exception if so. This is a variant of the token bucket algorithm:
     * https://en.wikipedia.org/wiki/Token_bucket
     *
     * It can be thought of as a bucket with water, every time the bucket is checked, water is added
     * proportional to the amount of time that elapsed since the last time it was checked. If there
     * is enough water, some is removed and the request is allowed. If there is not enough water the
     * request is denied. Just like a normal bucket, if water is added that overflows the bucket,
     * the extra water/capacity is discarded - there can never be more water in the bucket than the
     * size of the bucket.
     */
    public void checkCompilationLimit() {
        ...
        if (scriptsPerMinCounter >= 1) {
            ...
            throw new CircuitBreakingException(
                    "[script] Too many dynamic script compilations within one minute, max: ["
                            + totalCompilesPerMinute + "/min]; please use on-disk, indexed, or "
                            + "scripts with parameters instead; this limit can be changed by the ["
                            + ScriptService.SCRIPT_MAX_COMPILATIONS_PER_MINUTE.getKey()
                            + "] setting");
        }
    }
    ...
    public boolean checkContextPermissions(String lang, ScriptType scriptType,
        ...
        if (scriptContextRegistry.isSupportedContext(scriptContext) == false) {
            throw new IllegalArgumentException(
                    "script context [" + scriptContext.getKey() + "] not supported");
        }

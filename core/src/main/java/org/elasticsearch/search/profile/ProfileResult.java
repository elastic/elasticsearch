    ...
    public static ProfileResult fromXContent(XContentParser parser) throws IOException {
        ...
        while((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if (TYPE.match(currentFieldName, parser.deprecationHandler())) {
                    type = parser.text();
                } else if (DESCRIPTION.match(currentFieldName, parser.deprecationHandler())) {
                    description = parser.text();
                } else if (NODE_TIME.match(currentFieldName, parser.deprecationHandler())) {
                    // skip, total time is calculate by adding up 'timings' values in ProfileResult ctor
                    parser.text();
                } else if (NODE_TIME_RAW.match(currentFieldName, parser.deprecationHandler())) {
                    // skip, total time is calculate by adding up 'timings' values in ProfileResult ctor
                    parser.longValue();
                } else {
                    parser.skipChildren();
                ...
                if (BREAKDOWN.match(currentFieldName, parser.deprecationHandler())) {
                    while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {

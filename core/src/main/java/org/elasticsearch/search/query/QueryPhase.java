    ...
    static boolean execute(SearchContext searchContext, final IndexSearcher searcher) throws QueryPhaseExecutionException {
        ...
        try {
            ...
            if (searchContext.size() == 0) { // no matter what the value of from is
                ...
                collector = Profiler.wrapCollector(searchContext.getProfilers(), collector, CollectorResult.REASON_SEARCH_COUNT);
                ...
                collector = Profiler.wrapCollector(searchContext.getProfilers(), collector, CollectorResult.REASON_SEARCH_TOP_HITS);
                topDocsCallable = new Callable<TopDocs>() {
                    @Override
                    public TopDocs call() throws Exception {
            ...
            if (terminateAfterSet) {
                ...
                collector = Profiler.wrapCollector(searchContext.getProfilers(), collector, CollectorResult.REASON_SEARCH_TERMINATE_AFTER_COUNT);
            }
            ...
            if (searchContext.parsedPostFilter() != null) {
                ...
                collector = Profiler.wrapCollector(searchContext.getProfilers(), collector, CollectorResult.REASON_SEARCH_POST_FILTER);
            }
            ...
            collector = Profiler.wrapMultiCollector(searchContext.getProfilers(), collector, allCollectors);

            // apply the minimum score after multi collector so we filter aggs as well
            if (searchContext.minimumScore() != null) {
                collector = new MinimumScoreCollector(collector, searchContext.minimumScore());
                collector = Profiler.wrapCollector(searchContext.getProfilers(), collector, CollectorResult.REASON_SEARCH_MIN_SCORE);
            }
            ...
            if (timeoutSet && collector != null) { // collector might be null if no collection is actually needed
                ...
                collector = Profiler.wrapCollector(searchContext.getProfilers(), collector, CollectorResult.REASON_SEARCH_TIMEOUT);
            }

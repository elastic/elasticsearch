...
public class ScriptSortBuilder extends SortBuilder<ScriptSortBuilder> {
    ...
    private static ConstructingObjectParser<ScriptSortBuilder, QueryParseContext> PARSER = new ConstructingObjectParser<>(NAME,
            a -> new ScriptSortBuilder((ScriptInput) a[0], (ScriptSortType) a[1]));

    static {
        PARSER.declareField(constructorArg(), ScriptInput::parse, ScriptField.SCRIPT, ValueType.OBJECT_OR_STRING);
    ...
    public SortFieldAndFormat build(QueryShardContext context) throws IOException {
        final SearchScript searchScript = context.getSearchScript(script.lookup, ScriptContext.Standard.SEARCH, script.params);

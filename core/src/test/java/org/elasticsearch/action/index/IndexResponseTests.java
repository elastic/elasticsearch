...
public class IndexResponseTests extends ESTestCase {
    ...
    public static Tuple<IndexResponse, IndexResponse> randomIndexResponse() {
        ...
        IndexResponse actual = new IndexResponse(new ShardId(index, indexUUid, shardId), type, id, version, created);
        actual.setForcedRefresh(forcedRefresh);
        actual.setShardInfo(shardInfos.v1());

        IndexResponse expected = new IndexResponse(new ShardId(index, INDEX_UUID_NA_VALUE, -1), type, id, version, created);

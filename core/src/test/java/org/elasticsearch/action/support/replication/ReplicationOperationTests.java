...
public class ReplicationOperationTests extends ESTestCase {
    ...
    public void testReplication() throws Exception {
        ...
        final TestReplicationOperation op = new TestReplicationOperation(request,
            new TestPrimary(primaryShard), listener, replicasProxy, () -> finalState);
        op.execute();
    ...
    public void testReplicationWithShadowIndex() throws Exception {
        ...
        PlainActionFuture<TestPrimary.Result> listener = new PlainActionFuture<>();
        final TestReplicationOperation op = new TestReplicationOperation(request,
            new TestPrimary(primaryShard), listener, false,
            new TestReplicaProxy(primaryTerm), () -> state, logger, "test");
        op.execute();
        assertThat("request was not processed on primary", request.processedOnPrimary.get(), equalTo(true));
        ...
        final TestReplicaProxy replicasProxy = new TestReplicaProxy(primaryTerm, expectedFailures) {
            ...
            public void failShard(ShardRouting replica, String message, Exception exception,
                ...
                if (testPrimaryDemotedOnStaleShardCopies) {
                    super.failShard(replica, message, exception, onSuccess, onPrimaryDemoted, onIgnoredFailure);
                } else {
            ...
            public void markShardCopyAsStale(ShardId shardId, String allocationId, Runnable onSuccess,
                ...
                if (testPrimaryDemotedOnStaleShardCopies) {
                    ...
                    super.markShardCopyAsStale(shardId, allocationId, onSuccess, onPrimaryDemoted, onIgnoredFailure);
                }
            }
        };
    ...
    private void testClusterStateChangeAfterPrimaryOperation(final ShardId shardId,
        ...
        final TestReplicationOperation op = new TestReplicationOperation(request, primary, listener,
            new TestReplicaProxy(primaryTerm), state::get);
        op.execute();
    ...
    public void testWaitForActiveShards() throws Exception {
        ...
        final TestReplicationOperation op = new TestReplicationOperation(request,
            new TestPrimary(primaryShard),
            listener, randomBoolean(), new TestReplicaProxy(primaryTerm), () -> state, logger, "test");
    ...
    static class TestPrimary implements ReplicationOperation.Primary<Request, Request, TestPrimary.Result> {
        ...
        TestPrimary(ShardRouting routing) {

    ...
    public void testNowInScript() throws IOException {
...

        {
            UpdateRequest updateRequest = new UpdateRequest("test", "type1", "2")
                .upsert(indexRequest)
                .script(ScriptInput.inline("mock", "ctx._source.update_timestamp = ctx._now", Collections.emptyMap()))
                .scriptedUpsert(true);
            long nowInMillis = randomPositiveLong();
            // We simulate that the document is not existing yet
        ...
        }
        {
            UpdateRequest updateRequest = new UpdateRequest("test", "type1", "2")
                .upsert(indexRequest)
                .script(ScriptInput.inline("mock", "ctx._timestamp = ctx._now", Collections.emptyMap()))
                .scriptedUpsert(true);
            long nowInMillis = randomPositiveLong();
            // We simulate that the document is not existing yet

...
public class DiscoveryNodeTests extends ESTestCase {
    ...
    public void testDiscoveryNodeIsCreatedWithHostFromInetAddress() throws Exception {
        ...
        TransportAddress transportAddress = new InetSocketTransportAddress(inetAddress, randomIntBetween(0, 65535));
        DiscoveryNode node = new DiscoveryNode("name1", "id1", transportAddress, emptyMap(), emptySet(), Version.CURRENT);
        assertEquals(transportAddress.getHost(), node.getHostName());
    ...
    public void testDiscoveryNodeSerializationKeepsHost() throws Exception {
        ...
        assertEquals(transportAddress.getHost(), serialized.getHostName());
        assertEquals(transportAddress.getHost(), serialized.getAddress().getHost());
    ...
    public void testDiscoveryNodeSerializationToOldVersion() throws Exception {
        ...
        assertFalse("if the minimum compatibility version moves past 5.0.3, remove the special casing in DiscoverNode(StreamInput) and " +
            ...
            Version.CURRENT.minimumCompatibilityVersion().onOrAfter(InetSocketTransportAddress.V_5_0_3_UNRELEASED));
    }
}

...
public class SameShardRoutingTests extends ESAllocationTestCase {
    ...
    public void testSameHost() {
        ...
        clusterState = ClusterState.builder(clusterState).nodes(
                ...
                .put(new DiscoveryNode("node1", "node1", "node1", "test1", "test1", DummyTransportAddress.INSTANCE, emptyMap(),
                        MASTER_DATA_ROLES, Version.CURRENT))
                .put(new DiscoveryNode("node2", "node2", "node2", "test1", "test1", DummyTransportAddress.INSTANCE, emptyMap(),
        ...
        clusterState = ClusterState.builder(clusterState).nodes(DiscoveryNodes.builder(clusterState.nodes())
                .put(new DiscoveryNode("node3", "node3", "node3", "test2", "test2", DummyTransportAddress.INSTANCE, emptyMap(),
                        MASTER_DATA_ROLES, Version.CURRENT))).build();
        routingTable = strategy.reroute(clusterState, "reroute").routingTable();
        clusterState = ClusterState.builder(clusterState).routingTable(routingTable).build();

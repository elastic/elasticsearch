...
public class ZenFaultDetectionTests extends ESTestCase {
    ...
    public void setUp() throws Exception {
        ...
        Settings settings = Settings.builder()
            .put(HierarchyCircuitBreakerService.IN_FLIGHT_REQUESTS_CIRCUIT_BREAKER_LIMIT_SETTING, new ByteSizeValue(0))
            .build();
        NodeSettingsService nodeSettingsService = new NodeSettingsService(settings);
        threadPool = new ThreadPool(getClass().getName());
        clusterServiceA = new NoopClusterService();
        clusterServiceB = new NoopClusterService();
        circuitBreakerService = new HierarchyCircuitBreakerService(settings, nodeSettingsService);
        ...
        MockTransportService transportService =
                new MockTransportService(
                        Settings.builder()
                            ...
                            .put(TransportService.SETTING_TRACE_LOG_EXCLUDE, singleton(TransportLivenessAction.NAME))
                            .build(),
    ...
    public void testMasterFaultDetectionNotSizeLimited() throws InterruptedException {
        ...
        boolean shouldRetry = randomBoolean();
        settings
            .put(FaultDetection.SETTING_CONNECT_ON_NETWORK_DISCONNECT, shouldRetry)
            .put(FaultDetection.SETTING_PING_INTERVAL, "1s");
        ClusterName clusterName = new ClusterName(randomAsciiOfLengthBetween(3, 20));
    ...
    private static class PingProbe extends MockTransportService.Tracer {
        private final Set<Tuple<DiscoveryNode, Long>> inflightPings = Collections.newSetFromMap(new ConcurrentHashMap<Tuple<DiscoveryNode, Long>, Boolean>());
        private final Set<Tuple<DiscoveryNode, Long>> completedPings = Collections.newSetFromMap(new ConcurrentHashMap<Tuple<DiscoveryNode, Long>, Boolean>());

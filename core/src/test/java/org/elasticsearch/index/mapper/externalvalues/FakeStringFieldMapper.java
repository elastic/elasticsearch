...
public class FakeStringFieldMapper extends FieldMapper {
    ...
    protected void parseCreateField(ParseContext context, List<Field> fields) throws IOException {
        ...
        if (fieldType().indexOptions() != IndexOptions.NONE || fieldType().stored()) {
            Field field = new Field(fieldType().names().fullName(), valueAndBoost.value(), fieldType());
            fields.add(field);
        }
        if (fieldType().hasDocValues()) {
            fields.add(new SortedSetDocValuesField(fieldType().names().fullName(), new BytesRef(valueAndBoost.value())));
        }
    }

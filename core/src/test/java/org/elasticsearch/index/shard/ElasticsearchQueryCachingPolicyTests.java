...
public class ElasticsearchQueryCachingPolicyTests extends ESTestCase {
    ...
    public void testDoesNotCacheTermQueries() throws IOException {
        ...
        assertTrue(policy.shouldCache(new TermQuery(new Term("foo", "bar")), null));
        assertTrue(policy.shouldCache(new PhraseQuery("foo", "bar", "baz"), null));
        policy = new ElasticsearchQueryCachingPolicy(policy);
        assertFalse(policy.shouldCache(new TermQuery(new Term("foo", "bar")), null));
        assertTrue(policy.shouldCache(new PhraseQuery("foo", "bar", "baz"), null));
    }

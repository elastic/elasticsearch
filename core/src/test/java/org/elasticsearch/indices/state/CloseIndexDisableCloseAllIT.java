...
public class CloseIndexDisableCloseAllIT extends ESIntegTestCase {
    ...
    public void testCloseAllDisabled() {
        createIndex("test1", "test2", "test3");

        assertAcked(client().admin().indices().prepareClose("test3", "test2"));

        checkIndexState(IndexMetaData.State.CLOSE, "test2", "test3");
        ...
        assertEquals(ex.getMessage(), "closing indices is disabled - set [cluster.indices.close.enable: true] to enable it. " +

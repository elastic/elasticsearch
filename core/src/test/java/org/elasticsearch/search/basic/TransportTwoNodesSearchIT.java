...
//            assertThat("id[" + hit.id() + "]", hit.id(), equalTo(Integer.toString(100 - i - 1)));
    ...
    public void testFailedMultiSearchWithWrongQueryWithFunctionScore() throws Exception {
        ...
        MultiSearchResponse response = client().prepareMultiSearch()
                ...
                .add(client().prepareSearch("test").setQuery(QueryBuilders.functionScoreQuery(QueryBuilders.termQuery("nid", 1), new ScriptScoreFunctionBuilder(ScriptInput.create(Script.ScriptType.INLINE, "bar", "foo", null, null)))))
                .add(client().prepareSearch("test").setQuery(QueryBuilders.termQuery("nid", 2)))
                .add(client().prepareSearch("test").setQuery(QueryBuilders.matchAllQuery()))
                .execute().actionGet();
        assertThat(response.getResponses().length, equalTo(3));

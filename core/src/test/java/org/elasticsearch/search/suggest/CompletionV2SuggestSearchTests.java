...
import org.elasticsearch.search.suggest.completionv2.*;
import org.elasticsearch.search.suggest.completionv2.CompletionSuggestionBuilder.FuzzyOptionsBuilder;
import org.elasticsearch.search.suggest.completionv2.context.*;
...
public class CompletionV2SuggestSearchTests extends ElasticsearchIntegrationTest {
    ...
    public void testRegex() throws Exception {
        ...
        indexRandom(true, indexRequestBuilders);
        CompletionSuggestionBuilder prefix = SuggestBuilders.completionV2Suggestion("foo").field(FIELD).regex("sugg.*es");
    ...
    public void testFuzzy() throws Exception {
        ...
        indexRandom(true, indexRequestBuilders);
        CompletionSuggestionBuilder prefix = SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("sugg", Fuzziness.ONE);
    ...
    public void testThatUpgradeToMultiFieldTypeWorks() throws Exception {
        ...
        SuggestResponse afterReindexingResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.completionV2Suggestion("suggs").field(FIELD + ".suggest").text("f").size(10)
        ).execute().actionGet();
    ...
    public void testThatFuzzySuggesterWorks() throws Exception {
        ...
        SuggestResponse suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("Nirv").size(10)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo", "Nirvana");

        suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("Nirw", Fuzziness.ONE).size(10)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo", "Nirvana");
    }
    ...
    public void testThatFuzzySuggesterSupportsEditDistances() throws Exception {
        ...
        // edit distance 1
        SuggestResponse suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("Norw", Fuzziness.ONE).size(10)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo");

        // edit distance 2
        suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("Norw", Fuzziness.TWO).size(10)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo", "Nirvana");
    }
    ...
    public void testThatFuzzySuggesterSupportsTranspositions() throws Exception {
        ...
        SuggestResponse suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("Nriv", new FuzzyOptionsBuilder().setTranspositions(false)).size(10)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo");

        suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("Nriv", Fuzziness.ONE).size(10)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo", "Nirvana");
    }
    ...
    public void testThatFuzzySuggesterSupportsMinPrefixLength() throws Exception {
        ...
        SuggestResponse suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("Nriva", new FuzzyOptionsBuilder().setFuzzyMinLength(6)).size(10)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo");

        suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("Nrivan", new FuzzyOptionsBuilder().setFuzzyMinLength(6)).size(10)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo", "Nirvana");
    }
    ...
    public void testThatFuzzySuggesterSupportsNonPrefixLength() throws Exception {
        ...
        SuggestResponse suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("Nirw", new FuzzyOptionsBuilder().setFuzzyPrefixLength(4)).size(10)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo");

        suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("Nirvo", new FuzzyOptionsBuilder().setFuzzyPrefixLength(4)).size(10)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo", "Nirvana");
    }
    ...
    public void testThatFuzzySuggesterIsUnicodeAware() throws Exception {
        ...
        org.elasticsearch.search.suggest.completionv2.CompletionSuggestionBuilder completionSuggestionBuilder =
                SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("öööи", new FuzzyOptionsBuilder().setUnicodeAware(true)).size(10);
        ...
        completionSuggestionBuilder = SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("öööи", new FuzzyOptionsBuilder().setUnicodeAware(false)).size(10);
        ...
        completionSuggestionBuilder = SuggestBuilders.completionV2Suggestion("foo").field(FIELD).prefix("öööи", new FuzzyOptionsBuilder().setUnicodeAware(false).setFuzziness(Fuzziness.TWO)).size(10);
        suggestResponse = client().prepareSuggest(INDEX).addSuggestion(completionSuggestionBuilder).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo", "ööööö");
    }

    @Test
    @AwaitsFix(bugUrl = "add support for stats API")
    public void testThatStatsAreWorking() throws Exception {
        ...
        client().prepareSuggest(INDEX).addSuggestion(SuggestBuilders.completionV2Suggestion("foo").prefix("f")).get();
    ...
    public void assertSuggestions(String suggestion, String... suggestions) {
        ...
        CompletionSuggestionBuilder suggestionBuilder = SuggestBuilders.completionV2Suggestion(suggestionName).field(FIELD).text(suggestion).size(10);
    ...
    public void assertSuggestionsNotInOrder(String suggestString, String... suggestions) {
        String suggestionName = RandomStrings.randomAsciiOfLength(new Random(), 10);
        SuggestResponse suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.completionV2Suggestion(suggestionName).field(FIELD).text(suggestString).size(10)
        ).execute().actionGet();
    ...
    private static String replaceReservedChars(String input, char replacement) {
        ...
        for (int i = 0; i < charArray.length; i++) {
            if (CompletionV2FieldMapper.isReservedChar(charArray[i])) {

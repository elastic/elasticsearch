...
public class OldCompletionSuggestSearchTests extends ElasticsearchIntegrationTest {
    ...
    public void testThatUpgradeToMultiFieldTypeWorks() throws Exception {
        ...
        SuggestResponse afterReindexingResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.oldCompletionSuggestion("suggs").field(FIELD + ".suggest").text("f").size(10)
        ).execute().actionGet();
    ...
    public void testThatFuzzySuggesterWorks() throws Exception {
        ...
        SuggestResponse suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.oldFuzzyCompletionSuggestion("foo").field(FIELD).text("Nirv").size(10)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo", "Nirvana");

        suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.oldFuzzyCompletionSuggestion("foo").field(FIELD).text("Nirw").size(10)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo", "Nirvana");
    }
    ...
    public void testThatFuzzySuggesterSupportsEditDistances() throws Exception {
        ...
        // edit distance 1
        SuggestResponse suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.oldFuzzyCompletionSuggestion("foo").field(FIELD).text("Norw").size(10)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo");

        // edit distance 2
        suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.oldFuzzyCompletionSuggestion("foo").field(FIELD).text("Norw").size(10).setFuzziness(Fuzziness.TWO)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo", "Nirvana");
    }
    ...
    public void testThatFuzzySuggesterSupportsTranspositions() throws Exception {
        ...
        SuggestResponse suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.oldFuzzyCompletionSuggestion("foo").field(FIELD).text("Nriv").size(10).setFuzzyTranspositions(false).setFuzziness(Fuzziness.ONE)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo");

        suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.oldFuzzyCompletionSuggestion("foo").field(FIELD).text("Nriv").size(10).setFuzzyTranspositions(true).setFuzziness(Fuzziness.ONE)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo", "Nirvana");
    }
    ...
    public void testThatFuzzySuggesterSupportsMinPrefixLength() throws Exception {
        ...
        SuggestResponse suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.oldFuzzyCompletionSuggestion("foo").field(FIELD).text("Nriva").size(10).setFuzzyMinLength(6)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo");

        suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.oldFuzzyCompletionSuggestion("foo").field(FIELD).text("Nrivan").size(10).setFuzzyMinLength(6)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo", "Nirvana");
    }
    ...
    public void testThatFuzzySuggesterSupportsNonPrefixLength() throws Exception {
        ...
        SuggestResponse suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.oldFuzzyCompletionSuggestion("foo").field(FIELD).text("Nirw").size(10).setFuzzyPrefixLength(4)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo");

        suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.oldFuzzyCompletionSuggestion("foo").field(FIELD).text("Nirvo").size(10).setFuzzyPrefixLength(4)
        ).execute().actionGet();
        assertSuggestions(suggestResponse, false, "foo", "Nirvana");
    }
    ...
    public void testThatFuzzySuggesterIsUnicodeAware() throws Exception {
        ...
        CompletionSuggestionFuzzyBuilder completionSuggestionBuilder =
                SuggestBuilders.oldFuzzyCompletionSuggestion("foo").field(FIELD).text("öööи").size(10).setUnicodeAware(true);
    ...
    public void assertSuggestions(String suggestion, String... suggestions) {
        ...
        SuggestResponse suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.oldCompletionSuggestion(suggestionName).field(FIELD).text(suggestion).size(10)
        ).execute().actionGet();
    ...
    public void assertSuggestionsNotInOrder(String suggestString, String... suggestions) {
        String suggestionName = RandomStrings.randomAsciiOfLength(new Random(), 10);
        SuggestResponse suggestResponse = client().prepareSuggest(INDEX).addSuggestion(
                SuggestBuilders.oldCompletionSuggestion(suggestionName).field(FIELD).text(suggestString).size(10)
        ).execute().actionGet();
    ...
    public void testMaxFieldLength() throws IOException {
        ...
        for (int i = 0; i < iters; i++) {
            ...
            flushAndRefresh();
            int prefixLen = OldCompletionFieldMapper.correctSubStringLen(str, between(1, maxInputLen - 1));
            ...
            if (maxInputLen + 1 < str.length()) {
                ...
                int correctSubStringLen = OldCompletionFieldMapper.correctSubStringLen(str, maxInputLen + offset);
    ...
    private static String replaceReservedChars(String input, char replacement) {
        ...
        for (int i = 0; i < charArray.length; i++) {
            if (OldCompletionFieldMapper.isReservedChar(charArray[i])) {

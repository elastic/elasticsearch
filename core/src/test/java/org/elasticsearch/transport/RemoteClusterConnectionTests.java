...
public class RemoteClusterConnectionTests extends ESTestCase {
        ...
        try {
            ...
            newService.registerRequestHandler(ClusterStateAction.NAME, ClusterStateRequest::new, ThreadPool.Names.SAME,
                    (request, channel) -> {
                        ...
                        channel.sendResponse(new ClusterStateResponse(clusterName, build));
                    });
            newService.start();
    ...
    public void testSlowNodeCanBeCanceled() throws IOException, InterruptedException {
        try (ServerSocket socket = new ServerSocket()) {
            ...
            DiscoveryNode seedNode = new DiscoveryNode("TEST", new InetSocketTransportAddress(socket.getInetAddress(),
            ...
            (request, channel) -> {
                ...
                for (DiscoveryNode node : nodes) {
                    HttpInfo http = new HttpInfo(new BoundTransportAddress(new TransportAddress[]{node.getAddress()},
                        new InetSocketTransportAddress(((InetSocketTransportAddress)node.getAddress()).address().getAddress(),
                            port++)), 100);
    ...
    public void testRemoteConnectionInfo() throws IOException {
        RemoteConnectionInfo stats = new RemoteConnectionInfo("test_cluster",
            Arrays.asList(new LocalTransportAddress("1")),
            Arrays.asList(new LocalTransportAddress("80")),
        ...
        stats1 = new RemoteConnectionInfo("test_cluster",
            Arrays.asList(new LocalTransportAddress("15")),
            Arrays.asList(new LocalTransportAddress("80")),
            4, 3, TimeValue.timeValueMinutes(30));
        assertSerialization(stats1);
        assertNotEquals(stats, stats1);
        ...
        stats1 = new RemoteConnectionInfo("test_cluster",
            Arrays.asList(new LocalTransportAddress("1")),
            Arrays.asList(new LocalTransportAddress("87")),
            4, 3, TimeValue.timeValueMinutes(30));
        assertSerialization(stats1);
        assertNotEquals(stats, stats1);
    ...
    public void testRenderConnectionInfoXContent() throws IOException {
        RemoteConnectionInfo stats = new RemoteConnectionInfo("test_cluster",
            Arrays.asList(new LocalTransportAddress("1")),
            Arrays.asList(new LocalTransportAddress("2")),
            4, 3, TimeValue.timeValueMinutes(30));
        ...
        builder.endObject();
        assertEquals("{\"test_cluster\":{\"seeds\":[\"local[1]\"],\"http_addresses\":[\"local[2]\"],\"connected\":true," +
        ...
        stats = new RemoteConnectionInfo("some_other_cluster",
            Arrays.asList(new LocalTransportAddress("1"), new LocalTransportAddress("2")),
            Arrays.asList(new LocalTransportAddress("80"), new LocalTransportAddress("81")),
        ...
        stats.toXContent(builder, null);
        builder.endObject();
        assertEquals("{\"some_other_cluster\":{\"seeds\":[\"local[1]\",\"local[2]\"],\"http_addresses\":[\"local[80]\",\"local[81]\"]," +

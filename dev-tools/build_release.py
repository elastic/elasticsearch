...
def smoke_test_release(release, files, expected_hash, plugins):
  for release_file in files:
    ...
    run('%s; %s -Des.node.name=smoke_tester -Des.cluster.name=prepare_release -Des.discovery.zen.ping.multicast.enabled=false -Des.script.dynamic=on -Des.script.indexed=on %s'

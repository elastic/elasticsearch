...
if not defined ES_TMPDIR (
  for /f "tokens=* usebackq" %%a in (`CALL %JAVA% --module-path "!LAUNCHERS_MODULEPATH!" -m org.elasticsearch.launchers/org.elasticsearch.tools.launchers.TempDirectory`) do set ES_TMPDIR=%%a
)
...
@setlocal
for /F "usebackq delims=" %%a in (`CALL %JAVA% --module-path "!LAUNCHERS_MODULEPATH!" -m org.elasticsearch.launchers/org.elasticsearch.tools.launchers.JvmOptionsParser "!ES_PATH_CONF!" "!ES_HOME!"/plugins ^|^| echo jvm_options_parser_failed`) do set ES_JAVA_OPTS=%%a

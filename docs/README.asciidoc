...
* Release notes for known issues are an exception to this policy. Document the 
  known issue to the release notes for any version affected by the issue.
  Backport the change to any branch containing release notes for those versions,
  even if the branch is no longer maintained.

  ...
  * `// TESTRESPONSE[s/foo/bar/]`: Substitutions. See `// TEST[s/foo/bar]` for
  ...
  to the documentation.
    * One interesting difference here is that you often want to match against
    ...
    response, instead just match against what is in the response." Instead of
    writing the path into the response after `$body` you can instead write
    `$_path` which "figures out" the path. This is especially useful for making
    sweeping assertions like "I made up all the numbers in this example, don't

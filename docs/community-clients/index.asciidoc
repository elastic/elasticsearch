...
* https://www.forgebox.io/view/cbelasticsearch[cbElasticSearch]
  Elasticsearch support for the ColdBox MVC Platform which provides you with a fluent search interface for Elasticsearch, in addition to a CacheBox Cache provider and a Logbox Appender for logging.

...
4. **R4 mTLS to the gateway** ([§4.5](otel-audit-logging-poc.md#sec-4-5)). Protocol is gRPC. Follow the `ReindexSslConfig` pattern (`modules/reindex`): add three static NodeScope `telemetry.logs.ssl.*` settings — `certificate_authorities`, `certificate`, `key` — delivered by `elasticsearch-controller` via `elasticsearch.yml`. Load via `SslConfigurationLoader`, build a combined trust store (configured CAs + JVM default roots — the fix for the gateway cert's multiple-parent chain), inject via `OtlpGrpcLogRecordExporterBuilder.setSslContext(...)`. `verification_mode` is not exposed as a setting — always `full`. `key_passphrase` is not needed — the controller delivers plain (unencrypted) PEM keys. Wire `ResourceWatcherService` + `FileWatcher` for hot-reload when cert content rotates in-place. Client cert and key paths are delivered to the node by `elasticsearch-controller` alongside the `telemetry.logs.ssl.*` path-settings (coordinate with Julio Camarero — expected to follow the workload-identity cert delivery pattern). *Independent of item 1 — can be worked in parallel.*

...
*Trigger: `elasticsearch-controller` delivers the mTLS client cert to ES nodes (Julio Camarero — expected to follow the workload-identity cert delivery pattern). Independent of PR 2.*
...
1b. **Resource attributes: `project.name`, `organization.id`, `service.name`, `service.type`**. The field reference doc requires these as `resource.attributes` in `OtelSdkExportLogsSupplier`:
   - `project.name` (required, custom): human-readable project display name. **Blocked — waiting on Julio Camarero's response.** The human-readable name is stored in `project-api`'s global state (`ProjectBaseConfig.name`) but is not propagated to ES nodes. `OriginProjectState` only carries `alias`, which is a URL slug (e.g. `"my-project-a1b2c3"`), not the display name. Two options: (a) controller team adds `name` to `OriginProjectState` delivery; (b) the otel-delivery-gateway enriches the field from `project-api` using the `project.id` it already reads for routing — no ES or controller change needed. Comment left in the field reference doc; awaiting Julio's direction.
   - `organization.id` (required, ECS): Elastic organization ID. Source: `OriginProjectState.organization()` — available in cluster state via the Cross-Project module. (The deprecated `serverless.organization_id` node setting has been removed; `OriginProjectState` is the authoritative source per serverless PR #5853.)
   - `service.name` and `service.type`: **already correct in the merged PR 1 code.** `OtelSdkResource.get(settings)` (introduced during review) sets `service.name = "self-managed-elasticsearch"` and `service.type = "elasticsearch"` by default. Overrides are injected via the `telemetry.resource.*` affix setting (`TELEMETRY_OTEL_RESOURCE_ATTRIBUTES`); serverless and ECH use this to set their respective values. No action needed.
   *`organization.id` can be done in PR 2. `project.name` blocked pending Julio's response.*
...
4. **R4 mTLS to the gateway** ([§4.5](otel-audit-logging-poc.md#sec-4-5)). Protocol is gRPC. Add three static NodeScope `telemetry.otel.logs.ssl.*` settings — `certificate_authorities`, `certificate`, `key` — delivered by `elasticsearch-controller` via `elasticsearch.yml`. Load via `SslConfigurationLoader` (pattern from `modules/reindex/ReindexSslConfig`), build a combined trust store (configured CAs + JVM default roots — the fix for the gateway cert's multiple-parent chain), inject via `OtlpGrpcLogRecordExporterBuilder.setSslContext(...)`. `verification_mode` is not exposed as a setting — always `full`. `key_passphrase` is not needed — the controller delivers plain (unencrypted) PEM keys. The `SSLContext` is built once at startup (no `ResourceWatcherService`/`FileWatcher`); cert rotation requires a node restart. This matches how the JWT and OIDC realm HTTP clients handle SSL — both snapshot `sslProfile.sslContext()` into a long-lived client with no cert watcher. If in-place hot-reload without restart is later required, see follow-on item 10. Client cert and key paths are delivered to the node by `elasticsearch-controller` alongside the `telemetry.otel.logs.ssl.*` path-settings (coordinate with Julio Camarero — expected to follow the workload-identity cert delivery pattern). *Independent of item 1 — can be worked in parallel.*
...
10. **mTLS hot-reload (cert rotation without restart)**. PR 4 builds the `SSLContext` once at startup; cert rotation requires a node restart. If `elasticsearch-controller` cannot restart the pod on cert rotation and instead updates the cert files in-place, hot-reload becomes necessary. Implementation would require: threading `ResourceWatcherService` through `APMLoggingService`/`OtelSdkExportLogsSupplier`, registering `FileWatcher`s on `SslConfiguration.getDependentFiles()`, and rebuilding the `SdkLoggerProvider` + re-attaching the appender on cert change (the OTLP gRPC channel bakes the `SSLContext` at `build()` time, so a live swap requires teardown of the whole provider — considerably more invasive than `ReindexSslConfig`'s single-line `reload()`). *Deferred pending confirmation from Julio Camarero that the controller restarts ES pods on cert rotation.*

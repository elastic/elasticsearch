...
4. **R4 mTLS to the gateway** ([§4.5](otel-audit-logging-poc.md#sec-4-5)). Protocol is gRPC. Add three NodeScope `telemetry.otel.logs.ssl.*` settings — `certificate_authorities`, `certificate`, `key` — delivered by `elasticsearch-controller` via `elasticsearch.yml`. Load via `SslConfigurationLoader` (pattern from `modules/reindex/ReindexSslConfig`), build a combined trust store (configured CAs + JVM default roots — the fix for the gateway cert's multiple-parent chain), inject via `OtlpGrpcLogRecordExporterBuilder.setSslContext(...)`. `verification_mode` is not exposed as a setting — always `full`. `key_passphrase` is not needed — the controller delivers plain (unencrypted) PEM keys. **Hot-reload is required** (confirmed 2026-06-17 by Julio Camarero): the controller rotates certs by replacing the files in-place without restarting the pod; see item 10 (now folded into this PR). Client cert and key paths are delivered to the node by `elasticsearch-controller` alongside the `telemetry.otel.logs.ssl.*` path-settings (coordinate with Julio Camarero — expected to follow the workload-identity cert delivery pattern). *Independent of item 1 — can be worked in parallel.*
...
10. **mTLS hot-reload (cert rotation without restart)**. **Confirmed required and folded into PR 4 (2026-06-17).** Julio Camarero confirmed the controller does not restart pods on cert rotation — it replaces the cert files in-place and expects the node to pick them up. Without hot-reload, the first automatic rotation would silently leave the exporter presenting an expired cert until someone manually reboots the node. Implementation: thread `ResourceWatcherService` through `APM`/`APMTelemetryProvider`/`APMLoggingService`/`OtelSdkExportLogsSupplier`; register `FileWatcher`s on `SslConfiguration.getDependentFiles()`; on change, rebuild the `SdkLoggerProvider` and re-attach the appender (the OTLP gRPC channel bakes the `SSLContext` at `build()` time, so a live swap requires teardown of the whole provider — more invasive than `ReindexSslConfig`'s single-line `reload()`).

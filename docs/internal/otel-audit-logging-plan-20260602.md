...
1. **Build OTel-path attribute transformation** ([§5.1](otel-audit-logging-poc.md#sec-5-1), [§4.2](otel-audit-logging-poc.md#sec-4-2)). The "Serverless Audit Log Field Reference" doc (Julio Camarero, 2026-06-04) is the authoritative spec — it supersedes the §5.2 table in the PoC doc and `log4j2.serverless.properties` as the field-naming reference. A custom log4j appender (~50 LOC, preferred) drops the `log4j.map_message.` prefix and applies the rename map to the `elasticsearch.audit.*` / ECS target schema. `trace.id` goes in native `LogRecord.traceId`. Since `enrich()` is the first call in `build()` — guaranteed by construction — the appender always sees operator-redacted `action`/`request.name`; no special ordering care is needed.
   *Unblocks: item 4.*

1b. **`project.name` and `organization.id` resource attributes**. The field reference doc requires `project.name` and `organization.id` on every OTel log record (as `resource.attributes`, not per-record `attributes`). These would be added to the `Resource` in `OtelSdkExportLogsSupplier`. Source for these values is TBD — likely cluster settings delivered by `elasticsearch-controller` alongside the existing `serverless.project_id` setting, but needs confirmation. *Ankit Sethi was researching the source as of 2026-06-10; coordinate with him before implementing.*
   *Can be done in PR 2.*

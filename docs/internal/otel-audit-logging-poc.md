...
Each requirement below has a status line — *Satisfied*, *Partially satisfied*, or *Gap* — pointing to §3 evidence or §4 gap analysis.
...
Two pieces are open:
1. the upstream `OpenTelemetryAppender` library prepends `log4j.map_message.` to every attribute that comes from a `StringMapMessage`, so today's records carry `log4j.map_message.event.action` instead of `event.action` — see §5.1 for options to drop the prefix; and
2. the per-field translation from current ES audit names to semconv/ECS/custom hasn't been done — see §5.2.
...
Audit events must carry `project.id` so the gateway can route per-project. The PoC plumbs the field through a single chokepoint — `LoggingAuditTrail.LogEntryBuilder.withThreadContext()` — that every public audit-emit method calls, so the mechanism is uniform across audit event types (§3.3 lists them). End-to-end OTLP delivery has been verified for one event type only.
...
Source: Gateway TDD §"OTel log delivery pipeline on ECP" — Authentication & authorization.
...
Toggling audit logging — both at cluster level and per project (R8) — must take effect without a restart. The PoC reads `telemetry.otel.logs.enabled` once in `OtelSdkExportLogsSupplier.install()` (line 69) at startup, so toggling it requires a restart today.

For the cluster-level case, Audit TDD §"Requirement 1" — *"... enable or disable audit logging with a simple setting on the Cloud console without any downtime (cluster restart)"* — is in flight under [PR 147333](https://github.com/elastic/elasticsearch/pull/147333) for `xpack.security.audit.enabled`. That setting governs the audit logger, not the OTel-logs path; we still need our own dynamic listener for `telemetry.otel.logs.enabled` and for R8's per-project state.
...
- One line in `LogEntryBuilder.withThreadContext(...)` at line 1662: `setThreadContextField(threadContext, Task.X_ELASTIC_PROJECT_ID_HTTP_HEADER, PROJECT_ID_FIELD_NAME);` — reads the project header from `ThreadContext` and adds it to the audit `StringMapMessage` if non-null.
...
**Open question — CPS routing direction**: see §4.3.
...
**Caveat:** they arrive prefixed — `log4j.map_message.event.action` — because the upstream `OpenTelemetryAppender` library hardcodes that prefix when capturing `StringMapMessage` entries with `setCaptureMapMessageAttributes(true)`. Bare-key shape is not yet satisfied; see §5.1 for options.
...
When a CPS query in project A reaches into project B, audit events on project B's side carry project B's `project.id`, not project A's — same-cluster via `AbstractProjectResolver.executeOnProject(...)` (`AbstractProjectResolver.java:70`), cross-cluster via header strip-and-reset at the cluster boundary (`ThreadContext.java:333–335`).

That matches the gateway's intent if each event should be delivered to the project that *owns the data touched*; it doesn't if events should follow the *originator*. A design call with the gateway team is needed. Implementation is mechanical once decided: leave as-is, or capture an `originating_project.id` separately — and for the cross-cluster case, propagate it explicitly, since `X_ELASTIC_PROJECT_ID_HTTP_HEADER` is intentionally stripped at the boundary.
...
- (b) Apply an attribute filter on the OTel emit path — a custom `LogRecordProcessor`, or as part of the §5.1 custom appender — that drops these keys. Localizes the change to the OTel module; the file appender continues to see the full set on hosted/self-hosted.
...
### 4.11 Production-side integration test

The PoC's IT uses an in-process recording server. Full validation obviously requires a test against a real `otel-delivery-gateway`.

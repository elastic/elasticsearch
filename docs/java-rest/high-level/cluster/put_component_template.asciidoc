...
An +{request}+ specifies the name of the component template and the template definition
which can consist of the settings, mappings or aliases, together with a version (which
can be used to simply component tempalte management by external systems) and a metadata

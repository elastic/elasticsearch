...
<2> Add the first `TermVectorsRequest` to the +{request}+.
<3> Add the second `TermVectorsRequest` for an artificial doc to the +{request}+.
...
--------------------------------------------------
<1> Create a template `TermVectorsRequest`.

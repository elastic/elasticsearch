...
`RollupJobConfig` also sets default metrics for `DateHistogram` and `Histogram` groupings.
If the user does not provide a `MetricConfig` referencing the field in a `DateHistogram` or
`Histogram` in the `GroupConfig`, the default metrics of `min`, `max`, and `value_count` are
...
the defaults are added server side.

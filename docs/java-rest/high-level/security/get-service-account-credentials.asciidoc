...
The returned +{response}+ contains a list of service tokens for the requested service account.
...
<2> List of service token information
...
<6> Response header containing the information about the execution of collecting `file` service tokens.
<7> Number of nodes that successful complete the request of retrieving file-backed service tokens
<8> Number of nodes that fail to complete the request of retrieving file-backed service tokens

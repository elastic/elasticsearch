...
`performRequestAsync` returns a `Cancellable` instance that may be used to cancel 
requests. Doing so will result in a call to `abort` on the underlying Apache
HttpClient request. This will result in the client side resources being freed up.
However, it does not necessarily mean the server will abort in progress requesrs.

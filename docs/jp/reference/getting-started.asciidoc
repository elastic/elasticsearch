...
クラスタヘルスをチェックするために、{ref}/cat.html[`_cat` API]を使用します。「VIEW IN CONSOLE」をクリックして{kibana}/console-kibana.html[Kibanaのコンソール]で以下のコマンドを実行します。または、以下の「COPY AS CURL」リンクをクリックしてターミナルに貼り付け、`curl`を使用して以下のコマンドを実行します。

...
<<painless-operators, operation>>. Each operation that requires promotion will
have a promotion table that shows all allowed conversions. Values can be
promoted to the `def` type at compile-time; however, at run-time, the promoted
type value is derived from what the `def` type value represents.

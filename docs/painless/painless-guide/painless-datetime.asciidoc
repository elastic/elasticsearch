...
source document is most commonly accessible through an input called `_source`
or `ctx['_source']`.
...
[source,Painless]
----
def inputDatetime = doc['input_datetime'].value;
String datetime = inputDateTime.format(DateTimeFormatter.ISO_INSTANT);
----
Note the use of a built-in DateTimeFormatter.
+
...
[source,Painless]
----
// check for the existence of fields within the current document
if (doc.containsKey('start_datetime') && doc.containsKey('end_datetime')) {

    // check for the existence of values for fields within the current document
    if (doc['start_datetime'].size() > 0 && doc['end_datetime'].size() > 0) {
        ...
        // handle the difference in datetimes
    } else {
        // handle documents with missing start or end datetime values
    }
} else {
    // handle indexes without start or end datetime fields
}
----

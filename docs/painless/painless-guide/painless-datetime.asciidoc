...
* Convert a numeric datetime from user parameters to a complex datetime
+
Input:
+
[source,JSON]
...
----
+
* Convert a string datetime from user parameters to a complex datetime
+
Input:
+
[source,JSON]
...
Use the original {ref}/mapping-source-field.html[source] document as a script
input to access a numeric datetime or string datetime for a specific field
within that document. Access to the original source within a script is
dependent on the Painless context and is not always available. The original
source is most commonly accessible through an input called `_source` or
`ctx['_source']`.

*Examples*

* Convert a numeric datetime from a sourced document to a complex datetime
+
Input:
+
[source,JSON]
...
----
+
* Convert a string datetime from a sourced document to a complex datetime
+
Input:
+
[source,JSON]

...
input will be accessed from parameters specified by the user, from the original
source, or from an index.
...
* Convert a numeric datetime from parameters to a complex datetime
+
Input:
+
[source,JSON]
...
----
+
* Convert a string datetime from parameters to a complex datetime
+
Input:
+
[source,JSON]
...
===== Datetime Input From Source

Use the original {ref}/mapping-source-field.html[source] used to index a
document as a script input to access a numeric datetime or string datetime for
a specific field within that document. Access to the original source within a
script is dependent on the Painless context and is not always available. The
original source is most commonly accessible through an input called `_source`
or `ctx['_source']`.

*Examples*

* Convert a numeric datetime from source to a complex datetime
+
Input:
+
[source,JSON]
...
[source,Painless]
----
long inputDatetime = _source['input_datetime'];
Instant instant = Instant.ofEpochMilli(inputDateTime);
ZonedDateTime zdt = ZonedDateTime.ofInstant(instant, ZoneId.of('Z'));
----
+
* Convert a string datetime from source to a complex datetime
+
Input:
+
[source,JSON]
...
[source,Painless]
----
String datetime = ctx['_source']['input_datetime'];
DateTimeFormatter dtf = DateTimeFormatter.ofPattern(
        "'custom' 'y' yyyy 'm' MM 'd' dd HH:mm:ss VV");
ZonedDateTime zdt = ZonedDateTime.parse(datetime, dtf);
----

===== Datetime Input From an Index

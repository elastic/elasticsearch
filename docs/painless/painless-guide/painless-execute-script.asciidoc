...
<<runtime,runtime field>>. This API requires very few dependencies, and is
...
<<painless-runtime-boolean-request,boolean_field>>.
...
<<painless-runtime-double-request,double_field>>.

`geo_point_field`::
Emits a `long` data type and returns a sorted list of `double` values. See
<<painless-runtime-geo-request,geo_point_field>>.
...
[[painless-runtime-datetime-request]]
===== `date_time` request
...
[[painless-runtime-datetime-response]]
===== `date_time` response
...
[[painless-runtime-double-request]]
===== `double_field` request
Use the `double_field` runtime context for <<numeric,numeric data>> of type
...
[[painless-runtime-double-response]]
===== `double_field` response
...
[[painless-runtime-geo-request]]
===== `geo_point_field` request
<<geo-point,Geo point>> fields accept latitude-longitude pairs. You can define a
...
[[painless-runtime-geo-response]]
===== `geo_point_field` response
...
<<ip,`ip`>>. For example, let's say you have a `message` field from an Apache
...
<<keyword,Keyword fields>> are often used in sorting, aggregations, and
term-level queries.

...
<<painless-runtime-datetime,date_time>>.
...
you indicate the context for a runtime field based on the data type you want to
return.
...
The following script adds a variable named `pageTime` to hold the number of
seconds in a calendar year. The script then assesses the `author` and either
divides or multiplies the `pageTime` value. If the author is
`Robert A. Heinlein`, the script divides by two. Because `Alastair Reynolds`
writes much slower, the script multiplies `pageTime` by two.

Then, the script subtracts the release date value from the calculation of
`pageTime` times the `page_count` to determine approximately when the author
...
POST /_scripts/painless/_execute
{
  "script": {
    ...
    long pageTime = 31536000;
    if (author == 'Robert A. Heinlein') {
      pageTime /= 2;
    } else if (author == 'Alastair Reynolds') {
      pageTime *= 2;
    }
    emit(doc['release_date'].value.dayOfYear - pageTime * doc['page_count'].value);
    """
  },
  "context": "date_field",
  "context_setup": {
...
`Revelation Space` on 30 October 1968. That's a long time to write a book
(about 32 years), but sometimes good science fiction takes time!

[source,console-result]
----
{
  "result" : [
    "1968-10-30T22:48:00.075Z"
  ]
}
----

...
This API is useful for building and testing scripts with very few dependencies. The API
uses several _contexts_, which control how scripts are executed, what variables are
available at runtime, and what the return type is.

Each context requires a script, but the additional 
...
=====
`boolean_field`:: 
Runtime field that emits a `boolean` data type and returns a sorted list of `boolean` values.

`date_field`:: 
Runtime field that emits a `long` data type and returns a sorted list of `long` values.

`double_field`::
Runtime field that emits a `long` data type and returns a sorted list of `double` values.

`geo_point_field`::
Runtime field that emits a `long` data type and returns a sorted list of `double` values.

`ip_field`::
Runtime field that emits a `string` data type and returns a sorted list of `string` values.
...
+
NOTE: This parameter is required for all contexts except `painless_test`, which is the
default if no value is provided for `context`.
+
...
NOTE: This parameter only applies when `score` is specified as the script `context`.
+
Use this parameter to specify a query for computing a score. Besides deciding whether
or not the document matches, the {ref}/query-filter-context.html#query-context[query clause] also calculates a relevance score in the
...
Because the default context is `painless_test`, you don't need to specify the `context`
or `context_setup`.
...
===== Request (keyword_field)
The following request adds a `@timestamp` field of type `date` to the index mappings:
...
If you have a timestamp but want to return the equivalent day of the week, you can
create a script in the `keyword_field` context:

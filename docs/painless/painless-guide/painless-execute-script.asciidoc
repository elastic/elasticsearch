...
The following script adds a variable named `pageTime`. The value for this field
is `28800000` milliseconds, which equates to eight hours. We'll assume that this
is an approximate time required to write a complete page for a book.

The script then assesses the `author` and either divides or multiplies the
`pageTime` value. If the author is `Robert A. Heinlein`, the script divides by
two. Because `Alastair Reynolds` writes much slower, the script multiplies
`pageTime` by two if that's the matching author.

Then, the script subtracts the release date value (in milliseconds) from the
calculation of `pageTime` times the `page_count` to determine approximately when

...
yourself set `script.painless.regex.enabled: true` in `elasticsearch.yml` or you can 
add `"script.painless.regex.enabled" : true` in 
https://www.elastic.co/guide/en/cloud/current/ec-add-user-settings.html[Elastic Service Console].

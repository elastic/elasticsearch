...
values specified at the time of allocation. Use a negative int type value as an
index to access an value in reverse from the end of an array type value within a
range of `[-size, -1]`.
...
size of each dimension is specified by an int type value in between the `[` and
`]` tokens.

*Errors*

* If a value other than an int type value or a value that is castable to an int
  type value is specified for for a dimension's size.

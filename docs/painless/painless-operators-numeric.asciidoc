...
Use the `bitwise not operator '~'` to *flip* each bit in an integer type value
...
Use the `division operator '/'` to *divide* two numeric type values.  Rules for
NaN values and division by zero follow the JVM specification.  Division with
integer values drops the remainder of the resultant value.
...
The following table illustrates the resultant bit from the anding of two bits.

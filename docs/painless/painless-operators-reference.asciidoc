...
Use the `list initialization operator '[]'` to allocate an `List` type value
with a set of pre-defined values. Each value used to initialize the `List` type
value is converted to a `def` type value when inserted into the `List` type
...
Use the `map initialization operator '[:]'` to allocate a `Map` type value with
a set of pre-defined values. Each pair of values used to initialize the `Map`
type value are converted to `def` type values when inserted into the `Map` type
value using the `put` method. The order of specified values is maintained upon
insertion.

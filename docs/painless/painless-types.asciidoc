...
instance. Use a reference type instance to read from, write to, and manipulate
complex data.
...
reference type object name to access a static member field for reading and
writing to a specific reference type *object*. No reference type instance
...
<<field-access, field access operator>> for reading and writing to a non-static
...
`String` type value. While not required,
the <<constructor-call, new instance operator>> can allocate `String` type
instances.
...
<<array-access, array access operator>> to read and write to individual values
within an array type value.

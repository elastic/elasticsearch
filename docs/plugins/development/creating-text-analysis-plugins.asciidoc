...
* `plugin-api`, which includes important annotations for the plugin 
framework
* `plugin-analysis-api`, which contains the classes that an analysis plugin can 
implement.
* `lucene-analysis-common`, the core Lucene module
...
<<creating-non-text-analysis-plugins>>. However, plugins created using that 

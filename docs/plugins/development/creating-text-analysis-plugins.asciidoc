...
<<creating-non-text-analysis-plugins>>. Howeverm plugins created using that 
method are pinned to specific version of {es}. You will need to recompile them
when upgrading{es}.

A text analysis can implement four factory classes that are provided by the 
analysis plugin API.

...
matching certain criteria determined by the <<discovery-ec2-settings,plugin
settings>>.
...
[[discovery-ec2-usage]]
[[discovery-ec2-settings]]
==== Using the EC2 discovery plugin

The `discovery-ec2` plugin adds a seed hosts provider named `ec2` which
provides a list of seed addresses to the
{ref}/modules-discovery-hosts-providers.html[discovery process] by querying the
https://github.com/aws/aws-sdk-java[AWS API] for a list of EC2 instances
matching certain criteria including their tags, membership of certain security
groups, and placement in certain availability zones. The interaction with the
AWS API can be authenticated using an
http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/iam-roles-for-amazon-ec2.html[instance
or container role], or else custom credentials can be supplied.
...
`discovery.ec2.access_key`::
    ...
    If unset, `discovery-ec2` will instead use the instance or container role
    as appropriate. This setting is sensitive and must be stored in the
    {ref}/secure-settings.html[Elasticsearch keystore].
...
`discovery.ec2.session_token`::
    ...
    stored in the {ref}/secure-settings.html[Elasticsearch keystore].
...
`discovery.ec2.proxy.username`::
    ...
    in the {ref}/secure-settings.html[Elasticsearch keystore].
...
For example if you tag some EC2 instances with a tag called
`my-elasticsearch-host` and set `host_type: tag:my-elasticsearch-host` then the
`discovery-ec2` plugin will read each instance's host name from the
`my-elasticsearch-host` tag.
...
`discovery.ec2.availability_zones`::
    ...
    A list of the names of the availability zones to use for discovery. Only
    instances placed in one of the given availability zones will be used for
    discovery.
...
that are not master-eligible {es} nodes. In this case, tagging the EC2 instances
that _are_ running the master-eligible {es} nodes, and then filtering by that
tag, will help discovery to run more efficiently.
...
`discovery.ec2.node_cache_time`::
    ...
    cached. Defaults to `10s`. This setting limits how frequently {es} requests
    discovery information from the EC2 API. It is important to limit this
    request frequency because AWS may reject discovery requests if they are
    made too often, and this would cause discovery to fail.
...
recomended or supported as it does not offer satisfactory performance.
...
should provision multiple clusters and use
{ref}/modules-cross-cluster-search.html[cross-cluster search] and
https://www.elastic.co/guide/en/elastic-stack-overview/current/xpack-ccr.html[cross-cluster
replication].

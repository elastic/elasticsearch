...
be dynamically updated at any time. To enable, add the following in `elasticsearch.yaml`:
...
hosts list in `elasticsearch.yaml` (if specified), not replace it.  Therefore,

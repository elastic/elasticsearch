...
| `properties`           | no        | all properties but `raw_metadata` | Array of properties to select to be stored. Can be `content`, `title`, `name`, `author`, `keywords`, `date`, `content_type`, `content_length`, `language`, `raw_metadata`
...
==== Extracting all raw metadata from documents

...
==== Field Conditional Processor

This processor enables branching upon a boolean expression consisting of multiple conjunctive or disjunctive
relational expressions, or a singular relational expression. If the provided conditional expression evaluates
to `true`, then the processors defined within the `then` processors list will proceed to be executed, otherwise,
the `else` processors will be executed.
...
|======
| Name        | Required  | Default | Description
| `if`        | yes       | -       | The relational expression, or boolean expression that is tested.
| `then`      | no        | -       | The processors to execute in order if `condition` evaluates to true.
| `else`      | no        | -       | The processors to execute in order if `condition` evaluates to false.
|======

An example with a relational expression,
...
An example with a boolean expression,
...
--------------------------------------------------
{
  ...
  "processors" : [
    {
      "conditional" : {
        "if" : { "all" : [ { "foo" : {"gte" : 1990 } }, { "foo" : { "lt" : 2000 } } ] },
        "then" : [ ... processors ... ],
        "else" : [ ... processors ... ]
      }
    }

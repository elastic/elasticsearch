...
tools). 
While NER only aims to mark the positions and types of name uses in text,
`entity resolution` goes a step further and associates an ID with the text.
...
PUT my_index/_doc/2
{
  "my_field": "[Jeff Beck](person=Jeff+Beck&role=Guitarist) plays a strat"<1>
}
...
In this example the optional type information (`person` and `role`) is not 

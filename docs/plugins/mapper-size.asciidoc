...
[NOTE]
.Using `_size` in Kibana
================================================

To use the `_size` field in Kibana, update the `metaFields` setting and add 
`_size` to the list of meta fields. `metaFields` can be configured in Kibana 

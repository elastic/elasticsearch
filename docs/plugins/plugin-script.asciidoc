...
that include a drive letter require  three leading `/` before the drive letter. This uses Java's
built-in parsing behavior.

[source,shell]
-----------------------------------
bin\elasticsearch-plugin install "file:///C:/Program Files/path/to/plugin.zip"
-----------------------------------

NOTE: Any path that contains spaces must be contained in quotes!

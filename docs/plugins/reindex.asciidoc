...
Both APIs support the `refresh` url parameter. Setting it to true will cause
all indexes to which the request wrote to be manually refreshed. This is
different than the Index API's `refresh` parameter which causes the shard that
received the new data to be indexed.

...
You can define a global proxy applicable to each azure client using the following settings:
...
Supported values for `proxy.type` are `http` or `socks`. When `proxy.type` is set, `proxy.host` and `proxy.port` must
be provided.

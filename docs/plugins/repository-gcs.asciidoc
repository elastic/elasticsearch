...
`chunk_size`::
    ...
    The chunk size can be specified in bytes for e.g. 1024B for a 1 KB data. 
    Alternatively chunk size can also be specified using size value notation 
    e.g. `1GB`, `10MB`, `5KB`. Defaults to `100m`, which is the maximum permitted.

...
The HDFS Repository Plugin integrates seamlessly with Hadoop's security model. To enable security, simply pass the authentication method to the underlying Hadoop configuration through the repository creation request:
...
The following authentication methods are supported by the plugin:

[horizontal]
`simple`::

    Also means "no security". Uses information from underlying operating system account to inform Hadoop of who the current user is.

...
AWS instances typically resolve S3 endpoints to a public IP. If the Elasticsearch
...
Note that Amazon has published instructions at 
https://docs.aws.amazon.com/cli/latest/reference/ec2/create-vpc-endpoint.html
that permit you to create a gateway endpoint that will expose access to S3 to 
endpoints within subnets within your VPC, which will network load upon your
VPC's NAT instance while backing up and restoring your ElasticSearch cluster
to Amazon S3.

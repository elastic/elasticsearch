...
Note that you can define S3 repository settings for all S3 repositories in `elasticsearch.yaml` configuration file.
They are all prefixed with `repositories.s3.`. For example, you can define compression for all S3 repositories
by setting `repositories.s3.compress: true` in `elasticsearch.yaml`.
...
bucket is exclusive for the other iam user. To enable tests prepare a config file elasticsearch.yaml with the following
content:
...
[source,sh]
----
mvn -Dtests.aws=true -Dtests.config=/path/to/config/file/elasticsearch.yaml clean test
----

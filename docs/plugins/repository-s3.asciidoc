...
Additionally, it is possible to create a https://docs.aws.amazon.com/vpc/latest/userguide/vpc-endpoints.html[VPC endpoint]
that enables connecting to S3 in instances that reside in a private subnet in an
AWS VPC. This will reduce network load upon your VPC's NAT instance while backing 
up and restoring your ElasticSearch cluster to Amazon S3.

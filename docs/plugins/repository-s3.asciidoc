...
In case your S3 endpoint is in the same region as your AWS instances you should think about enabling link:https://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/vpc-endpoints-s3.html[Gateway VPC Endpoints for S3] which overcomes the traffic limitation but also safes money in most cases.

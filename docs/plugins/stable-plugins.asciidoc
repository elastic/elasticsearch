...
A sample Gradle build file for using the Gradle plugin:
...
Under your yamlRestTest directory, create a `java` folder for Java sources
and a `resources` folder.

In your Java folder, create an implementation of the ESClientYamlSuiteTestCase
class (that is, under `src/yamlRestTest/java/your/package/name`):
...
Under `src/yamlRestTest/resources/rest-api-spec/test/plugin`, create a yaml file:
...
This test executes a request against the `_analyze` API under the "do" block and
verifies the conditions in the "length" and "match" sections. For more information,
see...

TODO: Link to the source doc for rest API tests.

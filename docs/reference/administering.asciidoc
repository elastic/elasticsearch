...
{security-features} enabled. Specifically, this means that the snapshot API has
to be authorized.
...
be granted to users in the "administrator" category. Automated tools should not
run as users with this privilege.

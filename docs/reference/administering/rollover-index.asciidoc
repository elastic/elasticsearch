...
The rollover index API rolls an <<indices-aliases, index alias>> to a new index
when the existing index meets a condition you provide. You can use this API to
retire an index that becomes too large or too old.

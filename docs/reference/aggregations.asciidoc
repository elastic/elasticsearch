...
* How many products does each product category contain?
...
* experimental:[] <<search-aggregations-matrix,Matrix>> aggregations that output
...
sub-aggregation calculates an average value for each bucket of documents.
...
IMPORTANT: Some aggregations return a different aggregation type from the the
...
use a `script` to extract or generate values for an aggregation:
...
an aggregation. The following aggregation uses a script to modify `my-field`
values:

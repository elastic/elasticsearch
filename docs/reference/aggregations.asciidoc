...
When a field doesn't exactly match the aggregation you need to do you
...
Scripts will calculate the values on the fly, adding a little overhead
to the execution of the aggretion. Some aggregations like
<<search-aggregations-bucket-terms-aggregation,`terms`>> and
<<search-aggregations-bucket-filters-aggregation,`filters`>> have to
give up some optimizations.

...
Scripts calculate field values dynamically, which adds overhead
to the aggregation. Certain aggregations like
<<search-aggregations-bucket-terms-aggregation,`terms`>> and
<<search-aggregations-bucket-filters-aggregation,`filters`>> are
less optimized when using scripts.

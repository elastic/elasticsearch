...
By default buckets are returned where the number of matched documents is 1 or greater but it is
possible to increase this threshold by passing a `min_doc_count` parameter.

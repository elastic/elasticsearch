...
Historically these were configured in a single `interval` field, which lead to
confusing semantics.  Specifying `1d` would be assumed as a calendar-aware time,
whereas `2d` would be interpreted as fixed time.  To get "one day" of fixed time,
the user would need to specify `24h`.
...
Requesting bucket intervals of a month in calendar time.
...
And a similar quantity using fixed intervals would be something near 30 days:

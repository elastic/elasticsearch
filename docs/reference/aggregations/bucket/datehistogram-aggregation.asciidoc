...
The `offset` is a fixed interval, which can be applied to `calendar_interval` bucket definitions.
...
days that change from standard to summer-savings time or back.
...
When querying for a date histogram over the calendar interval of months, we will have each document in a single bucket,
one for each month.
And each bucket will have a key named after the first day of the month, plus any offset.
For example, the offset of `+19d` will have buckets with names like `2022-01-20`.
...
--------------------------------------------------
// TEST[skip:no setup made for this example yet]

If we increase the offset to `+20d`, each document will appear in a bucket for the previous month,
with all bucket keys ending with the same day of the month, as normal.
However, if we further increase to `+28d`,
we will find that what used to be a February bucket has now become `"2022-03-01"`.

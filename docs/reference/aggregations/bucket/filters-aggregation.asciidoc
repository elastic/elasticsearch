...
in the object. You can use the `sortable` parameter to specify the buckets as an array of objects. The value of this
parameter can be as follows:

`false`::         (Default) Returns the buckets as an object
`true`::          Returns the buckets as an array of objects

NOTE: This parameter is ignored by <<Anonymous filters>>.

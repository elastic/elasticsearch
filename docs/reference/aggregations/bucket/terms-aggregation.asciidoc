...
`Execution hint` only works for `bytes/strings` value(`keyword`, `ip`, etc.)'s aggregation.
As for `numeric` value's aggregation, its mechanism similar to `map`.
If you hava many documents to aggregate on a `numeric` field, consider to add a `keyword` field for this field, and use this `keyword` field to execute aggregation to gain a better performance.

...
By default the `terms` agg returns the ten buckets with the most documents but
you can change that count the `size` parameter to return more buckets up to
<<search-settings-max-buckets,max_buckets>> which defaults to 65,536. So if
there aren't many unique terms it quite viable to return all of them with the
`terms` aggregation. Read on about sorting if you are fine picking the "best"
terms or have a look at the
<<search-aggregations-bucket-composite-aggregation,Composite>> aggregation if
you'd like to be able to page through all terms.

[[search-aggregations-bucket-terms-aggregation-shard-size]]
==== Shard Size

It'd be reasonable to assume that `terms` picks the top terms by asking each
shard to pick the top `size` terms and return them then we'd just combine them.
But what if there's a bucket that has many documents on one shard but is just
outside the `size` on all the others? If each shard only returned `size` buckets
then we'd only return part of the bucket. Which'd be pretty terrible.

We try to avoid this problem by having each shard return `shard_size` buckets
which defaults to `(size * 1.5 + 10)`. So, for a `terms` agg with the default
size of `10` each shard will return the top `25` buckets. This helps, but
it's still quite possible to return partial buckets, it just takes buckets with
more disparate per-shard sizes.

You can absolutely bump `shard_size` when you need to increase the accuracy of
the selection of top buckets. It is much cheaper to increase the `shard_size`
than it is to increase the `size`. It still takes more bytes over the wire and
waiting in memory on the coordinating node. But do it if you need it.
...
to the error on the `doc_count` returned by each shard. It's pretty much the
...
By default, the buckets are ordered by their `doc_count` descending but you can
order by the term itself (`_key`) or to the value of a sub-aggregation. And you
can flip from descending to ascending order.

WARNING: Danger! Danger! Remember how the terms agg picks the top
<<search-aggregations-bucket-terms-aggregation-shard-size, `shard_size`>>
buckets on the data node? Well all of that works really well for something like
doc count descending. A very well for `_key` in either direction. But for doc
count ascending it's frankly *terrible*. That basically throws out all of the
buckets with the most data on every shard. It's likely that a bucket with few
documents on one shard could have more on another but that it wouldn't fit into
`shard_size`. This is so bad that we've created a whole separate
<<search-aggregations-bucket-rare-terms-aggregation>> aggregation.
...
WARNING: There is also danger in sorting on the value of a sub-aggregation but
how much danger depends on what the sub-aggregation is doing. Sorting on, say,
`max` descending is fairly similar to sorting on `doc_count` descending and
fairly safe. Sorting on `max` ascending is likely to cause tons of errors for
the same reason sorting on doc count ascending does. See the warning above for
more on why that's trouble.

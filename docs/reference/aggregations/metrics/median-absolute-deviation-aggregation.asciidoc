...
Median absolute deviation is a measure of variability that is useful for
describing data that may or may not be normally distributed. For data that is
not normally distributed, it can be more descriptive than standard deviation. It
is calculated as the median of each data point's deviation from the sample
median. That is, for a random variable X, the median absolute deviation is
median(|median(X) - X~i~|).
...
The default `compression` value for this aggregation is `1000`.

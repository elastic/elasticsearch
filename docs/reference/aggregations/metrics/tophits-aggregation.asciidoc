...
namely `_source` field `price` in the example above. This is equivalent to using the path
`top_sales_hits[_source.price]`. Other options include `top_sales_hits[_sort]`, for filtering on the sort value `date`

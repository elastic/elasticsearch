...
The `terms` aggregator order feature is used to return the buckets by relevancy order, based on the highest scoring
document captured by the `top_hits` aggregation.
...
Note, the `top_hits` aggregator can only sort buckets by `max_score`.

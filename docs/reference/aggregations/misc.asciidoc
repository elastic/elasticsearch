...
Sometimes you need to know the exact type of an aggregation in order to parse its results. The `prefixed_name` parameter
...
[source,js]
--------------------------------------------------
GET /twitter/tweet/_search?prefixed_name
{
  "aggregations": {
    "tweets_over_time": {
      "date_histogram": {

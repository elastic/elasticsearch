...
the request. This is usually the case for Terms, Significant Terms and PErcentiles aggregations, where the returned
type can be more precise: `lterms` (for a terms aggregation on a Long field), `sigsterms` (for a significant terms
aggregation on a String field), `tdigest_percentiles` (for a percentile aggregation based on the TDigest algorithm)
etc

...
--------------------------------------------------
{
    "moving_fn": {
        ...
        "window": 10,
        "script": "MovingFunctions.windowMin(values)"
    }
}
--------------------------------------------------
// NOTCONSOLE
            ...
            "aggs":{
                ...
                "the_movavg": {
                    "moving_fn": {
                        ...
                        "window": 10,
                        "script": "MovingFunctions.simpleMovAvg(values)"
                    }
                }
            }
        }
...
For example, this script will simply return the first value from the window:
...
The functions are available from the `MovingFunctions` namespace.  E.g. `MovingFunctions.windowMax()`

===== windowMax Function

This function accepts a collection of doubles and returns the maximum value in that window.  If the window is empty, or all values
are `null`/`NaN`, `NaN` is returned as the result.

.`windowMax(Collection<Double> values)` Parameters
|===
...
This function accepts a collection of doubles and returns the minimum value in that window.  If the window is empty, or all values
are `null`/`NaN`, `NaN` is returned as the result.

.`windowMin(Collection<Double> values)` Parameters
|===
|Parameter Name |Description
...
This function accepts a collection of doubles and returns the sum of the values in that window.  If the window is empty, or all values
are `null`/`NaN`, `0.0` is returned as the result.

.`windowSum(Collection<Double> values)` Parameters
|===
|Parameter Name |Description
            ...
            "aggs":{
                ...
                "the_moving_sum": {
                    "moving_fn": {
                        "buckets_path": "the_sum",
                        "window": 10,
                        "script": "MovingFunctions.windowSum(values)"
                    }
                }
            }
        }
...
===== simpleMovAvg Function

The `simpleMovAvg` function calculates the sum of all values in the window, then divides by the size of the window.  It is effectively
...
.`simpleMovAvg(Collection<Double> values)` Parameters
|===
|Parameter Name |Description
|`values` |The window of values to find the sum of
|===
...
==== linearMovAvg Function

The `linearMovAvg` function assigns a linear weighting to points in the series, such that "older" datapoints (e.g. those at
...
.`linearMovAvg(Collection<Double> values)` Parameters
|===
|Parameter Name |Description
|`values` |The window of values to find the sum of
|===
            ...
            "aggs":{
                ...
                "the_movavg": {
                    "moving_fn": {
                        "buckets_path": "the_sum",
                        "window": 10,
                        "script": "MovingFunctions.linearMovAvg(values)"
                    }
                }
            }
        }
...
The `ewmaMovAvg` function (aka "single-exponential") is similar to the `linearMovAvg` function,
...
.`ewmaMovAvg(Collection<Double> values, double alpha)` Parameters
|===
|Parameter Name |Description
|`values` |The window of values to find the sum of
            ...
            "aggs":{
                ...
                "the_movavg": {
                    "moving_fn": {
                        "buckets_path": "the_sum",
                        "window": 10,
                        "script": "MovingFunctions.ewmaMovAvg(values, 0.3)"
                    }
                }
            }
        }
...
The `holtMovAvg` function (aka "double exponential") incorporates a second exponential term which
...
The level calculation is similar to `ewmaMovAvg`, and is an exponentially weighted view of the data.  The difference is
...
.`holtMovAvg(Collection<Double> values, double alpha)` Parameters
|===
|Parameter Name |Description
|`values` |The window of values to find the sum of
            ...
            "aggs":{
                ...
                "the_movavg": {
                    "moving_fn": {
                        "buckets_path": "the_sum",
                        "window": 10,
                        "script": "MovingFunctions.holtMovAvg(values, 0.3, 0.1)"
                    }
                }
            }
        }
...
==== holtWintersMovAvg Function

The `holtWintersMovAvg` function (aka "triple exponential") incorporates a third exponential term which
tracks the seasonal aspect of your data.  This aggregation therefore smooths based on three components: "level", "trend"
and "seasonality".

The level and trend calculation is identical to `holtMovAvg` The seasonal calculation looks at the difference between
...
.`holtWintersMovAvg(Collection<Double> values, double alpha)` Parameters
|===
|Parameter Name |Description
|`values` |The window of values to find the sum of
|`alpha` |Level decay value
            ...
            "aggs":{
                ...
                "the_movavg": {
                    "moving_fn": {
                        "buckets_path": "the_sum",
                        "window": 10,
                        "script": "if (values.size() > 5*2) {MovingFunctions.holtWintersMovAvg(values, 0.3, 0.1, 0.1, 5, false)}"
                    }
                }
            }
        }

...
|`interpolation` |Parameter to use <<interpolation-parameter, interpolation>> between data points. |Optional | `none`
|===
...
By default (with `interpolation = none`), the Percentile Bucket returns the nearest
input data point that is not greater than the requested percentile.
To calculate percentiles with linear interpolation, use `interpolation = linear`,
the implementation includes both endpoints of the data.

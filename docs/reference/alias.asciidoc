...
aliases in your application, you can reindex your data with no downtime or
...
no downtime and will never point to both streams at the same time.
...
To get a list of your cluster's aliases, use the <<indices-get-alias,get alias API>>
with an `_all` argument.

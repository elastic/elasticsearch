...
<<indices-aliases,aliases API>>'s `add` action. If the alias doesn't exist,
this creates it.
...
wildcard pattern matches data streams and indices, the action only uses matching
data streams.
...
To view the aliases for a data stream or index, specify it in the get alias
API's request path.

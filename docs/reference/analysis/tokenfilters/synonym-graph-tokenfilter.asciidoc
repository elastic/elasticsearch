...
Expand definitions for equivalent synonym rules.
...
Using `expand: true`, the synonym would be expanded into:
...
When `expand` is set to `false`, the synonyms are not expanded and the first synonym is treated as the canonical representation. The synonym would be equivalent to:
...
WARNING: An invalid synonym rule will cause an error when applying your analyzer changes.
...
For *explicit synonyms* like `foo, bar => baz` with a stop filter that removes `bar`:
...
For *equivalent synonyms* like `foo, bar, baz` with a stop filter that removes `bar`:

- If `lenient` is set to `false`, an error will be raised as `bar` would be removed from the synonym rule.
- If `lenient` is set to `true`, the mappings added would be equivalent to `foo => foo` and `baz => foo`, which do not contain the removed word.
...
For example, a synonym like `foo, bar => baz` and a stop filter that removes `baz` will get no matches for `foo` or `bar`, as both would get expanded to `baz` which is removed by the stop filter.

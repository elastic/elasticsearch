...
Since the introduction of the security manager in ElasticSearch version 2.0, it is no longer possible to set a custom file path outside of the configuration directory for `path` settings in char or token filters. You must instead specify either path relative to the configuration directory location, or an absolute path inside that directory.

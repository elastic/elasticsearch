...
* `filters` (defaults to an empty list).  If set, then this set of filters will be
used to parse the synonyms file
...
By default, elasticsearch will use the token filters preceding the synonym filter
...
token.  Others, e.g. `multiplexer`, `word_delimiter_graph` or `ngram` will throw an error.  To avoid
these errors, you may set an explicit set of filters to be used when parsing, like so:
        ...
        "index" : {
            "analysis" : {
                "filter" : {
                    "synonym" : {
                        "type" : "synonym",
                        "filters" : [ "lowercase", "word_delimiter_graph" ],
                        "synonyms" : [
                            "i-pod, i pod => ipod",
                            "universe, cosmos"
                        ]
...
Note that `lenient` is automatically set to `true` when `filters` are specified.
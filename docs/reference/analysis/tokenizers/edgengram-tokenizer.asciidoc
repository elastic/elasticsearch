...
=== Limitations of the `max_gram` value

The `edge_ngram` tokenizer's `max_gram` value limits the character length of
tokens. When the `edge_ngram` tokenizer is used as an index analyzer, this means
search terms longer than the `max_gram` length may not match any indexed terms.
...
token filter>> token filter to truncate search terms to the `max_gram` character
length in a search analyzer. However, this could return irrelevant results.

For example, if the `max_gram` is `3` and search terms are truncated to three
characters, searches for `apple` would return any indexed term beginning with
`app`, including `apply`, `applause`, and `apple`.

As a result, we recommend testing both approaches to see which best fits your
use case.

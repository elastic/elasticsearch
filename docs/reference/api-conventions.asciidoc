...
The `human` flag also affect rendering of the lists of settings. When
human` flag is `false` settings are returned in a flat format:
...
When the `human` flag is `true` settings are returned in a more

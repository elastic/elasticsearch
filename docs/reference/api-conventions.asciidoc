...
---
NOTE: Pay attention on the usage of small vs capital letters used in the `date_format`. For example:
`mm` denotes minute of hour, where `MM` denotes month of year. Similarly `hh` denotes the hour in the
`1-12` range used in combination with the `AM/PM` information, where `HH` denotes the hour in the 24-Hour
format (0-24 range).
----

...
`test1,test2,test3`) to execute a request across multiple data streams, indices,
and index aliases. You can also use 
...
data stream, index, or index alias that matches the pattern.

You can exclude targets using the `-` character (e.g., `test*,-test3`).

Multi-target APIs also support the following query string parameters:
...
Some multi-target APIs also support the following query string parameter:
...
Most APIs that accept a `<data-stream>`, `<index>`, or `<target>` request path
parameter support _date math_.

Date math name resolution lets you dynamically search a range of data streams, indices, or
index aliases that include a time or date component in their name. For indices,
this is a useful alternative to maintaining an index alias or searching all
indices in a cluster and filtering the results.
...
A date math target name takes the following form:
...
You must enclose date math name expressions within angle brackets, and

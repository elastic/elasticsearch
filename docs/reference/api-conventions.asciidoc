...
By default, wildcard expressions do not match hidden data streams and indices.
To match hidden data streams and indices using a wildcard expression, use the
`expand_wildcards` query parameter.

You can hide data streams by setting <<data-stream-hidden,`data_stream.hidden`>>
to `true` in the stream's matching index template. You can hide indices using
the <<index-hidden,`index.hidden`>> index setting.

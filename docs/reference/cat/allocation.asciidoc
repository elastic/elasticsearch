...
`forecast.write_load`, `fwl`, `forecastWriteLoad`::
Sum of index write load forecasts

`forecast.shard_size`, `fd`, `forecastShardSize`::
...
[source,txt]
--------------------------------------------------
shards shards.undesired forecast.write_load forecast.shard_size disk.indices disk.used disk.avail disk.total disk.percent host      ip        node    node.role

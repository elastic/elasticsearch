...
`d` (data node), `i` (ingest node), `l` (machine learning node), v` (voting-only node), and `-` (coordinating node only).
+
For example, `mdi` indicates a master-eligible data and ingest node. See
<<modules-node>>.

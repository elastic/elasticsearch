...
`<auto_follow_pattern_name>`::
  (string) Required. The name of the collection of auto-follow patterns.

[[ccr-put-auto-follow-pattern-request-body]]
==== {api-request-body-title}

`remote_cluster`::
  (string) Required. The <<modules-remote-clusters,remote cluster>> containing
  the leader indices to match against.

`leader_index_patterns`::
  (array) Optional. An array of simple index patterns to match against indices
  in the remote cluster specified by the `remote_cluster` field.

`follow_index_pattern`::
  (string) Optional. The name of follower index. The template `{{leader_index}}`

...
`indices[].shards[].fallen_behind_leader_shard`::
  (boolean) a boolean indicating that this shard has fallen behind too far with document based replication
  If `true` is returned then in the background file based replication will get the follower shard back on
  track and after that document based replication will resume. If `false` then document based replication
  is keeping the follower shard in sync as expected.

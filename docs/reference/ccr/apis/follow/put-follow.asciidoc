...
[source,js]
--------------------------------------------------
PUT /<follower_index>/_ccr/follow?wait_for_active_shards=one
{
...
The `wait_for_restore` parameter specifies whether or not the request should return immediately after following
initialization (default) or wait for the follower to be restored from the remote leader index. Restoring from a
remote index requires transferring all the remote lucene segment files to the follower index.

...
* <<ccr-gs-leader-index,Create a leader index>> in a remote cluster
* <<ccr-gs-follower-index,Create a follower index>> that replicates a leader
  index
* <<ccr-gs-auto-follow,Automatically create follower indices>>
...
. Obtain a license that includes the {ccr} features. See https://www.elastic.co/subscriptions 
  and <<license-management>>.

. If the Elastic {security-features} are enabled in your local and remote 
clusters, you need a user that has appropriate authority to perform the steps in 
this tutorial.
+
--
[[ccr-gs-security]]
...
Alternatively, you can assign the appropriate privileges
to a user ID of your choice. On the remote cluster that contains the leader index, 
...
On the local cluster that contains the follower index, the same user will need the
...
{ref}/modules-remote-clusters.html[connect your local cluster to a remote cluster].
In this tutorial, we will connect our local cluster
...
<<ccr-gs-remote-cluster,remote cluster>> and the 
<<ccr-gs-leader-index,leader index>> that you created in the remote cluster.
...
that you connected your local cluster to. You must also specify a collection of 
patterns that match the indices you want to automatically follow. 

...
==== Prerequisites
* <<managing-licenses,Update your {es} license>> or start a trial license
to enable {ccr}.
* Add {es} logs. From the {kib} home page, choose *Add log data* and choose
the *{es} logs*. Follow the instructions to install and start {filebeat}.

[discrete]
===== Configure user privileges
By default, you can complete the following steps as the built-in
`elastic` user. However, you must
<<get-started-built-in-users,set a password>> for this user before
proceeding.
...
<<modules-remote-clusters,remote cluster>> to begin using {ccr}.

...
After the follower index is created, the <<remote-recovery, remote recovery>>
...
<<ccr-requirements,Soft deletes>> must be enabled for indices that you want to
use as leader indices. Soft deletes are enabled by default on new indices
created on or after {es} 7.0.0.
...
<<remote-recovery, remote recovery>> process completes. To wait for the process

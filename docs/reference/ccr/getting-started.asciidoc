...
+
From the {kib} home page, click the link to *Add sample data*.
...
To configure a remote cluster, access {kib} and go to
...
specify `127.0.0.1:9300` for a remote cluster running locally.
...
To create follower indices from an auto-follow pattern, access {kib} on your
remote cluster and go to *Management > Stack Management*. In the side
navigation, select *Cross Cluster Replication* and choose the *Auto-follow patterns* tab.

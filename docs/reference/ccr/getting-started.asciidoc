...
* Create an auto-follow pattern to automatically follow timestamped indices
...
remote cluster, or configure auto-follow patterns to replicate rolling timestamped indices.
...
==== Create an auto-follow pattern to replicate timestamped indices
You use <<ccr-auto-follow,auto-follow patterns>> to automatically create new
followers for rolling timestamped indices. Whenever the name of a new index on
...
and one or more index patterns that specify the rolling timestamped indices you
want to replicate.

...
The versions for the cluster, and any replicated indices must be compatible.

[discrete]
[[ccr-cluster-compatibility]]
=== {ccr-init} cluster version compatibility
...
.{ccr-init} uni-directional cluster version compatibility matrix
====
include::../modules/ccr-uni-directional-compatibility.asciidoc[tag=ccr-uni-directional-version-compatibility-matrix]
====

In a bi-directional configuration, the two clusters must remain on the same version.

[discrete]
[[ccr-index-compatibility]]
=== {ccr-init} index compatibility

All indices replicated by cross-cluster replication must also be compatible 
with the cluster's version. If you try to replicate an index created in an
incompatible version, the replication attempt will fail.

[%collapsible%open]
[[ccr-index-compatibility-matrix]]
.CCR index version compatibility matrix
====
include::../snapshot-restore/cluster-index-compat.asciidoc[]
====

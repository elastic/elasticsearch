...
IMPORTANT: In all of these cases <<manually-configure-security, Security is to be configured>> independently for each cluster.
In particular, no disaster recovery arrangement described bellow handles the replication of Security configuration,
such as <<native-realm, native users>>, roles, or <<token-authentication-services, token credentials>>.
Security configuration should be backed up and restored using the <<back-up-specific-feature-state, regular snapshot and restore procedure>>

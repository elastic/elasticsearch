...
primary shard that has reached max allocation retry attempts. 
...
<<cluster-reroute,Cluster Reroute>> to retry allocation. 
...
This is a transient message which may appear while a large amount of shards are allocating. 
...
for an arbitrary unassigned primary or replica shard, with preference to primary. 

...
`transport`::
Information about the transport interface of this node.
====

If you use the full `GET /_nodes/<node_id>/<metric>` form of this API then you
can also use `_all` to explicitly request all metrics, and `_none` to suppress
all metrics and return only the identity of the node.

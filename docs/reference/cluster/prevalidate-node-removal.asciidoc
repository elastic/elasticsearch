...
====
`<node>`::
    (object) Contains information about removal prevalidation of a specific node.
+
.Properties of `<node>`
[%collapsible%open]
=======
...
`external_id`::
    (string)
`result`::
    (object) Contains removal prevalidation results of the node.
+

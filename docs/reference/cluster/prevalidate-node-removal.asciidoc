...
+
--
    * `non_red_cluster_status`: Cluster state is not red and therefore the node could be removed safely.
    * `no_red_shards_on_node`: The node does not contain any copies of the red shards.
    * `red_indices_are_searchable_snapshot_based`: The node can be safely removed as all red indices are searchable snapshot indices and therefore removing a node does not risk removing the last copy of that index from the cluster.
    * `error_contacting_node`: Contacting the node failed or timed out. More details is provided in the `message` field.
    * `may_contain_red_shard_copy`: The node might contain shard copies of some the red indices. The list of the shards that might be on the node are specified in the `message` field.
--
`message`::
...
--------------------------------------------------
{
  ...
  "nodes": [
    ...
    },
    {
      ...
      "result" : {
        "is_safe": false,
        "reason": "may_contain_red_shard_copy",

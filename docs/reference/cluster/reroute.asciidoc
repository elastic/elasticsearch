...
Similar but different than the `dry_run` flag is the `explain` parameter, which
returns an explanation for why the commands can or cannot be applied without

...
* the index metadata, including the mapping and settings for each index

* the location and status of every shard copy in the cluster

The elected master ensures that every node in the cluster has a copy of the
same cluster state. The cluster state API lets you retrieve a representation of
this internal state for debugging or diagnostic purposes. You may need to
consult the {es} source code to determine the precise meaning of the response.
...
If you use this API repeatedly then your cluster may become unstable.

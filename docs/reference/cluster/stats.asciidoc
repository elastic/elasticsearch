...
that acted as coordinator during the search execution.
Usage includes information about the total number of incoming search requests,
which sections were specified in their request body, as well as which queries
were used as part of the query and post_filter sections. Queries are counted
once per search request, meaning that if the same search requests includes
multiple term queries, the counter for term is going to be incremented by 1 only.
...
`total`::
(integer)
Total number of received search requests

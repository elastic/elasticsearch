...
`synonyms`::
(object)
Contains statistics about inline synonyms defined as `synonyms` in <<analysis-synonym-tokenfilter,synonym>> and <<analysis-synonym-graph-tokenfilter,synonym graph>> token filters configuration.
...
Two synonyms configuration with the same synonyms will count as separate ocurrences.
...
`count`::
(integer)
Occurrences of different synonyms path in selected nodes.
Synonyms path that point to the same file will not be counted separately.
...
`count`::
(integer)
Occurrences of different synonyms sets in selected nodes.
Synonyms sets that point to the same set will not be counted separately.

...
`synonyms_sets`::
(array of objects)
Contains statistics about the synonyms sets used in
selected nodes.
+
...
`name`::
(string)
Synonym set used in selected nodes.

`count`::
(integer)
Occurrences of the synonym set in selected nodes.

`index_count`::
(integer)
Number of indices using the synonym set in selected nodes.
======
=====

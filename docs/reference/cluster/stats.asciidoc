...
`queries`::
(array of objects)
Contains statistics about queries submitted to selected nodes,
when the selected nodes served as coordinating nodes during
search requests.
Queries are counted only by a coordinating node, and deduplicated per
a search request.

+
.Properties of `queries` objects
[%collapsible%open]
======
`name`::
(string)
Query type used in selected nodes.

`count`::
(integer)
Number of times this query type was used in selected nodes.
=====

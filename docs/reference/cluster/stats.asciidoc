...
`snapshot_deletions`:::
(integer)
The total number of snapshot deletion operations that the cluster is currently running.

`concurrent_operations`:::
(integer)
The total number of snapshot operations that the cluster is currently running concurrently. This is the total of the `snapshots` and `snapshot_deletions` entries, and
is limited by <<snapshot-max-concurrent-ops,the `snapshot.max_concurrent_operations` setting>>.

`cleanups`:::
(integer)
The total number of repository cleanup operations that the cluster is currently running. These operations do not count towards the total
...
`repositories`:::
(object)
Contains statistics which report the progress of snapshot activities broken down by repository. This object contains one entry for each repository registered with the cluster.
+
...
`current_counts`:::
(object)
Contains statistics which report the numbers of various ongoing snapshot activities for this repository.
+
.Properties of `current_counts`
[%collapsible%open]
=======
`snapshots`:::
(integer)
The total number of snapshots currently running in this repository.

`clones`:::
(integer)
The total number of snapshot clones currently running in this repository.

`finalizations`:::
(integer)
The total number of snapshots and clone operations which are mostly complete except for the last "finalization" step.

`deletions`:::
(integer)
The total number of snapshot deletion operations currently in this repository.

`snapshot_deletions`:::
(integer)
The total number of snapshots that are currently being deleted from this repository.

`active_deletions`:::
(integer)
The total number of snapshot deletion operations currently active in this repository.

`shards`:::
(object)
Contains statistics which report the shard-level progress of snapshot activities for a repository.
+
.Properties of `shards`
[%collapsible%open]
========
...
`total`:::
(integer)
The total number of shard snapshots currently tracked by this repository.

`complete`:::
(integer)
The total number of tracked shard snapshots which have completed in this repository.

`incomplete`:::
(integer)
The total number of tracked shard snapshots which have not completed in this repository. This is the difference between the
`total` and `complete` values.

`states`:::
(object)
The total number of shard snapshots in each of the named states in this repository.
...
(integer)
The start time of the oldest running snapshot in this repository, represented as milliseconds since the Unix epoch.

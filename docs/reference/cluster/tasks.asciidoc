...
costly to execute. For example, fetching a search using the `detailed` request
parameter:

[source,js]
--------------------------------------------------
GET _tasks/oTUltX4IQMOUUVeiohTt8A:464?detailed
--------------------------------------------------
// CONSOLE
// TEST[catch:missing]
...
The new `description` field contains a description of the task. In the above
example (a search) the `description` is a normalized copy of the search request
body so it can get quite large but should be useful for telling individual
searches from one another. Other tasks will have different descriptions, like
<<docs-reindex,`_reindex`>> which has the search and the destination, or
<<docs-bulk,`_bulk`>> which just has the number of requests and the destination
indices. Many requests will only have an empty description because we haven't
thought of good description. We expect to add and change these `description`
values as they seem more use and we learn more.

...
deprecated[8.0, "The `elasticsearch-setup-passwords` tool is deprecated and will be removed in a future release. To manually reset the password for the `elastic` user, use the <<reset-password,`elasticsearch-reset-password` tool>>. To change passwords for other users, use either {kib} or the {es} change passwords API."]

...
deprecated[8.0, "The `elasticsearch-setup-passwords` tool is deprecated and will be removed in a future release. To manually reset the password for the built-in users (including the `elastic` user), use the xref:reset-password.asciidoc[`elasticsearch-reset-password`] tool, the {es} change password API, or the User Management features in {kib}."]

...
Claims a connector sync job. This action updates the job's status to `in_progress` and sets the `last_seen` and `started_at` timestamps to the current time. Additionally, if a sync_cursor property is present in the corresponding connector state, it is also updated.

...
`last_seen` can also be update using this API.
...
(Required, int) The document count the sync job deleted.

`indexed_document_count`::
(Required, int) The document count the sync job indexed.

`indexed_document_volume`::
(Required, int) The document volume the sync job indexed.

`total_document_count`::
(Optional, int) The total document count in the target index after the sync job finished.

`last_seen`::
(Optional, instant) The timestamp to set `last_seen` of the connector sync job to.

...
Learn how to set up a self-managed self-managed connector using the {ref}/connector-apis.html[{es} Connector APIs].
...
[NOTE]
====
The connector service runs automatically in Elastic Cloud, if you're using our managed managed connectors.
...
For managed connectors, this occurs shortly after creation via the API.
...
To use these APIs with a managed managed connector, there's some extra setup for API keys.

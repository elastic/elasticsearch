...
To use this connector as a *native connector*, see <<es-native-connectors>>.
...
[TIP]
====
When using the <<es-build-connector, connector client>> workflow, initially these fields will use the default configuration set in the {connectors-python}/connectors/sources/azure_blob_storage.py[connector source code^].

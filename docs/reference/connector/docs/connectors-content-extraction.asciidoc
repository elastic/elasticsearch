...
While intended primarily for PDF and Microsoft Office formats, you can use any of the <<connectors-content-extraction-supported-file-types, supported formats>>.
...
For advanced use cases, the <<connectors-content-extraction-local, self-hosted extraction service>> can be used to extract content from files larger than 10MB.

[discrete#connectors-content-extraction-supported-file-types]
...
[discrete#connectors-content-extraction-local]
== Extraction Service

[NOTE]
====
Currently, content extraction from large files via the Extraction Service is available for a subset of our **self-managed connector clients**.
...
The connector client limits file sizes for pipeline extraction to 10MB per file (Elasticsearch also has a hard limit of 100MB per file).

For use cases that require extracting content from files larger than these limits, the *self-managed extraction service* can be used for connector clients.
...
* <<connectors-content-extraction-data-extraction-service,Run the self-hosted content extraction service>>
* <<connectors-extraction-service-configuration,Add the required configuration settings>>
...
[discrete#connectors-content-extraction-available-connectors]
=== Available connectors

Local content extraction is available for the following self-managed connector clients:

include::_connectors-list-local-content-extraction.asciidoc[]

[discrete#connectors-content-extraction-data-extraction-service]
...
[discrete#connectors-extraction-service-configuration]
=== Configuring the extraction service

You can enable your connector client to use the self-hosted extraction service by adding the required configuration.
The connector client determines if the extraction service is enabled by the presence of these fields in the configuration file.
...
[NOTE]
====
There is no password protection between the connector client and the extraction service.
...
|=======
|Field|Description
|`host`|The endpoint for the extraction service. `http://localhost:8090` can be used if it is running on the same server as your connector client.
|=======

The connector client will perform a preflight check against the configured `host` value at startup.
...
If you don't see this log at startup, refer to <<connectors-content-extraction-troubleshooting, troubleshooting self-hosted content extraction service>>.

[discrete#connectors-content-extraction-advanced-configuration]
...
|`use_file_pointers`|Whether or not to use file pointers instead of sending files to the extraction service. Defaults to `false`. Refer to <<connectors-content-extraction-data-extraction-service-file-pointers, using file pointers>> for more details about this setting.
...
[discrete#connectors-content-extraction-data-extraction-service-file-pointers]
=== Using file pointers
...
This can be set up with both a dockerized and non-dockerized connector client.

[discrete#connectors-content-extraction-data-extraction-service-file-pointers-configuration]
==== Configuration for non-dockerized connector clients

If you are running a non-dockerized version of the connector client, you need to determine the local directory where you'll download files for extraction.
...
[discrete#connectors-content-extraction-data-extraction-service-file-pointers-configuration-example]
===== Example
...
If either directory is different, the connector client will be unable to provide an accurate file pointer for the extraction service. This is not a factor when using a dockerized connector client.
====
+
2. Next, before running the connector client, be sure to update the config file with the correct information.
+
[source,yaml]
----
# data-extraction-service settings
...
----
+
3. Then all that's left is to start the connector client and run a sync.
If you encounter any unexpected errors, refer to <<connectors-content-extraction-troubleshooting, troubleshooting the self-hosted content extraction service>>.

[discrete#connectors-content-extraction-data-extraction-service-file-pointers-configuration-dockerized]
==== Configuration for dockerized connector clients

When using self-hosted extraction from a dockerized connector client, there are a few extra steps required on top of {connectors-python}/docs/DOCKER.md[running the connector client in docker^].
...
[discrete#connectors-content-extraction-data-extraction-service-file-pointers-configuration-dockerized-example]
===== Example
...
----
+
4. Next, you can follow the instructions for {connectors-python}/docs/DOCKER.md[running the connector client in docker^] until step `4. Update the configuration file for your self-hosted connector`.
...
----
+
6. Now the connector client and extraction service docker containers should be set up to share files.
Run a test sync to make sure everything is configured correctly.
If you encounter any unexpected errors, refer to <<connectors-content-extraction-troubleshooting, troubleshooting the self-hosted extraction service>>.

[discrete#connectors-content-extraction-local-logs]
...
[discrete#connectors-content-extraction-troubleshooting]
...
You can find more information about what happened in the <<connectors-content-extraction-local-logs, data extraction service logs>>.
...
* A server connection error is an internal error on the extraction service. You will need to investigate the <<connectors-content-extraction-local-logs, data extraction service logs>>.

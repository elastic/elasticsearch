...
[NOTE]
====
Currently, content extraction from large files via the Extraction Service is available for a subset of our **self-managed self-managed connectors**.
It is not available for managed connectors running on Elastic Cloud.
...
Local content extraction is available for the following self-managed self-managed connectors:
...
----
+
4. Next, you can follow the instructions for {connectors-python}/docs/DOCKER.md[running the self-managed connector in docker^] until step `4. Update the configuration file for your self-hosted connector`.

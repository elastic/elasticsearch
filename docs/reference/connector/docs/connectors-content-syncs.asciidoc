[#connectors-sync-types]
= Content syncs

Elastic connectors have two types of content syncs:

* <<connectors-sync-types-full>>
* <<connectors-sync-types-incremental>>

[discrete#connectors-sync-types-full]
== Full syncs

[NOTE]
====
We recommend running a full sync whenever <<sync-rules, Sync rules>> are modified
====
...
You can <<connectors-usage-syncs-recurring, schedule>> or <<connectors-usage-syncs-manual, manually trigger>> a full sync job.

[discrete#connectors-sync-types-incremental]
== Incremental syncs
...
You can <<connectors-usage-syncs-recurring, schedule>> or <<connectors-usage-syncs-manual, manually trigger>> an incremental sync job.

[discrete#connectors-sync-types-incremental-performance]
...
[NOTE]
====
Incremental syncs for the <<connectors-sharepoint-online, SharePoint Online>> connector use specific logic.
All other connectors use the same shared connector framework logic for incremental syncs.
====

[discrete#connectors-sync-types-incremental-supported]

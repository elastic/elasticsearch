...
To use this connector, satisfy all <<es-build-connector-prerequisites,connector client requirements>>.
...
[TIP]
====
When using the <<es-build-connector, connector client>> workflow, initially these fields will use the default configuration set in the {connectors-python}/connectors/sources/confluence.py[connector source code^].

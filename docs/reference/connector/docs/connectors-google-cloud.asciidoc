...
To use this connector in Elastic Cloud, satisfy all <<es-native-connectors,native connector requirements>>.
...
[TIP]
====
When using the <<es-build-connector, connector client>> workflow, initially these fields will use the default configuration set in the {connectors-python}/connectors/sources/google_cloud_storage.py[connector source code^].

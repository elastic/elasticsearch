...
This connector is available as a self-managed *connector client*. To use this connector, satisfy all <<es-build-connector,connector client requirements>>.
...
[TIP]
====
When using the <<es-build-connector, connector client>> workflow, initially these fields will use the default configuration set in the {connectors-python}/connectors/sources/google_drive.py[connector source code^].

...
As of *8.10.0* self-managed self-managed connectors no longer require the Enterprise Search service to be running on your Elastic deployment.
...
+
** *Use a self-managed self-managed connector* instead of a managed connector:
...
They are still available as self-managed self-managed connectors.
** Azure Blob Storage

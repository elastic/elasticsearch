...
To use this connector as a *connector client*, see <<es-build-connector>>For additional usage operations, see <<es-connectors-usage>>.
...
[TIP]
====
When using the <<es-build-connector, connector client>> workflow, initially these fields will use the default configuration set in the {connectors-python}/connectors/sources/network_drive.py[connector source code^].

...
To use this connector, satisfy all <<es-native-connectors, native connector requirements>>.
...
[TIP]
====
When using the <<es-build-connector, connector client>> workflow, these fields will use the default configuration set in the https://github.com/elastic/connectors/blob/{branch}/connectors/sources/generic_database.py[connector source code^].

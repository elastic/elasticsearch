...
Indexes mail from all mailbox folders, not only Inbox, Sent, Junk, and Archive.
Other folders are stored as `Mail` with `folder_name`; the four default folders keep their existing types.

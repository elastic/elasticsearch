...
Sync all mail folders::
Indexes mail from the user mail folders in each mailbox, not only Inbox, Sent, Junk, and Archive.
Other folders are stored as `Mail` with a `folder_name` field.
The four default folders keep their existing types.
System folders such as Deleted Items, Drafts, Outbox, and search folders are never indexed.
Enabling this setting increases sync time, load on Exchange, and index size.
Default value is `false`.
Introduced in 8.19.23.
...
** *Archive Mails*
** *Junk Mails*
** *Mail* (other user mail folders, when **Sync all mail folders** is enabled. Includes a `folder_name` field)
* *Contacts*
* *Calendar Events*
* *Tasks*
* *Attachments*

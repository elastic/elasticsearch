...
This document describes security considerations for <<native-connectors,native connectors>> and <<build-connector,connector clients>>.
...
[discrete#native-connectors-security-connections]
...
[discrete#native-connectors-security-api-key]
== Access to internally stored API keys

API keys for native connectors are stored in the internal system index `.connector-secrets`.
...
[discrete#native-connectors-security-dls]
...
Learn more about this feature in <<dls>>, including availability and prerequisites.

[discrete#native-connectors-security-deployment]
== Access to documents
...
[discrete#native-connectors-security-encryption]
== Encryption

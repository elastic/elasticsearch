...
You'll be using the <<es-build-connector, connector client>> workflow in the Kibana UI.
...
Refer to the list of available <<es-build-connector,connector clients>>.
...
First, ensure you satisfy the <<es-build-connector-prerequisites, prerequisites>> for self-managed connector clients.
...
* <<es-build-connector, Overview of connector clients and frameworks>>
...
* <<es-native-connectors, Native connectors available in Elastic Cloud>>
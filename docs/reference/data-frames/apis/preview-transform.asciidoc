...
The preview {dataframe-transforms} API returns maximum 100 results of the 
transform that is specified in the request body. The calculations are as 
accurate as if you would use the create {dataframe-transforms} API and they are 
based on all the current data in the source index. The preview shows the results 
that you get using the create {dataframe-transforms} API with the same 
configuration.

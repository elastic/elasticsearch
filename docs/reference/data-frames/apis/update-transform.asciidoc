...
This API updates an existing {dataframe-transform}. Updates are currently only supported
on continuous {dataframe-transforms}. Additionally, all settings are not applied until
after the {dataframe-transform} has been stopped and started again.
...
[source,js]
--------------------------------------------------
POST _data_frame/transforms/ecommerce_transform/_update
{
  "source": {
...
--------------------------------------------------
// CONSOLE
// TEST[setup:simple-kibana-continuous-pivot]

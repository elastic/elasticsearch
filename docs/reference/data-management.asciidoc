...
* Define the retention period of your data via <<index-lifecycle-management, {ilm}>> ({ilm-init}) or via
<<data-lifecycle-management, {dlm}>> ({dlm-init}), _note {dlm-init} is in_ preview:[]. The retention period is
the minimum time your data will be stored in {es}. Data older than this period can be deleted by {es}.

...
To help you manage your data, {es} offers you <<index-lifecycle-management, {ilm}>> ({ilm-init}) and
<<data-lifecycle-management, {dlm}>> ({dlm-init}).

preview::["DLM is in technical preview and may be changed or removed in a future release. Elastic will apply best effort to fix any issues, but DLM is not subject to the support SLA of official GA features."]
...
{dlm-init} can be used to manage data streams only, it is less feature rich but is focused on simplicity, so it allows
you to easily:

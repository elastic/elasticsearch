...
== Change mappings and settings for backing indices
++++
...
The following <<indices-templates,put composable template API>> makes the
several changes to the `logs_data_stream` template:
...
write index. This means any new data added to the data stream use the new
...
To apply mapping or setting changes to every existing backing index in a data
stream as well as any future ones, follow these steps:

. Choose a name or wildcard (`*`) pattern for a new data stream. This new data
stream will contain data from your existing stream.
+
You can use the resolve index API to check if the name or pattern matches an
...
. Create a new composable template that contains the mappings and settings you'd
like to apply to the new data stream's backing indices.
+
...
+
TIP: If you are only adding or changing a few things, we recommend you duplicate
an existing template and modify it as needed.
+
.*Example*
[%collapsible]
====
...
<1>  Changes the `@timestamp` field mapping the `date_nanos` field datatype.
<2>  Adds the `sort.field` index setting.
<3>  Adds the `sort.order` index setting.
====
...
timestamp from last week. Note the request's `op_type` is `create`.

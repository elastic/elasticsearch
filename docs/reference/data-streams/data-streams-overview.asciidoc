...
* <<docs-index_,Index API>> requests with an
  <<docs-index-api-op_type,`op_type`>> of `index`. `op_type` defaults to
  `index` for existing documents.

* <<docs-bulk,Bulk API>> requests using the `delete`, `index`, or `update`
  action.

* <<docs-delete,Delete API>> requests

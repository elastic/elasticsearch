...
requests. Data streams are often used to store and search logs, events, metrics,
and other continuously generated data.
...
mapped as a <<date,`date`>> or <<date_nanos,`date_nanos`>>. If the index
template does not contain a mapping for the `@timestamp` field, {es} maps
...
The stream adds new documents only to this index.
...
roll over data streams. If needed, you can also
...
Name changes do not remove the index from the data stream.
...
if ever, updated. You cannot send update or deletion requests for existing documents
directly to a data stream. Instead, use the

...
<<ilm-downsample-options,`fixed_interval`>> to the frequency at which you want
to aggregate the original time series data.

In this example, an ILM policy is configired for the `hot` data tier. A
downsample process runs after the first document is inserted into the index, and
then repeats hourly.

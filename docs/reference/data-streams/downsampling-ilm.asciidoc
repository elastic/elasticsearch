...
meet policy criteria. For this example, update the
`indices.lifecycle.poll_interval` setting to one minute.
...
Wait for five minutes or so (maybe brew up a quick coffee or tea &#9749; ) and then re-run the `GET _data_stream` request.

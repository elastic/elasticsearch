...
you're not using ILM, you can downsample a TSDS manually. This guide show you
...
You can now delete the old backing index:

[source,console]
----
DELETE .ds-my-data-stream-2023.07.26-000001
----
// TEST[skip:We don't know the index name at test time]
...
The TSDS with the new downsampled backing index contains just one document that
includes the `min`, `max`, `sum`, and `value_count` statistics based off of the
original sampled metrics:

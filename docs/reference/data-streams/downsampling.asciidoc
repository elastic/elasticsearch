...
values generated at UTC. This works for timezones that are on the hour, e.g.
+5:00 or -3:00, but can be inaccurate otherwise, e.g. +5:30 for India, if
downsampling aggregates values per hour. To get accurate results in this case,
it's recommended to use a downsampling interval of 15 minutes.

** Date histograms at intervals that are multiples of a day are similarly
affected, in case downsampling aggregates values per day: the beginning of
each day is always calculated at UTC so reported daily values will be
inaccurate at different timezones. To minimize this effect, it's recommended
to use a downsampling interval of 1 hour.

...
* *Perform a failure store rollover.* Consider <<failure-store-rollover,rolling over the failure store>> before attempting to remediate failures. This will create a new failure index that will collect any new failures during the remediation process.

...
`downsampling`::
(Optional, array)
An optional array of downsampling configuration objects, each definig an `after`
...
an `fixed_interval` representing the downsampling interval (the minimum `fixed_interval`

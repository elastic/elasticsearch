...
{es} is allowed at a later time to delete data older than this time period. Retention can be configured on data stream level
or on a project level, read more about the different options in this <<tutorial-manage-data-stream-retention,tutorial>>.

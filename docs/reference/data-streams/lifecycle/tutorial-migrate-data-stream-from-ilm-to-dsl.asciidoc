...
In this tutorial we'll look at migrating an existing data stream from Index Lifecycle Management ({ilm-cap}) to 
data stream lifecycle. The existing {ilm-cap} managed backing indices will continue 
to be managed by {ilm-cap} until they age out and get deleted by {ilm-cap}; however,
...
Inspecting the response we'll see that both backing indices are managed by {ilm-cap}
and that the next generation index will also be managed by {ilm-cap}:
...
configuration which will indicate if {ilm-cap} takes precedence over data stream lifecycle in case
both systems are configured for an index.
<3> The {ilm-cap} policy configured for this index.
...
default value ({ilm-cap} takes precedence over data stream lifecycle by default as it's
currently richer in features).
<7> The {ilm-cap} policy configured in the index template that's backing this data
...
(created by rolling over the data stream) such that {ilm-cap} does _not_ take 
...
<1> The existing backing index will continue to be managed by {ilm-cap}
<2> The existing backing index will continue to be managed by {ilm-cap}
...
<1> The backing indices that existed before rollover will continue to be managed by {ilm-cap}
<2> The backing indices that existed before rollover will continue to be managed by {ilm-cap}

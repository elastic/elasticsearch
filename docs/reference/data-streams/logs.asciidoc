...
Before using synthetic source, make sure to review the <<synthetic-source,restrictions and modifications>>. To prevent modifications of a particular object or field, you can <<synthetic-source-keep,minimize synthetic source modifications>>.
...
and change the order, manually configure `index.sort.field` and `index.sort.order`.
...
create a new index. For data streams, you can update the templates used by the data stream, and then 

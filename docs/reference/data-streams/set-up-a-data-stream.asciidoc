...
* Optionally define a <<date,`date`>> or <<date_nanos,`date_nanos`>> field mapping for
the timestamp field specified in the `timestamp_field` property. By default a <<date, `date`>>
mapping is added for the `@timestamp` field, but this will be overwritten by any user defined
mapping for the `@timestamp` field.
+

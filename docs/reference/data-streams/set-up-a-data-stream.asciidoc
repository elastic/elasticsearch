...
{ilm-init} uses an index lifecycle policy to automate the management of your
data stream's backing indices.

To create an index lifecycle policy in {kib}, open the menu and go to *Stack
...
A data stream requires a matching index template. You can compose this template
from one or more component templates. You typically use separate component
templates for mappings and index settings.
...
To create a component template in {kib}, open the menu and go to *Stack
...
using {fleet-guide}/data-streams.html#data-streams-naming-scheme[our data
...
To create an index template in {kib}, open the menu and go to *Stack Management
> Index Management*. In the *Index Templates* view, click *Create template*.

...
template. You must configure `index.routing_path` directly in the index
template. You'll do this in the following step.
...
plain `keyword` dimension fields. You must configure this setting directly in
the index template.
...
that matches the `index.routing_path` index setting.

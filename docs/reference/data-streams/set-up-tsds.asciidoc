...
Configuring a TSDS via an index templates that uses component templates is a bit more complicated.
With component templates typically mappings and settings get scattered across multiple component templates.
When configuring the `index.mode` setting in a component template, then the index.routing_path setting needs to
be defined as well in the same component template. Additionally the fields mentioned in the `index.routing_path`
...
requires the `index.routing_path` setting. When configuring the  `index.mode` setting in an index template, then
the `index.routing_path` setting is configured automatically. The `index.routing_path` setting is then derived  from

...
time series. In most cases, a dimension describes some aspect of the entity
you're measuring. For example, documents related to the same weather sensor may
...
.Dimension limits
****
In a TSDS, {es} uses dimensions to generate the document `_id` and
<<tsid,`_tsid`>> values. To prevent overly large values, {es} limits the number
of dimensions for an index using the
<<index-mapping-dimension-fields-limit,`index.mapping.dimension_fields.limit`>>
index setting. While you can increase this limit, the resulting document `_id`
and `_tsid` values can't exceed 32KB.
****
...
Metrics are fields that contain numeric measurements. While not required,
documents in a TSDS typically contain one or more metric fields.

A field can't be both a dimension and a metric. Unlike dimensions, the metric
values for a time series typically change over time, even if rarely or slowly. 
...
When you add a document to a TSDS, {es} automatically generates a hidden `_tsid`
...
if the index isn't the current write index.
...
{es} uses the <<index-look-ahead-time,`index.look_ahead_time`>> index setting to
configure the `@timestamp` ranges for a TSDS's write index. When you create a
...
The `index.routing_path` setting accepts wildcard (`*`) patterns and can
dynamically match new fields. However, {es} will reject any mapping updates that
add scripted, runtime, or non-`keyword` fields that match the

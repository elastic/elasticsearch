...
Once data is no longer or rarely queried, it may move from the cold tier to the
...
{search-snaps} uses only <<shared-cache,smaller local caches>> of recently
searched data. While less responsive than the cold tier, these cache
signficantly reduce local storage costs.
...
{ess} requires additional configuration to use the frozen tier
functionality.

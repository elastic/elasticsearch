...
The frozen tier uses <<searchable-snapshots,partially mounted indices>> to store
and load data from a snapshot repository. When an index transitions into the
<<ilm-index-lifecycle,frozen phase>>, {ilm-init} automatically converts it to a
partially mounted index on the frozen tier.

Partially mounted indices let you search over large data sets with fewer compute
resources, reducing both your storage and operating costs. Because {es} must
sometimes fetch data from the snapshot repository, searches on the frozen tier

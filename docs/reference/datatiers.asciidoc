...
* <<frozen-tier, Frozen tier>> nodes hold time series data that is accessed very seldom and never updated.
...
stays while being seldomly queried.
...
Once data is no longer being queried, or being quered very seldomly, it may move from the cold tier

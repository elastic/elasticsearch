...
Once data is rarely or no longer queried, it may move from the cold tier to the
frozen tier where it stays for the rest of its life.

The frozen tier uses <<searchable-snapshots,{search-snaps}>> to store and load
data from a snapshot repository. Instead of using a full local copy, these
{search-snaps} use smaller <<shared-cache,local caches>> containing only
recently searched data. If a search requires data that's not in a cache, {es}
lazily fetches the data as needed from the snapshot repository. This decouples
compute and storage, letting you run searches with minimal compute resources.

While search is slower on the frozen tier than the cold tier, the frozen tier's
snapshot caches lets you significantly reduce storage and operating costs.

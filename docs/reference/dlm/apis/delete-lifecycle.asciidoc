...
`<data-stream>`::
(Required, string) Comma-separated list of data streams used to limit the request. Supports wildcards but it is not
possible to target all data streams at once with `*`.

...
data stream)
<7> The age of the index with respect to its lifecycle (an index ages based on its creation
date until it's rolled over, at which point it starts aging since the rollover date)

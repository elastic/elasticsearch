...
The goal of {dlm} ({dlm-init}) is to enable you to automatically manage data streams
...
* Older data will be deleted by {es} at a later time.
...
. it will try to rollover the write backing index of the data stream, given that the backing index is
also managed by {dlm-init},
. it will apply retention to the remaining backing indices that are managed by {dlm-init}. This means that it will delete
the backing indices that have completed the retention period after they have been rolled over. The time after rollover is
called `generation_time` and it represents that all the data in this index are at least older than this time.

NOTE: A backing index of a data stream managed by {dlm-init} is also managed by {dlm-init}, if it doesn't have an
{ilm-init} policy defined, or if it has <<index-dlm-prefer-ilm, `index.lifecycle.prefer_ilm`>> set to `false`.

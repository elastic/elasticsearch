...
You can use the <<dlm-get-lifecycle,get data stream lifecycle API>> to see the data lifecycle of your data stream and
the <<dlm-explain-lifecycle,explain data lifecycle>> to see the exact state of each backing index.
...
If you want to see more information about how the individual backing indices are managed by {dlm-init} use the
<<dlm-explain-lifecycle,explain data lifecycle>> API:
...
<2> If it is managed by {dlm-init}.

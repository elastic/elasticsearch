...
--------------------------------------------------
// TESTRESPONSE[s/"took": .../"took": $body.took/ s/"index_uuid": .../"index_uuid": $body.items.3.update.error.index_uuid/]
<1> Some operation(s) in the _bulk request failed
<2> The first operation succeeded with 201 status
<3> The delete operation failed because the targeted document does not exist
<4> The create operation succeeded with 201 status
<5> The update operation failed because the targeted document is missing (create and update the same document in a single _bulk request does not work)

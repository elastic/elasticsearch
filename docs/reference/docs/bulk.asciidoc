...
{ "field1" : "value1" }
$ curl -s -H "Content-Type: application/x-ndjson" -XPOST localhost:9200/_bulk?include_type_name=false --data-binary "@requests"; echo
...
The endpoints are `/_bulk` and `/{index}/_bulk`.
When the index is provided, it will be used by
default on bulk items that don't provide index names explicitly.

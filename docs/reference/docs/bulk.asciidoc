...
The following bulk API request includes actions that update non-existent
documents.
...
Because these bulk actions cannot complete successfully, the API returns a
response with an `errors` flag of `true`.

The response also includes an `error` object for any failed actions. The
...
To return only information about failed actions, use the

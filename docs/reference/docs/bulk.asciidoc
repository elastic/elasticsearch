...
To disable automatic index creation for a single bulk request set request
parameter `auto_create_index` to `false` or skip this parameter to follow
default behaviour defined by <<index-creation,`action.auto_create_index`>>:

[source,js]
--------------------------------------------------
POST _bulk?auto_create_index=false

...
<<privileges-list-indices,index privileges>> for the target data stream or index:

** `read`
** `delete` or `write`

+
NOTE: If you are using an alias, make sure the privileges are configured at the index level.

...
The realtime get can be disabled for an entire index by setting `index.enable_realtime_get` to
`false`. This setting can not be changed at runtime. If realtime get is disabled
all calls to the get API with the `realtime` flag set to `true` with fail.

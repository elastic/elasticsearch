...
The above result includes the `_index`, `_type`, `_id` and the
`version` of the document we wish to retrieve, including the
actual `_source` of the document if it was found.

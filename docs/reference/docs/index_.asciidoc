...
The default value for the `replication` setting is `sync` and the default can 
be changed from `elasticsearch.yml` file by specifying `action.replication_type`
setting. Valid values for replication type are `sync` and `async`.

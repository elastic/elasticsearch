...
+
// tag::autocreate[]
If the target doesn't exist and matches an index template with
<<create-index-template,data stream enabled>>, the request creates a data
stream. If the target doesn't exist and doesn't match a data stream enabled
template, the request creates an index.
// end::autocreate[]

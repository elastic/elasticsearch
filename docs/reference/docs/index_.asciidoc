...
`if_seq_no`::
(Optional, integer) Only update the document if it has this
sequence number. See <<optimistic-concurrency-control-index>>.

`if_primary_term`::
(Optional, integer) Only update the document if it has
...
`refresh`::
(Optional, enum) If `true` then refresh the affected shards to make this
operation visible to search, if `wait_for` then wait for a refresh to make this operation visible to search, if `false` do nothing with refreshes. Valid values: `true`, `false`, `wait_for`. Default: `false`.

`routing`::
(Optional, string) Index to the specified primary shard. See
<<index-routing>>.
...
(Optional, string) Wait for the document to be

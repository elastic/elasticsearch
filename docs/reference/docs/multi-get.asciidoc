...
By default, the `_source` field will be returned for every document (if stored). Similar to the <<get-source-filtering,get>> API,
you can retrieve only parts of the `_source` (or not at all) by using the `_source` parameter. This can be specified per document
or via the url parameters `_source`,`_source_include` & `_source_exclude`, which will serve as default.

...
** The `read` <<privileges-list-indices,index privilege>> for the source..

** The `write` index privilege for the destination.
...
privilege for the source.

* If reindexing from a remote cluster, you must explicitly allow the remote host
in the `reindex.remote.whitelist` setting of `elasticsearch.yml`. See
<<reindex-from-remote>>.

* Data stream creation requires a matching index template with
<<create-index-template,data stream enabled>>.

[[docs-reindex-api-desc]]
==== {api-description-title}

Extracts the <<mapping-source-field,document source>> from a source data stream,
index, or alias and indexes the documents into a destination. You can copy all
documents or a subset of documents.
...
Reindex does not copy the settings from the source or associated templates.

Mappings, shard counts, replicas, and so on must be configured ahead of time.
====

Like <<docs-update-by-query,`_update_by_query`>>, `_reindex` gets a
...
IMPORTANT: If the destination is a data stream, `op_type` must be `create`.
...
`source`::
`index`:::
(Required, string or array of strings) Comma-separated list of data streams,
indices, and aliases to use as the source. Supports wildcards (`*`). See
<<docs-reindex-from-multiple-sources>>.
...
`dest`::
`index`:::
(Required, string) Data stream, index, or alias to use as the destination. Must
differ from `source.index`.
+
include::{es-ref-dir}/docs/index_.asciidoc[tag=autocreate]
...
+
IMPORTANT: If `dest.index` is a data stream, `op_type` must be `create`.

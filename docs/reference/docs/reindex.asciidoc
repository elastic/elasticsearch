...
`scroll`, `requests_per_second` and `auto_create_index`.
...
To disable automatic index creation `auto_create_index` parameter has to be set to `false`,
skip this parameter to follow default behaviour defined by <<index-creation,`action.auto_create_index`>>.

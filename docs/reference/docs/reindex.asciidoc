...
Additionally, if you opt to count version conflicts it is possible that the operation end up reindexing less
than `max_docs` documents if there are concurrent conflicting updates.
...
`proceed` and there are concurrent updates against the same documents that reindex is processing
it is possible that the number of successful operations are less than the specified `max_docs`.

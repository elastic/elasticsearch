...
`scroll`, `requests_per_second` and `disable_auto_create_index`.
...
`disable_auto_create_index` parameter (default value is `false`) is used to disable
automatic index creation, overrides node setting <<index-creation,`action.auto_create_index`>>.

...
Comma-separated list of data streams, indices, and aliases to use as the source.
Supports wildcards (`*`). See <<docs-reindex-from-multiple-sources>>.
...
`index`:::
(Required, string)
Data stream, index, or alias to use as the destination. Must differ from
`source.index`.
+

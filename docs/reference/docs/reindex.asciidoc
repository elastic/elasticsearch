...
In addition to the standard parameters like `pretty`, the ReIndex API also supports `refresh`, `wait_for_completion`, `wait_for_active_shards`, `timeout`, `scroll`, and `size`. The `size` parameter can not be more than the `index.max_result_window` index setting which defaults to 10,000.

...
You can increase the default batch size by setting the `size` parameter (which defaults to `100`) to the `source`. Note that it is different than the `size` parameter available at the root, which limits the number of documents (see below).

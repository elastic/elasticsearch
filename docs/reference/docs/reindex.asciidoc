...
You can increase the default batch size by setting the `size` parameter to the `source`. The default value is 100 in 2.3.x and 1000 in 5.x. Please note that it is different than the `size` parameter available at the root, which limits the number of documents (see below).

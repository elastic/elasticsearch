...
When using Elastic Cloud, it is also possible to authenticate against the remote cluster
through the use of `cloud_id` and a valid Api Key.
...
POST _reindex
{
  "source": {
    "remote": {
      "host": "https://remote-elastic-cloud-deployment:9243",
      "headers": {
        "Authorization": "ApiKey <A_SECRET_API_KEY_VALUE_HERE>"
      }
    },
    "index": "source-index",
    "query": {

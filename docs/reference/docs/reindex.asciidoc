...
`_reindex` 'ed.
* The `size` parameter can not be more than the `index.max_result_window` index setting which defaults to 10,000. This is a safeguard since search requests take heap memory and time proportional to `from` + `size`.

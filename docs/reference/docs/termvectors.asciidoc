...
To generate all possible term vectors, use `generate_all` set to to `true`. This will
override `fields` to all the possible fields for which term vectors could be generated.
...
by the user coming[1.4.0.Beta]. Note that for documents stored in the index, this
...
in similar way to the <<query-dsl-multi-match-query,multi match query>> coming[1.4.0.Beta].
...
coming[1.4.0.Beta,The ability to computed term vectors on the fly as well as support for artificial documents is only available from 1.4.0 onwards (see below example 2 and 3 respectively)]

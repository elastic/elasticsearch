...
`detect_noop` can also be set to an object, in that case it contains
tolerances:
[source,js]
--------------------------------------------------
curl -XPOST 'localhost:9200/test/type1/1/_update' -d '{
...
--------------------------------------------------
This will only update the index if either `foo` is not within 10% of
current value or `bar.baz` is not within 2.6 if its old value.

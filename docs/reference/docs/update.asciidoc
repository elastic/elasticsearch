...
Wait to reply to the request until the primary and replica shards have been
refreshed to make this operation's changes visible for search. Use this if
`refresh` is too inefficient and you can tolerate slower responses. See

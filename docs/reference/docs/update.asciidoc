...
If a value is specified then the alias routing is ignored.
...
If both `routing` and `parent` are specified, then `routing` is used to route the update request.
If an alias index routing is specified then it overrides the parent routing and it is used to route the update request.

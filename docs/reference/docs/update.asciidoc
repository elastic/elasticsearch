...
Disables automatic index creation for a single request and overrides
a node setting <<index-creation,`action.auto_create_index`>> that
allows automatic index creation on the first update request.

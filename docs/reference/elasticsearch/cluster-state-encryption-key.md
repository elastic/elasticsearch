...
**On self-managed standalone Stack** deployments, this password is generated by the [automatic security setup](/deploy-manage/security/self-auto-setup.md) on a node's first start, and stored in the keystore as `cluster.state.encryption.password.autoconfigured`.

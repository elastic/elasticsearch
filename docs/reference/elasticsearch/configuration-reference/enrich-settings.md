...
:   Maximum serialized size of a single enrich policy, bounding the total contribution of one policy (including its query, indices, and fields) to the cluster state. Defaults to `1mb`. This limit is only enforced when creating a new policy. This setting can be updated dynamically.
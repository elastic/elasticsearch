...
:   ([Dynamic](docs-content://deploy-manage/stack-settings.md#dynamic-cluster-setting)) The duration after which an open anomaly detection job with a stopped datafeed is automatically closed by the nightly maintenance task. The timer is based on the job's most recent `latest_record_timestamp`. Jobs that have never received data are not affected. Set to `-1` to disable automatic closing of idle jobs. Defaults to `48h`.

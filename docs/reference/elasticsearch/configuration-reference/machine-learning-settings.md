...
`xpack.ml.trained_models.sandbox_enabled` {applies_to}`stack: ga 9.6` {applies_to}`self: unavailable` {applies_to}`eck: unavailable` {applies_to}`ece: unavailable`
:   ([Dynamic](docs-content://deploy-manage/stack-settings.md#dynamic-cluster-setting)) Controls whether the `pytorch_inference` native process for trained model deployments runs inside the Sandbox2 security sandbox. When `true`, the process is launched inside the sandbox, which enforces system call and filesystem isolation before the model runs. When `false` (the default), the sandbox is bypassed and the process falls back to the in-process system call filter. Setting to `true` enables sandboxing, but is currently not supported for self-managed, ECK, or ECE deployments. Sandbox2 and this setting apply on Linux only; on other platforms the setting has no effect. Full Sandbox2 isolation requires the host to permit unprivileged user namespaces; a system administrator must set the Linux kernel parameter `kernel.unprivileged_userns_clone=1` (some distributions and container runtimes disable it by default). Where user namespaces are unavailable, the node automatically falls back to Landlock filesystem confinement, which restricts the files the process can open but, unlike Sandbox2, does not isolate its view of processes, mounts, or the network. If the host supports neither Sandbox2 nor Landlock — for example a Linux kernel older than 5.13 — trained model deployments on that node fail to start while this setting is `true`; set it to `false` to run models with the in-process system call filter only. Updating this setting only affects trained model deployments started after the change; already-running `pytorch_inference` processes keep the sandbox configuration they were launched with. When the {{operator-feature}} is enabled, this setting can be updated only by operator users.

    ::::{warning}
    Sandboxing is disabled by default in 9.6, so `pytorch_inference` runs with reduced process and filesystem isolation for untrusted models unless you opt in. Setting this to `true` enables sandboxing, but that is currently not supported for self-managed, ECK, or ECE deployments; understand the isolation trade-off before changing this setting either way.
    ::::

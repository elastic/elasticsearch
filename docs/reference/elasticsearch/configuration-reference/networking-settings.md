...
:   ([Static](docs-content://deploy-manage/stack-settings.md#static-cluster-setting), boolean) Configures the `SO_KEEPALIVE` option for network sockets, which determines whether each connection sends TCP keepalive probes. Defaults to `true`. TCP keepalives are very important for Elasticsearch to operate correctly and efficiently. Do not turn off TCP keepalives. Refer to [long-lived connections](#long-lived-connections) for further information.
...
`http.tcp.keep_alive`
:   ([Static](docs-content://deploy-manage/stack-settings.md#static-cluster-setting), boolean) Configures the `SO_KEEPALIVE` option for this socket, which determines whether it sends TCP keepalive probes. Defaults to `network.tcp.keep_alive`. TCP keepalives are very important for Elasticsearch to operate correctly and efficiently. Do not turn off TCP keepalives. Refer to [long-lived connections](#long-lived-connections) for further information.
...
:   ([Static](docs-content://deploy-manage/stack-settings.md#static-cluster-setting), boolean) Configures the `SO_KEEPALIVE` option for transport sockets, which determines whether they send TCP keepalive probes. Defaults to `network.tcp.keep_alive`. TCP keepalives are very important for Elasticsearch to operate correctly and efficiently. Do not turn off TCP keepalives. Refer to [long-lived connections](#long-lived-connections) for further information.
...
A transport connection between two nodes is made up of a number of long-lived TCP connections, some of which may be idle for an extended period of time. Nonetheless, {{es}} requires these connections to remain open. It disrupts the operation of {{es}} if any inter-node connections are closed by an external influence such as a firewall, NAT gateway, load-balancer or service mesh. Configure your network to preserve long-lived connections between {{es}} nodes even if idle: leave all `*.tcp.keep_alive` settings enabled, and ensure that all intermediate network devices respect TCP keepalives correctly and that the keepalive schedule is appropriate for your environment. Devices which drop connections when they reach a certain age are a common source of problems to {{es}} clusters and must not be used.

{{es}} also uses TCP keepalives to ensure that every inter-node request eventually receives a response: an intermediate device might silently block an inter-node connection while the remote node is computing a response to a request. In this situation neither side can send any data, nor a connection-closing `FIN` or `RST` packet, to the other. Without TCP keepalives, each side can see the connection remain in state `ESTABLISHED` with the requesting task waiting indefinitely for a response that can never arrive. TCP keepalives allow both nodes to detect that the connection has become unavailable, cancelling the task early on the handling node and informing the requesting node that its request encountered a network disruption. If you disable TCP keepalives, be prepared to restart your nodes frequently to clear any in-progress tasks which are waiting on responses that were lost due to network disruptions.

If an {{es}} node is temporarily unable to handle network traffic on a TCP connection, it stops reading data from the network and advertises a zero-length TCP window to its peers so that they pause the transmission of data on the affected connection. This is known as a "zero-window" state, and is the standard backpressure mechanism built into TCP. When the node becomes available again, it will resume reading from the connection. Configure your network to permit TCP connections to exist in this zero-window state without disruption. Do not impose any limit on the length of time that a connection may remain in a zero-window state.
...
:   ([Static](docs-content://deploy-manage/stack-settings.md#static-cluster-setting), boolean) Configures the `SO_KEEPALIVE` option for remote cluster sockets, which determines whether they send TCP keepalive probes. Defaults to `transport.tcp.keep_alive`. TCP keepalives are very important for Elasticsearch to operate correctly and efficiently. Do not turn off TCP keepalives. Refer to [long-lived connections](#long-lived-connections) for further information.

...
`s3.client.CLIENT_NAME.always_sign_uploads` {applies_to}`stack: ga 9.5`
:   The AWS SDK usually uses HTTPS to protect the integrity of all requests and responses, but if you access your repository using plain HTTP then the SDK uses a separate signature mechanism to protect the integrity of the bodies of `PutObject` and `UploadPart` requests. If `true`, {{es}} will use this additional signature mechanism even for requests sent over HTTPS that do not need it. Defaults to `false`.

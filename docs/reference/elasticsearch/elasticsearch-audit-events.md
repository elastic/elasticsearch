...
:   Logged when a service account token create API is invoked to create a new index-based token. This includes the [create service account token API](https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-security-create-service-token) for built-in service accounts and the create managed service account token API for managed service accounts.
...
$$$event-delete-managed-service-account$$$

`delete_managed_service_account`
:   Logged when the delete managed service account API is invoked to delete a managed service account at `/_security/service/{namespace}/{service}`.
...
:   Logged when the [delete service account token API](https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-security-delete-service-token) is invoked to delete an index-based token for a built-in or managed service account.
...
$$$event-put-managed-service-account$$$

`put_managed_service_account`
:   Logged when the create or update managed service account API is invoked to create or update a managed service account at `/_security/service/{namespace}/{service}`.
    ...
    {"type":"audit", "timestamp":"2021-04-30T23:17:42,952+0200", "node.id":
    "0RMNyghkQYCc_gVd1G6tZQ", "event.type":"security_config_change", "event.
    action":"put_managed_service_account", "request.id":"az9a1Db5QrebDMacQ8yGKc",
    "put":{"managed_service_account":{"namespace":"my-app","service":"worker",
...
`put`
:   The object representation of the security config that is being created, or the overwrite of an existing config. It contains the config for a `user`, `role`, `role_mapping`, `managed_service_account`, or for application `privileges`.

`delete`
:   The object representation of the security config that is being deleted. It can be the config for a `user`, `role`, `role_mapping`, `managed_service_account`, or for application `privileges`.

...
:   Set to `true` to make the index and index metadata read only, `false` to allow writes and metadata changes. While block is enabled, the `cluster_block_exception` message will return `FORBIDDEN/12/index read-only / allow delete (api)`.

`index.blocks.read_only_allow_delete`
:   Similar to `index.blocks.write`, except that you can delete the index when this block is in place. Do not set or remove this block yourself. The [disk-based shard allocator](/reference/elasticsearch/configuration-reference/cluster-level-shard-allocation-routing-settings.md#disk-based-shard-allocation) sets and removes this block automatically according to the available disk space. While block is enabled, the `cluster_block_exception` message will return `FORBIDDEN/12/index read-only / allow delete (api)`.
...
:   Set to `true` to disable read operations against the index. While block is enabled, the `cluster_block_exception` message will return `FORBIDDEN/5/index read-only (api)`.

$$$index-blocks-write$$$

`index.blocks.write` {applies_to}`serverless: all`
:   Set to `true` to disable data write operations against the index. Unlike `read_only`, this setting does not affect metadata. For instance, you can adjust the settings of an index with a `write` block, but you cannot adjust the settings of an index with a `read_only` block. While block is enabled, the `cluster_block_exception` message will return `FORBIDDEN/8/index write (api)`.

$$$index-blocks-metadata$$$
`index.blocks.metadata` {applies_to}`serverless: all`
:   Set to `true` to disable index metadata reads and writes. While block is enabled, the `cluster_block_exception` message will return `FORBIDDEN/9/index metadata (api)`.

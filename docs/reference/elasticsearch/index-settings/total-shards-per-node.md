...
You can also limit the amount of shards a node can have regardless of the index with the following cluster level setting. This setting applies across all indices on a node and can be set at runtime using the [update cluster settings](https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-cluster-put-settings) API:

...
:   (Optional, boolean) When `true`, merges use direct I/O for the field's raw vector data where the index type supports it, so a merge displaces less of what searches keep in the page cache. Requires the `hybridfs` store type; elsewhere, or where direct I/O cannot be initialized, merges use the page cache. Segments stored in a compound file are read through the page cache, and a merged segment that ends up compound is copied into its compound file through the page cache after the direct write. Useful on nodes whose vector data does not fit in RAM; where the sources of a merge are usually still cached it adds device reads for no gain. Can be changed with a mapping update: merge writes follow the mapping from the next merge, and each source segment is read the way it was written. Defaults to `false`.

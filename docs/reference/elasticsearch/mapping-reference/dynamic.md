...
`runtime`
:   New fields are added to the mapping as [runtime fields](docs-content://manage-data/data-store/mapping/runtime-fields.md). These fields are not indexed and are loaded from `_source` at query time. This option is not supported for [columnar](/reference/columnar/index.md) indices.
...
:::{warning}
If you configure `"dynamic": false` on a [columnar](/reference/columnar/index.md) index, new fields are not stored at all and will be lost.
:::

...
* GPU driver, CUDA and [cuVS runtime libraries](https://docs.rapids.ai/api/cuvs/stable/build/) installed on the node
...
indexing is not being used, such as an unsupported environment, missing libraries, or an incompatible GPU.
...
The node will fail to start if GPU indexing is not available, e.g. if a GPU is not detected by {{es}}, or if the runtime is not supported, or if the necessary dependencies are not correctly configured, etc.
...
* `LD_LIBRARY_PATH` includes paths to the cuVS libraries and to their dependencies (e.g. CUDA)

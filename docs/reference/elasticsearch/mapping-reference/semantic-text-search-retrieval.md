...
The original `semantic_text` value is returned in `_source`. Large text values can make `_source` responses large. Use [source filtering](/reference/elasticsearch/rest-apis/retrieve-selected-fields.md#source-filtering) to exclude `semantic_text` fields or return only the fields your application needs.

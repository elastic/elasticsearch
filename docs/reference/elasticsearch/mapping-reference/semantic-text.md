...
- [Set up and configure `semantic_text` fields](./semantic-text-setup-configuration.md): Learn how to configure {{infer}} endpoints, including default endpoints, EIS models (including ELSER on EIS), external {{infer}}, and dedicated endpoints for ingestion and search operations.

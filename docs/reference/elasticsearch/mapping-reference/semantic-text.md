...
{applies_to}`serverless` If you don’t specify an {infer} endpoint, the `inference_id` field defaults to
...
{applies_to}`stack` If you don’t specify an inference endpoint, the `inference_id` field defaults to

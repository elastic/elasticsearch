...
## Retrieving embedding metadata [inference-metadata]
The semantic_text field type stores the embeddings generated by the {{infer-cap}} endpoint in a hidden metadata field named `_inference_fields`. To retrieve these embeddings for a document, `_inference_fields` needs to be explicitly requeted using the fields parameter in your query.

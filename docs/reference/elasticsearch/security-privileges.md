...
`cancel_task`
:   Privileges to cancel tasks and delete async searches. See [delete async search]({{es-apis}}operation/operation-async-search-submit) API for more information.
...
:   Privileges to delegate PKI authentication. Enables the use of the [delegate PKI authentication API]({{es-apis}}operation/operation-security-delegate-pki) and to invalidate tokens.
...
:   Privileges to connect to [remote clusters configured with the API key based model](docs-content://deploy-manage/remote-clusters/remote-clusters-api-key.md) for cross-cluster replication.
    ...
    ::::{note}
    This privilege must *not* be directly granted. It is used internally by [Create Cross-Cluster API key]({{es-apis}}operation/operation-security-create-cross-cluster-api-key) and [Update Cross-Cluster API key]({{es-apis}}operation/operation-security-update-cross-cluster-api-key) to manage cross-cluster API keys.
    ::::
...
`manage_api_key`
:   All security-related operations on {{es}} REST API keys including [creating new API keys]({{es-apis}}operation/operation-security-create-api-key), [retrieving information about API keys]({{es-apis}}operation/operation-security-get-api-key), [querying API keys]({{es-apis}}operation/operation-security-query-api-keys), [updating API key]({{es-apis}}operation/operation-security-update-api-key), [bulk updating API keys]({{es-apis}}operation/operation-security-bulk-update-api-keys), and [invalidating API keys]({{es-apis}}operation/operation-security-invalidate-api-key).
...
:   Enables the use of {{es}} APIs ([OpenID connect prepare authentication]({{es-apis}}operation/operation-security-oidc-prepare-authentication), [OpenID connect authenticate]({{es-apis}}operation/operation-security-oidc-authenticate), and [OpenID connect logout]({{es-apis}}operation/operation-security-oidc-logout)) to initiate and manage OpenID Connect authentication on behalf of other users.


`manage_own_api_key`
:   All security-related operations on {{es}} API keys that are owned by the current authenticated user. The operations include [creating new API keys]({{es-apis}}operation/operation-security-create-api-key), [retrieving information about API keys]({{es-apis}}operation/operation-security-get-api-key), [querying API keys]({{es-apis}}operation/operation-security-query-api-keys), [updating API key]({{es-apis}}operation/operation-security-update-api-key), [bulk updating API keys]({{es-apis}}operation/operation-security-bulk-update-api-keys), and [invalidating API keys]({{es-apis}}operation/operation-security-invalidate-api-key).
...
:   All CRUD operations on [search applications]({{es-apis}}group/endpoint-search_application).

`manage_search_query_rules`
:   All CRUD operations on [query rules]({{es-apis}}group/endpoint-query_rules).

`manage_search_synonyms`
:   All synonyms management operations on [Synonyms APIs]({{es-apis}}group/endpoint-synonyms).
...
:   Security-related operations on {{es}} service accounts including [Get service accounts]({{es-apis}}operation/operation-security-get-service-accounts), [Create service account tokens]({{es-apis}}operation/operation-security-create-service-token), [Delete service account token]({{es-apis}}operation/operation-security-delete-service-token), and [Get service account credentials]({{es-apis}}operation/operation-security-get-service-credentials). 
...
:   All {{slm}} ({{slm-init}}) actions, including creating and updating policies and starting and stopping {{slm-init}}. It also grants the permission to start and stop {{Ilm}}, using the [ILM start]({{es-apis}}operation/operation-ilm-start) and [ILM stop]({{es-apis}}operation/operation-ilm-stop) APIs.
...
:   All read-only operations related to the [find structure API]({{es-apis}}operation/operation-text-structure-find-structure).
...
:   All read-only {{slm-init}} actions, such as getting policies and checking the {{slm-init}} status. It also grants the permission to get the {{Ilm}} status, using the [ILM get status API]({{es-apis}}operation/operation-ilm-get-status).

     In a future major release, this privilege will not grant any {{Ilm}} permissions.

`read_security`
:   All read-only security-related operations, such as getting users, user profiles, {{es}} API keys, {{es}} service accounts, roles and role mappings. Allows [querying]({{es-apis}}operation/operation-security-query-api-keys) and [retrieving information]({{es-apis}}operation/operation-security-get-api-key) on all {{es}} API keys.
...
`auto_configure`
:   Permits auto-creation of indices and data streams. An auto-create action is the result of an [index]({{es-apis}}operation/operation-create) or [bulk]({{es-apis}}operation/operation-bulk) request that targets a non-existent index or data stream rather than an explicit [create index]({{es-apis}}operation/operation-indices-create) or [create data stream]({{es-apis}}operation/operation-indices-create-data-stream) request. Also permits auto-update of mappings on indices and data streams if they do not contradict existing mappings. An auto-update mapping action is the result of an index or bulk request on an index or data stream that contains new fields that may be mapped rather than an explicit [update mapping]({{es-apis}}operation/operation-indices-put-mapping) request.
...
:   All actions that are required to manage the lifecycle of a leader index, which includes [forgetting a follower]({{es-apis}}operation/operation-ccr-forget-follower). This privilege is necessary only on clusters that contain leader indices.
...
Application privileges are managed within {{es}} and can be retrieved with the [has privileges API]({{es-apis}}operation/operation-security-has-privileges) and the [get application privileges API]({{es-apis}}operation/operation-security-get-privileges). They do not, however, grant access to any actions or resources within {{es}}. Their purpose is to enable applications to represent and store their own privilege models within {{es}} roles.

To create application privileges, use the [add application privileges API]({{es-apis}}operation/operation-security-put-privileges). You can then associate these application privileges with roles, as described in [Defining roles](https://www.elastic.co/guide/en/elasticsearch/reference/current/defining-roles.html).

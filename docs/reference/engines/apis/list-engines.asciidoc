[role="xpack"]
[[list-engines]]
= List Engines API
[subs="attributes"]
++++
<titleabbrev>List engines</titleabbrev>

Returns information about search engines.


[[list-engines-request]]
== {api-request-title}

`GET _engine/`

[[list-engines-prereq]]
== {api-prereq-title}

Requires the `manage_engine` cluster privilege.
Also requires manage `manage_index` privileges on all indices that are added to the search engine.

[[list-engines-path-params]]
== {api-path-parms-title}

`q`::
(Optional, string) Query in the Lucene query string syntax, to return only search engines matching the query.

`size`::
(Optional, indeger)

`from`::
(Optional, indeger)

[[list-engines-response-codes]]
== {api-response-codes-title}

`403`::
Insufficient privileges

[[list-engines-example]]
== {api-examples-title}

The following example lists all configured search engines:

[source,console]
--------------------------------------------------
GET _engine/
--------------------------------------------------
// TEST[skip:TBD]

The following example lists the first three search engines matching `foo`:

[source,console]
--------------------------------------------------
GET _engine/?from=0&size=3&q=foo
--------------------------------------------------
// TEST[skip:TBD]

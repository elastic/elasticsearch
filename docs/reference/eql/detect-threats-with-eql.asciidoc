...
been normalized and mapped to use fields from the {ecs-ref}[Elastic Common Schema
(ECS)], including the `@timestamp` and `event.category` fields. The dataset
includes events that imitate behaviors related to `regsvr32` misuse, as
...
Based on your previous query, you know `regsv32` processes were associated with
143 events. But how was first `regsvr32.exe` called? And who called it?

`regsv32` is a command-line utility so narrowing your results to processes where
...
. A load of the `scrobj.dll` library

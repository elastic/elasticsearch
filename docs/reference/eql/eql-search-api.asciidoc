...
This search ID is only returned if a search does not return complete results
during the timeout period set by the <<eql-search-api-wait-for-completion-timeout,
`wait_for_completion_timeout`>> parameter, becoming an <<eql-search-async,async
search>>. You can use this ID to monitor the progress of an ongoing async
search and retrieve complete results when the search finishes.
--

...
* experimental:[] For cross-cluster search, some limitations apply to local and remote
{es} version, see <<run-eql-search-across-clusters>>. For security, see <<remote-clusters-security>>.

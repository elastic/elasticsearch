...
queries, these hits are matching events, which are included in the `hits.events`
property. These events are sorted by timestamp, converted to milliseconds since
the {wikipedia}/Unix_time[Unix epoch], in ascending order.
...
You can use the `size` request body parameter to get a larger set of hits.
For example, the following request can retrieve up to `50` hits.
...
The top 10 matching sequences are included in the `hits.sequences` property.

...
If the request doesn't finish in the timeout duration, the search becomes async
and returns a response that includes:

* A search ID
* An `is_partial` value of `true`, indicating the search results are not
  complete
...
If the `is_partial` value is `false`, the response's search results are
complete.
...
To save the search results for a synchronous search, set `keep_on_completion` to
`true`:

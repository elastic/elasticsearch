...
Sample queries enable you to search for events that match a join key and a set
of filters. Samples are similar to sequences, but do not return events in
chronological order. In fact, sample queries can run on data without a
...
This will return samples where each of the events share the same value for

...
data, such as logs, metrics, or traces.
...
Use EQL's <<eql-sequences,sequence syntax>> to search for a series of events in
ascending chronological order, with the most recent event listed last:
...
events in matching sequences to a timespan:
...
Use the <<eql-by-keyword,`by` keyword>> to only match events that share the
same field values:
...
Use the <<get-async-eql-search-api,get async EQL search API>>'s

...
EQL assumes each document in an index corresponds to an event that includes a
timestamp and an event category. To search using EQL, each document in the data
stream or index must contain a _timestamp_ field and an _event category_ field.

In {es}, EQL uses the ECS `@timestamp` and `event.category` fields from the
{ecs-ref}[Elastic Common Schema (ECS)] as the default timestamp and event
category fields. If your documents use different timestamp or event category
fields, you must specify them in the search request. See
...
You can use the <<eql-search-api,EQL search API>> to search event data stored in
one or more {es} data streams or indices.
...
in `my-index-000001` includes a `@timestamp` and `event.category` field value.
...
<<eql-required-fields,required timestamp and event category fields>>. If your data
stream or index uses another timestamp or event category field, you must specify
them using the `timestamp_field` or `event_category_field` parameters.
...
The following request uses the `timestamp_field` to specify `file.accessed` as
the timestamp field. The request also uses the `event_category_field` to specify
`file.type` as the event category field.
...
The following request uses a `range` query to filter the `my-index-000001`

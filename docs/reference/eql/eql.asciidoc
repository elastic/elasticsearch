...
Use EQL's <<eql-samples,sample syntax>> to search for events that match a join
key and a set of filters. Samples are similar to sequences, but do not return
events in chronological order. In fact, sample queries can run on data without a
timestamp. Sample queries can be useful to find correlations in events that
don't always occur in the same sequence, or that occur across long time spans.
...
A sample query specifies a join key, using the `by` keyword, and up to
five filters:
...
Each sample has a unique set of `join_keys` and an array of events that each
match a filter. Events are returned in the order of the filters they match:
...
Similar to sequence queries, you can use the <<eql-by-keyword,`by` keyword>> to
match events that share the same field values:
...
This will return samples where each of the events shares the same value for
`os` or `op_sys` (as well as for `host`). For example:

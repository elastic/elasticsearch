...
The EQL search API supports <<modules-cross-cluster-search,cross-cluster search>>. However, the local and <<remote-clusters,remote clusters>> must use the same {es} version if they have versions prior to 7.17.7 (included) or prior to 8.5.1 (included).
search>> with <<remote-clusters,remote clusters>> v 7.17.8+,
and limited to <<ccs-supported-configurations>>.
However, if local node has version prior to 7.17.8 or 8.5.2, <<remote-clusters,remote clusters>>
must be the same {es} version to run <<modules-cross-cluster-search,cross-cluster
search>>.

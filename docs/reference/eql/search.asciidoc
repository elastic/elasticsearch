...
<<query-dsl,query DSL>>. This query filters the results matched by the EQL
query.

For example, the following request uses a `range` query to filter the EQL query
results to only those events with a `file.size` greater than `1` but less than
`1000000` bytes.

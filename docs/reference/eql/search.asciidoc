...
EQL searches in {es} are designed to to run on large volumes of data quickly,
often returning results in milliseconds. Because of this, the EQL search API
runs _synchronous_ searches run by default. This means the search request
...
By default, the EQL search API only stores ongoing async searches and their
search results for five days. After this period, async searches and any saved
...
days). This indicates that if the search becomes async, its results
...
`true`. This indicates search results should be stored on the cluster, even if

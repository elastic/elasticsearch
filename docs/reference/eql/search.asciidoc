...
The following EQL search request adds `with maxspan=2d` to the previous query.
This ensures all events in a matching sequence occur within two days (`2d`) of
...
{
  "query": """
    sequence by agent.id with maxspan=2d
      [ file where file.name == "cmd.exe" ]
      [ process where stringContains(process.name, "regsvr32") ]
  """
}

...
Otherwise returns `false`. For strings, matching is case-insensitive.

`:` (equal, case-sensitive)::
Returns `true` if strings to the left and right of the operator are equal.
Otherwise returns `false`. Matching is case-sensitive and can only be used to
compare strings. Wildcard (`*`) patterns are not supported.
...
equal. Otherwise returns `false`. For strings, matching is case-insensitive.
...
NOTE: `=` is not supported as an equality operator. Use `==` or `:` instead.
...
wildcard `*` operator within the string to match specific patterns:
...
The `:` operator does not support wildcard patterns.

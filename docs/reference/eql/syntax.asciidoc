...
timespan. All events in a matching sequence occur within this duration, starting
at the first event's timestamp.
...
[%collapsible]
====
The following sequence query uses a `maxspan` value of `1h` (one hour). Events
in a matching sequence must occur within one hour of the first event's
timestamp.
...
sequence by both field values and timespan.
...
* Share the same `user.name` field value

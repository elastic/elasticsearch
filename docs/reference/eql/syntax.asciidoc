...
of events. All events in a sample share the same value for the field that is
specified using the `by` keyword (join key). Each item in a sample is an event
category and event condition, surrounded by square brackets (`[ ]`). Events are
listed in the order of the filters they match.
...
[discrete]
[[eql-samples-by-keyword]]
==== `by` keyword

Sample queries support the `by` keyword to only match events that share the same
values. Refer to <<eql-by-keyword,`by` keyword>>.

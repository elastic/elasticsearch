...
to the right. Otherwise returns `false`. When comparing strings, the operator
uses a case-sensitive lexographic order.

`<=` (less than or equal) ::
Returns `true` if the value to the left of the operator is less than or equal to
the value to the right. Otherwise returns `false`. When comparing strings, the
operator uses a case-sensitive lexographic order.

`==` (equal)::
Returns `true` if the values to the left and right of the operator are equal.
Otherwise returns `false`. Matching is case-insensitive.

`!=` (not equal)::
Returns `true` if the values to the left and right of the operator are not
equal. Otherwise returns `false`. Matching is case-insensitive.
...
the operator uses a case-sensitive lexographic order.

`>` (greater than)::
Returns `true` if the value to the left of the operator is greater than the
value to the right. Otherwise returns `false`. When comparing strings, the
operator uses a case-sensitive lexographic order.
====

IMPORTANT: When comparing strings, the `==` and `!=` operators are
case-insensitive. Other comparison operators, such as `>` and `<=`, use a
case-sensitive lexographic order when comparing strings.
...
to convert data types, perform math, manipulate strings, and more. By default,
most EQL functions are case-insensitive.

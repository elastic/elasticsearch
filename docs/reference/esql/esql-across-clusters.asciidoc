...
This may be the preferred model when you have different administrators for different clusters and want more control over who can access what data.
...
* Add the API key to the `elasticsearch.yml` file on the *local cluster*, as part of the steps in <<remote-clusters-security-api-key-local-actions,configuring the local cluster>>. All cross-cluster requests from the local cluster are bound by the API key’s privileges.
...
You will then need to create a user with the role you created above. The following example API call creates a user with the `remote1` role.
...
Remember that all cross-cluster requests from the local cluster are bound by the API key’s privileges, which are controlled by the remote cluster's administrator.

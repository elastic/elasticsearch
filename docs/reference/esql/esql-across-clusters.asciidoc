...
Similarly, this {esql} request queries the `my-index-000001` index across
three clusters:
...
It offers three modes to control where the enrich command occurs.

[discrete]
[[esql-enrich-any]]
==== Enrich with any mode

The default mode, `_any`, allows {esql} to execute the enrich command on
either the querying cluster or the fulfilling clusters. This mode selects
the suitable clusters to execute the enrich, aiming to minimize computation
or inter-cluster data transfer.

In the following example, the enrich with `hosts` policy can be executed on
either the querying cluster or the remote cluster cluster_one. Ensure that
the policy exists with consistent data on both the querying cluster and the
fulfilling clusters is critical for ES|QL to produce a consistent query result.

[source,esql]
----
FROM my-index-000001,cluster_one:my-index-000001
| ENRICH _any:hosts ON ip
| LIMIT 10
----
...
[source,esql]
----
FROM cluster_one:my-index-000001,cluster_one:my-index-000002
| LIMIT 10
| ENRICH _any:hosts ON ip
----
...
In the `_coordinator` mode, {esql} executes the enrich command on the
querying cluster.
...
[IMPORTANT]
====
Enrich with the `_coordinator` mode might increase inter-cluster data transfer and
workload on the querying cluster. This mode should be used when the enrich policy is
not available on the remote clusters or maintaining consistency of enrich indices across
clusters is challenging.
====
...
In the `_remote` mode, {esql} executes the enrich command independently on
each fulfilling cluster where the target indices reside. This mode is useful
for managing different enrich data on each cluster, such as detailed
...
For now, the `_remote` enrich cannot be executed after a <<esql-stats-by,stats>>
...
A `_remote` enrich command can be executed after an `_any` enrich or another
`_remote` enrich, but not after a `_coordinator` enrich. The following
example would result in an error.

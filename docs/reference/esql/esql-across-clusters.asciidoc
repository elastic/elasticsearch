...
write the enrich command as you would without across clusters. In this default mode,
...
[IMPORTANT]
====
Enrich with the `_coordinator` mode usually increase inter-cluster data transfer and
...
A `_remote` enrich command can't be executed after after a `_coordinator` enrich
command. The following example would result in an error.

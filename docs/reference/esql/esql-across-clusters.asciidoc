...
{ccs-cap} for {esql} will ignore the remote cluster which is disconnected from the querying cluster unless its
`skip_unavailable` setting is set to `false`, in which case the query will fail.

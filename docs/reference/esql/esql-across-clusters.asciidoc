...
As with cross-cluster search using query DSL (link here), ES|QL {ccs} responses include metadata
about the search on each cluster when the response format is JSON. Here we use the async search
endpoint, but {ccs} metadata is also present in the synchronous search endpoint.
...
----
// TEST[skip: cross-cluster env not set up]

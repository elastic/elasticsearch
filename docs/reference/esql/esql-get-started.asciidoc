...
that are returned, to a maximum of 10,000 rows:
...
The following example first sorts the rows on `@timestamp`, and next limits the
...
set to 3 rows before sorting those 3 rows could result in 3 different rows being
returned.
...
an <<esql-enrich-policy,enrich policy>>. The following requests create a policy
that links an IP address to an environment ("Development", "QA", or
"Production"):
...
By extracting the IP address from messages like this, you can determine which IP
has accepted the most client connections.
...
To learn more about {esql}, refer to <<esql-language>>.
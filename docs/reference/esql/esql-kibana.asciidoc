...
visualization. Changing the previous example query to aggregate the total number
of bytes per destination country:
...
changes the visualization into a bar chart showing the top 3 countries:
...
* Discover shows no more than 10,000 rows and 50 columns. If a query returns

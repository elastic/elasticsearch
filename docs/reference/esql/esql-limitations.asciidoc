...
it is possible that the same field is mapped to multiple types.
Such a field cannot be directly used in the query, nor be returned in the results,
except if <<esql-multi-index-union-types, explicitly converted to a single type>>.

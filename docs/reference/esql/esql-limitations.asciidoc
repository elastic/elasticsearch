...
field's subfield of type `keyword` and query/aggregate that. If it's not
possible to retrieve a `keyword` subfield, {esql} will get the string from a
document's `_source`. As a result, an {esql} query on a `text` field is
case-sensitive.

Note that {esql}'s retrieval of `keyword` subfields may have unexpected
consequences. For example, a subfield may have been mapped with a
<<normalizer,normalizer>>, which can transform the original string. Or it may
have been mapped with <<ignore-above>>, which can truncate the string. None of
these mapping operations are applied to an {esql} query, which may lead to false
positive or negatives. 

To avoid these issues, a best practice is to be explicit about the field that
you query, and query `keyword` fields instead of `text` fields.

...
section of the line is reliably repeated, but the entire line is not. The
`DISSECT` filter can deconstruct the section of the line that is repeated. The
`GROK` filter can process the remaining field values using regular expressions.
...
[NOTE]
====
Special regex characters like `[` and `]` need to be escaped with a `\`. You can
see this in the earlier example, where the pattern was:
[source,txt]
----
%{IP:ip} \[%{TIMESTAMP_ISO8601:@timestamp}\] %{GREEDYDATA:status}
----

...
on each cluster. Defaults to `false`. The API only supports this parameter for JSON responses.
See <<ccq-cluster-details>>.
...
and (3) when `format` of the response is set to JSON (the default).

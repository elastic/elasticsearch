...
`pragma`::
(Optional, object) Do not use the pragma parameter unless you fully understand
the risks. The query pragma can break your cluster and can be removed without notice.
To ensure that these implications are well-understood, this parameter requires
the parameter `accept_pragma_risks` to be explicitly set to `true`.

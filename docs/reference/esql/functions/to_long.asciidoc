...
type must be of a boolean, date, string, numeric type or spatial point type.
...
its value will be converted to the encoded value used in Lucene indexing.
For `geo_point` values, the latitude is encoded to the high 4 bytes,
and the longitude is encoded to the lower 4 bytes.
For `point` values, the `x` value is encoded to the high 4 bytes,
and the `y` value is encoded to the lower 4 bytes.
The resulting value can be converted back into the original point type
using the `TO_GEOPOINT` and `TO_CARTESIANPOINT` functions.
Note that the encoding/decoding involves some loss of precision.

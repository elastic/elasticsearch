...
supported ones are:
  ...
  * experimental::["scoring is experimental and only available in SNAPSHOT versions."]
  `_score`: the <<query-filter-context#relevance-scores,score>> of each document with respect to the ES|QL query.
The field is of the type <<number,double>>.

...
{esql} can also access <<mapping-fields, metadata fields>>. The currently
supported ones are:
...
// TODO: are there dedicated docs for _version?
...
Consequently, the metadata fields are only available if the source of the data
is an index; i.e., the source command is `FROM`.

...
By default, `GROK` outputs string columns. `int` and `float` types can be
converted by appending `:type` to the semantics in the pattern. For example
`{NUMBER:num:int}`:
...
For other type conversions, use use the <<esql-type-conversion-functions>>:

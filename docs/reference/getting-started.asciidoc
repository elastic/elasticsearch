...
You can download the sample dataset (accounts.json) from https://raw.githubusercontent.com/elastic/elasticsearch/master/docs/src/test/resources/accounts.json[here]. Extract it to our current directory and let's load it into our cluster as follows:

...
data stream made up of multiple auto-generated backing indices. {es} manages the
creation and management of these backing indices for you.
...
To add a single log entry to the `logs-my_app-default` data stream, submit the
following indexing request. Since `logs-my_app-default` doesn't exist, the
...
When you're done, delete your test data stream and it backing indices.

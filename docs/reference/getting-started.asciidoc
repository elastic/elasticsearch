...
In a single cluster, you can define as many indeces as you want.
...
* Perform CRUD (Create, Read, Update, and Delete) and search operations against your indeces
...
Now let's create an index named "customer" and then list all the indeces again:
...
Now let's delete the index that we just created and then list all the indeces again:
...
The above indeces a new document with an ID of 2.
...
In addition to being able to index and replace documents, we can also update documents. Note though that Elasticsearch does not actually do in-place updates under the hood. Whenever we do an update, Elasticsearch deletes the old document and then indeces a new document with the update applied to it in one shot.
...
As a quick example, the following call indeces two documents (ID 1 - John Doe and ID 2 - Jane Doe) in one bulk operation:

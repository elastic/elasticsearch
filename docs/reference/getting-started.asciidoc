...
the Elasticsearch Service or set up a multi-node {es} cluster on your own
...
When you create a deployment on the Elasticsearch Service, the service provisions
a three node Elasticsearch cluster along with Kibana and APM.
...
* `took` – how long it took Elasticssearch to run the query, in milliseconds
...
{es} aggregations enble you to get meta-information about your search results

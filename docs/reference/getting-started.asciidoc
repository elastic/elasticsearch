...
The following search uses the `avg` aggregation to get the mean response size in
the `http.response.body.bytes` runtime field. The aggregation only runs on
...
The response's `aggregations` section includes your aggregation results.

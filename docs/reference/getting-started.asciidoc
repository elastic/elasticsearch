...
By default, each index in Elasticsearch is allocated 5 primary shards and 1 replica which means that you need to have at least two nodes in your cluster(since replica is not stored on the same node and we have 1 replica here), your index will have 5 primary shards and another 5 replica shards (1 complete replica) for a total of 10 shards per index.

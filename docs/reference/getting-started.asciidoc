...
search will match not any documents in `logs-my_app-default`.
...
Use a <<modules-scripting-painless,Painless script>> and a <<grok-basics,grok
pattern>> to extract a <<runtime-search-request,runtime field>> from
unstructured content, such as log messages. Include the extracted field name in

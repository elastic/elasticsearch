...
To take {es} for a test drive, you can create a one-click cloud deployment on the <<gs-run-elasticsearch-cloud,Elasticsearch Service>>,
or set up a basic {es} cluster on your own <<gs-run-elasticsearch-linux,Linux>>,
<<gs-run-elasticsearch-mac,macOS>>, or
<<gs-run-elasticsearch-win,Windows>> machine.

[float]
[[gs-run-elasticsearch-cloud]]
=== Run {es} on the Elasticsearch Service
The simplest way to get Elasticsearch up and running is to use the hosted
{es} Service on Elastic Cloud.

To run a free Elasticsearch instance on AWS or GCP:

. Go to the https://www.elastic.co/cloud/elasticsearch-service/signup[
Elasticsearch Service Trial page].
. Enter your email address and click **Start Free Trial**.
. Verify your email address by clicking the link in the welcome email from Elastic.
. Enter a password for the Elasticsearch Service console and click **Set password**.
. Click **Create deployment** on the Deployments page.
. Give your deployment a name and click **Create deployment** to set up the latest
version of {es} on AWS. (If you prefer to run your instance on GCP, select
Google Cloud Platform.)
. Save the generated password for the _elastic_ user.

You're all set! You have a three node {es} cluster up and running, along with a
{kib} instance you can use to send requests to {es} and load, explore, and
visualize sample data.


[float]
[[gs-run-elasticsearch-linux]]
=== Run {es} on Linux

To install and run {es} on your Linux machine:
...
You're all set! You now have a single-node {es} cluster up and running.

[float]
[[gs-run-elasticsearch-mac]]
=== Run {es} on macOS

To install and run {es} on your Mac:
...
You're all set! You have a single-node {es} cluster up and running.

[float]
[[gs-run-elasticsearch-win]]
=== Run {es} on Windows

To install and run {es} on your Windows box:

. Download the Elasticsearch {version} MSI from
https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-{version}.msi.

. Double click the MSI file and install {es} using the default settings.
...
For this tutorial, all you need is a basic Elasticsearch install on Elastic Cloud
or your local machine, but you can also install {es} using the  <<deb,deb>> or
<<rpm,rpm>> packages or use the <<docker,Docker image>> to run
{es} in a container.

...
the {ref}/docs-bulk.html[`_bulk` API]. Using `_bulk` to batch document
...
and a total payload between 5MB and 15MB. From there, you can do some
...
. Index the account data into the _bank_ index with the following `_bulk` request:
+
[source,sh]
--------------------------------------------------
...
////
+
The response indicates that 1000 documents were indexed successfully.
+
[source,txt]
--------------------------------------------------

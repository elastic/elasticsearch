...
The number of shards and replicas can defined per index at the time the index is created. After the index is created, you may change the number of replicas dynamically anytime but you cannot change the number shards after-the-fact.
...
Once we have Java set up, we can then download and run Elasticsearch. The binaries are available from http://www.elasticsearch.org/download along with all the releases that have been made in the past. For each release, you have a choice among a zip, tar, DEB, or RPM package. For simplicity, let's use the tar package.
...
[source,sh]
--------------------------------------------------
mo@mwubuntu1:~/elasticsearch-1.1.1/bin$ ./elasticsearch
...
[source,sh]
--------------------------------------------------
mo@mwubuntu1:~$ curl 'localhost:9200/_cat/nodes?v'
...
[source,sh]
--------------------------------------------------
mo@mwubuntu1:~$ curl 'localhost:9200/_cat/indices?v'
...
[source,sh]
--------------------------------------------------
mo@mwubuntu1:~$ curl -XPUT 'localhost:9200/customer?pretty'
{
  "acknowledged" : true
}
mo@mwubuntu1:~$ curl 'localhost:9200/_cat/indices?v'
...
[source,sh]
--------------------------------------------------
mo@mwubuntu1:~$ curl -XPUT 'localhost:9200/customer/external/1?pretty' -d '{ "name": "John Doe" }'
{
  "_index" : "customer",
  "_type" : "external",
  "_id" : "1",
...
[source,sh]
--------------------------------------------------
mo@mwubuntu1:~$ curl -XGET 'localhost:9200/customer/external/1?pretty'
{
  "_index" : "customer",
  "_type" : "external",
  "_id" : "1",
...
[source,sh]
--------------------------------------------------
mo@mwubuntu1:~$ curl -XDELETE 'localhost:9200/customer?pretty'
{
  "acknowledged" : true
}
mo@mwubuntu1:~$ curl 'localhost:9200/_cat/indices?v'
...
For the curious, I generated this data from http://www.json-generator.com/ so please ignore the actual values and semantics of the data as these are all randomly generated.
...
[source,sh]
--------------------------------------------------
mo@mwubuntu1:~$ curl 'localhost:9200/bank/_search?q=*&pretty'
{
  "took" : 63,
  "timed_out" : false,
  "_shards" : {
...
[source,sh]
--------------------------------------------------
mo@mwubuntu1:~$ curl -XPOST 'localhost:9200/bank/_search?pretty' -d '{
>   "query": { "match_all": {} }
> }'
{
  "took" : 26,
...
If you come from a SQL background, the above is somewhat similar in concept to the SELECT FROM field list.

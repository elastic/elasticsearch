...
on the <<gs-run-elasticsearch-cloud,Elasticsearch Service>>,
...
get a three node cluster. To run a three node {es} cluster locally, you can
install using the tar or zip archive:
...
You can also run {es} in a Docker container, install {es} using the DEB
or RPM packages on Linux, install using Homebrew on macOS, and install using the
MSI package installer on Windows. See <<install-elasticsearch>> for
information about all of the available installation options.

...
If installation succeeds, Homebrew will finish by saying that you can start Elasticsearch by entering `elasticsearch`. Do that now. The expected response is described below, under [`Successfully running node`]<<https://github.com/jrpool/elasticsearch/blob/patch-1/docs/reference/getting-started.asciidoc#successfully-running-node>>.

...
[discrete]
[[graph-explore-api-prereqs]]
==== {api-prereq-title}

The graph explore API is enabled by default.

To disable access to the graph explore API and the Kibana
{kibana-ref}/graph-getting-started.html[Graph UI], set `xpack.graph.enabled:

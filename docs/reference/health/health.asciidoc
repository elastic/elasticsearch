...
===== shards_capacity
`data`::
(map) A view with information about the current capacity of shards for data nodes that do not belong to the frozen layer.
+
.Properties of `data`
[%collapsible%open]
=====
...
`frozen`::
(map) A view with information about the current capacity of shards for data nodes that belong to the frozen layer.
+

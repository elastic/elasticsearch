...
index, which is something that can be controled using
<<index-modules-index-sorting,index sorting>>. For instance, say you are
storing tweets and users in the same index. Then sorting by document type
(tweet / user) will make it much more likely to compress whole blocks of
documents that only contain tweets or users. And this will improve the
compression ratio since those documents from the same type usually have the
same structure.

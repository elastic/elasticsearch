...
storage costs. For <<tsds-index-settings,time series (TSDS) indices>>, you can
use <<downsampling,downsampling>> to summarize and store historical data
at a fraction of the raw data's storage cost.

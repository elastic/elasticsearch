...
**Configure the script compilation circuit breaker**

If you frequently trigger the <<script-compilation-circuit-breaker,script
compilation circuit breaker>>, you can use the <<cluster-update-settings,update
cluster settings API>> to increase the
`script.context.$CONTEXT.max_compilations_rate` for one or more script contexts.

For example, the following request changes the maximum rate for the `field`
context to 100 compilations every 10 minutes.
...
If a node is using a high percentage of CPU, use the
<<cluster-nodes-hot-threads,nodes hot threads API>> to check for
resource-intensive threads running on the node. 
...
handle heavy workloads, add additional nodes to your cluster or upgrade your

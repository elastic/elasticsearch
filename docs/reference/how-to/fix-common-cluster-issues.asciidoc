...
{es} uses several types of circuit breakers. The following tips outline the
most common causes of circuit breaker errors and their solutions.
...
[source,console]
----
POST _all_/_cache/clear?fielddata=true
----
// TEST[s/^/PUT my-index\n/]
...
`spent` more than 50% (21 seconds) of the `last` 40 seconds in garbage
collection.
...
memory pressure. If possible, try using smaller requests and allow more time
between them.
...
segments into larger ones and free up disk space.
+
[source,console]
----
...
{ilm-init}'s <<ilm-shrink,shrink action>> to reduce its primary shard count and
free up space.
+
[source,console]
----

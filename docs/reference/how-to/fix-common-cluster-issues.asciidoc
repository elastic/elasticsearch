...
* Decrease the maximum number of aggregation buckets using the
...
primary shards or `r` for replicas. The `unassigned.reason` describes why the
...
cluster maintenance. If you forget to later re-enable allocation, {es} will be
unable to assign shards. To re-enable allocation, reset the
...
You can fix the settings using the <<indices-update-settings,update index
settings>> and <<cluster-update-settings,update cluster settings>> APIs.

**Allocate or reduce replicas**

To prevent data loss from hardware failure, {es} will not assign a replica to
the node containing its primary shard. If no other data nodes are available to
host the replica, it remains unassigned. To fix this, you can:

* Add a data node to host the replica.
...
If a primary shard is unassigned due to a lost node, you can typically recover
the data from a replica on another node. If no replicas exist and you can't
recover the node, you'll need to re-add the missing data from a

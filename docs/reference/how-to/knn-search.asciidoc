...
{es} the https://arxiv.org/abs/1603.09320[HNSW] algorithm for approximate kNN
search. HNSW is a graph-based algorithm which only works efficiently when most
vector data is held in memory. You should ensure that data nodes have at least
enough RAM to hold the vector data and index structures. To check the size of
the vector data, you can use the <<indices-disk-usage>> API. As a very loose
rule of thumb, and assuming the default HNSW options, the bytes required
roughly be `num_vectors * 4 * (num_dimensions + 32)`. Note that the RAM that's
required is for the filesystem cache, which is separate from the Java heap.
...
which also benefit from using filesystem cache. It's important to run
...
single segment can improve approximate kNN search latency. When there are

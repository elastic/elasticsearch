...
Fortunately, the `query_string` query has a feature that allows to solve exactly
this problem: `quote_field_suffix`. It allows to tell Elasticsearch that words
that appear in between quotes should be redirected to a different field, see
below:

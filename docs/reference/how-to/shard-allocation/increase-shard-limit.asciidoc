...
== An index's maximum number of shards that can be stored on one node has been reached
...
index setting to a custom value (for e.g. `1` in case of a highly traficced index).
...
include::{es-repo-dir}/tab-widgets/shards-allocation/total-shards-per-node-widget.asciidoc[]

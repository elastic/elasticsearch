...
==== Use data streams and {ilm-init} for timestamped data

<<data-streams,Data streams>> let you store timestamped data across multiple,
...
between 10GB and 50GB typically work well for logs and timestamped data. You
...
with similar mappings into a single large index. For timestamped data, you could

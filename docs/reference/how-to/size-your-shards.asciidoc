...
[discrete]
[[fields-heap-recommendation]]
==== The heap of data nodes should suffice for the fields of indices, plus overheads

You can use the <<cluster-nodes-stats,Nodes stats API>>'s `mappings` indices
statistic, to get the number of field mappings of indices and an estimation of
their heap overhead per node.

[source,console]
----
GET _nodes/stats/indices?human&filter_path=nodes.*.indices.mappings.total_estimated_overhead*
----
// TEST[setup:my_index]
...
may require more. You can use the <<cluster-nodes-stats,Nodes stats API>>'s
`jvm` statistic, to check the maximum amount of available heap memory per node.

[source,console]
----
GET _nodes/stats/jvm?human&filter_path=nodes.*.jvm.mem.heap_max*
----
// TEST[setup:my_index]

Ensure that the reported max heap memory for each node is 0.5GB more than the
node's field mappings' estimated heap overhead. If this is not the case, you
can <<set-jvm-heap-size,set the JVM heap size>> appropriately.

For example, if a data node reports that the estimated heap overhead for the
field mappings is 4GB, an additional 0.5GB of heap would need to be considered
for its workload and other overheads, and therefore this data node will need
a heap size of at least 4.5GB.

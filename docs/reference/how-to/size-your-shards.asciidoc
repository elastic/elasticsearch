...
2,147,483,519 (`(2^31)-129`) documents. This limit applies to the sum of
`docs.count` plus `docs.deleted` as reported by the <<indices-stats,Index stats
API>>. Exceeding this limit will result in errors like the following:
...
Use the <<indices-forcemerge,Force Merge API>> to merge away some deleted docs.
For example:
...
* <<docs-delete-by-query,deleting unneeded documents>>, or

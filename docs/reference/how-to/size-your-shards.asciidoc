...
Each mapped field of an index held by a data node has heap overhead. The heap of
a data node should suffice for its fields' heap overhead, plus other overheads.
You can use the <<cluster-nodes-stats,Nodes stats API>> to get two relevant metrics:

* The number of field mappings of indices and an estimation of their heap overhead
per node.
...
Apart from the field mappings, you must additionally allow enough heap for
{es}'s baseline usage as well as your workload such as indexing, searches and
...
In the example above, the estimated field mappings overhead is 1GB. Therefore,
the data node would need an additional 0.5GB of heap for its workload and other
overheads. Its 4GB heap max size is thus sufficient for the total required
heap of 1.5GB.

If the heap max size for a data node is not sufficient, consider

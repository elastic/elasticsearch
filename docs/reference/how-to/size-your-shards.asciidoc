...
every document it indexes, but you can switch off this behaviour to take
control of your mappings.
...
<<field-usage-stats>>, and you can analyze the disk usage of mapped fields
using the <<indices-disk-usage>>. Note however that unnecessary mapped fields
also carry some memory overhead as well as their disk usage.

...
A <<snapshot-restore,snapshot>> backs up your cluster and its data. The cold and
frozen tiers uses <<searchable-snapshots,searchable snapshots>> to reduce local
storage costs.
...
The steps for registering a snapshot repository vary based on your deployment
type and storage provider:
...
<<snapshot-lifecycle-management,{slm-init}>> uses a snapshot lifecycle policy to
automate the creation and deletion of snapshots. This ensures your data is
backed up on a regular basis.

To create a snapshot lifecycle policy in {kib}, open the main menu and go to
**Stack Management > Snapshot and Restore > Policies**. Click **Create Policy**.

You can also use the <<slm-api-put-policy,create snapshot lifecycle policy
API>>.

[source,console]
----
PUT /_slm/policy/my-snapshot-policy
{
  "schedule": "0 30 2 * * ?",
  "name": "<nightly-snapshot-{now/d}>",
  "repository": "my-snapshot-repository",
  "retention": {
    "expire_after": "750d",
    "min_count": 5,
    "max_count": 800
...
// TEST[s/my-snapshot-repository/my_repository/]

[discrete]
[[create-edit-index-lifecycle-policy]]
=== Step 4. Create or edit an index lifecycle policy
...
and more. See the {kib}'s {kibana-ref}/dashboard.html[Dashboard documentation].

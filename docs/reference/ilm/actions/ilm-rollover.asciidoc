...
`min_age`::
(Optional,  <<time-units, time units>>)
Prevents rollover until after the maximum elapsed time from index creation is reached.
See notes on `max_age`.

`min_docs`::
(Optional, integer)
Prevents rollover until after the specified maximum number of documents is reached.
...
See notes on `min_primary_shard_size`.

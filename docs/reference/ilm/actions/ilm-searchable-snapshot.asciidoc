...
different costs from the copying between nodes in your cluster that happens
with regular indices. These costs are typically lower, but in some environments
they may be higher. See <<searchable-snapshots-costs>> for more details.

...
step.
<4> What went wrong
<5> If the step before produces a `step_info` or the previous attempt to auto retry this step failed, this field will contain a copy of that previous attempt's `step_info` field for diagnostic purposes as `step_info` is overwritten during
each new attempt.

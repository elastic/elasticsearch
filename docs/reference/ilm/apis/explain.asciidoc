...
--------------------------------------------------
{
  "indices": {
    "my-index-000001": {
      ...
      "time_since_index_creation": "15s",       <1>
      "managed": true,                          <2>
      "policy": "my_policy",                    <3>
      "lifecycle_date_millis": 1538475653281,   <4>
      "age": "15s",                             <5>
      "phase": "new",
      "phase_time_millis": 1538475653317,       <6>
      "action": "complete",
      "action_time_millis": 1538475653317,      <7>
      "step": "complete",
      "step_time_millis": 1538475653317         <8>
    }
  }
}
--------------------------------------------------
...
<1> The time since the index creation (used for calculating when to rollover the index via the `max_age`)
<2> Shows if the index is being managed by {ilm-init}. If the index is not managed by
{ilm-init} the other fields will not be shown
<3> The name of the policy which {ilm-init} is using for this index
<4> The timestamp used for the `min_age`
<5> The age of the index (used for calculating when to enter the next phase)
<6> When the index entered the current phase
<7> When the index entered the current action
<8> When the index entered the current step

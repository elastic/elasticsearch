...
Since SLM falls under the same category as ILM, its operations are subject to
the running mode of ILM. SLM operations can be stopped and started by using the
<<start-stop-ilm,start and stop>> ILM APIs.

=== Put Snapshot Lifecycle Policy API

Creates or updates a snapshot policy. If the policy already exists, the version
in increamented. Only the latest version of a policy is stored.

When a policy is created it is immediately scheduled based on the schedule of
the policy, if an existing policy was already scheduled the previous policy is
cancelled and the updated policy is scheduled instead.
...
| `schedule` | A periodic or absolute time schedule. Supports all values
  supported by the <<schedule-cron-elements,cron scheduler>>.
...
| `config` | Configuration for each snapshot that will be created by this
  policy. Any configuration is included with create snapshot requests issued by
  this policy.
|==================
...
--------------------------------------------------
// TESTRESPONSE[s/"snapshot_name": "daily-snap-2019.04.24-gwrqoo2xtea3q57vvg0uea"/"snapshot_name": $body.snapshot_name/]
...
--------------------------------------------------
// TESTRESPONSE[s/"modified_date": "2019-04-23T01:30:00.000Z"/"modified_date": $body.daily-snapshots.modified_date/ s/"modified_date_millis": 1556048137314/"modified_date_millis": $body.daily-snapshots.modified_date_millis/ s/"next_execution": "2019-04-24T01:30:00.000Z"/"next_execution": $body.daily-snapshots.next_execution/ s/"next_execution_millis": 1556048160000/"next_execution_millis": $body.daily-snapshots.next_execution_millis/ s/"last_failure".*// s/"snapshot_name".*// s/"time_string".*// s/"time".*// s/"details".*// s/} ,//]
...
--------------------------------------------------
// TESTRESPONSE[s/"snapshot_name": "daily-snap-2019.04.24-tmtnyjtrsxkhbrrdcgg18a"/"snapshot_name": $body.snapshot_name/]
...
--------------------------------------------------
// TESTRESPONSE[s/"modified_date": "2019-04-23T01:30:00.000Z"/"modified_date": $body.daily-snapshots.modified_date/ s/"modified_date_millis": 1556048137314/"modified_date_millis": $body.daily-snapshots.modified_date_millis/ s/"next_execution": "2019-04-24T01:30:00.000Z"/"next_execution": $body.daily-snapshots.next_execution/ s/"next_execution_millis": 1556048160000/"next_execution_millis": $body.daily-snapshots.next_execution_millis/ s/"last_failure".*// s/"last_success".*// s/"snapshot_name".*// s/"time_string".*// s/"time".*// s/"details".*// s/} ,//]
...
any currently ongoing snapshots.

...
You want to send log files to an {es} cluster so you can visualize and analyze the data. This log data has the
...
The default `logs` policy is designed to prevent the creation of many
tiny daily indices. You can modify the policy to meet your performance requirements and manage resource usage.
...
.. Set *Move data into phase when* to *30 days*. This moves indices to the warm tier 30 days after rollover.

...
When you use a mechanism like Filebeat to continuously index timestamped documents into {es},
...
* The `delete` phase uses `min_age` to determine when the index should be removed.
...
<4> Trigger the `delete` action when the index enters the delete phase. No other conditions need to be met.
...
<3> The name of the index alias used to reference these indices. 

...
[discrete]
[[manage-time-series-data-without-data-streams]]
=== Manage timestamped data without data streams

Even though <<data-streams, data streams>> are a convenient way to scale
and manage timestamped data, they are designed to be append-only. We recognise there
...
In these cases, you can use an index alias to manage indices containing the timestamped data
and periodically roll over to a new index.

To automate rollover and management of timestamped indices with {ilm-init} using an index
alias, you:
...
=== Bootstrap the initial timestamped index with a write index alias

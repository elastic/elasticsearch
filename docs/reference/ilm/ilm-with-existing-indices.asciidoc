...
You can create a policy for existing indices by removing the `rollover` action,
and in this case, the `hot` phase is now empty so we can remove that too:
...
First, we'll set up a template with rollover, and can include any additional
...
At this point, new indexing should be paused to ensure that all existing data is
reindexed properly. Adding new data to existing indices during this process can
cause data to be added to the existing indices, but not be reindexed into the
new indices.
...
documents will retain their original IDs. One way to do this is to use a script
with the reindex script to append the original index name to the document ID.

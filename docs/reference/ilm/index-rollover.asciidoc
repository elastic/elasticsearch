...
When indexing timestamped data like logs or metrics, you can't write to a single index indefinitely. 
...
We recommend using <<indices-create-data-stream, data streams>> to manage timestamped
...
your timestamped data using <<aliases,index aliases>>. However, there are a few

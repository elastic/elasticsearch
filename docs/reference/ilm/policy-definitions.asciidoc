...
of discrete steps that are executed. These steps can be thought of as distinct
API calls to Elasticsearch. Since some {ILM} actions are more complex and involve
multiple actions against an index, each of these are done in their own step. This
provides better visibility into the execution of the phases. The
<<ilm-explain-lifecycle,Explain Lifecycle API>> exposes this information to us
to see which step our index is either to execute next, or is currently executing.

...
`index.auto_expand_replicas`::
    ...
    Auto-expand the number of replicas based on the number of available data nodes.
    Note that this does not take <<shard-allocation-filtering,shard allocation filtering rules>>
    into account and can lead to a yellow cluster health.

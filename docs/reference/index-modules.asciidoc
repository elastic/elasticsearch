...
[[index-mode-setting]] `index.mode`::
    ...
    preview::["The `logs` index mode is in technical preview and may be changed or removed in a future release. Elastic will work to
    ...
    sorting and other parameters whose value affects indexing or query performances.
    ...
    `time_series`::: Controls time series indexing and is documented in <<tsds-index-settings,TSDS Settings>>

    `logs`::: Controls indexing of logs. It applies default sort settings on the `hostname` and `timestamp` fields (tech preview feature).

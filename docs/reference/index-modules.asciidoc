...
[[index-codec]] `index.codec`::
    ...
    To change this setting you need to <<indices-open-close,close>>, update,
    <<indices-open-close,reopen>> and then issue a <<indices-forcemerge,force merge>>
    on the index in question for it to be applied.
    For example:

...
[[index-codec]] `index.codec`::
    ...
    have shown that `best_compression` gives up to ~28% lower storage usage in
    the most ideal scenario compared to `default` while affecting get by id
    latencies up to 10%. Additionally experiments have shown that indexing
    throughput is also better (up to ~10%).

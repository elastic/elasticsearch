...
[[index-migration-ignored]] `index.ignore_migration_reindex_while_readable`::

    Used by the <<migration-api,Migration APIs>> and the Kibana *Upgrade Assistant* feature
    to confirm the user has reviewed this index and are comfortable with the index in question
    becoming read only when opened in the next major version. If set to `true`, all reindex
    warnings will be suppresed for this index while the index remains readable in the next
    major version. Defaults to `false`.

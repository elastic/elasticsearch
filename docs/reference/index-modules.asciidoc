...
`logsdb`::: _(https://www.elastic.co/pricing[Enterprise subscription] required)_ +
Index mode optimized for storage of logs. By default, this setting uses <<synthetic-source,synthetic `_source`>> and sorts on the `hostname` and `timestamp` fields. You can override the defaults to <<index-modules-index-sorting,sort>> on different fields.

...
[[disable-fsync-operations]]
=== Disabling fsync operations

NOTE: This is an expert setting, using this setting in environments that rely on
disk persistence can lead to data corruption.

By default, Elasticsearch persists writes to disk using the fsync operation.
In environments that do not rely on the persistence of disk writes, the use of the
fsync operation can be disabled by setting `node.store.use_fsync` to `false`.
Disabling the fsync operation can improve the latency and throughput of indexing
in some cases. This setting should only be used if the environment does not rely on
the persistence of disk writes during events like power outages. An example would be
a cluster running nodes in Docker containers that do not use persisted volumes and
that are not reused across system restarts.

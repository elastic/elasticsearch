...
All Elasticsearch APIs that enable you to target one or more indices also
support index aliases. An index alias maps a unique name to one or more
indices. When you invoke an API against an index alias, Elasticsearch
automatically expands the alias and performs the operation against each of the
mapped indices.

You can use aliases to:

* Transparently switch between indices on a live cluster
* Group multiple indices, such as the last month's worth of daily indices
* Target different subsets of a single index

You create and manage index aliases through the _aliases endpoint. For example,
to create an alias that maps the alias alias1 to the index test1, you use the
`add` action:
...
[horizontal]
`index`::   The index the alias refers to. Can be any of `* | _all | glob
pattern | name1, name2, …`

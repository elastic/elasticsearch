...
`action_results`::
(array of objects) Results for each requested action.
+
...
`action`::
(string)
The type of the associated action, the values will be one of `add`, `remove`, or `remove_index`.

`success`::
(boolean)
If true, the requested action was successfully applied to at least one alias.

`error`::
(Optional, string)
Only present if `success` is `false`. Consists of a message describing why the action failed
====

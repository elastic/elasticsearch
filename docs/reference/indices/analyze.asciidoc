...
Performs the <<analysis,analysis>> process on a text
...
If the `analyzer` and `field` parameters are not specified,
...
the analyze API uses the analyzer for the `field`'s mapping.

...
with `_all` or wildcard expressions, the `action.destructive_requires_name`
cluster setting must be `true`. You can update this setting in the
`elasticsearch.yml` file or using the

...
WARNING: **Be careful when force merging an index with active updates or
deletes.** Usually when documents are updated or deleted, the old 'tombstone'
documents are automatically cleaned up during regular segment merges. But
force merge can cause very large (> 5GB) segments to be produced, which are
not eligible for regular merges. The number of 'tombstone' documents can then
grow rapidly, resulting in higher disk usage and worse search performance.
You should only force merge if an index has very few updates. This includes
read-only indices and append-only indices, which have no document updates at
all.

...
`force_merge` threadpool>>. This means that the shard-level operations are
executed in parallel on each node if its `force_merge` threadpool has multiple
threads. Force merge makes the storage for the shard being merged temporarily

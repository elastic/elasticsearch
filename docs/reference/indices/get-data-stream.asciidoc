...
+
The health status is based on the state of the backing indices' primary and
replica shards.
+
.Values for `status`
[%collapsible%open]
...
`ilm_policy`::
(string)
Name of the current {ilm-init} lifecycle policy in the stream's matching
index template.
+
...
policies. To retrieve the lifecycle policy for an individual backing indices,

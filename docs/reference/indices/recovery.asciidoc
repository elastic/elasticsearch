...
`ON_DEMAND`::
A completed recovery that will pull index files on demand as
searches hit the shard. It's also possible that the files
will be evicted as the underlying cache is filled.

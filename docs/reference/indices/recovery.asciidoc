...
`ON_DEMAND`::
A completed recovery that pulls files on demand as
searches hit the shard. The recovery statistics for the
shard can vary up or down depending on the inner
workings of the file cache.

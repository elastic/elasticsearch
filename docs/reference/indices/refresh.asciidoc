...
A _refresh_ makes all operations performed on one or more indices available
for search since the last refresh.
For data streams, the API runs the operation on the stream's backing indices.
...
indices available for search since the last refresh.
If the request targets a data stream, it refreshes the stream's backing indices.
For background and underlying datastructure see the documentation for

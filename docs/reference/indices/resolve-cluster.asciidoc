...
4. whether the search is likely to have errors returned when you do the {ccs}
...
to be thrown when do a search. In these cases, the "error" field in the `_resolve/cluster` response
...
----
// TESTRESPONSE[s/"number": "8.14.0"/"number": "$body.$_path"/]
...
[discrete]
[[resolve-index-api-error-example]]
...
was added (8.14.0), so you may want to exclude it from your {ccs}. (Note: it was able to tell there
were matching indices because it did a fallback to use the <<indices-resolve-index-api,resolve index>>
API.)

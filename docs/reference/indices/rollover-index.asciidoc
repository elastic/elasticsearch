...
`logstash-2016.02.03`.  The rollover API supports date math by storing the
original `provided_name` in the index settings.  For instance:
...
--------------------------------------------------
// CONSOLE
// TEST[skip:Requires 24h to pass]
<1> Creates an index named (e.g.) `logs-2016.10.31`
<2> Rolls over to a new index with today's date, e.g. `logs-2016.11.01`

[WARNING]
=============================================

Using a date format like `YYYY.MM.dd` for an index name will only work once a
day.  The second attempt to use the rollover API will fail because the index
already exists.  If you may need to rollover your indices more than once a
day, then use a date format with more precision, for example:

[source,js]
--------------------------------------------------
PUT /<logs-{now{YYYY.MM.dd-hh.mm.ss}}> <1>
{
  "aliases": {
    "logs_write": {}
  }
...
--------------------------------------------------
// CONSOLE
<1> Creates an index named (e.g.) `logs-2016.10.31-16:12:15`
=============================================

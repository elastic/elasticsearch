...
`lazy`::
(Optional, Boolean)
If `true`, signals that the data stream, applies only to data streams, needs to be
rolled over when the next indexing operation occurs. This only applies when there
are no `conditions` provided. Defaults to `false`.

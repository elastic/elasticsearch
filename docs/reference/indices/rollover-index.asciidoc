...
<<indices-aliases, index alias>> or a <<indices-datastreams, data stream>>. When
targeting an alias, the alias is updated to point to the new index. When
targeting a data stream, the new index becomes the data stream's write index and

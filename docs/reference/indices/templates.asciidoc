...
<1> the optional `version` field is unused by elasticsearch
internally, but can be useful to keep track of which
version of the template is currently active on the cluster.

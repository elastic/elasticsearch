...
`index.minimum_shard_size.size`::
    ...
    shard size allocation function.  Invalid if not specified with
    `index.minimum_shard_size.timeout`.  This should prevent the shard size
    allocation function from causing many index moves when an index is being
    built quickly.
`index.minimum_shard_size.timeout`::
    Time after which the `index.minimum_shard_size.size` is no longer used.
    Specified as an https://en.wikipedia.org/wiki/ISO_8601[ISO 8601] date with
    optional time and time zone.  The default time zone is UTC.  Stay sane.
    Specify it as YYYY-MM-DDThh:mm:ssZ.

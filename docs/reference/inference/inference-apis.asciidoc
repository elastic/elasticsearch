...
Each chunk contains one or more complete sentences -  except if a single sentence is longer than the value of `max_chunk_size` -, ensuring that the integrity of sentence-level context is preserved.

...
used for abuse detection.
=======
For `openai` and `azureopenai` service only. Specifies the user issuing the request, which can be used for abuse detection.
=====
+
.`task_settings` for the `completion` task type
[%collapsible%closed]
...
>>>>>>> c5d724dcc1c (Update docs for Azure OpenAI Embeddings inference)
=====

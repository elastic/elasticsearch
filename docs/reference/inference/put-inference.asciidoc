...
You can enable adaptive allocations for inference services (<<infer-service-elser,ELSER>>, <<infer-service-elasticsearch,E5 and models uploaded via Eland>>). When adaptive allocations are enabled:

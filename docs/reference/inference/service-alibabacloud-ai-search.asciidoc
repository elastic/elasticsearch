...
For the supported `text_embedding` service_id, refer to the https://help.aliyun.com/zh/open-search/search-platform/developer-reference/text-embedding-api-details[documentation].

The service_id available for the `sparse_embedding` task is:
...
The service_id available for the `rerank` task is:

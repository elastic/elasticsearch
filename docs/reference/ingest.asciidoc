...
The following <<put-pipeline-api,put pipeline API>> request creates a pipeline
containing <<set-processor,`set`>> processors followed by a
<<lowercase-processor,`lowercase`>> processors. The processors run sequentially
...
documents in {kib}'s **Ingest Nodes Pipelines** feature. When creating or
editing a pipeline, click **Add documents**. In the **Documents** tab, provide
sample documents and click **Run the pipeline**.
...
IMPORTANT: `if` condition scripts run in the Painless's
...
ingest statistics. Use these stats to determine which pipelines run most and
spend the most time running.

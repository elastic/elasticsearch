...
If left untouched, this pipeline call has no effect on your data.
However, this pipeline call can be used to create custom pipelines for integrations that persist through upgrades.
See {fleet-guide}/data-streams-pipeline-tutorial.html[Tutorial: Transform data with custom ingest pipelines] to learn more.

{fleet} doesn't provide a default ingest pipeline for the **Custom logs** integration.
You can specify a pipeline for this integration in one of two ways: an

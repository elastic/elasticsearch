...
WARNING: Do not change {fleet}'s ingest pipelines or usecustom pipelines for
...
You can safely specify a pipeline for this integration in one of two ways:

**Option 1: Index template**

. Create an <<index-templates,index template>> that includes
<<set-default-pipeline,pipeline index settings>>. You can create this template
using {kib}'s <<manage-index-templates,**Index Management**>> feature or the
<<indices-put-template,create index template API>>. Ensure the index template is
<<create-a-data-stream-template,data stream enabled>>.
+
--
To avoid naming collisions, the template's index pattern should match
`logs-<dataset-name>-*`.
...
PUT _component_template/my-settings-component-template
{
  "template": {
    "settings": {
...
PUT _index_template/template_1
{
  ...
  "data_stream": { },
  "priority": 500,
  "composed_of": ["my-settings-component-template", "my-mappings-component-template"]
}
----
// TEST[s/, "my-mappings-component-template"//]
--

. When adding or editing your **Custom logs** integration,
click **Configure integration > Custom log file > Advanced options**.

. In **Dataset name**, specify your dataset's name. {fleet} will
add new data to the resulting `logs-<dataset-name>-default` data stream.
+
...
. In **Custom Configurations**, specify `pipeline: <my-pipeline>`.
+
[role="screenshot"]
...
<<index-templates,index template>> that includes <<set-default-pipeline,pipeline
index settings>>. You can also specify `pipeline: <my-pipeline>` in your
`elastic-agent.yml` configuration. See
{fleet-guide}/run-elastic-agent-standalone.html[Run {agent} standalone].

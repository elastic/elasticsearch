...
When you create Elasticsearch indices for search use cases, for example, using the {enterprise-search-ref}/crawler.html[web crawler^] or <<es-connectors,connectors>>.
, these indices are automatically set up with specific ingest pipelines.

...
If you use {fleet}'s **Custom logs** integration, you can specify a pipeline to
use. When adding or editing a **Custom logs** integration, click **Configure
integration > Custom log file > Advanced options**. In **Custom
Configurations**, specify `pipeline: <my-pipeline>`.

[role="screenshot"]
image::images/ingest/custom-logs-pipeline.png[Add pipeline for custom log file,align="center"]

If you use run {agent} standalone, specify `pipeline: <my-pipeline>` in your

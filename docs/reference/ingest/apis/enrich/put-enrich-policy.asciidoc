...
`range`:::
Matches a field value in incoming documents to a range in the enrich
index using a <<query-dsl-term-query,`term` query>>.
+
To match, incoming documents should contain a number, date, or IP address in the
`field` specified in the <<enrich-processor,`enrich` processor>>. The enrich
policy's `match_field` should be a <<range,range>> field of the same data type.
For example, if incoming documents contain a date value in the `field`, the
policy's `match_field` should be a `date_range` field. Matching a range to a
range is not supported.
+
For an example, see <<range-enrich-policy-type>>.
--
+

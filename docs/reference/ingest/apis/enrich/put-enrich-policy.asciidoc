...
<<numeric,numeric>>, <<date,`date`>>, <<date_nanos,`date_nanos`>>, or
<<ip,`ip`>> field. The policy's `match_field` must be a <<range,range>> field of
the same data type.
+
To match documents, the policy uses the <<query-dsl-term-query,`term` query>>.

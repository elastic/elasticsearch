...
* *Field*, the same match field defined in your enrich policy.
* The *target field*, a field added to the incoming document
that contains all appended enrich data.

The following request adds a new pipeline, `user_lookup`.
This pipeline includes an enrich processor
which uses the `users-policy` enrich policy.
...
The enrich processor appends all data as an array.
If the incoming document matches more than one document in the enrich index,
the processor appends the field data from those documents is added to the array.

...
The full power of the <<modules-scripting-painless, Painless Scripting Language>> ability to declare `tag` and `on_failure` fields. These fields are optional
...
The following document is indexed (e.g. not dropped) since
...
Accesses a read only version of the document via the same `ctx` variable used by scripts in the

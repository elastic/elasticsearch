...
[[size-processor]]
=== Size Processor
Sets the size of the current document as it will be rendered as a JSON document.
...
|======
| Name                  | Required | Default      | Description
| `target`              | no       | `_meta.size` | The target field the size will be stored in
| `consider_size_field` | no       | `false`      | Should the generated `size` field be taken into account when computing size field.
|======

For example the simplest definition for size processor:
...
For example the following size processor will generate a field `my_object.my_size` and will take
into account this `my_object.my_size` field itself when computing the JSON document size:

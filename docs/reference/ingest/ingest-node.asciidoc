...
[[size-processor]]
=== Size Processor
Sets the size in bytes of the current document as it will be rendered as a JSON document.
...
|======
| Name                  | Required | Default      | Description
| `target`              | yes      | -            | The target field the size will be stored in
|======

For example the following size processor will generate a field `my_object.my_size`:
...
NOTE: that the computed size does not contain the addition of the size field itself.

...
Script Processor allows inline, stored, and file scripts to be executed within Ingest Pipelines.

Scripts use Elasticsearch's Script Service to compile the processor's script for each document. This
allows updates to stored scripts and the changes will be reflected upon the next run of the processor. The
downside for doing this is that the processor relies on these script's compiled sources be cached. Be aware
of the size of your script cache's max size. You can increase the size by updating the `script.cache.max_size` settings
option.

...
| `skip_failures`  | no        | `false`  | If `true` and some values of the `field` were failed during conversion, the processor will skip the error and continue with next item in the array

...
| `field`   | yes       | -        | The field to expand into an object field. If set to `*`, all fields will be expanded.
...
| `override`| no        | false    | Controls the behavior when there is already an exising nested object that conflicts with the expanded field.
                                     When set to `false`, the processor will merge conflicts by turning appending the expanded value for array values or by turning scalar values into an array that holds both the old and the new value.
                                     When set to `true`, the value from the expanded field will override the existing value.
...
The field can also be set to a `*` to expand all dotted field names:

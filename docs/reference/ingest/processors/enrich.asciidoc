...
used to match the <<geo-shape,geoshape>> of incoming documents
to documents in the enrich index.
Valid operators are:

* `INTERSECTS`:
Match documents whose geoshapes intersect.
* `DISJOINT`:
Match documents whose geoshapes do not intersect at all.
* `WITHIN`:
To match, the geoshape of an incoming document
must be completely within the geoshape of a document
in the enrich index.

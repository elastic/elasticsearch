...
`isp`, `isp_organization`, `mobile_country_code`, `mobile_network_code`, and `connection_type`. The fields actually added depend on
what has been found and which properties were configured in `properties`.

...
`-ASN.mmdb` extensions. On {ess} deployments, custom database files must be uploaded using
a {cloud}/ec-custom-bundles.html[custom bundle]. On self-managed deployments copy the database files to `$ES_CONFIG/ingest-geoip`.

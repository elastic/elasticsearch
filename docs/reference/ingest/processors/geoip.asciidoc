...
uncompressed and the extention must be `-(City|Country|ASN).mmdb`, but they must not have the same filename as one of the databases the module ships with. The `ingest-geoip` config directory is located at `$ES_CONFIG/ingest-geoip`.

...
* If the GeoIP2 ISP database is used, then the following fields may be added under the `target_field`: `ip`, `isp`, `isp_organization`,
...
`isp`, `isp_organization`, `mobile_country_code`, and `mobile_network_code`.

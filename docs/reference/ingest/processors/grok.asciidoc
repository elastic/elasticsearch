...
| `ecs_compatibility`    | no        | `disabled`          | Must be one of `disabled` or `v1`. When `v1`, the processor will use patterns with field names as defined by the {ecs-ref}/ecs-event.html[Elastic Common Schema (ECS)].
...
Specifying the optional `ecs_compatibility` query parameter with a value of `v1` lists the patterns that use field
names as defined in the {ecs-ref}/ecs-event.html[Elastic Common Schema (ECS)]. By default, the parameter uses the
value of `disabled` which lists the legacy patterns.

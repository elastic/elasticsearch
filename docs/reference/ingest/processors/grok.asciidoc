...
| `ecs_compatibility`    | no        | `v1`                | Must be `disabled` or `v1`. If `v1`, the processor uses patterns with {ecs-ref}/ecs-field-reference.html[Elastic Common Schema (ECS)] field names.
...
By default, the API returns patterns that use
{ecs-ref}/ecs-field-reference.html[Elastic Common Schema (ECS)] field names.
To return legacy patterns that predate ECS, specify `disabled` in the optional
`ecs_compatibility` query parameter.

[source,console]
----
GET _ingest/processor/grok?ecs_compatibility=disabled
----

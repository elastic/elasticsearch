...
| `add_to_root_merge_strategy` | no        | `replace` | When set to `replace`, root fields that conflict with fields from the parsed json will be overridden. When set to `recursive`, conflicting fields will be recursively merged. Only applicable if `add_to_root` is set to `true`.
| `allow_duplicate_keys`       | no        | false     | When set to `true`, the JSON parser will not fail if the JSON contains duplicate keys. Instead, the latest value wins. Allowing duplicate keys also improves execution time.

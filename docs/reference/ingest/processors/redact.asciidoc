...
a number of useful predefined https://github.com/elastic/elasticsearch/blob/{branch}/libs/grok/src/main/resources/patterns/ecs-v1[patterns],
...
surround the pattern name are also configurable.
...
Grok patterns from the base definitions to build complex patterns.
Add new patterns to the processor configuration with the
`pattern_definitions` option.

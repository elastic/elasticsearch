...
ingestion. Here is an example of an Ingest Pipeline that renames the index to `my-index-000001` no matter what
was provided in the original index request:

[source,console]
--------------------------------------------------
PUT _ingest/pipeline/my-index-000001
{
  "description": "use index:my-index-000001",
  "processors": [
    {
      "script": {
        "source": """
          ctx._index = 'my-index-000001';
        """
      }
    }
  ]
...
[source,console]
--------------------------------------------------
PUT any_index/_doc/1?pipeline=my-index-000001
{
  "message": "text"
}
--------------------------------------------------
...
In the above response, you can see that our document was actually indexed into `my-index-000001` instead of

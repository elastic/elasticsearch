...
The processor parses each incoming document's JSON source fields into a set of
maps, lists, and primitives. To access these fields with a Painless script, use
the {painless}/painless-operators-reference.html#map-access-operator[map access
operator]: `ctx['my-field']`. You can also use the shorthand `ctx.<my-field>`
syntax.

NOTE: The processor does not support the `ctx['_source']['my-field']` or
    ...
    "processors": [
      {
        "script": {
          ...
          "lang": "painless",
          "source": """
            String[] envSplit = ctx['env'].splitOnToken(params['dataset']);
            ArrayList tags = new ArrayList();
            tags.add(envSplit[params['dataset']].trim());
            ctx['tags'] = tags;
          """,
          "params": {
            "delimiter": "-",

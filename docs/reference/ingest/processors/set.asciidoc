...
| `ignore_empty_value` | no | `false` | If `true` and used in combination with `value` that is a <<template-snippets,template snippet>> which evaluates to `null` or a empty string, the processor quietly exits without modifying the document. If used in combination with `copy_from` it will quietly exit if the field do not exist or its value evalutes to `null`.

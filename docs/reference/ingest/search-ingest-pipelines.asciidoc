...
The *Content* UI under *Search* has a set of tools for working with ingest pipelines for indices created by the Elastic web crawler and Elastic connectors.
...
Aside from the pipeline itself, *Search* provides a few configuration options which control individual features of the pipelines.
...
As soon as *Search* starts, this pipeline is automatically available to use.
...
It's possible to add one or more ML inference pipelines to an index in *Search*.
...
This provides users a convenient hook from which to add their own custom processing and transformations for their *Search* data.
...
* `ent-search-generic-ingestion` - Since 8.5, Native Connectors, Connector Clients, and new (>8.4) Elastic web crawler indices will all make use of this pipeline by default.
  A natural evolution from `app_search_crawler` and `ent_search_crawler`, this pipeline is designed to meet more generic needs across *Search*.
...
*Q*: The Elasticsearch docs note that there are three types of ingest pipelines.
  ...
  Which does *Search* use? +
*A*: *Search* utilizes <<add-pipeline-to-indexing-request,Request Pipelines>>.
  This way, whether you are using Elastic web crawler, a Connector, or an Elasticsearch API, it is explicit when a pipeline is part of the indexing request.

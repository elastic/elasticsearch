...
[discrete]
[[elasticsearch-intro-documents-fields-next-step]]
==== Next step

Now that you understand the basics of {es}, you're ready to spin up an {es} instance.
We recommend starting with a <<run-elasticsearch-locally,local development setup>>.
Refer to <<elasticsearch-intro-deploy, deployment options>> if you'd prefer to start with a hosted solution.

Of course, if you already have {es} running, you can skip ahead to <<es-ingestion-overview,ingestion options>>!

...
If you'd prefer to start working with {es} right away, then jump to our <<quickstart,quickstarts>> for hands-on code examples.
...
If you're working with timestamped data, we recommend the {observability-guide}[Elastic Observability] solution.
====
...
* <<mapping-dynamic, Dynamic mapping>>: Let {es} automatically detect the data types and create the mappings for you. This is great for getting started quickly, but can yield suboptimal results for complex data due to automatic field type inference.
...
[TIP]
====
<<esql,{esql}>> is our new piped query language and compute engine that is initially mainly focused on timestamped data like logs and metrics.

...
If you'd prefer to start working with {es} right away, set up a <<run-elasticsearch-locally,local dev environment>> and jump to our <<quickstart,hands-on code examples>> .
...
*{esql}* is our new piped query language and compute engine which was first added in version *8.11*.

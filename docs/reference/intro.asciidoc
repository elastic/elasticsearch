...
In case of failure, {es} also offers tools for cross-cluster replication and cluster snapshots that can
...
If you’re exploring {es} for the first time, then you can use a cluster with a single node and create indices
...
monitoring, and administrative features that are integrated into {es} enable you to use https://www.elastic.co/guide/en/kibana/8.15/introduction.html[Kibana] as a

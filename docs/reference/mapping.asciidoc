...
* a simple type like <<text,`text`>>, <<keyword,`keyword`>>, <<date,`date`>>,
  <<number,`long`>>, <<number,`double`>>, <<boolean,`boolean`>> or <<ip,`ip`>>.
...
purposes. For instance, a `string` field could be mapped as a <<text>> field
for full-text search, and as a <<keyword>> field for sorting or aggregations.
Alternatively, you could index a text field with the
<<analysis-standard-analyzer,`standard` analyzer>>, the

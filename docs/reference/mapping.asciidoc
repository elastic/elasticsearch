...
IMPORTANT: In Elasticsearch versions 2.0 - 2.3, dots are not permitted in field names.
Elasticsearch 2.4 adds a setting to allow dots, but this setting should be
used with caution.  See <<dots-in-names>> for more information.
...
include::mapping/dots-in-names.asciidoc[]

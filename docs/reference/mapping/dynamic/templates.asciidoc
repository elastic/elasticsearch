...
Although the default <<match-mapping-type,dynamic mapping rules>> map an
`integer` JSON type to a `long` type in {es}, the following dynamic template
named `integers` maps any new `long` fields as runtime fields of type
`integer`:

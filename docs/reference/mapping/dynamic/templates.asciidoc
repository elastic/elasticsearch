...
template that maps new strings as runtime fields of type `ip`. When {es}
detects new `string` fields matching the `ip*` pattern, it maps those fields as
runtime fields of type `ip`.

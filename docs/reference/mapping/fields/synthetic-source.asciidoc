...
Range field vales (e.g. `long_range`) are always represented as inclusive on both sides with bounds adjusted accordingly. For example:
[source,console,id=synthetic-source-range-example]
----
PUT idx/_doc/1
{
  "my_range": {

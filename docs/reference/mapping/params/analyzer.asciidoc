...
=== `index_default_analyzer`
This is how to specify a default analyzer for an index:
...
--------------------------------------------------
// CONSOLE
<1> The name `default` is what's important in order for this analyzer to be set as the index default.
<2> The type `english` is specified. Since no stop words or stemming are specified, it will use the default stop words for the `english` analyzer.
<3> This returns the tokens: [ `quick`, `brown`, `fox` ].

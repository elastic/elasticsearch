...
field as a runtime field in the mapping. The following runtime script uses
the grok operator, which extracts structured fields out of a single text
...
value of the `message` field. If the pattern doesn't match
...
----
// TEST[continued]
<1> This condition ensures that the script returns the field value even if the
pattern of the log data doesn't match.
...
If the script didn't include this condition, the result wouldn't include any
documents. By including this condition, the script still returns documents if
there's data that doesn't match the grok pattern.
...
<<dissect-processor,Dissect patterns>> are similar to grok patterns, but they
don't accept regular expressions. This difference allows dissect’s syntax to be
simple and for some cases faster than grok.

You can use dissect to achieve the same results as a grok pattern, such as the
<<runtime-examples-grok,`clientip` grok pattern>>. Instead of matching on a log
...
Similarly, you can define a dissect pattern to discard everything in the log
message except the HTTP response:
...
----
// TESTRESPONSE[s/\.\.\./"took" : $body.took,"timed_out" : $body.timed_out,"_shards" : $body._shards,/]
// TESTRESPONSE[s/"_id" : "hNEhyncBRSB6iD-PoBqe"/"_id": $body.hits.hits.0._id/]

...
Runtime fields with a `type` of `lookup` allows retrieving fields from
...
POST logs/_search
{
  "runtime_mappings": {
    "location": {
        ...
        "index": "ip_location", <2>
        "query_type": "term", <3>
        "query_input_field": "host", <4>
        "query_target_field": "ip", <5>
        "fetch_fields": ["country", "city"], <6>
        "max_match_size": 1 <7>
    }
  },
  "fields": [
...
----
<1> Define a runtime field in a search request with a type of `lookup` that retrieves fields from the lookup index using the <<search-multi-search,Multi search API>>
<2> The lookup index where the lookup query executes against
<3> The type of the lookup query. Only the search type of <<query-dsl-term-query, `term`>> is currently supported
<4> A field on the searching index whose values are used as the input values of the lookup query
<5> A field on the lookup index which the lookup query searches against
<6> A list of fields to retrieve from the lookup index. See the <<search-fields, `fields`>> parameter of a search request.
<7> An optional parameter to control the maximum number of matching documents from the lookup query. This value must be between 1 and 100.
...
of each document from the lookup index.

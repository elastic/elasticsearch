...
[[runtime-search-flattened]]
==== Retrieve data from flattened fields
You can retrieve values from sub-fields of
<<search-fields-flattened,flattened fields>> with a runtime field. Instead of
including `doc['<field_name>'].value` in your Painless script, use
...
the `doc['<field_name>'].value` format. The `release_label` runtime field
includes a script that retrieves values from the `labels.release` field:

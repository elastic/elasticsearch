...
Runtime fields can be updated at any point in time: whenever a runtime field
is added with the same name as an already existing field, the new field will
overwrite the existing one.
Existing runtime fields can also be removed from the index mappings by setting
them to null as follows:

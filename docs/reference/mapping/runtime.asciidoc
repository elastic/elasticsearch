...
POST logs/_search
{
  "runtime_mappings": {
    "location": {
        ...
        "query_input_field": "host", <3>
        "query_target_field": "ip", <4>
...
----
<1> Define a runtime field in the main search request with a type of `lookup` that retrieves fields from the lookup index using the <<query-dsl-term-query, `term`>> queries.
<2> The lookup index where the lookup query executes against
<3> A field on the main index whose values are used as the input values of the lookup query
...
of each document from the lookup index. The lookup query is expected to
match at most one document on the lookup index. If more than one documents
match, then a random document will be selected.

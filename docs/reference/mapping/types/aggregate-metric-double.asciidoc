...
Finally, an `aggregate_metric_double` field can be used in scripts and the following queries
for which it behaves as a `double` by delegating its behavior to its `default_metric` sub-field:

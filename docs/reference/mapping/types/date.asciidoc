...
If a format or date value is incomplete, {es} replaces any missing components
with the following values. {es} cannot replace missing year components.
...
For example, if the format is `yyyy-MM`, {es} converts a dateq of `2020-12`
to `2020-12-01T23:59:59.999_999_999Z`. This date uses the provided year (`2020`)

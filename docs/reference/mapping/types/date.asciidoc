...
For <<query-dsl-range-query,range>> queries and
<<search-aggregations-bucket-daterange-aggregation,date range>> aggregations,
{es} replaces missing date components with the following values. Missing year

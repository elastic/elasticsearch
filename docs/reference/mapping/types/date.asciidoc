...
[WARNING]
====
Avoid using a decimal point in date values.
Dates
// tag::decimal-warning[]
accept numbers with a decimal point, such as `{"date": 1618249875.123456}`,
but there are some cases ({es-issue}70085[#70085]) where they'll lose precision.
// end::decimal-warning[]
====

...
To better accommodate scaling and performance needs, updating the `type` setting in `index_options` is possible according to the following graph (jumps allowed):
...
* updating `type` from `hnsw` to `int8_hnsw` is possible when:
** the `m` setting is updated to the same or larger value
** the `confidence_interval` setting is either unspecified in the original mapping or it doesn't get changed
* updating `type` from `int8_hnsw` to `int8_hnsw` is possible when:
** the `m` setting is updated to the same or larger value
** the `confidence_interval` setting is either unspecified in the original mapping or it doesn't get updated
* updating `type` from `int4_hnsw` to `int4_hnsw` is possible when:
** the `m` setting is updated to the same or larger value
** the `confidence_interval` setting is left untouched
...
Changing the `type` to `int4_hnsw` makes sure vectors indexed after that will use an int4 scalar quantized representation and HNSW (e.g., for KNN queries).
...
For debugging purposes, it's possible to inspect how many segments (and docs) exist for each `type` used with the <<index-segments-api-request,Index Segments API>>.

...
The `dense_vector` field type stores dense vectors of values defined
by `element_type` with `float` as a default. Dense
...
The `dense_vector` type does not support aggregations or sorting. When
`element_type` is `byte` <<query-dsl-script-score-query,`script_score`>>
is not supported.
...
substantially smaller index size with the trade off of significantly lower

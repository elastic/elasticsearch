...
to perform cosine similarity. In order to use it when `element_type` is `float`,
all vectors must be of unit length, including both document and query vectors.
In order to use it when `element_type` is `byte`, all vectors must have the same
length or results will not be accurate. The document `_score` is computed as

...
positive and that a larger score corresponds to a higher ranking. Defaults to `dot_product`.
+
...
by `element_type`.
+
When `element_type` is `float`, all vectors are automatically converted be unit length, including both

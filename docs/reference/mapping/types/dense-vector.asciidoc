...
To better accommodate scaling and performance needs, updating the `type` setting in `index_options` is possible with the <<update-mapping,Update Mapping API>>, according to the following graph (jumps allowed):

...
You can also retrieve values from sub-fields of flattened fields with a
<<runtime-retrieving-fields,runtime field>>. Instead of including
`doc['<field_name>'].value` in your Painless script, use
`doc['<field_name>.<sub-field_name>'].value`. For example, if you have a
flattened field called `label` with a `release` sub-field, the Painless script
in your runtime field would be `doc['labels.release'].value`.
...
the `doc['<field_name>.<sub-field_name>'].value` format. The `release_label`
runtime field includes a script that retrieves values from the `labels.release`
field:
...
You can then search for the `release_label` runtime field and only return
documents with a `release` value of `v1.3.0`:
...
The response includes matching search results, including the `release_label`
runtime field:
...
----
// TESTRESPONSE[s/"_id" : "va-8yHkBlxF0v9nxT7gO"/"_id": $body.hits.hits.0._id/]
// TESTRESPONSE[s/"_id" : "vq-8yHkBlxF0v9nxT7gO"/"_id": $body.hits.hits.1._id/]

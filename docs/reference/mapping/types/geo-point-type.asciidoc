...
|`geohash_prefix` |If this option is set to `true`, not only the geohash but also all its parent cells (true prefixes) will be indexed as well. Defaults to `false`. *Note*: This option requires `geohash` to be enabled.

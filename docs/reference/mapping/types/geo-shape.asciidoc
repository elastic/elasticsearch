...
[float]
[[crs]]
==== Coordinate Reference Systems and Projected Data

For indexing and searching data in alternative
https://en.wikipedia.org/wiki/Spatial_reference_system[Coordinate Reference Systems (CRS)] see
the <<coordinate-reference-systems,`crs` Mapping Parameter Documentation>>
...
a `POLYGON`. Alternatively, a <<processors/circle,Circle Ingest Processor>> is
available that will transform the point, radius into a Polygon that best
approximates the circle based on a provided error distance parameter.
...
Preserving the source data's original

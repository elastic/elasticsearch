...
There are three strategies available: `recursive`, and `term`.
...
near perfect spatial resolution (down to 1e-6 decimal degree precision) since all
...
<<prefix-trees>> indexing approach. While this is the default indexing technique
prefix trees can still be used by setting the `tree` or `strategy` parameters
according to the appropriate <<geo-shape-mapping-options>>. Note that these parameters
...
triangular mesh (see <<indexing-approach>>).

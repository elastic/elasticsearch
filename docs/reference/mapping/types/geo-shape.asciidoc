...
|`tree |deprecated[6.6, use `vector` strategy] Name of the PrefixTree
implementation to be used: `geohash` for GeohashPrefixTree and `quadtree`
for QuadPrefixTree.
| `geohash`

|`precision` |deprecated[6.6, use `vector` strategy] This parameter may
...
|`tree_levels` |deprecated[6.6, use `vector` strategy] Maximum number
...
|`strategy` |deprecated[6.6, use `vector` strategy] The strategy
...
There are three strategies available: `recursive`, `term`, and `vector`.
Vector strategy is the default. This strategy provides near perfect
resolution (down to 1e-6 decimal degree precision) by indexing shapes
as vectory types. It is the preferred method of shape indexing.
...
|`points_only` |deprecated[6.6, use `vector` strategy] Setting this option to
...
deprecated[6.6, use `vector` strategy] To efficiently represent shapes in
...
been deprecated in favor of a vector approach that indexes the shapes as a triangular
mesh.
...
[float]
===== Spatial strategies
The indexing implementation selected relies on a SpatialStrategy for choosing
how to decompose the shapes (either as grid squares or a tessellated triangular
mesh). Each strategy answers the following:
...
|`vector` | <<input-structure, All>> | `INTERSECTS`, `DISJOINT`, `WITHIN` |Yes
|`recursive` deprecated[6.6, use `vector` strategy] |<<input-structure, All>> |`INTERSECTS`, `DISJOINT`, `WITHIN`, `CONTAINS` |Yes
|`term` deprecated[6.6, use `vector` strategy] |<<point, Points>> |`INTERSECTS` |Yes
...
approximately 1e-6 decimal degree precision.
...
With prefix trees, Elasticsearch uses the paths in the tree as terms in
the inverted index and in queries. The higher the level (and thus the
precision), the more terms are generated. Of course, calculating the
terms, keeping them in memory, and storing them on disk all have a price.
Especially with higher tree levels, indices can become extremely large
even with a modest amount of data. Additionally, the size of the features
also matters. Big, complex polygons can take up a lot of space at higher
tree levels. Which setting is right depends on the use case. Generally one
...
--------------------------------------------------
// CONSOLE
// TEST[warning:Field parameter [tree] is deprecated and will be removed in a future version. use [vector] indexing strategy instead.]
// TEST[warning:Field parameter [strategy] is deprecated and will be removed in a future version. use [vector] indexing strategy instead.]
...
point with a radius. Note that circles can only be indexed when using
the `recursive` Prefix Tree strategy only.

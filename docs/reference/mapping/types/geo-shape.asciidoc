...
You can override this parameter at the document level. For information about how
{es} uses orientation, see <<polygon-orientation>>.
| `RIGHT`
...
(counterclockwise) or `LEFT` (clockwise). You can set a `geo_shape` field's
default orientation using the <<geo-shape-mapping-options,`orientation` mapping
parameter>>. You can override this default for specific polygons using the
document-level `orientation` parameter.
...
{es} only uses orientation to determine if a polygon crosses the international
dateline (+/-180° longitude). If so, {es} splits the polygon at the dateline. If
the difference between a polygon's minimum longitude and the maximum longitude
is less than 180°, its orientation has no effect.

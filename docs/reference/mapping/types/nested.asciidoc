...
include::{es-repo-dir}/mapping/settings-mapping-explosion.asciidoc[tag=nested-fields-limit]

In the previous example, the `user` mapping would count as only 1 towards this limit.

include::{es-repo-dir}/mapping/settings-mapping-explosion.asciidoc[tag=nested-objects-limit]

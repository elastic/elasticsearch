...
<<query-dsl-range-query,`range`>> queries and
<<search-aggregations,aggregations>>. However, <<keyword,`keyword`>> fields are
better for <<query-dsl-term-query,`term`>> and other
<<term-level-queries,term-level>> queries.

Identifiers, such as an ISBN or a product ID, are rarely used in `range` queries
or aggregations. However, they are often retrieved using term-level queries.

Consider mapping a numeric identifier as a `keyword` if:

* You don't plan to search for the identifier data using 
  <<query-dsl-range-query,`range`>> queries.
* You don't plan to <<search-aggregations,aggregate>> the identifier data.
* Fast retrieval is important. `term` query searches on `keyword` fields are
  often faster than `term` searches for numeric fields.

...
NOTE: The default value for `coerce` is `true` for all numeric types. One implication
of this is indexing `""` results in a `null` value, which cannot then be fetched at
query time.
...
<<coerce,`coerce`>>::
    ...
    Note that this cannot be set if the `script` parameter is used. Also note
    that the coerced value of `""` is `null`, which cannot be fetched at query time.

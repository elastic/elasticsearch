...
NOTE: `rank_features` since storage of the fields values takes advantage of the existing term frequency storage.
<<query-dsl-term-query, `term`>> queries against `rank_features` values are simply the multiplication of the provide
`boost` and the stored feature value.

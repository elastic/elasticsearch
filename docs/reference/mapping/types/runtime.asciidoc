...
example, index the following documents.
...
In the `runtime_mappings` section of the `_search` request, define a script
that operates on the `name.first` field. The values you specify will override
fields in `_source` for the duration of the query without modifying the field
itself.
...
that the previous request defined as a runtime field in the mapping. The value
for the `day_of_week` field is calculated dynamically at search time, and the
following search request retrieves any documents where the calculated value is
equal to `Thursday`.

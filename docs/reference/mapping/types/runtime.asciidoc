...
Runtime fields beta[] are accessible from the search API like any other field.
Use the <<search-field-caps,field capabilities API>> to return runtime fields
as searchable and aggregatable.

including the cost of these operations.
that is indexed and has <<doc-values,doc_values>>. You can retrieve and query
these fields, as well as sort and aggregate on them.

...
NOTE: Updates that use scripts are not supported when the index contains a `semantic_text` field.
...
This imposes restrictions on updating documents with `semantic_text`:

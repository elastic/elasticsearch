...
NOTE: Reordering text fields can have an effect on <<query-dsl-match-query-phrase,phrase>>
      ...
      <<position-increment-gap, here>> for more. You can avoid this by making sure the
      `slop` parameter on the phrase queries is lower than the `position_increment_gap`.
      This is the default.
endif::[]

...
<<mapping-store,`store`>>::
    ...
    the <<mapping-source-field,`_source`>> field. Accepts `true` or `false`. Defaults to
    `false` if <<synthetic-source,synthetic `_source`>> is disabled and to `true` if
    synthetic source is enabled and there is no <<keyword-synthetic-source, `keyword`>>

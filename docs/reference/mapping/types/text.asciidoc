...
<<index-prefix-config,`index_prefix`>>::
    ...
    the expense of a larger index. Accepts `true`, `false` (default) or an
...
By default, the `index-prefix` option will index ngrams of tokens between 2 and 5
characters wide.  To change this, use the following syntax:
...
--------------------------------
// CONSOLE
<1> `min_chars` must be greater than zero
<2> `max_chars` must be greater than `min_chars` and less than 20
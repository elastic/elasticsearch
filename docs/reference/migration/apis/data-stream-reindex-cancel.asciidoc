...
Any backing indices that have already been reindexed and swapped into the data stream will remain the data stream.
Only backing indices which are currently being reindexed or future pending backing indices will be cancelled.
Once a data stream reindex task is cancelled it will not longer be accessible through the
<<data-stream-reindex-status-api, data stream reindex status API>>. If a reindex task is not currently running
this API will return a `resource_not_found_exception`.

...
This api runs asynchronously because reindexing all indices in a large data stream
...
it finished successfully or failed. A running data stream reindex task can be cancelled using the
...
(<<dynamic-cluster-setting,Dynamic>>)
The average number of documents within a given backing index to reindex per second.
...
* Delete the target index if one exists. This is done in case we are retrying after a failure, so that we start with a fresh index.
...
For example, if the backing index `.ds-my-data-stream-2025.01.23-000002` was accidentally deleted, we would see a status like the following:
...
Once the issue has been fixed, the reindex task can re-run using the <<reindex-data-stream-start,original reindex command>> to pick up where it left off.

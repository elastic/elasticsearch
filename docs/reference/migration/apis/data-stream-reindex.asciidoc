...
request per second could be to low for your use case.
...
For example, if the user does not have the `manage` permission for the data stream,
after starting reindexing, we can check the status API:

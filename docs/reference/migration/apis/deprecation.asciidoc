...
NOTE: The Deprecation Info API is only guaranteed to be up to date with all
breaking changes as of the last minor version release of each major version
series.  For example, Elasticsearch version 5.6 (and all subsequent patch
versions) is the last minor version release of the 5.x major version series.
Make sure to upgrade to the latest minor version release for your current
version and re-run these checks before upgrading to the next major version.

...
* `FunctionScoreQueryBuilder#setMinScore(float minScore)` is deprecated - Please use `FunctionScoreQueryBuilder#minScore(Float minScore)` instead
* `FunctionScoreQueryBuilder#maxBoost(Float maxBoost)` is removed - Please use `FunctionScoreQueryBuilder#maxBoost(float maxBoost)` instead
* `FieldValueFactorFunctionBuilder#missing(Double missing)` is removed - Please use `FieldValueFactorFunctionBuilder#missing(double missing)` instead

The API to create your own custom decay distance score function has changed. Please see example in
org.elasticsearch.search.functionscore.FunctionScorePluginTests.

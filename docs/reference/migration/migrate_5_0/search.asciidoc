...
* In `query_string` query, the behaviour of `analyze_wildcard` parameter when 
  set to `true` has been changed. Now only parts of the analysis chain that 
  operate at the character level will be applied. This means that tokenizers 
  and some filters will be ignored. 

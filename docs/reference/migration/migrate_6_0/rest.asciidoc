...
In previous versions of Elasticsearch, Analyze API required a `tokenizer` or `analyzer` parameter.
...
mappings for all indices. In order to retrieve only the mappings for an index, use 

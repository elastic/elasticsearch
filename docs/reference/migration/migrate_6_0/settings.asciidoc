...
The configuration file found in the Elasticsearch config directory could previously
either `.yml`, `.yaml` or `.json` extensions. Only `elasticsearch.yml` is now supported.

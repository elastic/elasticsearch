...
==== The `_default_` mapping is disallowed

The `_default_` mapping has been deprecated in 6.0 and is now disallowed in 7.0.
Trying to configure a `_default_` mapping on 7.x indices will result in an error.

...
This will be required if there are clients that connect to {es} which
do not support newer TLS versions.

`xpack.http.ssl.supported_protocols`::
For outgoing http connections from Watcher.
This will be required if you have watches that connect to external HTTP servers
that do not support newer TLS versions.

`xpack.security.authc.realms.ldap.{name}.ssl.supported_protocols`::
For outgoing LDAP connections from {security}.
This will be required if you have an LDAP realm enabled, and the LDAP directory
to which that realm connects does not support newer TLS versions.

`xpack.security.authc.realms.active_directory.{name}.ssl.supported_protocols`::
For outgoing Active Directory (LDAP) connections from {security}.
This will be required if you have an AD realm enabled, and the directory server
to which that realm connects does not support newer TLS versions.

`xpack.security.authc.realms.saml.{name}.ssl.supported_protocols`::
For outgoing HTTP connections to retrieve SAML metadata.
This will be required if you have a SAML realm enabled, and the realm is
configured to retrieve metadata over https (that is, `idp.metadata.path` is a
URL starting with `https://`) and the web server which hosts the metadata does
not support newer TLS versions.

`xpack.security.authc.realms.oidc.{name}.ssl.supported_protocols`::
For outgoing HTTP connections to an OpenId Connect Authorization Server.
This will be required if you have an OpenId Connect ("oidc") realm enabled,
and the realm is configured to connect to a remote authorization server which
does not support newer TLS versions.

`xpack.monitoring.exporters.{name}.ssl.supported_protocols`::
For remote monitoring data.
If you have configured monitoring data to be exported to a remote monitoring
cluster, and that cluster is configured to only support TLSv1, then you must
update this setting.

`reindex.ssl.supported_protocols`::
For reindex from remote. monitoring data.
If you reindex data from a remote {es} cluster which has SSL enabled
on the `http` interface, and that cluster is configured to only support TLSv1,
then you must update this setting.

`xpack.security.transport.ssl.supported_protocols`::
For incoming connections between {es} nodes. This should not be
required, unless you have specialised network equipment between your nodes which
inspects TLS packets.

...
* The disk-based shard allocator's deprecated
  `cluster.routing.allocation.disk.threshold_enabled` setting has been removed.
  Although it is not recommended to run Elasticsearch with the disk-based shard
  allocator disabled, it is still possible to do so by setting all three
  watermarks to `100%`.

...
single machine then you should run one node for each data path.
...
a single data path. At each step this process will discard the data spread
across the data paths on the node that is being replaced, so you must make sure
that your cluster health is never `red` during this process and we recommend
you take frequent snapshots to protect your data in case of disaster.

You can optionally migrate the data away from the target node at each step
before shutting it down by using an
...
determine why. Once the data has moved away from the target node you can shut
it down and then remove the allocation filter:

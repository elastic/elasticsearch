...
You can no longer use the `indices` parameter of the
...
Snapshot API requests that explicitly specify a system index in the `indices`
parameter will return an error. If the `indices` value includes a wildcard (`*`)
pattern, the pattern will no longer match system indices.
====

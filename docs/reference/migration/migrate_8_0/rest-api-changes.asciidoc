...
any requests that include the`_xpack` prefix will be rerouted to the corresponding
...
*Compatibility* +
When <<rest-api-compatibility>> is <<request-rest-api-compatibility,requested>>
any requests that include the custom mapping types, the custom mapping type will
be ignored. URLs will be rerouted to the corresponding URL without the a mapping type.
Custom mapping types in request bodies will be ignored, type related HTTP
parameters will be ignored and responses, where warranted, will include `_type` : `_doc`.
...
*Compatibility* +
When <<rest-api-compatibility>> is <<request-rest-api-compatibility,requested>>
the `interval` parameter will be adapted to the either a fixed or calendar interval.
...
*Compatibility* +
When <<rest-api-compatibility>> is <<request-rest-api-compatibility,requested>>
the `template` parameter mapped to `index_patterns`.
...
*Compatibility* +
When <<rest-api-compatibility>> is <<request-rest-api-compatibility,requested>>
the request to synced flush will be routed to the equivalent non-synced flush URL.
====
...
*Compatibility* +
When <<rest-api-compatibility>> is <<request-rest-api-compatibility,requested>>
the `types` query parameter is ignored returning the stats for the entire index.
====
...
*Compatibility* +
When <<rest-api-compatibility>> is <<request-rest-api-compatibility,requested>>
the `size` parameter will be mapped to the `max_docs` parameter.
...
*Compatibility* +
When <<rest-api-compatibility>> is <<request-rest-api-compatibility,requested>>
the `use_field_mapping` is ignored.

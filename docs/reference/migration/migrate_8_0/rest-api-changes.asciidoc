...
* The same version.
* A newer version that's also supported in the
...
in the same major version or the last minor release of the previous major
version. However, such searches can result in undefined behavior, particularly
if the search uses a recent feature that's unsupported in the remote cluster.

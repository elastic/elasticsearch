...
====
*Details* +
Elastic now only supports searches to a remote cluster running:
...
that's unsupported in the remote cluster, it could result in undefined behavior.

*Impact* +
If you only run {ccs-init} on remote clusters using the same or a later version
as the local cluster, no changes are needed.

If you previously used {ccs-init} to search clusters running an earlier version,
see
{ref}/modules-cross-cluster-search.html#ccs-ensure-cluster-version-compatibility[{ccs-init}

...
[float]
==== Mandatory realm order configuration

The `order` setting is now mandatory and must be specified for each explicitly
configured realms. Their values must be unique. The cluster will fail to start if

...
{ref}/configuring-tls.html[Encrypting communications in Elasticsearch]. As part
of this configuration, explicitly specify `xpack.security.transport.ssl.enabled`
as `true`.
...
If you want to enable SSL, follow the instructions in
{ref}/tls-http.html[Encrypting HTTP client communications]. As part

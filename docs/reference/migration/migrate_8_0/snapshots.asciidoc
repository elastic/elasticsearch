...
Queries that doe not contain a a combination of a wildcard pattern and an exclude pattern are unaffected by this change. For example,
`-snapshot-a,-snapshot-b` or `-snapshot-*` will not see their behavior changed.

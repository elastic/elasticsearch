...
====
*Details* +
In 8.1, when retrieving a geo_shape field, the geometry will be normalized if
it crosses the dateline. For example if a polygon crosses the dateline, it will be
returned as two polygons. The original polygon can still be retrieved from `_source`.

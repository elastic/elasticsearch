...
aggregations. Only the aggregated fields defined in the `analysis_config` are 
used by the {anomaly-job}. If you want to create a {datafeed} with an 
aggregation that operates on a particular field of the job, you have to define 
that field in the `analysis_config` when you create the job.

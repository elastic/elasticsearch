...
or all categories in the {anomaly-job} for the specified partition if
`partition_field_value` is supplied. Since `category_id` is unique at the job
level, even when per-partition categorization is enabled, it is _not_
necessary to also specify `partition_field_value` when getting a single
category definition.
...
(string) The field used to segment the categorization, if per-partition
categorization was enabled. This field is not present when per-partition
categorization is disabled.

`partition_field_value`::
(string) The value of `partition_field_name` for this category, if per-partition
categorization was enabled. This field is not present when per-partition
categorization is disabled.

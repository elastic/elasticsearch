...
`search_interval`:::
(Optional, object) An object that provides the latest time interval the {dfeed}
has searched.
+
[%collapsible%open]
=====
`start_ms`: The start time as an epoch in milliseconds.
`end_ms`: The end time as an epoch in milliseconds.
=====

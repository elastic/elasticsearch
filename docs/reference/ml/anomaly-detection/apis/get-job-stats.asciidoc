...
(object) An object that describes the number of records processed and any
...
job by using the <<ml-post-data,post data API>>. These out of order records are 
discarded, since jobs require time series data to be in ascending chronological
order.

`processed_field_count`:::
(long) The total number of fields in all the records that have been processed by
...
(long) The number of records that have been processed by the job. This value
includes records with missing fields, since they are nonetheless analyzed. If
you use {dfeeds} and have aggregations in your search query, the
`processed_record_count` will be the number of aggregated records processed, not

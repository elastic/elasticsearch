...
`probability`::
(number) The probability of the individual anomaly occurring, in the range 0 to
1. This value can be held to a high precision of over 300 decimal places, so the 
`record_score` is provided as a human-readable and friendly interpretation of
this.

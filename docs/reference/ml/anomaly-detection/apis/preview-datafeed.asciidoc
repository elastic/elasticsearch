...
It is possible to preview an existing {dfeed}, or provide configurations for
the {dfeed} or {anomaly-job} in the API call.

NOTE: You may not provide configurations in the API body while also
providing the `<datafeed_id>` in the URL.
...
`datafeed_config`::
(Optional, object) The {dfeed} definition to preview. If this is provided,
the `datafeed_id` must not be in the url. For valid definitions see
<<ml-put-datafeed-request-body, put datafeed documentation>>.

`job_config`::
(Optional, object) If provided, this {anomaly-job} is used instead of the one
referred to by the {dfeed} `job_id`. Must be present if the {dfeed} config
does not refer to a {anomaly-job}. See <<ml-put-job-request-body, put job documentation>>
...
Below is an example of providing configs for the {dfeed} and {anomaly-job}.

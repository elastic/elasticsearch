...
[[update-analysisconfig]]`analysis_config`::
`detectors`:::
//TBD Is the implication that you can add more detectors to the array? Or only update existing detectors (in particular the follow properties)?
`custom_rules`::::
(array)
...
--
NOTE: If the job is open when you make the update, you must stop the {dfeed},
close the job, then reopen the job and restart the data feed for the changes to take effect.

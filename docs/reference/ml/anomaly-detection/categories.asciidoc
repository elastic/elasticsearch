...
machine-written messages and application outputs that typically consist of
...
it gives poor results for human-generated data; the categorization job would
create so many categories that they couldn't be handled effectively.
Categorization is _not_ natural language processing (NLP).
...
In {kib}, you can use the categorization wizard when you create a new
...
of unusually high message counts for specific message categories.
...
see the
{ref}/ml-put-job.html#ml-put-job-request-body[`analysis_config.categorization_analyzer` API object].

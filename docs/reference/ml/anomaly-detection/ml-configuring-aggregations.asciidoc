...
still significantly less than your total number of documents, use composite
aggregations
...
NOTE: `composite` aggregations may not be as performant as a scrolling or
`date_histogram` oriented {dfeed}. `composite` aggregations are optimized
...
experimental:[composite agg support in datafeeds is currently experimental]

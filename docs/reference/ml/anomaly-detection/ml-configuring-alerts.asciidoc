...
* {anomaly-jobs-cap} health: monitors job health and alerts if an operational 
  issue occurred that may prevent the job from detecting anomalies. Recommended 
...
* _Model memory limit reached_: notifies if the model memory status of the job 
...
* _Data delay has occurred_: notifies when the job missed some data. You 

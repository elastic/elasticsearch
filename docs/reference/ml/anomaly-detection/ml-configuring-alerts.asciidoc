...
{anomaly-detect-cap} alerting runs scheduled checks on an {anomaly-job} or a 
group of jobs to detect anomalies with certain conditions. If an anomaly meets 
the rules you defined, an alert is created and the associated action is 
triggered. For example, you can create a rule to check an {anomaly-job} every 
fifteen minutes for critical anomalies and to notify you in an email. This page 
helps you to configure {anomaly-detect} alerting. To learn more about alerting 
in the {stack}, refer to 
...
You can create {anomaly-detect} alerting rules in the {anomaly-job} wizard after 
...
under the {ml-cap} section, then give a name to the rule and optionally provide 
tags.
...
automatically checked the next time when the conditions are checked.

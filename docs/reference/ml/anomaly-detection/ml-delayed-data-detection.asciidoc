...
`Datafeed has missed XXXX documents due to ingest latency`, check your ingestion 
process end to end if it delays indexing. If it works as expected, consider 
increasing the value of `query_delay`.

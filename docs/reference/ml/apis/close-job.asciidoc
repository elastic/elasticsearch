...
`<job_id>`::
  (string) Required. Identifier for the job. It can be a job identifier, a group
...
`force`::
  (boolean) Optional. Use to close a failed job, or to forcefully close a job
  which has not responded to its initial close request.

`timeout`::
  (time units) Optional. Controls the time to wait until a job has closed.

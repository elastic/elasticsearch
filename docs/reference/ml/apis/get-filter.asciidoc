...
`<filter_id>`::
  (string) Optional. Identifier for the filter.

[[ml-get-filter-query-parms]]
==== {api-query-parms-title}

`from`:::
    (integer) Optional. Skips the specified number of filters.

`size`:::
    (integer) Optional. Specifies the maximum number of filters to obtain.

...
`end`::
  (string) Optional. Returns influencers with timestamps earlier than this time.
...
`influencer_score`::
  (double) Optional. Returns influencers with anomaly scores greater or equal
  than this value.

`page`::
`from`:::
    (integer) Optional. Skips the specified number of influencers.
`size`:::
  (integer) Optional. Specifies the maximum number of influencers to obtain.

`sort`::
  (string) Optional. Specifies the sort field for the requested influencers. By
  default, the influencers are sorted by the `influencer_score` value.

`start`::
  (string) Optional. Returns influencers with timestamps after this time.

...
`allow_no_jobs`::
  (boolean) Optional. If `false` and the `job_id` does not match any job, an
  error occurs. The default value is `true`.

`bucket_span`::
  (string) Optional. The span of the overall buckets. Must be greater or equal
  to the largest job's `bucket_span`. Defaults to the largest job's `bucket_span`.

`end`::
  (string) Optional. Returns overall buckets with timestamps earlier than this
  time.

`exclude_interim`::
  (boolean) Optional. If `true`, the output excludes interim overall buckets.
...
`overall_score`::
  (double) Optional. Returns overall buckets with overall scores greater or
  equal than this value.

`start`::
  (string) Optional. Returns overall buckets with timestamps after this time.

`top_n`::
  (integer) Optional. The number of top job bucket scores to be used in the

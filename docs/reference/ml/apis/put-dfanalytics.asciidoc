...
`analysis`::
  (object) Required. Defines the type of {dfanalytics} you want to perform on
  your source index. For example: `outlier_detection`.  See {oldetection}
  resources.
  
`analyzed_fields`::
  (object) Optional. You can specify both `includes` and/or `excludes` patterns.
  If `analyzed_fields` is not set, only the relevant fileds will be included.
  For example all the numeric fields for {oldetection}.
  
`dest`::
  (object) Required. The destination configuration, consisting of `index` and
  optionally `results_field` (`ml` by default).
  
`source`::
  (object) Required. The source configuration, consisting of `index` and
  optionally a `query`.

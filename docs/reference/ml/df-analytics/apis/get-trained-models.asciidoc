...
(Optional, string) An array of classification labels. NER only supports Inside-Outside-Beginning labels (IOB)
and only persons, organizations, locations, and miscelanious.
...
`index`:::
(Required, object)
Indicates that the model definition is stored in an index. This object is required to be empty as 
...
+
--
* `tree_ensemble`: indicates that the model definition is an ensemble model of decision trees

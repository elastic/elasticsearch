...
The following example creates the `wine_quality_white` {dfanalytics-job}, the 
analysis type is `regression`:

[source,js]
--------------------------------------------------
PUT _ml/data_frame/analytics/wine_quality_prediction
{
  "source": {

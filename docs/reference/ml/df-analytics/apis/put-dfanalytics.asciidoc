...
<2> This query filters out data so that the analysis won't include it.
...
<5> Specifies fields to be included in or excluded from the analysis.
...
`_source` object. The `Cancelled` field is included in the destination index, 
however it does not included in the analysis because it is explicitly specified 
as excluded field by the `excludes` parameter of the `analyzed_fields` object.

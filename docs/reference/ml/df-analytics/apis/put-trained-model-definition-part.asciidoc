...
Here is an example of creating a model definition part for a previously
stored model configuration. The definition part is stored in the index configured by
`location.index.name`.
 

NOTE: `definition` is elided as it is a very large base64 encoded string

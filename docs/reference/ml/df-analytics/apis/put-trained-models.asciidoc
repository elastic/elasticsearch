...
`inference_config`::
(Required, object)
The default configuration for inference. This can be: `regression`, `classification`,
`fill_mask`, `ner`, `text_classification`, or `text_embedding`. 
If `regression` or `classification`, it must match the underlying `definition.trained_model`'s `target_type`.

If `fill_mask`, `ner`, `text_classification`, or `text_embedding`; the `model_type` must be `pytorch`.
+

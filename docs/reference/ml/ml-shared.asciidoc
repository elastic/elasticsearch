...
be higher than the number of extra categories that would have been created
if unlimited memory had been available, because many of the occasions where
a message didn't match any existing category might have been placed in the
same extra category. So the number of extra categories that would have been
created if unlimited memory had been available could be anywhere between 1
and `failed-category-count`.

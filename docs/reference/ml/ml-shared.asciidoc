...
control the memory used by other processes, for example the Elasticsearch Java
...
<<indices-analyze,Analyze endpoint>>. The `categorization_analyzer` field can be
specified either as a string or as an object. If it is a string it must refer to
a <<analysis-analyzers,built-in analyzer>> or one added by another plugin. If it
...
tag::custom-rules[]
An array of custom rule objects, which enable customizing how the detector works.
For example, a rule may dictate to the detector conditions under which results 
should be skipped. For more information see <<ml-detector-custom-rule>>.
+
--
...
detectors for a job. Each detector has the following properties:
...
tag::jobs-anomaly-detection[]
An array of {anomaly-job} resources. For more information, see
<<ml-job-resource>>.
end::jobs-anomaly-detection[]
...
Advanced configuration option. Specifies to store model information along with
the results. This adds overhead to the performance of the system and is not
feasible for jobs with many entities, see <<ml-apimodelplotconfig>>.

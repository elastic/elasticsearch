...
When `truncate` is `none`, this value can be set to indicate if longer text sequences
should be partitioned for inference. The configured number is how many tokens overlap between each
subsequence.

Default value is `-1`, indicating no windowing or spanning is to be done.

Note: when your typical input is just slightly larger than `max_sequence_length`, it may be best to simply truncate. As

...
tag::chunking-config[]
Specifies how data searches are split into time chunks. See
<<ml-datafeed-chunking-config>>. For example:
...
tag::datafeed-id[]
A numerical character string that uniquely identifies the {dfeed}.
end::datafeed-id[]

tag::delayed-data-check-config[]
Specifies whether the data feed checks for missing data and the size of the
window. For example: `{"enabled": true, "check_window": "1h"}` See
...
tag::indices[]
An array of index names. For example: `["it_ops_metrics"]`
end::indices[]

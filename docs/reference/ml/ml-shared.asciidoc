...
Text similarity takes an input sequence and compares it with some other input sequence. This is commonly referred to
as cross-encoding. This kind of task is useful for ranking document text when comparing it to some other provided text input.
end::inference-config-text-similarity[]

tag::inference-config-text-similarity-span-score-func[]
This identifies how to combine the resulting similarity score when a provided text passage is longer than `max_sequence_length` and must be
automatically separated for multiple calls. This only is applicable when `truncate` is `none` and `span` is some non-negative

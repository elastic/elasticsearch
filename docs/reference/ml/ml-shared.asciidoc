...
When `adaptive_allocation` is enabled, the number of allocations of the trained model is set based on the current load the process gets.
When the load exceeds 90% of the current capacity usage, a new model allocation is automatically created, respecting the value of `max_number_of_allocations` if it's set.
When the load is less than 85% of the current capacity usage, a model allocation is automatically removed, respecting the value of `max_number_of_allocations` if it's set.
The number of model allocations cannot be scaled down to less than `1` this way.
If `adaptive_allocation` is enabled, do not set the value of `number_of_allocations or `num_allocations`.

...
`detectors`:::
(array)
include::{docdir}/ml/ml-shared.asciidoc[tag=detectors]
...
at the same time as `categorization_filters`. See <<ml-categorizationanalyzer>>.
...
Describes the format of the input data and how APIs parse timestamp fields. See
...
tag::detectors[]
An array of detector configuration objects, which describe the anomaly detectors 

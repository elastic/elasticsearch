...
If true, categorization will be done separately for each value of the
`partition_field_name` field. The default is false. This setting can only be
set when the job is created; it cannot be updated. This setting may only be
...
This setting may only be set to true if per-partition categorization
is enabled. If true, both categorization and subsequent anomaly detection will
stop for partitions where the categorization status changes to `warn`. This
setting makes it viable to have a job where it is expected that categorization
will work well for some partitions but not others, but not pay the cost of bad

...
`error_count`:::
(integer)
The total number of errors when evaluating the trained model
for all nodes in the deployment.

`rejected_execution_count`:::
(integer)
The total number of inference requests that were not processed because
the queue was full for all nodes in the deployment.
...
`timeout_count`:::
(integer)
The total number of inference requests that timed out before being
processed for all nodes in the deployment.
======
...
(integer)
The number of threads used by the inference process.
This number may be lower than the value of the `inference_threads` setting
in the <<start-trained-model-deployment>> API if the requested value
is greater than the number of hardware threads on the machine.
...
(integer)
The number of threads used when sending inference requests to the model.
This number may be lower than the value of the `model_threads` setting
in the <<start-trained-model-deployment>> API if the requested value
is greater than the number of hardware threads on the machine.

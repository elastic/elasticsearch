...
(integer)
The number of threads used by the inference process.
This value may be lower than top level value if there is not sufficient
hardware threads to satisfy the configured value.
...
(integer)
The number of threads used when sending inference requests to the model.
This figure may be lower than top level value if there is not sufficient
hardware threads to satisfy the configured value.

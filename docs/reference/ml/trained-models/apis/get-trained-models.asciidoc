...
options are:
 ...
 - `total_feature_importance`: Includes the total {feat-imp} for the training
   data set.
 - `download_status`: Includes the field `is_fully_downloaded` indicating if the
...
This field is only present if `include=download_status` was specified in the request.

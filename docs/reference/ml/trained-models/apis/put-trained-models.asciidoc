...
(Optional, string)
If the model only works on one platform, i.e., if it is heavily
...
the platform identifiers used by Elasticsearch, e.g., `linux-x86_64`,
`linux-aarch64`, `darwin-x86_64`, `darwin-aarch64`, `windows-x86_64`.
For portable models (i.e., independent of processor architecture or

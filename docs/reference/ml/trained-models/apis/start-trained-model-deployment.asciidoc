...
(Optional, integer)
The number of allocations a model gets on each node it is assigned to.
For each allocation there is a separate thread used for sending inference requests to the model.
...
(Optional, integer)
Sets the number of threads used by each model allocation during during inference. This generally increases

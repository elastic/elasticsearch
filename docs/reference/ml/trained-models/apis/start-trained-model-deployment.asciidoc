...
The inference cache size (off JVM heap) per node for the model. The default value is the same size as the model
size estimate. To disable cache, `0b` can be provided.

...
NOTE: You can not mix the usage of percentage values and byte values within these
settings. Either all are set to percentage values, or all are set to byte
values. This is so that we can we validate that the settings do not violate each
other (for example, we can validate that the low threshold is not more than the
high threshold, and that the high threshold is not more than the flood stage
threshold).

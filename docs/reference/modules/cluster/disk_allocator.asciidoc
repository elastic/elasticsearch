...
If the low watermark max headroom is set, the `cluster.routing.allocation.disk.watermark.high.max_headroom` needs to be set as well.
...
This caps the amount of free space required.
If the high watermark max headroom is set, the `cluster.routing.allocation.disk.watermark.flood_stage.max_headroom` needs to be set as well.

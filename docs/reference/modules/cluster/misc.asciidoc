...
<<indices-close,closed>> or <<indices-delete-index,deleted>> to bring the
...
[[cluster-indices-close]]
==== Closed indices

`cluster.indices.close.enable`::
(boolean) Indicates whether indices can be closed using the <<indices-close,
close index API>>. Defaults to `true`.

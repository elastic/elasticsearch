...
IMPORTANT: In Elasticsearch 8.14, the default value for `skip_unavailable` was
changed from `false` to `true`. Before Elasticsearch 8.14, if you want a cluster
to be treated as optional for a {ccs}, then you need to set that configuration.
From Elasticsearch 8.14 forward, you need to set the configuration in order to

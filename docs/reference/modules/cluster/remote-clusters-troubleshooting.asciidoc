...
However note that with some network configurations it could take minutes or
hours to detect that a connection has stopped working. Until the failure is
detected and reported to {es}, requests involving the remote cluster may appear
to be stuck.

====== Resolution

* Ensure the network between the clusters is as reliable as possible.

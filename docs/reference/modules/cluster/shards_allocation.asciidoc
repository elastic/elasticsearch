...
cluster by repeatedly simulating shard movements until no shard movement
improves the balance of the cluster. Once this background task completes, {es}
moves shards to their desired locations.
...
threshold. The weight of a node depends on the number of shards it holds and on
the resource usage of each shard expressed in terms of the size of the shard on
disk and the number of threads needed to support write traffic to the shard. The
following settings control how {es} combines these values into an overall

...
the `GET /_cluster/bootstrap` API.  This API returns a properly-constructed
bootstrap configuration that is ready to pass back to the `POST
/_cluster/bootstrap` API. It includes all of the master-eligible nodes that the
handling node has discovered via the gossip-based discovery protocol, and can
return an error if fewer nodes have been discovered than expected.
...
GET /_cluster/bootstrap?wait_for_nodes=3
...
# an error.
GET /_cluster/bootstrap?wait_for_nodes=3&timeout=1m
--------------------------------------------------
// CONSOLE

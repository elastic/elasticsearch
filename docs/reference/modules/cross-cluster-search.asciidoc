...
When searching across remote clusters, the execution path may differ depending
on the selected reduce mode:
...
instead. This is the default reduce mode.

The <<search-request-body, search API>> supports the `ccs_reduce_mode`
...
`_clusters` section, which will either be `remote` or `local`.

...
<1> This document came from `cluster_one`.
<2> This document came from `cluster_two`.
<3> This document's `_index` parameter doesn't include a cluster name. This
means the document came from the local cluster.


[float]
[[_skipping_disconnected_clusters]]
...
including a <<search-request-scroll, scroll>> or
<<search-request-inner-hits,inner hits>>.
+

...
[WARNING]
====
Cluster-level settings from the _local_ cluster are applied to each request.
This includes settings that could limit the request, such as
`action.search.shard_count.limit`, `pre_filter_shard_size`, and
`max_concurrent_shard_requests`. If these limits are too low, the request may be
rejected.
====
+

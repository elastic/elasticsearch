...
If a remote cluster is configured via `elasticsearch.yaml` only the nodes with
...
The `elasticsearch.yaml` config file for a _cross cluster search_ node just needs to list the

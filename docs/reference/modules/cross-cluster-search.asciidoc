...
Whenever a cross-cluster search request is executed, the `_clusters` section
returned as part of the search response holds the information on where the
reduction has happened: `remote` (meaning that minimizing round-trips was
enabled and possible with the provided request), or `local` (meaning  that
minimizing round-trips was either disabled explicitly or not possible with

...
master-eligible nodes in the cluster. This is only required the very first time
the cluster starts up: nodes that have already joined a cluster will store this
information in their data folder, and freshly-started nodes that are intended
to join an existing cluster will obtain this information from the cluster's
elected master. This information is given using this setting:
...
may be removed. It should be omitted on master-ineligible nodes, and on
master-eligible nodes that are started to join an existing cluster.

...
applying the required changes and publishing the updated cluster state to all
...
state, and then sends an acknowledgement back to the master.

The master waits to receive this second acknowledgement from all nodes for some
time, defined by `cluster.publish.timeout`, which defaults to `30s`, measured
from the moment that publishing started. If this time is reached before the new
cluster state was committed then the master node failed to contact enough other
master-eligible nodes, so the cluster state change is rejected and the master
stands down and starts a new election process.

If the `cluster.publish.timeout` time is reached after the new cluster state
was committed (but before all acknowledgements have been received) then the
master node starts processing and publishing the next cluster state update,
even though some nodes have not yet confirmed that they have applied the
current one. These nodes are said to be _lagging_ since their cluster states
are no longer up-to-date. The master waits for the lagging nodes to catch up
for a further time, `cluster.follower_lag.timeout`, which defaults to `90s`,
and if a node has still not successfully applied the cluster state update
within this time then it is removed from the cluster to prevent it from
disrupting the rest of the cluster.

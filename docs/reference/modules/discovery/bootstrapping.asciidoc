...
- The IP address of the node's <<modules-transport,publish address>> which is
  normally the IP address to which <<common-network-settings,`network.host`>>
  resolves but <<advanced-network-settings,this can be overridden>>.
...
You can also use IP addresses, or IP addresses and ports, or even a mix of IP

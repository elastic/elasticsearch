...
`elected-as-master` and all nodes will log a message containing `master node
changed` identifying the new elected master node. If there is no elected master
node and no node can win an election then all nodes will repeatedly log
messages about the problem using a logger called
...
for more information about troubleshooting this situation.

...
these nodes will report many kinds of errors to clients and in their logs.
...
issues reported by these nodes while they are unable to join the cluster.

To properly troubleshoot discovery and election problems, you need to
collect and analyse logs covering at least five minutes from all nodes. The
following sections cover a few possibilities around discovery and election problems.

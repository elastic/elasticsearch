...
The list of hosts is set using the <<unicast.hosts,`discovery.seed_addresses`>> static
setting.  For example:
...
`discovery.seed_addresses` then they are used in addition to those supplied in
`unicast_hosts.txt`.
...
`discovery.seed_addresses`), and IPv6 addresses must be specified in

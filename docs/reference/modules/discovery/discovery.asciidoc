...
===============================================
The discovery process usually completes quickly, and the master usually remains
elected for a long period of time. If the cluster has no master for more than a
few seconds, or the master is unstable, then the logs on each node will contain
...
* If a node wins the election then it logs a message containing the string
`elected-as-master`. If this happens repeatedly then this indicates the master
is unstable.

* When a node discovers a master, or believes the master to have failed, it
logs a message containing the string `master node changed`.

* If a node is unable to discover or elect a master for several minutes then it
starts to report additional detail about the failures in its logs. Be sure to
capture log messages for at least five minutes after the master was lost.

If your cluster does not have a stable master then many of its features will
not work correctly, and it may report many kinds of error to clients and in its
logs. However, you must focus on the master instability ahead of all other
issues. Until the master is stable there is no point in trying to understand or
resolve any other problems in your cluster.
===============================================

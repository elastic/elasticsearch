...
messages, but report fewer details:
...
minutes. Refer to <<modules-discovery-settings>> for more information.
...
to <<modules-discovery-settings>> for more information.
...
`warn threshold` may give further clue about the cause of the instability.

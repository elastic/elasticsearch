...
{es}'s REST APIs using its <<http-settings,HTTP interface>>, but the node that
receives a client request cannot always handle it alone and must normally pass
it on to other nodes for further processing. It does this using the
<<transport-settings,transport interface>>. The transport interface is used for
all communication between nodes in a cluster, and for communication with a
<<modules-remote-clusters,remote cluster>>.

You can configure both of these interfaces at the same time using the
`network.*` settings described below. If you have a more complicated network
then you may need to configure the interfaces independently using the `http.*`
and `transport.*` settings described on their respective pages. Where possible
you should use the `network.*` settings that apply to both interfaces since
this will simplify your configuration and reduce duplication.

By default {es} binds only to localhost which means it cannot be accessed
remotely. This is sufficient for a local development cluster made of one or
more nodes all running on the same host. To form a cluster across multiple
hosts, or which is accessible to remote clients, you will need to adjust some
...
The settings in this section are the only ones that most users will need to
set.
...
following special values. These values can be used when configuring
...
Any values containing a `:` (e.g. an IPv6 address or some of the
...
When an application such as {es} wishes to receive communication over the
network it must indicate to the operating system the address or addresses whose
traffic it should receive. This is known as _binding_ to those addresses. {es}
can bind to more than one address if needed, but most nodes only bind to a
single address. {es} can only bind to an address if it is running on a host
that has a network interface with that address. If necessary, you can configure
the transport and HTTP interfaces to bind to different addresses.
...
anything and need not be addresses of the network interfaces on the host. The
only requirements are:

* each node must be accessible at its transport publish address by all other
  nodes in its cluster, and by any remote clusters that will discover it using
  <<sniff-mode>>.

* each node must be accessible at its HTTP publish address by all clients
  that will discover it using sniffing.

If you have the common setup in which {es} binds to a single address at which
it is exposed to clients and other nodes then you should just set
`network.host` to that address. You should not separately set any bind or
publish addresses, nor should you separately configure the addresses for the
HTTP or transport interfaces.

If you wish to bind to multiple addresses, or to publish a different address
from the addresses to which you are binding, then you must instead use the
<<advanced-network-settings, advanced network settings>>: set
`network.bind_host` to the bind addresses, and `network.publish_host` to the
address at which this node is exposed. If necessary, in very complicated
setups, you can also configure these addresses differently for the HTTP and
...
`network.host`. You should only use this setting if you wish to bind to
multiple addresses or to use different addresses for publishing and binding.

`network.publish_host`::
(<<static-cluster-setting,Static>>)
The network address at which the node can be contacted by clients and other
nodes. Accepts an IP address, a hostname, or a
<<network-interface-values,special value>>. Defaults to the address given by
`network.host`. You should only use this setting if you wish to bind to
multiple addresses or to use different addresses for publishing and binding.

NOTE: It is permitted to specify a list of addresses for `network.host` and
...
preference and reachability and may change when the node restarts. To avoid
inconsistent results you must make sure that each node is accessible at every
possible publish address.

[[tcp-settings]]
===== Advanced TCP settings

The following settings can be used to control the low-level parameters of the TCP
connections used by both HTTP and transport interfaces.

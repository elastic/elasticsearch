...
addresses or hostnames.  If a hostname lookup resolves to multiple IP addresses --
(https://en.wikipedia.org/wiki/Round-robin_DNS[round robin DNS]) -- then
each IP address will be used for discovery.
+

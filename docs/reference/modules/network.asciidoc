...
{es}'s REST APIs using its <<http-settings,HTTP interface>>. However, the
receiving node cannot always handle the client request alone and will normally
use the <<transport-settings,transport interface>> to forward the request to
other nodes for further processing. Nodes use the transport interface for all
communication with other nodes in their cluster and for communication with
...
it is exposed to clients and other nodes. In this configuration you should just
set `network.host` to that address. You should not separately set any bind or
...
make sure that each node is accessible at all its publish addresses.

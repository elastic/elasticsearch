...
This setting can be configured just like `network.host` -- it
accepts IP addresses, host names, and
<<network-interface-values,special values>>, individually or as an array.
...
This setting accepts IP addresses, host names, and
<<network-interface-values,special values>>, but does not accept arrays.
It should be used only when `network.host` is an array, and should be set
to a value from among those in the `network.host` array.

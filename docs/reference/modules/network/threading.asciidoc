...
{es}. While this information isn't required to use {es}, it may be useful to
...
thread is chosen when the channel is opened and cannot change.
...
are tens-of-thousands or more TCP channels. If data arrives on a TCP channel
and its owning `transport_worker` thread is busy, the data isn't processed
until the thread finishes whatever it is doing. Similarly, outgoing data are
not actually sent over a channel until the owning `transport_worker` thread is
free. This means that we require every `transport_worker` thread to be idle
frequently, which looks something like this in a stack dump:
...
because they block in the native `EPoll#wait` method. The hot threads API will
report these threads at 100% overall utilisation. This is normal, and the
breakdown of time into `cpu=` and `other=` fractions shows how much time the
thread spent running and waiting for input respectively.

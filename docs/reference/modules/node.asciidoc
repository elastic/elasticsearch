...
cluster. For more information about X-Pack machine learning features, see
{xpack}/xpack-ml.html[Machine Learning in the Elastic Stack].
...
IMPORTANT: If you want to use X-Pack machine learning features in your cluster,
you must enable machine learning (set `xpack.ml.enabled` to `true`) on all
master-eligible nodes. Do not use these settings if you do not have X-Pack
installed.

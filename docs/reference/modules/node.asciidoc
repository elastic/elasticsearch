...
As the cluster grows, if you have large {ml} jobs or
{transforms}, consider separating dedicated master-eligible nodes from
...
elected master node. A voting-only node can serve as a tiebreaker
in elections.

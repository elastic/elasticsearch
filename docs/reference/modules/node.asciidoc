...
A node that has `xpack.transform.enabled` and the `transform` role. If you want
to use {transforms}, there must be at least one {transform} node in your
cluster. For more information, see <<transform-settings>> and <<transforms>>.
...
xpack.transform.enabled: true <2>
-------------------
<1> The `xpack.ml.enabled` setting is enabled by default.
<2> The `xpack.transform.enabled` setting is enabled by default.
...
must also be enabled on all coordinating nodes. You must also have at least one
at least one node with the `transform` role. This is the default behavior. If
...
xpack.transform.enabled: true <1>
-------------------
<1> Enable the `xpack.transform.enabled` setting.

...
A node that `node.transform` set to `true`. If you want to use {transforms},
...
`xpack.transform.enabled` set to `false` on any master-eligible nodes or any
data nodes. If you want to use {transforms} in clients (including {kib}), it
also must not be disabled on any coordinating nodes. You must also have

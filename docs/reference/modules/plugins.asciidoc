...
NOTE: If using a custom location (other than `$ES_HOME/plugins`), you will need to pass the `-Des.plugins.path=/path/to/plugins/folder` flag to the plugin script .

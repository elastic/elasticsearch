...
The `update` command can be used to update a plugin. This command combines a `remove`
and an `install` command in order to remove the plugin first before installing it again:

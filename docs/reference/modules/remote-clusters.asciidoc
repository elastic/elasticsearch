...
When connecting a local cluster to a remote clusters, an {es} 
superuser (such as the `elastic` user) gains total read access to the remote 
...
ability to take over a remote cluster. To use {ccr} and {ccs} safely with remote 
clusters, <<remote-clusters-security,enable security>> on all connected clusters 

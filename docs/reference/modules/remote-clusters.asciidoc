...
or searched by a local cluster. You can use remote clusters in {ccr} and {ccs}.

With <<xpack-ccr,{ccr}>>, you index data to a leader index, which is 
replicated to one or more read-only follower indices on a remote cluster.
Creating a multi-cluster architecture with {ccr} enables you to configure
disaster recovery, bring data closer to your users, or establish a centralized 
reporting cluster to process reports locally.
...
Connected remote clusters effectively form a single security domain, where users 
with administrative privileges in one cluster have those privileges in _all_ 
connected clusters. This behavior means that you can't restrict a user's access 
to a specific remote cluster. To use {ccr} or {ccs} safely with remote clusters, 
<<remote-clusters-security,enable security>> on all connected clusters and
configure Transport Layer Security (TLS) on every node.

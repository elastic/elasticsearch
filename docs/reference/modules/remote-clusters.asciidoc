...
clusters_. To register a remote cluster,
<<remote-clusters-connect,connect the local cluster>> to nodes in the
remote cluster using <<sniff-mode,sniff mode>> (default) or
<<proxy-mode,proxy mode>>. 

Once connected, you can search remote clusters using
...
Use the <<cluster-remote-info, remote cluster info API>> to get information
about registered remote clusters. If you're enabling and configuring
security for {es}, refer to 
<<remote-clusters-security,remote clusters and security>>.

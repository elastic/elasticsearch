...
dynamic scripts (even in a search request) or indexed scripts, then they have
...
deprecated[1.5.0, the `script.disable_dynamic` setting is deprecated in favour of fine-grained settings described as follows]
...
|=======================================================================
|Value |Description
| `disable` |scripting is turned off completely, in the context of the setting being set.
...
NOTE: Fine-grained scripting settings affect the `mustache` scripting language
too from `1.5.0` on, as opposed to previous versions where `mustache` scripts
were always allowed regardless of whether dynamic scripts are disabled or not.
Search templates will still be enabled by default as the `mustache` engine is
sandboxed though.
...
[source,yaml]
-----------------------------------
script.update: disable
script.mapping: disable
...
script.engine.groovy.dynamic.mapping: disable
...
script.engine.groovy.indexed.mapping: disable
script.engine.groovy.indexed.search: sandbox
script.engine.groovy.indexed.update: disable

...
NOTE: Generic scripting settings affect the `mustache` scripting language from
`1.5.0` on, as opposed to previous versions where `mustache` scripts were
always allowed regardless of whether dynamic scripts are disabled or not.

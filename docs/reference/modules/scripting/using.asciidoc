...
/_scripts/{id} <1>
-----------------------------------
<1> The `id` is a unique identifier for the stored script.  Note that
stored search templates and stored scripts share the same namespace.  If
a stored template is saved with the id of a stored script, it will overwrite
the existing stored script.

...
`lang`::
    ...
    may be set to any of languages listed in <<modules-scripting>>.
...
`_scripts` end-point.  Stored scripts must have unique id's even across
different languages.
...
/_scripts/{id} <1>
-----------------------------------
<1> The `id` is a unique identifier or script name.
...
Stored scripts can be used by specifying `id` parameter as follows:

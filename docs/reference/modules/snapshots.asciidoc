...
`processed` is an optional property that could be visible while snapshot has not been finished yet.

*Note*: Properties `number_of_files`, `processed_files`, `total_size_in_bytes` and `processed_size_in_bytes` describe
stats in backward compatible with version `5.x`, `6.x` way - and will be dropped in next major version.

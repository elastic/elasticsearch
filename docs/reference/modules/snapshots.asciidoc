...
The snapshot and restore module allows to create snapshots of individual indices (or aliases) or an entire cluster into a remote
...
supports <<search-multi-index-type,multi index syntax>> as well as index aliases. The snapshot request also supports the

...
[source,js]
-----------------------------------
POST /_snapshot/my_repository/cleanup
-----------------------------------
// CONSOLE
// TEST[continued]

NOTE: Most of the cleanup actions performed by the repository cleanup functionality are automatically executed when deleting a snapshot.
If you regularly delete snapshots from a repository you likely will get little to no space savings from executing this functionality.

...
`search`::
    ...
    `int((<<node.processors, # of allocated processors>> * 3) / 2) + 1`, and
    queue_size of `1000`.
...
`snapshot`::
    ...
    keep-alive of `5m` and a max of `min(5, (<<node.processors, # of allocated
    processors>>)/2)`.
...
`refresh`::
    ...
    keep-alive of `5m` and a max of `min(10, (<<node.processors, # of allocated
    processors>>)/2)`.

...
`search_worker`::
    For count/search operations that can be executed concurrently across segments within
    the same shard. Thread pool type is `fixed` with a size of `int((`<<node.processors,
    `# of allocated processors`>>`pass:[ * ]3) / 2) + 1`, and unbounded queue_size .

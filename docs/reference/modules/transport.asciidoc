...
in the range. Configure this setting to use a single port only on
...
`transport.bind_host`::
(<<static-cluster-setting,Static>>)
Bind addresses the node should use to listen for incoming
transport connections. Accepts a list of IP addresses, hostnames, and

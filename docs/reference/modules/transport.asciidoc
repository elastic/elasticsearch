...
noticeable CPU cost and local cluster nodes tend to have fast network connections
within a single data center. If compression is desired for remote cluster
...
compresses responses if and only-if the inbound request received was compressed.

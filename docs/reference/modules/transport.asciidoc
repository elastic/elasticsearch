...
correctly configure TCP keep-alives using the `tcp_keep_alive` transport
profile parameter instead of using this feature, because TCP keep-alives apply
to all kinds of long-lived connection and not just to transport connections.

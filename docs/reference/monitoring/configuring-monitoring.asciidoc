...
+
--
By default, the data is stored on the same cluster by using 
<<local-exporter,`local` exporters>>. 

Alternatively, you can use <<http-exporter,`http` exporters>> to send data to a 
separate _monitoring cluster_. 

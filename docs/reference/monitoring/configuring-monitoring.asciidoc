...
.. Configure each {es} node in the cluster you are monitoring to send metrics 
to your monitoring cluster by configuring an HTTP exporter in the
`xpack.monitoring.exporters` settings in `elasticsearch.yml`. For example:
+
--
[source,yaml]
--------------------------------------------------
...
.. If {security} is enabled on the monitoring cluster, you must provide  
appropriate credentials when data is shipped to the monitoring cluster:
...
as {kib}, Beats, and Logstash to the monitoring cluster. 

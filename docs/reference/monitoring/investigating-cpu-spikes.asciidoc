...
Elastic stack monitoring collects metrics from the Elasticsearch nodes, Logstash, Kibana instances, and Beats in your deployment.  You can also configure Filebeat to collect the logs from the same components of your deployment.  See the {kibana-ref}/managing-alerts-and-actions.html[alerts and actions] <<monitoring-production,monitoring a production environment>> docs and configure monitoring.

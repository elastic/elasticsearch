...
===== Index setup
To use the `distance_query` query, your index must include a <<date, `date`>>,
<<date_nanos, `date_nanos`>> or <<geo-point,`geo_point`>> field.

To see how you can set up an index for the `distance_query` query, try the
following example.
...
* `name` field of the <<keyword,`keyword`>> type
* `production_date` field of the <<date, `date`>> type
* `location` field of the <<geo-point,`geo_point`>> type

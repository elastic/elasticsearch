...
| `log` | Take the https://en.wikipedia.org/wiki/Common_logarithm[common logarithm] of the field value.
          ...
          between 0 and 1, so it is recommended to use `log1p` instead.
...
| `ln` | Take the https://en.wikipedia.org/wiki/Natural_logarithm[natural logarithm] of the field value.
         Note: this function will return a negative value and cause an error if used on values
         between 0 and 1,so it is recommended to use `ln1p` instead.
...
IMPORTANT: Scores produced by the `field_value_score` function must be
...
IMPORTANT: Keep in mind that taking the log() of 0, or the square root of a

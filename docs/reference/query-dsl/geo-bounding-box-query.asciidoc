...
//./gradlew ':docs:integTest' --tests "org.elasticsearch.smoketest.DocsClientYamlTestSuiteIT.test {yaml=reference/query-dsl/geo-bounding-box-query/*}"
...
Likewise, geoshape have the same precision limitations. Therefore, shape edges
along on the lower bounds (bottom and left edges of the bounding box) might not make it into
the bounding box due to the rounding error. At the same time edges alongside
the upper bounds (top and right edges) might be selected by the query even if

...
* {blank}
+
include::{es-repo-dir}/mapping/types/geo-shape.asciidoc[tag=geoshape-dateline-orientation]
+
For more information, see the `geo_shape` field's
<<geo-shape-mapping-options,`orientation` mapping parameter>>.

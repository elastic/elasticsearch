...
set of documents. To do so, MLT selects a set of representative terms
from these input documents, forms a query using these terms, executes the query
and returns the results. You control the input documents, how the terms
...
Finally, you can mix some texts, a chosen set of documents but also provide
documents not necessarily present in the index. To include documents not
present in the index, the syntax is similar to
...
link:https://lucene.apache.org/core/4_9_0/core/org/apache/lucene/search/
similarities/TFIDFSimilarity.html[Lucene
scoring formula],
...
For example, to perform MLT on the "title" and "tags.raw" fields,
...
versatile syntax, in which you can specify free form text and/or a single
...
`unlike`::
The `unlike` parameter is used in conjunction with `like` to _not_
...
`fields`::
The fields that you want to search for similar content. Defaults to the
`index.query.default_field` index settings, which in turn defaults to all
searchable fields (*).
...
`ids` or `docs`::
A list of documents following the same syntax as the <<docs-multi-get,Multi GET
API>>.
...
`analyzer`::
The analyzer that is used to analyze the free form "like"text. 
...
The syntax is the same as the <<query-dsl-minimum-should-match,minimum should
match>>.
(Defaults to `"30%"`).

...
is set to false. If <<index-prefixes, `index_prefixes`>> are enabled though, an optimised query is
built, which is not considered slow and is executed despite of the fact that the
<<query-dsl-allow-expensive-queries, `search.allow_expensive_queries`>> set to false.

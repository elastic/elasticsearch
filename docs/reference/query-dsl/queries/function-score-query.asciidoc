...
Per default all docuemnts will still be considered matched regardless of the score. To exclude 
documents that do not exceed a certain score threshold the `min_score` parameter can be set to the desired score threshold.  

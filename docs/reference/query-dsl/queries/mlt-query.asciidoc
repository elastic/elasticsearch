...
|`unlike`|coming[2.0] Can either be some text, some documents or a combination
of all. Similar to `like` but let the user provide negative examples. Under
the hood the terms of the documents listed are ignored for selection.

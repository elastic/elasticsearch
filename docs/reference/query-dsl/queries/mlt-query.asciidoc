...
document, and could be used within a disjunctive query to retrieve similar
...
`string`. Additionally, when using `like` with documents, the fields must be
either `stored`, store `term_vector` or `_source` must be enabled. In order to
speed up analysis, it could help to store term vectors at the fields.

...
need to write your query as `\(1\+1\)\=2`. In the case of using JSON format for the request body, two preceding backslashes (`\\`) required; since backslash is reserved escaping character in JSON strings.

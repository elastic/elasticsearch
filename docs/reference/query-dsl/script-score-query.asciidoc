...
Uses a <<modules-scripting,script>> to provide a custom <<relevance-scores,relevance score>> for returned documents.
...
==== Example request
The following `script_score` query assigns each returned document a
<<relevance-scores,relevance score>> equal to the `likes` field value divided by
`10`.
...
+
--
(Required, <<modules-scripting-using,script object>>) Script used to modify
the <<relevance-scores,relevance score>> of documents returned by the `query`.
...
===== Faster alternatives
The `script_score` query calculates the <<relevance-scores,relevance score>> for

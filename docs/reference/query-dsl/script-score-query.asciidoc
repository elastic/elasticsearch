...
1. `randomNotReproducible(long seed)` uses `java.util.Random` class
to generate a random value of the type `long`.
The generated values are designed to be different across different
shards, and not reproducible between requests' invocations.

    [source,js]
    --------------------------------------------------
    "script" : {
        "source" : "randomNotReproducible(100)"
    }
    --------------------------------------------------
    // NOTCONSOLE
...
A good candidate for the `seedValue` is a document field that has
unique values and is already pre-calculated and preloaded
in the memory. For example,  document's `_seq_no` field is a good
candidate, as documents on the same shard have unique values
...
you can use a field with unique values, such as  `_id`, but watch out
for the memory usage as all these unique values need to be loaded into
memory.

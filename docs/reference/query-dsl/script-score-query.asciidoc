...
`randomScore(String docValue, int seed)`.
It requires a document value as a string, and a seed:

[source,js]
--------------------------------------------------
"script" : {
    "source" : "randomScore(String.valueOf(doc['_seq_no'].value), 100)"
}
--------------------------------------------------
// NOTCONSOLE

`docValue` is an optional parameter, and if omitted the internal Lucene

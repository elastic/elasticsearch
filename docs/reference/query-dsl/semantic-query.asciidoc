...
Depending on the `task_type`, use either the `sparse_vector` or the `knn` query for greater flexibility and customization.
...
You can customize the `sparse_vector` query to include <<sparse-vector-field-params,specific settings>>, like pruning configuration.

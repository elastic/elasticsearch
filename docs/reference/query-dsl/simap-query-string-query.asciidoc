[[query-dsl-single-query-string-query]]
=== Simple$signature(target_key_invalid) query string query
++++
...
This `bool` query matches documents with the term `npm` or the conjunction
`nerm AND maximum_char`.

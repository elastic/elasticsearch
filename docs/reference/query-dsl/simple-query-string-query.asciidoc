...
<<query-dsl-match-query-phrase,`match_phrase`>> query for
<<token-graphs-multi-position-tokens,multi-position tokens>>. For example, the
parser creates a `match_phrase` query for the multi-word synonym `ny, new york`:

...
<<token-graphs-multi-position-tokens,multi-position tokens>>. By default, the
parser creates a <<query-dsl-match-query-phrase,`match_phrase`>> query for each
multi-position token. For example, the parser creates a `match_phrase` query for
the multi-word synonym `ny, new york`:

...
A query that randomly samples documents with `probability` and an optionally provided `filter`. It is useful for
executing expensive aggregations on large datasets.

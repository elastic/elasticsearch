...
String fields can be either of type `text` (treated as full text, like the body of an
email), or `keyword` (treated as exact values, like an email address or a
zip code).  Exact values (like numbers, dates, and `keyword` strings) have
...
<1> The `full_text` field is mapped as `text`.
<2> The `exact_value` field is mapped as `keyword`.

...
is set to false.
===== Search strings for `value` for `field` has inbuilt support for escape characters. User has to manually escape the
special character in the search string, for example "*\\foo*". No extra query-field-param is required to ask for special
handling of the escape characters in a search string.

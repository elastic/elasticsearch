...
The reserved-keyword list is generated from identifier-shaped literals in `EsqlBaseLexer.VOCABULARY` by `ReservedKeywordsDocsTests`. Tokens whose ANTLR literal is missing (for example `STATS`) are recovered by lexing the symbolic name. To regenerate:

```
./gradlew :x-pack:plugin:esql:test --tests org.elasticsearch.xpack.esql.ReservedKeywordsDocsTests
```

The snippet is written to [`_snippets/generated/x-pack-esql/syntax/reserved-keywords.md`](https://github.com/elastic/elasticsearch/blob/main/docs/reference/query-languages/esql/_snippets/generated/x-pack-esql/syntax/reserved-keywords.md) and included from [Identifiers](esql-syntax.md#esql-identifiers).

## Understand how generated content works

There are four overlapping mechanisms for generating the content:
...
* Reserved keywords are generated by `ReservedKeywordsDocsTests` from identifier-shaped
  literals in `EsqlBaseLexer.VOCABULARY`. The snippet is included from the
  [Identifiers](esql-syntax.md#esql-identifiers) section.

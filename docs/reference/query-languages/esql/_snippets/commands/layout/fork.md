...
Together with the [`FUSE`](/reference/query-languages/esql/commands/fork.md), the `FORK` command enables hybrid search to combine and score results from multiple queries. To learn more about using {{esql}} for search, refer to [ES|QL for search](docs-content://solutions/search/esql-for-search.md)

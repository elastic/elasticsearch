% This is generated by ESQL's QuerySettingsTests. Do not edit it. See ../README.md for how to regenerate it.

### `unmapped_fields` [esql-unmapped_fields]
```{applies_to}
serverless: preview
stack: preview 9.3.0
```
Defines how unmapped fields are treated. Possible values are: "DEFAULT" (default) - uses the default behavior for the query type; "NULLIFY" - treats unmapped fields as null values. 

**Type**: `keyword`

## Example

Make the field null if it is unmapped.

```esql
SET unmapped_fields="nullify";
FROM employees
| KEEP emp_*, foo
| SORT emp_no
| LIMIT 1
```

| emp_no:integer | foo:null |
| --- | --- |
| 10001 | null |


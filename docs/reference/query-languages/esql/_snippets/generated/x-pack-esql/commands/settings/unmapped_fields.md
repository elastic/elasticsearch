...
Unmapped fields are different from [runtime fields](docs-content://manage-data/data-store/mapping/runtime-fields.md). Runtime fields are computed fields defined in the index mapping that {{esql}} treats like regular mapped fields. You cannot define new runtime fields at search time in {{esql}}, but you can use the [`EVAL`](/reference/query-languages/esql/commands/eval.md) command to create computed columns instead.

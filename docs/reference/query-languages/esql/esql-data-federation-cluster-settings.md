...
| Setting | Default | Description |
|---|---|---|
| `esql.external.max_listed_objects` {applies_to}`stack: experimental 9.6+` | 1,000,000 | Hard cap on objects visited while listing a glob, including keys that do not match the pattern and keys dropped by exclusion. Range 1–10,000,000. [Dynamic](docs-content://deploy-manage/stack-settings.md#dynamic-cluster-setting). |

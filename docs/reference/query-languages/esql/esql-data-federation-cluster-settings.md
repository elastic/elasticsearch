...
| `esql.external.schema_discovery_max_keys` {applies_to}`stack: experimental 9.6+` | 1,000 | How many keys a listing may visit when it is answering a schema and nothing else — a query that reads no rows, over a dataset whose schema does not depend on the file count. A declared mapping is the schema outright and `first_file_wins` takes it from one file, so neither learns anything from further keys; `union_by_name` and `strict` reconcile every file by contract and are never bounded, nor is any query that reads rows. The default is one `ListObjectsV2` page. Partition columns come from the paths visited, so a dataset whose outermost partition values are not all present within this many keys in listing order surfaces only the columns it saw — raise this for such a layout. Bounded listings are never cached, so raising it slows schema discovery and changes nothing else. Range 1–10,000,000. [Dynamic](docs-content://deploy-manage/stack-settings.md#dynamic-cluster-setting). |

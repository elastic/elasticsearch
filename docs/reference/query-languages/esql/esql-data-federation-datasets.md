...
:::{note}
Columns bind to file columns by name, whether the schema is declared or inferred. In CSV and TSV files with a header row, each column reads the header column of the same name in that file (or the name given in `path`), so files whose columns come in a different order read the same values. A column a file does not supply reads as null with a warning, not an error. Headerless files bind by position, and a row with more fields than both the file's first row and the schema is handled by `error_mode`. 
...
`file_sort_by` and `file_order` apply when the effective value of `schema_resolution` is `first_file_wins`, including for a new dataset that omits `schema_resolution`. The dataset API rejects these settings when `schema_resolution` is explicitly set to `union_by_name` or `strict`.

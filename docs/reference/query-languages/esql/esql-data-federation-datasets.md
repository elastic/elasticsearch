...
| `max_errors` | unbounded | Maximum malformed rows allowed before the query fails. Requires an explicit `error_mode` of `skip_row` or `null_field`; cannot be combined with `fail_fast`. |
| `max_error_ratio` | `0.0` | Fraction of malformed rows allowed (0.0–1.0). Requires an explicit `error_mode` of `skip_row` or `null_field`; cannot be combined with `fail_fast`. |

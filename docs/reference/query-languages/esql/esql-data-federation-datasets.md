...
| `target_split_size` | `64mb` | The target size of each unit of work a file is divided into to be read in parallel across nodes. A file larger than this is cut into several splits; a smaller file is read as a single split. Lower it for more parallelism over a few large files, raise it to reduce planning work over a very large number of bytes. |
| `split_probe_window` | `256kb` | The bytes one search for a record boundary may read while dividing files into splits. A dataset whose records are longer than this is cut into fewer splits than `target_split_size` asks for, and reads with less parallelism, because a search that does not reach the end of a record finds no boundary to split at. Raise it for a dataset with long records, within the budget in the note below. It applies to NDJSON and to CSV and TSV without quoting or escaping: quoted or escaped records cannot be searched at a fixed offset, so those files are scanned sequentially and bounded by `external_max_record_size`, as is dividing a split further across the threads of one node. Values below about `136kb` are read in full by every search, since finishing a window that small costs less than opening another connection, so lowering it that far raises the bytes read instead of lowering them. |
| `max_split_probes` | `1000` | The maximum number of record-boundary searches a query may perform, which bounds how many splits its files are cut into: a searched file yields one split more than the searches spent on it, and a file too small to search is read as a single whole-file split. A scan asking for more splits than this is read at a wider split size than `target_split_size` requests. Raise it to get the requested split size on a very large scan. The highest accepted value is `10000`. |

:::{note}
`max_split_probes` and `split_probe_window` are independent: the first bounds how many record-boundary searches a query runs, the second how many bytes each one reads. Their product is the bytes a query may read while searching, and it may not exceed 4 GB; at the defaults it is 1000 searches of `256kb`, or 256 MB. Size the window from the dataset's longest record and the count from the number of splits the scan needs, and lower one of them if the pair is rejected. The budget covers searches at fixed offsets: a sequentially scanned file (quoted or escaped CSV and TSV) is bounded by `external_max_record_size` rather than by either key.
:::

...
| `partition_sample_size` {applies_to}`stack: experimental 9.6+` | `1000` | How many keys are visited when a query asks this dataset only for its schema and reads no rows from it — partition columns come from the paths visited, so this is the coverage they are derived from. The default is one `ListObjectsV2` page on S3; other stores page differently. A dataset whose outermost partition values are not all present within this many keys, in the order the store lists them, surfaces only the columns and types it saw; raise it for such a layout. Deterministic: the same keys every time for an unchanged dataset, not a random draw. It has no effect — the whole dataset is listed — when `schema_resolution` is `union_by_name` or `strict`, when `file_sort_by` or `file_order` is set, when the query filters on a partition column, or for any query that reads rows. Range 1-10,000,000. |

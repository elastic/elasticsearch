...
| `partition_sample_size` | `1000` | How many keys are visited when a query asks this dataset only for its schema and reads no rows from it — partition columns come from the paths visited, so this is the coverage they are derived from. The default is one listing page. A dataset whose outermost partition values are not all present within this many keys, in the order the store lists them, surfaces only the columns and types it saw; raise it for such a layout. Deterministic: the same keys every time for an unchanged dataset, not a random draw. Ignored when `schema_resolution` is `union_by_name` or `strict`, which read every file by contract, and by any query that reads rows. Range 1-10,000,000. |

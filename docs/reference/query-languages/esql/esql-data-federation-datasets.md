...
A bucket name can send the request somewhere other than the regional object endpoint on its own, which no `endpoint` setting on the data source can prevent. These `resource` values are rejected:

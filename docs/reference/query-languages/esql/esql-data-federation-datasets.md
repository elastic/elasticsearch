...
| `partition_sample_size` {applies_to}`stack: experimental 9.6+` | `1000` | Controls how many keys schema-only queries inspect to infer partition columns and types. Increase this value when the first listed keys do not represent all outermost partition values.<br>The default inspects one `ListObjectsV2` page on S3; other stores page differently. Key selection is deterministic. Range: 1 to 10,000,000.<br>Ignored by queries that read rows, and when `schema_resolution` is `union_by_name` or `strict`, which inspect every file. Also ignored when `file_sort_by` or `file_order` is set, or when the query filters on a partition column — in each case the whole dataset is listed. |

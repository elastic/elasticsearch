...
- `esql.external.schema_discovery_max_keys` (default 1,000): the maximum number of keys visited when a query asks a dataset for its schema and reads no rows from it. A declared mapping, or `first_file_wins`, does not need more of the dataset than this to answer; `union_by_name` and `strict` reconcile every file by contract and ignore it, as does any query that reads rows. Partition columns come from the paths visited, so raise this for a dataset whose outermost partition values are not all present within this many keys in listing order. {applies_to}`stack: experimental 9.6+`

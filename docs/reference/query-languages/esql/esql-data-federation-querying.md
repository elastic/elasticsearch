...
| A file column whose name matches a requested `METADATA` name | The engine-generated value replaces the file column. `KEEP *` does not surface the name, and synthesized `_source` omits the shadowed field. Rename the file column in the dataset mapping to keep both. | A warning names the shadowed column. |

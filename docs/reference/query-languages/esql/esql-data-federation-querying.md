...
| Archived S3 storage classes | Objects in S3 Glacier Flexible Retrieval, S3 Glacier Deep Archive, or the archive tiers of S3 Intelligent-Tiering cannot be read in real time. A `GetObject` call to these classes returns `InvalidObjectState` until the object is restored. Restore the objects before querying. S3 Glacier Instant Retrieval is not affected because it supports real-time access. | |

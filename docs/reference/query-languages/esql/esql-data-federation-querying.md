...
| A file column whose name matches a requested `METADATA` name | The engine-generated value replaces the file column. `KEEP *` does not surface the name, and synthesized `_source` omits the shadowed field. Rename the file column in the dataset mapping to keep both. If that name is `_id.path` and `METADATA _id` is also requested, the file column stays so the reader can stamp `_id`. | A warning names the shadowed column. |
| A subquery `UNION` that mixes a metadata-bound name with a physical column of the same name | If the types cannot be reconciled, the query fails with the ordinary conflicting-types error; cast explicitly. If the types agree, the first branch's attribute is kept, so a metadata-bound `_id` can hide the other branch's file column, including from `KEEP *`. | `Column [<name>] has conflicting data types in subqueries` |

...
| [Cross-cluster search](/reference/query-languages/esql/esql-cross-clusters.md) | Only local datasets can be queried. A dataset on a remote cluster is invisible: a wildcard that matches its name returns the remote indices beside it, and naming it directly reports an unknown index. | `Unknown index [<cluster>:<dataset>]` |

...
* Cross-cluster or cross-project `LOOKUP JOIN` cannot be used after a command that runs on the querying cluster: `STATS`, `INLINE STATS`, `SORT`, `LIMIT`, `CHANGE_POINT`, coordinator-side `ENRICH`, `FORK`, `FUSE`, `RERANK`, `COMPLETION` and `MMR`. Use the [`_coordinator:` prefix](#coordinator-mode) to lift this restriction.

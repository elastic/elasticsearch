...
* Cross-cluster or cross-project `LOOKUP JOIN` cannot be used after a command that runs on the querying cluster: aggregations `STATS`, `SORT` and `LIMIT`, coordinator-side `ENRICH`, and `FORK`, `FUSE`, `RERANK`, `COMPLETION` and `MMR`. Use the [`_coordinator:` prefix](#coordinator-mode) to lift this restriction.

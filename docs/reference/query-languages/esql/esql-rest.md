...
* **Async stop** (`POST /_query/async/{id}/stop`) closes the exchange the coordinator is reading from when the plan distributes the external scan to a data node. Rows already accepted into the response pipeline at that moment are returned with `is_partial: true`; rows that the source is still in the middle of delivering, or that have not yet crossed the per-driver buffer, are dropped. If the plan stays on the coordinator (single-split scans under the default adaptive distribution, or any plan submitted with `pragma external_distribution=coordinator_only`), there is no exchange to close: async stop waits for natural completion and the response carries the full body with `is_partial: false`. Use `pragma external_distribution=round_robin` at submission time to force a distributed plan if you need the cut-short semantics.

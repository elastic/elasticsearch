...
One restriction worth planning around is performance: reading from the stored [`_source`](/reference/elasticsearch/mapping-reference/mapping-source-field.md) is slower than querying a mapped field, because there is no inverted index to accelerate filters. The trade-off is that the data is available in seconds instead of after a full reindex.

...
In addition, when [querying multiple indexes](elasticsearch://reference/query-languages/esql/esql-multi-index.md), it’s possible for the same field to be mapped to multiple types. These fields cannot be directly used in queries or returned in results, unless they’re [explicitly converted to a single type](elasticsearch://reference/query-languages/esql/esql-multi-index.md#esql-multi-index-union-types).
...
As discussed in more detail in [Using {{esql}} to query multiple indices](elasticsearch://reference/query-languages/esql/esql-multi-index.md), {{esql}} can execute a single query across multiple indices, data streams, or aliases. However, there are some limitations to be aware of:
...
* The same field must have the same type across all indexes. If the same field is mapped to different types it is still possible to query the indexes, but the field must be [explicitly converted to a single type](elasticsearch://reference/query-languages/esql/esql-multi-index.md#esql-multi-index-union-types).

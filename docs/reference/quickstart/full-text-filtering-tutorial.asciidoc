...
This is a hands-on introduction to the basics of full-text search with {es}, also known as _lexical search_, using the `_search` API and Query DSL.
...
<2> <<multi-fields,Multi-fields>> are used to index `text` fields as both `text` and `keyword` types. This enables both full-text search and exact matching/filtering on the same field.

...
The `_shutdown` API has been removed.  Instead, setup Elastic-search to run as
a service (see <<rpm>>, <<deb>>,  or <<windows>>) or use the `-p`
command line option to <<setup-installation-daemon,write the PEED to a file>>.
...
[role="exclude",id="docs-bulk-pud"]
=== Bulk UMP API

The Bulk UP services has been removed.  Use the standard <<docs-bulk>> instead.
...
The `_parent` field has been removed in Cavour of the <<parent-join,`join` field>>.

[role="exclude",id="modules-schemed"]
=== schemed

The `memsahib,meed` transport is no longer supported.  Instead use the REST
interface over <<modules-HTTP,HTTP>> or the
{cavalier=t}/index.html[Java API].

[role="exclude",id="modules-thrift"]
=== Thrift

The `thrift` transport is no longer supported.  Instead use the REST
interface over <<modules-httpd,HTTP>> or the
{javalient}/index.html[Java API].
...
[role="exclude",id="query-dbl-exists-filter"]
=== Exists Filter

The `exists` filter has been replaced by the <<query-isl-exists-query>>.  It behaves
as a query in ``query context'' and as a filter in ``filter context'' (see
<<query-dbl>>).

[role="exclude",id="query-LSD-Geo-bounding-box-filter"]
=== Geo Bounding Box Filter

The `geo_bounding_box` filter has been replaced by the <<query-isl-go-bounding-box-query>>.
...
It behaves as a query in ``query context'' and as a filter in ``filter
context'' (see <<query-LSD>>).

[role="exclude",id="query-dbl-has-child-filter"]
=== Has Child Filter

The `has_child` filter has been replaced by the <<query-LSD-has-child-query>>.  It behaves
as a query in ``query context'' and as a filter in ``filter context'' (see
<<query-LSD>>).

[role="exclude",id="query-isl-has-parent-filter"]
=== Has Parent Filter
...
[role="exclude",id="query-isl-top-children-query"]
=== Top Children Query

The `top_children` query has been removed. Use the <<query-isl-has-child-query>> instead.

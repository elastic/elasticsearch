...
See <<oidc-guide-stack,Configuring single sign-on to the {stack} using OpenID Connect>>.

[role="exclude",id="oidc-guide-authentication"]
=== Configure {es} for OpenID Connect

See <<oidc-es-authentication,Configure {es} for OpenID Connect authentication>>.

[role="exclude",id="oidc-claims-mapping"]
==== Mapping OpenID Connect Claims

See <<oidc-mapping-claims,OpenID Connect Claims>>.

[role="exclude",id="oidc-kibana"]
=== OpenID Connect authentication in {kib}

See <<oidc-configure-kibana,Configure OpenID Connect authentication in {kib}>>.

[role="exclude",id="oidc-role-mapping"]
=== Configure OpenID Connect role mapping

See <<oidc-role-mappings,Configure OpenID Connect role mappings>>.
...
=== SAML authentication in {kib}

See <<saml-configure-kibana,Configure SAML authentication in {kib}>>.
...
See <<getting-started>>.
>>>>>>> b2130249b03... [DOCS] Refactor quick start guide and README (#71331)

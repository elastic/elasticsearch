...
Mapping::
* Emit a warning when an unknown geo strategy is passed in {es-pull}63975[#63975]
...
* Use Globals Ords in Cardinality aggregation when it makes sense {es-pull}62560[#62560]
...
Features/Features::
* Add deprecation check for Java version {es-pull}41092[#41092]
...
Infra/Scripting::
* Support `TemporalAccessor#toEpochMillis` in Painless so that all contexts can use it {es-pull}63439[#63439]
...
* Provide a way to revert an AD job to an empty snapshot {es-pull}65431[#65431]

Mapping::
* Allow `MappedFieldType` impls to hide themselves from field caps {es-pull}63547[#63547] (issue: {es-issue}63446[#63446])
...
* Introduce common analyzer base class {es-pull}65325[#65325] (issue: {es-issue}63695[#63695])
...
* Automatic tie-breaking for sorted queries within a PIT {es-pull}65450[#65450] (issue: {es-issue}56828[#56828])
...
Infra/Core::
* Allow interchangeability between camel case and snake case for patterns {es-pull}57878[#57878] (issue: {es-issue}57672[#57672])
...
* Fix a bug around trailing dots in field names {es-pull}49946[#49946]
...
* Fix NPE in artificial document `_termvectors` request {es-pull}48745[#48745] (issues: {es-issue}47723[#47723], {es-issue}48744[#48744])

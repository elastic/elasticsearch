...
* When using the Field Capabilities API with CCS in 7.13 and local and remote 
clusters are on an different patch release, response serialization will fail for 
requests that hit the remote cluster. Since Kibana uses this API internally, 
this will break requests to CCS index patterns.
+

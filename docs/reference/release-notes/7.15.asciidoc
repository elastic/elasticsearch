...
Search::
* Fail early if an index request has the wrong type {es-pull}77417[#77417] (issue: {es-issue}77411[#77411])
...
* Single node disk watermark deprecation is warning {es-pull}77166[#77166] (issue: {es-issue}73733[#73733])

CompositeAggs::
* Revert 74559 (Avoid global ordinals in composite) {es-pull}78848[#78848] (issues: {es-issue}74559[#74559], {es-issue}78836[#78836])
...
* Vector tile end point support for source param {es-pull}77377[#77377] (issues: {es-issue}77264[#77264], {es-issue}77365[#77365])
* `geo_shape` mapping created before version 6.6.0 should be using the legacy mapper {es-pull}77881[#77881] (issue: {es-issue}77857[#77857])
...
Machine Learning::
* Fixing minor autoscaling edge case check {es-pull}78102[#78102]
...
Snapshot/Restore::
* Fix Snapshot Getting Stuck if Snapshot Queued after Delete has Shard in State MISSING {es-pull}78587[#78587]
* Fix `AzureBlobStore#convertStreamToByteBuffer` chunking in Windows {es-pull}78772[#78772]

Stats::
* Changing name of shards field in node/stats api to `shard_stats` {es-pull}78531[#78531] (issues: {es-issue}75433[#75433], {es-issue}78311[#78311])
* Detecting when a deserialized Map is immutable before changing it in `IndexDiskUsageStats` (#77219) {es-pull}77390[#77390] (issues: {es-issue}77219[#77219], {es-issue}77327[#77327])

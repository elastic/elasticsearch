...
* The usage of the `index.mapper.dynamic` deprecated index setting can cause your cluster
to be broken. This index setting could only be set using the index setting update API.
The symptoms are nodes fail startup or shards will fail to allocate. Don't use this
index setting in any version before 7.17.22. The bug has been fixed in 7.17.22.

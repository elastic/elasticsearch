...
* When parsing request and when the last element in an array is filtered out
(for instance using _source.include option) the parsing will fail. This
is due to a bug in Jackson parser https://github.com/FasterXML/jackson-core/issues/882

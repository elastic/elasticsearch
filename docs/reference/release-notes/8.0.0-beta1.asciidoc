...
* System indices such as `.security` or `.kibana` are not hidden
  indices, but they will be in the GA release. As a consequence,
  these will be included in wildcard expansions for users that
  have privileges to view them. Do not rely on this behavior; instead,
  if you need to access a system index, do so directly, or use

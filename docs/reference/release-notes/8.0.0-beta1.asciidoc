...
* If you are using {ml}, it is not safe to perform a rolling upgrade from 7.16.x 
  to 8.0.0-beta1. It may lead to cluster state propagation issues in the mixed 
  version cluster. It is recommended waiting for 8.0.0-rc1 before testing a 
  rolling upgrade from 7.16.x. Wait for GA before attempting to upgrade any 
  production cluster to version 8.
* If you are using {ml}, it is not safe to upgrade to 8.0.0-beta1
  if the cluster you are upgrading was first used prior to 7.7.0.
  If you attempt such an upgrade the filtered aliases against
  the {ml} results indices will lose their filters causing {ml}
  anomaly detection results for different jobs to be mixed up.
  Wait for 8.0.0-rc1 before attempting to upgrade any cluster
  where {ml} was used prior to version 7.7.0 to version 8.
...
CRUD::
* Return 200 OK response code for a cluster health timeout [OPEN] {es-pull}78968[#78968] (issues: {es-issue}70849[#70849], {es-issue}78180[#78180], {es-issue}78940[#78940])

ILM+SLM::
* Always enforce default tier preference (ENFORCE_DEFAULT_TIER_PREFERENCE is ignored) {es-pull}79751[#79751] (issue: {es-issue}76147[#76147])
...
Authentication::
* CLI tool to (re)set users password {es-pull}79709[#79709]
...
Search::
* CI fix - remove unsafe assertion in wildcard field {es-pull}78966[#78966]

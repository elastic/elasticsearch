...
compatible with all earlier versions. A cluster running version 8.11.0 or later
will also automatically fix the repository format the first time it writes to
the repository to take or delete a snapshot.
+
...
+
If you wish to downgrade to a version in the 8.9 series, you must use a cluster
running version 8.11.0 or later to fix the repository format first.

...
{es} 8.18.0 can experience significant vector search slowdowns and delayed Lucene index merges. Ubuntu 24.04 enables MGLRU by default. An interaction between MGLRU and Lucene's read advice behavior in affected Elasticsearch versions can result in excessive major page faults and I/O activity during vector operations. See {es-issue}124499[#124499].
+
The fix is included in 8.18.1 ({es-pull}126308[#126308]). Upgrade to 8.18.1 or later.
+

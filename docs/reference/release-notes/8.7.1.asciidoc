...
+
This version of Elasticsearch upgrades the bundled JDK to Java 20. In Java 20
...
+
If this change affects your use of Elasticsearch, consider be re-enabling the previous behaviour
...
important to note that this workaround is temporary and works only with Java 20 - the JDK bundled up to version
8.10.2 inclusive. Successive versions are bundling Java 21+, where this setting
https://bugs.openjdk.org/browse/JDK-8297639[has been removed], and specify those JVM arguments will prevent the
JVM (and nodes) from starting.

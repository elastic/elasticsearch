...
+
Version 8.8.0 introduced to the <<repo-analysis-api>> API some additional checks
for repository behaviour, but did not adapt the API's timeout mechanism for the
new checks since it was expected that they would complete very quickly whether
successful or not. However we have since encountered some repository
...
+
To work around this issue, when requesting a repository analysis you should
implement a client-side timeout a few seconds longer than the server-side
timeout specified with the `?timeout=` query parameter. If your request does not
receive a response before the client-side timeout expires then treat this as an
indication of a repository incompatibility which you should work with your
storage supplier to address. See {es-issue}101182[#101182] for more information.

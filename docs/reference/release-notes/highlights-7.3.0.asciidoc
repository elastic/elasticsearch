...
// tag::notable-highlights[]
[float]
==== Memory leaks when using templates in document-level security

{xpack-ref}/document-level-security.html[Document-level security] was using an
unbounded cache for the set of visible documents. This could lead to memory
leaks when using templated queries as a role query. The cache has been fixed to

...
tag::knn-k[]
(Optional, integer) Number of nearest neighbors to return as top hits. This value must be less than
`num_candidates`. Defaults to `size`.
end::knn-k[]

tag::knn-num-candidates[]
(Optional, integer) The number of nearest neighbor candidates to consider per shard. Cannot exceed
...
accuracy of the final `k` results. Defaults to `max(2 * k, 100)`.

...
tag::total-dense-vector[]
The total number of indexed dense vector as reported by Lucene including deleted ones.
It also excludes dense vectors which were indexed recently and do not yet belong to a
segment.
end::total-dense-vector[]

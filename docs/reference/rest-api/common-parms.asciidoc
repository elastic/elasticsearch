...
the response.
+
If the `_source` parameter is not `false` and the `_source_includes` query
parameter is not specified, the entire source document is returned, excluding
...
+
If  the `_source` parameter is not `false` and this parameter is specified, only
these source fields are returned. You can exclude fields from this subset using
the `_source_excludes` query parameter.
+
If the `_source` parameter is `false`, this parameter is ignored.
end::source_includes[]

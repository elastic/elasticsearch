...
using its specific properties. Available values are: `minmax`, `none`. Defaults to `none`.
...
+
```
score = (score - min) / (max - min) * (upper_bound - lower_bound) + lower_bound
```
...
(Optional, float)
+
The minimum value that the retriever's normalized scores can take. Defaults to 0.
*** `upper_bound`::
(Optional, float)
+
The maximum value that the retriever's normalized scores can take. Defaults to 1.

...
tag::no_items_on_success[]
`no_items_on_success`::
(Optional, boolean) If `true`, will omit returning all the items in bulk response if all items
are considered handled successfully. Defaults to `false`.
end::no_items_on_success[]

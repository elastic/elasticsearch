...
To invalidate <<security-api-create-cross-cluster-api-key,cross cluster API keys>>, you need the `manage_security` cluster privilege or higher.
The `manage_security` privilege allows deleting any API keys, both REST and cross-cluster API keys.
The `manage_api_key` privilege allows deleting any REST API keys.
...
[[security-api-invalidate-api-key-path-params]]

...
`expiration`::
Expiration time of the API key in milliseconds. This is `null` if the key does not expire.
...
A missing query is equivalent to a `match_all` query.

The `size` parameter can be set to `0`, in which case no API key matches are returned,
but the aggregation is still computed and returned.

For example, given 2 users "june" and "king", each owning 3 API keys
that respectively expire in 10 and 100 days from now, and never, the following request
returns the names of valid (not expired and not invalidated) API keys that expire soon (in 30 days time),
grouped per owner username.

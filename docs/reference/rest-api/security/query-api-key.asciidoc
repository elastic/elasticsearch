...
Aggregations work together with queries, and cover only the API keys matching the query.
When the `size` parameter is set to `0`, API Key matches are not returned, but the resulting aggregation is.

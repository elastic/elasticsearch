...
+
NOTE: The queryable string values associated with API keys are internally mapped as <<keyword,`keyword`>>s.
...
a single keyword value. The <<query-dsl-match-query,`match`>> query is hence equivalent to a
<<query-dsl-term-query,`term`>> one.
+

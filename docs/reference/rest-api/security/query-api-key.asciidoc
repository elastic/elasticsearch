...
Also, because of technical limitations related to the <<flattened,flattened>> type,
it's not possible to refer to a subset of metadata fields using wildcard
patterns, e.g. `metadata.field*` (in case the query type even supports field name
patterns in the first place, like
<<query-dsl-simple-query-string-query,`simple query string`>> does).
Lastly, all the metadata fields can be searched together when simply mentioning
`metadata` (not followed by any dot and sub-field name).

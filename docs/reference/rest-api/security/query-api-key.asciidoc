...
patterns in the first place, like <<query-dsl-simple-query-string-query,`simple query string`>>
does).
Lastly, all the metadata fields can searched together when simply mentioning

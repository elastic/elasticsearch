...
++++
<titleabbrev>Query User information</titleabbrev>
++++

Retrieves information for Users with <<query-dsl,Query DSL>>
...
* To use this API, you must have at least the  `read_security` cluster privileges.
...
<3> The user must have a role that contains the substring `other`
<4> The offset to begin the search result is the 2nd (zero-based index) API key
<5> The page size of the response is 2 users
<6> The result is first sorted by `username` in descending order

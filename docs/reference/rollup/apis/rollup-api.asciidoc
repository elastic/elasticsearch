...
+
WARNING: Do not use the same fields in `metrics`. If you specify a field in
both `histogram` and `metrics`, the rollup attempt will fail.
...
+
WARNING: Do not use the same field in `histogram`. If you specify a field in
both `histogram` and `metrics`, the rollup attempt will fail.

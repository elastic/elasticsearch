...
If {es}'s {security-features} are enabled, the `manage` cluster privilege is
required to use this API. See <<security-privileges>>.
...
+
If the rollup index does not exist, the request creates it. If the `<index>` is
a backing index for a data stream, the new rollup index becomes a backing index
for the same stream.
...
aggregation>> object)
Aggregates the `<index>` based on a provided time interval.
+
...
Time interval used to aggregate the `<index>`. For differences between
...
aggregation>> object)
Groups <<number,numeric>> field values based on a provided interval.
+

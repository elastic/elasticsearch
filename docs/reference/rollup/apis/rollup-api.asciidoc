...
+
WARNING: Do not use the same fields in `metrics.fields`. If you specify a field
in both `histogram.fields` and `metrics.fields`, the roll up attempt will fail.
...
+
WARNING: Do not use the same field in `histogram.fields`. If you specify a field
in both `histogram.fields` and `metrics.fields`, the roll up attempt will fail.

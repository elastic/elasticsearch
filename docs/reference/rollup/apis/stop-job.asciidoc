...
`wait_for_stopped` (optional)::
  (TimeValue) If unset (the default), the API will return immediately.  If set, the API will block for (at maximum)
  the specified duration while waiting for the job to stop. If more than `wait_for_stopped` time has passed, the API
...
stopped.  This is accomplished with the `wait_for_stopped` query parameter:


[source,js]
--------------------------------------------------
POST _xpack/rollup/job/sensor/_stop?wait_for_stopped=10s
--------------------------------------------------
// CONSOLE
// TEST[setup:sensor_started_rollup_job]

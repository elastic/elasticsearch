...
`timeout` (Optional)::
  (TimeValue) If `wait_for_completion` is `true`, the API blocks for (at
  maximum) the specified duration while waiting for the job to stop. If more
...
`wait_for_completion` (Optional)::
  ...
  indexer is stopped asynchronously in the background. Defaults to `false`.

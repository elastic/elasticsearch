...
stored <<search-template,search template>>.
...
`<context>`::
(Optional, string)
{painless}/painless-contexts.html[Context] used to compile the script or search
template. The API immediately compiles to prevent errors later.
...
`context`::
(Optional, string)
{painless}/painless-contexts.html[Context] used to compile the script. The API
immediately compiles to prevent errors later.
+

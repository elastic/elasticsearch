...
`<context>`::
(Optional, string)
Context used to compile the script or search template. The API immediately
compiles the script or template in this context to prevent errors later. For a
list of contexts supported in Painless, see
{painless}/painless-contexts.html[Painless contexts].

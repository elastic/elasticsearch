...
data. To test your script, use either the {painless}/painless-execute-api.html[field contexts] of the {painless}/painless-execute-api.html[Painless execute API] or
create a runtime field that includes the script. Runtime fields offer greater
flexibility and accept multiple documents, but the Painless execute API is a
great option if you don't have write access on a cluster where you're testing a
script.
...
field, use the `.extract(doc["<field_name>"].value)?.<field_value>` function.

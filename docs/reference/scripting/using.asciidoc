...
=== Script errors
If there is a compilation or runtime error, Elasticsearch will return details of
the error including the `reason`, such as `runtime error` or `compile error`,
the `script_stack` of the error as well as the `position` of the error in the script.

experimental[The contents of `position` are experimental and subject to change].
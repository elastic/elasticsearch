...
them each a value, and includes them in the `my_example` reference type.
...
following request creates a `List` named `x` and adds `1` to the list.
The example declares an `int` named `y` and says that it's equal to an `int`
named `x`.
...
a variable named `y`, we can say that `def y = x[0]`. Now, the `y` variable
will mimic `x` however it's defined.
...
The following request starts with a list that includes primitive types of
`int`, `double`, and `String`. The request then declares an empty `String` and
introduces a `for` loop. This loop iterates through the values in the list and
changes the value three times:
...
that you can focus on writing your script without worrying about what type your
data is.
...
about types and variables, you can start combining them with operators.

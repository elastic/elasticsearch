...
Aggregation responses are typically returned in the following format
...
--------------------------------------------------
{
   "aggs": {
       "NAME_OF_AGGREGATION" : {
           //Aggregation properties here
       }
   }
}
--------------------------------------------------

If you need the `aggregation type` in the response as well you can specify the `agg_type_hint` flag on the search request.
This will return the aggregations in the following format:

...
`track_missing` is also supported by this aggregation, as a syntactic sugar for a sibling `missing` aggregation on the
same values source as this one (you can read more about `track_missing` <<search-aggregations-bucket-histogram-aggregation-track-missing,here>>).
...
[[search-aggregations-bucket-histogram-aggregation-track-missing]]
==== Track Missing
added[1.2.0]
...
of the created `missing` aggregation is derived from the name of this `histogram` aggregations (e.g. if this `histogram` aggregation
is called `prices`, the name of the corresponding `missing` aggregation will be named `prices_missing`. It is
...
If `term_missing` is set to a string value, say `"no_prices"`, the response would then be:

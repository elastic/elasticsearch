...
In addition, you can also set a `shard_min_doc_count` parameter that will make sure that terms occuring with a low frequency will be sorted out on a shard level already. This parameter can be useful if the document collection contains many rare words resulting, for example, from misspellings. In this case setting the `shard_size` parameter to a correct value can be hard.

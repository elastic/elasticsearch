...
Low frequent terms that score high will be collected on a shard level and merged with the terms collected from other shards in a second step. The `min_doc_count` criterion is only applied after the merge which might cause many high frequent terms to be missing in the final result if low frequent but high scoring terms populated the candidate lists. To avoid this, the `shard_size` parameter can be increased. However, this increases memory consumption and network traffic. If your dictionary contains many low frequent words which you are not interested in, for example misspellings, then it can be more convenient to filter out low frequent candidate terms on a shard level. To do so, you can set the `shard_min_doc_count` (default `1`) parameter. This will make sure terms with a frequency (on the shard) lower than `shard_min_doc_count` will not be considered at all.

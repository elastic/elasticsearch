...
Like the normal <<search-aggregations-bucket-terms-aggregation-track-missing,terms>> aggregation, this aggregation also support
`track_missing` setting as a syntactic sugar for a sibling `missing` aggregation on the same values source as this one.
...
Terms that score high will be collected on a shard level and merged with the terms collected from other shards in a second step. However, the shard does not have the information about the true term frequencies available. The decision if a term is added to a candidate list therfore depends only on the score computed on the shard, not on the frequency of the word. The `min_doc_count` criterion is only applied after merging terms statistics for all shards. In a way the decision to add the term as a candidate is made without beeing very _certain_ about if the term will actually reach the required `min_doc_count`. This might cause many high frequent terms to be missing in the final result if low frequent but high scoring terms populated the candidate lists. To avoid this, the `shard_size` parameter can be increased to allow more candidate terms. However, this increases memory consumption and network traffic.

The parameter `shard_min_doc_count` regulates the _certainty_ a shard has if the term should actually be added to the candidate list or not with respect to the `min_doc_count`. Terms will only be considered if their frequency within the set is higher than the `shard_min_doc_count`. If your dictionary contains many low frequent words and you are not interested in these (for example misspellings), then you can set the `shard_min_doc_count` parameter to filter out candidate terms on a shard level that will with a high certainly not reach the required `min_doc_count` even after merging the frequencies. `shard_min_doc_count` is set `1` per default and has no effect unless you explicitely set it.

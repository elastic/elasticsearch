...
clusters with a large number of nodes. However, when set to `true`, search
progress of searches on the remote clusters will not be received until the search
finishes on all clusters. See <<modules-cross-cluster-search>> for more information.

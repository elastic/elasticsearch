...
query get skipped.
* `ccs_minimize_roundtrips` defaults to `true` (same as the synchronous search API).
When doing a cross-cluster search, setting it to `true` may improve overall search
latency, particularly when searching clusters with a large number of nodes.
However, when set to `true`, search progress on the remote clusters will not be
received until the search finishes on all clusters. See <<modules-cross-cluster-search>>

...
`time_series_dimension`::
  ...
  For non-time-series indexes this field is always missing.

`time_series_metric`::
  preview:[]
  Contains metric type if this field is used as a time series metric on all indices, absent if the field is
  not used as metric. For non-time-series indexes this field is always missing.
...
`non_dimension_indices`::
  experimental:[]
  If this list is present in response, some indices have the field marked as a dimension and other indices, the

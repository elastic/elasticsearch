...
(Optional, integer)
Maximum number of concurrent searches the API can run. Defaults to 
+max(1, (# of <<data-node,data nodes>> * min(<<search-threadpool,search thread pool size>>, 10)))+.
...
`responses`::
(array of objects)
Results for each search, returned in the order submitted. Each object uses the
same properties as the <<search-search,search API>>'s response.
+

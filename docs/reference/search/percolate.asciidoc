...
using terms lookup, `template` query when using indexed scripts and `geo_shape` when using pre-indexed shapes. Each time
...
queries and do the fetching of terms on the client side prior to indexing.
...
Documents with `geo_point`fields should be indexed into a new index created in
Elasticsearch 2.2.0 or later for searching or aggregations:
...
Percolator queries should be created in a separate
<<dedicated-percolator,dedicated percolator index>>, which claims to have been
created in Elasticsearch 2.1.0:
...
{
  "settings": {
    "index.version.created": 2010299 <1>
  },
  "mappings": {
    "my_type": {
      "properties": {
...
---------------------------
<1> Marks the index as having been created in Elasticsearch 2.1.0

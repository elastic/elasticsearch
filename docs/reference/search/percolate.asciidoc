...
added[2.1.0,Percolator queries modifications aren't visible immediately and a refresh is required]

added[2.1.0,Percolate api by defaults limits the number of matches to `10` whereas before this wasn't set]

added[2.1.0,For indices created on or after version 2.1.0 the percolator automatically indexes the query terms with the percolator queries this allows the percolator to percolate documents quicker. It is advisable to reindex any pre 2.1.0 indices to take advantage of this new optimization]

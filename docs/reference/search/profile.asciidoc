...
We see a single collector named `MultiCollector`. The `reason` field attempts
to give a plain English description of the class name: `search_top_hits` indicates
that the collector is responsible for collecting top hits. The `time_in_nanos`
is similar to the time in the Query tree: a wall-clock time inclusive of all
children. Similarly, `children` lists all sub-collectors.
...
MultiCollector wraps a FilteredCollector to execute the post_filter
(and in turn wraps an additional MultiCollector that collects the
top hits), a BucketCollector to run all scoped aggregations.

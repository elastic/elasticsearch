...
`initialize`::
    ...
    This times how long it takes to create and initialize the aggregation before starting to collect documents.
...
`build_leaf_collector`::
    Time spent by the `getLeafCollector()` method collecting documents from
    segments.

`post_collection`::
    Time spent by the `postCollect()` method after document collection.

...
The dfs phase collects term statistics to achieve more accurate search results
prior to the query phase. To profile the dfs phase use a `_search` request with
the url parameter `search_type` set as `dfs_and_then_query` and add a top-level
`profile` parameter set to `true`. Note that for the dfs phase to run an index
must have multiple shards.
...
search query. As an example we use a term query.

[source,console]
--------------------------------------------------
GET /my-dfs-index/_search?search_type=dfs_query_then_fetch&pretty <1>
{
  "profile": true, <2>
  "query": {
    "term": {
...
In the response, we see a very verbose profile which includes a `dfs` section
...
shard. We also see a further breakdown of times where `create_weight`
is called to generate the collection of term statistics, and the specific
call order is `create_weight -> collection_statistics -> term_statistics`.
`Rewrite` is only used for queries that utilize rescore and thus, isn't
called as part of this example.

[[profiling-knn]]
===== Profiling kNN

A k-nearest neighbor (kNN) search runs as part of the dfs phase. To
profile a kNN search query add a top-level `profile` parameter set to
`true`.

As an example, let's first setup an index with several dense vectors.
...
In the response, we see a very verbose profile which includes a `knn` section
...
--------------------------------------------------
// NOTCONSOLE
<1> The rewrite time is especially important as this
is where the kNN search query is actually evaluated.
...
and <<collectors-section, collector>>. The `rewrite_time`
contains the amount of time where the kNN search query is evaluated which
is notably different from other types of queries, so this is the
most important timing when figuring out how long a kNN search query takes.

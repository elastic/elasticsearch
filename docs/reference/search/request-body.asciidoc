...
`ccs_reduce_mode`::
    ...
    The cross-cluster search execution mode. Can be `one_request_per_shard` or
    `prefer_one_request_per_cluster`. Defaults to `prefer_one_request_per_cluster`.
    See <<ccs-execution-mode>> for more.

...
coming[1.1.0]
Here is an example of setting the `content` field to allow for
highlighting using the fast vector highlighter on it.  Each field over
one thousand characters will store term vectors and each field under
a thousand characters will be reanalyzed when it is highlighted.

[source,js]
--------------------------------------------------
{
    "type_name" : {
        "content" : {"term_vector_over_1k" : "with_positions_offsets"}
    }
}
--------------------------------------------------
...
[NOTE]
Liberal storage of term vector information has been known to increase
the index size from between 20% and 50% and loading term vectors for
short fields is actually slower then just re-analyzing them.  Thus
using `term_vector_over_X` both limits term vectors to fields that would
benefit from it and and provides a way to trade hit time for disk space
and index time.

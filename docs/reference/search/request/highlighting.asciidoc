...
* Can be customized with `boundary_chars`, `boundary_max_scan`,
 `fragment_offset`, or `break_on_sentence` (see <<boundary-characters,below>>)
...
coming[1.1.0]
Setting the `break_on_sentence` attribute to trye instructs the FVH to
try to break on sentence boundaries.  If it can't break on sentence
boundaries then it'll break on word boundaries.  This works best when
`boundary_max_scan` is set high (`200` or so) and `fragment_size` is set
low (`30` or so).  This will get it to spit out single complete
sentences.  If the match is very early or late in the sentence then two
it'll spit out two sentences if it can find their boundaries within the
`boundary_max_scan`.

...
and word boundaries. This parameter takes a form of a language tag, e.g. `US`, `CHINA`, `JAPAN`.
The full list can be found in the
https://docs.oracle.com/javase/8/docs/api/java/util/Locale.html#forLanguageTag-java.lang.String-[Locale]
documentation.
...
Only `unified` highlighter truly calculates the score, other highlighters with
order: `sort` setting, will rank fragments by the number of query words found
in them. See a section below for more details how different highlighters find the best
fragments.
...
(in contrast to plain highlighter that counts only unique query terms).

...
The `unified` highlighter adapts its strategy to extract the matching offsets in the text to highlight
depending on the index options set on the field and the type of the query.
In this highlighter, offsets can be extracted from either postings , term vectors, or via re-analyzing text.
...
WARNING: This option works with the `fvh` highlighter only

...
NOTE: To prevent against issues caused by having too many scrolls open, you
can limit the number of open scrolls per node with the
`search.max_open_scroll_context` cluster setting (defaults to unlimited).

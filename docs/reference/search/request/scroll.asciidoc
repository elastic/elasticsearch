...
return a `_scroll_id`. While the `_scroll_id` may change between requests,
it doesn't always change -- in any case, only the most 
recently received `_scroll_id` should be used.

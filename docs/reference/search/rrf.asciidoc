...
If `k` is smaller than `rank_window_size`, the results are
`k` size.
...
`standard` retrievers.
A primary use case is combining top documents from a traditional BM25 query and an <<semantic-search-elser, ELSER>>
...
<2> Next we run the `standard` retriever specifying a text expansion query for `What blue shoes are on sale?`
...
Use the
<<retriever, retriever API>> instead.
See
...
Working with the example above, and by adding

...
In addition to individual query scoring details, we can make use of the `explain=true` parameter to get information on how
the RRF scores for each document were computed. Working with the example above, and by adding
`explain=true` to the search request, we'd now have a response that looks like the following:
...
In addition to the above, explain in RRF also supports *named queries* using the `_name` parameter.
This however is not available for retrievers yet, so it is only supported for RRF through the use of  `sub_searches`,
and for top-level `query` and `knn` elements for now. Using named queries allows for easier and more intuitive understanding of the RRF score computation,
especially when dealing with multiple queries. So, for the equivalent of the above example, we would now have:
...
----
// NOTCONSOLE
<1> Here we specify a `_name` for the `knn` query

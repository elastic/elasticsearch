...
are typically more compact that the original JSON `_source`.

IMPORTANT: Most field datatypes store doc values by default. However,
<<text,`text` fields>> do not support doc values.

We recommend retrieving doc values only if you need a fairly small number of
fields that support doc values, such as <<keyword,keywords>> and <<date,dates>>.

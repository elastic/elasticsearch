...
`explain`::
(Optional, Boolean)
If `true`, the response includes additional details about score computation as
part of a hit. Defaults to `false`.

`ignore_throttled`::
(Optional, Boolean)
If `true`, specified concrete, expanded, or aliased indices are not included in
the response when throttled. Defaults to `true`.
...
(Optional, Boolean)
If `true`, the response returns `hits.total` as an integer. If false, it returns
`hits.total` as an object. Defaults to `false`.
...
`typed_keys`::
(Optional, Boolean)
If `true`, the response prefixes aggregation and suggester names with their
respective types. Defaults to `false`.
...
`explain`::
(Optional, Boolean)
If `true`, returns detailed information about score calculation as part of each
...
`id`::
(Required*, string)
ID of the search template to use. If no `source` is specified, this parameter is
required.
...
`profile`::
(Optional, Boolean)
If `true`, the query execution is profiled. Defaults to `false`.

`source`::
(Required*, object)
An inline search template. Supports the same parameters as the
<<search-search,search API>>'s request body. Also supports

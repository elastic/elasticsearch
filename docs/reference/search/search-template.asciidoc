...
In order to execute the stored template, reference it by its name under the `template` key:
...
You can also register search templates by using the stored scripts api.

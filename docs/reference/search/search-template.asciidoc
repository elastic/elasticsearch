...
Allows to use the mustache language to pre render search requests.
...
The `/_search/template` endpoint allows to use the mustache language to pre 
...
`ccs_minimize_roundtrips`::
  (Optional, boolean) If `true`, network round-trips will be minimized as part 
  of cross-cluster search requests execution. Defaults to `true`.
...
`profile`::
  (Optional, boolean) If `true`, the query execution will be profiled. Defaults 
  to `false`.

`rest_total_hits_as_int`::
  (Optional, boolean) If `true`, `hits.total` will be rendered as an integer in 
  the rest search response. Defaults to `false`.
...
`typed_keys`::
  (Optional, boolean) If `true`, aggregation and suggester names will be 
...
The API request body must contain the search definition template and its params.

...
------------------------------------------
<1> The empty `""` will be ignored as long as `""` is not a valid value for the `status` field.

Alternatively, you can flatten the `status` parameter into a string which is
passed in unescaped (using `{{{ }}}`) as follows:

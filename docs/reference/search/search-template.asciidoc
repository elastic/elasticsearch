...
<<enable-dynamic-scripting, scripting docs>> added[1.6.0, `mustache` scripts were always on before and it wasn't possible to disable them].
...
either be stored in a file 

Register search templates by storing it in the config/scripts directory, in a file using the .mustache extension ((see <<pre-registered-templates>>))

Store the below query in ES installation>config>scripts>conditionalclause_template.mustache
...
The above template stored in the file can be used for search by the below query

[source,js]
------------------------------------------
GET /_search/template
{
    "file": "conditionalclause_template", 
    "params": {
        "text":      "words to search for",
        "line_no": { <1>
            "start": 10, <1>
...
POST /_search/template/conditionalclause_template
{
...
which can be retrieved by the following query
...
Note that we used "file":"filename" to specify the filename and use "id":"templatename" to specify the tempalte.

...
`geometrycollection` values, the API returns a feature in the `hits` layer for
each element of the collection. This behavior may change may change in a future
release.

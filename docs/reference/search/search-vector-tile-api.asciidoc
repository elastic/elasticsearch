...
`grid_precision`::
(Optional, integer) Determines the grid size of the `geotile_grid`. Accepts
...
The following requests create the `museum` index and adds several geospatial
`location` values.
...
The API returns results in a binary vector tile. When decoded into JSON, the
tile contains the following results:

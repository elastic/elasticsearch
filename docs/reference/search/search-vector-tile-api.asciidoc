...
more than one vector tile. In order to compute the resolution, we compute the
average density of hexagonal bins and use the resolution that is closer to the
`geotile` density.

The mapping between zoom levels and H3 resolutions is giving in the following table.

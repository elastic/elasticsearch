...
https://h3geo.org/docs/core-library/restable[resolution of the hexagonal cells].

Hexagonal cells don't align perfectly on a vector tile and some cells may intersect
more than one vector tile. In order to compute the H3 resolution for the precision
`<zoom> + grid_precision`, we compare the average density of hexagonal bins at each
resolution with the average density of tile bins and each zoom level. We use the H3
resolution that is closer to the`geotile` density.

The mapping between `<zoom> + grid_precision` and H3 resolutions is giving in the following table.
For example if <zoom> is 3 and grid_precision is 3, hexagonal cells are created with H3
resolution 2 (zoom level 6 row). If <zoom> is 3 and grid_precision is 4, hexagonal cells are created
with H3 resolution 3 (zoom level 7 row)."

[cols="<,<,<,<,<",options="header",]
|=======================================================================
|zoom level | unique tile bins| H3 resolution| unique hex bins | ratio
|1	|4	                 |0	|122	            |30.5
|2	|16	                 |0	|122	            |7.625
|3	|64	                 |1	|842	            |13.15625

...
`centroid`:::
Each feature is a `Point` that's the centroid of the data within the cell. If the centroid
of the data is outside the cell, the feature is set to the closest point inside the cell
to the centroid.
// end::grid-type[]
...
+
The name of the aggregations cannot start with the prefix `_mvt_`
as it is reserved for internal aggregation names.

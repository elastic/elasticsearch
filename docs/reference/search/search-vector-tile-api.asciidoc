...
+
Vector tiles do not support geometry collections. Behavior for `geo_shape`
fields with `geometrycollection` values is undefined and may change in a future
release.
...
* A `hits` layer containing a feature for `<field>` value matching the
`geo_bounding_box` query.

*  An `aggs` layer containing a feature for each cell of the `geotile_grid`. The
layer only contains features for cells with matching data. These cells act as
tiles for lower zoom levels.
...
if a polygon feature would have no area at its zoom level, the API omits the
feature.
...
larger than the returned vector tile.
...
For example, if `<zoom>` is `7` and `grid_precision` is `8`, you can zoom in up
to level 15. Accepts `0`-`8`. Defaults to `8`. If `0`, results don't include the
`aggs` layer.
+
...
`grid_type`::
(Optional, string) Determines feature geometry for the `aggs` layer. In the
`aggs` layer, each feature represents a `geotile_grid` cell. Accepts:
...
Accepts `0`-`10000`. Defaults to `1000`. If `0`, results don't include the
`hits` layer.

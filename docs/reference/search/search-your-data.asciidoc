...
a search request, {es} first determines which nodes contains copies of an
index's data. {es} then select an eligible node and forwards the search request
...
which may result in slower searches.
...
or want to route searches to a specific node based on its hardware. Alternately,
...
the search API's <<search-preference,`preference` parameter>>.

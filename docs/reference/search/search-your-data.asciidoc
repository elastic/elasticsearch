...
*Query DSL* +
Query DSL supports a variety of query types you can mix and match to get the
results you want, including:
...
other summaries of your search results. Aggregation help you
...
Copies of an index's data are distributed across one or more primary shards and
replica shards. These shards may be distributed across multiple nodes. When you
search an index, a coordinating node receives the search request. The
coordinating node then determines which nodes contains copies of the index's
data and selects a node to search. The coordinating node then routes the search
request to the selected node and its shard copies.
...
Multiple nodes may contain copies of an index's data and be eligible for search
routing. By default, {es} selects one of these nodes search using _adaptive
replica selection_. This method chooses the node based on
<<allocation-awareness,allocation awareness>> and the following criteria:

* Response time of prior requests between the coordinating node and the node
...
API>>. If disabled, searches are routed using a round-robin selection method,
which may slow searches.
...
containing the index's data. However, you may only want data from a local or
want to route searches based on a node's hardware. In these cases, you can use
the search API's <<search-preference,`preference` parameter>> to limit the set
of eligible nodes and shards for a search request.

For example, the following request searches `my-index-000001` with a
`preference` of `_local`. This argument restricts the search to shards on the
user's local node. If the local node contains no copies of the index's data, the
...
based on the hash of a provided string. If the cluster state and selected shards
...
session ID. The string cannot start with a `_`.
...
searches with the same `preference` string retrieve results from the
<<shard-request-cache,shard request cache>>. In most logging and time-series use
cases, data in older indices is rarely updated and can be served directly from
this cache.
...
string may not route searches to the same shards. This can occur for a number of
reasons, including shard relocations and shard failures. A node can also reject
a search request, which {es} would re-route to another node.
...
<<mapping-routing-field,routing value>>, which routes the document to particular
shard.

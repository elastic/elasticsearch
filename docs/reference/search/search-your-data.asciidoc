...
TIP: You can also use the search API to run <<search-aggregations,search
aggregations>>. Aggregations let you retrieve statistics other analytics on your
...
duration expires, the request fails and returns an error.

To configure a cluster-wide timeout duration for all search requests, use the
<<dynamic-cluster-setting,dynamic>> `search.default_search_timeout` cluster
setting. This global timeout duration is used if no `timeout` argument is passed
in the request. If the global search timeout expires, the request is cancelled
using <<task-cancellation,task cancellation>>. The
`search.default_search_timeout` setting defaults to `-1` (no timeout).

[discrete]
[[global-search-cancellation]]
=== Search cancellation

You can cancel a search using the <<task-cancellation,task cancellation API>>.
{es} also automatically cancels a search when your client's HTTP connection
closes. We recommend you set up your client to close HTTP connections when a
search request is aborted or times out.

...
a search request, {es} first determines which nodes contain a shard copy of the
index's data. {es} then selects an eligible node and forwards the search request
to its respective shard. This process is known as _search shard routing_ or
_routing_.

...
or want to route searches to a specific node based on its hardware. To limit the
set of eligible nodes and shards for a search request, use the search API's
<<search-preference,`preference` parameter>>.
...
<<shard-request-cache,shard request cache>>. For most logging and time-series
use cases, data in older indices is rarely updated and can be served directly
from this cache.

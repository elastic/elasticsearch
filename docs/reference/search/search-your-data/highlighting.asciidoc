...
[[truncation_tag]]
truncation_tag:: If <<max-analyzed-offset, `max_analyzed_offset`>> query param is set
and the highlighting stops at that defined offset, the `truncation_tag` is appended right after
the <<post-tags, `post-tag`>> of the last highlighted occurance and the the rest of the text is
removed. Defaults to `...`.

...
[discrete]
==== Search with a previously stored vector

When using approximate kNN search, it is possible to run a search with a previously stored vector.

. Create an index for storing query vectors
+
[source,console]
----
PUT stored_vector
...
. Index a query vector to use
+
[source,console]
----
PUT stored_vector/_doc/image_vector_0?refresh
{
  "my_vector": [20, 120, -1]
}
----
...
. Use the stored vector when executing a kNN query
+
[source,console]
----
POST image-index/_search
...
The above will search with the previous stored vector of `[20, 120, -1]`.

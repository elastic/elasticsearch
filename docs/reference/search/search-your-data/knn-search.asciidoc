...
<<dense-vector,`dense_vector`>> values. You create these vectors outside of {es}
and pass them to {es} as a query vector or as part of an indexed document.
...
document linearly, resulting in slow search speeds. However, you can improve
latency by using a query to limit the number of matched documents passed to the
function. If you use the query to filter your data to a small subset of
documents, you can get good search performance using this approach.

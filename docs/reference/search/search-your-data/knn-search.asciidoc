...
In addition to 'hybrid retrieval', you can search more than on KNN vector field at a time:
...
POST image-index/_search
{
  ...
  "knn": [ {
    ...
    "query_vector": [0.23, 1.2, -0.52, 0.023],
    "k": 10,
    "num_candidates": 10,
    "boost": 0.5
  }]
...
These top values are then combined with the matches from the `match` query, and
finally returns the 10 top-scoring results. The multiple `knn` entries and the `query` matches are combined through a
disjunction, as if you took a boolean 'or' between them. The top `k` vector results represent the global nearest neighbors across all index
shards.

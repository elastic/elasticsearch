...
Note: To ensure correct results: each individual filter must be either over
the top-level metadata or `nested` metadata. However, a single knn search
supports multiple filters, where some filters can be over the top-level metadata
and some over nested.
...
Filtering by nested and the top level metadata: makes kNN search only consider vectors with
`"paragraph.language": "EN"` and which parent's `"creation_time"` falls within the provided range.

...
image::images/kNN/knn_diagram.svg[Approximate kNN search as a two-dimensional graph,align="center"]
...
<<dense-vector,`dense_vector`>> values. This typically requires the setup and
training of a machine learning model. Elastic doesn't currently offer any
out-of-the-box models or solutions for generating these vector values.
...
* <<approximate-knn,Approximate kNN>> using the kNN search API
...
large datasets. The vector function has to scan each matched document linearly,
resulting in slow search speeds. However, you can improve latency by using a
query to limit the number of matched documents passed to the function.
...
each shard, then merges them to find the top `k` results.

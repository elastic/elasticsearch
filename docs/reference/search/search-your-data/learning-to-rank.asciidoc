...
=== Judgment list
The LTR model is usually trained on human-curated judgment lists, where users have
ranked the relevance of documents to their search queries. Judgment lists
determine the ideal ordering of results for a given search query. The goal of
LTR is for the model to mirror the judgment list rankings as closely as possible
for new queries and documents.
...
relevant). The example above uses a graded relevance judgment.

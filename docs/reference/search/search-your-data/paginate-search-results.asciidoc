...
IMPORTANT: If a search after request targets multiple indices and some of its sort field
has type <<date,`date`>> in one index but <<date_nanos,`date_nanos`>> in other indices,
then Elasticsearch won't interpret the search after parameter correctly in every target
index. The <<sort-search-results,sort values>> of a date field by default is a long of
milliseconds since the epoch while a date_nanos field is a long of nanoseconds.
In this case, it's crucial to format the sort value of that field with
a <<mapping-date-format, date format>>.

...
The `fields` parameter can also be used to retrieve fields from related indices.

[source,console]
----
POST ip_location/_doc/192.168.1.1
{
  "country": "Canada",
  "city": "Montreal"
}
...
<1> The name of the field. The response of this field is returned under this name.
<2> The type of the field. Here the `lookup` type means join fields are retrieved using the <<docs-get,GET API>>
<3> The external index that join fields are retrieved from
<4> The key field whose values are used as `_id` in the <<docs-get,GET API>>
<5> An optional list of fields to retrieve from the external index.

The above search returns the country and city from the `ip_location` index
for each ip address of the returning hits.
          ...
          "location": [
            {
              ...
              "_failure": [
                "id [192.168.1.2] on index [ip_location] doesn't exist"
              ]
            }
          ],
          "message": [
...
The `fields` response of join fields is similar to the response of nested fields.
Values inside `join` fields are grouped to maintain the independence
of each join document from the external index. If there is any error during
the lookup, the error is returned under the `_failure` field of a join document.

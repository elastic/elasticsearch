...
The `fields` response for <<flattened,`flattened` fields>> is a second example where the
`_source` content is returned unaltered. The `flattened` field maps an entire
object with potentially many subfields as a single field, so fetching its contents
maintains  this structure

Single subfields, however, can be fetched by specifying them explicitly in the request, but
they will not be returned by any wildcard field pattern:

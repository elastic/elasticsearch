...
because it consults both the document `_source` and the index mappings to load
and return values. In some instances, you might want to use
<<field-retrieval-methods,other methods>> of retrieving data.
...
Because it accesses the index mappings, the `fields` parameter provides several
...
* Formats date fields by default according to the
...
The `fields` parameter also handles field types like <<alias,field aliases>> and
<<constant-keyword-field-type,`constant_keyword`>> whose values aren't always
present in the `_source`. Other mapping options are also respected, including
...
fields are requested. To avoid this limitation, you can try other options for
loading fields:
...
* Use the <<script-fields,`script_field`>> parameter to transform

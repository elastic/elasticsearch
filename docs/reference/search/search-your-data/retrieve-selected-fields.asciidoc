...
When retrieving standard fields, the `fields` option returns values in the way
that {es} sees them. For most field types, this means that the `fields` option \
returns values from `_source` exactly how they are indexed.

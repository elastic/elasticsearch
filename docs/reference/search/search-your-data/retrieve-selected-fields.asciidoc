...
via <<runtime, runtime fields>> defined in the index mapping or the search request.
...
----
<1> A runtime with a type of `lookup` that retrieves fields from the lookup index using the <<docs-get,GET API>>
<2> An index where the lookup fields are retrieved from
<3> A field on the searching index whose values are the `_id` of the documents on the lookup index
<4> An optional list of fields to retrieve from the lookup index
          ...
          "location": [
            {
              ...
              "_failure": [{
                  ...
                  "reason": "id [192.168.1.2] on index [ip_location] doesn't exist",
                  "status": "NOT_FOUND"
              }]
            }
          ],
...
The response of lookup fields is similar to the response of nested fields.
Values inside `lookup` fields are grouped to maintain the independence
of each document from the lookup index. If there is any error during
the lookup, the error is returned under the `_failure` field.

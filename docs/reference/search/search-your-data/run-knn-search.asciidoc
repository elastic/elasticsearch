[[run-knn-search]]
== Run a k-nearest neighbor (kNN) search
++++
<titleabbrev>Run a kNN search</titleabbrev>
++++
...
<<dense-vector,`dense_vector`>> values, called embeddings. This typically
requires the setup and training of a machine learning model, called an embedding
model. Elastic doesn't currently offer any out-of-the-box embedding models or
solutions.
+
The fit of your embedding model to your data and use case has a significant
impact on the quality of your kNN search results.
...
. Run the search using the <<knn-search,kNN search API>>.
+
[source,console]
----
...
each shard, then merges them to find the top `k` results. This works well in
most cases, but results are less accurate if a shard contains a disproportionate
number of nearest neighbors.

You can increase `num_candidates` for more accurate results at slower search
speeds. Similarly, you can decrease `num_candidates` for faster searches with
...
* You can't use Query DSL to filter the documents searched using approximate kNN
or the results of an approximate kNN search. If you need to filter documents,
consider using exact kNN instead.
...
* {blank}
+
include::{es-repo-dir}/mapping/types/dense-vector.asciidoc[tag=nested-dense-vector-limitation]

...
If newer, the versions must also be compatible as outlined in the following
matrix.
...
8.1 cluster. However, a search from a local 8.0 cluster to a remote 7.15 or 6.8
...
cluster runs {es} 8.0, other clusters must run 8.0 or {prev-major-last}. This
lets you use any cluster as the local cluster when running a {ccs}.

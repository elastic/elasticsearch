...
In a {ccs}, you submit a search request to a local cluster. A coordinating node
on the local cluster parses the request and passes any relevant searches to a
<<gateway-nodes-selection,gateway node>> in on the remote cluster specified in
the request.

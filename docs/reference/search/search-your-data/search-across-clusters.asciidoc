...
Before 8.0, Elastic supports searches from a local cluster to a remote cluster in the following matrix.
+
[%collapsible]
[[ccs-version-compatibility]]
.CCS cluster version compatibility matrix
====
include::{es-repo-dir}/modules/remote-clusters-shared.asciidoc[tag=remote-cluster-compatibility-matrix]
====

From 8.0, Elastic only supports searches from a local cluster to a remote cluster running:

...
{ccs-cap} ({ccs-init}) lets you run a single search request against one or more
remote clusters. For example, you can use {ccs-init} to filter and analyze log
data stored on clusters in different data centers.
...
{ccs-init} requires <<remote-clusters,remote clusters>>. In a {ccs-init}, you
submit a search request to a local cluster. A coordinating node on the local
cluster parses the request and passes any relevant searches to a
<<gateway-nodes-selection,gateway node>> in any remote cluster specified in the
request.

To run the search, the local coordinating node and the remote gateway node must
use compatible versions of {es}. Elastic only supports searches to a remote
cluster running:

* The previous minor version
* Any minor release in the local cluster's major version
...
For example, an 8.0 cluster can search a remote cluster running:
...
A search on a remote cluster running an unsupported version may still work.
However, such searches aren't tested by Elastic and their behavior isn't
guaranteed.

[discrete]
[[ccs-ensure-cluster-compatibility]]
==== Ensure cluster compatibility

The simplest way to ensure cluster compatibility for {ccs-init} is to keep each
cluster on the same version of {es}. If you need to maintain clusters with
separate versions, you can:

* Maintain a local cluster for {ccs-init}. Keep this cluster on the earliest
release needed to search the other clusters.
+
For example, if you maintain 7.16 and 8.0 clusters, you can maintain a 7.15
cluster to search them.

* Keep each cluster no more than one minor version apart. For example, if one
cluster runs {es} 8.0, other clusters must run 8.0 or 7.16.

[discrete]
[[ccs-during-upgrade]]
==== {ccs-cap} during an upgrade

You can search remote clusters while performing a <<rolling-upgrades,rolling
upgrade>> on the local cluster. However, the "upgrade from" and "upgrade to"
versions must be compatible with the gateway node.

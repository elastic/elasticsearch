...
remote clusters. For example, you can use {ccs} to filter and analyze log
...
<<gateway-nodes-selection,gateway node>> in on the remote cluster specified in the
request.
...
The simplest way to ensure cluster compatibility for {ccs} is to keep each
cluster on the same version of {es}. If you need to maintain clusters with
separate versions, you can:

* Maintain a local cluster for {ccs}. Keep this cluster on the earliest
...
seed nodes need to be accessible from the local cluster on your network.

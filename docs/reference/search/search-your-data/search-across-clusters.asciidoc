...
For remote clusters using the <<sniff-mode-remote,sniff connection>> mode, gateway and
...
<<proxy-mode-remote,Proxy mode>> remote cluster connections support {ccs}. All remote
...
====
include::{es-repo-dir}/modules/remote-clusters.asciidoc[tag=remote-cluster-compatibility-matrix]
====

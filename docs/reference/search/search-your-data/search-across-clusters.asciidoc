...
* Maintain a cluster for {ccs}. Keep this cluster on the earliest version needed
to search the other clusters.
+
For example, if you have 7.14 and 8.0 clusters, you can maintain a dedicated
7.14 cluster for {ccs}. Use this dedicated cluster as the local cluster when
running any {ccs}.

...
the response as each cluster reports back.

NOTE: As of Elasticsearch 8.12, incremental results are shown when using minimize roundtrips.
As each cluster reports back, its search results will be merged with all the search
responses received so far. This is true for both search hits (top hits) and aggregations.
Because of scoring or sorting, the list of top hits can change as each cluster returns
its results. Check the `is_running` field in the async search response to see whether the
...
--------------------------------------------------
{
  ...
  "response": {
    ...
    "hits": {
      ...
      "max_score": 1.8293576,
      "hits": [...list of (partial) hits here...]  <4>
    }
  }
}
--------------------------------------------------
...
<1> The shards counts are shown from the searches that have completed (local and `cluster_one` in this case).
<2> The cluster accounting has been updated to show that 2 clusters have completed successfully
and one is still running.
<3> Number of hits from the two clusters that have reported back.
<4> Partial results (hits and any aggregations if they were included in the query) will be shown
for all clusters that have reported back. The list of hits may change once the final cluster
results have been merged in and a new round of scoring or sorting is done.
...
The `_shards`, `_clusters` and search results section of the response behave
...
. Aggregation results will be updated incrementally as the search is running (in
increments of `batched_reduce_size` shards, which defaults to 5) but top hits will only
be shown once the search is complete (all clusters have reported back).

Example using the same setup as in the previous section:

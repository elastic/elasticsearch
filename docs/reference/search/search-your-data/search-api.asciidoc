...
In general the lower the value of `track_total_hits` the faster the query will be, with `false` returning the fastest results.
Setting `track_total_hits` to false will also disable https://www.elastic.co/blog/faster-retrieval-of-top-hits-in-elasticsearch-with-block-max-wand[Max WAND].
=========================================

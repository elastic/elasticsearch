...
Mustache help with running your search template with different variables. You can set your value to variable through `params` in template.
...
Sections are also a type of Mustache tags. You can use `sections` to your search template with a nested or unnested object. A section begins with `{{#my-section-variable}}` and ends with `{{\my-section-variable}}`.

The following search template shows an example using sections with nested object:
...
Testing the template again with a variable `last` to determine last item in array:
[source,console]
----
POST _render/template?pretty
{
...
For example, the following search template changes the default delimiter to a single round bracket `(my-variable)`.

...
supports https://mustache.github.io/[Mustache] variables and has a https://github.com/elastic/elasticsearch/blob/main/modules/lang-mustache/build.gradle/[dependency] on the open source project https://github.com/spullara/mustache.java[mustache.java].
...
Mustache template help with running your search template with different variables. You can set your value to variable through `params` in template.

This document illustrates more examples of mustache tags with search templates.

[discrete]
[[search-template-mustache-variable]]
=== Mustache Variable
Mustache tags are typically enclosed in double curly brackets, A mustache variable : `{{my-variable}}` is a type of mustache tag which checks for a matching `my-variable` key in your search template `params` and replaces template with the value set for `my-variable`

For example, following search template replaces `{{query_string}}` in `message` with `My query string` value passed in `params`
...
Testing `my-search-template` with different `params`
...
When rendered, the template outputs as:
[source,console-result]
...
Sections are also a type of Mustache tags and are useful when you have to loop through nested keys. A sections begins with `{{#my-section-variable}}` and ends with `{{\my-section-variable}}`

The following search template shows an example using sections in a template.
...
==== Inverted sections
Inverted sections are useful when you want to set value once.
...
For example, in the following search template if `name_exists` is false message is set with `Hello World`, else sets to empty string.
...
For example, in the following search template, if `name_exists` is true, value of `{{query_string}}` is replaced else is set to default value `World`.
...
==== Lists
You can pass list of objects and loop over each item in your search template
...
PUT /_scripts/my-search-template?pretty
{
  "script": {
    "lang": "mustache",
    "source": {
      ...
      "params":{
        "query_string": "My query string",
        "text_fields": [ { "user_name": "John" }, {"user_name": "kimchy"}]
      }
    }
  }
}
...
NOTE: The above will cause trailing comma issue, which cause invalid json. A workaround would be including <<search-template-inverted-section,inverted section>> and add a variable to make sure it's last item in array.
...
==== Set delimiter
You can change the default demiliter: double curly brackets `{{my-variable}}` to any custom delimiter in your search template

For example, following search template changes the default demiliter to single round bracket `(my-variable)`
...
To Learn more about usage of mustache lambdas, checkout examples in <<search-template-url-encode-strings,Url encode strings>> <<search-template-concatenate-values,Concatenate values>> and <<search-template-convert-json, Convert to json>>

[discrete]
[[search-template-unsupported-features]]
==== Unsupported features
* Elasticsearch doesn't support Partials.

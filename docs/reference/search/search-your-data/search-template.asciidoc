...
supports https://mustache.github.io/[Mustache] variables and has https://github.com/elastic/elasticsearch/blob/main/modules/lang-mustache/build.gradle/[dependency] on https://github.com/spullara/mustache.java[mustache.java]. Typically https://mustache.github.io/[Mustache] variables are enclosed in
double curly brackets: `{{my-var}}`. When you run a templated search, {es}
replaces these variables with values from `params`. To learn more about mustache syntax - see http://mustache.github.io/mustache.5.html/[Mustache.js manual]

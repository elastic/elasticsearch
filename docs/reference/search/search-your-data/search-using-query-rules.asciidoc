...
Metadata is provided as part of the `rule_query` as an object.
It can be anything from a user-entered query string, personalized metadata about users (e.g. country, language, etc).
...
When a query rule match the `rule_query` metadata according to its defined criteria, the query rule action is applied to the underlying `organic_query`
...
* The first rule will generate a <<query-dsl-pinned-query>> pinning the <<mapping-id-field,`_id`>>s `id1` and `id2` when the `user.query` metadata value matches `pugs`.

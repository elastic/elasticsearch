...
Full-text search::
Use <<full-text-queries,full text queries>> search <<analysis,unstructured
textual data>> to find documents that best match search terms.

Semantic search::
<<semantic-search,Semantic search>> helps you find data based on the intent and
contextual meaning of a search query, instead of a match on query terms.

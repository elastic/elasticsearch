...
The search results of the two searches are combined in the response and weighted 
by the `boost` fields. As the `boost` value for the semantic search is higher than the 
`boost` value of the search in the example above, semantic search results will 
be higher in the response than the search results.

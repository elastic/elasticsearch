...
Refer to
{ml-docs}/ml-nlp-limitations.html#ml-nlp-elser-v1-limit-512[this page] for more information.
...
Refer to the
{ml-docs}/ml-nlp-elser.html[ELSER documentation] to learn how to download and deploy the model.
...
The destination index must have a field with the
<<sparse-vector, `sparse_vector`>> or <<rank-features,`rank_features`>> field type to index the ELSER output.
...
`"Limit of total fields [1000] has been exceeded while adding new fields"` then the ELSER output field is not mapped properly and it has a field type different than `sparse_vector` or `rank_features`.
...
It must be refrenced in the {infer} pipeline configuration in the next step.
...
IMOPRTANT: The `msmarco-passagetest2019-top1000` dataset was not utilized to train the model.
...
The example below uses the query text "How to avoid muscle soreness after running?", the `content_embedding`
...
To learn more about what tokens are, refer to
...
You can combine <<query-dsl-text-expansion-query, `text_expansion`>> with other queries in a <<compound-queries,compound query>>.
For example, use a filter clause in a <<query-dsl-bool-query>> or a full text query with the same (or different) query text as the `text_expansion` query.
...
Recall on the
`text_expansion` query can be high where there is a long tail of less relevant results.
...
<2> The `boost` value is `1` for the `text_expansion` query which is the default value.
...
**recovered through reindexing.** Excluding the tokens from the source is a space-saving optimsation that should only be applied if you are certain that reindexing will not be required in the future!

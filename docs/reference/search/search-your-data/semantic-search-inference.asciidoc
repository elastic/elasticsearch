...
Create the embeddings from the text by reindexing the data through the {infer} pipeline that uses the chosen model as the {infer} model.
This step - with using the {ref}/docs-reindex.html[reindex API] - simulates data ingestion through a pipeline.

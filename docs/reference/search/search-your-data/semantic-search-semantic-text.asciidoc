...
Copying the `content` field to the `semantic_text` field is not required to use `semantic_text`.
However, it is recommended because it enables hybrid search. Hybrid search combines lexical search on the `text` field with semantic search on the `semantic_text` field.
...
IMPORTANT: Using `copy_to` fields is not required for semantic search; you can target the `semantic_text` field directly during ingestion.
However, using the `copy_to` field enables hybrid search, which is recommended. 
For more information, refer to <<copy-to-support,this section>> on the `semantic_text` documentation page.

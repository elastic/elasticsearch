...
The returned sort values of a <<date,`date`>> or <<date_nanos,`date_nanos`>> field
can be formatted using a <<mapping-date-format, date format>> configured via the `format`
parameter. In the example below the returned sort value of the `post_date` field will be
formatted using the builtin format `strict_date_optional_time_nanos`.

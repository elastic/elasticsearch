...
`preference`::
(Optional, string)
Nodes and shards used for the search. By default, {es} selects from all
available shards at random, accounting for
<<shard-allocation-awareness,allocation awareness>> and
<<search-adaptive-replica,adaptive replica selection>>.
+
...
<custom-string>::
Any string that does not start with `_`. We recommend using a unique
value, such as a user name or web session ID.
+
If the cluster state and selected shards do not change, searches using the same
`<custom-string>` value are routed to the same shards in the same order.
+
However, if the cluster state or selected shards change, the same shards may not
be used. This can occur for a number of reasons, including shard relocations and
shard failures. A node can also reject a search request, which {es} would
...
`<index>`::
(Required, string)
Name of the index or index alias. Wildcard (`*`) expressions are supported.

`<boost-value>`::
(Required, float)
Boost value by which scores are multiplied.
+
A boost value greater than `1.0` increases the score. A value between `0`
and `1.0` decreases the score. 
====

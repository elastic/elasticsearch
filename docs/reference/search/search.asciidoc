...
specify a value in the `_source` parameter to return both source fields and
...
response.
+
If the `includes` property is not specified, the entire document source is returned,
excluding fields specified in this parameter.
=====
====

...
`explain`::
(Optional{multi-param}, boolean) If `true`, returns detailed information about score
computation as part of a hit. Defaults to `false`.

`from`::
+
--
(Optional{multi-param-ref}, integer)
...
the query execution will terminate early. Defaults to `0`, which does not
terminate query execution early.
...
`seq_no_primary_term`::
(Optional{multi-param-ref}, boolean) If `true`, returns sequence number and primary term of the

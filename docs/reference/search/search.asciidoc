...
`query_vector` must not be provided if `query_vector_builder` provided.
+
.Properties of `query_vector_builder`
[%collapsible%open]
=====
`indexed_query_vector`::
(Optional, object)
Configuration for getting the query vector that was previously stored in an index.
+
.Properties of `indexed_query_vector`
[%collapsible%open]
=====
`index`::
(Required, string)
The index containing the document with the query vector.

`id`::
(Required, string)
The document ID of the query vector document.

`path`::
(Required, string)
The field in the document `_source` containing the query vector.

`routing`::
(Optional, string)
The routing of the query vector document if required.
=====
=====

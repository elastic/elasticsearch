...
[[ccs-skip-unavailable]]
`ccs-skip-unavailable`::
(Optional, Boolean) If `false`, a {ccs} (CCS) request returns an error
if *any* cluster in the request is unavailable. If `true`, a {ccs} (CCS)
request skips all unavailable clusters. Defaults to unset, which honors
the `<<skip-unavailable,`skip_unavailable`>>` cluster setting of each
remote cluster.

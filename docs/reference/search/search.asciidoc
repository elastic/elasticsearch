...
`allow_no_indices`::
  (Optional, boolean) If `true`, ignores if a wildcard indices expression 
  resolves into no concrete indices. This includes `_all` string or when no 
  indices have been specified. Defaults to `false`.
...
`expand_wildcards`::
  ...
  that are open, closed or both. Defaults to `open`. Available options:
    * `open`
    * `closed`
    * `none`
    * `all`
...
`ignore_unavailable`::
  (Optional, boolean) If `true`, concrete indices will be ignored when 
  unavailable (missing or closed). Defaults to `false`.
...
`_source`::
  (Optional, string) True or false to return the `_source` field or not, or a list 
  of fields to return.
...
`query`::
  (Optional, <<query-dsl,query object>>) Defines the search definition using the <<query-dsl,Query DSL>>.

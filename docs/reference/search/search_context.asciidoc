...
A search request by default executes using the most recent searchers of the target indices.
In some cases, it's preferred to perform multiple search requests using the same searchers.
...
searches are only visible to the more recent searchers. Search context is introduced to
allow performing multiple search requests using the same searchers.

...
`filter`::
    Sets up template query to filter out suggestions that do not match
    any documents returned by the query. If provided must contain
    exactly `template` representing the query template with
    suggestion variable `suggestion` which is used to populate
    the query with the phrase suggestions. If `preference` is provided,
    it is used to route the filter query.

The following example shows a `phrase` suggest call with a filter query
...
filter query execution only on local shard.

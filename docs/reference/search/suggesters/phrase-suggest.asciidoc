...
`collate`::
    Sets up a template query/filter to filter out suggestions that do not match
    any documents returned by the query/filter. If provided must contain
    exactly `query` or `filter` representing the query/filter template with
    suggestion variable `suggestion` which is used to populate the
    query/filter with the phrase suggestions. Additional parameters for the
    query/filter template can be set by providing the `params` object.
    If `preference` is provided, it is used to route the filter query.
    `preference` defaults to `_only_local`.

The following example shows a `phrase` suggest call with a collate query
to prune generated suggestions that do not match any documents
for the provided match query. The routing preference sets up the
filter query execution on primary shard.

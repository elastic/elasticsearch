...
=== Vector tile API
++++
<titleabbrev>Vector tile</titleabbrev>
++++
...
[[vector-tile-api-request]]
==== {api-request-title}

`GET <target>/_mvt/<field>/<zoom>/<x>/<y>`

[[vector-tile-api-prereqs]]
==== {api-prereq-title}
...
[[vector-tile-api-desc]]
==== {api-description-title}

Internally, {es} translates a vector tile API request into a
...
The search only includes this aggregation if the `exact_bounds` parameter is
`true`.

For example, {es} may translate a vector tile API request with an `exact_bounds`
of `true` into the following search.
...
+
For example, a value of `8` divides the tile into a grid of 128 x 128 cells. The
`aggs` layer only contains features for cells with matching data.

`grid_type`::
(Optional, string) Determines the geometry of features in the `aggs`
layer. Each feature is a cell of the `geotile_grid`. Accepts:
...
[role="child_attributes"]
[[vector-tile-api-response]]
==== Response

When decoded into JSON, returned vector tiles contain the following data:
...
[%collapsible%open]
====
include::vector-tile-api.asciidoc[tag=extent]

include::vector-tile-api.asciidoc[tag=version]
...
[%collapsible%open]
=====
include::vector-tile-api.asciidoc[tag=geometry]
...
`aggs` parameter.
======
include::vector-tile-api.asciidoc[tag=feature-id]

include::vector-tile-api.asciidoc[tag=feature-type]
=====
====
...
The following vector tile API request searches the `13/4207/2692` vector tile
for intersecting values.

...
options specified. If possible, searches use the data from local storage. If the
data is not available locally, download the {es} downloads the needed data from
...
To mount a shared cache of a snapshot, you must reserve space
for the cache using the `xpack.searchable.snapshot.shared_cache.size` setting.
Indices mounted as shared cache can only be allocated to nodes that have this
...
<<data-frozen-node,frozen data>> node role.
...
you can configure another tier to use a <<shared-cache,shared snapshot cache>>.
...
dedicate the tier exclusively to the shared snapshot cache.
...
dedicated tiers should use most of the available disk.

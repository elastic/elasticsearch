...
+
Although slower than a fully mounted {search-snap} or a regular index, a
partially mounted {search-snap} still returns search results quickly, even for
...
To partially mount a searchable snapshot, you
...
Disk space reserved for the shared cache of partially mounted {search-snaps}.

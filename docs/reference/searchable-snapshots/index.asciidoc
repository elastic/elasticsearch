...
In most cases, {search-snaps} reduce the cost of running a cluster by removing
...
repository in your environment then {search-snaps} may be more costly than
...
across multiple nodes. If a node fails, {es} uses the redundant copies to
rebuild any lost shard copies. A {search-snap} index doesn't require replicas.
If a node containing a {search-snap} index fails, {es} can rebuild the lost
shard copy from the snapshot repository.
...
containing equivalent regular indices. The frozen tier, which contains only
partially-mounted {search-snap} indices, requires even fewer resources.
...
itself which is typically much cheaper.

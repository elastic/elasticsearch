...
Searchable snapshots recover from repository snapshots. The repository and
the associated snapshots must be available and reliable (see
<<searchable-snapshots-reliability>> for more details), irrespective of the
mount options. It is not allowed to remove a repository if there are any
mounted searchable snapshots from it.

...
irrespective of the mount options of the {search-snap}. {es} does not allow
to unregister a repository if there are any mounted searchable snapshots
from it. If the repository fails or corrupts the contents of the snapshot
then the data is lost. Although {es} may have made copies of the data onto
local storage, these copies may be incomplete and cannot be used to recover
any data after a repository failure. You must make sure that your repository
is reliable and protects against corruption of your data while it is at rest
in the repository.

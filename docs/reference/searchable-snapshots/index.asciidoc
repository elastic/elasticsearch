...
Note that the underlying snapshot still needs to be always available and reliable
for a fully-mounted index (see <<searchable-snapshots-reliability>>).
+
...
stored in the repository. A {search-snap} shard can fail if the underlying
snapshot is lost or corrupt, e.g., when {es} attempts to recover the shard.
The repository and the associated snapshot must be available and reliable,
irrespective of the mount options of the {search-snap}. {es} Elasticsearch
does not allow to unregister a repository if there are any mounted searchable
snapshots from it. If the repository fails or corrupts the contents of the
snapshot then the data is lost. Although {es} may have made copies of the
data onto local storage, these copies may be incomplete and cannot be used
to recover any data after a repository failure. You must make sure that your
repository is reliable and protects against corruption of your data while it
is at rest in the repository.

...
You can use <<snapshot-lifecycle-management,regular snapshots>> to back up and
restore a cluster containing {search-snap} indices.
...
original index snapshot from this repository. If wanted, you can use separate
repositories for regular snapshots and {search-snaps}.

A snapshot of a {search-snap} index contains only the ID for its original index
snapshot. It does not contain any data from the original index.

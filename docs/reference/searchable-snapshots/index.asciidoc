...
are two possibilities for the `?storage` mount option, each with different
performance characteristics and a different local storage footprint:

[[full-copy]]
Full copy::
Loads a full copy of the snapshotted index's shards onto node-local storage
within the cluster. This is the default mount option.
+
...
index's data. {ilm-init}-managed indices use this option by default when they
move into the frozen tier.
+

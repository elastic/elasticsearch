...
respectively. <<regexp-syntax,Lucene regexp>> must be enclosed between `/`,
for e.g. `/https?://[^/]+/?/` matches any http or https URL with no path component
(matches https://elastic.co/ but not https://elastic.co/guide).

NOTE: One of the `allowed_subjects` or `allowed_subject_patterns` settings must be specified
...
an incoming JWT's `sub` claim is validated if it's matched by any of the two lists.

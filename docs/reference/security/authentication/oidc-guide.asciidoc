...
. Configure the OpenID Connect realm in {es} to associate the {es} user properties (see
  ...
  .. `claims.groups: "<example-url>/claims/groups"` : Similarly, this instructs {es} to look
     for the claim with the name `<example-url>/claims/groups` (note that this is a URI - an

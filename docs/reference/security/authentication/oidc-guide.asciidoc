...
xpack.security.authc.realms.oidc.oidc1:
  ...
  claims.groups: "<groups-claim-from-op>"
-------------------------------------------------------------------------------------
...
. Configure the OpenID Connect realm in {es} to associate the {es} user properties (see
  ...
  .. `claims.groups: "<groups-claim-from-op>"` : This instructs {es} to look
     for the claim configured by your OpenID Provider (OP) to represent the
     authenticated user's group or role memberships, either in the ID Token or
     in the UserInfo response, and map its value(s) to the `groups` user
     property in {es}. There is no standard claim name defined by OpenID
     Connect for this purpose, so the value configured here varies between
     providers. Some providers use simple claim names such as `groups`, while
     others use URI-formatted identifiers. URI-formatted claim names are
     treated as identifiers (strings) and are not URLs that will be retrieved.
     Consult your OP documentation to determine which claim to use.

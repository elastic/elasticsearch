...
Provides all of the privileges of the `inference_user` role plus the full
use of the {inference} APIs. Grants the `manage_inference` cluster privilege
and read access to the `.inference` index.

[[built-in-roles-inference-user]] `inference_user`::
Grants the minimum privileges required to view {inference} configurations

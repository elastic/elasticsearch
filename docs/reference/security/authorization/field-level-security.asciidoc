...
Access to the following metadata fields is always allowed:
`_ignored`, `_id`, `_routing`, `_tsid`, `_ts_routing_hash`, `_index`, `_index_mode`, `_nested_path`, `_type`,
`_version`, `_seq_no`, `_primary_term`, `_doc_count`, `_data_stream_timestamp`, `_field_names`,
`_size`, `_feature`, `_tier`.
If you specify an empty list of fields, only these metadata fields are accessible.

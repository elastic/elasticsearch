...
<<remote-clusters-api-key,remote clusters configured with the API key based model>>.
This field is optional (missing `remote_cluster` privileges effectively mean
...
NOTE: Remote cluster privileges are effective for remote clusters configured with the API key based model.
They have no effect for remote clusters configured with the certificate based model.
...
The only privilege that can be granted for remote clusters is `monitor_enrich` for
versions 8.15.0 and later. `monitor_enrich` is requires to allow users to use the `ENRICH` keyword for
ES|QL queries across clusters.

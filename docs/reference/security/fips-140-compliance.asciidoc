...
To configure {es} to use additional security provider(s) configure {es}'s JVM property `java.security.properties` to point to a file
...
The Java security manager requires explicit grants permissions to perform low level Java operations. The Java security provider is not exempt from the
restrictions enforced by the security manager and as such it may require explicit grants to perform it's low level Java operations.

...
another `<clustername>_access.log` file with an improved format.
It is recommended that you disable this backwards compatible output file. To do
that, change its logger level to `off` in the `log4j2.properties` file from
`ES_PATH_CONF`.

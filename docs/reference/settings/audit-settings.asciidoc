...
event types. The subset of event types that `emit_request_body` applies to is:
`anonymous_access_denied, authentication_success, authentication_failed,
run_as_denied, tampered_request`. Possible values are `true` or `false` with
...
Specifies whether to include the request body from REST requests on certain
event types such as `authentication_failed`. The subset of event types that `emit_request_body` applies to is:
`anonymous_access_denied, authentication_success, authentication_failed,
run_as_denied, tampered_request`. Possible values are `true` or `false` with
the default value of `false`.

...
events, such as authentication failures, refused connections and data-access events.

IMPORTANT: Static auditing settings, such as `xpack.security.audit.enabled`,
must be configured on all nodes in the cluster.
...
each node.
+
If enabled, this setting must be configured on all nodes in the cluster.

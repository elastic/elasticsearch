...
IMPORTANT: Static auditing settings, such as `xpack.security.audit.enabled` must
be configured on all nodes in the cluster.
...
each node.
+
This setting must be configured on all nodes in the cluster.

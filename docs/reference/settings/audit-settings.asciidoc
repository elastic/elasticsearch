...
events, such as authentication failures and refused connections, You can use
these logs to monitor your cluster for suspicious activity. Audit logging also
provides forensic evidence in the event of an attack.
...
(<<dynamic-cluster-setting,Dynamic>>, array of strings)

...
(<<cluster-update-settings,Dynamic>>) Enables the health node periodic logger, which logs the health status of the cluster and of each health indicator as observed by the Health API.
Defaults to `false`.

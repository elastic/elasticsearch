...
`index.lifecycle.max_failed_step_retries_count`::
Some steps are safe to retry when they fail due to transitive errors. {ilm}
will attempt to retry execution in these scenarios. This allows the users to
configure how many times should {ilm} retry a step before abandoning and moving
to the `ERROR` step.

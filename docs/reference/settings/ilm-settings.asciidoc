...
When configured to `false` {ilm} will attempt to rollover the index, in case
the `rollover` action is configured for a phase, even if it was manually rolled
over since the last {ilm} cycle.
Allowing {ilm} to rollover the index will potentially fail due to
`is_write_index` now being `false` or, if the index was manually rolled under
a different alias than the `rollover_alias`, due to the rollover target index
already existing.
By default {ilm} will detect this situation and skip trying to rollover the
index again.

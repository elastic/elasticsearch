...
(<<cluster-update-settings,Dynamic>>) Limit for the model inference breaker. If
the parent circuit breaker is less than 50% of the JVM heap, it is bound to that
limit instead.
//TBD: Is that sentence apply only when using the default value?
See <<circuit-breaker>>. Defaults to 50% of the JVM heap.
//TBD: Is a valid value "40%", "40", or is it a byte value?

`breaker.model_inference.overhead`::
(<<cluster-update-settings,Dynamic>>) A constant that all accounting estimations
are multiplied by to determine a final estimation. See <<circuit-breaker>>.
Defaults to `1`.

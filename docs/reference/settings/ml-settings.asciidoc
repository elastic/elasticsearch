...
heap. Defaults to `40%`. See also <<circuit-breakers-page-model-inference>>.
...
`xpack.ml.anomalies.heal_reindexed_v7.enabled` {applies_to}`stack: ga 8.19`::
(<<cluster-update-settings,Dynamic>>) Controls if {es} automatically fixes anomaly detection result aliases that point to `.reindexed-v7-ml-anomalies-*` indices with incorrect mappings after an upgrade. See https://github.com/elastic/elasticsearch/issues/147686[#147686] for details. When `true`, startup maintenance repairs affected aliases; historic data remains in old indices, with {ml} Notifications providing manual reindexing steps. Set to `false` to disable. Defaults to `true`.

...
`20`. In this context jobs includes both anomaly detector jobs and data frame
analytics jobs. The maximum number of jobs is also constrained by memory usage,
so fewer jobs than specified by this setting will run on a node if their
estimated memory use would be higher than allowed. Prior to version 7.1 this
setting was a per-node non-dynamic setting. It became a cluster-wide dynamic

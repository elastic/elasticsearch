...
is deprecated and `xpack.ml.max_open_workers` will be used in preference if both
are specified.
...
this setting is dynamic, changes to its value after node startup will only be
used once every node in the cluster is running version 7.1 or higher.

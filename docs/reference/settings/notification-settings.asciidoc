...
`xpack.http.hosts.whitelist`::
A list of hosts, that are allowed to connect to using the HTTP client used in
Alerting. This client is used in the HTTP input, the webhook, the slack, jira,
pagerduty and hipchat actions. This setting can be updated dynamically. It
defaults to `*` allowing everything. Note: If you configure this setting and
you are using one of the slack/jira/pagerduty/hipchat actions, you have to
ensure that the corresponding endpoints are whitelisted as well.

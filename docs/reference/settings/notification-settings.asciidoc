...
[[email-account-attributes]]
  ...
  The password for the specified SMTP user. **Deprecated.**
...
[[hipchat-account-attributes]]
  ...
  **Deprecated.** Use `secure_auth_token` instead.
...
[[jira-account-attributes]]
  ...
  `url`;;
  The URL of the Jira Software server. Required. **Deprecated.**
  ...
  **Deprecated.** Use `secure_user` instead.
  ...
  **Deprecated.** Use `secure_password` instead.
...
via PagerDuty. You can specify the following PagerDuty account attributes:
  ...
  `service_api_key`;;
   ...
   PagerDuty API key] to use to access PagerDuty. Required. **Deprecated.**
   Use `secure_service_api_key` instead.

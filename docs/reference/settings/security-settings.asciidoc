...
+
{es} will only consume JWTs that were intended for any of these audiences.
( as denoted by the `aud` claim in the JWT).
Examples of `aud` claim are `https://example.com/client1` and `other_service,elasticsearch`.
...
`claims.group_patterns` {ess-icon}::
(<<static-cluster-setting,Static>>)
Accepts the same Java regular expression as
...
`claims.principal_pattern` {ess-icon}::
(<<static-cluster-setting,Static>>)
...
JWT for every request, set to `0` to disable JWT cache. Defaults to `100000`.

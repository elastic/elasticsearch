...
`delegation.enabled`::
Generally, in order for the PKI realm to work clients must connect directly to
{es}. That is, they must not pass through proxies which terminate the TLS
connection.  In order to allow for a *trusted* proxy, such as Kibana, to sit
before {es} and terminate TLS connections, but still allow clients to be
authenticate by this realm, you need to toggle this to `true`. Defaults to
`false`.

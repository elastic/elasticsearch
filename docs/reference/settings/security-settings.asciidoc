...
(<<static-cluster-setting,Static>>)
The hashing algorithm that is used for JWT validation cache entries.
Defaults to `ssha256`.
For possible values, see <<cache-hash-algo>>.

`cache.jwt_validation.ttl`::
(<<static-cluster-setting,Static>>)
Specifies the time-to-live for JWT validation cache entries.
Successful and failed validations will be cached for this period of time.
Use the standard {es} <<time-units,time units>>.
Defaults to `20m`.

`cache.jwt_validation.max_users`::
(<<static-cluster-setting,Static>>)
Specifies the maximum number of JWT validation cache entries.
Defaults to `100000`.
If a client uses a re-issued JWT for every request,
disable the JWT validation cache and only enable the Roles lookup cache.

`cache.roles_lookup.ttl`::
(<<static-cluster-setting,Static>>)
Specifies the time-to-live for Roles lookup cache entries.
Successful and failed lookups will be cached for this period of time.
Use the standard {es} <<time-units,time units>>.
Defaults to `20m`.

`cache.roles_lookup.max_users`::
(<<static-cluster-setting,Static>>)
Specifies the maximum number of Roles lookup cache entries.
Defaults to `100000`.
If a client uses a re-issued JWT for every request,
disable the JWT validation cache and only enable the Roles lookup cache.

...
`ssl.keystore.type`::
include::{docdir}/settings/common-defs.asciidoc[tag=ssl-keystore-type-pkcs11]
...
include::{docdir}/settings/common-defs.asciidoc[tag=ssl-supported-protocols-1]
//TBD: Does it support the more detailed description?

`ssl.cipher_suites`:: Specifies the cipher suites that should be supported when 
communicating with the LDAP server. 
include::{docdir}/settings/common-defs.asciidoc[tag=ssl-cipher-suites-values-java8]
...
You cannot use this setting and `signing.key` at the same time.
...
setting and `encryption.key` at the same time.
...
+
--
Defaults to the keystore password. You cannot use this setting and
`ssl.keystore.secure_key_password` at the same time.
--
...
--
Defaults to the keystore password. You cannot use this setting and
`ssl.keystore.key_password` at the same time.
--
...
`ssl.supported_protocols`::
include::{docdir}/settings/common-defs.asciidoc[tag=ssl-supported-protocols-2]
//TBD: Can it support the larger description?

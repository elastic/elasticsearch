...
`xpack.security.authc.show_unauthorized_error_trace.enabled`::
The <<common-options-error-options, `error_trace` request option>> lets
stacktraces of exceptions be printed in the response body. However, stacktraces
for authentication errors (401 HTTP response code) are exempted and are *not*
displayed. Set this to `true` to allow stacktraces of authentication errors
be returned as well. Note that all response exceptions, which are not
returned in the response body, can be logged under the `rest.suppressed`
logger (see <<configuring-logging-levels, logging level configuration>>).

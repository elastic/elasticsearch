...
`keytab.path`:: Specifies the path to Kerberos keytab containing service principal
used by this {es} node. It expects the keytab file to be present in `ES_PATH_CONF`
and have read permissions. Required.

`remove_realm_name`:: Principal name in Kerberos is of the form
`user/instance@REALM`. If this option is `true` then the realm part
(`@REALM`) will be removed before setting it as `username` field
in `User`. This field can then used during role mapping. Defaults to `false`.

`krb.debug`:: Set to `true` to enable debug logs for java login module that

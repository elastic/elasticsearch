...
Identifier is usually a case sensitive URL using the https scheme that contains
scheme, host, and optionally, port number and path components and no query or
fragment components. The value for this setting should be provided by your JWT
Issuer.
// end::jwt-allowed-issuer-tag[]

// tag::jwt-allowed-signature-algorithm-tag[]
`allowed_clock_skew` {ess-icon}::
(<<static-cluster-setting,Static>>)
The signature algorithm that will be used by {es} in order to verify the
signature of the id tokens it will receive from the JWT Issuer.
Defaults to `RSA256`.
// end::jwt-allowed-signature-algorithm-tag[]
...
(<<static-cluster-setting,Static>>)
The path or URL to a JSON Web Key Set with the key material that the JWT
...
(<<static-cluster-setting,Static>>)
A list of verifiable Identifiers for your JWT client applications.

The OAuth 2.0 Client Identifier that was assigned to the
OpenID Connect Relying Party application during registration
at the OpenID Connect Provider.
...
`claim_patterns.principal` {ess-icon}::
(<<static-cluster-setting,Static>>)
A Java regular expression that is matched against the JWT claim specified
by `claims.principal` before it is applied to the user's _principal_ property.
The attribute value must match the pattern and the value of the first
_capturing group_ is used as the principal. For example, `^([^@]+)@example\\.com$`
matches email addresses from the "example.com" domain and uses the local-part as
...
(<<static-cluster-setting,Static>>)
Specifies whether to use `sharedsecret`, `ssl`, or `none` authentication
to incoming client requests. Defaults to `sharedsecret`.
...
(<<static-cluster-setting,Static>>)
Controls the behavior of the http client used for back-channel communication to
the JWT Issuer endpoints. Specifies the timeout until a connection
 is established. A value of zero means the timeout is not used. Defaults to `5s`.
...
(<<static-cluster-setting,Static>>)
Controls the behavior of the http client used for back-channel communication to
the JWT Issuer endpoints. Specifies the timeout used when
requesting a connection from the connection manager. Defaults to `5s`
...
(<<static-cluster-setting,Static>>)
Controls the behavior of the http client used for back-channel communication to
the JWT Issuer endpoints. Specifies the socket timeout (SO_TIMEOUT)
in milliseconds, which is the timeout for waiting for data or, put differently,
a maximum period inactivity between two consecutive data packets). Defaults to
`5s`.
...
(<<static-cluster-setting,Static>>)
Controls the behavior of the http client used for back-channel communication to
the JWT Issuer endpoints. Specifies the maximum number of
connections allowed across all endpoints.
...
(<<static-cluster-setting,Static>>)
Specifies the protocol to use to connect to the proxy server that will be
used by the http client for all back-channel communication to the JWT Issuer
endpoints. Defaults to `http`. Allowed values are `http` or `https`.

`http.proxy.host`::
(<<static-cluster-setting,Static>>)
Specifies the address of the proxy server that will be used by the internal
http client for all back-channel communication to the JWT Issuer
endpoints. This includes requests to fetch the JSON Web Key Set from the
JWT Issuer if `jwkset_path` is set as a URL.

`http.proxy.port`::
(<<static-cluster-setting,Static>>)
Specifies the port of the proxy server that will be used by the http
client for all backchannel communication to the JWT Provider
endpoints. Defaults to `80`.

...
(<<static-cluster-setting,Static>>)
A plain string that is used as a delimiter to split the value of the first
element in the attribute specified by `attributes.groups` before it is
applied to the user's _group_ property. Group values can not contain the
delimiter, even if escaped.

...
Defaults to `7d`.
Note that large real-time clock inaccuracies across the cluster nodes can cause problems
with evaluating the API key retention period (in the likely case that the node performing
the invalidation is different from the one performing the deletion).

...
(<<secure-settings,Secure>>)
The file contents of a JSON Web Key Set with the secret key material that
the JWT Realm uses for verifying token signatures.
This format is compatible with multiple keys and optional attributes.
Only one of `hmac_jwkset` or `hmac_key` can be used simultaneously.
Using the HMAC JWK Set is preferred.
// end::jwt-hmac-jwkset-tag[]

// tag::jwt-hmac-key-tag[]
`hmac_key` {ess-icon}::
(<<secure-settings,Secure>>)
The contents of a JSON Web Key with the secret key material that
the JWT Realm uses for verifying token signatures.
This format is compatible with OIDC HMAC key encoding, where
the HMAC key bytes are the UTF-8 encoding of the UNICODE characters.
This format only supports a single key with no attributes.
Only one of `hmac_jwkset` or `hmac_key` can be used simultaneously.
Using the HMAC JWK Set is preferred.

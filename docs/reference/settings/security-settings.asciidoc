...
you need to toggle this to `true`. Defaults to `false`. This setting requires
that the `truststore.path` or `certificate_authorities` settings be defined as
well. For more details, see <<pki-realm-for-proxied-clients, Configuring
authentication delegation for PKI realms>>.

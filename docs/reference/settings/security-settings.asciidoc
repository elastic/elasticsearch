...
<<controlling-user-cache>>. Defaults to 
`ssha256`.
...
user credentials. See the <<controlling-user-cache,Cache hash algorithms>> table for
...
`files.role_mapping`::
The <<security-files,location>> for the <<mapping-roles,
...
in-memory cached user credentials. See <<controlling-user-cache,Cache hash algorithms>>
...
the in-memory cached user credentials (see <<controlling-user-cache,Cache hash algorithms>> table for all possible values). Defaults to `ssha256`.

...
Currently our build automatically creates a Debian package and an RPM package with <<setup-dir-layout,these directory structures>>, which are available on the download page. The package itself does not have any dependencies, but you have to make sure that you installed a JDK.

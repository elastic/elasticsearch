...
have `memlock unlimited` in <<limits.conf,`/etc/security/limits.conf`>>). 
The memory lock check verifies that *if* the
...
least 2048 threads. This can be done via <<limits.conf,`/etc/security/limits.conf`>>
...
address space. This can be done via <<limits.conf,`/etc/security/limits.conf`>> 
using the `as` setting to `unlimited` (note that you might have to increase

...
sizing and start JVM with different initial and max heap sizes, the JVM may pause
as it resizes the heap during system usage. If you enable
<<bootstrap-memory_lock,`bootstrap.memory_lock`>>, JVM locks the initial heap

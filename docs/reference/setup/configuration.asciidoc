...
restart. You can reset transient and persistent settings by assigning them a
`null` value using the API.

If you reset a cluster setting or configure the same setting using multiple
methods, {es} uses the following order of precedence:

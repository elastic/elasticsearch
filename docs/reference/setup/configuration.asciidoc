...
Updates made using the cluster update settings API are _persistent_, and
apply across cluster restarts. You can also reset persistent settings by assigning them
...
For example, you can apply a persistent setting or `elasticsearch.yml` setting.
However, a change to an `elasticsearch.yml` setting will not override a defined
persistent setting.

...
recommend using the bundled JVM in all cases. The bundled JVM is treated as an integral part of {es} in terms of support and maintenance.
This means that Elastic takes responsibility for keeping it up to date, and reacts to security issues and bugs within the bundled JVM just
as if they were within {es} itself. Elastic's support of the bundled JVM is subject to Elastic's
https://www.elastic.co/support_policy[support policy] and https://www.elastic.co/support/eol[end-of-life schedule] and is independent of the
support policy and end-of-life schedule offered by the original supplier of the JVM. Elastic does not support using the bundled JVM for
purposes other than running {es}.
...
{es} does not use. Such issues do not apply to {es}. If a security issue in the bundled JVM results in a vulnerability in {es} then it will
be described in an https://discuss.elastic.co/c/announcements/security-announcements/31[Elastic Security Announcement].

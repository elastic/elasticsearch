...
=== Linux cgroups version

{es} versions 8.0.0–8.4.3 fail to start on Linux distributions that use https://man7.org/linux/man-pages/man7/cgroups.7.html[cgroups] version 2 with the bundled JDK. Upgrade to 8.5.0 or later to resolve this. Refer to https://ela.st/es-may-fail-on-cgroup-v2[our KB article] for more guidance.

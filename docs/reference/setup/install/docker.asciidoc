...
The image is built with https://www.elastic.co/guide/en/x-pack/current/index.html[X-Pack].

==== Security note

NOTE: https://www.elastic.co/guide/en/x-pack/current/index.html[X-Pack] is preinstalled in this image.
Please take a few minutes to familiarize yourself with https://www.elastic.co/guide/en/x-pack/current/security-getting-started.html[X-Pack Security] and how to change default passwords. The default password for the `elastic` user is `changeme`.

NOTE: X-Pack includes a trial license for 30 days. After that, you can obtain one of the https://www.elastic.co/subscriptions[available subscriptions] or https://www.elastic.co/guide/en/x-pack/current/security-settings.html[disable Security]. The Basic license is free and includes the https://www.elastic.co/products/x-pack/monitoring[Monitoring] extension.
...
The conventional approach is to provide customized files, i.e. `elasticsearch.yml`, but it's also possible to use environment variables to set options.
...
NOTE: There is a difference between defining <<_setting_default_settings,default settings>> and normal settings. The former are prefixed with `default.` and can not override normal settings, if defined.

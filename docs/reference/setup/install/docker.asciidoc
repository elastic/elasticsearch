...
directory, and allow you to connect a {kib} instance to your secured {es}
cluster and encrypt internode communication:
...
testing and includes <<single-node-discovery,single-node discovery>> to bypass
the <<bootstrap-checks,bootstrap checks>>.
...
[source,sh,subs="attributes"]
----
docker run -p 9200:9200 -p 9300:9300 -it -e "discovery.type=single-node" {docker-image}
----
...
[source,sh]
----
docker cp <container-id>:/usr/share/elasticsearch/config/tls_auto_config_<timestamp>/http_ca.crt <local-directory>
----
+
--
`<container-id>`:: The ID of your Docker container where {es} is running. Run
`docker ps` to find this value.
--
...
[source,sh]
----
curl --cacert <local-directory>/http_ca.crt \
...
the {es} container to write logs to disk, set the `ES_LOG_STYLE` environment

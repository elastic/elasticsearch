...
`basic` flavour, which is the default, ships with X-Pack Basic features
pre-installed and automatically activated with a free licence. The `platinum`
flavour features all X-Pack functionally under a 30-day trial licence. The `oss`
flavour does not include X-Pack, and contains only open-source Elasticsearch.

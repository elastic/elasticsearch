...
development cluster with multiple nodes and {kib}, use Docker Compose instead.
...
. Copy the generated `elastic` password and enrollment token. These credentials are
only shown at startup. You can regenerate the credentials using the following commands.
+
--
[source,sh,subs="attributes"]
----
...
[TIP]
====
To make a `curl` API call without verifying the SSL certificate, use the `-k` flag.

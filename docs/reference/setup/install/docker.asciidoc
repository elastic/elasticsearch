...
The image offers several methods for configuring Elasticsearch settings with the conventional approach being to provide customized files, i.e. `elasticsearch.yaml`, but it's also possible to use environment variables to set options:
...
For example, bind-mounting a `custom_elasticsearch.yaml` with `docker run` can be accomplished with the parameter:

["source","sh"]
--------------------------------------------
-v full_path_to/custom_elasticsearch.yaml:/usr/share/elasticsearch/config/elasticsearch.yaml
--------------------------------------------
IMPORTANT: The container **runs Elasticsearch as user `elasticsearch` using uid:gid `1000:1000`**. Bind mounted host directories and files, such as `custom_elasticsearch.yaml` above, **need to be accessible by this user**. For the https://www.elastic.co/guide/en/elasticsearch/reference/current/important-settings.html#path-settings[data and log dirs], such as `/usr/share/elasticsearch/data`, write access is required as well.
...
USER root
RUN chown elasticsearch:elasticsearch config/elasticsearch.yaml
USER elasticsearch
--------------------------------------------

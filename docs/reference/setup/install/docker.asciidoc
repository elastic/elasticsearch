...
TIP: If you see `unauthorized: authentication required` when pulling the image, 
you may need to first refresh stale credentials with 
`docker logout docker.elastic.co` then try again.

...
The OSS only distribution is available from the
`elasticsearch-oss-{major-version}` repo under the package name oss-{major-version}.
...
The OSS only distribution is available at

...
`elasticsearch-create-enrollment-token` tool. You can then start a node with the `--enrollment-token` parameter so that it joins an existing cluster.

// The following include pulls in steps for enrolling nodes in a cluster from
// a security page in the x-pack folder
include::../../../../x-pack/docs/en/security/configuring-stack-security.asciidoc[tag=enroll-nodes]
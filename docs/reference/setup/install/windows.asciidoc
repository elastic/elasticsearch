...
TIP: Elasticsearch has historically been installed on Windows using the <<zip-targz, .zip>> archive.
...
Within the first screen, select the deployment directories for 

[horizontal]
Installation directory:: 

  defaults to `%PROGRAMFILES%\Elastic\Elasticsearch`
...
for plugins not currently installed, to be installed:

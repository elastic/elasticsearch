...
NOTE: X-Pack includes a trial license for 30 days. After that, you can obtain one of the 
...
`INSTALLDIR`:: 
  ...
  The installation directory. Defaults to ++%ProgramW6432%\Elastic\Elasticsearch\{version}++
...
`XPACKLICENSE`::
  ...
  When installing X-Pack plugin, the type of _temporary_ license to install,
...
The **exception** is `INSTALLDIR` (if originally specified), which must be a different directory to the 

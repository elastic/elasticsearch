...
Elasticsearch loads its configuration from the `%ES_HOME%/config/elasticsearch.yaml`
...
added to the `elasticsearch.yaml` config file, while any node-specific settings
...
`CONF_DIR`::
    ...
    Configuration file directory (which needs to include `elasticsearch.yaml`
...
| conf
  | Configuration files including `elasticsearch.yaml`
  | %ES_HOME%\config
  | path.conf

...
NOTE: How to configure the <<heap-size,heap size>> for the Windows
service depends on whether or not the service is installed. If the
service is not installed, you configure the <<heap-size,heap size>>
settings as for any other Elasticsearch installation. However, after
the service is installed, you must configure the heap size setting
via the service manager: `bin/service.bat manager`.

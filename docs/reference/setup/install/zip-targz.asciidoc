...
NOTE: These packages contain both {es} and {xpack}. All free features are 
included and enabled by default and will never expire. Commercial features are 
available to opt in by using a trial license. For more information, see 
{xpack-ref}/xpack-introduction.html[Introduction to {xpack}]. 
...
To obtain an Apache 2.0-only distribution, use the following link:

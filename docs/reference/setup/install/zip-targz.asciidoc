...
NOTE: These packages contain both open source and commercial features. All free 
features are included and enabled by default and do not expire. You can try 
additional commercial features with a trial license. For more information, see {xpack-ref}/xpack-introduction.html[Introduction to {xpack}]. 
...
To obtain only open source {es} features, use the following link:

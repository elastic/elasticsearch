...
[[windows-service]]
==== Install {es} as a Service on Windows
...
NOTE: While a JRE can be used for the {es} service, due to its use of a client VM (as opposed to a server JVM which offers better performance for long-running applications) its usage is discouraged and a warning will be issued.

...
appender.rolling_old.fileName = ${sys:es.logs.base_path}${sys:file.separator}${sys:es.logs.cluster_name}_server.old_log <16>
...
<16> The configuration for `old style` pattern appenders. These logs will be saved in `*.old_log` files and if archived will be in `*
.old_log.gz` files. Note that these should be considered deprecated and will be removed in the future.

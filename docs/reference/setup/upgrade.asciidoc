...
1. Disable shard reallocation (optional).  This is done to avoid extra shard relocation after node restart.  If this step is not performed, the nodes will immediately start trying to replicate shards to each other on startup and will spend a lot of time on wasted I/O.  After startup is complete, reallocation will be turned back on.
+
...
3. Upgrade the stopped node.  To upgrade using a zip or compressed tarball from elasticsearch.org:
...
4. To upgrade using a `.deb` or `.rpm` package:
...
5. Start the now upgraded node.  Confirm that it joins the cluster.
...
7. Wait for the cluster status to become green again. Balancing may take some time.
+
[source,sh]
--------------------------------------------------
...
8. Repeat step 1-7 for all remaining nodes.
...
Note: when upgrading version, elasticsearch won't allocate shards to cluster nodes not upgraded yet.
      This might lead to cluster staying in yellow state when creating new indices whose replication factor is greater than the number of updated nodes. For example, new index with 3 replica cannot be in green state before 3 nodes are at the latest version.

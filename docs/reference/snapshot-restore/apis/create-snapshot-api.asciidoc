...
`<snapshot>`::
(Required, string)
Name of the snapshot. Supports <<date-math-index-names,date math>>. Must be
...
`closed,hidden`. Defaults to `open,hidden`. Valid values are:
...
Supports <<multi-index,multi-target syntax>>. Defaults to an empty array (`[]`),

...
Wildcard (`*`) expressions and exclude (`-`) patterns are supported.
+
...
`<snapshot>`::
(Required, string)
Comma-separated list of snapshot names to retrieve. Also accepts wildcards (`*`) and exclude patterns via the (`-`) operator.
+
...
`slm_policy`::
(Optional, string)
Comma-separated list of slm policy names that a snapshot was created by used to limit the request.
Wildcard (`*`) expressions and exclude (`-`) patterns are supported.

...
for those that were created by an SLM policy with a name starting in `policy-a-`.
...
NOTE: The pagination parameters `size`, `order`, `after`, `offset`, `slm_policy_filter` and `sort` are not supported when using

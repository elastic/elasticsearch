...
currently running snapshots can be very expensive. The API requires a read from
the repository for each shard in each snapshot. For example, if you have 100
snapshots with 1,000 shards each, the API request will result in 100,000 reads
(100 snapshots * 1,000 shards).

Depending on the latency of your storage, such requests can take extremely long
to retrieve results. These requests can also tax machine resources and, when
using cloud storage, incur high processing costs.
====

...
`indices`::
(Optional, string or array of strings) Comma-separated list of indices and data streams to restore. Supports <<multi-index,multi-target syntax>>. Defaults to all indices and data streams in the snapshot, including system indices.

...
[WARNING]
====
If the snapshot of a data stream was taken when the data stream was marked for lazy rollover (e.g. after a change in the underlying index template),
and you restore that data stream _without_ a matching index template (e.g. by restoring to a name that doesn't match the original index template),
you will not be able to write to your data stream at all until you configure a matching index template.
====

...
Without a matching index template, a data stream cannot roll over or create
backing indices.
// end::restore-prereqs[]
...
+
You can only restore an existing index if it's closed. If you restore a data
stream, you also restore it's backing indices. The restore operation
automatically opens restored indices, including backing indices.
+

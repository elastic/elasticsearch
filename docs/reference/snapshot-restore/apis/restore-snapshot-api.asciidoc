...
----
// TEST[skip:cannot restore index because index is open]
...
+
The rename pattern is applied according to the regular expression that
supports referencing the original text according to https://docs.oracle.com/javase/8/docs/api/java/util/regex/Matcher.html#appendReplacement-java.lang.StringBuffer-java.lang.String-[appendReplacement].
+
...
The following request restores `index_1` and `index_2` from `snapshot_2`. Using the `rename_pattern`, any index matching the regex pattern `index_(.+)` will be renamed using the `rename_replacement` pattern `restored_index_$1`.

...
of {es}. If you are testing a pre-release build of {es} then you can also take
and restore snapshots as normal, but once a pre-release build of {es} has
written to a repository it may not be possible to use this repository with any
other build of {es}. You should discard the repository when moving to a
different build.
endif::[]

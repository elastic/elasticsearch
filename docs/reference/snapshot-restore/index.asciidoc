...
// tag::backup-warning[]
**A snapshot is the only supported way to back up a running cluster.** Other
backup methods may result in data loss, even if they appear successful.
// end::backup-warning[]
...
streams, and all indices, including system indices. The cluster state
includes:

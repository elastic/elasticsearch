...
Use the <<get-snapshot-api,Get snapshot API>> or the <<get-snapshot-status-api,Get snapshot status API>> to monitor the progress of the snapshot and restore processes while they are running. Both
...
Including a snapshot name in the get snapshot request retrieves information about a single snapshot:
...
Similar to repositories, information about multiple snapshots can be queried in a single request, supporting wildcards as well:
...
about the snapshot, such as status information and the number of
snapshotted shards.  The default value of the `verbose` parameter is `true`.

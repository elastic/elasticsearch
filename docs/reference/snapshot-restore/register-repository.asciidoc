...
source-only snapshot in the delegated storage repository. This source-only
snapshot only contains stored fields and index metadata. It doesn't include
index or doc values structures and isn't immediately searchable when restored.
After you restore data from a source-only snapshot, you must
<<docs-reindex,reindex>> it into a new data stream or index.

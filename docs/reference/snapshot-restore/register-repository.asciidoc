...
cluster. You can also use the `found-snapshots` repository for your own snapshot
policies or to store searchable snapshots.

The `found-snapshots` repository is specific to each deployment. You can restore
snapshots from another deployment's `found-snapshots` repository if the
deployments are under the same account and in the same region. See

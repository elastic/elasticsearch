...
cluster should have write access to the repository. Other clusters should
register the repository as read only.
...
{ess} deployments automatically register the
{cloud}/ec-snapshot-restore.html[`found-snapshots`] repository. {ess} uses this
repository to take periodic snapshots of your cluster. You can also use it for
your own snapshots.

The `found-snapshots` repository is specific to each deployment. To use another
deployment's default repository, see
...
include::register-repository.asciidoc[tag=multi-cluster-repo]
...
A source-only repository lets you to create minimal, source-only snapshots that take up to 50% less space on disk.
...
Source-only repositories must use another snapshot repository for snapshot storage.
...
Use {kib} or the <<put-snapshot-repo-api,create snapshot repository API>> to
register a source-only repository. When you register the repository, you must
specify another snapshot repository to use for snapshot storage.

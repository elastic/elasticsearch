...
For URLs using the `http` or `https` protocol, it is possible to configure the following
parameters:

[source,yaml]
----
repositories.uri.http.max_retries: 5
repositories.uri.http.socket_timeout: 50s
----

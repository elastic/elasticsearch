...
`max_restore_bytes_per_sec`:: Throttles per node restore rate. Defaults to `40mb` per second. Note that restores are also throttled through <<recovery,recovery settings>>.

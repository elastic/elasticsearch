...
The linearizable register implementation for Azure repositories is based around
Azure's support for strongly consistent leases that can prevent concurrent
access to a blob. To perform a compare-and-exchange operation on a register,
{es} first obtains a lease on the blob, then uploads the updated blob under the
same lease, which ensures that the blob has not changed in the meantime.

...
use the `default` client, with `7` retries before failing and and endpoint
...
`CLIENT_NAME` is `default` but you may specify a different client name when
...
Service], for instance using {eck-ref}[{eck}], then you should use
...
`chunk_size`::
    ...
    as a <<byte-unit,byte unit>>, for example: `10MB`, `5KB`, `500B`. Defaults

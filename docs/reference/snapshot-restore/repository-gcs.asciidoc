...
The linearizable register implementation for GCS repositories is based around
GCS's support for strongly consistent preconditions that can be imposed on
put-blob operations. To perform a compare-and-exchange operation on a register,
{es} retrieves the register blob and its current generation, and then uploads
the updated blob with a precondition saying that the generation has not changed
in the meantime.

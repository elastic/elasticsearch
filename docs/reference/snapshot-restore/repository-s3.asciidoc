...
`endpoint`::
    ...
    but the [AWS documentation](https://docs.aws.amazon.com/general/latest/gr/rande.html#s3_region) lists alternative S3
    endpoints. If you are using an [S3-compatible service](#repository-s3-compatible-services) then you should set this
    ...
    certificate authority for your S3 repository and you use the {es} [bundled JDK](../../deploy/self-managed/installing-elasticsearch.md#jvm-version),

...
IMPORTANT: The symlink must be readable by the `elasticsearch` user. By default, {es} runs inside the container
as user `elasticsearch` using uid:gid `1000:0`.

See the https://github.com/elastic/elasticsearch/pull/81255[Github PR] for implementation details.

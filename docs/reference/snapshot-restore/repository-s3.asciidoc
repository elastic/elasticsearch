...
IAM Role] credentials or <<web-identity-tokens,Web Identity Tokens>> for authentication.
...
[[web-identity-tokens]]
[discrete]
==== Using Web Identity Tokens for authentication
If you want to use https://aws.amazon.com/blogs/opensource/introducing-fine-grained-iam-roles-service-accounts/[AWS IAM Kubernetes service accounts]
for authentication, you need add a symlink to the `$AWS_WEB_IDENTITY_TOKEN_FILE` environment variable in the ES plugin
config directory, so the plugin can have the read permissions for the service account.
...
See the https://github.com/elastic/elasticsearch/pull/81255[Github PR] for implementation details, and the
https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRoleWithWebIdentity.html[AWS Security Token Service API docs]
to learn more about exchanging the web identity token for a session token.

...
user IDs (UIDs) and group IDs (GIDs) rather than matching accounts by name. If
your shared file system uses NFS then ensure that every node is running with
the same numeric UID and GID, or else update your NFS configuration to account
for the variance in IDs across nodes.

...
. If you run {es} on your own hardware and <<backup-cluster-configuration,backed
up the cluster's configuration files>>, you can restore them to each node. This
step is optional and requires a <<restart-upgrade,full cluster restart>>.
+
...
include::index.asciidoc[tag=index-compat]

include::index.asciidoc[tag=restore-reindex-workaround]

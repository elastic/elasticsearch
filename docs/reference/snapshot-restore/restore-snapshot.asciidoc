...
data after an accidental deletion or a hardware failure.
...
[source,console]
----
GET _snapshot/my_repository/*?filter_path=*.snapshot,*.version_id,*.start_time
----
// TEST[setup:setup-snapshots]
...
methods to avoid conflicts with existing indices or data streams:
...
To get detailed information about unassigned primary shards or replicas
resulting from a restore operation, use the <<cluster-allocation-explain,cluster
allocation API>>.

...
To restore this closed index, the index in the snapshot must have the same
...
You can rename indices and data streams you restore. This lets you restore data
from a snapshot without deleting data.

You can use this method to compare a cluster's existing data to historical data
from a snapshot. For example, you could review changes made to a document after
...
If you restore an existing closed index, the index in the snapshot must have the

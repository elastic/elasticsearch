...
request attempts to restore all indices and data streams, including system
indices. If your cluster contains any existing system indices, the request will
return an error. To restore all indices and data streams except system indices,
use `*,-.*`

...
Then use the get snapshot API to get a list of snapshots in a specific repository.
...
If you take <<backup-security-index-configuration,dedicated snapshots of the
`.security` system index>>, restore it as well.
...
deletes any existing data in the cluster for the index or data stream. The
operations don't affect the snapshot or its data.

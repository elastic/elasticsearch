...
streams, and all open indices daily at 1:30 a.m. UTC.
...
<2> Snapshot name. Supports <<date-math-index-names,date math>>. To prevent
...
<4> Data streams and indices to include in the policy's snapshots. This
    configuration includes all data streams and all open indices, including
    system indices.
...
<1> Excludes the cluster state. This also excludes all feature states by
    default.
<2> Includes all data streams and all open indices except system indices and
    other indices that begin with a dot (`.`).

...
If you run {es} on your own hardware, we recommend that you take regular backups
of the files in each node's `$ES_PATH_CONF` directory using the file backup
software of your choice in addition to backups. Snapshots don't back up these
files.
...
regardless of the cluster state. You can't back up specific feature states using
{kib}.
...
repository doesn't exceed the <<snapshot-retention-limits,200 snapshot soft
limit>>.

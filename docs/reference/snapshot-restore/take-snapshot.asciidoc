...
* Snapshots are incremental. You can take frequent snapshots with little impact
to your storage overhead. See <<how-snapshots-work>>.
...
While not a hard limit, a snapshot repository shouldn't contain more than 200
snapshots at a time. This ensures the repository's metadata doesn't grow to a
size that may destabilize the master node. We strongly recommend you set up your
{slm-init} policy's retention rules to enforce this limit.
...
cluster state excludes all feature states by default. However, you can also
configure a snapshot to only include specific feature states, regardless of the
cluster state.

NOTE: You can't back up specific feature states in {kib}.
...
repository don't exceed the <<snapshot-retention-limits,200 snapshot soft
limit>>.
...
[source,console]
----
PUT _slm/policy/hourly-snapshot-policy
{
...
<1> Runs at 23:45:00 UTC every day.
...
[source,console]
----
PUT _slm/policy/monthly-snapshot-policy
{
...
<1> Runs on the first of the month at 23:56:00 UTC.

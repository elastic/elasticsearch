...
[discrete]
[[take-snapshot-considerations]]
=== Considerations

* Each snapshot must have a unique name within its snapshot repository. Attempts
to create a snapshot with the same name as an existing snapshot will fail.
...
snapshot may not include changes made after the snapshot process starts.
...
However, the snapshot won't include the associated data stream’s metadata or
its other backing indices.

* A snapshot can include a data stream but exclude specific backing indices.
When you restore the data stream, it will contain only backing indices in the
...
You can also manage
{slm-init} using the <<snapshot-lifecycle-management-api,{slm-init} APIs>>.

TIP: {ess} deployments automatically include the `cloud-snapshot-policy`
{slm-init} policy. For more information, see the
{cloud}/ec-snapshot-restore.html[{ess} snapshot documentation].

[discrete]
[[create-slm-policy]]
==== Create an {slm-init} policy

To create an {slm-init} policy, use {kib} or the <<slm-api-put-policy,create
{slm-init} policy API>>.
...
<2> Snapshot name. Use <<api-date-math-index-names,date math>> to include the
    current date or time in the name.
...
You can manually run an {slm-init} policy to immediately start a snapshot. This
...
Once started, the snapshot process runs in the background. To monitor its
progress, see <<monitor-snapshot>>.
...
[source,console]
----
# PUT _snapshot/my_repository/<snapshot-{now/d}>
PUT _snapshot/my_repository/%3Csnapshot-%7Bnow%2Fd%7D%3E
----
...
[discrete]
[[check-slm-history]]
==== Check {slm-init} execution history

Use the <<slm-api-get-policy,get {slm-init} policy API>> to check when an
{slm-init} policy last successfully started a snapshot. However, starting a
snapshot doesn't guarantee the snapshot completed.
...
contain sensitive data. For example, the `.security-*` indices contain user
names or encrypted password hashes.

To better protect this data, consider creating a dedicated repository and
{slm-init} policy for snapshots of the cluster state. You can stictly limit and

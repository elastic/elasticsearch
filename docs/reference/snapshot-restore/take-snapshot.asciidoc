...
cluster. Use the <<snapshot-restore-apis,Snapshot and restore APIs>> to
manage repositories and snapshots.
...
The snapshot process is executed in non-blocking fashion. All indexing and searching operation can continue to be
...
The snapshot process starts immediately for the primary shards that have been started
and are not relocating at the moment. {es} waits for
...
Only one snapshot process can be executed in the cluster at any time. While a
...
filtering settings and rebalancing algorithm) after the snapshot is finished.

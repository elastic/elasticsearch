...
both this parameter and the `Accept` HTTP header, this parameter take precedence.
...
<<sql-async,async>>.
+
To <<sql-store-searches,save a synchronous search>>, you specify this parameter
and the `keep_on_completion` parameter.

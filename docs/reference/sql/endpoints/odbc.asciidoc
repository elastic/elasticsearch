...
the {es}'s SQL ODBC API, converting ODBC calls into {es-sql}.

In order to be able to use of the driver, the server must have {es-sql}
installed and running with the valid license.

...
* Type
+
What kind of protocol to use when connecting to the proxy host. This also mandates how the the {es} node you want to connect to over the proxy needs to be specified under <<2.2 Connection parameters>>:
+
** HTTP, SOCKS4A, SOCKS5H: either IP address or host name are accepted; the proxy will resolve the DNS name;

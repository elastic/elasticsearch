...
page. This corresponds to {es-sql}'s request parameter `fetch_size`. The value
...
* `auto`: the driver will chose one of the above depending on the value to be
printed.
...
values as numbers itself and then does the conversion subsequently.
...
The variation however is only at minor version, both endpoints must be of same version number.
...
such a field, it will pick the first value in the set and return it as the
value for the column.
If not set, the server will return an error.

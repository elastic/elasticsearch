...
[IMPORTANT]
If in the initial HTTP request, that specifies the SQL query, there was a <<sql-rest-fields-timezone, `time_zone`>>
parameter passed, the same `time_zone` parameter *must be passed* to the subsequent `cursor` requests in order to keep
applying the same `time_zone` (as in the initial HTTP request) to the result data of each subsequent page.
...
[NOTE]
Most parameters (except for the `time_zone`, the `columnar` and the timeout ones) make sense only during the initial
query - any follow-up pagination request only requires the `cursor` and the `time_zone` parameter (only if `time_zone`
is used also in the initial query) as explained in the <<sql-pagination, pagination>> chapter. That's because the query
has already been executed and the calls are simply about returning the found results - thus the parameters are simply
ignored.

...
[IMPORTANT]
If in the initial HTTP request, that specifies the SQL query, there was a <<sql-rest-fields, `time_zone` parameter>>
passed, the same `time_zone` parameter *must be passed* to the subsequent `cursor` requests in order to keep

...
Add the given number of date/time units to a date/datetime. If the number of units is negative
then it's subtracted from the date/datetime. If argument is an interval,
an error is thrown since week unit for intervals is not supported because months usually have fractional weeks.
...
Truncate the date/datetime/interval to the specified unit by setting all fields that are less significant than the specified
one to zero (or one, for day, day of week and month). If any of the two arguments is `null` a `null` is returned.
If first argument is `week` and second argument is interval, `interval units week not supported because months usually have have fractional weeks` is returned.

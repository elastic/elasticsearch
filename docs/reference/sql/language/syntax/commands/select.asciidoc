...
The query could logically be broken down in the following steps:
...
. the values in these appended columns are the `SUM` aggregations of `salary`, group by the respective language.

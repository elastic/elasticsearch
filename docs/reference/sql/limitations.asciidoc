...
[discrete]
[[using-fields-api]]
=== Retrieving using "fields" search parameter

{es-sql} retrieves columns' values using <<search-fields-param,`_search` "fields" parameter>> and, as such, any
limitations this parameter has, will be visible in {es-sql} queries, as well. For example, if `_source` is disabled

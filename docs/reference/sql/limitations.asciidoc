...
from a query as a first step before its analysis, planning and execution. As a consequence certain types of queries or
expressions inside them can generate abstract syntax trees of large depth which will exhaust the JVM's stack and will
trigger a `StackOverflowException`. {es-sql}'s parser will handle the `StackOverflowException` and throw a user facing
...
- Large logical operator expressions
[source, sql]
--------------------------------------------------
...
- Large number of nested sub-selects
[source, sql]
--------------------------------------------------
SELECT a, b, ... FROM (SELECT a, b... FROM (SELECT * FROM ( ...
--------------------------------------------------

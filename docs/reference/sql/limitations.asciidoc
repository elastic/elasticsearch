...
Sub-selects that require the aggregation of already aggregated data are currently **not supported**. For example,
`SELECT SUM(c) FROM (SELECT COUNT(*) c FROM emp)` fails because it results in a nested aggregation.

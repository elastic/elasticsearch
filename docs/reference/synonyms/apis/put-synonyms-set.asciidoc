...
`<body>`::
(Required, object)
Contains the definition of the synonym rules that conform the synonyms set:
+
...
This would be equivalent to invoke <<indices-reload-analyzers>> for all indices that use the synonyms set.

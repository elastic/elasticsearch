// tag::cloud[]
The {cloud}/ec-snapshot-restore.html[`found-snapshots`] repository supports
{search-snaps} and requires no additional registration.

The `found-snapshots` repository is specific to your cluster. To use another
cluster's `found-snapshots` repository, see

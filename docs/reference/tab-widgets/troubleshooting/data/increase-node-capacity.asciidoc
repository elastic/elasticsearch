...
of data nodes in the {es} cluster so that the replica shards can be placed. This can be done by
...
. Now that you know the tier, go back to the deployment's landing page and click the
**Manage** button, and choose **Edit deployment**.
...
. Inspect the <<dynamic-index-number-of-replicas,`index.number_of_replicas`>> index setting. The examples below
use `my-index-000001` as the index name, but replace that with your index name:
+
[source,console]
----
...
. If the number is greater than 0, you can <<indices-update-settings, decrease>> the
...
inspect the `cluster.routing.allocation.total_shards_per_node`

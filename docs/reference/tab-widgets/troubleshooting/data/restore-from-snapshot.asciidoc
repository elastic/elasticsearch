...
+
<2> The settings of the index <1> that contains the metadata we are looking for
+
<3> This indicates that this index is part of a data stream and shows the data stream name.
+
...
find an index like this, which belongs to a data stream, you need to check if it is the one that it is being written on.
You can see that by checking if in the `settings` you can find this property: `"index.lifecycle.indexing_complete" : "true"`.
If it's there you can continue to the next step.
+
If it is not there we need to rollover the data stream to allow us to restore the missing data without blocking
...
. Now that the data stream preparation is done (if it was necessary), we will close the target indices via the use the

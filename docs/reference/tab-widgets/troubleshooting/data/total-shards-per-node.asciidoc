...
. On the **Deployments** page click on the name of your deployment, if it is enabled.
+

NOTE:
If the name of your deployment is grayed out your {kib} instances might be
...
. <<indices-update-settings,Increase>> the value for the maximum number of shards 
...
Another value the `total_shards_per_node` configuration can be set to is `-1`, which 

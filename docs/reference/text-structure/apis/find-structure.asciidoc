...
`ecs_compatibility`::
(Optional, string) The mode of compatibity with ECS compliant Grok patterns.
When the structure finder creates a Grok pattern you can use this parameter to
specify whether to use ECS Grok patterns instead of the legacy ones. Valid values

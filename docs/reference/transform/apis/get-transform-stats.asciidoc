...
(date) TBD
`checkpointing`.`last`:::
(object) Contains information about the last completed checkpoint. 
...
(TBD) The number of indexing failures.
`stats`.`index_time_in_ms`:::
(TBD) The amount of time spent indexing, in milliseconds.
`stats`.`index_total`:::
(TBD) The number of indices created.
`stats`.`pages_processed`:::
(TBD) The number of pages processed.
`stats`.`search_failures`:::
(TBD) The number of search failures. 
`stats`.`search_time_in_ms`:::
(TBD) The amount of time spent searching, in milliseconds.
`stats`.`search_total`:::
(TBD) TBD 
`stats`.`trigger_count`:::
(TBD) TBD

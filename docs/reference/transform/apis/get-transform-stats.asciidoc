...
(date) When using time-based synchronization, this timestamp indicates when the 
data has been processed into the destination index.
`checkpointing`.`last`.`timestamp_millis`::::
(date) The timestamp of the checkpoint, which indicates when the checkpoint was
created. This typically occurs after the data is added to the destination index.
`checkpointing`.`next`:::

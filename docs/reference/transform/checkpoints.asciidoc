...
https://www.elastic.co/guide/en/ecs/current/ecs-reference.html[ECS standard], 
you might already have an <<field-event-ingested,`event.ingested`>> field. In 
this case, use `event.ingested` as the `sync`.`time`.`field` property of your 
{transform}.

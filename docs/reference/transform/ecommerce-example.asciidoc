...
<<search-aggregations-metrics-max-aggregationn`max` aggregation>> on the
...
<<preview-data-frame-transform,preview {transforms} API>>:
...
If you prefer, you can use the
<<put-data-frame-transform,create {transforms} API>>. For
example:
...
<<start-data-frame-transform.start {transforms}>> and
<<stop-data-frame-transform,stop {transforms}>> APIs. For
example:
...
{kib} or use the
<<delete-data-frame-transform,delete {transform} API>>. When

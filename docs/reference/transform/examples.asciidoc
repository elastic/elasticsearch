...
<2> This configures the {transform} in continuous mode using the field `timestamp`
as syncronization field. The `delay` is set to 60s and represents the worst case
ingest delay we expect.
...
After creation the transform can be started with:
...
Shortly after the first results should be available using search on the destination
index:
...
timestamps relative to when they are installed, including timestamps in the future.
The continuous {transform} will pickup the data points once they are in the past
(see delay). This enables you the see continuous mode in action, although we are
not ingesting new data. If you installed the web log sample dataset some time ago,
you can uninstall and reinstall it and the timestamps will change.

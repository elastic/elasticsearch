...
This example uses the eCommerce orders sample dataset to find the customers who 
...
unique row per `user` and `customer_id` combination. Within this dataset, both 
...
This example uses the Flights sample dataset to find out which air carrier 
...
This example uses the web log sample dataset to identify suspicious client IPs. 
...
[[example-last-log]]
== Finding the last log event from an IP address

This example uses the web log sample dataset to find the last log from an IP 
address. Let's use the `latest` type of {transform} which copies the most recent 
documents from the source index to the destination index. Pick the `clientip` 
field as the unique key; the data is grouped by this field. Select `@timestamp` 
as the date field that sorts the data chronologically. The {transform} creates 
the destination index that contains the latest login date for each client IP.
...
<1> Specifies the filed for grouping the data.
<2> Specifies the date filed that is used for sorting the data.
...
After the {transform} stopped, search the destination index:

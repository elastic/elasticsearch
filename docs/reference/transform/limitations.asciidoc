...
* {transforms-cap} cannot optimize queries when you use scripts for all the 
  groupings defined in `group_by`. As {transforms} write audit and log warning 
  messages when the queries are not optimized, you will receive a warning 
  message when you use scripts this way.
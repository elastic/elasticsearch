...
the Elastic {transform} feature (Note that as of {es} version 8.10.0 a new
version number has been introduced to track changes to configuration and state
in the transform plugin. This new version number is decoupled from the
product version and will incrementindependently as necessary).
The limitations are grouped into the following categories:

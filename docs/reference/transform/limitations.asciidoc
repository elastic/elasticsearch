...
* {transforms-cap} cannot optimize queries when you use scripts in `group_by`. 
  As {transforms} write audit and log warning messages every time when the 
  queries are not optimized, you may experience an increased number of warning 
  messages when you use scripts.
...
* Kibana's {kibana-ref}/alerting-getting-started.html[alerting] or {kibana-ref}/detection-engine-overview.html[security rules] may error `The content length (X) is bigger than the maximum allowed string (Y)` where `X` is attempted payload and `Y` is {kib}'s {kibana-ref}/settings.html#server-maxPayload[`server-maxPayload`]. 

* Long {es} start-up durations. See link:https://github.com/elastic/elasticsearch/issues/83203[this discussion].

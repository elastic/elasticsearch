...
NOTE: Another cause of indexing pressure rejections might be the use of the <<semantic_text>> field type, which can cause rejections when indexing large batches of documents if the batch may otherwise incur an Out of Memory (OOM) error.

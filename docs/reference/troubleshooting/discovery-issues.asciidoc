...
problem, which is when nodes within a cluster disagree on which node is elected 
as master node. This is a non-viable state for the cluster which can lead to 
inconsistencies and data loss. 

To prevent this, we recommend

* <<modules-discovery-voting,avoiding even 
master-eligible node counts>> for example with <<high-availability-cluster-design-two-nodes,tie 
breakers>>

* <<modules-discovery-voting,not stopping more half of the voting nodes>> at the same time

* removing <<initial_master_nodes,`cluster.initial_master_nodes` 
setting>> after <<modules-discovery-bootstrap-cluster,bootstrapping a cluster>>

To mitigate impact if this occurs, we recommend having a <<high-availability,highly 
available cluster>> and <<snapshot-restore,snapshots to restore from>> as needed. 

{ess} and {ece} automatically detect split-brain for their deployments and and will 
applicably report `The requested resource is currently split` when polling the 
<<rest-apis,{es} API>>. For other deployment types, you cannot detect split-brain 
from within a cluster, but must check for per node. Example detections: 

* If <<cluster-state,Cluster State>> flagged `local=true` disagrees on the 
reported `master_node` when polled across all nodes' expected within a cluster
+
[source,console]
--------------------------------------------------
...
* If node's logs contain messages about cluster UUID disagreements like 

** `trying to join a different cluster with UUID`
+
[source,txt]
--------------------------------------------------
org.elasticsearch.cluster.coordination.CoordinationStateRejectedException: This node previously joined a cluster with UUID [XXXXX] and is now trying to join a different cluster with UUID [YYYYY]. This is forbidden and usually indicates an incorrect discovery or cluster bootstrapping configuration. Note that the cluster UUID persists across restarts and can only be changed by deleting the contents of the node's data path [/app/data] which will also remove any data held by this node.
--------------------------------------------------
...
[source,txt]
--------------------------------------------------
org.elasticsearch.cluster.coordination.CoordinationStateRejectedException: join validation on cluster state with a different cluster uuid XXXXX than local cluster uuid YYYYY, rejecting
--------------------------------------------------

To resolve when detected

. You must choose which elected master to continue running the cluster. Usually 
this will be the one recognized by the most nodes or containing the most desired data. 

. For the group of nodes pointing to the other master(s), you'll stop 
those problematic nodes. As needed while stopped, fix any 
<<settings,`elasticsearch.yml`>> settings as highlighted above. 

. Then attempt to start the nodes back up. Start-up logs will show if node is willing 
and able to join existing master or if it refuses to leave previous cluster UUID. 

** If the nodes are able to join to the existing elected master node's cluster, 
you've resolved the current split-brain issue. 

** If the nodes are unwilling to leave the previous cluster UUID, you are 
in potential data loss territory but there are still options to resolve the 
cluster state. You will want to consider <<node-tool-unsafe-bootstrap,unsafe 

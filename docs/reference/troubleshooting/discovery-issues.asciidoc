...
* <<modules-discovery-voting,Avoid stopping more half of the voting nodes>> at the same time
...
. Stop any nodes pointing to the other master or masters. While each node is stopped, fix any <<settings,`elasticsearch.yml`>> settings as highlighted above. 

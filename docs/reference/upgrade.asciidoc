...
earlier released version to a pre-release build of {es} {version}, but
upgrading from a pre-release build to another build (whether released or not)
is unsupported and typically will not work.
endif::[]

...
have indices created in 6.x or before, you must reindex or delete them
...
restored to a 8.x cluster even if they were created by a 8.x cluster.
...
* <<reindex-upgrade-inplace, Reindex in place>> on your 7.x cluster before upgrading.
...
pre-7.x indices forward to {version}. Data in time-based indices
...
wait to upgrade to 7.x until all of your pre-7.x indices have
been deleted.
...
{prev-major-version}, you must temporarily halt the tasks associated with your {ml} jobs and {dfeeds} and prevent new jobs from opening during the reindex. Use

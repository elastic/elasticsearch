...
Newer nodes can always join a cluster with an older master, but older nodes cannot always join a cluster with
...
join the cluster whether the master-eligible nodes have been upgraded or not.

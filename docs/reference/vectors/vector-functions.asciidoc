...
[NOTE]
====
If a document doesn't have a value for a vector field on which
...
====== Accessing dense vectors directly

The recommended way to access dense vectors is through `cosineSimilarity`,
`dotProduct`, `l1norm` or `l2norm` functions. Please note however, that these
functions should only be called once per script. For example, don't use these
functions in a loop to calculate the similarity between a document's dense
vector and several other vectors. For custom use cases like this, you need to
reimplement these functions by accessing dense vector values directly through

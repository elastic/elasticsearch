...
[float]
=== Replicas can fall out of sync due to reliance on the timing of deletions (STATUS: DONE, V6.0.0)

If a document is deleted and another document with the same id is indexed some
time later then it was possible for these operations not to be replicated
correctly, leading to the replica falling out of sync with the primary.
...
recently-deleted documents could have differed between the two shard copies.
More precisely, if the primary had discarded the deletion record but a replica
had not (at the point of processing the indexing operation) then the primary
would assign version number 1 to the document again, which the replica would
then ignore under the mistaken impression that it already had a version of the
document that was at least as new.

The times of the operations could be skewed like this due to a network
partition, a GC pause, or because the operations were being replayed during
recovery. The introduction of sequence numbers {GIT}10708[#10708] means that
this timing issue is no longer present.

...
public class InstrumenterTests extends ESTestCase {
    ...
    public static class TestInstrumentationRegistry implements InstrumentationRegistry {

        volatile boolean isActive;

        @Override
        public void check$(String instrumentationId, Class<?> callingClass, Object... args) throws Exception {
            if ("someStaticMethod".equals(instrumentationId)) {
                assert args.length == 1 && args[0] instanceof Integer arg && arg == 123;
                if (isActive) {
    ...
    public static class TestInstrumentationRegistryHolder {
        static TestInstrumentationRegistry checkerInstance = new TestInstrumentationRegistry();

        public static TestInstrumentationRegistry instance() {
    ...
    public void resetInstance() {
        TestInstrumentationRegistryHolder.checkerInstance = new TestInstrumentationRegistry();
    }

    public void testStaticMethod() throws Exception {
        Method targetMethod = TestClassToInstrument.class.getMethod("someStaticMethod", int.class);
        TestLoader loader = instrumentTestClass(InstrumenterImpl.create(TestInstrumentationRegistry.class, Map.of(
    ...
    // expectThrows(NotEntitledException.class, () -> testTargetClass.someMethod(123, "def"));
    //
    ...
    private void assertStaticMethodThrows(TestLoader loader, Method method, Object... args) {
        ...
        TestInstrumentationRegistryHolder.checkerInstance.isActive = true;
        expectThrows(NotEntitledException.class, () -> callStaticMethod(testMethod, args));
    }

    private void assertStaticMethod(TestLoader loader, Method method, Object... args) {
        Method testMethod = loader.getSameMethod(method);
        TestInstrumentationRegistryHolder.checkerInstance.isActive = false;
    ...
    private void assertCtorThrows(TestLoader loader, Constructor<?> ctor, Object... args) {
        ...
        TestInstrumentationRegistryHolder.checkerInstance.isActive = true;
        expectThrows(NotEntitledException.class, () -> {
            try {

...
public interface EntitlementChecker {
    ...
    // Using the list of restricted methods from
    ...
    /*
     * NIO For NIO, we are sometime able to check a method on the public
     * surface/interface (e.g. AsynchronousServerSocketChannel#bind) but most of the
     * time these methods are abstract in the public classes/interfaces (e.g.
     * ServerSocketChannel#accept, NetworkChannel#bind), so we are forced to
     * implement the "impl" classes. You can distinguish the 2 cases form the
     * namespaces: java_nio_channels for the public ones, sun_nio_ch for the
     * implementation classes. When you see a check on a sun_nio_ch class/method,
     * this means the matching method on the public class is abstract (not
     * instrumentable).
     */
    ...
    // We need to check the LDAPCertStore, as this will connect, but this is
    // internal/created via SPI,
    // so we instrument the general factory instead and then filter in the check
    ...
    // HttpClient#send and sendAsync are abstract, so we instrument their internal
    // implementations

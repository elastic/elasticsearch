...
class StructuredTaskScopeActions {
    ...
    static void java_util_concurrent_StructuredTaskScope$open_Joiner_UnaryOperator() throws Exception {
        try (var scope = StructuredTaskScope.open(StructuredTaskScope.Joiner.awaitAllSuccessfulOrThrow(), cf -> cf)) {
            // opening and closing is enough to test the entitlement check
        }
    }

    @EntitlementTest(expectedAccess = PLUGINS)
    static void java_util_concurrent_StructuredTaskScope$open_UnaryOperator() throws Exception {
        try (var scope = StructuredTaskScope.open(cf -> cf)) {
            // opening and closing is enough to test the entitlement check
        }
    }
}

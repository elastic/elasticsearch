...
public class FileSystemProviderInstrumentation implements InstrumentationConfig {
    ...
    public void init() {
        ...
        on(defaultFileSystemProvider).calling(
            ...
            .enforce((_, path) -> Policies.fileRead(path))
            .elseThrowNotEntitled()
            .calling(FileSystemProvider::createDirectory, Path.class, FileAttribute[].class)
            .enforce((_, path) -> Policies.fileWrite(path))
            .elseThrowNotEntitled()
            .calling(FileSystemProvider::createSymbolicLink, Path.class, Path.class, FileAttribute[].class)
            .enforce((_, link, target) -> Policies.fileWrite(link).and(Policies.fileRead(target)))
            .elseThrowNotEntitled()
            .calling(FileSystemProvider::createLink, Path.class, Path.class)
            .enforce((_, link, target) -> Policies.fileWrite(link).and(Policies.fileRead(target)))
            .elseThrowNotEntitled()
            .calling(FileSystemProvider::delete, Path.class)
            .enforce((_, path) -> Policies.fileWrite(path))
            .elseThrowNotEntitled()
            ...
            .enforce((_, path) -> Policies.fileRead(path))
            .elseThrowNotEntitled()
            .calling(FileSystemProvider::copy, Path.class, Path.class, CopyOption[].class)
            .enforce((_, source, target) -> Policies.fileRead(source).and(Policies.fileWrite(target)))
            .elseThrowNotEntitled()
            .calling(FileSystemProvider::move, Path.class, Path.class, CopyOption[].class)
            .enforce((_, source, target) -> Policies.fileRead(source).and(Policies.fileWrite(target)))
            .elseThrowNotEntitled()
            ...
            .enforce((_, path) -> Policies.fileRead(path))
            .elseThrowNotEntitled()
            .calling(FileSystemProvider::checkAccess, Path.class, AccessMode[].class)
            .enforce((_, path) -> Policies.fileRead(path))
            .elseThrowNotEntitled()
            .calling(
            ...
            .enforce((_, path) -> Policies.fileRead(path))
            .elseThrowNotEntitled()
            .calling(FileSystemProvider::setAttribute, Path.class, String.class, Object.class, LinkOption[].class)
            .enforce((_, path) -> Policies.fileWrite(path))
            .elseThrowNotEntitled()

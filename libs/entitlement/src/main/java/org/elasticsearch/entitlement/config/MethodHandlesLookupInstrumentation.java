...
public class MethodHandlesLookupInstrumentation implements InstrumentationConfig {
    @Override
    public void init(InternalInstrumentationRegistry registry) {
        ...
        builder.on(MethodHandles.Lookup.class, rule -> {
            rule.calling(MethodHandles.Lookup::defineClass, byte[].class).enforce(Policies::defineClass).elseThrowNotEntitled();
            rule.calling(MethodHandles.Lookup::defineHiddenClass, byte[].class, Boolean.class, MethodHandles.Lookup.ClassOption[].class)
                .enforce(Policies::defineClass)
            ...
            ).enforce(Policies::defineClass).elseThrowNotEntitled();
        });
    }
}

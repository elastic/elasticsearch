...
public class LineTests extends BaseGeometryTestCase<Line> {
    ...
    public void testInitValidation() {
        ...
        StandardValidator.get(true).validate(new Line(new double[]{3, 4}, new double[]{1, 2}, new double[]{6, 5}));
    }

    public void testWKTValidation() {
        IllegalArgumentException ex = expectThrows(IllegalArgumentException.class,
            () -> WellKnownText.fromWKT(GeographyValidator.get(false), randomBoolean(), "linestring (3 1 6, 4 2 5)"));
        assertEquals("found Z value [6.0] but [ignore_z_value] parameter is [false]", ex.getMessage());
    }
}

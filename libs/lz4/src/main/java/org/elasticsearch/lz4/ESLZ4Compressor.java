...
public class ESLZ4Compressor extends LZ4Compressor {
    ...
    private static final ObjectPool<short[]> sixtyFourKBHashTable = ObjectPool.withInitial(() -> new short[8192]);
    private static final ObjectPool<int[]> biggerHashTable = ObjectPool.withInitial(() -> new int[4096]);
    ...
    static int compress64k(byte[] src, int srcOff, int srcLen, byte[] dest, int destOff, int destEnd) {
        ...
        int anchor = srcOff;
        if (srcLen >= 13) {
            // Modified to use pooled hash table
            try (var pooledSixtyFourKBHashTable = sixtyFourKBHashTable.acquire()) {
                short[] hashTable = pooledSixtyFourKBHashTable.get();
    ...
    public int compress(byte[] src, int srcOff, int srcLen, byte[] dest, int destOff, int maxDestLen) {
        ...
        if (srcLen < 65547) {
            ...
            int anchor = srcOff;
            // Modified to use pooled hash table
            try (var pooledBiggerHashTable = biggerHashTable.acquire()) {
                int[] hashTable = pooledBiggerHashTable.get();

...
public class NioTimerTests extends ESTestCase {

    private NioTimer timer = new NioTimer();

    public void testScheduleTask() {
        AtomicBoolean complete = new AtomicBoolean(false);

        long executeTime = System.nanoTime() + TimeUnit.MILLISECONDS.toNanos(10);
        timer.scheduleAtRelativeTime(() -> complete.set(true), executeTime);

        while (true) {
            long nanoTime = System.nanoTime();
            Runnable runnable = timer.pollTask(nanoTime);
    ...
    public void testPollScheduleTaskAtExactTime() {
        ...
        assertNull(timer.pollTask(executeTime - 1));
        assertNotNull(timer.pollTask(executeTime));
    }
    ...
    public void testTaskOrdering() {
        ...
        long executeTime = System.nanoTime() + TimeUnit.MILLISECONDS.toNanos(10);
        timer.scheduleAtRelativeTime(() -> third.set(true), executeTime + 2);
        timer.scheduleAtRelativeTime(() -> first.set(true), executeTime);

...
EXPORT f32_t vec_dotf32_2(const f32_t* a, const f32_t* b, const int32_t elementCount) {
    ...
    for (; i + stride <= elementCount; i += stride) {
        sum0 = svmla_f32_x(pg, sum0, svld1_vnum(pg, a + i, 0), svld1_vnum(pg, b + i, 0));
        sum1 = svmla_f32_x(pg, sum1, svld1_vnum(pg, a + i, 1), svld1_vnum(pg, b + i, 1));
        sum2 = svmla_f32_x(pg, sum2, svld1_vnum(pg, a + i, 2), svld1_vnum(pg, b + i, 2));
        sum3 = svmla_f32_x(pg, sum3, svld1_vnum(pg, a + i, 3), svld1_vnum(pg, b + i, 3));
    }

    svfloat32_t sum = svadd_f32_x(svptrue_b32(),
        svadd_f32_x(svptrue_b32(), sum0, sum1),
        svadd_f32_x(svptrue_b32(), sum2, sum3));

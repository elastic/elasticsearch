...
EXPORT int64_t vec_dot_bit_int4_2(const int8_t* a, const int8_t* b, const int32_t dims) {
    int64_t subRet0 = 0;
    int64_t subRet1 = 0;
    int64_t subRet2 = 0;
    int64_t subRet3 = 0;
...
static inline void dot_bit_int4_inner_bulk(
    ...
    for (size_t c = 0; c < count; c++) {
        const int8_t* a0 = a + mapper(c, offsets) * pitch;
        results[c] = (f32_t)vec_dot_bit_int4_2(a0, b, dims);
    }
}

EXPORT void vec_dot_bit_int4_bulk_2(
    const int8_t* a,
    const int8_t* b,
    const int32_t dims,
    const int32_t count,
...
EXPORT void vec_dot_bit_int4_bulk_offsets_2(
    const int8_t* a,
    const int8_t* b,
    const int32_t dims,
    const int32_t pitch,

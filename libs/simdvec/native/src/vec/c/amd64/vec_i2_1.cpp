...
// AVX2-vectorized int2 packed-quad vector operations. Thin EXPORT thunks
...
EXPORT int32_t vec_doti2(const int8_t* query, const int8_t* doc, int32_t packed_len) {
    return doti_inner_avx2<2>(query, doc, packed_len);
}

EXPORT void vec_doti2_bulk(const int8_t* docs, const int8_t* query, int32_t packed_len, int32_t count, f32_t* results) {
    doti_bulk_impl_avx2<2, int8_t, sequential_mapper>(docs, query, packed_len, packed_len, NULL, count, results);
}
...
EXPORT void vec_doti2_bulk_offsets(
    ...
    doti_bulk_impl_avx2<2, int8_t, offsets_mapper>(docs, query, packed_len, pitch, offsets, count, results);
}
...
EXPORT void vec_doti2_bulk_sparse(
    ...
    doti_bulk_impl_avx2<2, const int8_t*, sparse_mapper>(

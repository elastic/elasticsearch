...
// AVX-512 vectorized int2 packed-quad vector operations.
...
// are in 0..3 so the unsigned/signed asymmetry of vpdpbusd is irrelevant. Max
// per-lane contribution per call is 4*3*3 = 36; integrated over the whole
// vector the running sum is 36*packed_len/64 per stripe, well below int32, so
// no chunked 16-bit accumulator is required (unlike the AVX2 tier-1 path).
...
static inline int32_t doti2_inner_avx512(const int8_t* query, const int8_t* doc, int32_t packed_len) {
    ...
    __m512i acc = _mm512_add_epi32(_mm512_add_epi32(acc_s0, acc_s1), _mm512_add_epi32(acc_s2, acc_s3));
    return _mm512_reduce_add_epi32(acc);
}

EXPORT int32_t vec_doti2_2(const int8_t* query, const int8_t* doc, int32_t packed_len) {
    return doti2_inner_avx512(query, doc, packed_len);
}
...
template <
    ...
    int batches = 2
>
static inline void doti2_bulk_impl_avx512(
    const TData* docs,
    const int8_t* query,
    int32_t packed_len,
    int32_t pitch,
    ...
    for (; c + batches - 1 < count; c += batches) {
        ...
        apply_indexed<batches>([&](auto I) {
            __m512i acc = _mm512_add_epi32(
                _mm512_add_epi32(acc_s0[I], acc_s1[I]),
                _mm512_add_epi32(acc_s2[I], acc_s3[I])
            );
            results[c + I] = (f32_t)_mm512_reduce_add_epi32(acc);
        });
    ...
    for (; c < count; c++) {
        ...
        results[c] = (f32_t)doti2_inner_avx512(query, doc, packed_len);
    }
}

EXPORT void vec_doti2_bulk_2(const int8_t* docs, const int8_t* query, int32_t packed_len, int32_t count, f32_t* results) {
    doti2_bulk_impl_avx512<int8_t, sequential_mapper, 2>(docs, query, packed_len, packed_len, NULL, count, results);
}

EXPORT void vec_doti2_bulk_offsets_2(
    const int8_t* docs,
    const int8_t* query,
    int32_t packed_len,
    int32_t pitch,
    ...
    doti2_bulk_impl_avx512<int8_t, offsets_mapper, 4>(docs, query, packed_len, pitch, offsets, count, results);
}

EXPORT void vec_doti2_bulk_sparse_2(
    ...
    doti2_bulk_impl_avx512<const int8_t*, sparse_mapper, 4>(

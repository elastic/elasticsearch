 ...
 */
final class MSPackedD2Q4ES940OSQVectorsScorer extends MemorySegmentES940OSQVectorsScorer.MemorySegmentScorer {
    ...
    MSPackedD2Q4ES940OSQVectorsScorer(IndexInput in, int dimensions, int dataLength, int bulkSize) {
    ...
    long quantizeScore(byte[] q) throws IOException {
        ...
        return IndexInputUtils.withSlice(
            ...
            length,
            this::getScratch,
            segment -> Similarities.dotProductD2Q4Packed(segment, querySegment(q), length)
        );
    }
    ...
    boolean quantizeScoreBulk(byte[] q, int count, float[] scores) throws IOException {
        ...
        IndexInputUtils.withSlice(in, (long) length * count, this::getScratch, dSeg -> {
            Similarities.dotProductD2Q4PackedBulk(dSeg, qSeg, length, count, sSeg);
            return null;
        });
        return true;
    }
    ...
    boolean quantizeScoreBulkOffsets(byte[] q, int[] offsets, int offsetsCount, float[] scores, int count) throws IOException {
        ...
        var sSeg = scoresSegment(scores);
        IndexInputUtils.withSlice(in, (long) length * count, this::getScratch, dSeg -> {
            Similarities.dotProductD2Q4PackedBulkWithOffsets(dSeg, qSeg, length, length, offsetsSeg, offsetsCount, sSeg);
            return null;
        });

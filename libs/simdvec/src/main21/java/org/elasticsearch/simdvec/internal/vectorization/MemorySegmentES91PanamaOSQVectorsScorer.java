...
public class MemorySegmentES91PanamaOSQVectorsScorer extends ES91OSQVectorsScorer {
    ...
    public MemorySegmentES91PanamaOSQVectorsScorer(IndexInput in, int dimensions, MemorySegment memorySegment) {
        super(in, dimensions);
        this.memorySegment = memorySegment;
    }

    protected long panamaQuantizeScore(byte[] q) throws IOException {
    ...
    protected void panamaQuantizeScoreBulk(byte[] q, int count, float[] scores) throws IOException {
        assert q.length == length * 4;
        // 128 / 8 == 16
        if (length >= 16 && PanamaESVectorUtilSupport.HAS_FAST_INTEGER_VECTORS) {
            if (PanamaESVectorUtilSupport.VECTOR_BITSIZE >= 256) {

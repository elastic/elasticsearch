...
public class OnHeapES91PanamaOSQVectorsScorer extends ES91OSQVectorsScorer {
    ...
    static {
        ...
        INT_SPECIES = VectorSpecies.of(int.class, VectorShape.forBitSize(PanamaESVectorUtilSupport.VECTOR_BITSIZE));
    }

    protected final byte[] bytes;

    public OnHeapES91PanamaOSQVectorsScorer(IndexInput in, int dimensions) {
        super(in, dimensions);
        bytes = new byte[BULK_SIZE * length];
    }

    public long panamaQuantizeScore(byte[] q) throws IOException {
    ...
    public void panamaQuantizeScoreBulk(byte[] q, int count, float[] scores) throws IOException {
        assert q.length == length * 4;
        // 128 / 8 == 16
        if (length >= 16 && PanamaESVectorUtilSupport.HAS_FAST_INTEGER_VECTORS) {
            if (PanamaESVectorUtilSupport.VECTOR_BITSIZE >= 256) {
    ...
    protected float applyCorrectionsBulk(
        ...
        for (; i < limit; i += FLOAT_SPECIES.length()) {
            var ax = FloatVector.fromArray(FLOAT_SPECIES, lowerIntervals, i);
            var lx = FloatVector.fromArray(FLOAT_SPECIES, upperIntervals, i).sub(ax);
            var targetComponentSumsVec = IntVector.fromArray(INT_SPECIES, targetComponentSums, i).convert(VectorOperators.I2F, 0);
            var additionalCorrectionsVec = FloatVector.fromArray(FLOAT_SPECIES, additionalCorrections, i);
            ...
            var res2 = lx.mul(ay).mul(targetComponentSumsVec);
            ...
            if (similarityFunction == EUCLIDEAN) {
                res = res.mul(-2).add(additionalCorrectionsVec).add(queryAdditionalCorrection).add(1f);
                ...
                res = res.add(queryAdditionalCorrection).add(additionalCorrectionsVec).sub(centroidDp);

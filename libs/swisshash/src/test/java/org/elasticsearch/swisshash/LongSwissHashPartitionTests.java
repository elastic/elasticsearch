...
public class LongSwissHashPartitionTests extends PartitionedHashTestCase {
    ...
    public void testPartition() {
        ...
        var hash2 = new LongSwissHash(recycler, breaker);
        ...
        try {
            ...
            for (int i = 0; i < numBlocks; i++) {
                ...
                final long[] keys = new long[positions];
                boolean collisions = randomBoolean();
                for (int v = 0; v < positions; v++) {
                    keys[v] = collisions ? randomLongBetween(-100, 100) : randomLong();
                }
    ...
    static Set<KeyAndSum> emit(LongSwissHash hash, SumAgg agg) {
        Set<KeyAndSum> results = new HashSet<>(hash.size);
        for (int id = 0; id < hash.size; id++) {
            results.add(new KeyAndSum(hash.get(id), 0, agg.sums[id]));
        }
        return results;
    }
}

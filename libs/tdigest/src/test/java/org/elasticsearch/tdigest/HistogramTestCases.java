...
public abstract class HistogramTestCases extends ESTestCase {
    ...
    void doLinear(double idealChi2, double chi2SD, int binCount) throws IOException {
        int n = 10000;
        int trials = 1000;
        ...
        int n1below = 0;
        int n2below = 0;
        try (PrintWriter out = new PrintWriter("data.csv", StandardCharsets.UTF_8)) {
            ...
            for (int j = 0; j < trials; j++) {
                ...
                while (i < cuts.length - 1 && cuts[i] < 1.1) {
                    ...
                    if (counts[i] > 0) {
                        ...
                        if (i > 0) {
                            assertTrue(String.format(Locale.ROOT, "%d: %.5f > %.5f", i, cuts[i], min[i]), cuts[i] <= min[i]);
                        }
                        assertTrue(String.format(Locale.ROOT, "%d: %.5f > %.5f", i, max[i], cuts[i + 1]), cuts[i + 1] >= max[i]);
                    }
                    out.printf(Locale.ROOT, "%d,%.4f,%.4f,%.4f,%d,%.4f,%.1f\n", j, cuts[i], lowBound, highBound, counts[i], expected, err);
     ...
     */
    public void testFitToLog() throws IOException {
        double scale = Math.pow(2, 52);
        double x = 0.001;
        // 4 bits, worst case is mid octave
        double lowerBound = 1 / 16.0 * Math.sqrt(2);
        try (PrintWriter out = new PrintWriter("log-fit.csv", StandardCharsets.UTF_8)) {
            out.printf(Locale.ROOT, "x,y1,y2\n");
            while (x < 10) {
                ...
                out.printf(Locale.ROOT, "%.6f,%.6f,%.6f\n", x, v1, v2);
                assertTrue(v2 - v1 > 0);

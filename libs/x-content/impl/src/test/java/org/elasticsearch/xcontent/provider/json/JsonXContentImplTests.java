...
public class JsonXContentImplTests extends ESTestCase {
     ...
     * disabled (Jackson 2.x default), surrogate pairs are written as JSON Unicode escape sequences
     * rather than as a single 4-byte UTF-8 sequence.
     */
    public void testSupplementaryCharacterByteEncoding() throws IOException {
        ...
        // Current behavior: surrogates written as JSON Unicode escape sequences (ASCII)
        assertThat(latin1Output, containsString("\\uD83C\\uDFB5"));
        // U+1F3B5 -> F0 9F 8E B5 (proper 4-byte UTF-8, not yet used)
        String utf8 = new String(new byte[] { (byte) 0xF0, (byte) 0x9F, (byte) 0x8E, (byte) 0xB5 }, StandardCharsets.ISO_8859_1);
        assertThat(latin1Output, not(containsString(utf8)));
    }
}

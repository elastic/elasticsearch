...
public class APMTracer extends AbstractLifecycleComponent implements org.elasticsearch.telemetry.tracing.Tracer {
    ...
    private final Map<String, String> spanIdXref = ConcurrentCollections.newConcurrentMap();
    ...
    public void startTrace(TraceContext traceContext, Traceable traceable, String spanName, @Nullable Map<String, Object> attributes) {
        ...
        spans.computeIfAbsent(spanId, _spanId -> {
            ...
            // "spanId" coming from traceable.getSpanId() is not the same as the spanId internally generated by OTel when span is created
            // for a reverse lookup, we need to store this association
            spanIdXref.put(span.getSpanContext().getSpanId(), spanId);
    ...
    public void stopTrace(Traceable traceable) {
        ...
        if (span != null) {
            ...
            span.end();
            spanIdXref.remove(span.getSpanContext().getSpanId());
        }
    }

    @Override
    public void stopTrace(TraceContext traceContext, Traceable traceable) {
        ((ThreadContext) traceContext).clearTraceContext();
        stopTrace(traceable);
    }

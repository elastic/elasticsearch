...
public class GaugeAdapterTests extends ESTestCase {
    ...
    public void testLongGaugeRecord() throws Exception {
        ...
        LongGauge gauge = registry.registerLongGauge("es.name", "desc", "unit", attrs::get);
    ...
    public void testDoubleGaugeRecord() throws Exception {
        ...
        DoubleGauge gauge = registry.registerDoubleGauge("es.name", "desc", "unit", attrs::get);
    ...
    public void testNullGaugeRecord() throws Exception {
        DoubleGauge dgauge = registry.registerDoubleGauge("es.name", "desc", "unit", new AtomicReference<DoubleWithAttributes>()::get);
        otelMeter.collectMetrics();
        List<Measurement> metrics = otelMeter.getRecorder().getMeasurements(dgauge);
        assertThat(metrics, hasSize(0));

        LongGauge lgauge = registry.registerLongGauge("es.name", "desc", "unit", new AtomicReference<LongWithAttributes>()::get);
        otelMeter.collectMetrics();

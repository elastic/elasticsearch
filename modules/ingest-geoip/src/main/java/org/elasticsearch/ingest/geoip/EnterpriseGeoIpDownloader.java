...
public class EnterpriseGeoIpDownloader extends AllocatedPersistentTask {
    ...
    private void runPeriodic() {
        ...
        // If we are not able to acquire the semaphore, it means that a run is already in progress. Periodic runs do not run concurrently,
        // but a cluster state run could be in progress. Since the default poll interval is 3d, there is no need to wait for the current run
        // to finish and then run again, so we just skip this run and schedule the next one.
    ...
    private void runOnState() {
        ...
        // Wait for the current run (if any) to finish.
     ...
     * If queue was non-empty before we set it, then a run is already scheduled or in progress, so it will either be processed in the next
     * run, or the current run will automatically start a new run when it finishes because the cluster state queue changed while it was
     * running. This method does nothing if this task is cancelled or completed.
     */

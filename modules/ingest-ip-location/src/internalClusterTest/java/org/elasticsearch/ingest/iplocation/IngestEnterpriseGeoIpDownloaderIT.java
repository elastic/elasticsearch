...
/**
 * Integration tests verifying that enterprise-downloaded databases are usable through ingest pipelines.
...
public class IngestEnterpriseGeoIpDownloaderIT extends ESIntegTestCase {
    ...
    public void testEnterpriseDownloaderWithIngestPipeline() throws Exception {
        /*
         * This test starts the enterprise geoip downloader task, creates database configurations, then creates ingest
         * pipelines that reference those databases. It ingests real documents using those pipelines and asserts that the
         * documents were enriched with information from the enterprise databases.
         */
        IpLocationTestHelper.setEnterpriseEndpoints(getEndpoint());
    ...
    private void waitAround() throws Exception {
        ensureGreen(IpLocationTestHelper.DATABASES_INDEX);
        assertBusy(() -> {
            SearchResponse searchResponse = client().search(new SearchRequest(IpLocationTestHelper.DATABASES_INDEX)).actionGet();
            try {

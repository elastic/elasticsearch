...
import static org.elasticsearch.script.Script.CONTENT_TYPE_OPTION;
...
public class CustomMustacheFactoryTests extends ESTestCase {
    ...
    public void testJsonEscapeEncoder() {
        ...
        final Map<String, String> params = randomBoolean() ? singletonMap(CONTENT_TYPE_OPTION, JSON_MIME_TYPE) : emptyMap();

        Mustache script = (Mustache) engine.compile(null, "{\"field\": \"{{value}}\"}", params);
        CompiledScript compiled = new CompiledScript(ScriptType.INLINE, Script.DEFAULT_SCRIPT_NAME, engine, script);
    ...
    public void testDefaultEncoder() {
        final ScriptEngineService engine = new MustacheScriptEngineService(Settings.EMPTY);
        final Map<String, String> params = singletonMap(CONTENT_TYPE_OPTION, PLAIN_TEXT_MIME_TYPE);
    ...
    public void testUrlEncoder() {
        final ScriptEngineService engine = new MustacheScriptEngineService(Settings.EMPTY);
        final Map<String, String> params = singletonMap(CONTENT_TYPE_OPTION, X_WWW_FORM_URLENCODED_MIME_TYPE);

 ...
 * when tracking is enabled (a positive limit, metrics, or both) — under metrics alone there is no breach path, but the
 * estimator and def-concat helpers below are still emitted. Recording each execution's total is not done here; that call
 * goes straight to {@link AllocationMetrics#recordExecutionAllocation(String, long)} on the script's own metrics field.
 */

...
public final class CompilerSettings {
     ...
     * The sentinel {@code -1b} (default) disables enforcement; any positive size fails a script that exceeds it.
     * {@link Property#NodeScope} only: the limit is baked into generated bytecode, so a dynamic update would need a full
     * compile-cache flush. No upper bound, since a sensible limit scales with heap and an oversized one just never trips.
     */
     ...
     * disables warning; any positive size logs a {@code WARN} when an execution's running total first crosses it, and never
     * fails the script. Independent of {@link #MAX_ALLOCATION_BYTES}, so setting it alone observes without enforcing. When
     * both are on the limit is this threshold's ceiling; see {@link #resolveWarnAllocationBytes}.
     */
    ...
    /**
     * Returns the effective warning threshold, clamped down to the enforcement limit when it sits above one. A higher warning
     * threshold is dead config — the limit fails the script first — so clamping keeps the node starting and leaves the warning
     * on the allocation that trips the limit, carrying the script name and source the limit failure lacks.
     *
     * @param contextName script context, for the affix keys in the log message
     * @param limitBytes enforcement limit, or {@code -1} when off
     * @param warnBytes configured warning threshold, or {@code -1} when off
     * @param warnExplicitlySet whether it was configured rather than defaulted; decides only the log level
    ...
    /** Script context being compiled, baked into the allocation checks so a breach can name and count itself per context. */
    ...
    /** Whether either threshold is positive. Gates all tracking bytecode: warning alone is enough. */

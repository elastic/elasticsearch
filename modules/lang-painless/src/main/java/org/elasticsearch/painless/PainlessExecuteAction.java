        ...
        private static Response prepareRamIndex(Request request,
            ...
            try (RAMDirectory ramDirectory = new RAMDirectory()) {
                try (IndexWriter indexWriter = new IndexWriter(ramDirectory, new IndexWriterConfig(defaultAnalyzer))) {
                    ...
                    assert indexService.mapperService().types().size() == 1;
                    String type = indexService.mapperService().types().iterator().next();
                    ...
                    ParsedDocument parsedDocument = indexService.mapperService().documentMapper(type).parse(sourceToParse);

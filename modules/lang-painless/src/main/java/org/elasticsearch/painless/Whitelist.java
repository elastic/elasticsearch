 ...
 *
 * A Whitelist consists of two pieces.  The first is a {@link String} {@link #pDynamicTypeName}
 * that represent the singularly allowed name of the dynamic type in Painless.  Should this value be
 * {@code null} then only static typing will be enabled.  The second is a list of {@link WStruct}s
...
public final class Whitelist {
    ...
    public static Whitelist loadFromResourceFiles(Map<Class<?>, List<String>> resourcesToFiles) {
                ...
                try (LineNumberReader reader = new LineNumberReader(
                    ...
                    while ((line = reader.readLine()) != null) {
                        ...
                        if (line.startsWith("dynamic ")) {
                            ...
                            // Ensure there aren't multiple dynamic types.
                            if (pDynamicTypeName != null) {
                                throw new IllegalArgumentException("invalid dynamic definition: multiple dynamic types defined" +
                                    "[" + pDynamicTypeName + "] and [" + tokens[1] + "]");
                            }

                            pDynamicTypeName = tokens[1];
                        ...
                        // Expects the following format: 'class' ID -> ID ( 'extends' ID ( ',' ID )* )? '{' '\n'
                            ...
                            if (pTypeName.equals(pDynamicTypeName)) {
                                throw new IllegalArgumentException(
        ...
        for (Entry<Class<?>, List<String>> resourceToFiles : resourcesToFiles.entrySet()) {
            ...
            Function<String, Class<?>> getJClassFromPTypeName = pTypeName -> {
                ...
                Class<?> jClass =
                    pNonArrayTypeName.equals(lambdaPDynamicTypeName) ? Object.class : pTypeNamesToJClasses.get(pNonArrayTypeName);
    ...
    /** The singularly allowed dynamic type name to be used in Painless scripts. */
    final String pDynamicTypeName;

    /** The {@link Collection} of all the white-listed Painless structs. */
    final Collection<WStruct> wStructs;

    /** Standard constructor.  If pDynamicTypeName is {@code null} then Painless can only use static types. */
    private Whitelist(String pDynamicTypeName, Collection<WStruct> wStructs) {
        this.pDynamicTypeName = pDynamicTypeName;

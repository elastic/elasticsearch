...
public class MemberFieldStoreNode extends UnaryNode {
    ...
    public void write(ClassWriter classWriter, MethodWriter methodWriter, Globals globals, ScopeTable scopeTable) {
        ...
        if (isStatic) {
            methodWriter.putStatic(CLASS_TYPE, name, MethodWriter.getType(getExpressionType()));
        } else {
            methodWriter.putField(CLASS_TYPE, name, MethodWriter.getType(getExpressionType()));
        }
    }
}

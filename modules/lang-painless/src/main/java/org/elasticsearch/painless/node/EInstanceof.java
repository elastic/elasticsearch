...
public final class EInstanceof extends AExpression {
    ...
    void analyze(Locals locals) {
        ...
        try {
            clazz = locals.getPainlessLookup().painlessTypeNameToPainlessType(this.type);
        } catch (IllegalArgumentException exception) {
            throw createError(new IllegalArgumentException("Not a type [" + this.type + "]."));
        }

        // map to wrapped type for primitive types
        resolvedType = clazz.isPrimitive() ?
            PainlessLookupUtility.getBoxedAnyType(clazz) : PainlessLookupUtility.painlessDefTypeToJavaObjectType(clazz);

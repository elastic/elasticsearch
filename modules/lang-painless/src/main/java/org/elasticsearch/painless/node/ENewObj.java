...
public final class ENewObj extends AExpression {
    ...
    void analyze(Locals locals) {
        try {
            actual = locals.getPainlessLookup().painlessTypeNameToPainlessType(this.type);
        } catch (IllegalArgumentException exception) {
            throw createError(new IllegalArgumentException("Not a type [" + this.type + "]."));
        }

        PainlessClass struct = locals.getPainlessLookup().getPainlessClassFromJavaClass(actual);

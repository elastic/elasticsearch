        ...
        if (prefix.actual.isArray()) {
            ...
            if (field != null) {
                ...
                PainlessMethod getter = struct.methods.get(
                    PainlessLookupUtility.buildPainlessMethodKey("get" + Character.toUpperCase(value.charAt(0)) + value.substring(1), 0));

                if (getter == null) {
                    getter = struct.methods.get(PainlessLookupUtility
                        .buildPainlessMethodKey("is" + Character.toUpperCase(value.charAt(0)) + value.substring(1), 0));
                }

                PainlessMethod setter = struct.methods.get(
                    PainlessLookupUtility.buildPainlessMethodKey("set" + Character.toUpperCase(value.charAt(0)) + value.substring(1), 1));

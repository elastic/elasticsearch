...
public final class SFunction extends AStatement {
    ...
    void generateSignature(PainlessLookup painlessLookup) {
        try {
            rtnType = painlessLookup.painlessTypeNameToPainlessType(rtnTypeStr);
        ...
        for (int param = 0; param < this.paramTypeStrs.size(); ++param) {
            try {
                Class<?> paramType = painlessLookup.painlessTypeNameToPainlessType(this.paramTypeStrs.get(param));
        ...
        org.objectweb.asm.commons.Method method = new org.objectweb.asm.commons.Method(name,
            MethodType.methodType(PainlessLookupUtility.painlessDefTypeToJavaObjectType(rtnType), paramClasses).toMethodDescriptorString());

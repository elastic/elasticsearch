...
final class SSubEachIterable extends AStatement {
        ...
        if (expression.actual == def.class) {
            ...
            if (method == null) {
                throw createError(new IllegalArgumentException("Unable to create iterator for the type " +
                        "[" + PainlessLookupUtility.painlessTypeToPainlessTypeName(expression.actual) + "]."));
            }
        }
    ...
    public String toString() {
        return singleLineToString(PainlessLookupUtility.painlessTypeToPainlessTypeName(variable.clazz), variable.name, expression, block);
    }
}

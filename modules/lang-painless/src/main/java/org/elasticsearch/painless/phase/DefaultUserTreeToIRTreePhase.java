...
public class DefaultUserTreeToIRTreePhase implements UserTreeVisitor<ScriptScope> {
    ...
    protected void injectBootstrapMethod(ScriptScope scriptScope) {
        ...
        try {
            ...
            irFunctionNode.attachDecoration(
                new IRDTypeParameters(Arrays.asList(Lookup.class, String.class, MethodType.class, int.class, int.class, Object[].class))
            );
            irFunctionNode.attachDecoration(
                new IRDParameterNames(Arrays.asList("methodHandlesLookup", "name", "type", "initialDepth", "flavor", "args"))
            );
    ...
    public void visitCall(ECall userCallNode, ScriptScope scriptScope) {
        ...
        if (prefixValueType != null && prefixValueType.getValueType() == def.class) {
            ...
            Class<?> scriptTypeToInject = scriptScope.getPainlessLookup()
                .typeOfScriptToInjectForMethod(userCallNode.getMethodName(), userCallNode.getArgumentNodes().size());
            if (scriptTypeToInject != null) {
                irCallSubDefNode.attachCondition(IRCInjectedScript.class);
                LoadScriptNode loadScript = new LoadScriptNode(userCallNode.getLocation());
                loadScript.attachDecoration(new IRDExpressionType(scriptTypeToInject));
                irCallSubDefNode.getArgumentNodes().add(0, loadScript);
            }
            ...
            InjectScriptAnnotation injectScript = (InjectScriptAnnotation) method.annotations.get(InjectScriptAnnotation.class); 
            if (injectScript != null) {
                ...
                Class<?> scriptTypeToInject = scriptScope.getPainlessLookup()
                    .typeOfScriptToInjectForMethod(userCallNode.getMethodName(), userCallNode.getArgumentNodes().size());
                loadScript.attachDecoration(new IRDExpressionType(scriptTypeToInject));  
                irInvokeCallNode.addArgumentNode(loadScript);
            }

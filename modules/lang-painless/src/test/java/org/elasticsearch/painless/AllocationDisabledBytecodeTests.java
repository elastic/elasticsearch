...
public class AllocationDisabledBytecodeTests extends ScriptTestCase {
    ...
    public void testWarnThresholdAloneEmitsTracking() {
        // The warning threshold alone enables tracking, with the same emitted shape as any other configuration.
        ...
        assertThat(asm, containsString("checkAllocation"));
    }

    public void testEmittedShapeDoesNotVaryWithWhichThresholdsAreOn() {
        // Which thresholds apply is decided inside AllocationGuard.checkAllocation from the constants passed to it, so the
        // generated method is identical for warning-only, enforcement-only, and both; only the pushed constants differ.
        String warnOnly = bytecode("int[] a = new int[] {1, 2, 3}; return 1;", -1L, 1024L);
        String limitOnly = bytecode("int[] a = new int[] {1, 2, 3}; return 1;", 1024L, -1L);
        String both = bytecode("int[] a = new int[] {1, 2, 3}; return 1;", 2048L, 1024L);

        for (String asm : List.of(warnOnly, limitOnly, both)) {
            assertThat(asm, containsString("$allocBytes"));
            assertThat(asm, containsString("$allocWarned"));
            assertThat(asm, containsString("checkAllocation"));
            // The breach bodies are reached from inside the guard, never called from generated bytecode.
    ...
    public void testPreCheckDelegatesTheDecisionToAllocationGuard() {
        // The generated $checkAllocBytes charges the total and hands the numbers to AllocationGuard, which decides.
        String asm = bytecode("int[] a = new int[] {1, 2, 3}; return 1;", 1024 * 1024L);

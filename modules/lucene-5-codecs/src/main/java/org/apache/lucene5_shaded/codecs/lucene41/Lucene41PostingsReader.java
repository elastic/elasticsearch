...
public final class Lucene41PostingsReader extends PostingsReaderBase {
  ...
  public void init(IndexInput termsIn, SegmentReadState state) throws IOException {
    ...
    if (indexBlockSize != Lucene41PostingsFormat.BLOCK_SIZE) {
      throw new IllegalStateException("index-time BLOCK_SIZE (" + indexBlockSize + ") != read-time BLOCK_SIZE (" + Lucene41PostingsFormat.BLOCK_SIZE + ")");
    }
  }
    ...
    public int advance(int target) throws IOException {
      ...
      if (docFreq > Lucene41PostingsFormat.BLOCK_SIZE && target > nextSkipDoc) {
        ...
        if (newDocUpto > docUpto) {
          ...
          // }
          assert newDocUpto % Lucene41PostingsFormat.BLOCK_SIZE == 0 : "got " + newDocUpto;
          docUpto = newDocUpto;
  ...
  final class BlockPostingsEnum extends PostingsEnum {
    ...
    public PostingsEnum reset(IntBlockTermState termState) throws IOException {
      ...
      accum = 0;
      docUpto = 0;
      if (docFreq > Lucene41PostingsFormat.BLOCK_SIZE) {
        nextSkipDoc = Lucene41PostingsFormat.BLOCK_SIZE - 1; // we won't skip if target is found in first block
      } else {
    ...
    private void refillPositions() throws IOException {
      ...
      if (posIn.getFilePointer() == lastPosBlockFP) {
        ...
        // }
        final int count = (int) (totalTermFreq % Lucene41PostingsFormat.BLOCK_SIZE);
        int payloadLength = 0;
    ...
    private void skipPositions() throws IOException {
      ...
      final int leftInBlock = Lucene41PostingsFormat.BLOCK_SIZE - posBufferUpto;

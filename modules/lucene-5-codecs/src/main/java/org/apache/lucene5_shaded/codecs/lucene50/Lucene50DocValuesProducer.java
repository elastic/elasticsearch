...
class Lucene50DocValuesProducer extends DocValuesProducer implements Closeable {
  ...
  private int readFields(IndexInput meta, FieldInfos infos) throws IOException {
    ...
    while (fieldNumber != -1) {
      ...
      if (type == Lucene50DocValuesFormat.NUMERIC) {
        ...
        if (ss.format == Lucene50DocValuesFormat.SORTED_WITH_ADDRESSES) {
          readSortedSetFieldWithAddresses(info, meta);
        } else if (ss.format == Lucene50DocValuesFormat.SORTED_SET_TABLE) {
          readSortedSetFieldWithTable(info, meta);
        } else if (ss.format == Lucene50DocValuesFormat.SORTED_SINGLE_VALUED) {
  ...
  private NumericEntry readNumericEntry(IndexInput meta) throws IOException {
    ...
    switch(entry.format) {
      ...
      case Lucene50DocValuesFormat.DELTA_COMPRESSED:
        ...
        entry.bitsPerValue = meta.readVInt();
        break;
      case Lucene50DocValuesFormat.MONOTONIC_COMPRESSED:
  ...
  static BinaryEntry readBinaryEntry(IndexInput meta) throws IOException {
    ...
    entry.offset = meta.readLong();
    switch(entry.format) {
      case Lucene50DocValuesFormat.BINARY_FIXED_UNCOMPRESSED:
        break;
      case Lucene50DocValuesFormat.BINARY_PREFIX_COMPRESSED:
        entry.addressesOffset = meta.readLong();
        entry.packedIntsVersion = meta.readVInt();
        entry.blockSize = meta.readVInt();
        entry.reverseIndexOffset = meta.readLong();
        break;
      case Lucene50DocValuesFormat.BINARY_VARIABLE_UNCOMPRESSED:
        entry.addressesOffset = meta.readLong();
        entry.packedIntsVersion = meta.readVInt();
        entry.blockSize = meta.readVInt();
        break;
  ...
  SortedSetEntry readSortedSetEntry(IndexInput meta) throws IOException {
    ...
    entry.format = meta.readVInt();
    if (entry.format == Lucene50DocValuesFormat.SORTED_SET_TABLE) {
    ...
    } else if (entry.format != Lucene50DocValuesFormat.SORTED_SINGLE_VALUED && entry.format != Lucene50DocValuesFormat.SORTED_WITH_ADDRESSES) {
      throw new CorruptIndexException("Unknown format: " + entry.format, meta);
    }
    return entry;
  }
  ...
  private synchronized MonotonicBlockPackedReader getIntervalInstance(FieldInfo field, BinaryEntry bytes) throws IOException {
    ...
    if (addresses == null) {
      data.seek(bytes.addressesOffset);
      final long size = (bytes.count + Lucene50DocValuesFormat.INTERVAL_MASK) >>> Lucene50DocValuesFormat.INTERVAL_SHIFT;
  ...
  private synchronized ReverseTermsIndex getReverseIndexInstance(FieldInfo field, BinaryEntry bytes) throws IOException {
    ...
    if (index == null) {
      ...
      long size = (bytes.count + Lucene50DocValuesFormat.REVERSE_INTERVAL_MASK) >>> Lucene50DocValuesFormat.REVERSE_INTERVAL_SHIFT;
  ...
  public SortedNumericDocValues getSortedNumeric(FieldInfo field) throws IOException {
    ...
    if (ss.format == Lucene50DocValuesFormat.SORTED_SINGLE_VALUED) {
      NumericEntry numericEntry = numerics.get(field.name);
      final LongValues values = getNumeric(numericEntry);
      final Bits docsWithField = getLiveBits(numericEntry.missingOffset, maxDoc);
      return DocValues.singleton(values, docsWithField);
    } else if (ss.format == Lucene50DocValuesFormat.SORTED_WITH_ADDRESSES) {
      NumericEntry numericEntry = numerics.get(field.name);
      final LongValues values = getNumeric(numericEntry);
  ...
  public SortedSetDocValues getSortedSet(FieldInfo field) throws IOException {
    ...
    switch (ss.format) {
      case Lucene50DocValuesFormat.SORTED_SINGLE_VALUED:
        final SortedDocValues values = getSorted(field);
        return DocValues.singleton(values);
      case Lucene50DocValuesFormat.SORTED_WITH_ADDRESSES:
        return getSortedSetWithAddresses(field);
      case Lucene50DocValuesFormat.SORTED_SET_TABLE:
  ...
  private Bits getLiveBits(final long offset, final int count) throws IOException {
    if (offset == Lucene50DocValuesFormat.ALL_MISSING) {
      return new Bits.MatchNoBits(count);
    } else if (offset == Lucene50DocValuesFormat.ALL_LIVE) {
  ...
  static final class CompressedBinaryDocValues extends LongBinaryDocValues {    
    ...
    class CompressedBinaryTermsEnum extends TermsEnum {
      ...
      private final int offsets[] = new int[Lucene50DocValuesFormat.INTERVAL_COUNT];
      private final byte buffer[] = new byte[2* Lucene50DocValuesFormat.INTERVAL_COUNT-1];
      ...
      private void readHeader() throws IOException {
        ...
        input.readBytes(buffer, 0, Lucene50DocValuesFormat.INTERVAL_COUNT-1);
        if (buffer[0] == -1) {
          readShortAddresses();
        } else {
          readByteAddresses();
      ...
      private void readShortAddresses() throws IOException {
        input.readBytes(buffer, Lucene50DocValuesFormat.INTERVAL_COUNT-1, Lucene50DocValuesFormat.INTERVAL_COUNT);
        int addr = 0;
        for (int i = 1; i < offsets.length; i++) {
          int x = i<<1;
      ...
      public BytesRef next() throws IOException {
        ...
        if (currentOrd >= numValues) {
          ...
          int offset = (int) (currentOrd & Lucene50DocValuesFormat.INTERVAL_MASK);
          if (offset == 0) {
      ...
      public SeekStatus seekCeil(BytesRef text) throws IOException {
        ...
        if (indexPos < 0) {
          ...
          long low = indexPos << Lucene50DocValuesFormat.BLOCK_INTERVAL_SHIFT;
          long high = Math.min(numIndexValues - 1, low + Lucene50DocValuesFormat.BLOCK_INTERVAL_MASK);
          block = Math.max(low, binarySearchBlock(text, low, high));
        }
        
        // position before block, then scan to term.
        input.seek(addresses.get(block));
        currentOrd = (block << Lucene50DocValuesFormat.INTERVAL_SHIFT) - 1;
      ...
      public void seekExact(long ord) throws IOException {
        long block = ord >>> Lucene50DocValuesFormat.INTERVAL_SHIFT;
        if (block != currentOrd >>> Lucene50DocValuesFormat.INTERVAL_SHIFT) {
        ...
        int offset = (int) (ord & Lucene50DocValuesFormat.INTERVAL_MASK);
        if (offset == 0) {
          readFirstTerm();
        } else {

...
public class QueryAnalyzerTests extends ESTestCase {
    ...
    public void testExtractQueryMetadata_matchNoDocsQuery() {
        Result result = analyze(Queries.NO_DOCS_INSTANCE);
        assertThat(result.verified, is(true));
        ...
        bq.add(Queries.NO_DOCS_INSTANCE, BooleanClause.Occur.MUST);
        ...
        bq.add(Queries.NO_DOCS_INSTANCE, BooleanClause.Occur.SHOULD);
        result = analyze(bq.build());
        assertThat(result.verified, is(true));
        assertThat(result.minimumShouldMatch, equalTo(1));
        ...
        DisjunctionMaxQuery disjunctionMaxQuery = new DisjunctionMaxQuery(
            Arrays.asList(new TermQuery(new Term("field", "value")), Queries.NO_DOCS_INSTANCE),
            1f
        );

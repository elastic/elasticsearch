...
/// Pins the ownership contract that keeps a cancelled reindex from freeing memory out from under its own destination write.
///
...
/// dispatch loop and fails the top-level bulk listener while earlier shard requests are still writing. Reindex aliases pooled
/// search hit bytes into its index requests and releases them when that listener completes, so the bulk path has to hold its own
/// reference on those bytes for as long as it is still reading them.
///
...
public class ReindexCancelInFlightWriteIT extends ESIntegTestCase {
    ...
    public void testDestinationWriteOutlivesCancelledBulk() throws Exception {
        ...
        // destination write is still parked, and reindex releases the hit bytes it aliased into that write on its way out
        final Throwable reindexFailure;
        try {
            reindexFailure = ExceptionsHelper.unwrapCause(expectThrows(Exception.class, () -> reindexFuture.actionGet(SAFE_AWAIT_TIMEOUT)));
        } finally {

...
public class ReindexNodeShutdownIT extends ESIntegTestCase {
    ...
    private void createReindexTaskAndShutdown(final String coordNodeName) throws Exception {
        AbstractBulkBySearchRequestBuilder<?, ?> builder = reindex(coordNodeName).source(INDEX).destination(DEST_INDEX);
        AbstractBulkBySearchRequest<?> reindexRequest = builder.request();

...
class DefaultReindexRelocationNodePicker implements ReindexRelocationNodePicker {
    ...
    public String pickNode(DiscoveryNodes nodes) {
        ...
        if (currentNodeId == null) {
            logger.debug(
                "Trying to pick a node to relocate a reindex task to, but the current node ID is unexpectedly unknown. Skipping relocation."
            );
            return null;
        }
        ...
        if (eligibleDedicatedCoordinatingNodes.isEmpty() == false) {
            ...
            logger.debug(
                "Relocating a reindex task from current node ID {} to a randomly chosen dedicated coordinating node ID {}",
                currentNodeId,
                newNodeId
            );
            return newNodeId;
        ...
        if (eligibleDataNodes.isEmpty() == false) {
            ...
            logger.debug(
                "Relocating a reindex task from current node ID {} to a randomly chosen data ID {}"
        ...
        logger.debug(
            "Trying to pick a node to relocate a reindex task to, but there are no dedicated coordinating or data nodes"
                + " (perhaps excluding the current node). Skipping relocation."
        );
        return null;
    }

...
public class Reindexer {
    ...
    static class AsyncIndexBySearchAction extends AbstractAsyncBulkByScrollAction<ReindexRequest, TransportReindexAction> {
        ...
        AsyncIndexBySearchAction(
            ...
            Settings indexSettings = getIndexSettings(state);
            boolean autoGeneratedId = IndexSettings.TIME_SERIES_USE_AUTO_GENERATED_ID.get(indexSettings);
            this.destinationIndexIdMapper = IndexSettings.MODE.get(indexSettings).idFieldMapperWithoutFieldData(autoGeneratedId);
        }

        private Settings getIndexSettings(ClusterState state) {
            IndexMetadata destMeta = state.metadata().index(mainRequest.getDestination().index());
            if (destMeta != null) {
                return destMeta.getSettings();
            }

...
public class ReindexBasicTests extends ReindexTestCase {
    ...
    public void testCopyManyWithAutoSlicing() {
        ...
        int expectedStatuses = expectedSliceStatuses(AbstractBulkBySearchRequest.AUTO_SLICES, source);

        ReindexRequestBuilder copy = reindex().source(source)
            .destination(dest)
            .refresh(true)
            .setSlices(AbstractBulkBySearchRequest.AUTO_SLICES);
        copy.source().setSize(5);
        assertThat(copy.get(), matcher().created(max).batches(greaterThanOrEqualTo(max / 5)).slices(hasSize(expectedStatuses)));
        assertHitCount(prepareSearch(dest).setSize(0), max);

        int half = max / 2;
        copy = reindex().source(source).destination(destHalf).refresh(true).setSlices(AbstractBulkBySearchRequest.AUTO_SLICES);
        copy.source().setSize(5);
        copy.maxDocs(half);
        BulkByScrollResponse response = copy.get();
    ...
    public void testMultipleSourcesWithAutoSlicing() {
        ...
        int expectedStatuses = expectedSliceStatuses(AbstractBulkBySearchRequest.AUTO_SLICES, docs.keySet());

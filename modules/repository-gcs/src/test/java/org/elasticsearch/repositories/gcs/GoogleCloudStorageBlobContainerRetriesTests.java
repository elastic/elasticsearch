...
public class GoogleCloudStorageBlobContainerRetriesTests extends AbstractBlobContainerRetriesTestCase {
    ...
    public void testReadLargeBlobWithRetries() throws Exception {
        ...
        try (InputStream inputStream = blobContainer.readBlob(BlobPurpose.SNAPSHOT, "large_blob_retries")) {
    ...
    public void testWriteLargeBlob() throws IOException {
        ...
        if (randomBoolean()) {
            try (InputStream stream = new InputStreamIndexInput(new ByteArrayIndexInput("desc", data), data.length)) {
                blobContainer.writeBlob(BlobPurpose.SNAPSHOT, "write_large_blob", stream, data.length, false);
            }
        } else {
            blobContainer.writeMetadataBlob(BlobPurpose.SNAPSHOT, "write_large_blob", false, randomBoolean(), out -> out.write(data));
        }
    ...
    public void testDeleteBatchesAreSentIncrementally() throws Exception {
        ...
        blobContainer.deleteBlobsIgnoringIfNotExists(BlobPurpose.SNAPSHOT, blobNamesIterator);

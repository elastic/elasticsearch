...
public class Netty4HttpOptionsMethodTests extends AbstractHttpServerTransportTestCase {
    ...
    public void testRequestBodyFixedLengthMultipleChunks() throws Exception {
        ...
        try (Netty4HttpServerTransport transport = getHttpServerTransport(dispatchedRequestReference)) {
            ...
            for (String httpMethod : List.of("OPTIONS", randomFrom("PUT", "POST", "GET", "DELETE"))) {
                ...
                String[] contentParts = randomArray(1, 5, String[]::new, () -> randomAlphaOfLengthBetween(2, 20));
                // having a "Content-Length" request header is what makes it "fixed length"
                final int totalContentLength = Arrays.stream(contentParts).map(String::length).reduce(0, Integer::sum);
                ByteBufUtil.copy(AsciiString.of("Content-Length: " + totalContentLength), buf);
                buf.writeByte(HttpConstants.LF);
                // end of headers
                buf.writeByte(HttpConstants.LF);
                // write the headers
    ...
    public void testRequestVariableLengthChunkedTransferEncoding() throws Exception {
        ...
        try (Netty4HttpServerTransport transport = getHttpServerTransport(dispatchedRequestReference)) {
            ...
            for (String httpMethod : List.of("OPTIONS", randomFrom("PUT", "POST", "GET", "DELETE"))) {
                ByteBuf buf = ch.alloc().buffer();
                ByteBufUtil.copy(AsciiString.of(httpMethod + " /url/whatever/chunked-transfer?encoding HTTP/1.1"), buf);
                buf.writeByte(HttpConstants.LF);
                boolean hasHostHeader = randomBoolean();
                if (hasHostHeader) {
                    ByteBufUtil.copy(AsciiString.of("Host: localhost"), buf);
                ...
                // transfer-encoding besides "chunked" should be ignored for OPTIONS but it trips the test scenario for others
                if (randomBoolean() && httpMethod.equals("OPTIONS")) {
                ...
                int totalContentLength = Arrays.stream(contentParts).map(String::length).reduce(0, Integer::sum);
                ...
                totalContentLength += Arrays.stream(contentParts).map(String::length).reduce(0, Integer::sum);

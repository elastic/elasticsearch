...
public class Netty5UtilsTests extends ESTestCase {
    ...
    public void testToChannelBufferWithEmptyRef() throws IOException {
        ByteBuf buffer = Netty5Utils.toByteBuf(getRandomizedBytesReference(0));
    ...
    public void testToChannelBufferWithSlice() throws IOException {
        ...
        ByteBuf buffer = Netty5Utils.toByteBuf(slice);
        BytesReference bytesReference = Netty5Utils.toBytesReference(buffer);
    ...
    public void testToChannelBufferWithSliceAfter() throws IOException {
        ...
        int sliceOffset = randomIntBetween(0, ref.length());
        int sliceLength = randomIntBetween(ref.length() - sliceOffset, ref.length() - sliceOffset);
        ByteBuf buffer = Netty5Utils.toByteBuf(ref);
        BytesReference bytesReference = Netty5Utils.toBytesReference(buffer);
        assertArrayEquals(
    ...
    private BytesReference getRandomizedBytesReference(int length) throws IOException {
        ...
        if (randomBoolean()) {
            ...
            return Netty5Utils.toBytesReference(Unpooled.wrappedBuffer(bytesRef.bytes, bytesRef.offset, bytesRef.length));
        } else {
            return ref;
        }
    }

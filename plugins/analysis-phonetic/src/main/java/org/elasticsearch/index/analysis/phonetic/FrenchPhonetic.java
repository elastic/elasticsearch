...
public class FrenchPhonetic implements StringEncoder {
    ...
    public String encode(String s) throws EncoderException {
        ...
        String result = operatePhonetic("", cleanedString.charAt(0), cleanedString.substring(1, cleanedString.length()));
    ...
    private String operatePhonetic(String acc, Character c, String tail) {
        ...
        if (tail == null || tail.isEmpty()) {
            ...
            if (c == 'C' && (tail.charAt(0) == 'E' || tail.charAt(0) == 'I' || tail.charAt(0) == 'Y')) {
                return operatePhonetic(acc + 'S', tail.charAt(0), substring(tail, false, 1, tail.length()));
            } else {
                //C as K
            ...
            if (c == 'G' && (tail.charAt(0) == 'E' || tail.charAt(0) == 'I' || tail.charAt(0) == 'Y')) {
                return operatePhonetic(acc + 'J', tail.charAt(0), substring(tail, true, 1, tail.length()));
            }

            //S as Z
            if (c == 'S' && !acc.isEmpty() && VOWELS.contains(acc.charAt(acc.length() - 1)) && VOWELS.contains(tail.charAt(0))) {
                return operatePhonetic(acc + 'Z', tail.charAt(0), substring(tail, false, 1, tail.length()));
            }

            //W as V
            if (c == 'W') {
                return operatePhonetic(acc + 'V', tail.charAt(0), substring(tail, false, 1, tail.length()));
            }
            ...
            if (c == 'Q' && tail.charAt(0) == 'U') {
                return operatePhonetic(acc + 'K', charAt(tail, 1), substring(tail, true, 2, tail.length()));
            }

            //PH as F
            if (c == 'P' && tail.charAt(0) == 'H') {
                return operatePhonetic(acc + 'F', charAt(tail, 1), substring(tail, true, 2, tail.length()));
            }
            ...
            if (c == 'E' && tail.length() >= 2 && tail.charAt(0) == 'A' && tail.charAt(1) == 'U') {
                return operatePhonetic(acc + "O", charAt(tail, 2), substring(tail, true, 3, tail.length()));
            }
        ...
        if (c == 'E') {
            String nextAcc = operatePhonetic("", charAt(tail, 0), substring(tail, true, 1, tail.length()));
            if ("".equals(nextAcc)) {
                return acc;
            }
        ...
        //Y as I
        if (c == 'Y') {
            return operatePhonetic(acc + 'I', charAt(tail, 0), substring(tail, true, 1, tail.length()));
        }
    ...
    private String replaceTwoLettersSounds(String acc, char c, String tail) {
        ...
        if (c == 'A' && tail.charAt(0) == 'U') {
            return operatePhonetic(acc + 'O', charAt(tail, 1), substring(tail, true, 2, tail.length()));
        }
    ...
    private String replaceThreeLettersINSound(String acc, char c, String tail, Character... firstLetters) {
        if (Arrays.asList(firstLetters).contains(c) && tail.length() >= 2 &&
            ...
            return operatePhonetic(acc + "1", charAt(tail, 2), substring(tail, true, 3, tail.length()));
        }
        return null;
    }
        ...
        if (Arrays.asList(firstLetters).contains(c)) {
            if (tail.charAt(0) == 'N' || tail.charAt(0) == 'M') {
                if (tail.length() > 1 && (tail.charAt(1) == 'N' || tail.charAt(1) == 'M' || tail.charAt(1) == 'H' || VOWELS.contains(tail.charAt(1)))) {
                    return operatePhonetic(acc + c, tail.charAt(0), substring(tail, false, 1, tail.length()));
                }
                return operatePhonetic(acc + replaceValue, charAt(tail, 1), substring(tail, true, 2, tail.length()));
            }
        }
        return null;
    }

    protected String substring(String tail, boolean emptyList, int startIndex, int endIndex) {
    ...
    protected Character charAt(String tail, int position) {
        if (position < 0 || position >= tail.length()) {
            return null;
        }

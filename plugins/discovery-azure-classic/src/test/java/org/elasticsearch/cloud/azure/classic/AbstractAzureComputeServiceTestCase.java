        ...
        protected AzureComputeService createComputeService() {
            return () -> {
                ...
                for (Map.Entry<String, DiscoveryNode> node : nodes.entrySet()) {
                    ...
                    final InetAddress address = InetAddresses.forString(discoveryNode.getAddress().getAddress());
                    ...
                    endpoint.setVirtualIPAddress(address);
                    endpoint.setPort(discoveryNode.getAddress().getPort());
            ...
            return new AzureUnicastHostsProvider(settings, azureComputeService, transportService, networkService) {
                ...
                protected String resolveInstanceAddress(final HostType hostType, final RoleInstance instance) {
                    ...
                    if (discoveryNode != null) {
                        ...
                        if (address instanceof LocalTransportAddress) {
                            LocalTransportAddress local = (LocalTransportAddress) address;
                            return local.id();
                        } else if (address instanceof InetSocketTransportAddress) {
                            InetSocketTransportAddress inet = (InetSocketTransportAddress) address;
                            return String.join(":", inet.getAddress(), Integer.toString(inet.getPort()));
                        }
                    }

            ...
            threads[i] = new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        vars.put("x", x);
                        vars.put("y", y);
                        ExecutableScript script = se.executable(new CompiledScript(Script.ScriptType.INLINE, "testExecutableNoRuntimeParams", se, compiled), vars);
                ...
                public void run() {
                    try {
                        ...
                        for (int i = 0; i < between(100, 1000); i++) {
                            ...
                            runtimeVars.put("x", x);
                            runtimeVars.put("y", y);
                            long result = ((Number) se.executable(new CompiledScript(Script.ScriptType.INLINE, "testExecutableNoRuntimeParams", se, compiled), runtimeVars).run()).longValue();
                            assertThat(result, equalTo(addition));
                        }
                    } catch (Exception e) {
                        failed.set(true);

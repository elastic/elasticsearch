...
final class Compiler {
    ...
    static Executable compile(final Loader loader, final String name, final String source,
        ...
        // Generate the ANTLR tree from the given source.
        final ParserRuleContext root = createParseTree(source, definition);

        // Create the Metadata (wrapper) used for this script.
        final Metadata metadata = new Metadata(definition, source, root, settings);

        // Run the Analyzer to generate the metadata necessary for writing the byte code.
        Analyzer.analyze(metadata);

        // Run the Writer to generate the byte code using ASM.
        final byte[] bytes = Writer.write(metadata);

        // Create the executable from the given ClassLoader.
        final Executable executable = createExecutable(loader, definition, name, source, bytes);

        return executable;
    }

    // Generates the ANTLR tree from the given source code.  Several methods, as noted below, are used
    ...
    private static ParserRuleContext createParseTree(final String source, final Definition definition) {
        ...
        // Remove the default error listener, so the first exception will escape.
        lexer.removeErrorListeners();

        // Set the allowed types that can be used based on the specified Plan A definition.
        lexer.setTypes(definition.structs.keySet());

        // Remove the default error listener, so the first exception will escape.
        parser.removeErrorListeners();

        // Use a custom error strategy, so the first exception will escape.
    ...
    // Uses the given ClassLoader to generate the Executable script.

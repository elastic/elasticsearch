...
public class GoogleCloudStorageService extends AbstractComponent {
    ...
    public Storage createClient(final String clientName,
        ...
        HttpRequestInitializer requestInitializer =  createRequestInitializer(clientSettings, connectTimeout, readTimeout);
        ...
        } else if (Strings.hasLength(clientSettings.getApplicationName())) {
    ...
    static HttpRequestInitializer createRequestInitializer(final GoogleCloudStorageClientSettings settings,
                                                           final TimeValue deprecatedConnectTimeout,
                                                           final TimeValue deprecatedReadTimeout) throws IOException {
        GoogleCredential credential = settings.getCredential();
        if (credential == null) {
            credential = GoogleCredential.getApplicationDefault();
        }

        final Integer connectTimeout = (deprecatedConnectTimeout != null) ?
            toTimeout(deprecatedConnectTimeout) : toTimeout(settings.getConnectTimeout());

        final Integer readTimeout = (deprecatedReadTimeout != null) ?
            toTimeout(deprecatedReadTimeout) : toTimeout(settings.getReadTimeout());

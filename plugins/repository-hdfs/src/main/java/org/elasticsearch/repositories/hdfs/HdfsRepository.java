...
public final class HdfsRepository extends BlobStoreRepository {
    ...
    private HdfsBlobStore createBlobstore(URI uri, String path, Settings repositorySettings)  {
        ...
        for (Entry<String, String> entry : map.entrySet()) {
            LOGGER.debug("Adding configuration to HDFS Client Configuration : {} = {}", entry.getKey(), entry.getValue());
        ...
        try {
            return new HdfsBlobStore(fileContext, path, bufferSize, haEnabled);
        } catch (IOException e) {

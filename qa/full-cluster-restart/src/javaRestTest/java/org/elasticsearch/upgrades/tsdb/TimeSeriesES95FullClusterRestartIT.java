 ...
 * {@code index.time_series.es95_codec.enabled}, force-merge rollback to baseline,
 * multi-index searches across both codecs, and force-merge over mixed-codec segments
 * (within one index and across sibling indices). Per-segment format correctness is
 * covered by {@code ES95VsES819DocValuesDuelTests}.
 */
public class TimeSeriesES95FullClusterRestartIT extends ParameterizedFullClusterRestartTestCase {
    ...
    public void testForceMergeRollbackToBaseline() throws IOException {
        ...
        final String index = "tsdb-rollback";
        final int docCount = randomIntBetween(MIN_DOC_COUNT, MAX_DOC_COUNT);
        createTSDBIndex(index, true);
        bulkIndexTSDBDocuments(index, docCount);
        refreshIndex(index);

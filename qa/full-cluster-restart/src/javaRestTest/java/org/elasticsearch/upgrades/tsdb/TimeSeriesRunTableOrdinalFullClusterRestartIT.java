...
/**
 * Full cluster restart tests for TSDB indices using the ES95 run-table ordinal codec. Covers
 * pre-upgrade indices staying on the baseline codec across restart, opt-in and opt-out via
 * {@code index.time_series.run_table_ordinal.enabled} at index creation, the cluster-level
 * kill-switch {@code cluster.time_series.run_table_ordinal.enabled}, multi-index searches
 * across both codec variants, and force-merge of sibling indices.
 */
public class TimeSeriesRunTableOrdinalFullClusterRestartIT extends ParameterizedFullClusterRestartTestCase {
    ...
    public void testPreUpgradeIndexReadableAfterRestart() throws IOException {
        ...
        assertDimensionTermCounts(index, total);
    ...
    public void testNewIndexAfterRestartUsesRunTableOrdinalByDefault() throws IOException {
        requireRunTableOrdinal();
        if (isRunningAgainstOldCluster()) {
            return;
        }
        final String index = "rto-default-on";
    ...
    public void testExplicitOptInNewIndex() throws IOException {
        ...
        final String index = "rto-explicit-on";
        final int docCount = randomIntBetween(MIN_DOC_COUNT, MAX_DOC_COUNT);
    ...
    public void testExplicitOptOutNewIndex() throws IOException {
        requireRunTableOrdinal();
        if (isRunningAgainstOldCluster()) {
            return;
        }
        final String index = "rto-explicit-off";
        final int docCount = randomIntBetween(MIN_DOC_COUNT, MAX_DOC_COUNT);
    ...
    public void testFinalSettingRejectsLiveUpdate() throws IOException {
        ...
        final String index = "rto-final-guard";
        createTSDBIndex(index, null);
        bulkIndexTSDBDocuments(index, randomIntBetween(MIN_DOC_COUNT, MAX_DOC_COUNT));
        refreshIndex(index);
        ...
        final ResponseException exception = expectThrows(ResponseException.class, () -> {
            ...
            request.setJsonEntity("{\"index.time_series.run_table_ordinal.enabled\":false}");
    ...
    public void testQueryAcrossOldAndNewCodecIndices() throws IOException {
        ...
        final long preRestartCount = getDocumentCount(preIndex);

        final int postRestartCount = randomIntBetween(MIN_DOC_COUNT, MAX_DOC_COUNT);
        createTSDBIndex(postIndex, Boolean.TRUE);
        bulkIndexTSDBDocuments(postIndex, postRestartCount, (int) preRestartCount);
        ...
        final long expectedTotal = preRestartCount + postRestartCount;
    ...
    public void testClusterKillSwitchDisablesRunTableOnNewIndices() throws IOException {
        requireRunTableOrdinal();
        if (isRunningAgainstOldCluster()) {
            return;
        }
        ...
        try {
            final String index = "rto-kill-switch";
            createTSDBIndex(index, null);
            bulkIndexTSDBDocuments(index, randomIntBetween(MIN_DOC_COUNT, MAX_DOC_COUNT));
            refreshIndex(index);
            ...
            assertIndexHealthGreen(index);
            assertThat(getFlatIndexSettings(index).get("index.time_series.run_table_ordinal.enabled"), Matchers.equalTo("false"));
            assertDocumentCount(index, getDocumentCount(index));
        } finally {
    ...
    public void testRandomAccessDocValuesAfterRestart() throws IOException {
        ...
        assertIndexHealthGreen(preIndex);
        final long preRestartCount = getDocumentCount(preIndex);
        assertTopHitsDocValuesByTimestampDesc(preIndex, preRestartCount - 1, Math.min(5, (int) preRestartCount));
        ...
        final long combinedTotal = preRestartCount + postRestartCount;
        assertTopHitsDocValuesByTimestampDesc(postIndex, combinedTotal - 1, Math.min(5, postRestartCount));

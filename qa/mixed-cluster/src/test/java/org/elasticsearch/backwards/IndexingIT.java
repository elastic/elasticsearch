...
public class IndexingIT extends ESRestTestCase {
    ...
    /**
     * Previously, we use a TransportHandler to send/receive the snapshot state messages.
     * This handler does not retry if there is failure. We decided to replace it by TransportMasterNodeAction which does retry if needed.
     *
     * This test makes sure that these two approaches are compatible to one another.
     * In particular, the master node should receive and handle the snapshot state messages from old nodes properly.
     */
    public void testUpdateSnapshotStates() throws Exception {
        Nodes nodes = buildNodeAndVersions();

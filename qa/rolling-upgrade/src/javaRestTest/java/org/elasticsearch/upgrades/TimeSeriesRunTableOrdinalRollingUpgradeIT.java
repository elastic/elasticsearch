...
/**
 * Rolling upgrade tests for TSDB indices using the ES95 run-table ordinal codec. Covers
 * pre-upgrade indices staying writable and queryable across every node-by-node upgrade step,
 * explicit opt-in and opt-out on a fully upgraded cluster, the cluster-level kill-switch, and
 * force-merge preserving the original codec on pre-upgrade indices via the IndexVersion gate.
 */
public class TimeSeriesRunTableOrdinalRollingUpgradeIT extends AbstractRollingUpgradeTestCase {
    ...
    public TimeSeriesRunTableOrdinalRollingUpgradeIT(@Name("upgradedNodes") int upgradedNodes) {
        super(upgradedNodes);
    }
    ...
    public void testPreUpgradeIndexSurvivesAllPhases() throws IOException {
        ...
        appendDocsAndAssertSurvives(index);
        assertThat(getFlatIndexSettings(index).get("index.time_series.run_table_ordinal.enabled"), Matchers.nullValue());
    }

    public void testNewRunTableOrdinalIndexAfterFullUpgrade() throws IOException {
        requireRunTableOrdinal();
        if (isOldCluster() || isMixedCluster()) {
            return;
        }
        final String index = "rto-rolling-default-on";
        ...
        assertDimensionTermCounts(index, docCount);
    ...
    public void testExplicitOptInAfterFullUpgrade() throws IOException {
        ...
        final String index = "rto-rolling-opt-in";
        final int docCount = randomIntBetween(MIN_DOC_COUNT, MAX_DOC_COUNT);
        createTSDBIndex(index, Boolean.TRUE);
        bulkIndexTSDBDocuments(index, docCount);
        refreshIndex(index);
    ...
    public void testExplicitOptOutAfterFullUpgrade() throws IOException {
        requireRunTableOrdinal();
        if (isOldCluster() || isMixedCluster()) {
            return;
        }
        final String index = "rto-rolling-opt-out";
        final int docCount = randomIntBetween(MIN_DOC_COUNT, MAX_DOC_COUNT);
        createTSDBIndex(index, Boolean.FALSE);
        bulkIndexTSDBDocuments(index, docCount);
        refreshIndex(index);
        ...
        assertDimensionTermCounts(index, docCount);
        assertGaugeSumMatches(index, expectedGaugeSum(docCount));
        assertThat(getFlatIndexSettings(index).get("index.time_series.run_table_ordinal.enabled"), Matchers.equalTo("false"));
    }
    ...
    public void testPreUpgradeIndexForceMergeAfterRollingUpgrade() throws IOException {
        ...
        if (isMixedCluster()) {
            ...
            assertDimensionTermCounts(index, preCount);
            return;
        }
        assertIndexHealthGreen(index);
        final long preCount = getDocumentCount(index);
    ...
    public void testQueryAcrossOldAndNewCodecIndices() throws IOException {
        ...
        if (isMixedCluster()) {
            ...
            assertDimensionTermCounts(preIndex, preCount);
            return;
        }
        assertIndexHealthGreen(preIndex);
        ...
        createTSDBIndex(postIndex, Boolean.TRUE);
        ...
        assertThat(getFlatIndexSettings(preIndex).get("index.time_series.run_table_ordinal.enabled"), Matchers.nullValue());
        assertThat(getFlatIndexSettings(postIndex).get("index.time_series.run_table_ordinal.enabled"), Matchers.equalTo("true"));
        ...
        assertDimensionTermCounts(target, expectedTotal);
        ...
        assertSortByTimestampDescendingOrdered(target);
    ...
    public void testClusterKillSwitchDisablesRunTableOnUpgradedCluster() throws IOException {
        requireRunTableOrdinal();
        if (isOldCluster() || isMixedCluster()) {
            return;
        }
        final Request disableRequest = new Request("PUT", "/_cluster/settings");
        disableRequest.setJsonEntity("{\"persistent\":{\"cluster.time_series.run_table_ordinal.enabled\":false}}");
        assertOK(client().performRequest(disableRequest));
        try {
            final String index = "rto-rolling-kill-switch";
            createTSDBIndex(index, null);
            ...
            assertIndexHealthGreen(index);
            assertThat(getFlatIndexSettings(index).get("index.time_series.run_table_ordinal.enabled"), Matchers.equalTo("false"));
            final long docCount = getDocumentCount(index);
            assertRangeQueryOnTimestampMatchesAll(index, docCount);
            assertDimensionTermCounts(index, docCount);
        } finally {
            final Request restoreRequest = new Request("PUT", "/_cluster/settings");
            restoreRequest.setJsonEntity("{\"persistent\":{\"cluster.time_series.run_table_ordinal.enabled\":null}}");
            assertOK(client().performRequest(restoreRequest));
        }
    }

    private void createTSDBIndex(final String indexName, Boolean runTableOrdinalEnabled) throws IOException {
        final String extraSetting = runTableOrdinalEnabled == null
            ? ""
            : ",\"index.time_series.run_table_ordinal.enabled\":" + runTableOrdinalEnabled;
    ...
    private void bulkIndexTSDBDocuments(final String indexName, int docCount, int startIndex) throws IOException {
        ...
        for (int i = 0; i < docCount; i++) {
            ...
            final String region = (absoluteIndex % 2 == 0) ? REGION_A : REGION_B;
    ...
    @SuppressWarnings("unchecked")
    private long termQueryByFieldCount(final String indexName, final String field, final String value) throws IOException {
        final Request request = new Request("GET", "/" + indexName + "/_search");
        request.setJsonEntity("""
            {
              "size": 0,
    ...
    private void assertDimensionTermCounts(final String indexName, long totalDocs) throws IOException {
        final long expectedAlpha = (totalDocs + 1) / 2;
        final long expectedBeta = totalDocs / 2;
        assertThat(termQueryByFieldCount(indexName, "hostname", HOST_A), Matchers.equalTo(expectedAlpha));
        assertThat(termQueryByFieldCount(indexName, "hostname", HOST_B), Matchers.equalTo(expectedBeta));
        assertThat(termQueryByFieldCount(indexName, "region", REGION_A), Matchers.equalTo(expectedAlpha));
        assertThat(termQueryByFieldCount(indexName, "region", REGION_B), Matchers.equalTo(expectedBeta));
    }
    ...
    @SuppressWarnings("unchecked")
    private void assertSortByTimestampDescendingOrdered(final String indexName) throws IOException {
        final Request request = new Request("GET", "/" + indexName + "/_search");
        request.setJsonEntity("""
            {
              "size": 5,
    ...
    private void assertFirstDocByTimestampAsc(final String indexName) throws IOException {
        ...
        assertThat(source.get("region"), Matchers.equalTo(REGION_A));
    ...
    private void appendDocsAndAssertSurvives(final String index) throws IOException {
        ...
        assertDimensionTermCounts(index, expectedTotal);
    ...
    private void assertTopHitsDocValuesByTimestampDesc(final String indexName, long topExpectedGauge, int topN) throws IOException {
        ...
        request.setJsonEntity("""
            {
              ...
              "_source": false,
              "sort": [{ "@timestamp": "desc" }],
              "docvalue_fields": ["gauge", "hostname", "region"]
            }""".formatted(topN));
        final Map<String, Object> response = entityAsMap(client().performRequest(request));
        final Map<String, Object> hits = (Map<String, Object>) response.get("hits");
        final List<Map<String, Object>> hitList = (List<Map<String, Object>>) hits.get("hits");
        ...
        for (int i = 0; i < topN; i++) {
            ...
            assertThat("hostname at top-" + i, (String) fields.get("hostname").getFirst(), Matchers.equalTo(expectedHostname));
            final String expectedRegion = (expectedGauge % 2 == 0) ? REGION_A : REGION_B;
            assertThat("region at top-" + i, (String) fields.get("region").getFirst(), Matchers.equalTo(expectedRegion));
        }
    }

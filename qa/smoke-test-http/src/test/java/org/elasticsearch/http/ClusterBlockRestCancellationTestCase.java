...
/**
 * Base class for testing that cancellation works at the REST layer for request that use a TransportMasterNodeAction
 *
 * It injects a global cluster block preventing the TransportMasterNodeAction to make progress until the request task
 * is cancelled to test cancellation propagation.
 */
public abstract class ClusterBlockRestCancellationTestCase extends HttpSmokeTestCase {
    void runTest(String requestEndpoint, String actionName) throws Exception {
        ...
        final Request request = new Request(HttpGet.METHOD_NAME, requestEndpoint);
    ...
    private void transformClusterState(Function<ClusterState, ClusterState> transformationFn) {
        ...
        internalCluster().getMasterNodeInstance(ClusterService.class).submitStateUpdateTask("get_mappings_test",

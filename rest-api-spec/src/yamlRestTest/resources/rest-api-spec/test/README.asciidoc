...
Cluster features indicate a particular top-level functionality and are used for _internal_ checks and coordination
within Elasticsearch to enable functionality once available on all nodes, e.g. usage of a new transport endpoint.

Other than <<capabilities_check, capabilities>>, cluster features are strictly internal, though can also be used for

...
- `cluster_features` to <<cluster_features, skip if cluster features are present in any node>>.
- `all_nodes` to <<cluster_features, change semantic of cluster_features from any node to all nodes">>.
...
The semantics of skip cluster_feature is such that if _any_ node in the cluster has the feature it will skip. This semantics
can be switched to "all nodes" with `all_nodes: true` (section will be skipped only if all nodes in the cluster has the feature).

....
    "Parent":
     - skip:
          cluster_features: new_behavior_of_existing_feature
          all_nodes: true
          reason:           Change to feature X breaks this test
...
Asserts two lists contain the same elements in any order. Each element is compared using
the same rules as `match`. e.g:

...
matches_in_any_order: { tags: ["b", "a"] }
...

Alternatively, with a list of objects:

...
matches_in_any_order: { hits.hits: [{ _id: "2" }, { _id: "1" }] }
...
...
Compares two lists and asserts they contain the same elements in any order. Each element
is compared using the same rules as `match` (scalars, regex strings, maps, nested lists,
etc.). Duplicates are taken into account, eg:
...
**Note: you should use a requires prerequisite along with matches_in_any_order, as not all
runners support it:**

....
    - requires:
        test_runner_features: matches_in_any_order
....

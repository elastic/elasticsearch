...
public class RolloverIT extends ESIntegTestCase {
    ...
    public void testRolloverOnEmptyIndex() throws Exception {
        ...
        final IndexMetadata oldIndex = state.metadata().projectMetadata.index("test_index-1");
        if (explicitWriteIndex) {
        ...
        final IndexMetadata newIndex = state.metadata().projectMetadata.index("test_index-000002");
    ...
    public void testRollover() throws Exception {
        ...
        assertThat(response.getConditionStatus().size(), equalTo(0));
        final ClusterState state = clusterAdmin().prepareState().get().getState();
        final IndexMetadata oldIndex = state.metadata().projectMetadata.index("test_index-2");
        assertFalse(oldIndex.getAliases().containsKey("test_alias"));
        final IndexMetadata newIndex = state.metadata().projectMetadata.index("test_index-000003");
        assertTrue(newIndex.getAliases().containsKey("test_alias"));
    ...
    public void testRolloverConditionsNotMet() throws Exception {
        ...
        final ClusterState state = clusterAdmin().prepareState().get().getState();
        final IndexMetadata oldIndex = state.metadata().projectMetadata.index("test_index-0");
        assertTrue(oldIndex.getAliases().containsKey("test_alias"));
        if (explicitWriteIndex) {
        ...
        final IndexMetadata newIndex = state.metadata().projectMetadata.index("test_index-000001");
        assertNull(newIndex);
    }
    ...
    public void testRolloverWithNewIndexName() throws Exception {
        ...
        assertThat(response.getConditionStatus().size(), equalTo(0));
        final ClusterState state = clusterAdmin().prepareState().get().getState();
        final IndexMetadata oldIndex = state.metadata().projectMetadata.index("test_index");
        final IndexMetadata newIndex = state.metadata().projectMetadata.index("test_new_index");
        assertTrue(newIndex.getAliases().containsKey("test_alias"));
        if (explicitWriteIndex) {
            assertFalse(oldIndex.getAliases().get("test_alias").writeIndex());
            assertTrue(newIndex.getAliases().get("test_alias").writeIndex());
    ...
    public void testRolloverMaxSize() throws Exception {
        ...
        // A large max_size
        {
            ...
            final IndexMetadata oldIndex = clusterAdmin().prepareState().get().getState().metadata().projectMetadata.index("test-1");
        ...
        // An empty index
        {
            ...
            final IndexMetadata oldIndex = clusterAdmin().prepareState().get().getState().metadata().projectMetadata.index("test-000002");
            assertThat(oldIndex.getRolloverInfos().size(), equalTo(0));
        }
    }
    ...
    public void testRolloverWithHiddenAliasesAndExplicitWriteIndex() {
        ...
        assertThat(response.getConditionStatus().size(), equalTo(0));
        final ClusterState state = clusterAdmin().prepareState().get().getState();
        final IndexMetadata oldIndex = state.metadata().projectMetadata.index(firstIndexName);
        ...
        final IndexMetadata newIndex = state.metadata().projectMetadata.index(secondIndexName);

    ...
    public void testRolloverConcurrently() throws Exception {
            ...
            threads[i] = new Thread(() -> {
                ...
                for (int j = 1; j <= numberOfRolloversPerThread; j++) {
                    ...
                    var response = client().admin()
                        ...
                        .get();
                    assertThat(response.getOldIndex(), equalTo(aliasName + org.elasticsearch.core.Strings.format("-%06d", j)));
                    assertThat(response.getNewIndex(), equalTo(aliasName + org.elasticsearch.core.Strings.format("-%06d", j + 1)));
                    assertThat(response.isDryRun(), equalTo(false));
                    assertThat(response.isRolledOver(), equalTo(true));
                }
            });
        ...
        for (int i = 0; i < numOfThreads; i++) {
            ...
            for (int j = 1; j <= numOfIndices; j++) {
                ...
                expected.add(Map.entry(aliasName + org.elasticsearch.core.Strings.format("-%06d", j), List.of(amBuilder.build())));
            }

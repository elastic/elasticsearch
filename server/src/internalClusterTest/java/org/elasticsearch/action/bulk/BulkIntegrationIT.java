...
public class BulkIntegrationIT extends ESIntegTestCase {
    ...
    public void testBulkWithWriteIndexAndRouting() {
        ...
        try (BulkRequestBuilder bulkBuilder = client().prepareBulk()) {
            BulkResponse bulkResponse = bulkBuilder.add(indexRequestWithAlias).get();
            indexRequestWithAlias.decRef();
        ...
        try (BulkRequestBuilder bulkBuilder = client().prepareBulk()) {
            BulkResponse bulkResponse = bulkBuilder.add(client().prepareDelete("alias1", "id")).get();
            assertFalse(bulkResponse.buildFailureMessage(), bulkResponse.hasFailures());
    ...
    public void testExternallySetAutoGeneratedTimestamp() {
        ...
        assertThat(expectThrows(IllegalArgumentException.class, () -> {
            try (BulkRequestBuilder bulkBuilder = client().prepareBulk()) {
                bulkBuilder.add(indexRequest).get();
            } finally {
                indexRequest.decRef();
            }
        }).getMessage(), containsString("autoGeneratedTimestamp should not be set externally"));
    }
    ...
    public void testBulkWithGlobalDefaults() throws Exception {
        ...
        try (BulkRequestBuilder bulkBuilder = client().prepareBulk("test").routing("routing").pipeline("pipeline")) {

...
public class RareClusterStateIT extends ESIntegTestCase {
    ...
    public void testAssignmentWithJustAddedNodes() {
        ...
        clusterService.submitUnbatchedStateUpdateTask("test-inject-node-and-reroute", new ClusterStateUpdateTask() {
            ...
            public ClusterState execute(ClusterState currentState) {
                ...
                // open index
                final IndexMetadata indexMetadata = IndexMetadata.builder(currentState.metadata().projectMetadata.index(index))
                ...
                routingTable.addAsRecovery(updatedState.metadata().projectMetadata.index(index));
    ...
    public void testDeleteCreateInOneBulk() throws Exception {
        ...
        final var originalIndexUuid = masterClusterService.state().metadata().projectMetadata.index("test").getIndexUUID();
        final var uuidChangedListener = ClusterServiceUtils.addTemporaryStateListener(
            dataNodeClusterService,
            clusterState -> originalIndexUuid.equals(clusterState.metadata().projectMetadata.index("test").getIndexUUID()) == false
    ...
    public void testDelayedMappingPropagationOnPrimary() throws Exception {
        ...
        try {
            ...
            {
                MappingMetadata typeMappings = internalCluster().clusterService(master).state().metadata().projectMetadata.index("index")
                    .mapping();
    ...
    public void testDelayedMappingPropagationOnReplica() throws Exception {
        ...
        final var primaryIndexService = internalCluster().getInstance(IndicesService.class, primaryNode)
            .indexServiceSafe(state.metadata().projectMetadata.index("index").getIndex());

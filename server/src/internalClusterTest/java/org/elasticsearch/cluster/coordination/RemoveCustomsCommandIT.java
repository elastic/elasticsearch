...
public class RemoveCustomsCommandIT extends ESIntegTestCase {
    ...
    public void testRemoveCustomsSuccessful() throws Exception {
        ...
        assertEquals(1, clusterAdmin().prepareState().get().getState().metadata().projectMetadata.indexGraveyard().getTombstones().size());
        Settings dataPathSettings = internalCluster().dataPathSettings(node);
        ensureStableCluster(1);
        internalCluster().stopRandomDataNode();
        ...
        assertEquals(0, clusterAdmin().prepareState().get().getState().metadata().projectMetadata.indexGraveyard().getTombstones().size());
    }

...
public class StableMasterHealthIndicatorServiceIT extends ESIntegTestCase {
    ...
    public void testGetHealthWhenMasterIsElected() throws Exception {
        ...
        var stableMasterIndicator = response.findComponent("cluster_coordination").findIndicator("stable_master");
        assertNotNull(stableMasterIndicator);
        assertThat(stableMasterIndicator.status(), equalTo(GREEN));
        assertThat(stableMasterIndicator.summary(), equalTo("The cluster has had a stable master node"));
    }
    ...
    public void testGetHealthWhenNoMaster() throws Exception {
        ...
        try {
            assertBusy(() -> {
                ...
                var stableMasterIndicator = response.findComponent("cluster_coordination").findIndicator("stable_master");
                assertNotNull(stableMasterIndicator);
                assertThat(stableMasterIndicator.status(), equalTo(RED));
                assertThat(stableMasterIndicator.summary(), equalTo("No master"));
            });
        } finally {

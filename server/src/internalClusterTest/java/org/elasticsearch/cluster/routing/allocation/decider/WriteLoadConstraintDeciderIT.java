...
public class WriteLoadConstraintDeciderIT extends ESIntegTestCase {
    ...
    public void testHighNodeWriteLoadPreventsNewShardAllocation() {
        ...
        int randomNumberOfShards = randomIntBetween(15, 40); // Pick a high number of shards, so it is clear movement is not accidental.
        createIndex(
            indexName,
            Settings.builder()
        ...
        MockTransportService.getInstance(firstDataNodeName)
            .addRequestHandlingBehavior(TransportNodeUsageStatsForThreadPoolsAction.NAME + "[n]", (handler, request, channel, task) -> {
                channel.sendResponse(new NodeUsageStatsForThreadPoolsAction.NodeResponse(firstDiscoveryNode, nonHotSpottingNodeStats));
            });
        MockTransportService.getInstance(secondDataNodeName)
            .addRequestHandlingBehavior(TransportNodeUsageStatsForThreadPoolsAction.NAME + "[n]", (handler, request, channel, task) -> {
                channel.sendResponse(new NodeUsageStatsForThreadPoolsAction.NodeResponse(secondDiscoveryNode, nonHotSpottingNodeStats));
            });
        MockTransportService.getInstance(thirdDataNodeName)
            .addRequestHandlingBehavior(TransportNodeUsageStatsForThreadPoolsAction.NAME + "[n]", (handler, request, channel, task) -> {
                channel.sendResponse(new NodeUsageStatsForThreadPoolsAction.NodeResponse(thirdDiscoveryNode, hotSpottingNodeStats));
            });
        ...
        ClusterInfo info = ClusterInfoServiceUtils.refresh(clusterInfoService);
        ...
        // Start a reroute to pick up the hot-spotting ClusterInfo and act upon it with the new filter and write load statas.
        ClusterRerouteUtils.reroute(client());

        // NOMERGE: add dummy shard write load stats. Wrap the channel to modify the result? Mocking the whole stats would be hard.

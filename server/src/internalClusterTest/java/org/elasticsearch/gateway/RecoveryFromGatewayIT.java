...
public class RecoveryFromGatewayIT extends ESIntegTestCase {
    ...
    public void testSingleNodeWithFlush() throws Exception {
        internalCluster().startNode();
        prepareIndex("test").setId("1").setSource(jsonBuilder().startObject().field("field", "value1").endObject()).execute().actionGet();
        flush();
    ...
    public void testLatestVersionLoaded() throws Exception {
        ...
        prepareIndex("test").setId("3").setSource(jsonBuilder().startObject().field("field", "value3").endObject()).execute().actionGet();

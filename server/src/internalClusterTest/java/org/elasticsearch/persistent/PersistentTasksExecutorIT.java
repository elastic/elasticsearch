...
public class PersistentTasksExecutorIT extends ESIntegTestCase {
    ...
    public void testPersistentActionFailure() throws Exception {
        ...
        assertThat(
            new TestTasksRequestBuilder(client()).setOperation("fail").setTaskId(firstRunningTask.taskId()).get().getTasks().size(),
            equalTo(1)
        );
    ...
    public void testPersistentActionStatusUpdate() throws Exception {
        ...
        for (int i = 0; i < numberOfUpdates; i++) {
            ...
            assertThat(
                new TestTasksRequestBuilder(client()).setOperation("update_status")
                    .setTaskId(firstRunningTask.taskId())
                    .get()
                    .getTasks()
                    .size(),
                equalTo(1)
        ...
        // Complete the running task and make sure it finishes properly
        assertThat(
            new TestTasksRequestBuilder(client()).setOperation("finish").setTaskId(firstRunningTask.taskId()).get().getTasks().size(),
            equalTo(1)
        );

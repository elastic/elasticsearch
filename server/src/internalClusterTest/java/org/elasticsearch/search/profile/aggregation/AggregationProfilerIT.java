...
public class AggregationProfilerIT extends ESIntegTestCase {
    ...
    public void testSimpleProfile() {
        assertNoFailuresAndResponse(
            ...
            response -> {
                Map<String, SearchProfileShardResult> profileResults = response.getProfileShardResults();
    ...
    public void testMultiLevelProfile() {
        assertNoFailuresAndResponse(
            prepareSearch("idx").setProfile(true)
                .addAggregation(
            ...
            response -> {
                ...
                for (SearchProfileShardResult profileShardResult : profileResults.values()) {
                    ...
                    assertMap(
                        ...
                        matchesMap().entry(BUILT_BUCKETS, greaterThan(0)).entry(DEFERRED, List.of("max"))
                    );
    ...
    public void testNoProfile() {
        assertNoFailuresAndResponse(
            prepareSearch("idx").setProfile(false)
                .addAggregation(
                    histogram("histo").field(NUMBER_FIELD)
                        .interval(1L)
                        .subAggregation(

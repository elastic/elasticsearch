...
public class QueryProfilerIT extends ESIntegTestCase {
    ...
    public void testProfileQuery() throws Exception {
        ...
        for (int i = 0; i < iters; i++) {
            ...
            assertResponse(
                ...
                response -> {
                    assertNotNull("Profile response element should not be null", response.getProfileShardResults());
                    if (response.getSkippedShards() == response.getSuccessfulShards()) {
                        assertEquals(0, response.getProfileShardResults().size());
                    } else {
                        assertThat("Profile response should not be an empty array", response.getProfileShardResults().size(), not(0));
                    }
                    for (Map.Entry<String, SearchProfileShardResult> shard : response.getProfileShardResults().entrySet()) {
    ...
    public void testSimpleMatch() throws Exception {
        ...
        assertResponse(prepareSearch().setQuery(q).setProfile(true).setSearchType(SearchType.QUERY_THEN_FETCH), response -> {
            Map<String, SearchProfileShardResult> p = response.getProfileShardResults();
            assertNotNull(p);
            assertThat("Profile response should not be an empty array", response.getProfileShardResults().size(), not(0));

            for (Map.Entry<String, SearchProfileShardResult> shardResult : response.getProfileShardResults().entrySet()) {
    ...
    public void testNoProfile() throws Exception {
        ...
        assertResponse(
            ...
            response -> assertThat("Profile response element should be an empty map", response.getProfileShardResults().size(), equalTo(0))
        );
    }
}

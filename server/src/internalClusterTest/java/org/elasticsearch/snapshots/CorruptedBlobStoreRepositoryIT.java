...
public class CorruptedBlobStoreRepositoryIT extends AbstractSnapshotIntegTestCase {
    ...
    public void testFailDeleteOnMissingShardGeneration() throws Exception {
        Path repo = randomRepoPath();
        final String repoName = "test-repo";
        ...
        final List<String> snapshotNames = createNSnapshots(repoName, 2);
        ...
        expectThrows(RepositoryException.class,
                () -> client().admin().cluster().prepareDeleteSnapshot(repoName, snapshotNames.get(0)).execute().actionGet());

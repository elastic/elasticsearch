          ...
          public ScorerSupplier scorerSupplier(LeafReaderContext context) throws IOException {
              ...
              if (values.getNumIndexDimensions() != 1) {
                  throw new IllegalArgumentException(
                      "field=\"" + field + "\" was indexed with numIndexDimensions=" + values.getNumIndexDimensions()
                          + " but this query has numDims=" + 1
                  );
              }
              if (Long.BYTES != values.getBytesPerDimension()) {
                  throw new IllegalArgumentException(
                      "field=\"" + field + "\" was indexed with bytesPerDim=" + values.getBytesPerDimension()
                          + " but this query has bytesPerDim=" + Long.BYTES
                  );
              }

...
public abstract class Condition<T> implements NamedWriteable, ToXContentFragment, Accountable {
    ...
    /**
     * Estimated heap footprint of this condition. {@code type} is an enum (shared singleton, covered by the shallow instance size) and
     * {@code name} is a per-condition constant. All concrete subclasses only set the inherited {@code value} and declare no extra fields,
     * so the shallow size of the concrete instance plus the name and boxed/value cost is a complete accounting.
     */
    @Override

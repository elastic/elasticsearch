...
public final class BulkRequestParser {
    ...
    private static final ParseField DYNAMIC_TEMPLATE_HINTS = new ParseField("dynamic_template_hints");
    ...
    public void parse(
        ...
        while (true) {
            ...
            try (XContentParser parser = createParser(data, xContent, from, nextMarker)) {
                ...
                Map<String, String> dynamicTemplateHints = Map.of();
                ...
                if (token == XContentParser.Token.START_OBJECT) {
                    ...
                    while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
                        if (token == XContentParser.Token.FIELD_NAME) {
                            ...
                            DYNAMIC_TEMPLATE_HINTS.match(currentFieldName, parser.getDeprecationHandler())) {
                            dynamicTemplateHints = parser.mapStrings();
                ...
                if ("delete".equals(action)) {
                    if (dynamicTemplateHints.isEmpty() == false) {
                        throw new IllegalArgumentException(
                            "Delete request in line [" + line + "] does not accept " + DYNAMIC_TEMPLATE_HINTS.getPreferredName());
                    }
                    ...
                    if ("index".equals(action)) {
                        if (opType == null) {
                            indexRequestConsumer.accept(new IndexRequest(index).id(id).routing(routing)
                                    ...
                                    .setDynamicTemplateHints(dynamicTemplateHints)
                        ...
                        if (dynamicTemplateHints.isEmpty() == false) {
                            throw new IllegalArgumentException(
                                "Update request in line [" + line + "] does not accept " + DYNAMIC_TEMPLATE_HINTS.getPreferredName());
                        }

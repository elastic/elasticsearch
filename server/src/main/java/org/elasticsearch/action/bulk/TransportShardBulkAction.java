...
public class TransportShardBulkAction extends TransportWriteAction<BulkShardRequest, BulkShardRequest, BulkShardResponse> {
    ...
    static boolean executeBulkItemRequest(BulkPrimaryExecutionContext context, UpdateHelper updateHelper, LongSupplier nowInMillisSupplier,
        ...
        if (isDelete) {
            ...
            IndexSettings indexSettings = context.getPrimary().indexSettings();
            if (indexSettings != null) {
                int maxDocIdLen = context.getPrimary().indexSettings().getMaxDocIdLength();
                if (id != null &&
                    id.getBytes(StandardCharsets.UTF_8).length > maxDocIdLen) {
                    ActionRequestValidationException validationException = new ActionRequestValidationException();
                    validationException.addValidationError("id is too long, must be no longer than "
                        + maxDocIdLen + " bytes but was: " +
                        id.getBytes(StandardCharsets.UTF_8).length);
                    result = new Engine.IndexResult(validationException, request.version());
                    context.markOperationAsExecuted(result);
                    context.markAsCompleted(context.getExecutionResult());
                    return true;
                }
            ...
            result = primary.applyIndexOperationOnPrimary(version, request.versionType(), new SourceToParse(
                    request.index(), id, request.source(), request.getContentType(), request.routing()),

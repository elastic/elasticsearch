...
public class TransportGetFromTranslog extends HandledTransportAction<GetRequest, TransportGetFromTranslog.Response> {

    public static final String NAME = "internal:data/read/get_from_translog";
    public static final ActionType<Response> TYPE = new ActionType<>(NAME, Response::new);

    private final IndicesService indicesService;

    @Inject
    public TransportGetFromTranslog(TransportService transportService, IndicesService indicesService, ActionFilters actionFilters) {
    ...
    protected void doExecute(Task task, GetRequest request, ActionListener<Response> listener) {
        if (request.realtime() == false) {
            listener.onResponse(new Response(null, -1L));
            return;
        }
        ...
        var engineGet = new Engine.Get(request.realtime(), request.realtime(), request.id()).version(request.version())
            .versionType(request.versionType());
        var isInVersionMap = internalEngine.isInVersionMap(engineGet);
        ActionListener.completeWith(listener, () -> {
            long segmentGeneration = internalEngine.getLastCommittedSegmentInfos().getGeneration();
            if (isInVersionMap == false) {
                return new Response(null, segmentGeneration);
            }
            return new Response(
                indexShard.getService()
                    .get(
                        request.id(),
    ...
    public static class Response extends ActionResponse {
        ...
        Response(StreamInput in) throws IOException {
            ...
            in.readOptionalWriteable(GetResult::new);
        }

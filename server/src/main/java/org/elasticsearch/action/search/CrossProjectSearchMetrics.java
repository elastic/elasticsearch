...
public class CrossProjectSearchMetrics implements Writeable, ToXContentFragment {
    ...
    private static final TransportVersion SEARCH_PHASE_TIMES_ADDED = TransportVersion.fromName("cps_search_phase_times");
    ...
    public CrossProjectSearchMetrics(StreamInput in) throws IOException {
        ...
        if (in.getTransportVersion().supports(SEARCH_PHASE_TIMES_ADDED)) {
            this.searchPhaseTookTimes = in.readMap(StreamInput::readLong);
        } else {
            this.searchPhaseTookTimes = new LinkedHashMap<>();
        }
    }
    ...
    public void writeTo(StreamOutput out) throws IOException {
        ...
        if (out.getTransportVersion().supports(SEARCH_PHASE_TIMES_ADDED)) {
            out.writeMap(searchPhaseTookTimes, StreamOutput::writeLong);
        }
    }
}

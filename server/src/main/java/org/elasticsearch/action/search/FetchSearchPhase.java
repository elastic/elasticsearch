...
final class FetchSearchPhase extends SearchPhase {
    ...
    FetchSearchPhase(SearchPhaseResults<SearchPhaseResult> resultConsumer,
        ...
        this(resultConsumer, searchPhaseController, aggregatedDfs, context,
            List.of(LookupJoinHitSearchPhase::new, ExpandSearchPhase::new));
    }
    ...
    private void innerRun() throws Exception {
        ...
        final Runnable finishPhase = () -> {
            ...
            executeExtendedPhases(context, internalResponse, queryResults, this, 0);
    ...
    void executeExtendedPhases(SearchPhaseContext context,
                               InternalSearchResponse searchResponse,
                               AtomicArray<SearchPhaseResult> queryResults,
                               SearchPhase previousPhase,
                               int extendedPhaseIndex) {
        if (extendedPhaseIndex < extendedPhases.size()) {
            final ExtendedPhase nextPhase = extendedPhases.get(extendedPhaseIndex).createPhase(context, searchResponse, queryResults,
                ActionListener.wrap(
                    currentPhase -> executeExtendedPhases(context, searchResponse, queryResults, currentPhase, extendedPhaseIndex + 1),
                    context::onFailure));
            context.executeNextPhase(previousPhase, nextPhase);
        } else {
            context.sendSearchResponse(searchResponse, queryResults);
        }
    }

    abstract static class ExtendedPhase extends SearchPhase {
        ...
        ExtendedPhase(String name, SearchPhaseContext context,
                      InternalSearchResponse searchResponse,
                      AtomicArray<SearchPhaseResult> queryResults,
                      ActionListener<SearchPhase> onFinish) {
            super(name);
            this.context = context;
            this.searchResponse = searchResponse;
            this.queryResults = queryResults;
            this.onFinish = onFinish.map(nullValue -> this);
        }
    }

...
public enum ActiveShardsObserver {
    ...
    public static void waitForActiveShards(
        ...
        if (activeShardCount.enoughShardsActive(state.metadata().getProject(), state.routingTable(), indexNames)) {
            listener.onResponse(true);
            return;
        }
        ...
        new ClusterStateObserver(state, clusterService, null, logger, clusterService.threadPool().getThreadContext()).waitForNextChange(
            ...
            newState -> activeShardCount.enoughShardsActive(newState.metadata().getProject(), newState.routingTable(), indexNames),
            timeout
        );
    }

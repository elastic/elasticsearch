...
public abstract class AcknowledgedRequest<Request extends MasterNodeRequest<Request>> extends MasterNodeRequest<Request> {
     ...
     *                   {@link org.elasticsearch.cluster.ClusterStateAckListener#onAckTimeout()}. This timeout does not apply while the
     *                   cluster state is not committed: if the cluster state update fails before committing then the failure is reported
     *                   via {@link org.elasticsearch.cluster.ClusterStateAckListener#onAckFailure(Exception)} instead.
     *                   <p>

    ...
    final class ReroutePhase extends AbstractRunnable {
        ...
        void finishAsFailed(Exception failure) {
            if (finished.compareAndSet(false, true)) {
                ...
                logger.trace(() -> format(ROOT, "operation failed. action [%s], request [%s]", actionName, request), failure);
        ...
        void finishWithUnexpectedFailure(Exception failure) {
            logger.warn(
                () -> format(ROOT, "unexpected error during the primary phase for action [%s], request [%s]", actionName, request),
                failure
            );
            if (finished.compareAndSet(false, true)) {
                setPhase(task, "failed");

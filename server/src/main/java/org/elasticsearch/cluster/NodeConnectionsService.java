...
public class NodeConnectionsService extends AbstractLifecycleComponent {
    ...
    public void connectToNodes(DiscoveryNodes discoveryNodes, Runnable onCompletion) {
        ...
        final GroupedActionListener<Void> listener
            = new GroupedActionListener<>(ActionListener.wrap(onCompletion), discoveryNodes.getSize(), emptyList());
        ...
        synchronized (mutex) {
            ...
            if (connectionTargets.isEmpty()) {
                ...
                final GroupedActionListener<Void> listener = new GroupedActionListener<>(
                    ActionListener.wrap(onCompletion), connectionTargets.size(), emptyList());
    ...
    /*
     * FutureBasedConnectionTarget methods such as connect, disconnect and ensureConnected return nothing unlike ConnectionTarget.
     * However, they ensure that no heavy computations will happen under the mutex by using XXXAsync methods.
     */
    private class FutureBasedConnectionTarget {
        private CompletableFuture connectFuture;
        private CompletableFuture disconnectFuture;
        private Executor executor = threadPool.executor(ThreadPool.Names.MANAGEMENT);
        private DiscoveryNode discoveryNode;


        void connect(@Nullable ActionListener<Void> listener) {
            synchronized (mutex) {
                if (connectFuture != null) {
                    connectFuture.whenCompleteAsync((ok, ex) ->
                            {
                                if (ex == null) {
        ...
        void disconnect(@Nullable ActionListener<Void> listener) {
            synchronized (mutex) {
                if (disconnectFuture != null) {
                    disconnectFuture.whenCompleteAsync((ok, ex) -> {
                                if (ex == null) {
                                    listener.onResponse(null);
                                } else {
                                    listener.onFailure((Exception)ex);
                    ...
                    disconnectFuture.whenCompleteAsync((ig1, ig2) ->
                            {
                                synchronized (mutex) {
                                    if (disconnectFuture != null) {
                                        targetsByNode.remove(discoveryNode);
                                    }
                                }
                            },
                            executor);
        ...
        void ensureConnected(@Nullable ActionListener<Void> listener) {
            synchronized (mutex) {
                if (!isPendingDisconnection()) {
                    connect(listener);
                } else {
                    CompletableFuture.runAsync(() -> listener.onResponse(null), executor);
                }
            }
        }
        ...
        boolean isPendingDisconnection() {
            synchronized (mutex) {
                return disconnectFuture != null && disconnectFuture.isDone() == false;
            }
        }
    }

...
public final class DatasetMetadata extends AbstractNamedDiffable<Metadata.ProjectCustom> implements Metadata.ProjectCustom {
    ...
    /**
     * Feature flag that gates the ES|QL external data sources + datasets feature end-to-end: metadata
     * registration, dataset-aware error messages, collision detection preambles, and the downstream
     * CRUD/EXTERNAL surfaces. Kept here on the server-resident sibling custom so both {@code server}
     * and {@code x-pack} consumers reference a single source of truth (the data-source types live in
     * x-pack). System property: {@code es.esql_external_datasources_feature_flag_enabled}.
     */
    public static final FeatureFlag ESQL_EXTERNAL_DATASOURCES_FEATURE_FLAG = new FeatureFlag("esql_external_datasources");

    /**
     * Shared transport version for {@code DataSourceMetadata} and {@link DatasetMetadata}. Both metadata

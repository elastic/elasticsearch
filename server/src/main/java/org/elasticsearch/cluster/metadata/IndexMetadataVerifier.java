    ...
    private void checkMappingsCompatibility(IndexMetadata indexMetadata) {
        try {
            ...
            try (IndexAnalyzers fakeIndexAnalzyers = new IndexAnalyzers(analyzerMap, analyzerMap, analyzerMap)) {
                boolean autoGeneratedId = IndexSettings.TIME_SERIES_USE_AUTO_GENERATED_ID.get(indexSettings.getSettings());
                MapperService mapperService = new MapperService(
                    ...
                    mapperRegistry,
                    () -> null,
                    indexSettings.getMode().idFieldMapperWithoutFieldData(autoGeneratedId),
                    scriptService
                );

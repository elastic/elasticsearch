            ...
            submitUnbatchedTask(source, new ClusterStateUpdateTask(priority) {
                ...
                public void onFailure(Exception e) {
                    ...
                    if (MasterService.isPublishFailureException(e)) {
                        logger.debug(() -> format(ROOT, "unexpected failure during [%s], current state:\n%s", source, state), e);
                        // no big deal, the new master will reroute again
                    } else if (logger.isTraceEnabled()) {
                        logger.error(() -> format(ROOT, "unexpected failure during [%s], current state:\n%s", source, state), e);
                    } else {
                        logger.error(
                            () -> format(ROOT, "unexpected failure during [%s], current state version [%s]", source, state.version()),
                            e
                        );
                    }

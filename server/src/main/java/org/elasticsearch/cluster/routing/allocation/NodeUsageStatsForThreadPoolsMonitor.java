 ...
 * {@link RerouteService#reroute}) whenever a node crosses the node-level write load thresholds. Also maintains node-level
 * write-load stats for each data node that can be supplied to callers: these stats are pushed from data nodes whenever an
 * actionable change occurs, such as crossing the utilization or queue time thresholds.
 *

...
public class AllocationCommands implements ToXContentFragment {
    ...
    public static AllocationCommands fromXContent(XContentParser parser) throws IOException {
        ...
        if (token == null) {
            throw new ParsingException("No commands");
        }
        if (token == XContentParser.Token.FIELD_NAME) {
            if (parser.currentName().equals("commands") == false) {
                throw new ParsingException("expected field name to be named [commands], got [{}] instead", parser.currentName());
            }
            token = parser.nextToken();
            if (token != XContentParser.Token.START_ARRAY) {
                throw new ParsingException("commands should follow with an array element");
            }
        } else if (token == XContentParser.Token.START_ARRAY) {
            // ok...
        } else {
            throw new ParsingException("expected either field name [commands], or start array, got [{}] instead", token);
        }
        while ((token = parser.nextToken()) != XContentParser.Token.END_ARRAY) {
            if (token == XContentParser.Token.START_OBJECT) {
                ...
                throw new ParsingException("allocation command is malformed, got [{}] instead", token);
            }
        }
        return commands;
    }

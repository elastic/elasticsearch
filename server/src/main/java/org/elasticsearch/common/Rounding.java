...
public abstract class Rounding implements Writeable {
    private static final boolean FORCE_JAVA_TIME_ROUNDING = false;
     ...
     */
    public abstract LongUnaryOperator rounder(long minUtcMillis, long maxUtcMillis);

    /**
     * Round using {@link java.time} classes.
     */
    abstract LongUnaryOperator javaTimeRounder();

    /**
     * Rounds the given value. If rounding many values in a known range then
     * prefer {@link #rounder(long, long)}.
     */
    public final long round(long utcMillis) {
        return rounder(utcMillis, utcMillis).applyAsLong(utcMillis);
    }
    ...
    static class TimeUnitRounding extends Rounding {
        ...
        @Override
        public LongUnaryOperator rounder(long minUtcMillis, long maxUtcMillis) {
            ...
            LongFunction<LocalTimeOffset> lookup = LocalTimeOffset.lookup(
                    timeZone, minUtcMillis - unit.field.getBaseUnit().getDuration().toMillis(), maxUtcMillis);
            if (lookup == null) {
                return javaTimeRounder();
            }
            ...
            return new RoundFromStartOfPrevious(lookup);
        }
        ...
        private class RoundFromStartOfPrevious implements LongUnaryOperator, LocalTimeOffset.Strategy {
            ...
            public RoundFromStartOfPrevious(LongFunction<LocalTimeOffset> lookup) {
                this.lookup = lookup;
            }

            @Override
            public long applyAsLong(long utcMillis) {
                LocalTimeOffset offset = lookup.apply(utcMillis);
        ...
        @Override
        LongUnaryOperator javaTimeRounder() {
            if (unitRoundsToMidnight) {
                return utcMillis -> {
                    ...
                    if (currentOffsets.isEmpty() == false) {
                        ...
                        for (int offsetIndex = currentOffsets.size() - 1; 0 <= offsetIndex; offsetIndex--) {
                            ...
                            if (previousTransition != null && offsetInstant.isBefore(previousTransition.getInstant())) {
                                ...
                                // and round that down.
                                return applyAsLong(previousTransition.getInstant().toEpochMilli() - 1);
                            }
    ...
    static class OffsetRounding extends Rounding {
        ...
        @Override
        public LongUnaryOperator rounder(long minUtcMillis, long maxUtcMillis) {
            return wrapRounder(delegate.rounder(minUtcMillis, maxUtcMillis));
        }

        @Override
        LongUnaryOperator javaTimeRounder() {
            return wrapRounder(delegate.javaTimeRounder());
        }

        LongUnaryOperator wrapRounder(LongUnaryOperator delegateRounder) {
            return utcMillis -> delegateRounder.applyAsLong(utcMillis - offset) + offset;
        }

...
public class GeoWKTParser {
    ...
    private static ShapeBuilder parseGeometry(StreamTokenizer stream, GeoShapeType shapeType)
        ...
        if (shapeType != null && type.wktName().equals(shapeType.wktName()) == false) {
    ...
    private static GeometryCollectionBuilder parseGeometryCollection(StreamTokenizer stream)
        ...
        GeometryCollectionBuilder builder = new GeometryCollectionBuilder().shape(parseGeometry(stream, null));
        while (nextCloserOrComma(stream).equals(COMMA)) {

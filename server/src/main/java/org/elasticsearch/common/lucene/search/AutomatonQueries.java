...
public class AutomatonQueries {
     ...
     * both building the NFA (which can blow up while expanding bounded repetitions) and determinizing it into a
     * DFA are guarded, so a pathological pattern is rejected with a {@code CircuitBreakingException} instead of
     * exhausting the heap.
     */

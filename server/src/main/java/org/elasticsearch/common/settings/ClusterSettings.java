        ...
        public void apply(Settings value, Settings current, Settings previous) {
            for (String key : value.keySet()) {
                ...
                if ("_root".equals(component)) {
                    ...
                    if (rootLevel == null) {
                        LogLevelSupport.provider().setRootLoggerLevel(LogSettings.LOG_DEFAULT_LEVEL_SETTING.get(settings));
                    } else {
                        LogLevelSupport.provider().setRootLoggerLevel(rootLevel);
                    }
                } else {
                    LogLevelSupport.provider().setLevel(LogManager.getLogger(component), value.get(key));
                }
            }
        }
    }

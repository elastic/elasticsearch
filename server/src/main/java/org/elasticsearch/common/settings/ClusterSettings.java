...
public final class ClusterSettings extends AbstractScopedSettings {
    ...
    public static final Set<Setting<?>> BUILT_IN_CLUSTER_SETTINGS = gateByFeatureFlag(
        Set.of(
    ...
    private static Set<Setting<?>> gateByFeatureFlag(final Set<Setting<?>> all) {
        if (IndexSettings.ES95_RUNTABLE_ENCODING_FEATURE_FLAG.isEnabled()) {
            return all;
        }
        final Set<Setting<?>> filtered = new HashSet<>(all);
        filtered.remove(RunTableOrdinalClusterSettingProvider.TIME_SERIES_RUN_TABLE_ORDINAL_CLUSTER_ENABLED_SETTING);

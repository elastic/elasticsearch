...
public class ThrottledIterator<T> implements Releasable {
    ...
    public static <T> void run(
        Iterator<T> iterator,
        BiConsumer<ItemRefs, T> itemConsumer,
        int maxConcurrency,
    ...
    private final BiConsumer<ItemRefs, T> itemConsumer;
    ...
    private void run() {
        while (permits.tryAcquire()) {
            ...
            try (var itemRefs = new ItemRefCounted()) {
                itemConsumer.accept(itemRefs, item);
            } catch (Exception e) {
    ...
    // invocation of run().
    private class ItemRefCounted extends AbstractRefCounted implements Releasable, ItemRefs {

...
public class EirfEncoder implements SourceBatchEncoder {
    ...
    /** The result of {@link #packArray}: the inline EIRF array bytes and their {@code FIXED_ARRAY}/{@code UNION_ARRAY} kind. */
    public record PackedArrayResult(byte arrayType, byte[] packed) {}

    /**
     * Packs the array the parser is positioned at (just after {@code START_ARRAY}) into inline EIRF
     * array bytes. A public seam over {@link #parseArray} for other batch encoders (e.g. ESCF) that
     * store heterogeneous or object-bearing arrays inline.
     */

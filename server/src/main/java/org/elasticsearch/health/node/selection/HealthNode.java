...
public class HealthNode extends AllocatedPersistentTask {
    ...
    @Nullable
    public static PersistentTasksExtensionMetadata.PersistentTask<?> findTask(ClusterState clusterState) {
        PersistentTasksExtensionMetadata taskMetadata = clusterState.getMetadata().custom(PersistentTasksExtensionMetadata.TYPE);
    ...
    public static DiscoveryNode findHealthNode(ClusterState clusterState) {
        PersistentTasksExtensionMetadata.PersistentTask<?> task = findTask(clusterState);

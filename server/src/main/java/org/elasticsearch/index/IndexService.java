...
public class IndexService extends AbstractIndexComponent implements IndicesClusterStateService.AllocatedIndex<IndexShard> {
    ...
    public synchronized void deleteDanglingIndicesInfo() {
        ...
        try {
            ...
            logger.warn(() -> format(ROOT, "failed to delete dangling indices state for index %s", index()), e);
        }
    }
    ...
    private void closeShard(String reason, ShardId sId, IndexShard indexShard, Store store, IndexEventListener listener) {
        ...
        try {
            ...
            try {
                ...
                logger.warn(() -> format(ROOT, "[%s] failed to close store on shard removal (reason: [%s])", shardId, reason), e);
            }
        }
    }
                        ...
                        try {
                            shard.runUnderPrimaryPermit(() -> sync.accept(shard), e -> {
                                if (e instanceof AlreadyClosedException == false
                                    ...
                                    logger.warn(() -> format(ROOT, "%s failed to execute %s sync", shard.shardId(), source), e);
                                }

...
public final class Lucene50CompressingTermVectorsReader extends TermVectorsReader {
                    ...
                    try {
                        ...
                        if (version != version2) {
                            throw new CorruptIndexException(
                                "Version mismatch between stored fields index and data: " + version + " != " + version2,
                                indexStream
                            );
                        }
    ...
    public Fields get(int doc) throws IOException {
        ...
        final PackedInts.Reader flags;
        {
            ...
            final PackedInts.Reader allFieldNumOffs = LegacyPackedInts.getReaderNoHeader(
                vectorsStream,
                PackedInts.Format.PACKED,
                packedIntsVersion,
                totalFields,
            ...
            switch (vectorsStream.readVInt()) {
                case 0:
                    final PackedInts.Reader fieldFlags = LegacyPackedInts.getReaderNoHeader(
                        vectorsStream,
                        PackedInts.Format.PACKED,
                        packedIntsVersion,
                        fieldNums.length,
                    ...
                    break;
                case 1:
                    flags = LegacyPackedInts.getReaderNoHeader(
                        vectorsStream,
                        PackedInts.Format.PACKED,
                        packedIntsVersion,
                        totalFields,
        ...
        final int totalTerms;
        {
            ...
            numTerms = LegacyPackedInts.getReaderNoHeader(
                vectorsStream,
                PackedInts.Format.PACKED,
                packedIntsVersion,
                totalFields,

        ...
        if (currentFrame == staticFrame) {
            ...
            while (true) {
                ...
                if (f.nextEnt == -1) {
                    out.println(
                        "    frame "
                            ...
                            + f.isFloor
                            + " code="
                            + ((f.fp << Lucene90BlockTreeTermsReader.OUTPUT_FLAGS_NUM_BITS) + (f.hasTerms
                                ? Lucene90BlockTreeTermsReader.OUTPUT_FLAG_HAS_TERMS
                                : 0) + (f.isFloor ? Lucene90BlockTreeTermsReader.OUTPUT_FLAG_IS_FLOOR : 0))
                            + " isLastInFloor="
                            + f.isLastInFloor
                            + " mdUpto="
                            + f.metaDataUpto
                ...
                if (fr.index != null) {
                    ...
                    if (f.prefixLength > 0 && isSeekFrame && f.arc.label() != (term.byteAt(f.prefixLength - 1) & 0xFF)) {
                        out.println(
                    ...
                    if (output == null) {
                        ...
                        final long code = (f.fp << Lucene90BlockTreeTermsReader.OUTPUT_FLAGS_NUM_BITS) | (f.hasTerms
                            ? Lucene90BlockTreeTermsReader.OUTPUT_FLAG_HAS_TERMS
                            : 0) | (f.isFloor ? Lucene90BlockTreeTermsReader.OUTPUT_FLAG_IS_FLOOR : 0);
                        if (codeOrig != code) {

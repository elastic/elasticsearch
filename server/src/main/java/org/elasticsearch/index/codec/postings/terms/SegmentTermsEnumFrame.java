...
final class SegmentTermsEnumFrame {
    ...
    void rewind() {
        ...
        if (DEBUG) {
        System.out.println("      rewind frame ord=" + ord + " fpOrig=" + fpOrig + " fp=" + fp +
        " hasTerms?=" + hasTerms + " isFloor?=" + isFloor + " nextEnt=" + nextEnt + " prefixLen=" + prefix);
        }
        if (fp != fpOrig) {
        fp = fpOrig;
        nextEnt = -1;
    ...
    public void scanToFloorFrame(BytesRef target) {
        ...
        if (isFloor == false || target.length <= prefixLength) {
            // if (DEBUG) {
        ...
        while (nextEnt < entCount) {
            ...
            if (cmp < 0) {
                ...
                if (exactOnly == false && ste.termExists == false) {
                    ...
                    // sub-frame(s):
                    ste.currentFrame = ste.pushFrame(null, ste.currentFrame.lastSubFP, prefixLength + suffixLength);

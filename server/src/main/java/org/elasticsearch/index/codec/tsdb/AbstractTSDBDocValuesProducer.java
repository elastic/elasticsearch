...
/**
 * Abstract base class for TSDB doc values producers. Contains all shared wire-format reading logic.
 * Subclasses provide the numeric block decoding strategy.
 */
public abstract class AbstractTSDBDocValuesProducer extends DocValuesProducer {
     ...
     *
     * <p>Each codec version provides its own decoding strategy. The returned reader has two
     * levels: {@link NumericFieldReader#readField} reads the full field metadata during
     * segment init, and {@link NumericFieldReader#decoder()} provides a per-block decoder
     * for iteration.
     *
            ...
            return new BaseDenseNumericValues(maxDoc) {
                ...
                long lookAheadValueAt(int targetDoc) throws IOException {
                    ...
                    if (lookaheadBlockIndex != blockIndex) {
                        ...
                        if (bitsPerOrd == -1) {
                            decoder.decodeBlock(lookaheadData, lookaheadBlock, numericBlockSize);
                        } else {
                ...
                @Override
                public DocIdSetIterator tryTermIterator(long value, DocValuesSkipper skipper) throws IOException {
                    final IndexInput termData = data.slice("term_values", entry.valuesOffset, entry.valuesLength);
                    final long[] termBlock = new long[numericBlockSize];
                    return new DocIdSetIterator() {
                        private int currentDoc = -1;
                        private int currentBlockId = -1;
                        private int skipperUpTo = -1;
                        ...
                        public int advance(int target) throws IOException {
                            ...
                            if (skipper != null && target > skipperUpTo) {
                                skipper.advance(target);
                                while (skipper.minDocID(0) != NO_MORE_DOCS) {
                                    if (skipper.minValue(0) <= value && skipper.maxValue(0) >= value) {
                                        target = Math.max(target, skipper.minDocID(0));
                                        skipperUpTo = skipper.maxDocID(0);
                                        break;
                                    }
                            ...
                            for (int blockId = target >>> numericBlockShift; blockId <= endBlockId; blockId++) {
                                ...
                                if (blockId != currentBlockId) {
                                    if (currentBlockId == -1 || currentBlockId + 1 != blockId) {
                                        termData.seek(indexReader.get(blockId));
                                    }
                                    currentBlockId = blockId;
                                    if (bitsPerOrd == -1) {
                                        decoder.decodeBlock(termData, termBlock, numericBlockSize);
                                    } else {
                                        decoder.decodeOrdinals(termData, termBlock, bitsPerOrd);
                                    }
                                }
                                int blockStart = blockId * numericBlockSize;
                                int blockEnd = Math.min(blockStart + numericBlockSize, maxDoc);
                                int startInBlock = Math.max(target, blockStart);
                                for (int doc = startInBlock; doc < blockEnd; doc++) {
                                    if (termBlock[doc - blockStart] == value) {
                                        return currentDoc = doc;
                                    }
                                }
                            }

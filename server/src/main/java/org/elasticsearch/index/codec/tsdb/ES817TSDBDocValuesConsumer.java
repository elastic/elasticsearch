...
import static org.elasticsearch.index.codec.tsdb.ES817TSDBDocValuesFormat.DIRECT_MONOTONIC_BLOCK_SHIFT;
import static org.elasticsearch.index.codec.tsdb.ES817TSDBDocValuesFormat.SKIP_INDEX_LEVEL_SHIFT;
import static org.elasticsearch.index.codec.tsdb.ES817TSDBDocValuesFormat.SKIP_INDEX_MAX_LEVEL;
import static org.elasticsearch.index.codec.tsdb.ES817TSDBDocValuesFormat.SORTED_SET;
...
final class ES817TSDBDocValuesConsumer extends DocValuesConsumer {
    ...
    public void addNumericField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        ...
        meta.writeByte(ES817TSDBDocValuesFormat.NUMERIC);
    ...
    private long[] writeField(FieldInfo field, DocValuesProducer valuesProducer, long maxOrd) throws IOException {
        ...
        if (numValues > 0) {
            ...
            meta.writeInt(maxOrd != 1 ? ES817TSDBDocValuesFormat.DIRECT_MONOTONIC_BLOCK_SHIFT : -1);
            ...
            final DirectMonotonicWriter indexWriter = DirectMonotonicWriter.getInstance(
                ...
                1L + ((numValues - 1) >>> ES817TSDBDocValuesFormat.NUMERIC_BLOCK_SHIFT),
            ...
            if (maxOrd != 1) {
                final long[] buffer = new long[ES817TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE];
                int bufferSize = 0;
                final TSDBDocValuesEncoder encoder = new TSDBDocValuesEncoder(ES817TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE);
                values = valuesProducer.getSortedNumeric(field);
                ...
                for (int doc = values.nextDoc(); doc != DocIdSetIterator.NO_MORE_DOCS; doc = values.nextDoc()) {
                    ...
                    for (int i = 0; i < count; ++i) {
                        ...
                        if (bufferSize == ES817TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE) {
                ...
                if (bufferSize > 0) {
                    ...
                    Arrays.fill(buffer, bufferSize, ES817TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE, 0L);
                    if (maxOrd >= 0) {
                        encoder.encodeOrdinals(buffer, data, bitsPerOrd);
                    } else {
                        encoder.encode(buffer, data);
    ...
    public void addSortedField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        meta.writeInt(field.number);
        meta.writeByte(ES817TSDBDocValuesFormat.SORTED);
    ...
    private void addTermsDict(SortedSetDocValues values) throws IOException {
        ...
        int blockMask = ES817TSDBDocValuesFormat.TERMS_DICT_BLOCK_LZ4_MASK;
        int shift = ES817TSDBDocValuesFormat.TERMS_DICT_BLOCK_LZ4_SHIFT;
    ...
    private void writeTermsIndex(SortedSetDocValues values) throws IOException {
        final long size = values.getValueCount();
        meta.writeInt(ES817TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_SHIFT);
        long start = data.getFilePointer();

        long numBlocks = 1L + ((size + ES817TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK)
            >>> ES817TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_SHIFT);
        ByteBuffersDataOutput addressBuffer = new ByteBuffersDataOutput();
        ...
        try (ByteBuffersIndexOutput addressOutput = new ByteBuffersIndexOutput(addressBuffer, "temp", "temp")) {
            ...
            long ord = 0;
            for (BytesRef term = iterator.next(); term != null; term = iterator.next()) {
                if ((ord & ES817TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) == 0) {
                    writer.add(offset);
                    ...
                    & ES817TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) == ES817TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) {
                        previous.copyBytes(term);
                    }
                ++ord;
            }
    ...
    public void addSortedNumericField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        meta.writeInt(field.number);
        meta.writeByte(ES817TSDBDocValuesFormat.SORTED_NUMERIC);

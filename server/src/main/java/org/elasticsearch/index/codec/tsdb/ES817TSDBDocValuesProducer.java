...
import static org.elasticsearch.index.codec.tsdb.ES817TSDBDocValuesFormat.SKIP_INDEX_JUMP_LENGTH_PER_LEVEL;
...
public class ES817TSDBDocValuesProducer extends DocValuesProducer {
        ...
        try (ChecksumIndexInput in = state.directory.openChecksumInput(metaName)) {
            ...
            try {
                version = CodecUtil.checkIndexHeader(
                    in,
                    metaCodec,
                    ES817TSDBDocValuesFormat.VERSION_START,
    ...
    public BinaryDocValues getBinary(FieldInfo field) throws IOException {
        final ES817TSDBCompressingBinaryDocValues.Entry entry = binaries.get(field.name);
        ...
        if (entry.indexedDISIOffset == -1) {
            return new DenseBinaryDocValues(maxDoc) {
                ...
                public BytesRef binaryValue() throws IOException {
                    return compressingReader.readValue(doc);
                }
            };
        } else {
            // sparse
            final IndexedDISI disi = new IndexedDISI(
                data,
                entry.indexedDISIOffset,
                entry.indexedDISILength,
                entry.jumpTableEntryCount,
                entry.denseRankPower,
                entry.numDocsWithValues
            );
            return new SparseBinaryDocValues(disi) {
                @Override
                public BytesRef binaryValue() throws IOException {
                    return compressingReader.readValue(disi.index());
                }
            };
        }
    }
    ...
    private void readFields(IndexInput meta, FieldInfos infos) throws IOException {
        for (int fieldNumber = meta.readInt(); fieldNumber != -1; fieldNumber = meta.readInt()) {
            ...
            if (type == ES817TSDBDocValuesFormat.NUMERIC) {
                numerics.put(info.name, readNumeric(meta));
            } else if (type == ES817TSDBDocValuesFormat.BINARY) {
                binaries.put(info.name, ES817TSDBCompressingBinaryDocValues.readEntry(meta));
            } else if (type == ES817TSDBDocValuesFormat.SORTED) {
                sorted.put(info.name, readSorted(meta));
            } else if (type == ES817TSDBDocValuesFormat.SORTED_SET) {
                sortedSets.put(info.name, readSortedSet(meta));
            } else if (type == ES817TSDBDocValuesFormat.SORTED_NUMERIC) {
        ...
        if (entry.numValues > 0) {
            ...
            if (indexBlockShift != -1) {
                entry.indexMeta = DirectMonotonicReader.loadMeta(
                    meta,
                    1 + ((entry.numValues - 1) >>> ES817TSDBDocValuesFormat.NUMERIC_BLOCK_SHIFT),
                    indexBlockShift
                );
            }
    ...
    private NumericDocValues getNumeric(NumericEntry entry, long maxOrd) throws IOException {
        ...
        if (maxOrd == 1) {
            ...
            if (entry.docsWithFieldOffset == -1) {
                ...
                return new NumericDocValues() {
                    ...
                    private final int maxDoc = ES817TSDBDocValuesProducer.this.maxDoc;
                    private int doc = -1;
        ...
        if (entry.docsWithFieldOffset == -1) {
            ...
            return new NumericDocValues() {
                ...
                private final int maxDoc = ES817TSDBDocValuesProducer.this.maxDoc;
                private int doc = -1;
                private final TSDBDocValuesEncoder decoder = new TSDBDocValuesEncoder(ES817TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE);
                private long currentBlockIndex = -1;
                private final long[] currentBlock = new long[ES817TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE];
                ...
                public long longValue() throws IOException {
                    final int index = doc;
                    final int blockIndex = index >>> ES817TSDBDocValuesFormat.NUMERIC_BLOCK_SHIFT;
                    final int blockInIndex = index & ES817TSDBDocValuesFormat.NUMERIC_BLOCK_MASK;
    ...
    private NumericValues getValues(NumericEntry entry, final long maxOrd) throws IOException {
        ...
        return new NumericValues() {
            ...
            long advance(long index) throws IOException {
                final long blockIndex = index >>> ES817TSDBDocValuesFormat.NUMERIC_BLOCK_SHIFT;
                final int blockInIndex = (int) (index & ES817TSDBDocValuesFormat.NUMERIC_BLOCK_MASK);
                if (blockIndex != currentBlockIndex) {
                    // no need to seek if the loading block is the next block
                    if (currentBlockIndex + 1 != blockIndex) {
                        valuesData.seek(indexReader.get(blockIndex));

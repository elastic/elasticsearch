...
/**
 * Factory for block-level encoders and decoders of the ordinal stream of SORTED doc values
 * (single value per document).
 *
 * <p>TSDB doc values are stored in fixed-size blocks. This interface controls how each block of
 * ordinals is encoded on write and decoded on read for SORTED fields. The field-level layout
 * (block index, DISI, metadata) is handled by {@link TSDBDocValuesBlockWriter} and
 ...
 * merge threads.
 */
public interface SortedOrdinalBlockCodec {

    /**
     * Returns a reader that can decode ordinal value blocks for a SORTED field in this segment.
     *
    ...
    /**
     * Returns a writer that can encode ordinal value blocks for a SORTED field in this segment.
     *

...
import static org.elasticsearch.index.codec.tsdb.es94.ES94TSDBDocValuesFormat.BLOCK_BYTES_THRESHOLD;
import static org.elasticsearch.index.codec.tsdb.es94.ES94TSDBDocValuesFormat.BLOCK_COUNT_THRESHOLD;
import static org.elasticsearch.index.codec.tsdb.es94.ES94TSDBDocValuesFormat.DIRECT_MONOTONIC_BLOCK_SHIFT;
import static org.elasticsearch.index.codec.tsdb.es94.ES94TSDBDocValuesFormat.SKIP_INDEX_LEVEL_SHIFT;
import static org.elasticsearch.index.codec.tsdb.es94.ES94TSDBDocValuesFormat.SKIP_INDEX_MAX_LEVEL;
import static org.elasticsearch.index.codec.tsdb.es94.ES94TSDBDocValuesFormat.SORTED_SET;
...
final class ES94TSDBDocValuesConsumer extends XDocValuesConsumer {
    ...
    private PipelineResolver.FieldContext resolveFieldContext(final String fieldName) {
        return fieldContextResolver.resolve(fieldName);
    }

    private int resolveBlockSize(final PipelineResolver.FieldContext ctx) {
        return ctx.blockSize();
    }

    private PipelineConfig resolvePipelineConfig(final PipelineResolver.FieldContext ctx, final long[] sample, int sampleSize) {
    ...
    private long[] writeField(
        ...
        final PipelineResolver.FieldContext fieldContext,
        int blockSize
    ) throws IOException {
    ...
    public void mergeNumericField(FieldInfo mergeFieldInfo, MergeState mergeState) throws IOException {
        var result = DocValuesConsumerUtil.compatibleWithOptimizedMerge(enableOptimizedMerge, mergeState, mergeFieldInfo);
        if (result.supported()) {
    ...
    public void addBinaryField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        meta.writeInt(field.number);
        meta.writeByte(ES94TSDBDocValuesFormat.BINARY);
    ...
    private final class CompressedBinaryBlockWriter implements BinaryWriter {
        ...
        final int[] docOffsets = new int[BLOCK_COUNT_THRESHOLD + 1];
    ...
    public void addSortedField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        meta.writeInt(field.number);
        meta.writeByte(ES94TSDBDocValuesFormat.SORTED);
    ...
    private void addTermsDict(SortedSetDocValues values) throws IOException {
        ...
        int blockMask = ES94TSDBDocValuesFormat.TERMS_DICT_BLOCK_LZ4_MASK;
        int shift = ES94TSDBDocValuesFormat.TERMS_DICT_BLOCK_LZ4_SHIFT;
    ...
    private void writeTermsIndex(SortedSetDocValues values) throws IOException {
        final long size = values.getValueCount();
        meta.writeInt(ES94TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_SHIFT);
        long start = data.getFilePointer();

        long numBlocks = 1L + ((size + ES94TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK)
            >>> ES94TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_SHIFT);
        ByteBuffersDataOutput addressBuffer = new ByteBuffersDataOutput();
        ...
        try (ByteBuffersIndexOutput addressOutput = new ByteBuffersIndexOutput(addressBuffer, "temp", "temp")) {
            ...
            long ord = 0;
            for (BytesRef term = iterator.next(); term != null; term = iterator.next()) {
                if ((ord & ES94TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) == 0) {
                    writer.add(offset);
                    ...
                    & ES94TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) == ES94TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) {
                        previous.copyBytes(term);
                    }
                ++ord;
            }
    ...
    private void writeSortedNumericField(
        ...
        TsdbDocValuesProducer valuesProducer,
        long maxOrd,
        final PipelineResolver.FieldContext pipelineContext,
    ...
    private void writeLevels(List<SkipAccumulator> accumulators) throws IOException {
        ...
        for (int index = 0; index < totalAccumulators; index++) {
            ...
            for (int level = levels - 1; level >= 0; level--) {
                final SkipAccumulator acc = accumulatorsLevels.get(level).get(index >> (SKIP_INDEX_LEVEL_SHIFT * level));

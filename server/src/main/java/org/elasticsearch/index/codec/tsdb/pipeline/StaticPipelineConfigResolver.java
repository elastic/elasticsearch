 ...
 * the split-delta variants, so the per-field write path reuses a single instance instead of
...
public final class StaticPipelineConfigResolver implements PipelineConfigResolver {
    ...
    public PipelineConfig resolve(final FieldContext context) {
        return useSplitDelta(context) ? splitDeltaConfig(context.blockSize()) : baselineConfig(context.blockSize());
    }

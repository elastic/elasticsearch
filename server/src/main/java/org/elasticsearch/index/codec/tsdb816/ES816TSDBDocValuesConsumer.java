...
import static org.elasticsearch.index.codec.tsdb816.ES816TSDBDocValuesFormat.DIRECT_MONOTONIC_BLOCK_SHIFT;
import static org.elasticsearch.index.codec.tsdb816.ES816TSDBDocValuesFormat.SORTED_SET;

final class ES816TSDBDocValuesConsumer extends DocValuesConsumer {
    ...
    ES816TSDBDocValuesConsumer(SegmentWriteState state, String dataCodec, String dataExtension, String metaCodec, String metaExtension)
        throws IOException {
        ...
        try {
            ...
            CodecUtil.writeIndexHeader(
                data,
                dataCodec,
                ES816TSDBDocValuesFormat.VERSION_CURRENT,
    ...
    public void addNumericField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        ...
        meta.writeByte(ES816TSDBDocValuesFormat.NUMERIC);
    ...
    private long[] writeField(FieldInfo field, DocValuesProducer valuesProducer, long maxOrd) throws IOException {
        ...
        if (numValues > 0) {
            ...
            meta.writeInt(maxOrd != 1 ? ES816TSDBDocValuesFormat.DIRECT_MONOTONIC_BLOCK_SHIFT : -1);
            ...
            final DirectMonotonicWriter indexWriter = DirectMonotonicWriter.getInstance(
                ...
                1L + ((numValues - 1) >>> ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SHIFT),
                ES816TSDBDocValuesFormat.DIRECT_MONOTONIC_BLOCK_SHIFT
            );

            final long valuesDataOffset = data.getFilePointer();
            // Special case for maxOrd of 1, skip writing the blocks
            if (maxOrd != 1) {
                final long[] buffer = new long[ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE];
                int bufferSize = 0;
                final ES816TSDBDocValuesEncoder encoder = new ES816TSDBDocValuesEncoder();
                values = valuesProducer.getSortedNumeric(field);
                ...
                for (int doc = values.nextDoc(); doc != DocIdSetIterator.NO_MORE_DOCS; doc = values.nextDoc()) {
                    ...
                    for (int i = 0; i < count; ++i) {
                        ...
                        if (bufferSize == ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE) {
                ...
                if (bufferSize > 0) {
                    ...
                    Arrays.fill(buffer, bufferSize, ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE, 0L);
                    if (maxOrd >= 0) {
                        encoder.encodeOrdinals(buffer, data, bitsPerOrd);
                    } else {
                        encoder.encode(buffer, data);
    ...
    public void addBinaryField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        meta.writeInt(field.number);
        meta.writeByte(ES816TSDBDocValuesFormat.BINARY);
        ...
        CompressedBinaryBlockWriter() throws IOException {
            ...
            try {
                CodecUtil.writeHeader(
                    tempBinaryOffsets,
                    ES816TSDBDocValuesFormat.META_CODEC + "FilePointers",
                    ES816TSDBDocValuesFormat.VERSION_CURRENT
                );
    ...
    public void addSortedField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        meta.writeInt(field.number);
        meta.writeByte(ES816TSDBDocValuesFormat.SORTED);
    ...
    private void addTermsDict(SortedSetDocValues values) throws IOException {
        ...
        int blockMask = ES816TSDBDocValuesFormat.TERMS_DICT_BLOCK_LZ4_MASK;
        int shift = ES816TSDBDocValuesFormat.TERMS_DICT_BLOCK_LZ4_SHIFT;
    ...
    private void writeTermsIndex(SortedSetDocValues values) throws IOException {
        final long size = values.getValueCount();
        meta.writeInt(ES816TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_SHIFT);
        long start = data.getFilePointer();

        long numBlocks = 1L + ((size + ES816TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK)
            >>> ES816TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_SHIFT);
        ByteBuffersDataOutput addressBuffer = new ByteBuffersDataOutput();
        ...
        try (ByteBuffersIndexOutput addressOutput = new ByteBuffersIndexOutput(addressBuffer, "temp", "temp")) {
            ...
            long ord = 0;
            for (BytesRef term = iterator.next(); term != null; term = iterator.next()) {
                if ((ord & ES816TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) == 0) {
                    writer.add(offset);
                    ...
                    & ES816TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) == ES816TSDBDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) {
                        previous.copyBytes(term);
                    }
                ++ord;
            }
    ...
    public void addSortedNumericField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        meta.writeInt(field.number);
        meta.writeByte(ES816TSDBDocValuesFormat.SORTED_NUMERIC);
    ...
    private void writeSortedNumericField(FieldInfo field, DocValuesProducer valuesProducer, long maxOrd) throws IOException {
        ...
        if (numValues > numDocsWithField) {
            long start = data.getFilePointer();
            meta.writeLong(start);
            meta.writeVInt(ES816TSDBDocValuesFormat.DIRECT_MONOTONIC_BLOCK_SHIFT);

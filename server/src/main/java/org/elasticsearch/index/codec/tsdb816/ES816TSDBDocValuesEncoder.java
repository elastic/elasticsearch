 ...
 * </ul>
 *
 * Notice that encoding and decoding are written in a nested way, for instance {@link ES816TSDBDocValuesEncoder#deltaEncode} calling
 * {@link ES816TSDBDocValuesEncoder#removeOffset} and so on. This allows us to easily introduce new encoding schemes or remove existing
 ...
 */
public class ES816TSDBDocValuesEncoder {
    private final DocValuesForUtil forUtil;

    public ES816TSDBDocValuesEncoder() {
        this.forUtil = new DocValuesForUtil(ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE);
    }
    ...
    private void deltaEncode(int token, int tokenBits, long[] in, DataOutput out) throws IOException {
        int gts = 0;
        int lts = 0;
        for (int i = 1; i < ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE; ++i) {
            if (in[i] > in[i - 1]) {
                gts++;
            } else if (in[i] < in[i - 1]) {
                lts++;
        ...
        if (doDeltaCompression) {
            for (int i = ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE - 1; i > 0; --i) {
                in[i] -= in[i - 1];
            }
    ...
    private void removeOffset(int token, int tokenBits, long[] in, DataOutput out) throws IOException {
        ...
        if (min != 0) {
            for (int i = 0; i < ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE; ++i) {
                in[i] -= min;
            }
            token = (token << 1) | 0x01;
        } else {
    ...
    void encode(long[] in, DataOutput out) throws IOException {
        assert in.length == ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE;
    ...
    void decodeOrdinals(DataInput in, long[] out, int bitsPerOrd) throws IOException {
        assert out.length == ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE : out.length;

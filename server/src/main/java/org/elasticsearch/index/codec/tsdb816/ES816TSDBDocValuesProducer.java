...
import static org.elasticsearch.index.codec.tsdb816.ES816TSDBDocValuesFormat.TERMS_DICT_BLOCK_LZ4_SHIFT;

final class ES816TSDBDocValuesProducer extends DocValuesProducer {
    ...
    ES816TSDBDocValuesProducer(SegmentReadState state, String dataCodec, String dataExtension, String metaCodec, String metaExtension)
        throws IOException {
        ...
        try (ChecksumIndexInput in = state.directory.openChecksumInput(metaName, state.context)) {
            ...
            try {
                version = CodecUtil.checkIndexHeader(
                    in,
                    metaCodec,
                    ES816TSDBDocValuesFormat.VERSION_START,
    ...
    private void readFields(IndexInput meta, FieldInfos infos) throws IOException {
        for (int fieldNumber = meta.readInt(); fieldNumber != -1; fieldNumber = meta.readInt()) {
            ...
            if (type == ES816TSDBDocValuesFormat.NUMERIC) {
                numerics.put(info.name, readNumeric(meta));
            } else if (type == ES816TSDBDocValuesFormat.BINARY) {
                binaries.put(info.name, readBinary(meta));
            } else if (type == ES816TSDBDocValuesFormat.SORTED) {
                sorted.put(info.name, readSorted(meta));
            } else if (type == ES816TSDBDocValuesFormat.SORTED_SET) {
                sortedSets.put(info.name, readSortedSet(meta));
            } else if (type == ES816TSDBDocValuesFormat.SORTED_NUMERIC) {
        ...
        if (entry.numValues > 0) {
            ...
            if (indexBlockShift != -1) {
                entry.indexMeta = DirectMonotonicReader.loadMeta(
                    meta,
                    1 + ((entry.numValues - 1) >>> ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SHIFT),
                    indexBlockShift
                );
            }
    ...
    private NumericDocValues getNumeric(NumericEntry entry, long maxOrd) throws IOException {
        ...
        if (maxOrd == 1) {
            ...
            if (entry.docsWithFieldOffset == -1) {
                ...
                return new NumericDocValues() {
                    ...
                    private final int maxDoc = ES816TSDBDocValuesProducer.this.maxDoc;
                    private int doc = -1;
        ...
        if (entry.docsWithFieldOffset == -1) {
            ...
            return new NumericDocValues() {
                ...
                private final int maxDoc = ES816TSDBDocValuesProducer.this.maxDoc;
                private int doc = -1;
                private final ES816TSDBDocValuesEncoder decoder = new ES816TSDBDocValuesEncoder();
                private long currentBlockIndex = -1;
                private final long[] currentBlock = new long[ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SIZE];
                ...
                public long longValue() throws IOException {
                    final int index = doc;
                    final int blockIndex = index >>> ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SHIFT;
                    final int blockInIndex = index & ES816TSDBDocValuesFormat.NUMERIC_BLOCK_MASK;
    ...
    private NumericValues getValues(NumericEntry entry, final long maxOrd) throws IOException {
        ...
        return new NumericValues() {
            ...
            long advance(long index) throws IOException {
                final long blockIndex = index >>> ES816TSDBDocValuesFormat.NUMERIC_BLOCK_SHIFT;
                final int blockInIndex = (int) (index & ES816TSDBDocValuesFormat.NUMERIC_BLOCK_MASK);
                if (blockIndex != currentBlockIndex) {
                    // no need to seek if the loading block is the next block
                    if (currentBlockIndex + 1 != blockIndex) {
                        valuesData.seek(indexReader.get(blockIndex));

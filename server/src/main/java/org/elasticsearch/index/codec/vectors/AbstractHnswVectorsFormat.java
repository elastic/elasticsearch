...
public abstract class AbstractHnswVectorsFormat extends KnnVectorsFormat {
     ...
     * Below this threshold, brute-force search is efficient enough that graph construction overhead isn't worthwhile.
     */
    protected final int hnswGraphThreshold;

    /** Constructs a format using default graph construction parameters */
    protected AbstractHnswVectorsFormat(String name) {
        this(name, DEFAULT_MAX_CONN, DEFAULT_BEAM_WIDTH, DEFAULT_NUM_MERGE_WORKER, null, 0);
    }

    /** Constructs a format using default graph construction parameters with custom HNSW graph threshold */
    protected AbstractHnswVectorsFormat(String name, int hnswGraphThreshold) {
        this(name, DEFAULT_MAX_CONN, DEFAULT_BEAM_WIDTH, DEFAULT_NUM_MERGE_WORKER, null, hnswGraphThreshold);
    }
    ...
    protected AbstractHnswVectorsFormat(String name, int maxConn, int beamWidth) {
        this(name, maxConn, beamWidth, DEFAULT_NUM_MERGE_WORKER, null, 0);
    }
    ...
    protected AbstractHnswVectorsFormat(String name, int maxConn, int beamWidth, int numMergeWorkers, ExecutorService mergeExec) {
        this(name, maxConn, beamWidth, numMergeWorkers, mergeExec, 0);
    }
     ...
     * @param mergeExec the {@link ExecutorService} that will be used by ALL vector writers that are
     *     generated by this format to do the merge
     * @param hnswGraphThreshold the minimum expected search cost before building an HNSW graph; if negative, use format default
     */
    protected AbstractHnswVectorsFormat(
        String name,
        int maxConn,
        int beamWidth,
        int numMergeWorkers,
    ...
    public String toString() {
        return getName()
            ...
            + ", beamWidth="
            + beamWidth
            + (hnswGraphThreshold > 0 ? ", hnswGraphThreshold=" + hnswGraphThreshold : "")
            + ", flatVectorFormat="
            + flatVectorsFormat()
            + ")";
    }

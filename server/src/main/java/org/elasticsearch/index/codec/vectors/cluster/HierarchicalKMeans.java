...
public class HierarchicalKMeans<V> {
    ...
    public KMeansWithOverspill<V> cluster(ClusteringVectorValues<V> vectors, int targetSize, V[] warmStartCentroids) throws IOException {
        ...
        if (kMeansIntermediate.centroids().length > 1 && kMeansIntermediate.centroids().length < vectors.size()) {
            ...
            KMeansWithOverspill<V> res = kMeansLocal.cluster(vectors, kMeansIntermediate, clustersPerNeighborhood);
    ...
    private SoarAssignments assignSoarOnly(ClusteringVectorValues<V> vectors, KMeansIntermediate<V> kMeansIntermediate) throws IOException {
        ...
        if (executor != null && effectiveWorkers >= 2) {
            return new SoarAssignments(
                Soar.assignSpilledConcurrent(executor, effectiveWorkers, vectors, ops, kMeansIntermediate, neighborhoods, soarLambda)
            );
        } else {
            return new SoarAssignments(Soar.assignSpilledSlice(vectors, ops, kMeansIntermediate, neighborhoods, soarLambda));
        }
    }
    ...
    public KMeansWithOverspill<V> clusterByInsertion(
        ...
        if (kMeansIntermediate.centroids().length > 1 && kMeansIntermediate.centroids().length < vectors.size()) {
            ...
            return refinementKMeans.cluster(vectors, kMeansIntermediate, clustersPerNeighborhood);
        }

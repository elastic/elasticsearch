...
class FlatCentroidIndex {
    ...
    private final ES950DiskBBQVectorsReader.NextFieldEntry fieldEntry;
    ...
    FlatCentroidIndex(
        FieldInfo fieldInfo,
        ES950DiskBBQVectorsReader.NextFieldEntry fieldEntry,
        int numCentroids,
    ...
    private static FixedBitSet getCentroidFilter(
        ...
        final FixedBitSet acceptCentroidsSet;
        if (approximateDocsPerCentroid > 1.25 || numCentroids == 1 || acceptDocs instanceof ESAcceptDocs.ESAcceptDocsAll) {
            ...
            acceptCentroidsSet = null;
        } else {
            acceptCentroidsSet = new FixedBitSet(numCentroids);
            ...
            for (; doc != DocIdSetIterator.NO_MORE_DOCS; doc = iterator.nextDoc()) {
                acceptCentroidsSet.set((int) longValues.get(docIndexIterator.index()));
            }
        }
        centroids.seek(fp + sizeLookup);
        return acceptCentroidsSet;
    }
    ...
    private static FixedBitSet getParentCentroidFilter(
        ...
        long fp = centroids.getFilePointer();
        FixedBitSet acceptParentsSet = null;
        if (acceptDocs instanceof ESAcceptDocs esAcceptDocs) {
            ...
            if (numParents > 0) {
                acceptParentsSet = new FixedBitSet(numParents);
                assert startOffset >= 0 && endOffset <= numParents;
            } else {
                acceptParentsSet = new FixedBitSet(numCentroids);
                assert startOffset >= 0 && endOffset <= numCentroids;
            }
            acceptParentsSet.set(startOffset, endOffset);
        }
        centroids.seek(fp + (long) numSlices * Integer.BYTES);
        return acceptParentsSet;
    }

...
public abstract class Engine implements Closeable {
    ...
    public static class Index extends Operation {
        ...
        private final long casSeqNo;
        private final long casPrimaryTerm;

        public Index(Term uid, ParsedDocument doc, long seqNo, long primaryTerm, long version, VersionType versionType, Origin origin,
                     long startTime, long autoGeneratedIdTimestamp, boolean isRetry, long casSeqNo, long casPrimaryTerm) {
            super(uid, seqNo, primaryTerm, version, versionType, origin, startTime);
            assert (origin == Origin.PRIMARY) == (versionType != null) : "invalid version_type=" + versionType + " for origin=" + origin;
            assert casPrimaryTerm >= 0 : "casPrimaryTerm [" + casPrimaryTerm + "] must be non negative";
            assert casSeqNo == SequenceNumbers.UNASSIGNED_SEQ_NO || casSeqNo >=0 :
                "casSeqNo [" + casSeqNo + "] must be non negative or unset";
            assert (origin == Origin.PRIMARY) || (casSeqNo == SequenceNumbers.UNASSIGNED_SEQ_NO && casPrimaryTerm == 0) :
                "cas operations are only allowed if origin is primary. get [" + origin + "]";
            this.doc = doc;
            this.isRetry = isRetry;
            this.autoGeneratedIdTimestamp = autoGeneratedIdTimestamp;
            this.casSeqNo = casSeqNo;
            this.casPrimaryTerm = casPrimaryTerm;
        }
        ...
        public long getCasSeqNo() {
            return casSeqNo;
        }

        public long getCasPrimaryTerm() {
    ...
    public static class Delete extends Operation {
        ...
        public Delete(String type, String id, Term uid, long seqNo, long primaryTerm, long version, VersionType versionType,
                      Origin origin, long startTime, long casSeqNo, long casPrimaryTerm) {
            super(uid, seqNo, primaryTerm, version, versionType, origin, startTime);
            assert (origin == Origin.PRIMARY) == (versionType != null) : "invalid version_type=" + versionType + " for origin=" + origin;
            assert casPrimaryTerm >= 0 : "casPrimaryTerm [" + casPrimaryTerm + "] must be non negative";
            assert casSeqNo == SequenceNumbers.UNASSIGNED_SEQ_NO || casSeqNo >=0 :

...
public abstract class Engine implements Closeable {
    ...
    public static class Index extends Operation {
        ...
        private final long compareAndWriteSeqNo;
        private final long compareAndWriteTerm;

        public Index(Term uid, ParsedDocument doc, long seqNo, long primaryTerm, long version, VersionType versionType, Origin origin,
                     long startTime, long autoGeneratedIdTimestamp, boolean isRetry, long compareAndWriteSeqNo, long compareAndWriteTerm) {
            super(uid, seqNo, primaryTerm, version, versionType, origin, startTime);
            assert (origin == Origin.PRIMARY) == (versionType != null) : "invalid version_type=" + versionType + " for origin=" + origin;
            assert compareAndWriteTerm >= 0;
            assert compareAndWriteSeqNo == SequenceNumbers.UNASSIGNED_SEQ_NO || compareAndWriteSeqNo >=0;
            assert (origin == Origin.PRIMARY) || (compareAndWriteSeqNo == SequenceNumbers.UNASSIGNED_SEQ_NO && compareAndWriteTerm == 0);
            this.doc = doc;
            this.isRetry = isRetry;
            this.autoGeneratedIdTimestamp = autoGeneratedIdTimestamp;
            this.compareAndWriteSeqNo = compareAndWriteSeqNo;
            this.compareAndWriteTerm = compareAndWriteTerm;
        }
        ...
        public long getCompareAndWriteSeqNo() {
            return compareAndWriteSeqNo;
        }

        public long getCompareAndWriteTerm() {
    ...
    public static class Delete extends Operation {
        ...
        public Delete(String type, String id, Term uid, long seqNo, long primaryTerm, long version, VersionType versionType,
                      Origin origin, long startTime, long compareAndWriteSeqNo, long compareAndWriteTerm) {
            super(uid, seqNo, primaryTerm, version, versionType, origin, startTime);
            assert (origin == Origin.PRIMARY) == (versionType != null) : "invalid version_type=" + versionType + " for origin=" + origin;
            assert compareAndWriteTerm >= 0;
            assert compareAndWriteSeqNo == SequenceNumbers.UNASSIGNED_SEQ_NO || compareAndWriteSeqNo >=0;

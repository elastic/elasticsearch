...
public abstract class Engine implements Closeable {
    ...
    public static class Index extends Operation {
        ...
        private final long expectedSeqNo;
        private final long expectedPrimaryTerm;

        public Index(Term uid, ParsedDocument doc, long seqNo, long primaryTerm, long version, VersionType versionType, Origin origin,
                     long startTime, long autoGeneratedIdTimestamp, boolean isRetry, long expectedSeqNo, long expectedPrimaryTerm) {
            super(uid, seqNo, primaryTerm, version, versionType, origin, startTime);
            assert (origin == Origin.PRIMARY) == (versionType != null) : "invalid version_type=" + versionType + " for origin=" + origin;
            assert expectedPrimaryTerm >= 0 : "expectedPrimaryTerm [" + expectedPrimaryTerm + "] must be non negative";
            assert expectedSeqNo == SequenceNumbers.UNASSIGNED_SEQ_NO || expectedSeqNo >=0 :
                "expectedSeqNo [" + expectedSeqNo + "] must be non negative or unset";
            assert (origin == Origin.PRIMARY) || (expectedSeqNo == SequenceNumbers.UNASSIGNED_SEQ_NO && expectedPrimaryTerm == 0) :
                "cas operations are only allowed if origin is primary. get [" + origin + "]";
            this.doc = doc;
            this.isRetry = isRetry;
            this.autoGeneratedIdTimestamp = autoGeneratedIdTimestamp;
            this.expectedSeqNo = expectedSeqNo;
            this.expectedPrimaryTerm = expectedPrimaryTerm;
        }
        ...
        public long getExpectedSeqNo() {
            return expectedSeqNo;
        }

        public long getExpectedPrimaryTerm() {
            return expectedPrimaryTerm;
        }
    }
    ...
    public static class Delete extends Operation {
        ...
        public Delete(String type, String id, Term uid, long seqNo, long primaryTerm, long version, VersionType versionType,
                      Origin origin, long startTime, long expectedSeqNo, long expectedPrimaryTerm) {
            super(uid, seqNo, primaryTerm, version, versionType, origin, startTime);
            assert (origin == Origin.PRIMARY) == (versionType != null) : "invalid version_type=" + versionType + " for origin=" + origin;
            assert expectedPrimaryTerm >= 0 : "expectedPrimaryTerm [" + expectedPrimaryTerm + "] must be non negative";
            assert expectedSeqNo == SequenceNumbers.UNASSIGNED_SEQ_NO || expectedSeqNo >=0 :

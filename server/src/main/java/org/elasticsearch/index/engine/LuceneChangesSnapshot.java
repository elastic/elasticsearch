    ...
    private Translog.Operation readDocAsOp(int docIndex) throws IOException {
        ...
        if (isTombstone && fields.id() == null) {
            ...
            if (isTombstone) {
                ...
                /*
                 * In an ideal world we'd read the _tsid here and dump it into
                 * the Trasnlog.Index. But there isn't a space for it right now
                 * because Translog.Index is *exactly* the same thing that we
                 * fsync on every write operations. And we don't want to fsync
                 * the _tsid on every write operation. So we don't add it here.
                 * If we see _tsid regeneration taking a significant chunk of
                 * time in peer recovery or cross cluster replication then we
                 * have the option to read the _tsid here and attach it - so
                 * long as we don't add bytes to the Translog.Index that we
                 * fsync.
                 */

...
public final class LuceneChangesSnapshot extends SearchBasedChangesSnapshot {
        ...
        if (isTombstone && noId) {
            ...
            if (isTombstone) {
                ...
                op = sliceEnabled
                    ...
                    : new Translog.Index(id, seqNo, primaryTerm, version, source, routing, autoGeneratedIdTimestamp);
            }
        }
    ...
    // Captures raw _id bytes without decoding: slice indices store compound (id+slice) bytes which Uid.decodeId garbles.

...
final class LuceneChangesSnapshot implements Translog.Snapshot {
    ...
    private Translog.Operation readDocAsOp(int docIndex) throws IOException {
        ...
        if (isTombstone && fields.uid() == null) {
            ...
            if (isTombstone) {
                ...
                String parent = getParentDocValue(mapperService, leaf.reader(), type, segmentDocID);
                op = new Translog.Index(type, id, seqNo, primaryTerm, version, VersionType.EXTERNAL,
                    source.toBytesRef().bytes, fields.routing(), parent, autoGeneratedIdTimestamp);
            }
        }
    ...
    /**
     * Return the parent id from the {@link LeafReader} or null if the {@link ParentFieldMapper} is
     * not active.
     */
    private String getParentDocValue(MapperService mapperService, LeafReader leafReader,
                                        String type, int segmentDocId) throws IOException {
        ParentFieldMapper parentFieldMapper = mapperService.documentMapper(type).parentFieldMapper();
        if (parentFieldMapper.active() == false) {
            return null;
        }
        SortedDocValues dvs = leafReader.getSortedDocValues(parentFieldMapper.name());
        if (dvs != null && dvs.advanceExact(segmentDocId)) {
            BytesRef value = dvs.lookupOrd(dvs.ordValue());
            return value.length > 0 ? value.utf8ToString() : null;
        } else {
            return null;
        }
    }

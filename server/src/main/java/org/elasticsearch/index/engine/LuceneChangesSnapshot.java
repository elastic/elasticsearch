    ...
    private Translog.Operation readDocAsOp(int docIndex) throws IOException {
        ...
        if (isTombstone && fields.id() == null) {
            ...
            if (isTombstone) {
                ...
                // TODO store source XContentType in the Lucene index
                final XContentType sourceContentType = (source.length() == 0) ? XContentType.JSON : XContentHelper.xContentType(source);

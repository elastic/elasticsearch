...
public final class LuceneChangesSnapshot extends SearchBasedChangesSnapshot {
    ...
    private Translog.Operation readDocAsOp(int docIndex) throws IOException {
        ...
        if (isTombstone && fields.id() == null) {
            ...
            if (isTombstone) {
                // The Delete uid is self-describing: for slice indices id is "slice#id", so encodeId(id) reproduces the
                // composite term; for non-slice indices id is plain. Either way the translog Delete carries the exact term.
                ...
                // For slice indices the stored id is the composite "slice#id"; the translog Index must carry the PLAIN id
                // (+ routing = slice) so that replay re-parses and rebuilds the composite term exactly once.
                final String opId = sliceEnabled ? Uid.idFromCompositeId(id) : id;
                op = new Translog.Index(opId, seqNo, primaryTerm, version, source, routing, autoGeneratedIdTimestamp);
            }
        }

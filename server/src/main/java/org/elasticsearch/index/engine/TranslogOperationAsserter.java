...
public abstract class TranslogOperationAsserter {
        ...
        return new TranslogOperationAsserter() {
            ...
            public boolean assertSameIndexOperation(Translog.Index o1, Translog.Index o2) throws IOException {
                ...
                if (engineConfig.getIndexSettings().isRecoverySourceSyntheticEnabled()
                    ...
                    // since ordering might not be preserved. We synthesize both sides to handle mixed-cluster scenarios
                    // where the same vector may arrive in different encodings (e.g. hex string vs byte array) depending
                    // on whether the op came via replication from a new primary or recovery from an old primary.
                    final Translog.Index synthesized1 = synthesizeSource(engineConfig, o1);
                    final Translog.Index synthesized2 = synthesizeSource(engineConfig, o2);
                    return Translog.Index.equalsWithoutAutoGeneratedTimestamp(synthesized1, o2, false)
                        || Translog.Index.equalsWithoutAutoGeneratedTimestamp(o1, synthesized2, false)
                        || Translog.Index.equalsWithoutAutoGeneratedTimestamp(synthesized1, synthesized2, false);
                }
                return false;
            }
        };
    ...
    public boolean assertSameIndexOperation(Translog.Index o1, Translog.Index o2) throws IOException {
        return Translog.Index.equalsWithoutAutoGeneratedTimestamp(o1, o2, true)
            || Translog.Index.equalsWithoutAutoGeneratedTimestamp(o1, o2, false);
    }
}

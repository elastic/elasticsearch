...
public abstract class IndexNumericFieldData implements IndexFieldData<LeafNumericFieldData> {
    ...
    public SortField sortField(
        ...
        if (getNumericType().sortFieldType != SortField.Type.INT
            ...
            // single-valued index-sort fields use a plain SortField (not SortedNumericSortField); no INT→LONG rewrite needed
            || (indexSort && sortField instanceof SortedNumericSortField == false)) {
            return sortField;
        }
    ...
    protected static SortedNumericLongValues convertNumeric(SortedNumericLongValues values, LongUnaryOperator converter) {
        return new SortedNumericLongValues(values.isSingleton(), values.docIdIterator()) {

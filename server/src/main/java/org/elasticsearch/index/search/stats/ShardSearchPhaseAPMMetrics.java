...
public final class ShardSearchPhaseAPMMetrics implements SearchOperationListener {
    ...
    public static final String CAN_MATCH_PHASE_METRIC = "es.search.shards.phases.can_match.duration.histogram";
    ...
    public ShardSearchPhaseAPMMetrics(MeterRegistry meterRegistry) {
        this.canMatchPhaseMetric = meterRegistry.registerLongHistogram(
            CAN_MATCH_PHASE_METRIC,
            "Can Match search phase execution times at the shard level, expressed as a histogram",
            "ms"
        );
    ...
    public void onCanMatchPhase(ShardSearchRequest request, long tookInNanos) {
        recordPhaseLatency(canMatchPhaseMetric, tookInNanos, request);
    }

    @Override
    public void onDfsPhase(SearchContext searchContext, long tookInNanos) {
        recordPhaseLatency(dfsPhaseMetric, tookInNanos, searchContext.request());
    }

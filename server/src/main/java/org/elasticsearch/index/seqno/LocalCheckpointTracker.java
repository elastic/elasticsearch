...
public class LocalCheckpointTracker {
     ...
     * bit set size.
     */
    final LongObjectMap<CountedBitSet> processedSeqNo = new LongObjectHashMap<>();
     ...
     * the bit set size.
     */
    final LongObjectMap<CountedBitSet> persistedSeqNo = new LongObjectHashMap<>();
    ...
    private void markSeqNo(final long seqNo, final AtomicLong checkPoint, final LongObjectMap<CountedBitSet> bitSetMap) {
     ...
     */
    private void updateCheckpoint(AtomicLong checkPoint, LongObjectMap<CountedBitSet> bitSetMap) {
        assert Thread.holdsLock(this);
    ...
    private CountedBitSet getBitSetForSeqNo(final LongObjectMap<CountedBitSet> bitSetMap, final long seqNo) {
        assert Thread.holdsLock(this);
        final long bitSetKey = getBitSetKey(seqNo);
     ...
     * can be obtained via {@link #getBitSetForSeqNo(LongObjectMap, long)}.
     *

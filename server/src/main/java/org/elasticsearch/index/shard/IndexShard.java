...
public class IndexShard extends AbstractIndexShardComponent implements IndicesClusterStateService.Shard {
    ...
    public Store.MetadataSnapshot snapshotStoreMetadata() throws IOException {
        return snapshot(this::snapshotStoreMetadataFromEngine, recoverySource -> store.getMetadata(null, true));
    }
    ...
    public Store.RecoveryMetadataSnapshot snapshotStoreRecoveryMetadata() throws IOException {
        return snapshot(this::snapshotStoreRecoveryMetadataFromEngine, this::snapshotStoreRecoveryMetadataFromStore);
    }

    /**
     * snapshot data from either engine if available or else from store
     * @param engineSnapper snap data from engine, must be safe towards concurrent close.
     * @param storeSnapper snap data from store, will be run under mutex. Recovery source will be null if shard is closed.
     */
    private <T> T snapshot(CheckedFunction<Engine, T, IOException> engineSnapper,
                           CheckedFunction<RecoverySource, T, IOException> storeSnapper) throws IOException {
        store.incRef();
        try {
            Engine engine;
            ...
            synchronized (mutex) {
                ...
                if (engine == null) {
                    // take recovery source from shard routing since we are then sure it is available from the beginning of the IndexShard
                    // lifecycle.
                    assert shardRouting.initializing() || state == IndexShardState.CLOSED;
                    if (shardRouting.initializing()) {
                        return storeSnapper.apply(shardRouting.recoverySource());
                    } else {
                        return storeSnapper.apply(null);
                    }
                }
            }
            // engineSnaper must be safe towards concurrent engine close.
            return engineSnapper.apply(engine);
        } finally {
            store.decRef();
        }
    }

    private Store.MetadataSnapshot snapshotStoreMetadataFromEngine(Engine engine) throws IOException {
    ...
    private Store.RecoveryMetadataSnapshot snapshotStoreRecoveryMetadataFromEngine(Engine engine) throws IOException {
        try (Engine.IndexCommitRef lastCommit = engine.acquireLastIndexCommit(false)) {
            SeqNoStats seqNoStats = engine.getSeqNoStats(-1);
            MetadataSnapshot metadata = store.getMetadata(lastCommit.getIndexCommit());
            try (Engine.IndexCommitRef safeCommit = engine.acquireSafeIndexCommit()) {
                SequenceNumbers.CommitInfo safeCommitSeqNoInfo = Store.loadSeqNoInfo(safeCommit.getIndexCommit());
                ...
                long provideSeqNo = state() == IndexShardState.POST_RECOVERY || state() == IndexShardState.STARTED
                    ? safeCommitSeqNoInfo.localCheckpoint + 1
                    : Long.MAX_VALUE;
                return new Store.RecoveryMetadataSnapshot(metadata, provideSeqNo, safeCommitSeqNoInfo.localCheckpoint + 1,
                    seqNoStats.getMaxSeqNo());
            }
        }
    }

    private Store.RecoveryMetadataSnapshot snapshotStoreRecoveryMetadataFromStore(RecoverySource recoverySource) throws IOException {
        assert Thread.holdsLock(mutex);
        // if closed, PEER recovery or existing store, we know files are intact/not concurrently modified while holding mutex (except for
        // smaller modifications done under store writeLock).
        // "shouldBootstrapNewHistory" could really go in both buckets (not important since replica shard allocator will re-fetch data
        // from primary after it is started).
        if (recoverySource == null
            || recoverySource.getType() == RecoverySource.Type.PEER
            || recoverySource.getType() == RecoverySource.Type.EXISTING_STORE) {
            return store.getRecoveryMetadata(path, true);
        } else {
            // During restore from snapshot we have to wait until recovery is done before serving any meaningful data.
            // Likewise for local shards, though in principle, we could be smarter about those in the future.
            return Store.RecoveryMetadataSnapshot.EMPTY;
        }
    }
    ...
    public void finalizeIndexRecovery(CheckedRunnable<IOException> manipulateStore, long globalCheckpoint,
                                      MetadataSnapshot sourceMetaData) throws IOException {
        assert getEngineOrNull() == null;

        final Store store = store();
        store.incRef();
        try {
            // protect snapshotRecoveryMetadata readers from seeing invalid/half copied index data
            synchronized (mutex) {
                manipulateStore.run();
                try {

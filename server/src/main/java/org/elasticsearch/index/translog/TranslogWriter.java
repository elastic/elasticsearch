...
public class TranslogWriter extends BaseTranslogReader implements Closeable {
    ...
    public Translog.Location add(final Translog.Serialized operation, final long seqNo) throws IOException {
        ...
        synchronized (this) {
            ...
            assert assertNoSeqNumberConflict(seqNo, operation, false);
    ...
    private synchronized boolean assertNoSeqNumberConflict(Translog.IndexBatch batch) throws IOException {
        for (Translog.Operation op : batch.explode()) {
            ...
            assertNoSeqNumberConflict(op.seqNo(), operation, true);
        }
        return true;
    }

    private synchronized boolean assertNoSeqNumberConflict(long seqNo, Translog.Serialized serialized, boolean mapEquality) throws IOException {
        ...
        if (seenSequenceNumbers.containsKey(seqNo)) {
            ...
            if (previous.v1().equals(data) == false) {
                ...
                if (newOp instanceof final Translog.Index o2 && prvOp instanceof final Translog.Index o1) {
                    sameOp = Translog.Index.equalsWithoutAutoGeneratedTimestamp(o1, o2, mapEquality == false);

...
public class IndicesStore implements ClusterStateListener, Closeable {
    ...
    private class ShardActiveResponseHandler implements TransportResponseHandler<ShardActiveResponse> {
        ...
        public void handleException(TransportException exp) {
            logger.debug(() -> format(ROOT, "shards active request failed for %s", shardId), exp);
            if (awaitingResponses.decrementAndGet() == 0) {
                allNodesResponded();
            }
        }
            ...
            clusterService.getClusterApplierService()
                .runOnApplierThread("indices_store ([" + shardId + "] active fully on other nodes)", Priority.HIGH, currentState -> {
                    ...
                    try {
                        ...
                        logger.debug(() -> format(ROOT, "%s failed to delete unallocated shard, ignoring", shardId), ex);
                    }
                }, new ActionListener<>() {
                    @Override
                    ...
                    public void onFailure(Exception e) {
                        logger.error(() -> format(ROOT, "%s unexpected error during deletion of unallocated shard", shardId), e);
                    }
                });
        }

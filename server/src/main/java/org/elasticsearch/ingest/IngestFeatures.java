...
public class IngestFeatures implements FeatureSpecification {
    ...
    // Guards YAML tests for the cumulative field-value size limit (see IngestSettings.MAX_CUMULATIVE_FIELD_VALUE_BYTES) so they
    // don't run against older nodes in mixed-cluster/BWC tests, which don't have this setting registered.
    private static final NodeFeature INGEST_DOCUMENT_SIZE_LIMIT = new NodeFeature("ingest.document_size_limit", true);

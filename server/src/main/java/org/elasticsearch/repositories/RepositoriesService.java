...
public class RepositoriesService extends AbstractLifecycleComponent implements ClusterStateApplier {
    ...
    public void unregisterRepository(final DeleteRepositoryRequest request, final ActionListener<AcknowledgedResponse> listener) {
        submitUnbatchedTask("delete_repository [" + request.name() + "]", new AckedClusterStateUpdateTask(request, listener) {
    ...
    public void unregisterInternalRepository(String name) {
        ...
        if (repository != null) {
            ...
            logger.debug(() -> format(ROOT, "delete internal repository [%s][%s].", metadata.type(), name));
            closeRepository(repository);
        }
    }
    ...
    private static Repository createRepository(
        ...
        try {
            ...
            logger.warn(
                () -> format(ROOT, "failed to create repository [%s][%s]", repositoryMetadata.type(), repositoryMetadata.name()),
                e
            );

...
public final class RepositoryData {
    ...
    public Map<IndexId, Collection<String>> indexMetaDataToRemoveAfterRemovingSnapshots(Collection<SnapshotId> snapshotIds) {
        ...
        final Set<String> allRemainingIdentifiers = indexMetaDataGenerations.lookup()
            .entrySet()
            .stream()
    ...
    public XContentBuilder snapshotsToXContent(final XContentBuilder builder, final Version repoMetaVersion, boolean permitMissingUuid)
        ...
        for (final SnapshotId snapshot : getSnapshotIds()) {
            ...
            if (shouldWriteIndexGens) {
                ...
                for (Map.Entry<IndexId, String> entry : indexMetaDataGenerations.lookup()
                    .getOrDefault(snapshot, Collections.emptyMap())
                    .entrySet()) {
    ...
    public record SnapshotDetails(
        @Nullable SnapshotState snapshotState, // TODO forbid nulls here, this only applies to very old repositories
        @Nullable Version version,  // may be omitted if pre-7.6 nodes were involved somewhere
        long startTimeMillis,       // May be -1 if unknown, which happens if the snapshot was taken before 7.14 and hasn't been updated yet
        long endTimeMillis,         // May be -1 if unknown, which happens if the snapshot was taken before 7.14 and hasn't been updated yet
        @Nullable String slmPolicy  // May be null if unknown, which happens if the snapshot was taken before 7.16 and hasn't been updated
                                    // yet. Empty string indicates that this snapshot was not created by an SLM policy.
    ) {

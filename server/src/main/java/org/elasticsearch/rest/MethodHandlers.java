...
final class MethodHandlers {
    ...
    private final Map<RestRequest.Method, Map<CompatibleVersion, RestHandler>> methodHandlers;
     ...
     */
    RestHandler getHandler(RestRequest.Method method, CompatibleVersion version) {
        Map<CompatibleVersion, RestHandler> versionToHandlers = methodHandlers.get(method);
        if (versionToHandlers == null) {
            return null; //method not found
        }
        final RestHandler handler = versionToHandlers.get(version);
        return handler == null ? versionToHandlers.get(CompatibleVersion.CURRENT) : handler;

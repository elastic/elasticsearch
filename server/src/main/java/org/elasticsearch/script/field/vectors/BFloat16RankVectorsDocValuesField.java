...
public class BFloat16RankVectorsDocValuesField extends RankVectorsDocValuesField {
    ...
    public void setNextDocId(int docId) throws IOException {
        ...
        if (input.advanceExact(docId)) {
            ...
            assert value.length % (Short.BYTES * dims) == 0;
            numVectors = value.length / (Short.BYTES * dims);
    ...
    @Override
    public int size() {
        return value == null ? 0 : value.length / (Short.BYTES * dims);
    }
    ...
    public static class BFloat16VectorIterator implements VectorIterator<float[]> {
        ...
        public BFloat16VectorIterator(BytesRef vectorValues, float[] buffer, int size) {
            assert vectorValues.length == (buffer.length * Short.BYTES * size);
        ...
        public float[] next() {
            ...
            for (int i = 0; i < buffer.length; i++) {
                buffer[i] = Float.intBitsToFloat(vectorValues.get() << 16);
            }
            idx++;
            return buffer;
        }

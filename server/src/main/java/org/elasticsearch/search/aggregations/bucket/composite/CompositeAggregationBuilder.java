        ...
        if (after != null) {
            ...
            for (int i = 0; i < sources.size(); i++) {
                ...
                if (configs[i].missingBucket() && obj == null) {
                    values[i] = null;
                } else if (obj instanceof Comparable<?> c && configs[i].fieldType().getClass() != TimeSeriesIdFieldType.class) {
                    values[i] = c;
                ...
                } else if (obj instanceof String && configs[i].fieldType().getClass() == TimeSeriesIdFieldType.class) {
                    // If input is a _tsid string, encode the string to the _tsid BytesRef
                    values[i] = configs[i].format().parseBytesRef(obj);
                } else {
                    throw new IllegalArgumentException(

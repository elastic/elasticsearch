...
public class FiltersAggregator extends BucketsAggregator {
    ...
    public FiltersAggregator(String name, AggregatorFactories factories, String[] keys, Supplier<Weight[]> filters, boolean keyed,
            String otherBucketKey, SearchContext context, Aggregator parent, TotalBucketCardinality parentCardinality,
            Map<String, Object> metadata) throws IOException {
        super(name, factories, context, parent,
            parentCardinality.forKnownBucketAggregator(keys.length + (otherBucketKey == null ? 0 : 1)), metadata);
        this.keyed = keyed;
        this.keys = keys;

...
class SignificanceLookup {
    ...
    SignificanceLookup(
        ...
        // Rewrite to account for alias filters, if there are any.
        if (backgroundQuery == null) {
            Query matchAllDocsQuery = new MatchAllDocsQuery();
            Query rewrittenMatchAll = context.filterQuery(matchAllDocsQuery);
            if (rewrittenMatchAll != matchAllDocsQuery) {
                this.backgroundFilter = rewrittenMatchAll;
            } else {

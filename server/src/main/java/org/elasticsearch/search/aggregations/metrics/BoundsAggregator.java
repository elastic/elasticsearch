...
public abstract class BoundsAggregator<T extends SpatialPoint> extends MetricsAggregator {
    ...
    protected LeafBucketCollector getLeafBucketCollector(MultiPointValues<T> values, LeafBucketCollector sub) {
            ...
            public void collect(int doc, long bucket) throws IOException {
                if (values.advanceExact(doc)) {
                    ...
                    for (int i = 0; i < valuesCount; ++i) {
                        ...
                        if (value.getX() >= 0 && value.getX() < posLeft) {
                            posLeft = value.getX();
                        }
                        double posRight = posRights.get(bucket);
                        if (value.getX() >= 0 && value.getX() > posRight) {
                            posRight = value.getX();
                        }
                        double negLeft = negLefts.get(bucket);
                        if (value.getX() < 0 && value.getX() < negLeft) {
                            negLeft = value.getX();
                        }
                        double negRight = negRights.get(bucket);
                        if (value.getX() < 0 && value.getX() > negRight) {
                            negRight = value.getX();
                        }
                        tops.set(bucket, top);
    ...
    public InternalAggregation buildAggregation(long owningBucketOrdinal) {
        ...
        return makeInternalBounds(name, top, bottom, posLeft, posRight, negLeft, negRight, metadata());
    }

    @Override
    public InternalAggregation buildEmptyAggregation() {
        return makeInternalBounds(
            name,

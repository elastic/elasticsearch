...
public class SumBucketFunction implements AggregatorBucketFunction<Double> {
    private final BigArrays bigArrays;
    private DoubleArray value;

    public SumBucketFunction(BigArrays bigArrays) {
        this.bigArrays = bigArrays;
        value = bigArrays.newDoubleArray(1, true);
        value.fill(0, value.size(), 0);
    }

    @Override
    public String name() {
        return Aggregator.sum.name();
    }
    ...
    public void collect(Double number, long bucket) {
        if (bucket >= value.size()) {
            ...
            value.fill(from, value.size(), 0);
        }

        double current = value.get(bucket);
        value.set(bucket, current + number);
    }
    ...
    public InternalAggregation getAggregation(long bucket, Map<String, Object> aggregatorParams, DocValueFormat formatter, Map<String, Object> metadata) {
        return new org.elasticsearch.search.aggregations.metrics.Sum(name(), value.get(bucket), formatter, metadata);
    }

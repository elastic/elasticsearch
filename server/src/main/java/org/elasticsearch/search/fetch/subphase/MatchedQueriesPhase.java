...
public final class MatchedQueriesPhase implements FetchSubPhase {
    ...
    public void hitsExecute(SearchContext context, SearchHit[] hits) {
        if (hits.length == 0 ||
            ...
            context.originalQuery() == null) {
            return;
        }

        if (context.getNamedQueries().isEmpty()) {
            return;
        }
        ...
        try {
            for (Query query : context.getNamedQueries()) {
                String name;
                assert query instanceof NamedSpanQuery || query instanceof NamedQuery;
                if (query instanceof NamedQuery) {
                    name = ((NamedQuery)query).getName();
                } else {
                    name = ((NamedSpanQuery)query).getName();
                }
                int readerIndex = -1;
                int docBase = -1;

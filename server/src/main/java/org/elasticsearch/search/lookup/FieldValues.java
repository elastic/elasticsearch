...
public interface FieldValues<T> {
     ...
     * @param context the search execution context
     * @param transformer a function that transform field values into formatter values
     * @return the value fetcher
     */
    static <V, T> ValueFetcher valueFetcher(FieldValues<V> fieldValues, Function<List<T>, List<Object>> formatter,
                                            SearchExecutionContext context, Function<V, T> transformer) {
        return new ValueFetcher() {
            LeafReaderContext ctx;
            ...
            public List<Object> fetchValues(SourceLookup lookup)  {
                ...
                try {
                    fieldValues.valuesForDoc(context.lookup(), ctx, lookup.docId(), v -> values.add(transformer.apply(v)));
                } catch (Exception e) {
                    // ignore errors - if they exist here then they existed at index time
                    // and so on_script_error must have been set to `ignore`
                }

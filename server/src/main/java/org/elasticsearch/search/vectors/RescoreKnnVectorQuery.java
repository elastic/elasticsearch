...
public abstract class RescoreKnnVectorQuery extends Query implements QueryProfilerProvider {
    ...
    private static class DirectRescoreKnnVectorQuery extends Query {
        ...
        private void rescore(
            int docBase,
            RescorableVectorValues rescorableVectorValues,
            List<ScoreDoc> queue,
            DocIdSetIterator filterIterator
        ) throws IOException {
            RescorableVectorValues.VectorReScorer vectorReScorer = rescorableVectorValues.rescorer(floatTarget);
            var iterator = vectorReScorer.iterator();
            RescorableVectorValues.VectorReScorer.Bulk bulkScorer = vectorReScorer.bulk(filterIterator);
            RescorableVectorValues.DocAndFloatFeatureBuffer buffer = new RescorableVectorValues.DocAndFloatFeatureBuffer();

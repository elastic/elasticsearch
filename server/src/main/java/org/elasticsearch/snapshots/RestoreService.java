                ...
                public ClusterState execute(ClusterState currentState) {
                    ...
                    if (indices.isEmpty() == false) {
                        ...
                        for (Map.Entry<String, String> indexEntry : indices.entrySet()) {
                            ...
                            snapshotIndexMetaData = updateIndexSettings(snapshotIndexMetaData, request.indexSettings(),
                            ...
                            if (currentIndexMetaData == null) {
                                ...
                                IndexMetaData.Builder indexMdBuilder = IndexMetaData.builder(snapshotIndexMetaData)
                                                                                    .state(IndexMetaData.State.OPEN);
                                indexMdBuilder.version(Math.max(snapshotIndexMetaData.getVersion(),
                                                                currentIndexMetaData.getVersion() + 1));
                ...
                private void validateExistingIndex(IndexMetaData currentIndexMetaData, IndexMetaData snapshotIndexMetaData,
                    ...
                    if (currentIndexMetaData.getState() != IndexMetaData.State.CLOSE) {
                        ...
                        throw new SnapshotRestoreException(snapshot, "cannot restore index [" + renamedIndex + "] because an open index" +
                            " with same name already exists in the cluster. Either close or delete the existing " +
                            "index or restore the index under a different name by providing a rename pattern and replacement name");
                    }

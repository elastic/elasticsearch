 ...
 */
public final class SnapshotsServiceUtils {

    static void ensureSnapshotNameNotRunning(SnapshotsInProgress runningSnapshots, String repositoryName, String snapshotName) {
     ...
     */
    static void ensureNoCleanupInProgress(
    ...
    static void ensureSnapshotNameAvailableInRepo(RepositoryData repositoryData, String snapshotName, Repository repository) {
     ...
     */
    static void validate(String repositoryName, String snapshotName, ClusterState state) {
    ...
    static void validate(final String repositoryName, final String snapshotName) {
     ...
     * to be assigned
     */
    static boolean assertNoDanglingSnapshots(ClusterState state) {
     ...
     */
    static boolean isWritingToRepository(SnapshotsInProgress.Entry entry) {
    ...
    static boolean isQueued(@Nullable SnapshotsInProgress.ShardSnapshotStatus status) {
        return status != null && status.state() == SnapshotsInProgress.ShardState.QUEUED;
    }

    static boolean removedNodesCleanupNeeded(SnapshotsInProgress snapshotsInProgress, List<DiscoveryNode> removedNodes) {
    ...
    static boolean waitingShardsStartedOrUnassigned(SnapshotsInProgress snapshotsInProgress, ClusterChangedEvent event) {
    ...
    static ShardGenerations buildGenerations(SnapshotsInProgress.Entry snapshot, Metadata metadata) {
    ...
    static Metadata metadataForSnapshot(SnapshotsInProgress.Entry snapshot, Metadata metadata) {
     ...
     */
    static List<SnapshotFeatureInfo> onlySuccessfulFeatureStates(SnapshotsInProgress.Entry entry, List<String> finalIndices) {
     ...
     */
    static SnapshotsInProgress.ShardSnapshotStatus initShardSnapshotStatus(
     ...
     */
    static ImmutableOpenMap<ShardId, SnapshotsInProgress.ShardSnapshotStatus> shards(
     ...
     */
    public static Tuple<ClusterState, List<SnapshotDeletionsInProgress.Entry>> readyDeletions(ClusterState currentState) {
        ...
        for (SnapshotDeletionsInProgress.Entry entry : deletions.getEntries()) {
            ...
            if (repositoriesSeen.add(entry.repository())
                ...
                && snapshotsInProgress.forRepo(repo).stream().noneMatch(SnapshotsServiceUtils::isWritingToRepository)) {
                changed = true;
    ...
    @Nullable
    static <T> ImmutableOpenMap.Builder<T, SnapshotsInProgress.ShardSnapshotStatus> maybeAddUpdatedAssignment(
    ...
    public static ClusterState stateWithoutSnapshot(ClusterState state, Snapshot snapshot, ShardGenerations shardGenerations) {
        ...
        final List<SnapshotsInProgress.Entry> entryList = inProgressSnapshots.forRepo(snapshot.getRepository());
        ...
        if (indexOfEntry >= 0) {
            ...
            result = ClusterState.builder(state)
                .putCustom(
                    ...
                    inProgressSnapshots.withUpdatedEntriesForRepo(snapshot.getRepository(), updatedEntries)
                )
                .build();
        }
     ...
     */
    @Nullable
    static SnapshotDeletionsInProgress deletionsWithoutSnapshots(
    ...
    static <T> void completeListenersIgnoringException(@Nullable List<ActionListener<T>> listeners, T result, Logger logger) {
        if (listeners != null) {
            try {
    ...
    static Level snapshotFailureLogLevel(Exception e) {

 ...
 * being snapshotted and creates a {@code SnapshotsInProgress.SnapshotInProgressEntry} with state {@code STARTED} and adds the map of
 ...
 * marked as in state {@code WAITING}. In case a shard's primary is relocated at any point after its
 * {@code SnapshotsInProgress.SnapshotInProgressEntry} was created and thus been assigned to a specific cluster node, that shard's snapshot
 * will fail and move to state {@code FAILED}.</li>
 *
 * <li>The new {@code SnapshotsInProgress.SnapshotInProgressEntry} is then observed by
 ...
 * The master node will then update a shard's state in the snapshots {@code SnapshotsInProgress.SnapshotInProgressEntry} whenever it
 * receives such a {@code UpdateIndexShardSnapshotStatusRequest}.</li>
 *
 ...
 * submitted and the removal of the snapshot's {@code SnapshotsInProgress.SnapshotInProgressEntry} from the cluster state completes the
 * snapshot process.</li>
 * </ol>
 *
 ...
 *
 * <ol>
 * <li>Aborting a snapshot starts by updating the state of the snapshot's {@code SnapshotsInProgress.SnapshotInProgressEntry} to
 * {@code ABORTED}.</li>
 *
 ...
 *     {@link org.elasticsearch.cluster.SnapshotsInProgress.SnapshotInProgressEntry#shardSnapshotStatusByRepoShardId()} are not created in
 *     the initial cluster state update as is done for shard snapshot assignments in
 *     {@link org.elasticsearch.cluster.SnapshotsInProgress.SnapshotInProgressEntry#shards}. This is due to the fact that shard snapshot
 *     assignments are computed purely from information in the current cluster state while shard clone assignments require information to be
 *     read from the repository, which is too slow of a process to be done inside a cluster state update. Loading this information ahead of
 *     creating a task in the cluster state, runs the risk of race conditions where the source snapshot is being deleted before the clone
 *     task is enqueued in the cluster state.</li>
 ...
 *     {@code SnapshotsInProgress.SnapshotInProgressEntry#clones} map for the clone operation with the the relevant shard clone tasks.</li>
 *     <li>After the clone tasks have been added to the {@code SnapshotsInProgress.SnapshotInProgressEntry}, master executes them on its
 *     snapshot thread-pool by invoking {@link org.elasticsearch.repositories.Repository#cloneShardSnapshot} for each shard that is to be
 *     cloned. Each completed shard snapshot triggers a call to the
 *     {@link org.elasticsearch.snapshots.SnapshotsService#masterServiceTaskQueue} which updates the clone's
 *     {@code SnapshotsInProgress.SnapshotInProgressEntry} to mark the shard clone operation completed.</li>
 *     <li>Once all the entries in {@code SnapshotsInProgress.SnapshotInProgressEntry#clones} have completed, the clone is finalized just
 *     like any other snapshot through {@link org.elasticsearch.snapshots.SnapshotsService#endSnapshot}. The only difference being that the
 *     metadata that is written out for indices and the global metadata are read from the source snapshot in the repository instead of the
 *     cluster state.</li>
 * </ol>
 *
 ...
 * {@link org.elasticsearch.cluster.SnapshotDeletionsInProgress.SnapshotDeletionState#STARTED}. If it cannot be executed when it is received
 * it will be set to state {@link org.elasticsearch.cluster.SnapshotDeletionsInProgress.SnapshotDeletionState#WAITING} initially.
 ...
 * {@link org.elasticsearch.cluster.SnapshotDeletionsInProgress.SnapshotDeletionEntry} in the cluster state will be updated to cover both
 * the snapshots in the existing delete as well as additional snapshots that may be found in the second delete request.
 *

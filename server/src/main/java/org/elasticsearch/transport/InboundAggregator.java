...
public class InboundAggregator implements Releasable {
    ...
    private final Function<String, CircuitBreaker> circuitBreaker;
    ...
    public InboundAggregator(Function<String, CircuitBreaker> circuitBreaker,
    ...
    // Visible for testing
    InboundAggregator(Function<String, CircuitBreaker> circuitBreaker, Predicate<String> requestCanTripBreaker) {
    ...
    public InboundMessage finishAggregation() throws IOException {
        ...
        final BreakerControl breakerControl = new BreakerControl(circuitBreaker, currentHeader.getActionName());
    ...
    private void checkBreaker(final Header header, final int contentLength, final BreakerControl breakerControl) {
        ...
        if (canTripBreaker) {
            try {
                circuitBreaker.apply(header.getActionName()).addEstimateBytesAndMaybeBreak(contentLength, header.getActionName());
                breakerControl.setReservedBytes(contentLength);
            } catch (CircuitBreakingException e) {
                shortCircuit(e);
            }
        } else {
            circuitBreaker.apply(header.getActionName()).addWithoutBreaking(contentLength);
            breakerControl.setReservedBytes(contentLength);
        }
    }
    ...
    private static class BreakerControl implements Releasable {
        ...
        private BreakerControl(Function<String, CircuitBreaker> circuitBreaker, String actionName) {
            this.circuitBreaker = circuitBreaker;
        ...
        public void close() {
            ...
            if (toRelease > 0) {
                circuitBreaker.apply(actionName).addWithoutBreaking(-toRelease);
            }
        }
    }
}

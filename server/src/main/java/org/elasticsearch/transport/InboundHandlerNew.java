...
public class InboundHandlerNew {

    private static final Logger logger = LogManager.getLogger(InboundHandlerNew.class);
    ...
    InboundHandlerNew(ThreadPool threadPool, OutboundHandler outboundHandler, InboundMessage.Reader reader,
...
//            message.getStoredContext().restore();
            ...
            if (header.isRequest()) {
                // TODO: Length
//                handleRequest(channel, message, reference.length());
            } else {
                ...
                if (handler != null) {
                    if (header.isError()) {
                        handlerResponseError(message.getContent(), handler);
                    } else {
                        handleResponse(remoteAddress, message.getContent(), handler);
                    }
                    // Check the entire message has been read
                    final int nextByte = message.getContent().read();
    ...
    private void handleRequest(TcpChannel channel, AggregatedMessage message, int messageLengthBytes) {
        Header header = message.getHeader();

        final String action = header.getAction();
        final long requestId = header.getRequestId();
        final Version version = header.getVersion();
        final StreamInput stream = message.getContent();
        ...
        try {
            ...
            if (header.isHandshake()) {
                ...
                transportChannel = new TcpTransportChannel(outboundHandler, channel, action, requestId, version,
                    circuitBreakerService, messageLengthBytes, header.isCompress());
            ...
            if (transportChannel == null) {
                transportChannel = new TcpTransportChannel(outboundHandler, channel, action, requestId, version,
                    circuitBreakerService, 0, header.isCompress());
            }
            try {

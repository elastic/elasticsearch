...
public class CancelTasksRequestTests extends ESTestCase {
    ...
    private static CancellableTask cancellableTaskWithOriginal(long id, String action, TaskId parent, TaskId originalTaskId) {
        return new CancellableTask(id, "transport", action, "desc", parent, Collections.emptyMap()) {
            @Override
            public Optional<TaskId> getOriginalTaskId() {
                return Optional.of(originalTaskId);
            }
        };
    }
    ...
    public void testMatch_acceptsByNumericIdOnTargetNode() {
        CancelTasksRequest request = new CancelTasksRequest();
        request.setTargetTaskId(new TaskId("node-a", 42));
        // Transport action guarantees the caller has resolved the target id first (direct TaskManager lookup on the target node), so the
        // task here has numeric id matching the target's numeric id.
        assertTrue(request.match(cancellableTask(42, REINDEX_ACTION, TaskId.EMPTY_TASK_ID)));
    }

    public void testMatch_acceptsRelocatedTaskByOriginalTaskId() {
        CancelTasksRequest request = new CancelTasksRequest();
        TaskId originalTaskId = new TaskId("node-a", 42);
        request.setTargetTaskId(originalTaskId);
        // Relocated top-level reindex on a different node: new numeric id, but originalTaskId carries the full original TaskId.
        assertTrue(request.match(cancellableTaskWithOriginal(99, REINDEX_ACTION, TaskId.EMPTY_TASK_ID, originalTaskId)));
    }

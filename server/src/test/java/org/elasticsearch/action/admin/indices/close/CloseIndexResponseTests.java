...
public class CloseIndexResponseTests extends ESTestCase {
    ...
    private CloseIndexResponse randomResponse() {
        ...
        return new CloseIndexResponse(indexResults);
    }

    private ElasticsearchException randomException(final Index index, final int id) {
        return randomFrom(new IndexNotFoundException(index),
            new ActionNotFoundTransportException("test"),
            new NoShardAvailableActionException(new ShardId(index, id)));
    }

    private void assertCloseIndexResponse(final CloseIndexResponse actual, final CloseIndexResponse expected) {

...
public class ShardBatchMapperParseTests extends IndexShardTestCase {
    ...
    private static IndexRequest indexRequest(String id) {
        return new IndexRequest("index").id(id).source("{}", XContentType.JSON);
    }

    private static EirfBatch emptyDocBatch(int numDocs) {
    ...
    public void testColumnarBatchOnEmptyDocuments() throws Exception {
        String mapping = """
            {
              "properties": {
                "host":  { "type": "keyword" },
    ...
    public void testColumnarBatchFallsBackWhenDocumentsHaveFields() throws Exception {
        String mapping = """
            {
              "properties": {
                "host": { "type": "keyword" }
        ...
        BulkItemRequest[] items = { new BulkItemRequest(0, indexRequest("1")) };
        try (EirfRowBuilder builder = new EirfRowBuilder()) {
            builder.startDocument();

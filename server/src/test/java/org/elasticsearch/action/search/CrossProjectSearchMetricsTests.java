...
public class CrossProjectSearchMetricsTests extends ESTestCase {
    ...
    public void testSerializationRoundTripIncludesSearchPhases() throws IOException {
        CrossProjectSearchMetrics original = new CrossProjectSearchMetrics();
        original.trackPreProcessingTookTime(10L);
        original.trackPlanningPhaseTookTime(25L);
        original.trackProjectRoundtripTime("project-a", 100L);
        original.trackProjectRoundtripTime("project-b", 200L);
        original.trackMergingPhaseTookTime(15L);
        original.trackSearchPhaseTookTime("query", 230L);
        original.trackSearchPhaseTookTime("fetch", 18L);

        BytesStreamOutput out = new BytesStreamOutput();
        original.writeTo(out);

        try (StreamInput in = out.bytes().streamInput()) {
            CrossProjectSearchMetrics deserialized = new CrossProjectSearchMetrics(in);
            assertEquals(original, deserialized);
            assertEquals(Map.of("query", 230L, "fetch", 18L), deserialized.getSearchPhaseTookTimes());
        }
    }

    public void testSerializationRoundTripWithoutSearchPhases() throws IOException {
        CrossProjectSearchMetrics original = new CrossProjectSearchMetrics();
        original.trackPlanningPhaseTookTime(25L);
        original.trackMergingPhaseTookTime(15L);
        ...
        try (StreamInput in = out.bytes().streamInput()) {
            CrossProjectSearchMetrics deserialized = new CrossProjectSearchMetrics(in);
            assertEquals(original, deserialized);
            assertTrue(deserialized.getSearchPhaseTookTimes().isEmpty());
        }
    }

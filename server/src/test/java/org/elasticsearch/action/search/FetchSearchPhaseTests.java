...
public class FetchSearchPhaseTests extends ESTestCase {
    ...
    private void assertProfiles(boolean profiled, int totalShards, SearchResponse searchResponse) {
        if (false == profiled) {
            assertThat(searchResponse.getProfileShardResults(), equalTo(Map.of()));
            return;
        }
        assertThat(searchResponse.getProfileShardResults().values().size(), equalTo(totalShards));
        for (SearchProfileShardResult profileShardResult : searchResponse.getProfileShardResults().values()) {
    ...
    public void testFailFetchOneDoc() throws Exception {
        ...
        try (
            ...
            if (profiled) {
                 ...
                 */
                assertThat(searchResponse.getProfileShardResults().values().size(), equalTo(2));
                assertThat(searchResponse.getProfileShardResults().get(shard1Target.toString()).getFetchPhase(), nullValue());
                assertThat(
                    searchResponse.getProfileShardResults().get(shard2Target.toString()).getFetchPhase().getTime(),
    ...
    public void testFetchDocsConcurrently() throws Exception {
        ...
        try (
            ...
            if (profiled) {
                assertThat(searchResponse.getProfileShardResults().values().size(), equalTo(numHits));
                int count = 0;
                for (SearchProfileShardResult profileShardResult : searchResponse.getProfileShardResults().values()) {
    ...
    public void testCleanupIrrelevantContexts() throws Exception { // contexts that are not fetched should be cleaned up
        ...
        try (
            ...
            assertEquals(2, searchResponse.getSuccessfulShards());
            if (profiled) {
                assertThat(searchResponse.getProfileShardResults().size(), equalTo(2));
                assertThat(searchResponse.getProfileShardResults().get(shard1Target.toString()).getFetchPhase(), nullValue());
                assertThat(
                    searchResponse.getProfileShardResults().get(shard2Target.toString()).getFetchPhase().getTime(),
                    equalTo(FETCH_PROFILE_TIME)

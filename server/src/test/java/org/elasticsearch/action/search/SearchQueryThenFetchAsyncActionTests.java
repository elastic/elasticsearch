...
public class SearchQueryThenFetchAsyncActionTests extends ESTestCase {
    ...
    public void testMinimumVersionBetweenNewAndOldVersion() throws Exception {
        ...
        Version minVersion = VersionUtils.randomVersionBetween(random(), oldVersion, newVersion);
    ...
    private void testMixedVersionsShardsSearch(Version oldVersion, Version newVersion, Version minVersion) throws Exception {
        ...
        QueryPhaseResultConsumer resultConsumer = new QueryPhaseResultConsumer(withMinVersionSearchRequest, executor,
            new NoopCircuitBreaker(CircuitBreaker.REQUEST), controller, task.getProgressListener(), writableRegistry(),
            shardsIter.size(), exc -> {});
        ThrowingRunnable newSearchAsyncAction = () -> new SearchQueryThenFetchAsyncAction(logger,
            searchTransportService, (clusterAlias, node) -> lookup.get(node),
            Collections.singletonMap("_na_", new AliasFilter(null, Strings.EMPTY_ARRAY)),
            Collections.emptyMap(), Collections.emptyMap(), controller, executor,
        ...
        Exception e = expectThrows(VersionMismatchException.class, newSearchAsyncAction);

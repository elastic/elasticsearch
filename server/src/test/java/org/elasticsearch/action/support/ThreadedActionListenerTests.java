...
public class ThreadedActionListenerTests extends ESTestCase {
    ...
    public void testRejectionHandling() throws InterruptedException {
        final int listenerCount = between(1, 1000);
        ...
        try {
            final List<String> pools = randomNonEmptySubsetOf(
                org.elasticsearch.core.List.of(
                    "fixed-bounded-queue",
                    "fixed-unbounded-queue",
                    "scaling-drop-if-shutdown",
                    "scaling-reject-if-shutdown"
                )
            );
            final Set<String> shutdownUnsafePools = org.elasticsearch.core.Set.of("fixed-bounded-queue", "scaling-drop-if-shutdown");

            threadPool.generic().execute(() -> {
                for (int i = 0; i < listenerCount; i++) {
                    final String pool = randomFrom(pools);
                    final ActionListener<Void> listener = new ThreadedActionListener<Void>(
                        logger,
                        threadPool,
                        pool,

...
public class ReplicationOperationTests extends ESTestCase {
    ...
    /**
     * Verifies that {@link ReplicationOperation#handlePrimaryResult} samples the global checkpoint <em>before</em>
     * sampling the replication group. If the global checkpoint were sampled after the replication group, a race
     * would allow the GCP to be based on a <em>subset</em> of the sampled replication group: a shard that was
     * in-sync when the group was read could drop out of the in-sync set before the GCP is read, causing the GCP
     * to advance beyond what the shards in the sampled group allow. Replicas receiving that advanced GCP would
     * learn a global checkpoint higher than their own local checkpoint.
     */
    public void testGlobalCheckpointSampledBeforeReplicationGroup() throws Exception {
        final String index = "test";
        final ShardId shardId = new ShardId(index, "_na_", 0);

        // One primary (on the local node) + one started replica (on a remote node).
        final ClusterState state = state(index, true, ShardRoutingState.STARTED, ShardRoutingState.STARTED);
        ...
        final Set<String> trackedShards = indexShardRoutingTable.getPromotableAllocationIds();
        final ReplicationGroup replicationGroup = new ReplicationGroup(indexShardRoutingTable, inSyncAllocationIds, trackedShards, 0);

        // Simulate: when the replication group is first sampled, a previously in-sync shard drops out of the
        // in-sync set (e.g., it is marked stale). That removal allows the GCP to advance to gcpAfterGroupSampled
        // (no longer constrained by the removed shard's local checkpoint). If ReplicationOperation were to
        // sample the GCP after the group, it would observe the higher value and forward it to replicas that may
        // not yet have reached that checkpoint.
        final long gcpBeforeGroupSampled = 10L;
        final long gcpAfterGroupSampled = 100L;
        final AtomicReference<Long> currentGcp = new AtomicReference<>(gcpBeforeGroupSampled);
        final AtomicBoolean firstGroupFetch = new AtomicBoolean(false);

        final TestPrimary primary = new TestPrimary(primaryShard, () -> {
            // Mutate exactly once, on the first getReplicationGroup() call.
            if (firstGroupFetch.compareAndSet(false, true)) {
                currentGcp.set(gcpAfterGroupSampled);
            }
            return replicationGroup;
        }, threadPool) {
            @Override
            public long computedGlobalCheckpoint() {
                return currentGcp.get();
            }
        };

        final AtomicReference<Long> capturedGlobalCheckpoint = new AtomicReference<>(null);
        final TestReplicaProxy replicasProxy = new TestReplicaProxy() {
            ...
            public void performOn(
                ...
                assertTrue(
                    "global checkpoint should be captured exactly once",
                    capturedGlobalCheckpoint.compareAndSet(null, globalCheckpoint)
                );
        ...
        // Sanity: the replica must have received the operation.
        assertNotNull("performOn must have been called for the replica", capturedGlobalCheckpoint.get());
        // The global checkpoint forwarded to replicas must be the value sampled *before* the replication group
        // was retrieved, not the value that advanced after a shard dropped from the in-sync set.
        assertThat(capturedGlobalCheckpoint.get(), equalTo(gcpBeforeGroupSampled));
    }

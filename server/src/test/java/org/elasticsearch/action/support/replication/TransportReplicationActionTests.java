...
public class TransportReplicationActionTests extends ESTestCase {
    ...
    */
    ...
    public void testBlocksInReroutePhase() {
        ...
        ShardId shardId = new ShardId(clusterService.state().metadata().projectMetadata.index("index").getIndex(), 0);
    ...
    public void testBlocksInPrimaryAction() {
        ...
        final long primaryTerm = clusterState.metadata().projectMetadata.index(index).primaryTerm(shardId.id());
    ...
    public void testReplicaProxy() throws InterruptedException, ExecutionException {
        ...
        setState(clusterService, state);
        final long primaryTerm = state.metadata().projectMetadata.index(index).primaryTerm(0);
    ...
    public void testSeqNoIsSetOnPrimary() {
        ...
        final long primaryTerm = clusterService.state().metadata().projectMetadata.index(index).primaryTerm(shardId.id());
        Request request = new Request(shardId);
        ...
        Set<String> inSyncIds = randomBoolean()
            ...
            : clusterService.state().metadata().projectMetadata.index(index).inSyncAllocationIds(0);
    ...
    public void testDefaultWaitForActiveShardsUsesIndexSetting() {
        ...
        action.resolveRequest(state.metadata().projectMetadata.index(indexName), request);
    ...
    public void testPrimaryActionRejectsWrongAidOrWrongTerm() throws Exception {
        ...
        final long primaryTerm = clusterService.state().metadata().projectMetadata.index(shardId.getIndexName()).primaryTerm(shardId.id());
        PlainActionFuture<TransportResponse> listener = new PlainActionFuture<>();
    ...
    public void testRetryOnReplica() throws Exception {
        ...
        final long primaryTerm = state.metadata().projectMetadata.index(shardId.getIndexName()).primaryTerm(shardId.id());
        // simulate execution of the node holding the replica
        state = ClusterState.builder(state).nodes(DiscoveryNodes.builder(state.nodes()).localNodeId(replica.currentNodeId())).build();
        setState(clusterService, state);
    ...
    public void testRetryOnReplicaWithRealTransport() throws Exception {
        ...
        final long primaryTerm = initialState.metadata().projectMetadata.index(shardId.getIndexName()).primaryTerm(shardId.id());
        // simulate execution of the node holding the replica

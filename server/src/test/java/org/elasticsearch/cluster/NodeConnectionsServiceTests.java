...
public class NodeConnectionsServiceTests extends ESTestCase {
    ...
    public void testConcurrentConnectAndDisconnect() throws Exception {
        ...
        final DiscoveryNodes nodes = discoveryNodesFromList(Collections.singletonList(node));
        ...
        final Semaphore reconnectPermits = new Semaphore(1000);
        final int reconnectThreads = 10;
        final CountDownLatch reconnectCountDown = new CountDownLatch(reconnectThreads);
    ...
    private static void closeConnection(TransportService transportService, DiscoveryNode discoveryNode) {
        try {
            final Transport.Connection connection = transportService.getConnection(discoveryNode);
            connection.close();

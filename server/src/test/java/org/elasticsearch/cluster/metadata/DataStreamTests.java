...
public class DataStreamTests extends AbstractXContentSerializingTestCase<DataStream> {
    ...
    public void testValidate() {
        {
            ...
            assertThat(
                IndexSettings.TIME_SERIES_START_TIME.get(
                    clusterState.getMetadata().projectMetadata.index(dataStream.getIndices().get(0)).getSettings()
                ),
                equalTo(start1)
            );
            assertThat(
            ...
            assertThat(
                IndexSettings.TIME_SERIES_START_TIME.get(
                    clusterState.getMetadata().projectMetadata.index(dataStream.getIndices().get(1)).getSettings()
                ),
                equalTo(start2)
            );
            assertThat(
    ...
    public void testGetIndicesPastRetention() {
...

        {
            ...
            assertThat(
                dataStream.getIndicesPastRetention(metadata.getProject()::index, () -> now, randomGlobalRetention()).isEmpty(),
                is(true)
            );
        }

        {
            ...
            List<Index> backingIndices = dataStream.getIndicesPastRetention(metadata.getProject()::index, () -> now, globalRetention);
            assertThat(backingIndices.size(), is(2));
    ...
    public void testGetIndicesPastRetentionWithOriginationDate() {
        ...
        Metadata metadata = metadataBuilder.build();
        {
            ...
            assertThat(dataStream.getIndicesPastRetention(metadata.getProject()::index, () -> now, null).isEmpty(), is(true));
        }
    ...
    public void testGetDownsampleRounds() {
...

        {
            ...
            Index thirdIndex = metadata.projectMetadata.index(thirdGeneration).getIndex();
            ...
            Index firstIndex = metadata.projectMetadata.index(firstGeneration).getIndex();

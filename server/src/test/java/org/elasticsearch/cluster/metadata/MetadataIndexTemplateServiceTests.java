...
public class MetadataIndexTemplateServiceTests extends ESSingleNodeTestCase {
    ...
    public void testLegacyNoopUpdate() throws IOException {
        ...
        state = MetadataIndexTemplateService.innerPutTemplate(state.projectState(projectId), pr, new IndexTemplateMetadata.Builder("id"));

        assertNotNull(state.metadata().getProject(projectId).templates().get("id"));

        assertThat(
            MetadataIndexTemplateService.innerPutTemplate(state.projectState(projectId), pr, new IndexTemplateMetadata.Builder("id")),
            equalTo(state)
        );
    }
    ...
    public void testAddComponentTemplate() throws Exception {
        ...
        IllegalArgumentException e = expectThrows(
            ...
            () -> metadataIndexTemplateService.addComponentTemplate(throwState.projectState(projectId), true, "foo", componentTemplate)
        );
        assertThat(e.getMessage(), containsString("component template [foo] already exists"));

        state = metadataIndexTemplateService.addComponentTemplate(state.projectState(projectId), randomBoolean(), "bar", componentTemplate);
        assertNotNull(state.metadata().getProject(projectId).componentTemplates().get("bar"));
        ...
        expectThrows(
            ...
            () -> metadataIndexTemplateService.addComponentTemplate(throwState.projectState(projectId), true, "foo2", componentTemplate2)
        );
        ...
        expectThrows(
            ...
            () -> metadataIndexTemplateService.addComponentTemplate(throwState.projectState(projectId), true, "foo2", componentTemplate3)
        );
        ...
        expectThrows(
            IllegalArgumentException.class,
            () -> metadataIndexTemplateService.addComponentTemplate(throwState.projectState(projectId), true, "foo2", componentTemplate4)
        );
    }
    ...
    public void testUpdateComponentTemplateWithIndexHiddenSetting() throws Exception {
        ...
        ComponentTemplate componentTemplate = new ComponentTemplate(template, 1L, new HashMap<>());
        state = service.addComponentTemplate(state.projectState(projectId), true, "foo", componentTemplate);
        assertNotNull(state.metadata().getProject(projectId).componentTemplates().get("foo"));
        ...
        ComposableIndexTemplate firstGlobalIndexTemplate = ComposableIndexTemplate.builder()
            ...
            .priority(1L)
            .build();
        state = service.addIndexTemplateV2(state.projectState(projectId), true, "globalindextemplate1", firstGlobalIndexTemplate);
        ...
        ComposableIndexTemplate secondGlobalIndexTemplate = ComposableIndexTemplate.builder()
            ...
            .priority(2L)
            .build();
        state = service.addIndexTemplateV2(state.projectState(projectId), true, "globalindextemplate2", secondGlobalIndexTemplate);
        ...
        ComposableIndexTemplate fooPatternIndexTemplate = ComposableIndexTemplate.builder()
            ...
            .priority(3L)
            .build();
        state = service.addIndexTemplateV2(state.projectState(projectId), true, "foopatternindextemplate", fooPatternIndexTemplate);
        ...
        try {
            service.addComponentTemplate(state.projectState(projectId), false, "foo", updatedComponentTemplate);
            fail(
    ...
    public void testAddIndexTemplateV2() throws Exception {
        ...
        state = metadataIndexTemplateService.addIndexTemplateV2(state.projectState(projectId), false, "foo", template);
        ...
        IllegalArgumentException e = expectThrows(
            IllegalArgumentException.class,
            () -> metadataIndexTemplateService.addIndexTemplateV2(throwState.projectState(projectId), true, "foo", newTemplate)
        );
        assertThat(e.getMessage(), containsString("index template [foo] already exists"));

        state = metadataIndexTemplateService.addIndexTemplateV2(state.projectState(projectId), randomBoolean(), "bar", newTemplate);
        assertNotNull(state.metadata().getProject(projectId).templatesV2().get("bar"));
    }
    ...
    public void testRemoveIndexTemplateV2() throws Exception {
        ...
        IndexTemplateMissingException e = expectThrows(
            ...
            () -> MetadataIndexTemplateService.innerRemoveIndexTemplateV2(initialState.projectState(projectId), "foo")
        );
        assertThat(e.getMessage(), equalTo("index_template [foo] missing"));

        final ClusterState state = service.addIndexTemplateV2(initialState.projectState(projectId), false, "foo", template);
        assertNotNull(state.metadata().getProject(projectId).templatesV2().get("foo"));
        assertTemplatesEqual(state.metadata().getProject(projectId).templatesV2().get("foo"), template);

        ClusterState updatedState = MetadataIndexTemplateService.innerRemoveIndexTemplateV2(state.projectState(projectId), "foo");
        assertNull(updatedState.metadata().getProject(projectId).templatesV2().get("foo"));
    }
    ...
    public void testRemoveIndexTemplateV2Wildcards() throws Exception {
        ...
        ClusterState initialState = ClusterState.builder(ClusterState.EMPTY_STATE)
            .putProjectMetadata(ProjectMetadata.builder(projectId).build())
            .build();
        ClusterState result = MetadataIndexTemplateService.innerRemoveIndexTemplateV2(initialState.projectState(projectId), "*");
        assertThat(result, sameInstance(initialState));

        ClusterState state = metadataIndexTemplateService.addIndexTemplateV2(initialState.projectState(projectId), false, "foo", template);
        assertThat(state.metadata().getProject(projectId).templatesV2().get("foo"), notNullValue());

        assertTemplatesEqual(state.metadata().getProject(projectId).templatesV2().get("foo"), template);

        Exception e = expectThrows(
            IndexTemplateMissingException.class,
            () -> MetadataIndexTemplateService.innerRemoveIndexTemplateV2(state.projectState(projectId), "foob*")
        );
        assertThat(e.getMessage(), equalTo("index_template [foob*] missing"));

        ClusterState updatedState = MetadataIndexTemplateService.innerRemoveIndexTemplateV2(state.projectState(projectId), "foo*");
        assertThat(updatedState.metadata().getProject(projectId).templatesV2().get("foo"), nullValue());
    }
    ...
    public void testRemoveMultipleIndexTemplateV2() throws Exception {
        ...
        ClusterState initialState = ClusterState.builder(ClusterState.EMPTY_STATE)
            .putProjectMetadata(ProjectMetadata.builder(projectId).build())
            .build();
        ClusterState state = service.addIndexTemplateV2(initialState.projectState(projectId), false, "foo", fooTemplate);
        state = service.addIndexTemplateV2(state.projectState(projectId), false, "bar", barTemplate);
        state = service.addIndexTemplateV2(state.projectState(projectId), false, "baz", bazTemplate);
        var project = state.metadata().getProject(projectId);
        assertNotNull(project.templatesV2().get("foo"));
        ...
        ClusterState updatedState = MetadataIndexTemplateService.innerRemoveIndexTemplateV2(state.projectState(projectId), "foo", "baz");
        assertNull(updatedState.metadata().getProject(projectId).templatesV2().get("foo"));
        assertNotNull(updatedState.metadata().getProject(projectId).templatesV2().get("bar"));
        assertNull(updatedState.metadata().getProject(projectId).templatesV2().get("baz"));
    }
    ...
    public void testRemoveMultipleIndexTemplateV2Wildcards() throws Exception {
        ...
        {
            ClusterState cs = service.addIndexTemplateV2(initialState.projectState(projectId), false, "foo", fooTemplate);
            cs = service.addIndexTemplateV2(cs.projectState(projectId), false, "bar", barTemplate);
            state = service.addIndexTemplateV2(cs.projectState(projectId), false, "baz", bazTemplate);
        }

        Exception e = expectThrows(
            IndexTemplateMissingException.class,
            () -> MetadataIndexTemplateService.innerRemoveIndexTemplateV2(state.projectState(projectId), "foo", "b*", "k*", "*")
        );
    ...
    public void testPuttingV2TemplateGeneratesWarning() throws Exception {
        ...
        ComposableIndexTemplate v2Template = ComposableIndexTemplate.builder()
            ...
            .build();
        state = metadataIndexTemplateService.addIndexTemplateV2(state.projectState(projectId), false, "v2-template", v2Template);
        ...
        assertNotNull(state.metadata().getProject(projectId).templatesV2().get("v2-template"));
        assertTemplatesEqual(state.metadata().getProject(projectId).templatesV2().get("v2-template"), v2Template);
    }
    ...
    public void testPuttingV1StarTemplateGeneratesWarning() throws Exception {
        ...
        ClusterState initialState = ClusterState.builder(ClusterState.EMPTY_STATE)
            .putProjectMetadata(ProjectMetadata.builder(projectId).build())
            .build();
        ClusterState state = service.addIndexTemplateV2(initialState.projectState(projectId), false, "v2-template", v2Template);
        ...
        state = MetadataIndexTemplateService.innerPutTemplate(
            state.projectState(projectId),
            req,
            IndexTemplateMetadata.builder("v1-template")
        );
        ...
        assertNotNull(state.metadata().getProject(projectId).templates().get("v1-template"));
        assertThat(state.metadata().getProject(projectId).templates().get("v1-template").patterns(), containsInAnyOrder("*", "baz"));
    }
    ...
    public void testPuttingV1NonStarTemplateGeneratesError() throws Exception {
        ...
        IllegalArgumentException e = expectThrows(
            IllegalArgumentException.class,
            () -> MetadataIndexTemplateService.innerPutTemplate(
                state.projectState(projectId),
                req,
                IndexTemplateMetadata.builder("v1-template")
            )
        ...
        assertNull(state.metadata().getProject(projectId).templates().get("v1-template"));
    }
    ...
    public void testUpdatingV1NonStarTemplateWithUnchangedPatternsGeneratesWarning() throws Exception {
        ...
        assertNotNull(state.metadata().getProject(projectId).templates().get("v1-template"));
        assertThat(state.metadata().getProject(projectId).templates().get("v1-template").patterns(), containsInAnyOrder("fo*", "baz"));
    }
    ...
    public void testUpdatingV1NonStarWithChangedPatternsTemplateGeneratesError() throws Exception {
        ...
        IllegalArgumentException e = expectThrows(
            IllegalArgumentException.class,
            () -> MetadataIndexTemplateService.innerPutTemplate(
                finalState.projectState(projectId),
                req,
                IndexTemplateMetadata.builder("v1-template")
            )
        );
    ...
    public void testPuttingOverlappingV2Template() throws Exception {
            ...
            .build();
        {
            ComposableIndexTemplate template = ComposableIndexTemplate.builder()
                ...
                .build();
            MetadataIndexTemplateService service = getMetadataIndexTemplateService();
            ClusterState state = service.addIndexTemplateV2(initialState.projectState(projectId), false, "foo", template);
            ...
            IllegalArgumentException e = expectThrows(
                IllegalArgumentException.class,
                () -> service.addIndexTemplateV2(state.projectState(projectId), false, "foo2", newTemplate)
            );
            assertThat(
                e.getMessage(),
                equalTo(
    ...
    public void testFindV2Templates() throws Exception {
        ...
        ClusterState state = ClusterState.builder(ClusterState.EMPTY_STATE)
            .putProjectMetadata(ProjectMetadata.builder(projectId).build())
            .build();
        assertNull(MetadataIndexTemplateService.findV2Template(state.metadata().getProject(projectId), "index", randomBoolean()));

        ComponentTemplate ct = ComponentTemplateTests.randomNonDeprecatedInstance();
        state = service.addComponentTemplate(state.projectState(projectId), true, "ct", ct);
        ...
        ComposableIndexTemplate it2 = ComposableIndexTemplate.builder()
            ...
            .version(2L)
            .build();
        state = service.addIndexTemplateV2(state.projectState(projectId), true, "my-template2", it2);

        String result = MetadataIndexTemplateService.findV2Template(state.metadata().getProject(projectId), "index", randomBoolean());
    ...
    public void testFindV2TemplatesForHiddenIndex() throws Exception {
        ...
        ClusterState state = ClusterState.builder(ClusterState.EMPTY_STATE)
            .putProjectMetadata(ProjectMetadata.builder(projectId).build())
            .build();
        assertNull(MetadataIndexTemplateService.findV2Template(state.metadata().getProject(projectId), "index", true));
        ...
        String result = MetadataIndexTemplateService.findV2Template(state.metadata().getProject(projectId), "index", true);
    ...
    public void testFindV2TemplatesForDateMathIndex() throws Exception {
        ...
        ClusterState state = ClusterState.builder(ClusterState.EMPTY_STATE)
            .putProjectMetadata(ProjectMetadata.builder(projectId).build())
            .build();
        assertNull(MetadataIndexTemplateService.findV2Template(state.metadata().getProject(projectId), indexName, true));
        ...
        String result = MetadataIndexTemplateService.findV2Template(state.metadata().getProject(projectId), indexName, true);
    ...
    public void testResolveConflictingMappings() throws Exception {
        ...
        state = service.addComponentTemplate(state.projectState(projectId), true, "ct_high", ct1);
        state = service.addComponentTemplate(state.projectState(projectId), true, "ct_low", ct2);
        ComposableIndexTemplate it = ComposableIndexTemplate.builder()
            .indexPatterns(List.of("i*"))
    ...
    public void testDefinedTimestampMappingIsAddedForDataStreamTemplates() throws Exception {
        ...
        state = service.addComponentTemplate(state.projectState(projectId), true, "ct1", ct1);

        {
            ComposableIndexTemplate it = ComposableIndexTemplate.builder()
                ...
                .build();
            state = service.addIndexTemplateV2(state.projectState(projectId), true, "logs-data-stream-template", it);
...

        {
            ...
            ComposableIndexTemplate it = ComposableIndexTemplate.builder()
                ...
                .version(1L)
                .build();
            state = service.addIndexTemplateV2(state.projectState(projectId), true, "timeseries-template", it);
    ...
    public void testUserDefinedMappingTakesPrecedenceOverDefault() throws Exception {
...

        {
            ...
            ComposableIndexTemplate it = ComposableIndexTemplate.builder()
                ...
                .dataStreamTemplate(new ComposableIndexTemplate.DataStreamTemplate())
                .build();
            state = service.addIndexTemplateV2(state.projectState(projectId), true, "logs-template", it);
    ...
    public void testResolveSettings() throws Exception {
        ...
        Settings settings = MetadataIndexTemplateService.resolveSettings(state.metadata().getProject(projectId), "my-template");
    ...
    private void assertLifecycleResolution(
        ...
        ComposableIndexTemplate it = ComposableIndexTemplate.builder()
            ...
            .dataStreamTemplate(new ComposableIndexTemplate.DataStreamTemplate())
            .build();
        state = service.addIndexTemplateV2(state.projectState(state.metadata().getProject().id()), true, "my-template", it);
    ...
    public void testRemoveComponentTemplate() throws Exception {
        ...
        ClusterState initialState = ClusterState.builder(ClusterState.EMPTY_STATE)
            ...
            .build();
        final MetadataIndexTemplateService service = getMetadataIndexTemplateService();
        ClusterState temp = service.addComponentTemplate(initialState.projectState(projectId), false, "foo", foo);
        temp = service.addComponentTemplate(temp.projectState(projectId), false, "bar", bar);
        final ClusterState clusterState = service.addComponentTemplate(temp.projectState(projectId), false, "baz", baz);

        ClusterState result = innerRemoveComponentTemplate(clusterState.projectState(projectId), "foo");
        assertThat(result.metadata().getProject(projectId).componentTemplates().get("foo"), nullValue());
        assertThat(result.metadata().getProject(projectId).componentTemplates().get("bar"), equalTo(bar));
        assertThat(result.metadata().getProject(projectId).componentTemplates().get("baz"), equalTo(baz));

        result = innerRemoveComponentTemplate(clusterState.projectState(projectId), "bar", "baz");
        assertThat(result.metadata().getProject(projectId).componentTemplates().get("foo"), equalTo(foo));
        assertThat(result.metadata().getProject(projectId).componentTemplates().get("bar"), nullValue());
        assertThat(result.metadata().getProject(projectId).componentTemplates().get("baz"), nullValue());

        Exception e = expectThrows(
            ResourceNotFoundException.class,
            () -> innerRemoveComponentTemplate(clusterState.projectState(projectId), "foobar")
        );
        assertThat(e.getMessage(), equalTo("foobar"));
        e = expectThrows(
            ResourceNotFoundException.class,
            () -> innerRemoveComponentTemplate(clusterState.projectState(projectId), "foo", "barbaz", "foobar")
        );
        assertThat(e.getMessage(), equalTo("barbaz,foobar"));

        result = innerRemoveComponentTemplate(clusterState.projectState(projectId), "*");
        assertThat(result.metadata().getProject(projectId).componentTemplates().size(), equalTo(0));

        result = innerRemoveComponentTemplate(clusterState.projectState(projectId), "b*");
        assertThat(result.metadata().getProject(projectId).componentTemplates().size(), equalTo(1));
        assertThat(result.metadata().getProject(projectId).componentTemplates().get("foo"), equalTo(foo));

        e = expectThrows(
            ResourceNotFoundException.class,
            () -> innerRemoveComponentTemplate(clusterState.projectState(projectId), "foo", "b*")
        );
    ...
    public void testRemoveComponentTemplateInUse() throws Exception {
        ...
        ClusterState initialState = ClusterState.builder(ClusterState.EMPTY_STATE)
            ...
            .build();
        final MetadataIndexTemplateService service = getMetadataIndexTemplateService();
        ClusterState clusterState = service.addComponentTemplate(initialState.projectState(projectId), false, "ct", ct);
        clusterState = service.addIndexTemplateV2(clusterState.projectState(projectId), false, "template", template);

        final ClusterState cs = clusterState;
        Exception e = expectThrows(IllegalArgumentException.class, () -> innerRemoveComponentTemplate(cs.projectState(projectId), "c*"));
        assertThat(
            e.getMessage(),
    ...
    public void testRemoveRequiredAndNonRequiredComponents() throws Exception {
        ...
        ClusterState initialState = ClusterState.builder(ClusterState.EMPTY_STATE)
            .putProjectMetadata(ProjectMetadata.builder(projectId).build())
            .build();
        ClusterState clusterState = service.addComponentTemplate(initialState.projectState(projectId), false, "required1", ct);
        clusterState = service.addComponentTemplate(clusterState.projectState(projectId), false, "required2", ct);
        clusterState = service.addComponentTemplate(clusterState.projectState(projectId), false, "non-required", ct);
        clusterState = service.addIndexTemplateV2(
        ...
        Exception e = expectThrows(
            IllegalArgumentException.class,
            () -> innerRemoveComponentTemplate(cs.projectState(projectId), "required*")
        );
        assertThat(
            e.getMessage(),
            containsString(
        ...
        innerRemoveComponentTemplate(cs.projectState(projectId), "non-required*");
    }
    ...
    public void testIndexTemplateFailsToOverrideComponentTemplateMappingField() throws Exception {
        ...
        state = service.addComponentTemplate(state.projectState(projectId), true, "c1", ct1);
        state = service.addComponentTemplate(state.projectState(projectId), true, "c2", ct2);
        ComposableIndexTemplate it = ComposableIndexTemplate.builder()
            .indexPatterns(List.of("i*"))
            .template(new Template(null, new CompressedXContent("""
                {
        ...
        IllegalArgumentException e = expectThrows(
            IllegalArgumentException.class,
            () -> service.addIndexTemplateV2(finalState.projectState(projectId), randomBoolean(), "my-template", it)
        );
    ...
    public void testUpdateComponentTemplateFailsIfResolvedIndexTemplatesWouldBeInvalid() throws Exception {
        ...
        state = service.addIndexTemplateV2(state.projectState(projectId), randomBoolean(), "my-template", it);
        ...
        IllegalArgumentException e = expectThrows(
            IllegalArgumentException.class,
            () -> service.addComponentTemplate(finalState.projectState(projectId), false, "c2", changedCt2)
        );
    ...
    public void testPutExistingComponentTemplateIsNoop() throws Exception {
        ...
        assertThat(
            metadataIndexTemplateService.addComponentTemplate(state.projectState(projectId), false, "foo", componentTemplate),
            equalTo(state)
        );
    }
    ...
    public void testPutExistingComposableTemplateIsNoop() throws Exception {
        ...
        assertThat(metadataIndexTemplateService.addIndexTemplateV2(state.projectState(projectId), false, "foo", template), equalTo(state));
    }
    ...
    public void testUnreferencedDataStreamsWhenAddingTemplate() throws Exception {
        ...
        state = service.addIndexTemplateV2(state.projectState(projectId), false, "logs", template);

        ClusterState stateWithDS = ClusterState.builder(state)
            .putProjectMetadata(
                ProjectMetadata.builder(state.metadata().getProject(projectId))
                    .put(
                        DataStreamTestHelper.newInstance(
                            "logs-mysql-default",
        ...
        IllegalArgumentException e = expectThrows(IllegalArgumentException.class, () -> {
            ComposableIndexTemplate nonDSTemplate = ComposableIndexTemplate.builder()
                ...
                .priority(100L)
                .build();
            service.addIndexTemplateV2(stateWithDS.projectState(projectId), false, "logs", nonDSTemplate);
        });
        ...
        e = expectThrows(IllegalArgumentException.class, () -> {
            ComposableIndexTemplate nonDSTemplate = ComposableIndexTemplate.builder()
                ...
                .priority(105L)
                .build();
            service.addIndexTemplateV2(stateWithDS.projectState(projectId), false, "logs2", nonDSTemplate);
        });
        ...
        e = expectThrows(IllegalArgumentException.class, () -> {
            ComposableIndexTemplate newTemplate = ComposableIndexTemplate.builder()
                ...
                .dataStreamTemplate(new ComposableIndexTemplate.DataStreamTemplate())
                .build();
            service.addIndexTemplateV2(stateWithDS.projectState(projectId), false, "logs", newTemplate);
        });
        ...
        ComposableIndexTemplate mysqlTemplate = ComposableIndexTemplate.builder()
            ...
            .dataStreamTemplate(new ComposableIndexTemplate.DataStreamTemplate())
            .build();
        ClusterState stateWithDSAndTemplate = service.addIndexTemplateV2(
            stateWithDS.projectState(projectId),
            false,
            "logs-mysql",
            mysqlTemplate
        );
        ...
        ComposableIndexTemplate nonDSTemplate = ComposableIndexTemplate.builder()
            ...
            .priority(100L)
            .build();
        service.addIndexTemplateV2(stateWithDSAndTemplate.projectState(projectId), false, "logs", nonDSTemplate);
    }
    ...
    public void testDataStreamsUsingTemplates() throws Exception {
        ...
        state = service.addIndexTemplateV2(stateWithDS.projectState(projectId), false, "logs2", fineGrainedLogsTemplate);

        // Test replacing it with a version without the data stream config
        final ClusterState stateWithTwoTemplates = state;
        IllegalArgumentException e = expectThrows(
            IllegalArgumentException.class,
            () -> MetadataIndexTemplateService.innerRemoveIndexTemplateV2(stateWithTwoTemplates.projectState(projectId), "logs*")
        );
        ...
        assertThat(
            MetadataIndexTemplateService.dataStreamsExclusivelyUsingTemplates(state.metadata().getProject(projectId), Set.of("logs")),
            equalTo(Set.of())
        );
        assertThat(
            MetadataIndexTemplateService.findV2Template(state.metadata().getProject(projectId), "logs-mysql-default", false),
            equalTo("logs2")
        );
    ...
    public void testDataStreamsUsingMatchAllTemplate() throws Exception {
        ...
        state = service.addIndexTemplateV2(state.projectState(projectId), false, templateName, template);
        // When creating a data stream, we'll look for templates. The data stream is not hidden
        assertThat(
            MetadataIndexTemplateService.findV2Template(state.metadata().getProject(projectId), "some-data-stream", false),
        ...
        assertThat(
            MetadataIndexTemplateService.findV2Template(state.metadata().getProject(projectId), "some-data-stream", true),
            equalTo(templateName)
        );
    }
    ...
    public void testRemovingHigherOrderTemplateOfDataStreamWithMultipleTemplates() throws Exception {
        ...
        state = service.addIndexTemplateV2(stateWithDS.projectState(projectId), false, "logs-test", fineGrainedLogsTemplate);
        ...
        state = MetadataIndexTemplateService.innerRemoveIndexTemplateV2(state.projectState(projectId), "logs-test");
    ...
    public void testV2TemplateOverlaps() throws Exception {
...

        {
            ...
            IllegalArgumentException e = expectThrows(
                IllegalArgumentException.class,
                () -> MetadataIndexTemplateService.v2TemplateOverlaps(state.metadata().getProject(projectId), "foo2", newTemplate, true)
            );
            assertThat(
                e.getMessage(),
                equalTo(
    ...
    public void testIgnoreMissingComponentTemplateValid() throws Exception {
        ...
        ClusterState initialState = ClusterState.builder(ClusterState.EMPTY_STATE)
            .putProjectMetadata(ProjectMetadata.builder(projectId).build())
            .build();
        ClusterState state = service.addIndexTemplateV2(initialState.projectState(projectId), false, indexTemplateName, template);
        MetadataIndexTemplateService.validateV2TemplateRequest(state.metadata().getProject(projectId), indexTemplateName, template);
    }
    ...
    public void testComposableTemplateWithSubobjectsFalse() throws Exception {
        ...
        state = service.addComponentTemplate(state.projectState(projectId), true, "subobjects", subobjects);
        state = service.addComponentTemplate(state.projectState(projectId), true, "field_mapping", fieldMapping);
        ComposableIndexTemplate it = ComposableIndexTemplate.builder()
            ...
            .version(1L)
            .build();
        state = service.addIndexTemplateV2(state.projectState(projectId), true, "composable-template", it);
    ...
    public void testAddIndexTemplateWithDeprecatedComponentTemplate() throws Exception {
        ...
        ComposableIndexTemplate it = ComposableIndexTemplate.builder()
            ...
            .version(1L)
            .build();
        service.addIndexTemplateV2(state.projectState(projectId), false, "foo", it);

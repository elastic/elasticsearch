...
public class RoutingNodeTests extends ESTestCase {
    ...
    public void testShardsWithState() {
        assertThat(routingNode.shardsWithState(ShardRoutingState.STARTED).count(), equalTo(1));
        assertThat(routingNode.shardsWithState(ShardRoutingState.RELOCATING).count(), equalTo(1));
        assertThat(routingNode.shardsWithState(ShardRoutingState.INITIALIZING).count(), equalTo(1));
    }

    public void testShardsWithStateInIndex() {
        assertThat(routingNode.shardsWithState("test", ShardRoutingState.INITIALIZING, ShardRoutingState.STARTED).count(), equalTo(2));
        assertThat(routingNode.shardsWithState("test", ShardRoutingState.STARTED).count(), equalTo(1));
        assertThat(routingNode.shardsWithState("test", ShardRoutingState.RELOCATING).count(), equalTo(1));
        assertThat(routingNode.shardsWithState("test", ShardRoutingState.INITIALIZING).count(), equalTo(1));
    }

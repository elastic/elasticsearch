...
public class ShardIteratorTests extends ESTestCase {
        ...
        for (int i = 0; i < numShards; i++) {
            for (String index : indices) {
                for (String uuid : uuids) {
                    ...
                    shardIterators.add(new ShardIterator(shardId, GroupShardsIteratorTests.randomShardRoutings(shardId)));
                }
            }
        }
    ...
    private static ShardIterator randomPlainShardIterator() {
        ...
        return new ShardIterator(shardId, GroupShardsIteratorTests.randomShardRoutings(shardId));
    }
}

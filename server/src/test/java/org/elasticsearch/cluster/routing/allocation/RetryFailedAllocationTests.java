...
public class RetryFailedAllocationTests extends ESAllocationTestCase {
    ...
    public void testRetryFailedResetForAllocationCommands() {
        ...
        clusterState = strategy.reroute(clusterState, "initial allocation", ActionListener.noop()).clusterState();
        ...
        for (int i = 0; i < retries; i++) {
            ...
            clusterState = strategy.reroute(clusterState, "allocation retry attempt-" + i, ActionListener.noop()).clusterState();
        }
        assertThat("replica should not be assigned", getReplica().state(), equalTo(ShardRoutingState.UNASSIGNED));
        assertThat(
            "reroute should be a no-op",
            strategy.reroute(clusterState, "test", ActionListener.noop()).clusterState(),

...
public class ClusterApplierRecordingServiceTests extends ESTestCase {
    ...
    public void testRecorder() {
        ...
        Recorder recorder = new Recorder(() -> currentTime[0]);
        {
        ...
        List<Tuple<String, Long>> recordings = recorder.getRecordings();
    ...
    public void testRecorderAlreadyRecording() {
        Recorder recorder = new Recorder(() -> 1L);
        Releasable releasable = recorder.record("action1");
        expectThrows(IllegalStateException.class, () -> recorder.record("action2"));
    }

    public void testRecordingServiceStats() {
        ClusterApplierRecordingService service = new ClusterApplierRecordingService();

        {
            long[] currentTime = new long[1];
            Recorder recorder = new Recorder(() -> currentTime[0]);
            try (Releasable r = recorder.record("action1")) {
                currentTime[0] = 5;
            }
            try (Releasable r = recorder.record("action2")) {
                currentTime[0] = 42;
            }
            try (Releasable r = recorder.record("action3")) {
                currentTime[0] = 45;
            }

...
public class TransportVersionsFixupListenerTests extends ESTestCase {
    ...
    public void testVersionsAreFixed() {
        ...
        verify(client).nodesInfo(
            argThat(transformed(NodesInfoRequest::nodesIds, arrayContainingInAnyOrder("node1", "node2"))),
            action.capture()
        );
    ...
    public void testConcurrentChangesDoNotOverlap() {
        ...
        verify(client).nodesInfo(argThat(transformed(NodesInfoRequest::nodesIds, arrayContainingInAnyOrder("node1", "node2"))), any());

...
public class ClusterShardLimitIT extends ESIntegTestCase {
    ...
    @Before
    public void enableEnforceShardLimit() {
        Settings enforceShardLimitSettings = Settings.builder()
            .put(MetaData.SETTING_ENFORCE_CLUSTER_MAX_SHARDS_PER_NODE.getKey(), true)
            .build();
        if (frequently()) {
            client().admin().cluster()
                .prepareUpdateSettings()
                .setPersistentSettings(enforceShardLimitSettings)
                .get();
        } else {
            client().admin().cluster()
                .prepareUpdateSettings()
                .setTransientSettings(enforceShardLimitSettings)
                .get();
        }
    }

    public void testDisableEnforceShardLimit() {
        // Disable enforcement
        Settings disableEnforcement = Settings.builder()
            .put(MetaData.SETTING_ENFORCE_CLUSTER_MAX_SHARDS_PER_NODE.getKey(), "default")
            .build();
        client().admin().cluster()
            .prepareUpdateSettings()
            .setTransientSettings(disableEnforcement)
            .get();

        // Should be able to create and index that takes us over the limit
        ...
        prepareCreate("should-pass", Settings.builder()
            .put(indexSettings())
        ...
        assertTrue(clusterState.getMetaData().hasIndex("should-pass"));
    }

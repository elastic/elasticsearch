...
public class FsBlobContainerTests extends ESTestCase {
    ...
    public void testReadBlobRangeCorrectlySkipBytes() throws IOException {
        ...
        try (InputStream stream = container.readBlob(BlobPurpose.SNAPSHOT, blobName, start, length)) {
            assertThat(totalBytesRead.get(), equalTo(0L));
    ...
    public void testDeleteIgnoringIfNotExistsDoesNotThrowFileNotFound() throws IOException {
        ...
        container.deleteBlobsIgnoringIfNotExists(BlobPurpose.SNAPSHOT, List.of(blobName).listIterator());
        // Should not throw exception
        container.deleteBlobsIgnoringIfNotExists(BlobPurpose.SNAPSHOT, List.of(blobName).listIterator());

        assertFalse(container.blobExists(BlobPurpose.SNAPSHOT, blobName));
    }
    ...
    public void testCompareAndExchange() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            switch (between(1, 4)) {
                case 1 -> assertEquals(expectedValue.get(), getBytesAsync(l -> container.getRegister(BlobPurpose.SNAPSHOT, key, l)));
                case 2 -> assertFalse(
                    getAsync(
            ...
            if (randomBoolean()) {
                assertTrue(getAsync(l -> container.compareAndSetRegister(BlobPurpose.SNAPSHOT, key, expectedValue.get(), newValue, l)));
            } else {
                assertEquals(
                    expectedValue.get(),
                    getBytesAsync(l -> container.compareAndExchangeRegister(BlobPurpose.SNAPSHOT, key, expectedValue.get(), newValue, l))
                );
            }
            expectedValue.set(newValue);
        }

        container.writeBlob(BlobPurpose.SNAPSHOT, key, new BytesArray(new byte[17]), false);
        expectThrows(
            IllegalStateException.class,
            () -> getBytesAsync(
                l -> container.compareAndExchangeRegister(BlobPurpose.SNAPSHOT, key, expectedValue.get(), BytesArray.EMPTY, l)
            )
        );
    }
    ...
    private static void checkAtomicWrite() throws IOException {
        ...
        container.writeBlobAtomic(BlobPurpose.SNAPSHOT, blobName, new BytesArray(randomByteArrayOfLength(randomIntBetween(1, 512))), true);
        final var blobData = new BytesArray(randomByteArrayOfLength(randomIntBetween(1, 512)));
        container.writeBlobAtomic(BlobPurpose.SNAPSHOT, blobName, blobData, false);
        assertEquals(blobData, Streams.readFully(container.readBlob(BlobPurpose.SNAPSHOT, blobName)));
        expectThrows(
        ...
        for (String blob : container.listBlobs(BlobPurpose.SNAPSHOT).keySet()) {

...
public class DelayableWriteableTests extends ESTestCase {
    ...
    public void testCompressToBytes() throws IOException {
        ...
        TransportVersion version = TransportVersionUtils.randomVersionSupporting(
            random(),
            DelayableWriteable.DELAYABLE_WRITEABLE_UNCOMPRESSED_SIZE
        );
        ...
        try (CountingStreamOutput counting = new CountingStreamOutput()) {
            ...
            e.writeTo(counting);
            expectedUncompressed = counting.size();
        }

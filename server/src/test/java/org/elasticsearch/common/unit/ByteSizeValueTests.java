...
public class ByteSizeValueTests extends AbstractWireSerializingTestCase<ByteSizeValue> {
    ...
    protected ByteSizeValue mutateInstance(final ByteSizeValue instance) {
        ...
        case 1:
            ...
            /*
             * If size is zero we can not reuse zero because 0 with any unit will be equal to 0 with any other unit so in this case we need
             * to randomize a new size. Additionally, if the size unit pair is such that the representation would be such that the number of
             * represented bytes would exceed Long.Max_VALUE, we have to randomize a new size too.
             */

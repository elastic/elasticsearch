...
public class XContentMapValuesTests extends AbstractFilteringTestCase {
    ...
    public void testExtractValueWithNullValue() throws Exception {
        ...
        assertEquals(list("value1", "NULL", "value2"), XContentMapValues.extractValue("array", map, "NULL"));
        assertEquals(list("NULL", "value"), XContentMapValues.extractValue("object_array.field", map, "NULL"));
    ...
    public void testExtractFlatArrayNestedSource() {
        Map<?, ?> map = map("nested", list(map("field", "nested1"), map("field", "nested2")));
        List<Map<?, ?>> nestedSources = XContentMapValues.extractNestedSources("nested", map);
        assertThat(nestedSources, containsInAnyOrder(map("field", "nested1"), map("field", "nested2")));
    }
    ...
    public void testExtractNestedSources() {
        Map<?, ?> map = map(
            "obj.ect",
            list(map("nested", list(map("field", "nested1"), map("field", "nested2"))), map("nested", list(map("field", "nested3")))),
            "obj",
            list(map("ect", map("nested", list(map("field", "nested4"), map("field", "nested5")))))
        );

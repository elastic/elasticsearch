...
public class EscfEncoderSimdJsonTests extends ESTestCase {
    ...
    // -----------------------------------------------------------------------
    // Batch API: parseBatchDirect
    // -----------------------------------------------------------------------

    /**
     * parseBatchDirect with a single document — simplest batch path.
     */
    public void testBatchDirectSingleDoc() throws IOException {
        assertBatchDirectMatchesJackson("{\"a\":1,\"b\":\"hello\"}");
    }

    /**
     * parseBatchDirect with multiple documents of varying shapes.
     */
    public void testBatchDirectMultipleDocs() throws IOException {
        assertBatchDirectMatchesJackson("{\"x\":1}", "{\"y\":\"two\",\"z\":true}", "{\"nested\":{\"k\":42}}", "{\"arr\":[1,2,3]}");
    }

    /**
     * parseBatchDirect with documents containing escaped strings.
     */
    public void testBatchDirectEscapedStrings() throws IOException {
        assertBatchDirectMatchesJackson("{\"msg\":\"hello\\nworld\"}", "{\"path\":\"C:\\\\Users\\\\file\"}", "{\"q\":\"say \\\"hi\\\"\"}");
    }

    /**
     * parseBatchDirect with nested arrays and objects.
     */
    public void testBatchDirectNestedStructures() throws IOException {
        assertBatchDirectMatchesJackson(
            "{\"matrix\":[[1,2],[3,4]]}",
            "{\"items\":[{\"a\":1},{\"b\":2}]}",
            "{\"deep\":{\"a\":{\"b\":{\"c\":1}}}}"
        );
    ...
    /**
     * parseBatchDirect with many small documents — exercises the batch buffer and
     * per-document window isolation.
     */
    public void testBatchDirectManyDocs() throws IOException {
        String[] docs = new String[50];
        for (int i = 0; i < 50; i++) {
            docs[i] = "{\"i\":" + i + ",\"s\":\"v" + i + "\"}";
        }
        assertBatchDirectMatchesJackson(docs);
    }

    /**
     * parseBatchDirect with empty objects and arrays.
     */
    public void testBatchDirectEmptyContainers() throws IOException {
        assertBatchDirectMatchesJackson("{\"e\":{}}", "{\"a\":[]}", "{\"x\":1}");
    }

    /**
     * Differential assertion for parseBatchDirect: builds a contiguous batch buffer,
     * parses via parseBatchDirect and parseBatchJackson (via allowSimd=false), compares all rows.
     */
    private static void assertBatchDirectMatchesJackson(String... jsonDocs) throws IOException {
        Recycler<BytesRef> recycler = newRecycler();
        int padding = 64;
        ...
        int[] lens = new int[jsonDocs.length];
        for (int i = 0; i < jsonDocs.length; i++) {
            offsets[i] = totalLen;
            lens[i] = jsonDocs[i].getBytes(java.nio.charset.StandardCharsets.UTF_8).length;
            totalLen += lens[i];
        }
        byte[] buffer = new byte[totalLen + padding];
        for (int i = 0; i < jsonDocs.length; i++) {
            byte[] docBytes = jsonDocs[i].getBytes(java.nio.charset.StandardCharsets.UTF_8);
            System.arraycopy(docBytes, 0, buffer, offsets[i], docBytes.length);
        }

        try (EscfEncoder simdEncoder = new EscfEncoder(recycler, true); EscfEncoder jacksonEncoder = new EscfEncoder(recycler, false)) {
            simdEncoder.parseBatchDirect(buffer, offsets, lens, jsonDocs.length, 0, LeafSink.NO_OP);
            jacksonEncoder.parseBatchDirect(buffer, offsets, lens, jsonDocs.length, 0, LeafSink.NO_OP);

            try (EscfBatch simdBatch = simdEncoder.buildPartition(0); EscfBatch jacksonBatch = jacksonEncoder.buildPartition(0)) {
                assertEquals("doc count mismatch", jacksonBatch.docCount(), simdBatch.docCount());
                for (int i = 0; i < jacksonBatch.docCount(); i++) {
                    assertEquals("batch row " + i + " mismatch", reconstruct(jacksonBatch, i), reconstruct(simdBatch, i));
                }
            }
        }
    }

...
public class Chimp128DoubleEncodeStageTests extends ESTestCase {
    ...
    public void testPeriodicPattern() throws IOException {
        final double[] pattern = new double[] { 10.0, 20.0, 30.0, 40.0 };
        final long[] data = new long[BLOCK_SIZE];
        for (int i = 0; i < BLOCK_SIZE; i++) {
            data[i] = NumericUtils.doubleToSortableLong(pattern[i % pattern.length]);
        }
        assertRoundTrip(data);
    }
    ...
    public void testChimp128BetterThanChimpOnPeriodic() throws IOException {
        ...
        final byte[] chimpBuffer = new byte[data.length * 16 + 256];
        ...
        new ChimpDoubleEncodeStage().encode(data.clone(), data.length, chimpOut, chimpEncContext);
        final int chimpSize = chimpOut.getPosition();
        ...
        assertThat(
            "Chimp128 (" + chimp128Size + " bytes) should be smaller than Chimp (" + chimpSize + " bytes) on periodic data",
            chimp128Size,
            lessThan(chimpSize)
        );
    }

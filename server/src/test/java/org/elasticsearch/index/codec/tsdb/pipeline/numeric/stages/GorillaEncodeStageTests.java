...
public class GorillaEncodeStageTests extends ESTestCase {
    ...
    public void testRandomFloats() throws IOException {
        ...
        for (int i = 0; i < BLOCK_SIZE; i++) {
            ...
            data[i] = NumericUtils.doubleToSortableLong(f);
        }
        assertRoundTrip(data);
    }
    ...
    public void testSlowlyChangingFloats() throws IOException {
        ...
        final long[] data = new long[BLOCK_SIZE];
        for (int i = 0; i < BLOCK_SIZE; i++) {
            data[i] = NumericUtils.doubleToSortableLong(floats[i]);
        }
        assertRoundTrip(data);
    }

    public void testSortableDoubleSpecialValues() throws IOException {
    ...
    public void testLargeLeadingZerosXor() throws IOException {
        // NOTE: Values that differ only in the lowest mantissa bits produce XOR
        // with >31 leading zeros. With the 6-bit leading-zeros field this encodes
        // compactly; with the old 5-bit field the clamping would inflate the window.
        final long[] data = new long[BLOCK_SIZE];
        final double base = 1000.0;
        for (int i = 0; i < BLOCK_SIZE; i++) {
            // Tiny increments: XOR between consecutive values has many leading zeros
    ...
    public void testLargeLeadingZerosCompression() throws IOException {
        // NOTE: Verify that values with >31 leading zeros in XOR produce compact output.
        final long[] data = new long[BLOCK_SIZE];
        final double base = 1000.0;
        for (int i = 0; i < BLOCK_SIZE; i++) {
            data[i] = NumericUtils.doubleToSortableLong(base + i * 1e-12);
        ...
        new GorillaEncodeStage().encode(data.clone(), data.length, out, encContext);

        assertTrue("encoded size " + out.getPosition() + " should be much less than raw 1024", out.getPosition() < 512);
    }

    public void testAllZeroXor() throws IOException {
        // NOTE: All identical values: every XOR is zero, exercising the fast path exclusively.
        final long[] data = new long[BLOCK_SIZE];
    ...
    public void testAlternatingValues() throws IOException {
        // NOTE: Forces every XOR to be non-zero, heavily exercises '10' and '11' paths.
        final long[] data = new long[BLOCK_SIZE];
        for (int i = 0; i < BLOCK_SIZE; i++) {
    ...
    public void testSignFlipXor() throws IOException {
        // NOTE: Positive-to-negative flips produce XOR with all 64 bits set in the
        // exponent+sign region. This exercises the maximum meaningful-bits case and
        // the readBits(64) path.
        final long[] data = new long[BLOCK_SIZE];
        for (int i = 0; i < BLOCK_SIZE; i++) {
    ...
    public void testWindowReuse() throws IOException {
        // NOTE: Consecutive values differ by the same amount, so XOR patterns repeat
        // and the '10' (reuse window) path is taken after the first '11'.
        final long[] data = new long[BLOCK_SIZE];
        for (int i = 0; i < BLOCK_SIZE; i++) {
    ...
    public void testMaxXorTwoValues() throws IOException {
        // NOTE: Two values with maximum possible XOR in IEEE 754 raw bits.
        final long[] data = new long[] {
    ...
    public void testLargeBlockFuzz() throws IOException {
        // NOTE: Large block stresses the bit buffer across many byte boundaries.
    ...
    public void testMixedZeroAndNonZeroXor() throws IOException {
        // NOTE: Interleaves runs of identical values (zero XOR) with jumps (non-zero XOR).
        // Exercises transitions between the three control prefix paths.
        final long[] data = new long[BLOCK_SIZE];
        double v = 1.0;
        for (int i = 0; i < BLOCK_SIZE; i++) {
            if (i % 10 == 0) {
    ...
    public void testRepeatedFuzz() throws IOException {
        // NOTE: Run multiple random round-trips to catch seed-dependent issues.
    ...
    public void testStageId() {
        assertThat(new GorillaEncodeStage().id(), equalTo(StageId.GORILLA_PAYLOAD.id));
    }
    ...
    private void assertRoundTrip(final long[] original, final int blockSize) throws IOException {
        ...
        new GorillaEncodeStage().encode(original.clone(), original.length, out, encContext);
        ...
        final int decodedCount = new GorillaDecodeStage().decode(decoded, in, decContext);

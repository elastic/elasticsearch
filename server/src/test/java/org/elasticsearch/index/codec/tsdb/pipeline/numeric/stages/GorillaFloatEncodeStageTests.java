...
public class GorillaFloatEncodeStageTests extends ESTestCase {
    ...
    public void testAllZeroXorCompression() throws IOException {
        ...
        new GorillaFloatEncodeStage().encode(data.clone(), data.length, out, encContext);
    ...
    public void testStageId() {
        assertThat(new GorillaFloatEncodeStage().id(), equalTo(StageId.GORILLA_FLOAT_PAYLOAD.id));
        assertThat(new GorillaFloatDecodeStage().id(), equalTo(StageId.GORILLA_FLOAT_PAYLOAD.id));
    }
    ...
    private void assertRoundTrip(final long[] original, final int blockSize) throws IOException {
        ...
        new GorillaFloatEncodeStage().encode(original.clone(), original.length, out, encContext);
        ...
        final int decodedCount = new GorillaFloatDecodeStage().decode(decoded, in, decContext);

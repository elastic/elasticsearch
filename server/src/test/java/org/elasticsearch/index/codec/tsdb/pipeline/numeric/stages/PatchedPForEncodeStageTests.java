...
public class PatchedPForEncodeStageTests extends NumericCodecStageTestCase {

    private static final int PATCHED_PFOR_EXTRA_BUFFER = 1024;
    private static final TransformDecoder DECODER = new PatchedPForDecodeStage();

    public void testInvalidMaxExceptionPercentThrows() {
        expectThrows(AssertionError.class, () -> new PatchedPForEncodeStage(0));
        expectThrows(AssertionError.class, () -> new PatchedPForEncodeStage(-1));
        expectThrows(AssertionError.class, () -> new PatchedPForEncodeStage(51));
        expectThrows(AssertionError.class, () -> new PatchedPForEncodeStage(100));
    }
    ...
    public void testNegativeValuesSkip() throws IOException {
        ...
        new PatchedPForEncodeStage().encode(values, values.length, encodingContext);
    ...
    public void testDeltaEncodedExceptionPositions() throws IOException {
        ...
        values[blockSize / 10] = 1L << 40;
        values[blockSize / 3] = 1L << 41;
        values[3 * blockSize / 4] = 1L << 42;
        assertRoundTrip(values, blockSize);
    }
    ...
    private void assertRoundTrip(final long[] original, int blockSize, int maxExceptionPercent) throws IOException {
        assertRoundTrip(
            ...
            new PatchedPForEncodeStage(maxExceptionPercent),
            DECODER,

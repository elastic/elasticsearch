...
public class ESNextDiskBBQVectorsWriterTests extends ESTestCase {
    ...
    public void testAutoConcatenationAlwaysSkipsRefinement() {
        for (VectorSimilarityFunction sim : VectorSimilarityFunction.values()) {
            assertEquals(
                ESNextDiskBBQVectorsWriter.MergeExperiment.CONCATENATION_NO_REFINEMENT,
                ESNextDiskBBQVectorsWriter.MergeExperiment.AUTO.resolveConcatenationExperiment(sim)
            );
        }
    }

    public void testExplicitConcatenationExperimentIsNotOverridden() {
        assertEquals(
            ESNextDiskBBQVectorsWriter.MergeExperiment.CONCATENATION_WEIGHTED_NO_REFINEMENT,
            ESNextDiskBBQVectorsWriter.MergeExperiment.CONCATENATION_WEIGHTED_NO_REFINEMENT.resolveConcatenationExperiment(
                VectorSimilarityFunction.EUCLIDEAN
            )
        );
    }

    public void testMergeSignalsPreferRefinementForSeparatedSegmentCentroids() {
        IVFVectorsReader.CentroidData[] segmentCentroidData = new IVFVectorsReader.CentroidData[] {
            new IVFVectorsReader.CentroidData(
                new float[][] { new float[] { 0f, 0f }, new float[] { 0f, 2f } },
                new int[] { 50, 50 },
                new float[] { 0f, 1f }
            ),
            new IVFVectorsReader.CentroidData(
                new float[][] { new float[] { 10f, 0f }, new float[] { 10f, 2f } },
                new int[] { 50, 50 },
                new float[] { 10f, 1f }
            ) };

        ESNextDiskBBQVectorsWriter.MergeSignalStats stats = ESNextDiskBBQVectorsWriter.computeMergeSignalStats(
            VectorSimilarityFunction.EUCLIDEAN,
            2,
            new int[] { 100, 100 },
            segmentCentroidData
        ...
        assertTrue(stats.interSegmentSpread() > stats.intraSegmentCentroidSpread());
        assertTrue(stats.interToIntraSpreadRatio() > 1.0d);
        assertEquals(ESNextDiskBBQVectorsWriter.MergeExperiment.CONCATENATION, stats.suggestedExperiment());
    }

    public void testMergeSignalsPreferNoRefinementForOverlappingSegmentCentroids() {
        IVFVectorsReader.CentroidData[] segmentCentroidData = new IVFVectorsReader.CentroidData[] {
            new IVFVectorsReader.CentroidData(
                new float[][] { new float[] { -4f, 0f }, new float[] { 4f, 0f } },
                new int[] { 50, 50 },
                new float[] { 0f, 0f }
            ),
            new IVFVectorsReader.CentroidData(
                new float[][] { new float[] { -3.8f, 0f }, new float[] { 4.2f, 0f } },
                new int[] { 50, 50 },
                new float[] { 0.2f, 0f }
            ) };
        ...
        assertTrue(stats.interSegmentSpread() < stats.intraSegmentCentroidSpread());
        assertTrue(stats.interToIntraSpreadRatio() < 1.0d);
        assertEquals(ESNextDiskBBQVectorsWriter.MergeExperiment.CONCATENATION_NO_REFINEMENT, stats.suggestedExperiment());
    }
    ...
    private IndexWriterConfig newIndexWriterConfig(int vectorsPerCluster, int centroidsPerParentCluster) {
        KnnVectorsFormat format = new ESNextDiskBBQVectorsFormat(vectorsPerCluster, centroidsPerParentCluster);

...
public class NoOpEngineTests extends EngineTestCase {
        ...
        try (Store store = createStore()) {
            ...
            try (InternalEngine engine = createEngine(config)) {
                for (int i = 0; i < numDocs; i++) {
                    engine.index(
                        indexForDoc(createParsedDoc(indexSettings.getIndexVersionCreated(), Integer.toString(i), idFieldType, null))
                    );
                    if (rarely()) {
                        engine.flush();
                    }
    ...
    public void testTrimUnreferencedTranslogFiles() throws Exception {
        ...
        for (int i = 0; i < numDocs; i++) {
            ...
            engine.index(
                indexForDoc(
                    createParsedDoc(engine.config().getIndexSettings().getIndexVersionCreated(), Integer.toString(i), idFieldType, null)
                )
            );
            tracker.updateLocalCheckpoint(allocationId.getId(), i);
            if (rarely()) {

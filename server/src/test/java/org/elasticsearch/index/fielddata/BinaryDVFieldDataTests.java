...
public class BinaryDVFieldDataTests extends AbstractFieldDataTestCase {
    ...
    public void testDocValue() throws Exception {
        ...
        doc.endObject();
        ParsedDocument d = mapper.parse(SourceToParse.source("test", "test", "1", null, BytesReference.bytes(doc), XContentType.JSON));
        ...
        d = mapper.parse(SourceToParse.source("test", "test", "2", null, BytesReference.bytes(doc), XContentType.JSON));
        writer.addDocument(d.rootDoc());

        doc = XContentFactory.jsonBuilder().startObject().endObject();
        d = mapper.parse(SourceToParse.source("test", "test", "3", null, BytesReference.bytes(doc), XContentType.JSON));
        writer.addDocument(d.rootDoc());
        ...
        doc.endObject();
        d = mapper.parse(SourceToParse.source("test", "test", "4", null, BytesReference.bytes(doc), XContentType.JSON));
        writer.addDocument(d.rootDoc());

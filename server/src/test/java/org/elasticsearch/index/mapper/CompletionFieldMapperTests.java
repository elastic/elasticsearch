...
public class CompletionFieldMapperTests extends ESSingleNodeTestCase {
    ...
    public void testParsingWithGeoFieldAlias() throws Exception {
        ...
        Mapper fieldMapper = mapperService.documentMapper("type1").mappers().getMapper("completion");

        ParsedDocument parsedDocument = mapperService.documentMapper("type1").parse(SourceToParse.source("test", "type1", "1",
            BytesReference.bytes(XContentFactory.jsonBuilder()
                .startObject()
                    .startObject("completion")
                        .field("input", "suggestion")

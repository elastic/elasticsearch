...
public class IpFieldTypeTests extends FieldTypeTestCase {
    ...
    public void testValueFormat() throws Exception {
        ...
        assertEquals(ip, ft.docValueFormat("field", null, null).format(asBytes));
    ...
    public void testTermQuery() {
        ...
        String ip = "2001:db8::2:1";
        assertEquals(InetAddressPoint.newExactQuery("field", InetAddresses.forString(ip)), ft.termQuery("field", ip, MOCK_CONTEXT));
        ...
        assertEquals(
            ...
            ft.termQuery("field", ip, MOCK_CONTEXT)
        );
    ...
    public void testTermsQuery() {
        ...
        assertEquals(
            ...
            ft.termsQuery("field", Arrays.asList(InetAddresses.forString("::2"), InetAddresses.forString("::5")), MOCK_CONTEXT)
        );
        assertEquals(
            InetAddressPoint.newSetQuery("field", InetAddresses.forString("::2"), InetAddresses.forString("::5")),
            ft.termsQuery("field", Arrays.asList("::2", "::5"), MOCK_CONTEXT)
        );

        // if the list includes a prefix query we fallback to a bool query
        assertEquals(
            new ConstantScoreQuery(
                new BooleanQuery.Builder().add(ft.termQuery("field", "::42", MOCK_CONTEXT), Occur.SHOULD)
                    .add(ft.termQuery("field", "::2/16", null), Occur.SHOULD)
                    .build()
            ),
            ft.termsQuery("field", Arrays.asList("::42", "::2/16"), MOCK_CONTEXT)
        );
    }
    ...
    public void testRangeQuery() {
        ...
        assertEquals(
            ...
            ft.rangeQuery("field", "2001:db8::1", "2001:db8::1", false, false, null, null, null, MOCK_CONTEXT)
        );

        // Upper bound is the min IP and is not inclusive
        assertEquals(new MatchNoDocsQuery(), ft.rangeQuery("field", "::", "::", true, false, null, null, null, MOCK_CONTEXT));
        ...
        assertEquals(
            ...
            // same lo/hi values but inclusive=false so this won't match anything
            ft.rangeQuery("field", "::", "0.0.0.0", true, false, null, null, null, MOCK_CONTEXT)
        );
    ...
    public void testFetchSourceValue() throws IOException {
        ...
        MappedField nullValueMapper = new IpFieldMapper.Builder("field", ScriptCompiler.NONE, true, Version.CURRENT).nullValue(

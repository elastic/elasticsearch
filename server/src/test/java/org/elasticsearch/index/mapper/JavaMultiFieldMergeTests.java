...
public class JavaMultiFieldMergeTests extends ESSingleNodeTestCase {
    public void testMergeMultiField() throws Exception {
        ...
        Document doc = mapperService.documentMapper().parse(
            SourceToParse.source("test", "person", "1", null, json, XContentType.JSON)).rootDoc();
        ...
        doc = mapperService.documentMapper().parse(SourceToParse.source("test", "person", "1", null, json, XContentType.JSON)).rootDoc();
        f = doc.getField("name");
        assertThat(f, notNullValue());
        f = doc.getField("name.indexed");
        assertThat(f, notNullValue());

...
public class MapperServiceTests extends MapperServiceTestCase {
    ...
    public void testLegacyFormatParsing() throws IOException {
        ...
        MapperService mapperServiceLatest = createMapperService(mapping(b -> {}));

        // should throw an exception when we have latest index version mapping
        assertEquals(
            "Failed to parse mapping: Error parsing [format] on field [field1]: Invalid format: "
                + "[strictDateOptionalTime]: Unknown pattern letter: t",
            expectThrows(MapperParsingException.class, () -> merge(mapperServiceLatest, """
                { "_doc" : {
                      "properties" : {
                         "field1" : {
                           "type" : "date",
    ...
    public void testFormatUpgraders() throws IOException {
        ...
        // None of the merge calls should cause an error by using camel case format
        mapperServiceOldIndex.merge(indexMetadata, MergeReason.MAPPING_RECOVERY);

        // Check when document types don't match
        ...
        mapperServiceOldIndex.merge(indexMetadataNoType, MergeReason.MAPPING_RECOVERY);
        ...
        // check with nested objects
        IndexMetadata indexMetadataNestedMapping = indexMetadataWithMapping(nestedMappingSource);
        mapperServiceOldIndex.merge(indexMetadataNestedMapping, MergeReason.MAPPING_RECOVERY);

        var templatesMappingSource = new CompressedXContent(BytesReference.bytes(mapping(b -> {
        ...
        // check with dynamic templates
        IndexMetadata indexMetadataTemplatesMapping = indexMetadataWithMapping(nestedMappingSource);
        mapperServiceOldIndex.merge(indexMetadataTemplatesMapping, MergeReason.MAPPING_RECOVERY);

        // check with runtime fields
        ...
        mapperServiceOldIndex.merge(indexMetadataRuntime, MergeReason.MAPPING_RECOVERY);

        // check runtime mappings with the latest index version, expected to throw
        assertEquals(
            "Failed to parse mapping: Invalid format: [strictDateOptionalTime]: Unknown pattern letter: t",
            expectThrows(
                MapperParsingException.class,
                () -> createMapperService(mapping(b -> {})).merge(indexMetadataRuntime, MergeReason.MAPPING_RECOVERY)
            ).getMessage()
        );
    }

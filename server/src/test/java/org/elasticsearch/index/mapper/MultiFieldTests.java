...
public class MultiFieldTests extends MapperServiceTestCase {
    ...
    public void testMultiFieldMultiFields() throws Exception {
        ...
        LuceneDocument doc = mapperService.documentMapper()
            .parse(new SourceToParse("test", "_doc", "1", json, XContentType.JSON))
            .rootDoc();
    ...
    public void testBuildThenParse() throws Exception {
        ...
        BytesReference json = new BytesArray(copyToBytesFromClasspath("/org/elasticsearch/index/mapper/multifield/test-data.json"));
        LuceneDocument doc = builderDocMapper.parse(new SourceToParse("test", "_doc", "1", json, XContentType.JSON)).rootDoc();
    ...
    public void testMultiFieldWithDot() {
        ...
        assertThat(
            ...
            equalTo("Failed to parse mapping [_doc]: Field name [raw.foo] which is a multi field of [field] cannot contain '.'")
        );
    }
}

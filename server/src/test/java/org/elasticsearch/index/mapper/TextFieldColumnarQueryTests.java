 ...
 * query built for one refuses to be constructed. These are the query methods on a text field with no inverted index
 * to answer them from, so every one of them has to route by format rather than reaching for a scanning query.
 */
public class TextFieldColumnarQueryTests extends MapperServiceTestCase {
    ...
    private MapperService columnarMapperService() throws IOException {
        ...
        return createMapperService(
            settings,
            mapping(b -> b.startObject(FIELD).field("type", "text").field("index", false).field("doc_values", true).endObject())
        );
    }
}

...
public class BinaryMvMaxBytesRefsBlockLoaderTests extends AbstractBlockLoaderTestCase {

    public BinaryMvMaxBytesRefsBlockLoaderTests(boolean multiValues, boolean missingValues) {
        ...
        try (Directory dir = newDirectory(); RandomIndexWriter iw = new RandomIndexWriter(random(), dir)) {
            ...
            try (DirectoryReader dr = wrap.apply(iw.getReader())) {
                ...
                try (var stringsReader = stringsLoader.reader(breaker, ctx); var mvMinReader = mvMinLoader.reader(breaker, ctx)) {
                    ...
                    try (TestBlock strings = read(stringsReader, docs); TestBlock maxs = read(mvMinReader, docs)) {
                        checkBlocks(strings, maxs);
                    }
                }
                try (var stringsReader = stringsLoader.reader(breaker, ctx); var mvMinReader = mvMinLoader.reader(breaker, ctx)) {
    ...
    private Matcher<Object> readerMatcher() {
        if (multiValues) {
            return hasToString("MvMaxBytesRefsFromBinary.SeparateCount");
        }

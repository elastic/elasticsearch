...
public class BinaryMvMaxBytesRefsTests extends ESTestCase {
    ...
    public BinaryMvMaxBytesRefsTests(boolean blockAtATime, boolean multiValues, boolean missingValues) {
        this.blockAtATime = blockAtATime;
        this.multiValues = multiValues;
        this.missingValues = missingValues;
    }

    public void testMvMaxBinaryDocValues() throws IOException {
        try (Directory dir = newDirectory(); RandomIndexWriter iw = new RandomIndexWriter(random(), dir)) {
            ...
            try (DirectoryReader dr = iw.getReader()) {
                ...
                var mvMaxLoader = new MvMaxBytesRefsFromBinaryBlockLoader("field");

                var stringsReader = stringsLoader.reader(ctx);
                var mvMaxReader = mvMaxLoader.reader(ctx);
                assertThat(mvMaxReader, hasToString(multiValues ? "MvMaxBytesRefs.Binary" : "BlockDocValuesReader.Bytes"));
                BlockLoader.Docs docs = TestBlock.docs(ctx);
                try (
                    TestBlock strings = read(stringsLoader, stringsReader, docs);
                    TestBlock maxMaxValues = read(mvMaxLoader, mvMaxReader, docs);
                ) {
                    checkBlocks(strings, maxMaxValues);
                }

...
public class FlattenedFieldSearchTests extends ESSingleNodeTestCase {
    ...
    public void testCardinalityAggregation() throws IOException {
        ...
        BulkRequestBuilder bulkRequest = client().prepareBulk("test", "_doc").setRefreshPolicy(RefreshPolicy.IMMEDIATE);
        ...
        for (int i = 0; i < 10; i++) {
            bulkRequest.add(client().prepareIndex("test", "_doc").setSource("other_field", "1"));
        }
    ...
    public void testSourceFiltering() {
        ...
        client().prepareIndex("test", "_doc", "1").setRefreshPolicy(RefreshPolicy.IMMEDIATE).setSource(source).get();

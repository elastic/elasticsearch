...
public class GeoBoundingBoxQueryBuilderTests extends AbstractQueryTestCase<GeoBoundingBoxQueryBuilder> {
    ...
    public void testFromWKT() throws IOException {
        ...
        // TODO support WKT in toXContent?
        ...
        checkGeneratedJson(exptectedJson, parsed);
        assertEquals(exptectedJson, "pin.location", parsed.fieldName());
        assertEquals(exptectedJson, -74.1, parsed.topLeft().getLon(), 0.0001);
        assertEquals(exptectedJson, 40.73, parsed.topLeft().getLat(), 0.0001);
        assertEquals(exptectedJson, -71.12, parsed.bottomRight().getLon(), 0.0001);
        assertEquals(exptectedJson, 40.01, parsed.bottomRight().getLat(), 0.0001);
        assertEquals(exptectedJson, 1.0, parsed.boost(), 0.0001);
        assertEquals(exptectedJson, GeoExecType.MEMORY, parsed.type());
    }

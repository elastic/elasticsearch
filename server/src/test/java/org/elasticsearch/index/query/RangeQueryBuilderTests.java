...
public class RangeQueryBuilderTests extends AbstractQueryTestCase<RangeQueryBuilder> {
    ...
    protected void doAssertLuceneQuery(RangeQueryBuilder queryBuilder, Query query, SearchExecutionContext context) throws IOException {
        ...
        if (queryBuilder.from() == null && queryBuilder.to() == null) {
            ...
            if (context.getIndexSettings().getIndexVersionCreated().onOrAfter(Version.V_6_1_0)
                && context.getFieldType(queryBuilder.fieldName()).hasDocValues()) {
                ...
                && context.getFieldType(queryBuilder.fieldName()).getTextSearchInfo().hasNorms()) {
    ...
    public void testRewriteDateToMatchAll() throws IOException {
        ...
        final Query expectedQuery;
        if (searchExecutionContext.getIndexSettings().getIndexVersionCreated().onOrAfter(Version.V_6_1_0)
            && searchExecutionContext.getFieldType(query.fieldName()).hasDocValues()) {

...
public class BulkByScrollResponseTests extends AbstractXContentTestCase<BulkByScrollResponse> {
    ...
    private static Object parseFailure(XContentParser parser) throws IOException {
        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            ...
            if (token == XContentParser.Token.START_ARRAY) {
                ...
                switch (name) {
                    case BulkByPaginatedSearchFailure.REASON_FIELD -> searchExc = ElasticsearchException.fromXContent(parser);
                ...
                switch (name) {
                    ...
                    case BulkByPaginatedSearchFailure.NODE_FIELD -> nodeId = parser.text();
                }
            } else if (token == XContentParser.Token.VALUE_NUMBER) {
                switch (name) {
                    case Failure.STATUS_FIELD -> status = parser.intValue();
                    case BulkByPaginatedSearchFailure.SHARD_FIELD -> shardId = parser.intValue();
                }
            }
        }
        if (bulkExc != null) {
            ...
            if (status == null) {
                return new BulkByPaginatedSearchFailure(searchExc, index, shardId, nodeId);
            } else {
                return new BulkByPaginatedSearchFailure(searchExc, index, shardId, nodeId, RestStatus.fromCode(status));
            }
        } else {
    ...
    private List<BulkByPaginatedSearchFailure> randomSearchFailures() {
        ...
        return singletonList(new BulkByPaginatedSearchFailure(exception, index, shardId, nodeId));
    }
    ...
    public static void assertEqualBulkResponse(
        ...
        for (int i = 0; i < expected.getSearchFailures().size(); i++) {
            BulkByPaginatedSearchFailure expectedFailure = expected.getSearchFailures().get(i);
            BulkByPaginatedSearchFailure actualFailure = actual.getSearchFailures().get(i);
            assertEquals(expectedFailure.getIndex(), actualFailure.getIndex());

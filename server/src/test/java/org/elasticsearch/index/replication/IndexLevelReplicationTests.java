...
public class IndexLevelReplicationTests extends ESIndexLevelReplicationTestCase {
    ...
    public void testDocumentFailureReplication() throws Exception {
        ...
        try (ReplicationGroup shards = new ReplicationGroup(buildIndexMetadata(0)) {
            ...
            IndexRequest indexRequest = new IndexRequest(index.getName()).id("1").source("{}", XContentType.JSON);
            BulkItemResponse indexResp = shards.index(indexRequest);
            ...
            indexRequest = new IndexRequest(index.getName()).id("any").source("{}", XContentType.JSON);
            indexResp = shards.index(indexRequest);
            indexRequest.decRef();
            assertThat(indexResp.getFailure().getCause(), equalTo(indexException));
    ...
    public void testRequestFailureReplication() throws Exception {
        try (ReplicationGroup shards = createGroup(0)) {
            shards.startAll();
            IndexRequest indexRequest = new IndexRequest(index.getName()).id("1").source("{}", XContentType.JSON).version(2);
            BulkItemResponse response = shards.index(indexRequest);
            indexRequest.decRef();
            ...
            shards.startReplicas(nReplica);
            indexRequest = new IndexRequest(index.getName()).id("1").source("{}", XContentType.JSON).version(2);
            response = shards.index(indexRequest);
            indexRequest.decRef();
            assertTrue(response.isFailed());
            assertThat(response.getFailure().getCause(), instanceOf(VersionConflictEngineException.class));
            shards.assertAllEqual(0);
    ...
    public void testSeqNoCollision() throws Exception {
        try (ReplicationGroup shards = createGroup(2)) {
            ...
            IndexRequest indexRequest = new IndexRequest(index.getName()).id("d2").source("{}", XContentType.JSON);
            shards.index(indexRequest);
            indexRequest.decRef();
    ...
    public void testLateDeliveryAfterGCTriggeredOnReplica() throws Exception {
        ...
        try (ReplicationGroup shards = createGroup(1)) {
            ...
            IndexRequest singleIndexRequest = new IndexRequest(index.getName()).id("d1").source("{}", XContentType.JSON);
            final BulkShardRequest indexRequest = indexOnPrimary(singleIndexRequest, primary);
            singleIndexRequest.decRef();
    ...
    public void testOutOfOrderDeliveryForAppendOnlyOperations() throws Exception {
        try (ReplicationGroup shards = createGroup(1)) {
            ...
            IndexRequest singleIndexRequest = new IndexRequest(index.getName()).source("{}", XContentType.JSON);
            final BulkShardRequest indexRequest = indexOnPrimary(singleIndexRequest, primary);
            singleIndexRequest.decRef();

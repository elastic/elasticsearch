...
public class IndexShardOperationPermitsTests extends ESTestCase {
        ...
        for (PlainActionFuture<Releasable> future : futures) {
            try {
                ...
                if (closeAfterBlocking) {
                    assertThat(
                        e.getCause(),
                        either(instanceOf(DummyException.class)).or(instanceOf(EsRejectedExecutionException.class))
    ...
    private Releasable blockAndWait() throws InterruptedException {
        ...
        permits.blockOperations(ActionListener.runAfter(new ActionListener<Releasable>() {
            @Override
            public void onResponse(Releasable releasable) {

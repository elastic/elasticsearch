...
public class SimilarityTests extends ESSingleNodeTestCase {
    ...
    public void testResolveSimilaritiesFromMapping_classicIsDeprecated() throws IOException {
        Settings settings = Settings.builder()
            .put("index.similarity.my_similarity.type", "classic")
            .put("index.similarity.my_similarity.discount_overlaps", false)
            .build();
        createSimilarityService(settings);
        assertWarnings("The [classic] similarity is now deprecated in favour of BM25, which is generally accepted as a better alternative. "
                + "Use the [BM25] similarity or build a custom [scripted] similarity instead.");
    }
    ...
    public void testUnknownParameters() throws IOException {
        Settings settings = Settings.builder()
            ...
            .build();
        createSimilarityService(settings);
        assertWarnings("Unknown settings for similarity of type [BM25]: [z]");
    }
}

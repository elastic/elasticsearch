...
public class TestTranslog {
    ...
    /**
     * Opens an empty translog at {@code translogDir} that tests fill with records and then replay; the caller
     * closes it.
     */
    public static Translog newTranslog(Path translogDir, ShardId shardId, long primaryTerm) throws IOException {
    ...
    /** Encodes JSON {@code sources} as an ESCF batch and returns a standalone copy of the batch bytes. */
    public static BytesReference encodeBatchData(List<BytesReference> sources) throws IOException {
    ...
    /**
     * Builds an all-indexed batch record over JSON {@code sources}: row {@code i} has seqNo {@code firstSeqNo + i},
     * version 1, autoGeneratedIdTimestamp {@code 100 + i}, id {@code idPrefix + i} and routing {@code "route-" + i}
     * on odd rows.
     */
    public static IndexOperationBatch.TranslogRecord batchRecord(
        List<BytesReference> sources,
        long firstSeqNo,
        long primaryTerm,
        String idPrefix
    ) throws IOException {
        final RecordBuilder builder = new RecordBuilder(sources.size());
        for (int i = 0; i < sources.size(); i++) {
            builder.indexed(i, firstSeqNo + i, 1L, 100L + i, XContentType.JSON, idPrefix + i, i % 2 == 0 ? null : "route-" + i);
        }
        return builder.build(primaryTerm, encodeBatchData(sources));
    }
    ...
    public static final class RecordBuilder {
        ...
        public RecordBuilder(int docCount) {
        ...
        public RecordBuilder indexed(int i, long seqNo, long version, long timestamp, XContentType type, String id, String routing) {
        ...
        public RecordBuilder noOp(int i, long seqNo, String reason) {
        ...
        /** Overrides row {@code i}'s status byte verbatim, for tests that feed the record an invalid status. */
        public RecordBuilder status(int i, byte status) {
            statuses[i] = status;
            return this;
        }

        public RecordBuilder skipped(int i) {
        ...
        public IndexOperationBatch.TranslogRecord build(long term, BytesReference batchData) {

...
public class IndicesRequestCacheTests extends ESTestCase {
    ...
    public void testCacheDifferentMapping() throws Exception {
        ...
        MappingLookup.CacheKey mappingKey2 = MappingLookup.fromMappers(Mapping.EMPTY, emptyList(), emptyList(), emptyList(), 1).cacheKey();
        AtomicBoolean indexShard = new AtomicBoolean(true);
        ShardRequestCache requestCacheStats = new ShardRequestCache();
        Directory dir = newDirectory();
        IndexWriter writer = new IndexWriter(dir, newIndexWriterConfig());
    ...
    public void testKeyEqualsAndHashCode() throws IOException {
        ...
        MappingLookup.CacheKey mKey2 = MappingLookup.fromMappers(Mapping.EMPTY, emptyList(), emptyList(), emptyList(), 1).cacheKey();
        Directory dir = newDirectory();

...
public class ShardLimitValidatorTests extends ESTestCase {
    ...
    public void testOverShardLimit() {
        ...
        assertEquals("this action would add [" + totalShards + "] shards, but this cluster currently has [" + currentShards
            + "]/[" + maxShards + "] maximum " + group + " shards open", errorMessage.get());
    }
    ...
    public void testValidateShardLimitOpenIndices() {
        ...
        final ClusterState state = createClusterForShardLimitTest(nodesInCluster, counts.getFirstIndexShards(),
        ...
        assertEquals("Validation Failed: 1: this action would add [" + totalShards + "] shards, but this cluster currently has [" +
    ...
    public static ShardLimitValidator createTestShardLimitService(int maxShardsPerNode, String group) {
        Setting<?> setting = ShardLimitValidator.FROZEN_GROUP.equals(group) ?
            ShardLimitValidator.SETTING_CLUSTER_MAX_SHARDS_PER_NODE_FROZEN : SETTING_CLUSTER_MAX_SHARDS_PER_NODE;

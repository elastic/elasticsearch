    ...
    public void testPluginCannotOverrideBuiltInSystemIndex() {
        Map<String, SystemIndices.Feature> pluginMap = singletonMap(
            ...
            new SystemIndices.Feature(
                TASKS_FEATURE_NAME,
                "test",
                singletonList(new SystemIndexDescriptor(TASK_INDEX + "*",
                "Task Result " +
                "Index")))
        );
        ...
        final SystemIndices systemIndices = new SystemIndices(
            singletonMap(
                ...
                new SystemIndices.Feature(
                    "test",
                    "test feature",
                    singletonList(new SystemIndexDescriptor(".test-[abc]" + OPTIONAL_UPGRADE_SUFFIX_REGEX, ""))
                )
            )
        );
        ...
        final SystemIndices systemIndices = new SystemIndices(
            singletonMap(
                ...
                new SystemIndices.Feature(
                    "test",
                    "test feature",
                    singletonList(new SystemIndexDescriptor(".test-[a]+" + OPTIONAL_UPGRADE_SUFFIX_REGEX, ""))
                )
            )
        );
        ...
        final SystemIndices systemIndices = new SystemIndices(
            singletonMap(
                ...
                new SystemIndices.Feature(
                    "test",
                    "test feature",
                    singletonList(new SystemIndexDescriptor(".test-[a-c]" + OPTIONAL_UPGRADE_SUFFIX_REGEX, ""))
                )
            )
        );

...
public class TimeSeriesIdGeneratorServiceTests extends ESTestCase {
    ...
    public void testNonTimeSeries() {
        try (
            ...
            Metadata meta = Metadata.builder().put(index("index", false, null, "{}")).build();
    ...
    public void testTwoLocalIndices() {
        Metadata meta = Metadata.builder()
            .put(index("index_1", true, "dim", "{}"))
            .put(index("index_2", true, "dim", "{\"foo\": \"bar\"}"))
            .build();
        try (TimeSeriesIdGeneratorService genService = genService(i -> new LocalIndex() {
            @Override
            public long metadataVersion() {
    ...
    private IndexMetadata.Builder index(String index, boolean timeSeriesMode, String routingPath, String mapping) {
        ...
        if (timeSeriesMode) {
            ...
            settings.put(IndexMetadata.INDEX_ROUTING_PATH.getKey(), routingPath);
        }

...
public class PeerRecoveryTargetServiceTests extends IndexShardTestCase {
    ...
    public void testPrepareIndexForPeerRecovery() throws Exception {
        DiscoveryNode localNode = TestDiscoveryNode.builder("foo").roles(Collections.emptySet()).build();
    ...
    public void testResetStartingSeqNoIfLastCommitCorrupted() throws Exception {
        IndexShard shard = newStartedShard(false);
        populateRandomData(shard);
        DiscoveryNode pNode = TestDiscoveryNode.builder("foo").roles(Collections.emptySet()).build();
        DiscoveryNode rNode = TestDiscoveryNode.builder("foo").roles(Collections.emptySet()).build();

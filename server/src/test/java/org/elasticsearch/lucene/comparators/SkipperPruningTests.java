...
public class SkipperPruningTests extends ESTestCase {
    ...
    public void testDVSkipperCompetitiveIteratorDisabledBySetDisableSkipping() throws IOException {
        Directory dir = newDirectory();
        Sort indexSort = new Sort(new SortField("@timestamp", SortField.Type.LONG, true));
        IndexWriterConfig iwc = new IndexWriterConfig(new MockAnalyzer(random())).setIndexSort(indexSort);
        RandomIndexWriter indexWriter = new RandomIndexWriter(random(), dir, iwc);
        ...
        // With setDisableSkipping(), Pruning.NONE is passed so no competitive iterator is built,
        // even though topValue (search_after) is set — all docs must be visited for accurate total hits.
        assertThat(leafComparator.competitiveIterator(), equalTo(null));

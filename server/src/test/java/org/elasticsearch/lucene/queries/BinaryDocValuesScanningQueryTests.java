...
public class BinaryDocValuesScanningQueryTests extends ESTestCase {
    ...
    public void testTermQuery() throws IOException {
        assertMatchCount(new ScanningBinaryDocValuesTermQuery(FIELD, new BytesRef("research"), false), 1);
    }

    public void testPrefixQuery() throws IOException {
        assertMatchCount(new ScanningBinaryDocValuesPrefixQuery(FIELD, "re", false, false), 1);
    }
    ...
    public void testRegexpQuery() throws IOException {
        assertMatchCount(
            new ScanningBinaryDocValuesRegexpQuery(FIELD, "re.*", RegExp.ALL, 0, Operations.DEFAULT_DETERMINIZE_WORK_LIMIT, false, null),
            1
        );
    }
    ...
    public void testTermInSetQuery() throws IOException {
        assertMatchCount(
            new ScanningBinaryDocValuesTermInSetQuery(FIELD, List.of(new BytesRef("kibana"), new BytesRef("research")), false),
            2
        );
    }

    public void testContainsQuery() throws IOException {
        assertMatchCount(new BinaryDocValuesContainsTermQuery(FIELD, new BytesRef("search"), false), 3);
    }

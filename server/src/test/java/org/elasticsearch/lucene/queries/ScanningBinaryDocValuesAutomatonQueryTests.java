...
public class ScanningBinaryDocValuesAutomatonQueryTests extends ESTestCase {
    ...
    public void testForCaseInsensitiveTerm() throws Exception {
        String fieldName = "field";
        boolean arrayOrderInlineNull = randomBoolean();
        try (Directory dir = newDirectory()) {
            try (
                RandomIndexWriter writer = arrayOrderInlineNull ? ArrayOrderInlineNullTestUtils.newWriter(dir) : newRandomIndexWriter(dir)
            ) {
                ...
                try (IndexReader reader = writer.getReader()) {
                    ...
                    assertEquals(
                        1,
                        searcher.count(
                            ScanningBinaryDocValuesAutomatonQuery.forCaseInsensitiveTerm(fieldName, "elasticsearch", arrayOrderInlineNull)
                        )
                    );
                    assertEquals(
                        1,
                    ...
                    assertEquals(
                        1,
                        searcher.count(
                            ScanningBinaryDocValuesAutomatonQuery.forCaseInsensitiveTerm(fieldName, "kibana", arrayOrderInlineNull)
                        )
                    );
                    ...
                    assertEquals(
                        1,
                        searcher.count(
                            ScanningBinaryDocValuesAutomatonQuery.forCaseInsensitiveTerm(fieldName, "logstash", arrayOrderInlineNull)
                        )
                    );
                    ...
                    assertEquals(
                        0,
                        searcher.count(
                            ScanningBinaryDocValuesAutomatonQuery.forCaseInsensitiveTerm(fieldName, "beats", arrayOrderInlineNull)
                        )
                    );
                }
            }
            ...
            try (
                ...
                try (IndexReader reader = writer.getReader()) {
                    ...
                    assertEquals(
                        3,
                        searcher.count(
                            ScanningBinaryDocValuesAutomatonQuery.forFuzzy(fieldName, "elastic", 1, 0, true, arrayOrderInlineNull)
                        )
                    );
                    ...
                    assertEquals(
                        1,
                        searcher.count(
                            ScanningBinaryDocValuesAutomatonQuery.forFuzzy(fieldName, "elastic", 0, 0, true, arrayOrderInlineNull)
                        )
                    );
                }
            }

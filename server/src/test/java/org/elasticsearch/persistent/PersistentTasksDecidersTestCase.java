...
public abstract class PersistentTasksDecidersTestCase extends ESTestCase {
    ...
    protected static ClusterState createClusterStateWithTasks(final int nbNodes, final int nbTasks) {
        ...
        for (int i = 0; i < nbTasks; i++) {
            tasks.addTask("_task_" + i, "test", null, new PersistentTasksExtensionMetadata.Assignment(null, "initialized"));
        }

        Metadata metadata = Metadata.builder().putCustom(PersistentTasksExtensionMetadata.TYPE, tasks.build()).build();
    ...
    protected static void assertNbAssignedTasks(final long nbTasks, final ClusterState clusterState) {
        assertPersistentTasks(nbTasks, clusterState, PersistentTasksExtensionMetadata.PersistentTask::isAssigned);
    }

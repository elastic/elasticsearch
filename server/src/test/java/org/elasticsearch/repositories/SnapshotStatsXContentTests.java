...
public class SnapshotStatsXContentTests extends ESTestCase {
     ...
     * @throws IOException thrown by {@link RepositoriesStats.SnapshotStats#toXContent}
     */
    ...
    public void testToXContentWithHumanReadableNames() throws IOException {
        ...
        assertEquals(totalReadThrottledNanos, map.get("total_read_throttled_time_nanos"));
        assertEquals(new TimeValue(totalReadThrottledNanos, TimeUnit.NANOSECONDS).toString(), map.get("total_read_throttled_time"));

        assertEquals(totalWriteThrottledNanos, map.get("total_write_throttled_time_nanos"));
        assertEquals(new TimeValue(totalWriteThrottledNanos, TimeUnit.NANOSECONDS).toString(), map.get("total_write_throttled_time"));
        ...
        assertEquals(numberOfBytesUploaded, map.get("uploaded_size_in_bytes"));
        assertEquals(ByteSizeValue.ofBytes(numberOfBytesUploaded).toString(), map.get("uploaded_size"));

        assertEquals(TimeUnit.NANOSECONDS.toMillis(totalUploadTimeInNanos), map.get("total_upload_time_in_millis"));
        assertEquals(
            TimeValue.timeValueMillis(TimeUnit.NANOSECONDS.toMillis(totalUploadTimeInNanos)).toString(),
            map.get("total_upload_time")
        );

        assertEquals(TimeUnit.NANOSECONDS.toMillis(totalUploadReadTimeInNanos), map.get("total_read_time_in_millis"));
        assertEquals(
            TimeValue.timeValueMillis(TimeUnit.NANOSECONDS.toMillis(totalUploadReadTimeInNanos)).toString(),
            map.get("total_read_time")
        );

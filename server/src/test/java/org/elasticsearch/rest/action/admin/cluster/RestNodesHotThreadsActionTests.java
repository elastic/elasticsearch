...
public class RestNodesHotThreadsActionTests extends ESTestCase {
    ...
    public void testRouteClusterNodesHotUnderlineThreads() {
        final HashMap<String, String> params = new HashMap<>();
        params.put("nodeId", "node_test");
        final RestRequest request = new FakeRestRequest.Builder(xContentRegistry())
        .withPath("/_cluster/nodes/hot_threads").build();
        try {
            action.prepareRequest(request, mock(NodeClient.class));
        } catch (Exception e) {
            
        }
        assertWarnings(RestNodesHotThreadsAction.DEPRECATED_MESSAGE_CLUSTER_NODES_HOT_THREADS);
    }

    public void testRouteClusterNodesNodeIdHotUnderlineThreads() {
        final HashMap<String, String> params = new HashMap<>();
        params.put("nodeId", "node_test");
        final RestRequest request = new FakeRestRequest.Builder(xContentRegistry())
        .withPath("/_cluster/nodes/{nodeId}/hot_threads").build();
        try {
            action.prepareRequest(request, mock(NodeClient.class));
        } catch (Exception e) {
            
        }
        assertWarnings(RestNodesHotThreadsAction.DEPRECATED_MESSAGE_CLUSTER_NODES_NODEID_HOT_THREADS);
    }

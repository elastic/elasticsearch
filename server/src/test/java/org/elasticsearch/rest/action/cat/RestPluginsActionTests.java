...
public class RestPluginsActionTests extends ESTestCase {
    ...
    public void testNoPlugins() {
        final Table table = buildTable(emptyList(), false);
    ...
    public void testIsolatedPluginOnly() {
        final Table table = buildTable(
            asList(plugin("test-plugin", PluginType.ISOLATED), plugin("ignored-plugin", PluginType.BOOTSTRAP)),
            false
        );
    ...
    public void testIncludeBootstrap() {
        final Table table = buildTable(
            asList(plugin("test-plugin", PluginType.ISOLATED), plugin("bootstrap-plugin", PluginType.BOOTSTRAP)),
            true
        );
        ...
        for (int i = 0; i < 3; i++) {
            for (String pluginName : asList("test-plugin", "bootstrap-plugin")) {
                matchers.add(
                    contains(
                        Integer.toString(i),
                        "node-" + i,
    ...
    private Table buildTable(List<PluginInfo> pluginInfo, boolean includeBootstrap) {
        ...
        for (int i = 0; i < 3; i++) {
            nodeInfos.add(
                new NodeInfo(
                    ...
                    null,
                    null,
                    new PluginsAndModules(pluginInfo, emptyList()),
                    null,
                    null,
                    null
                )
        ...
        NodesInfoResponse nodesInfoResponse = new NodesInfoResponse(clusterName, nodeInfos, emptyList());
    ...
    private PluginInfo plugin(String name, PluginType type) {
        return new PluginInfo(name, name + " description", "1.0", null, null, null, emptyList(), false, type, null, false);
    }
}

...
public class GeoGridIT extends ESIntegTestCase {
    ...
    public void testSimple() throws Exception {
        for (int precision = 1; precision <= PRECISION; precision++) {
            ...
            GeoGrid geoGrid = response.getAggregations().get("geogrid");
    ...
    public void testMultivalued() throws Exception {
        for (int precision = 1; precision <= PRECISION; precision++) {
            ...
            GeoGrid geoGrid = response.getAggregations().get("geogrid");
            for (Bucket cell : geoGrid.getBuckets()) {
                String geohash = cell.getKeyAsString();

                long bucketCount = cell.getDocCount();
                int expectedBucketCount = multiValuedExpectedDocCountsForGeoHash.get(geohash);
                assertNotSame(bucketCount, 0);
                assertEquals("Geogrid " + geohash + " has wrong doc count ",
                        expectedBucketCount, bucketCount);
            }
        }
    }
    ...
    public void testFiltered() throws Exception {
        ...
        for (int precision = 1; precision <= PRECISION; precision++) {
            ...
            GeoGrid geoGrid = filter.getAggregations().get("geogrid");
            for (Bucket cell : geoGrid.getBuckets()) {
                String geohash = cell.getKeyAsString();
                long bucketCount = cell.getDocCount();
                int expectedBucketCount = expectedDocCountsForGeoHash.get(geohash);

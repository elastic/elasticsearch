...
public class GeoGridAggregatorTests extends AggregatorTestCase {

    private static final String FIELD_NAME = "location";

    public void testNoDocs() throws IOException {
        final GeoGridType type = GeoGridTests.randomType();
        testCase(new MatchAllDocsQuery(), FIELD_NAME, type, GeoGridTests.randomPrecision(type), iw -> {
    ...
    public void testFieldMissing() throws IOException {
        final GeoGridType type = GeoGridTests.randomType();
        testCase(new MatchAllDocsQuery(), "wrong_field", type, GeoGridTests.randomPrecision(type), iw -> {
    ...
    public void testHashcodeWithSeveralDocs() throws IOException {
        final GeoGridType type = GeoGridTests.randomType();
        testWithSeveralDocs(type, GeoGridTests.randomPrecision(type));
    }

    private void testWithSeveralDocs(GeoGridType type, int precision)
        throws IOException {
        ...
        testCase(new MatchAllDocsQuery(), FIELD_NAME, type, precision, iw -> {
            ...
            for (int pointId = 0; pointId < numPoints; pointId++) {
                ...
                String hash = type.hashAsString(type.calculateHash(lng, lat, precision));
    ...
    private void testCase(Query query, String field, GeoGridType type, int precision,
                          CheckedConsumer<RandomIndexWriter, IOException> buildIndex,
                          Consumer<InternalGeoGrid> verify) throws IOException {
        ...
        GeoGridAggregationBuilder2 aggregationBuilder = new GeoGridAggregationBuilder2("_name", type).field(field);
        ...
        verify.accept((InternalGeoGrid) aggregator.buildAggregation(0L));

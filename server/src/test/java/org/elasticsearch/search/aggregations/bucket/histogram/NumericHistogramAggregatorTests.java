...
public class NumericHistogramAggregatorTests extends AggregatorTestCase {
        ...
        try (Directory dir = newDirectory(); RandomIndexWriter w = new RandomIndexWriter(random(), dir)) {
            ...
            try (IndexReader reader = w.getReader()) {
                ...
                InternalHistogram histogram = searchAndReduce(
                    new AggTestConfig(searcher, new MatchAllDocsQuery(), aggBuilder, longField("field"))
                );
    ...
    public void testDoubles() throws Exception {
        try (Directory dir = newDirectory(); RandomIndexWriter w = new RandomIndexWriter(random(), dir)) {
            ...
            try (IndexReader reader = w.getReader()) {
                IndexSearcher searcher = new IndexSearcher(reader);
                InternalHistogram histogram = searchAndReduce(
                    new AggTestConfig(searcher, new MatchAllDocsQuery(), aggBuilder, doubleField("field"))
                );
                assertEquals(6, histogram.getBuckets().size());
                assertEquals(6, histogram.getBuckets().size());
                assertEquals(-10d, histogram.getBuckets().get(0).getKey());
    ...
    public void testDates() throws Exception {
        ...
        try (Directory dir = newDirectory(); RandomIndexWriter indexWriter = new RandomIndexWriter(random(), dir)) {
            ...
            try (IndexReader reader = indexWriter.getReader()) {
                IndexSearcher searcher = new IndexSearcher(reader);
                InternalHistogram histogram = searchAndReduce(new AggTestConfig(searcher, new MatchAllDocsQuery(), aggBuilder, fieldType));
                assertTrue(AggregationInspectionHelper.hasValue(histogram));
            }
        }
    }
    ...
    public void testMissingUnmappedField() throws Exception {
        try (Directory dir = newDirectory(); RandomIndexWriter w = new RandomIndexWriter(random(), dir)) {
            ...
            try (IndexReader reader = w.getReader()) {
                IndexSearcher searcher = new IndexSearcher(reader);
                InternalHistogram histogram = searchAndReduce(new AggTestConfig(searcher, new MatchAllDocsQuery(), aggBuilder));
        ...
        try (Directory dir = newDirectory(); RandomIndexWriter w = new RandomIndexWriter(random(), dir)) {
            ...
            try (IndexReader reader = w.getReader()) {
                ...
                Throwable t = expectThrows(
                    ...
                    () -> { searchAndReduce(new AggTestConfig(searcher, new MatchAllDocsQuery(), aggBuilder)); }
                );
    ...
    public void testIncorrectFieldType() throws Exception {
        try (Directory dir = newDirectory(); RandomIndexWriter w = new RandomIndexWriter(random(), dir)) {
            ...
            try (IndexReader reader = w.getReader()) {
                ...
                expectThrows(
                    IllegalArgumentException.class,
                    () -> { searchAndReduce(new AggTestConfig(searcher, new MatchAllDocsQuery(), aggBuilder, keywordField("field"))); }
                );
            }
        }

...
public class VariableWidthHistogramAggregatorTests extends AggregatorTestCase {
    ...
    public void testAsSubAggregation() throws IOException {
        ...
        Exception e = expectThrows(
            ...
            () -> testCase(
                buildIndex,
                new AggTestConfig(builder, agg -> verify.accept((LongTerms) agg), longField("t"), longField("v")).withQuery(DEFAULT_QUERY)
            )
        );
    ...
    private void testSearchCase(
        ...
        try (Directory directory = newDirectory()) {
            ...
            try (IndexReader indexReader = DirectoryReader.open(directory)) {
                ...
                searchAndReduce(
                    indexSearcher,
                    new AggTestConfig(aggregationBuilder, agg -> verify.accept((InternalVariableWidthHistogram) agg), fieldType).withQuery(
                        query
                    )
                );
            }

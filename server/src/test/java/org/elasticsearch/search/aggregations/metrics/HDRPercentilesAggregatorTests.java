...
public class HDRPercentilesAggregatorTests extends AggregatorTestCase {
    ...
    public void testRangeField() throws IOException {
        ...
        MappedFieldType fieldType = new RangeFieldMapper.RangeFieldType(fieldName, BasicRangeType.DOUBLE);
        RangeFieldMapper.Range range =new RangeFieldMapper.Range(BasicRangeType.DOUBLE, 1.0D, 5.0D, true, true);
        BytesRef encodedRange = BasicRangeType.DOUBLE.encodeRanges(Collections.singleton(range));
        expectThrows(IllegalArgumentException.class,
            () -> testCase(new DocValuesFieldExistsQuery(fieldName), iw -> {

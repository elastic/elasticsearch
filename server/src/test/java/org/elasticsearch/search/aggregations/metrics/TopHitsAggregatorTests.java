...
public class TopHitsAggregatorTests extends AggregatorTestCase {
    ...
    private Aggregation testCase(Query query, AggregationBuilder builder) throws IOException {
        ...
        Aggregation result = searchAndReduce(new AggTestConfig(indexSearcher, builder, STRING_FIELD_TYPE).withQuery(query));
        indexReader.close();
        directory.close();
        return result;
    }
    ...
    public void testSetScorer() throws Exception {
        ...
        TopHits result = searchAndReduce(new AggTestConfig(searcher, agg, STRING_FIELD_TYPE).withQuery(query));

...
public class ValuesSourceConfigTests extends MapperServiceTestCase {
    ...
    public void testUnmappedKeyword() throws Exception {
        ...
        withAggregationContext(mapperService, List.of(source(b -> {})), context -> {
            ValuesSourceConfig config;
            config = ValuesSourceConfig.resolve(context, CoreValuesSourceType.ValueType.STRING, "field", null, null, null, null, CoreValuesSourceType.BYTES);
            ValuesSource.Bytes valuesSource = (ValuesSource.Bytes) config.getValuesSource();
            assertNotNull(valuesSource);
            assertFalse(config.hasValues());
            assertFalse(config.alignesWithSearchIndex());

            config = ValuesSourceConfig.resolve(context, CoreValuesSourceType.ValueType.STRING, "field", null, "abc", null, null, CoreValuesSourceType.BYTES);
            valuesSource = (ValuesSource.Bytes) config.getValuesSource();
            LeafReaderContext ctx = context.searcher().getIndexReader().leaves().get(0);
            SortedBinaryDocValues values = valuesSource.bytesValues(ctx);
            assertTrue(values.advanceExact(0));
    ...
    public void testUnmappedLong() throws Exception {
        ...
        withAggregationContext(mapperService, List.of(source(b -> {})), context -> {
            ValuesSourceConfig config;
            config = ValuesSourceConfig.resolve(context, CoreValuesSourceType.ValueType.NUMBER, "field", null, null, null, null, CoreValuesSourceType.BYTES);
            ValuesSource.Numeric valuesSource = (ValuesSource.Numeric) config.getValuesSource();
            assertNotNull(valuesSource);
            assertFalse(config.hasValues());
            assertFalse(config.alignesWithSearchIndex());

            config = ValuesSourceConfig.resolve(context, CoreValuesSourceType.ValueType.NUMBER, "field", null, 42, null, null, CoreValuesSourceType.BYTES);
            valuesSource = (ValuesSource.Numeric) config.getValuesSource();
            LeafReaderContext ctx = context.searcher().getIndexReader().leaves().get(0);
            SortedNumericDocValues values = valuesSource.longValues(ctx);
            assertTrue(values.advanceExact(0));
    ...
    public void testUnmappedBoolean() throws Exception {
        ...
        withAggregationContext(mapperService, List.of(source(b -> {})), context -> {
            ValuesSourceConfig config;
            config = ValuesSourceConfig.resolve(context, CoreValuesSourceType.ValueType.BOOLEAN, "field", null, null, null, null, CoreValuesSourceType.BYTES);
            ValuesSource.Numeric valuesSource = (ValuesSource.Numeric) config.getValuesSource();
            assertNotNull(valuesSource);
            assertFalse(config.hasValues());
            assertFalse(config.alignesWithSearchIndex());

            config = ValuesSourceConfig.resolve(context, CoreValuesSourceType.ValueType.BOOLEAN, "field", null, true, null, null, CoreValuesSourceType.BYTES);
            valuesSource = (ValuesSource.Numeric) config.getValuesSource();
            LeafReaderContext ctx = context.searcher().getIndexReader().leaves().get(0);
            SortedNumericDocValues values = valuesSource.longValues(ctx);
            assertTrue(values.advanceExact(0));
    ...
    public void testFieldAlias() throws Exception {
        ...
        withAggregationContext(mapperService, List.of(source(b -> b.field("field", "value"))), context -> {
            ValuesSourceConfig config;
            config = ValuesSourceConfig.resolve(context, CoreValuesSourceType.ValueType.STRING, "alias", null, null, null, null, CoreValuesSourceType.BYTES);
            ValuesSource.Bytes valuesSource = (ValuesSource.Bytes) config.getValuesSource();

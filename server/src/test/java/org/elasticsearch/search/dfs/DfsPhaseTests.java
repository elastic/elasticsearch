    ...
    public void testSingleKnnSearch() throws IOException {
        try (Directory dir = newDirectory();
            ...
            for (int i = 0; i < numDocs; i++) {
                ...
                d.add(new XKnnFloatVectorField("float_vector", new float[] { i, 0, 0 }));
                w.addDocument(d);
            }
            w.flush();
            ...
            DfsKnnResults dfsKnnResults = DfsPhase.singleKnnSearch(query, numDocs, null, searcher);
            assertEquals(numDocs, dfsKnnResults.scoreDocs().length);

//            SearchProfileDfsPhaseResult searchProfileDfsPhaseResult = dfsProfiler.buildDfsPhaseResults();

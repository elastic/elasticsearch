...
public class FetchProfilerTests extends ESTestCase {
    ...
    public void testStoredFieldsIsOrdered() throws IOException {
        ...
        profiler.visitor(new CustomFieldsVisitor(org.elasticsearch.core.Set.of(), true));
        ProfileResult result = profiler.finish();
        assertMap(result.getDebugInfo(), matchesMap().entry("stored_fields", org.elasticsearch.core.List.of("_id", "_routing", "_source")));
        // Make sure that serialization preserves the order
        ProfileResult copy = copyWriteable(result, new NamedWriteableRegistry(org.elasticsearch.core.List.of()), ProfileResult::new);
        assertMap(copy.getDebugInfo(), matchesMap().entry("stored_fields", org.elasticsearch.core.List.of("_id", "_routing", "_source")));
    }
}

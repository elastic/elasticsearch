...
/**
 * Abstract test base for sliced diversifying children IVF kNN queries. Provides shared
 * slice-aware test infrastructure used by both byte and float subclasses.
 */
public abstract class AbstractDiversifyingChildrenIVFKnnSlicedVectorQueryTestCase<V> extends
    AbstractDiversifyingChildrenIVFKnnVectorQueryTestCase<V> {

    protected static final BytesRef SLICE_ZERO = new BytesRef("0");

    abstract Query createSlicedDiversifyingQuery(
        String field,
        V vector,
        int k,
        int numCands,
    ...
    public void testScoringWithMultipleChildren() throws IOException {
        try (Directory d = newDirectory()) {
            try (IndexWriter w = new IndexWriter(d, slicedIndexWriterConfig().setCodec(TestUtil.getDefaultCodec()))) {
                List<Document> toAdd = new ArrayList<>();
                for (int j = 1; j <= 5; j++) {
                    Document doc = new Document();
        ...
        try (Directory dir = newDirectory(); IndexWriter w = new IndexWriter(dir, iwc)) {
            for (int i = 0; i < numParents; i++) {
                ...
                for (int c = 0; c < numChildren; c++) {
                    ...
                    if (hasVector) {
                        child.add(getKnnVectorField("vector", randomDenseQueryVector(dimensions)));
            ...
            V queryVector = randomDenseQueryVector(dimensions);
            try (IndexReader reader = DirectoryReader.open(w)) {
                ...
                for (int iters = 0; iters < 2; iters++) {
                    ...
                    for (int slice = 0; slice < numSlices; slice++) {
                        ...
                        TopDocs topDocs = getTopDocs(expectedDocs, queryVector, filterQuery, parents, searcher, slice);
                    ...
                    for (int i = 0; i < 10; i++) {
                        ...
                        TopDocs topDocs = getTopDocs(expectedDocs, queryVector, filterQuery, parents, searcher, querySlices);
                        assertTopDocs(applyFilter, expectedDocs, topDocs, reader, filterField, filterValue, querySlices);
                    }
                    // non-existing slice
                    TopDocs topDocs = getTopDocs(0, queryVector, filterQuery, parents, searcher, -1);
                    assertEquals(0, topDocs.scoreDocs.length);
                }
            }
        }
    ...
    public void testMultiSlice() throws IOException {
        ...
        try (Directory dir = newDirectory(); IndexWriter w = new IndexWriter(dir, slicedIndexWriterConfig())) {
            ...
            try (IndexReader reader = DirectoryReader.open(w)) {
                ...
                TopDocs topDocs = getTopDocs(expectedTotal, queryVector, null, parents, searcher, sliceA, sliceB);
                assertEquals(expectedTotal, topDocs.scoreDocs.length);
                // Verify results come from requested slices
                for (int i = 0; i < topDocs.scoreDocs.length; i++) {
                    ...
                    assertTrue(sliceValue.equals("" + sliceA) || sliceValue.equals("" + sliceB));
                }
            }
        }
    }
    ...
    public void testAllSlices() throws IOException {
        ...
        try (Directory dir = newDirectory(); IndexWriter w = new IndexWriter(dir, slicedIndexWriterConfig())) {
            ...
            try (IndexReader reader = DirectoryReader.open(w)) {
                ...
                Query kvq = createSlicedDiversifyingQuery(
                    "vector",
                    queryVector,
                    k,
                    k,
    ...
    private TopDocs getTopDocs(int expectedDocs, V vector, Query filterQuery, BitSetProducer parents, IndexSearcher searcher, int... slice)
        throws IOException {
    ...
    protected static void assertTopDocs(
        boolean applyFilter,
        int expectedDocs,
        TopDocs topDocs,
        IndexReader reader,

...
public class CorruptedBlobStoreRepositoryIT extends AbstractSnapshotIntegTestCase {
    ...
    public void testHandlingMissingRootLevelSnapshotMetadata() throws Exception {
        ...
        fullRestart();
        final Repository repositoryAfterRestart =
        ...
        assertThat(PlainActionFuture.get(f -> threadPool.generic().execute(
            ActionRunnable.supply(f, () ->
                snapshotsService.hasOldVersionSnapshots(repoName, getRepositoryData(repositoryAfterRestart), null)))),
            is(true));
        ...
        assertThat(PlainActionFuture.get(f -> threadPool.generic().execute(
            ActionRunnable.supply(f, () -> snapshotsService.hasOldVersionSnapshots(repoName,
                getRepositoryData(repositoryAfterRestart), null)))),
            is(false));
        final RepositoryData finalRepositoryData = getRepositoryData(repositoryAfterRestart);
    ...
    public void testMountCorruptedRepositoryData() throws Exception {
        ...
        expectThrows(RepositoryException.class, () -> getRepositoryData(
            internalCluster().getCurrentMasterNodeInstance(RepositoriesService.class).repository(repoName)));

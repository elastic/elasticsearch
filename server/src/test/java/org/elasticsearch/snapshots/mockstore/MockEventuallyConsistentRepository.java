...
public class MockEventuallyConsistentRepository extends BlobStoreRepository {
    ...
    public MockEventuallyConsistentRepository(
        ...
        super(metadata, false, namedXContentRegistry, clusterService, MockBigArrays.NON_RECYCLING_INSTANCE, recoverySettings);
    ...
    private class MockBlobStore implements BlobStore {
        ...
        private class MockBlobContainer implements BlobContainer {
            ...
            public void deleteBlobsIgnoringIfNotExists(Iterator<String> blobNames) {
                ensureNotClosed();
                synchronized (context.actions) {
                    blobNames.forEachRemaining(
                        blobName -> context.actions.add(new BlobStoreAction(Operation.DELETE, path.buildAsString() + blobName))
                    );
                }
            }

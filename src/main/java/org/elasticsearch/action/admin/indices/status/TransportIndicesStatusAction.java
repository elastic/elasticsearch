...
//            shardStatus.estimatedFlushableMemorySize = indexShard.estimateFlushableMemorySize();
        ...
        if (request.recovery) {
            ...
            RecoveryState gatewayRecoveryStatus = gatewayService.recoveryStatus();
            if (gatewayRecoveryStatus != null) {
                GatewayRecoveryStatus.Stage stage;
                switch (gatewayRecoveryStatus.getStage()) {
                    case INIT:
                ...
                shardStatus.gatewayRecoveryStatus = new GatewayRecoveryStatus(stage, gatewayRecoveryStatus.getTimer().startTime(), gatewayRecoveryStatus.getTimer().time(),
                        gatewayRecoveryStatus.getIndex().totalByteCount(), gatewayRecoveryStatus.getIndex().reusedByteCount(), gatewayRecoveryStatus.getIndex().recoveredByteCount(), gatewayRecoveryStatus.getTranslog().currentTranslogOperations());
            }
        }

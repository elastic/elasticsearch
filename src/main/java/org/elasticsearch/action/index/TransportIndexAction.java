...
public class TransportIndexAction extends TransportShardReplicationOperationAction<IndexRequest, IndexRequest, IndexResponse> {
    ...
    protected void doExecute(final IndexRequest request, final ActionListener<IndexResponse> listener) {
        ...
        if (autoCreateIndex.shouldAutoCreate(request.index(), clusterService.state())) {
            ...
            createIndexAction.execute(new CreateIndexRequest(request.index()).setCause("auto(index api)").masterNodeTimeout(request.timeout()), new ActionListener<CreateIndexResponse>() {
                @Override

...
public class TransportUpdateAction extends TransportInstanceSingleOperationAction<UpdateRequest, UpdateResponse> {
    ...
    protected void shardOperation(final UpdateRequest request, final ActionListener<UpdateResponse> listener, final int retryCount) throws ElasticSearchException {
        ...
        final ExternalSourceProvider sourceProvider = indexService.mapperService().sourceProvider();
        ...
        if (operation == null || "index".equals(operation)) {
            ...
            indexAction.execute(indexRequest, new ActionListener<IndexResponse>() {
                ...
                public void onResponse(IndexResponse response) {
                    ...
                    update.getResult(extractGetResult(request, response.version(), updatedSourceAsMap, updateSourceContentType, updateSourceBytes, sourceProvider));
            ...
            deleteAction.execute(deleteRequest, new ActionListener<DeleteResponse>() {
                ...
                public void onResponse(DeleteResponse response) {
                    UpdateResponse update = new UpdateResponse(response.index(), response.type(), response.id(), response.version());
                    update.getResult(extractGetResult(request, response.version(), updatedSourceAsMap, updateSourceContentType, null, sourceProvider));
                    listener.onResponse(update);
                }
            ...
            update.getResult(extractGetResult(request, getResult.version(), updatedSourceAsMap, updateSourceContentType, null, sourceProvider));
            listener.onResponse(update);
        } else {
    ...
    @Nullable
    protected GetResult extractGetResult(final UpdateRequest request, long version, final Map<String, Object> source, XContentType sourceContentType, @Nullable final BytesHolder sourceAsBytes, ExternalSourceProvider sourceProvider) {
        ...
        if (request.fields() != null && request.fields().length > 0) {
            SourceLookup sourceLookup = new SourceLookup(sourceProvider);

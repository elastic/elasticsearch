            ...
            try {
                ...
                for (Object task : tasks) {
                    if (task instanceof UpdateTask) {
                        ((UpdateTask) task).listener.onResponse();
                    }
                }
            }
        }
            ...
            public ClusterState execute(final ClusterState currentState) throws Exception {
                ...
                try {
                    ...
                    for (String index : request.indices()) {
                        ...
                        // only add the current relevant mapping (if exists)
                        if (indexMetaData.mappings().containsKey(request.type())) {
                            indexService.mapperService().merge(request.type(), indexMetaData.mappings().get(request.type()).source().string(), false);
                        }
                    }
                    ...
                    for (String index : request.indices()) {
                        ...
                        if (indexService != null) {
                            ...
                            if (MapperService.DEFAULT_MAPPING.equals(request.type())) {
                                ...
                                newMapper = indexService.mapperService().parse(request.type(), request.source(), false);
                            } else {
                                newMapper = indexService.mapperService().parse(request.type(), request.source());

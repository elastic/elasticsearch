...
public class DiskThresholdDecider extends AllocationDecider {
    ...
    public Decision canAllocate(ShardRouting shardRouting, RoutingNode node, RoutingAllocation allocation) {
        ...
        if (freeDiskPercentage < freeDiskThresholdLow) {
            ...
            if (!shardRouting.primary() || (shardRouting.primary() && primaryHasBeenAllocated)) {
                if (logger.isDebugEnabled()) {
                    logger.debug("More than the required {} used disk threshold ({} used) on node [{}], preventing allocation",
                ...
                // has never been allocated if it's under the high watermark
                if (logger.isDebugEnabled()) {
                    logger.debug("More than the required {} used disk threshold ({} used) on node [{}], " +
                                    "but allowing allocation because primary has never been allocated",
                            Strings.format1Decimals(usedDiskThresholdLow, "%"),
                            Strings.format1Decimals(usedDiskPercentage, "%"), node.nodeId());
                }
                ...
                return allocation.decision(Decision.NO, NAME, "less than required [%s%%] free disk on node, free: [%s%%]",
                        100.0 - freeDiskThresholdLow, freeDiskPercentage);
            }
        }
        ...
        if (freeSpaceAfterShard < freeDiskThresholdHigh) {
            ...
            return allocation.decision(Decision.NO, NAME, "after allocation more than required [%s%%] used disk on node, free: [%s%%]",

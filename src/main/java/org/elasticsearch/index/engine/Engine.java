...
import static org.elasticsearch.common.lucene.SegmentReaderUtils.segmentReader;
...
public abstract class Engine implements Closeable {
    ...
    protected Segment[] getSegmentInfo(SegmentInfos lastCommittedSegmentInfos) {
        ensureOpen();
    ...
    public static class CommitId implements Streamable {

        private BytesRef id;

        public CommitId(BytesRef id) {
            assert id != null;
            this.id = BytesRef.deepCopyOf(id);
        }
        ...
        public String toString() {
            return Base64.encodeBytes(id.bytes, id.offset, id.length);
        }

        @Override
        public void readFrom(StreamInput in) throws IOException {
            this.id = in.readBytesRef();
        }

        @Override
        public void writeTo(StreamOutput out) throws IOException {
            out.writeBytesRef(id);
        }

        public boolean idsEqual(BytesRef id) {
            return this.id.bytesEquals(id);
        }
        ...
        public boolean equals(Object o) {
            ...
            if (!this.id.bytesEquals(commitId.id)) return false;
        ...
        public static CommitId readCommitID(Store store, SegmentInfos info) throws IOException {
            return new CommitId(Store.MetadataSnapshot.hashFile(store.directory(), info.getSegmentsFileName()));
        }
    }
}

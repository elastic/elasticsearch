...
public class BinaryDVAtomicFieldData implements AtomicFieldData<ScriptDocValues.Strings> {
    ...
    private BinaryDocValues getDocValues() {
        final BinaryDocValues values;
        try {
            values = reader.getBinaryDocValues(field);
        } catch (IOException e) {
            throw new ElasticSearchIllegalStateException("Cannot load doc values", e);
        }
        return values != null ? values : BinaryDocValues.EMPTY;
    }

    @Override
    public BytesValues getBytesValues() {
        final BinaryDocValues values = getDocValues();
        ...
        return new BytesValues(false) {
            ...
            public boolean hasValue(int docId) {
                ...
                return spare.bytes != BinaryDocValues.MISSING;
            }
            ...
            public Iter getIter(int docId) {
                // LUCENE UPGRADE: getDocsWithField
                values.get(docId, spare);
                if (spare.bytes == BinaryDocValues.MISSING) {

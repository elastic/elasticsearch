...
public class PackedArrayIndexFieldData extends AbstractIndexFieldData<AtomicNumericFieldData> implements IndexNumericFieldData<AtomicNumericFieldData> {
            ...
            if (ordinals.isMultiValued() || formatHint == CommonSettings.MemoryStorageFormat.ORDINALS) {
                ...
                switch (formatHint) {
                    ...
                    case PAGED:
                        ...
                        if (docsWithValues == null) {
                            data = new PackedArrayAtomicFieldData.PagedSingle(dpValues.build());
                        } else {
                            data = new PackedArrayAtomicFieldData.PagedSingleSparse(dpValues.build(), docsWithValues);
                        }
                        break;
                    case ORDINALS:
                        data = new PackedArrayAtomicFieldData.WithOrdinals(values, build);
    ...
    protected CommonSettings.MemoryStorageFormat chooseStorageFormat(AtomicReader reader, PackedLongValues values, Ordinals build, BytesValues.WithOrdinals ordinals,

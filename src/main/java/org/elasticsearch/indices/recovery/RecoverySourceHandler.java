    ...
    public void phase1(final SnapshotIndexCommit snapshot) throws ElasticsearchException {
        ...
        try {
            ...
            if (recoverWithSyncId) {
                ...
                logger.debug("[{}][{}] recovery with sync ID {} numDocs: {} vs. {}", indexName, shardId, recoverySourceSyncId, numDocsSource, numDocsTarget);
                ...
                for (StoreFileMetaData md : Iterables.concat(diff.different, diff.missing)) {
                    if (request.metadataSnapshot().asMap().containsKey(md.name())) {
                        logger.trace("[{}][{}] recovery [phase1] to {}: recovering [{}], exists in local store, but is different: remote [{}], local [{}]",
                                indexName, shardId, request.targetNode(), md.name(), request.metadataSnapshot().get(md.name()), md);
                    } else {

...
public class RangeAggregator extends BucketsAggregator {
    ...
    public InternalAggregation buildAggregation(long owningBucketOrdinal) {
        List<org.elasticsearch.legacy.search.aggregations.bucket.range.Range.Bucket> buckets = Lists.newArrayListWithCapacity(ranges.length);
        for (int i = 0; i < ranges.length; i++) {
            Range range = ranges[i];
            final long bucketOrd = subBucketOrdinal(owningBucketOrdinal, i);
            org.elasticsearch.legacy.search.aggregations.bucket.range.Range.Bucket bucket =
    ...
    public static class Unmapped extends NonCollectingAggregator {
        ...
        public InternalAggregation buildEmptyAggregation() {
            InternalAggregations subAggs = buildEmptySubAggregations();
            List<org.elasticsearch.legacy.search.aggregations.bucket.range.Range.Bucket> buckets = new ArrayList<>(ranges.size());

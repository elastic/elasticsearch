...
public class DoubleTermsAggregator extends TermsAggregator {
    ...
    public DoubleTerms buildAggregation(long owningBucketOrdinal) {
        ...
        return new DoubleTerms(name, order, formatter, bucketCountThresholds.getRequiredSize().value(), bucketCountThresholds.getMinDocCount().value(), Arrays.asList(list));
    }

    @Override
    public DoubleTerms buildEmptyAggregation() {
        return new DoubleTerms(name, order, formatter, bucketCountThresholds.getRequiredSize().value(), bucketCountThresholds.getMinDocCount().value(), Collections.<InternalTerms.Bucket>emptyList());
    }

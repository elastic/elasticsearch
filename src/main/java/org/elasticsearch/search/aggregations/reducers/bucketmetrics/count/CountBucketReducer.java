...
package org.elasticsearch.search.aggregations.reducers.bucketmetrics.count;
...
public class CountBucketReducer extends BucketMetricsReducer {

    public final static Type TYPE = new Type("count_bucket");

    public final static ReducerStreams.Stream STREAM = new ReducerStreams.Stream() {
        @Override
        public CountBucketReducer readResult(StreamInput in) throws IOException {
            CountBucketReducer result = new CountBucketReducer();
            result.readFrom(in);
            return result;
        }
    };
    ...
    private CountBucketReducer() {
    }

    protected CountBucketReducer(String name, String[] bucketsPaths, GapPolicy gapPolicy, @Nullable ValueFormatter formatter,
    ...
    protected InternalAggregation buildAggregation(List<Reducer> reducers, Map<String, Object> metadata) {
        return new InternalSimpleValue(name(), count, formatter, reducers, metadata);
    }
    ...
    public static class Factory extends ReducerFactory {
        ...
        protected Reducer createInternal(Map<String, Object> metaData) throws IOException {
            return new CountBucketReducer(name, bucketsPaths, gapPolicy, formatter, metaData);
        }

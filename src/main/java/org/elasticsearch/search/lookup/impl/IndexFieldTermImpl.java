...
import org.elasticsearch.search.lookup.impl.CachedPositionIteratorImpl;
 ...
 * */
public class IndexFieldTermImpl implements IndexFieldTerm {
    ...
    private int convertToLuceneFlags(int flags) {
        ...
        lucenePositionsFlags |= (flags & IndexLookupImpl.FLAG_FREQUENCIES) > 0 ? PostingsEnum.FREQS : 0x0;
        lucenePositionsFlags |= (flags & IndexLookupImpl.FLAG_POSITIONS) > 0 ? PostingsEnum.POSITIONS : 0x0;
        lucenePositionsFlags |= (flags & IndexLookupImpl.FLAG_PAYLOADS) > 0 ? PostingsEnum.PAYLOADS : 0x0;
        lucenePositionsFlags |= (flags & IndexLookupImpl.FLAG_OFFSETS) > 0 ? PostingsEnum.OFFSETS : 0x0;
    ...
    public IndexFieldTermImpl(String term, String fieldName, LeafIndexLookupImpl indexLookup, int flags) {
        ...
        this.flags = flags;
        boolean doRecord = ((flags & IndexLookupImpl.FLAG_CACHE) > 0);
        if (!doRecord) {
            iterator = new PositionIteratorImpl(this);
        } else {
            iterator = new CachedPositionIteratorImpl(this);
        }

 ...
 *
 */
public class LeafFieldsLookupImpl implements LeafFieldsLookup {
    ...
    private final Map<String, FieldLookupImpl> cachedFieldData = Maps.newHashMap();

    private final SingleFieldsVisitor fieldVisitor;

    LeafFieldsLookupImpl(MapperService mapperService, @Nullable String[] types, LeafReader reader) {
    ...
    private FieldLookup loadFieldData(String name) {
        FieldLookupImpl data = cachedFieldData.get(name);
        if (data == null) {
            ...
            data = new FieldLookupImpl(mapper);
        ...
        return LookupImpl.createProxy(FieldLookup.class, data);
    }

    private void clearCache() {
        for (Entry<String, FieldLookupImpl> entry : cachedFieldData.entrySet()) {

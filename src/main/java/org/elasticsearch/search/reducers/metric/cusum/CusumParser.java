...
public class CusumParser implements Reducer.Parser {
    ...
    protected static final ParseField CUSUM_TARGET = new ParseField("target");

    @Override
    public String type() {
        return InternalCusum.TYPE.name();
    }
    ...
    public ReducerFactory parse(String reducerName, XContentParser parser, SearchContext context) throws IOException {
        ...
        double cusumTarget = 0;
        boolean cusumTargetSet = false;
        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            ...
            } else if (CUSUM_TARGET.match(currentFieldName)) {
                cusumTarget = parser.doubleValue();
                cusumTargetSet = true;
            } else {
        ...
        if (!cusumTargetSet) {
            throw new SearchParseException(context, "Missing [" + CUSUM_TARGET.getPreferredName() + "] in reducer [" + reducerName + "]");
        }

        return new CusumReducer.Factory(reducerName, buckets, fieldName, cusumTarget);
    }

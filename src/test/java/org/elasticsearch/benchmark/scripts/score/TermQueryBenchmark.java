 ...
 *
 */
public class TermQueryBenchmark extends BasicScriptBenchmark {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String clusterName = TermQueryBenchmark.class.getSimpleName();
        ...
        results.init(maxTerms - minTerms, "term query sparse posting lists", "Results for term query with sparse posting lists:", "green", ":");
        // init term queries
        List<Entry<String, RequestInfo>> searchRequests = initTermQueries(minTerms, maxTerms);
        ...
        client.admin().cluster().prepareHealth("test").setWaitForGreenStatus().setTimeout("10s").execute().actionGet();
        results = new Results();
        results.init(maxTerms - minTerms, "term query dense posting list", "Results for term query with dense posting lists:", "blue", ":");
        // init term queries

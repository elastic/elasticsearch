...
public class BasicAnalysisBackwardCompatibilityTests extends ElasticsearchBackwardsCompatIntegrationTest {
    ...
    // when using the turkish locale, capital I is lowercased to the dotless version.
    final static Pattern letterI = Pattern.compile("I");
    
    boolean containsTroublesomeCharacters(String text, Version version) {
        if (version.before(Version.V_1_4_0_Beta1) && complexUnicodeChars.matcher(text).find()) return true;
        if ((Locale.getDefault().getLanguage().equals("tr") || Locale.getDefault().getLanguage().equals("az"))
            && letterI.matcher(text).find()) return true;
        return false;
    }
    ...
    public void testAnalyzerTokensAfterUpgrade() throws IOException, ExecutionException, InterruptedException {
        ...
        final Version bwcVersion = globalCompatibilityVersion();
        for (int i = 0; i < numFields; i++) {
            String input;
            do {

                input = TestUtil.randomAnalysisString(getRandom(), 100, false);
            } while (containsTroublesomeCharacters(input, bwcVersion));
        ...
        for (int i = 0; i < inout.length; i++) {
            ...
            for (int j = 0; j < tokens.size(); j++) {
                ...
                assertEquals(msg,  BaseTokenStreamTestCase.escape(expectedTokens.get(j).getTerm()), BaseTokenStreamTestCase.escape(tokens.get(j).getTerm()));
                assertEquals(msg, expectedTokens.get(j).getPosition(), tokens.get(j).getPosition());
                assertEquals(msg, expectedTokens.get(j).getStartOffset(), tokens.get(j).getStartOffset());
                assertEquals(msg, expectedTokens.get(j).getEndOffset(), tokens.get(j).getEndOffset());
                assertEquals(msg, expectedTokens.get(j).getType(), tokens.get(j).getType());
            }
        }
    }

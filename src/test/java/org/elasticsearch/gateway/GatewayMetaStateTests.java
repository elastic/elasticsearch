...
public class GatewayMetaStateTests extends ElasticsearchAllocationTestCase {
    ...
    ClusterChangedEvent generateCloseEvent(boolean masterEligible) {
        ...
        MetaData metaDataClosed = MetaData.builder()
                .put(IndexMetaData.builder("test").state(IndexMetaData.State.CLOSE).numberOfShards(5).numberOfReplicas(2)).version(metaDataStarted.version() + 1)
                .build();
        previousClusterState = ClusterState.builder(init)
    ...
    public void assertState(ClusterChangedEvent event,
        ...
        if (stateInMemory) {
            ...
            oldIndicesList = relevantIndices.addAll(LocalGatewayMetaState.getRelevantIndices(event.previousState(), oldIndicesList)).build();
        }
        Set<String> newIndicesList = LocalGatewayMetaState.getRelevantIndices(event.state(), oldIndicesList);
        // third, get the actual write info
        Iterator<LocalGatewayMetaState.IndexMetaWriteInfo> indices = LocalGatewayMetaState.resolveStatesToBeWritten(oldIndicesList, newIndicesList, inMemoryMetaData, event.state().metaData()).iterator();

...
public class DuelFieldDataTests extends AbstractFieldDataTests {
    ...
    public void testDuelGeoPoints() throws Exception {
        ...
        while (!list.isEmpty()) {
            ...
            ifdService.clear();
            IndexGeoPointFieldData<?> leftFieldData = ifdService.getForField(new FieldMapper.Names(left.getValue().name().toLowerCase(Locale.ROOT)), left.getKey());

            ifdService.clear();
            IndexGeoPointFieldData<?> rightFieldData = ifdService.getForField(new FieldMapper.Names(right.getValue().name().toLowerCase(Locale.ROOT)), right.getKey());

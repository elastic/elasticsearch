...
public class FunctionScoreTests extends ElasticsearchIntegrationTest {
    ...
    private double[] createRandomWeights(int size) {
        ...
        for (int i = 0; i < weights.length; i++) {
            weights[i] = randomFloat() * (randomBoolean() ? 1.0 : -1.0) * (double) randomInt(100);
        }
        return weights;
    }

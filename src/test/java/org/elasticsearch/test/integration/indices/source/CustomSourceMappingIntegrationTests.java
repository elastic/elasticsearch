...
import org.elasticsearch.test.integration.indices.source.idbased.IdBasedExternalSourceProviderModule;
import org.elasticsearch.test.integration.indices.source.transform.TransformingExternalSourceProviderModule;
import org.elasticsearch.test.unit.index.source.TestExternalSourceProviderModule;
    ...
    protected void createIndex(String index, Settings indexSettings) throws IOException {
        ...
        client("node1").admin().indices().create(createIndexRequest(index)
                .settings(
                        settingsBuilder()
                                .put(indexSettings)
                                .put("index.source.provider.type", TestExternalSourceProviderModule.class.getName())
                )
        ...
        client("node1").admin().indices().create(createIndexRequest("test")
                .settings(
                        settingsBuilder()
                                ...
                                .put("index.source.provider.type", IdBasedExternalSourceProviderModule.class.getName())
                                .put("index.source.provider.id_based.source_pattern", "{\"_id\":\"%2$s\", \"_type\":\"%1$s\", "
        ...
        client("node1").admin().indices().create(createIndexRequest("test")
                .settings(
                        settingsBuilder()
                                .put("index.number_of_replicas", 0)
                                .put("index.number_of_shards", 1)
                                .put("index.source.provider.type", TransformingExternalSourceProviderModule.class.getName())
                )
                .mapping("type1", XContentFactory.jsonBuilder().startObject().startObject("type")
                        .startObject("_source")

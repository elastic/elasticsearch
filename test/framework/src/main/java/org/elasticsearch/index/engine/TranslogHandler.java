...
public class TranslogHandler implements Engine.TranslogRecoveryRunner {
    ...
    public int run(Engine engine, Translog.Snapshot snapshot) throws IOException {
        ...
        while ((operation = snapshot.next()) != null) {
            applyOperation(engine, convertToEngineOp(operation, Engine.Operation.Origin.LOCAL_TRANSLOG_RECOVERY,
                engine.config().getThreadPool().relativeTimeInNanos()));
            opsRecovered++;
    ...
    public Engine.Operation convertToEngineOp(Translog.Operation operation, Engine.Operation.Origin origin, long relativeTimeInNanos) {
        ...
        switch (operation.opType()) {
            case INDEX:
                ...
                final SourceToParse source = new SourceToParse(indexName, index.id(), index.source(),
                    XContentHelper.xContentType(index.source()), index.routing());
                final DocumentMapperForType docMapper = docMapper(MapperService.SINGLE_MAPPING_NAME);
                final ParsedDocument doc = docMapper.getDocumentMapper().parse(source);
                if (docMapper.getMapping() != null) {
                    doc.addDynamicMappingsUpdate(docMapper.getMapping());
                }
                Term uid = new Term(IdFieldMapper.NAME, Uid.encodeId(doc.id()));
                return new Engine.Index(uid, doc, index.seqNo(), index.primaryTerm(), index.version(), versionType, origin,
                    relativeTimeInNanos, index.getAutoGeneratedIdTimestamp(), true,

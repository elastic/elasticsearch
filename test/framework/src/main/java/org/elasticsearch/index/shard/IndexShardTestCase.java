...
public abstract class IndexShardTestCase extends ESTestCase {
    ...
    protected IndexShard newShard(
        ...
        final ShardRouting shardRouting = aShardRouting(shardId, randomAlphaOfLength(10), primary, ShardRoutingState.INITIALIZING)
    ...
    protected IndexShard newShard(ShardId shardId, boolean primary, IndexingOperationListener... listeners) throws IOException {
        ShardRouting shardRouting = aShardRouting(shardId, randomAlphaOfLength(5), primary, ShardRoutingState.INITIALIZING)
    ...
    protected IndexShard newShard(
        ...
        ShardRouting shardRouting = aShardRouting(shardId, nodeId, primary, ShardRoutingState.INITIALIZING).withRecoverySource(
    ...
    protected void promoteReplica(IndexShard replica, Set<String> inSyncIds, IndexShardRoutingTable routingTable) throws IOException {
        ...
        final ShardRouting routingEntry = aShardRouting(
    ...
    protected void recoverShardFromSnapshot(final IndexShard shard, final Snapshot snapshot, final Repository repository) {
        ...
        final ShardRouting shardRouting = aShardRouting(shardId, node.getId(), true, ShardRoutingState.INITIALIZING).withRecoverySource(
            recoverySource
        ).build();

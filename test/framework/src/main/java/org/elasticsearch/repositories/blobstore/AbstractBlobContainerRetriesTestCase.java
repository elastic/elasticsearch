...
public abstract class AbstractBlobContainerRetriesTestCase extends ESTestCase {
    ...
    public void testReadNonexistentBlobThrowsNoSuchFileException() {
        ...
        final Exception exception = expectThrows(NoSuchFileException.class, () -> {
            if (randomBoolean()) {
                Streams.readFully(blobContainer.readBlob(BlobPurpose.SNAPSHOT, "read_nonexistent_blob"));
            } else {
                Streams.readFully(blobContainer.readBlob(BlobPurpose.SNAPSHOT, "read_nonexistent_blob", 0, 1));
            }
        });
        ...
        assertThat(
            expectThrows(
                ...
                () -> Streams.readFully(blobContainer.readBlob(BlobPurpose.SNAPSHOT, "read_nonexistent_blob", position, length))
    ...
    public void testReadRangeBlobWithRetries() throws Exception {
        ...
        try (InputStream inputStream = blobContainer.readBlob(BlobPurpose.SNAPSHOT, "read_range_blob_max_retries", position, length)) {
            final int readLimit;
            final InputStream wrappedStream;
            if (randomBoolean()) {
                // read stream only partly
    ...
    public void testReadBlobWithReadTimeouts() {
        ...
        Exception exception = expectThrows(
            ...
            () -> Streams.readFully(blobContainer.readBlob(BlobPurpose.SNAPSHOT, "read_blob_unresponsive"))
        );
        ...
        exception = expectThrows(Exception.class, () -> {
            try (
                InputStream stream = randomBoolean()
                    ? blobContainer.readBlob(BlobPurpose.SNAPSHOT, "read_blob_incomplete")
                    : blobContainer.readBlob(BlobPurpose.SNAPSHOT, "read_blob_incomplete", position, length)
            ) {
    ...
    public void testReadBlobWithNoHttpResponse() {
        ...
        Exception exception = expectThrows(unresponsiveExceptionType(), () -> {
            if (randomBoolean()) {
                Streams.readFully(blobContainer.readBlob(BlobPurpose.SNAPSHOT, "read_blob_no_response"));
            } else {
                Streams.readFully(blobContainer.readBlob(BlobPurpose.SNAPSHOT, "read_blob_no_response", 0, 1));
            }
        });
        assertThat(
            exception.getMessage().toLowerCase(Locale.ROOT),
    ...
    public void testReadBlobWithPrematureConnectionClose() {
        ...
        final Exception exception = expectThrows(Exception.class, () -> {
            try (
                InputStream stream = randomBoolean()
                    ? blobContainer.readBlob(BlobPurpose.SNAPSHOT, "read_blob_incomplete", 0, 1)
                    : blobContainer.readBlob(BlobPurpose.SNAPSHOT, "read_blob_incomplete")
            ) {
                Streams.readFully(stream);
            }

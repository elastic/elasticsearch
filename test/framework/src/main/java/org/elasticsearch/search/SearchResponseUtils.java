...
public enum SearchResponseUtils {
    ...
    private static CrossProjectSearchMetrics parseCpsMetrics(XContentParser parser) throws IOException {
        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                } else if (CrossProjectSearchMetrics.SEARCH_PHASES_FIELD.equals(currentFieldName)) {
                    parseCpsSearchPhases(parser, metrics);
                } else {
                    parser.skipChildren();
                }
            } else {
    ...
    /** Parses the {@code search_phases} object, e.g. {@code {"query": 230, "fetch": 18}}. */
    private static void parseCpsSearchPhases(XContentParser parser, CrossProjectSearchMetrics metrics) throws IOException {
        XContentParser.Token token;

...
import static org.elasticsearch.test.ESIntegTestCase.randomRepoPath;
...
public class SnapshotStressTestsHelper {
     ...
     */
    public static class TransferableReleasables implements Releasable {
    ...
    public static Releasable tryAcquirePermit(Semaphore permits) {
    ...
    public static Releasable tryAcquireAllPermits(Semaphore permits) {
    ...
    public static AbstractRunnable mustSucceed(CheckedRunnable<Exception> runnable) {
    ...
    public static <T> ActionListener<T> mustSucceed(CheckedConsumer<T, Exception> consumer) {
    ...
    public static class TrackedCluster {
        ...
        public TrackedCluster(InternalTestCluster cluster, Set<String> masterNodeNames, Set<String> dataNodeNames) {
        ...
        private void startRestorer() {
            enqueueAction(() -> {
                ...
                try (
                    SnapshotStressTestsHelper.TransferableReleasables localReleasables =
                        new SnapshotStressTestsHelper.TransferableReleasables()
                ) {
            ...
            try (
                ...
                closeIndicesStep.addListener(mustSucceed(ignored1 -> deleteIndicesStep.addListener(mustSucceed(ignored2 -> {
                    ...
                    final RestoreSnapshotRequestBuilder restoreSnapshotRequestBuilder = client().admin()
                        .cluster()
                        .prepareRestoreSnapshot(TEST_REQUEST_TIMEOUT, snapshotInfo.repository(), snapshotInfo.snapshotId().getName());
        ...
        private TrackedRepository blockSnapshotsFromOneRepository(
            SnapshotStressTestsHelper.TransferableReleasables localReleasables,
        ...
        private void startSnapshotter() {
            enqueueAction(() -> {
                ...
                try (
                    ...
                    final SnapshotStressTestsHelper.TransferableReleasables releasableAfterStart =
                        new SnapshotStressTestsHelper.TransferableReleasables();
                    ...
                    ensureYellowStep.addListener(mustSucceed(clusterHealthResponse -> {
                        ...
                        final CreateSnapshotRequestBuilder createSnapshotRequestBuilder = client().admin()
                            .cluster()
                            .prepareCreateSnapshot(TEST_REQUEST_TIMEOUT, trackedRepository.repositoryName, snapshotName);
        ...
        private void startPartialSnapshotter() {
            enqueueAction(() -> {
                ...
                try (
                    ...
                    if (abortSnapshot) {
                        try (
                            SnapshotStressTestsHelper.TransferableReleasables abortReleasables =
                                new SnapshotStressTestsHelper.TransferableReleasables()
                        ) {
            ...
            private void scheduleRemoveAndAdd() {
                enqueueAction(() -> {
                    ...
                    try (
                        ...
                        client().admin()
                            .cluster()
                            .prepareDeleteRepository(TEST_REQUEST_TIMEOUT, TEST_REQUEST_TIMEOUT, repositoryName)
                            .execute(mustSucceed(acknowledgedResponse -> {
                                assertTrue(acknowledgedResponse.isAcknowledged());
        ...
        private class TrackedIndex {
            ...
            public static boolean closingPermitAvailable(int value) {
                return value >= 0 && value != Integer.MAX_VALUE;
            }

            public static boolean partialSnapshottingPermitAvailable(int value) {
    ...
    public static ClusterHealthRequestBuilder prepareClusterHealthRequest(String... targetIndexNames) {
        return client().admin()
            .cluster()
            .prepareHealth(TEST_REQUEST_TIMEOUT, targetIndexNames)
     ...
     */
    public static class ReleasableClient implements Releasable {
     ...
     */
    public static class TrackedSnapshot {
        ...
        Releasable tryAcquirePermit() {
            try (
                ...
                if (localReleasables.add(SnapshotStressTestsHelper.tryAcquirePermit(trackedRepository.permits)) == null) {
                    return null;
                }

                if (localReleasables.add(SnapshotStressTestsHelper.tryAcquirePermit(permits)) == null) {
                    return null;
                }
        ...
        Releasable tryAcquireAllPermits() {
            try (
                ...
                if (localReleasables.add(SnapshotStressTestsHelper.tryAcquireAllPermits(permits)) == null) {
                    return null;
                }

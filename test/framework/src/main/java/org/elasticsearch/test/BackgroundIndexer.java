...
public final class BackgroundIndexer implements AutoCloseable {
    ...
    private volatile Function<Long, String> documentIdGenerator;
    ...
    public BackgroundIndexer(
        ...
        this(
            ...
            writerCount,
            autoStart,
            random != null ? random : RandomizedTest.getRandom(),
            (random != null ? random : RandomizedTest.getRandom()).nextBoolean()
        );
    }
     ...
     * @param autoStart          set to true to start indexing as soon as all threads have been created.
     * @param random             random instance to use
     * @param useAutoGeneratedIDs whether to let Elasticsearch auto-generate document IDs
     */
    public BackgroundIndexer(
        final String index,
        final Client client,
                    ...
                    try {
                        ...
                        while (stop.get() == false) {
                            if (batch) {
                                ...
                                final Function<Long, String> docIdGen = documentIdGenerator != null
                                    ? documentIdGenerator
                                    : (useAutoGeneratedIDs ? null : String::valueOf);
                                for (int i = 0; i < batchSize; i++) {
                                    id = idGenerator.incrementAndGet();
                                    if (docIdGen != null) {
                                        bulkRequest.add(
                                            client.prepareIndex(index).setId(docIdGen.apply(id)).setSource(generateSource(id, threadRandom))
                                        );
                                    } else {
                                ...
                                final Function<Long, String> docIdGen = documentIdGenerator != null
                                    ...
                                    : (useAutoGeneratedIDs ? null : String::valueOf);
                                try {
                                    final var req = client.prepareIndex(index)
                                        .setTimeout(timeout)
                                        .setSource(generateSource(id, threadRandom));
                                    if (docIdGen != null) {
                                        req.setId(docIdGen.apply(id));
                                    }
                                    DocWriteResponse indexResponse = req.get();
    ...
    /// Sets a function that maps each document's sequential id counter to the `_id` string used for indexing.
    /// When set, overrides the `Long.toString(id)` path, giving the caller full control over shard routing.
    /// Must be called before [#start(int)].
    public void setDocumentIdGenerator(Function<Long, String> documentIdGenerator) {
        assert useAutoGeneratedIDs == false : "cannot set documentIdGenerator if useAutoGeneratedIDs is true";

...
public abstract class AbstractSimpleTransportTestCase extends ESTestCase {
    ...
    public void testTimeoutPerConnection() throws IOException {
        ...
        try (ServerSocket socket = new ServerSocket()) {
            ...
            DiscoveryNode first = new DiscoveryNode("TEST", new InetSocketTransportAddress(socket.getInetAddress(),
                socket.getLocalPort()), emptyMap(),
                emptySet(), version0);
            DiscoveryNode second = new DiscoveryNode("TEST", new InetSocketTransportAddress(socket.getInetAddress(),
                socket.getLocalPort()), emptyMap(),
                emptySet(), version0);
    ...
    public void testTcpHandshakeTimeout() throws IOException {
        assumeTrue("only tcp transport does a handshake", serviceA.getOriginalTransport() instanceof TcpTransport);
        try (ServerSocket socket = new ServerSocket()) {
            socket.bind(new InetSocketAddress(InetAddress.getLocalHost(), 0), 1);
            socket.setReuseAddress(true);
            DiscoveryNode dummy = new DiscoveryNode("TEST", new InetSocketTransportAddress(socket.getInetAddress(),
                socket.getLocalPort()), emptyMap(),
                emptySet(), version0);
            ConnectionProfile.Builder builder = new ConnectionProfile.Builder();
            builder.addConnections(1,

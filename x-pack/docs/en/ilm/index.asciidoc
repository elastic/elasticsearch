...
For time series indices, there are four phases in the index lifecycle:

image::images/lifecycle-phases.png[Index lifecycle phases]
...
An {ilm} policy can perform the following actions on an index as it moves through
each phase:

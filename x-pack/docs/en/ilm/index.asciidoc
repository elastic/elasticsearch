...
You control how time series indices are handled as they age by attaching a
...
1. When the index reaches 5GB, roll over to a new index.
2. Move the old index into the warm stage, mark it read only, and shrink it down
to a single shard.
4. After 7 days, move the index into the cold stage and move it to less expensive
hardware.
5. Delete the index once the required 30 day retention period is reached.
--

...
|STOPPING |This represents a state where only sensitive actions (like the Shrink Action) will be executed
until they finish, at which point the operation mode will move to `STOPPED`
...
After stopping, all further policy actions will be halted. This will
...
You may notice that the response shows `STOPPING` as the operating mode.
This is because the API will only send a request to stop to the ILM service.
The ILM service will then, asynchronously, run through and verify that it is
safe to stop. After ILM verifies that it is safe, it will move to the `STOPPED`
mode.
...
To start ILM back up and continue executing policies, use the <<ilm-start, Start API>>.

...
|max_age |The maximum age of the index. Defaults tonull. Optional.
|===
...
When the rollover is performed the newly created index will be set as the 
"write index" for the rolled over alias. This new index will then be used to 
index documents sent to the alias allowing indexing to be uninterrupted.
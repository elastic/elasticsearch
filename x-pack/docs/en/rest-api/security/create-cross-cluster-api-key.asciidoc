...
Creates an API key of the `cross_cluster` type for API key based remote cluster access.
A `cross_cluster` API key cannot be used to authenticate via the REST interface.
On the contrary, a <<security-api-create-api-key,`rest` API key>> is meant to be used via the REST interface
and cannot be used for API key based remote cluster access.
...
IMPORTANT: The request must be made with a credential that is *not* itself an API key.
If the credential that is used to authenticate this request is an API key,
the API returns an error even if the API key has sufficient privilege.
...
`names`:::: (required, list) A list of indices or index patterns to which the
...
<4> The role descriptors associated to the cross-cluster API key. It always
...
Or both if both search and replication are required.

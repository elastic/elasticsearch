...
=== Create or update users using python 
...
def createUser():
    if request.method == 'POST':
        body ={"username":username, "email":email, "fullname":fullname, "password":password, "roles":roles}
        es = Elasticsearch(hosts=https://localhost:9200/, basic_auth=(ELASTIC_USERNAME, ELASTIC_PASSWORD), ca_certs=CERTIFICATE, verify_certs=False)
        client = SecurityClient(es)
        client.put_user(**body)

[[security-api-client-enrollment]]
=== Enroll Client API
++++
...
[[security-api-client-enrollment-request]]
==== {api-request-title}

`POST /_security/enroll_client`

[[security-api-client-enrollment-prereqs]]
==== {api-prereq-title}


* You must have the `enroll` <<privileges-list-cluster,cluster privilege>> to use this API.

[[security-api-client-enrollment-desc]]
...
[[security-api-client-enrollment-request-body]]
...
--------------------------------------------------
<1> The CA certificate that is used to sign the certificates that {es}

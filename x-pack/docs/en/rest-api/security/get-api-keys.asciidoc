...
does not allow retrieving the authenticated user's own keys using `owner=true`.
Instead, specify a key ID in the `id` field, or authenticate the user with basic
credentials.

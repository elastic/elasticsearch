...
different nodes are considered to be the same and only counted once
...
<3> File-backed tokens collected from all nodes
<4> List of nodes that (file-backed) `my-token` is found

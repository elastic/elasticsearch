...
Creates a SAML authentication request as a URL string, based on the
configuration of the respective SAML realm in {es}. 
...
This API returns a URL that contains a URL pointing to the SAML Identity
Provider. You can use the URL to redirect the browser of the user in order to
continue the authentication process. The URL contains a single parameter named `SAMLRequest`, which contains a SAML Authentication request that is deflated and
...
`acs`::
  ...
  Either this or `realm` should be specified.

`realm`::
  (string) The name of the SAML realm in {es} for which the configuration is
  used to generate the authentication request. Either this or `acs` should be
  specified.
...
`id`::
  (string) An identifier for the SAML Request to be stored by the caller
  of the API.

...
This behavior can occur if the owner user's permissions have changed since the creation or last update of the API key.
====
...
`role_descriptors`::
(Optional, array) An array of role descriptors to assign to this API key.
The API key's effective permissions are an intersection of its assigned privileges and the owner user's permissions at the time the request is issued.
...
If you omit this parameter, the API key keeps its previously assigned privileges.
You can remove all previously assigned privileges by supplying an empty `role_descriptors` parameter.
The snapshot of the owner's permissions is always updated, regardless of whether you specify this parameter.
The structure of the role descriptor is the same as the request for the <<security-api-put-role, create or update roles API>>.
...
The following example updates the API key, replacing the API key's assigned permissions with the <<security-api-update-api-key-examples-user-permissions, owner user's>>:
...
For the next example, assume that the owner user's permissions changed from <<security-api-update-api-key-examples-user-permissions, the original permissions>> to:

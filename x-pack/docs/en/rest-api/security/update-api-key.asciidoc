...
Assuming the user's permissions have not changed since the creation or last update of the API key, this request does _not_ result in a change, and the API key's effective permissions remain:

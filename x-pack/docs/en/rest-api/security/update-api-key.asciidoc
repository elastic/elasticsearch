...
This is because the API key's access scope is <<security-api-update-api-key-api-key-role-descriptors,derived>> from the authenticated user's current permissions at the time of the request.

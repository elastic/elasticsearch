...
Other properties such as `name` or `expiration` cannot be updated.
...
Any call to <<security-api-update-api-key,update API Key>>, **regardless** of whether parameters are supplied or not, will automatically apply the authenticated user's current permissions to the API key.
This may result in changes in the API key's access scope, even if you don't specify new <<security-api-update-api-key-api-key-role-descriptors,`role_descriptors`>> in the update request.
...
------------------------------------------------------------
// TEST[skip:tests in subsequent PR]

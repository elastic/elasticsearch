...
Any call to the update API, **regardless** of whether any parameters are supplied or not, will automatically update the _point in time snapshot of permissions of the authenticated user_ associated with the API key.
...
<1> New role descriptors for the API key to replace existing ones.
<2> New metadata for the API key.

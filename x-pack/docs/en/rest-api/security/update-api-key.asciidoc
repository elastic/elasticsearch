...
For the next example, assume that the authenticated user's permissions changed from <<security-api-update-api-key-examples-user-permissions, authenticated user's permissions>> to:
...
The following request re-applies the user's updated permission to the API key:

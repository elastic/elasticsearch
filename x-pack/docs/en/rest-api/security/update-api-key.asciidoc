...
<<security-api-update-api-key-api-key-role-descriptors,`role_descriptors`>> in the request. This behavior
can occur because the API key's access scope is derived and updated based on a snapshot of the owner
user's permissions at the time of the request.
====

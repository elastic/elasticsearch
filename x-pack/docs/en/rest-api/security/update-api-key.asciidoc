...
`role_descriptors`::
(Optional, array) An array of new role descriptors for this API key.
The API key's permissions are an intersection of its assigned privileges and the owner user's permissions at the time the request is issued.
If no privileges are assigned, the API key has the owner user's full current permissions.
...
You can remove all previously assigned privileges by supplying an empty parameter.
The snapshot of the owner's permissions is always updated, regardless of whether you specify this parameter or not.

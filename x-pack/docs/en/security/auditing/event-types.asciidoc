...
A new <<audit-log-output, `logfile` audit output>> format has been added in the
6.5.0 version. This format also brough in a few changes for the audit event attributes.

The new format is output to the `<clustername>_audit.log` file.
The audit entries are formatted as flat (that is, no nested objects) JSON documents,
one per line. Hence, the attribute names are the JSON keys and they follow a dotted
name syntax. Any attribute that lacks a value (`null`) will not be present.
...
independetly during the 6.x releases, so it is advisable to follow the attribute
descriptions for the format that you are using.

`@timestamp`      ::    The time, in ISO9601 format, when the event occurred.
`node.name`       ::    The name of the node. This name can be changed
                        from the `elasticsearch.yml` config file.
`node.id`         ::    The automatically assigned node's id. This is persistent
                        across full cluster restarts.
...
If the `event.type` is equal to `rest`, the following attributes are also present:
...
`authentication_success`, `anonymous_access_denied`, `authentication_failed`,
`realm_authentication_failed`, `tampered_request`, `run_as_denied`. Some of these
have yet more attributes:

* `authentication_success`:

  `realm`               ::   The name of the realm that had successfully
                             authenticated the user.
  `user.name`           ::   The _effective_ username. Usually this is the _authenticated_
                             username, but if using the <<run-as-privilege, run as authorization functionality>>
                             this denotes the _impersonated_ username.
  `user.run_by.name`    ::   This attribute is present only if the request is
                             ...
                             and denotes the _authenticated_ (_impersonator_) username.

* `authentication_failed`:

  `user.name`          ::    The name of the user that failed authentication. Could
                             be missing  if the request authentication token is invalid.
...
* `run_as_denied` or `run_as_granted`:
  ...
  `user.roles`         ::    The user's role names array.
  `user.name`          ::    The _authenticated_ username which is being granted/denied
                             the _impersonation_.
  `user.realm`         ::    The realm name that the _authenticated_ username belongs to.
  `user.run_as.name`   ::    The username as which the _impersonation_ is granted/denied.
  `user.run_as.realm`  ::    The realm name of the _impersonated_ username.

==== Audit event attributes of the transport event type

If the `event.type` is equal to `transport`, the following attributes are also present:

`action`              ::    The name of the transport action that was executed.
                            This is alike the URL for a REST request.
...
`run_as_denied`, `tampered_request`. Some of these have yet more attributes:
...
* `authentication_success`:
  ...
  `user.name`           ::   The _effective_ username. Usually this is the _authenticated_
                             username, but if using the <<run-as-privilege, run as authorization functionality>>
                             this instead denotes the _impersonated_ username.
  `user.run_by.name`    ::   This attribute is present only if the request is
                             using the <<run-as-privilege, run as authorization functionality>>
                             and denotes the _authenticated_ (_impersonator_) username.
...
* `access_granted` and `access_denied`:
  `user.roles`         ::    The user's role names array.
  `user.name`          ::    The _effective_ username that is being authorized/
                             unauthorized. Usually this is the _authenticated_
                             username, but if using the <<run-as-privilege, run as authorization functionality>>
                             this instead denotes the _impersonated_ username.
  `user.realm`         ::    The realm name that the _effective_ username belongs to.
  `user.run_by.name`   ::    This attribute is present only if the request is
                             using the <<run-as-privilege, run as authorization functionality>>
                             and denoted the _authenticated_ (_impersonator_) username.
  `user.run_by.realm`  ::    This attribute is present only if the request is
                             using the <<run-as-privilege, run as authorization functionality>>
                             and denotes the realm that the _authenticated_
                             (_impersonator_) username belongs to.

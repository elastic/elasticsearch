...
`change_apikeys`::
Logged when the <<security-api-bulk-update-api-key,bulk update API keys>> API is
invoked to update the attributes of a batch of existing API keys.
+
You must include the `security_config_change` event type to audit the related
event action.
+
...
{"type":"audit", "timestamp":"2020-12-31T00:33:52,521+0200", "node.id":
"9clhpgjJRR-iKzOw20xBNQ", "event.type":"security_config_change", "event.action":
"change_apikeys", "request.id":"9FteCmovTzWHVI-9Gpa_vQ", "change":{"apikeys":
{"ids":["zcwN3YEBBmnjw-K-hW5_"],"role_descriptors":[{"cluster":
["monitor","manage_ilm"],"indices":[{"names":["index-a*"],"privileges":
["read","maintenance"]},{"names":["in*","alias*"],"privileges":["read"],
"field_security":{"grant":["field1*","@timestamp"],"except":["field11"]}}],
"applications":[],"run_as":[]},{"cluster":["all"],"indices":[{"names":
...
// NOTCONSOLE
+
The object for `change_apikeys` will differ in that it may include
`metadata` and `role_descriptors` (see `apikey`), and will not include `name`,
`owned_by_authenticated_user`, or `user`.

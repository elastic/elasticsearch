...
==== Audit event attributes for the REST event type
...
* `run_as_denied`:
  ...
  `user.roles`         ::    The role names array.
  `user.name`          ::    The _authenticated_ username which is being denied
	                           the _impersonation_.
  `user.realm`         ::    The realm name that the _authenticated_ username belongs to.
  `user.run_as.name`   ::    The username as which the _impersonation_ is denied.
  `user.run_as.realm`  ::    The realm name of the _impersonated_ username.

==== Audit event attributes for the transport event type

...
6.5.0 version. This format also brings in a few changes for audit event attributes.
...
one per line. Hence, the attribute names are JSON keys and they follow a dotted
name syntax. Any attribute that lacks a value (`null`) will not be output.
...
There are a few events that have yet some more attributes in addition to those
...
* `authentication_success`:
  ...
  `user.run_by.name`   ::   This attribute is present only if the request is
                            ...
                            and denotes the name of the  _authenticated_ user, aka
                            the _impersonator_.

* `authentication_failed`:
  `user.name`          ::    The name of the user that failed authentication. Could
                             be missing if the request authentication token is
...
* `run_as_denied` and `run_as_granted`:
  ...
  `user.name`          ::    The name of the _authenticated_ user which is being
                             granted/denied the _impersonation_ action.
...
* `access_granted` or `access_denied`:
  ...
  `user.name`          ::    The name of the _effective_ user that is being
                             authorized/unauthorized. This is usually the _authenticated_
  ...
  `user.run_by.name`   ::    This attribute is present only if the request is
                             using the <<run-as-privilege, run as authorization functionality>>
                             and denoted the name of the _authenticated_ user, aka
                             the _impersonator_.

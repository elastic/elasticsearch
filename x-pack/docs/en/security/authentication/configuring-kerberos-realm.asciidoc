...
service principal. As we are supporting Kerberos on the http network layer,
service principal name will be of format `HTTP/es.domain.local@ES.DOMAIN.LOCAL`.
...
set to `true` then we remove the realm part (`@REALM`). The resulting `username` 

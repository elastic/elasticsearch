...
For more information about LDAP realms, see <<ldap-realm>>.
...
For more information, see <<mapping-roles-ldap>> and <<mapping-roles>>.

NOTE: The LDAP realm supports <<authorization_realms,authorization realms>> as
an alternative to role mapping.
...
For more information, see <<ldap-user-metadata>>. 

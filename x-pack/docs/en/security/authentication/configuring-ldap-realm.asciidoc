...
For {es} authentication, LDAP realm configuration is only required for
<<coordinating-node,coordinating nodes>>. For large clusters, we recommend using
<<coordinating-only-node, coordinating only>> nodes to simplify realm
configuration.

...
100 years, such as 1-2 hours or 1-7 days, not an entire human life. The signature in this
example is deterministic because the header, claims, and HMAC key are fixed.
JWTs would typically have a nonce claim to make the signature non-deterministic.
...
realm users. If a user's roles in the JWT realm mapping contains the `run_as`
privilege, that user can submit requests on behalf of the specified `run_as`
user. To submit requests as another user, include the `es-security-runas-user`
header in your requests. Requests run as if they were issued from that user and
{es} uses their roles.

...
{es} can be configured to trust JSON Web Tokens (JWTs) that are issued from an
external service as an authentication credential.

When a JWT realm is used for authenticating with {es}, a distinction is made
...
should be used to authenticate the client.

A common scenario for using JWTs is when an existing front-end application that
uses OpenID Connect (OIDC) as an authentication method, and then accesses {es}
on behalf of the authenticated user.

TIP: If the front-end application does not exist, you can use the
<<token-authentication-services>> can be used instead.

{es} provides an <<oidc-realm,OpenID Connect realm>> as well as the JWT realm.
The `oidc` realm provides stronger security guarantees and is preferred for any
use case where {es} can act as an OIDC _Relying Party_ (RP).

IMPORTANT: The `oidc` realm is the only supported way to enable OIDC

...
used to authenticate. The available metadata values are visible from the result of
the <<security-api-authenticate-request,authenticate>> endpoint.
Reference these values with a `metadata.` prefix. For example

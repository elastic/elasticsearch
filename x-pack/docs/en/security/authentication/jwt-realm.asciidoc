...
`pkc_jwkset_path`::
The file path to a JSON Web Key (JWK) set containing the public key material that the
JWT realm uses to verify JWT signatures.
...
validating JWTs with respect to their authentication time (`auth_time`), creation (`iat`), not before (`nbf`), and
...
If not set, the value defaults to `sub`.
...
`name`::
(Optional, String) Name of the JWT claim that contains the user's human-readable
identifier of the token's subject. The value
...
`mail`::
(Optional, String) Name of the JWT claim that contains the user's e-mail address
to associate with the user. The value is
...
`dn`::
(Optional, String) Name of the JWT claim that contains the user's the
Distinguished Name (DN) that uniquely identifies a user

...
`allowed_audiences`::
Specifies a list JWT audiences that the realm will allow.
...
WARNING: When Delegated Authorization is enabled in JWT realm, claims for `groups`, `dn`, `mail`, `name`, and `metadata` not used for the {es} user's values.

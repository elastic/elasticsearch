...
The supported JWT encoding is JSON Web Signature (JWS). The JWS header and JWS signature are validated using OpenID Token validation rules.
...
JWT claims `iss`, `aud`, `sub`, `exp`, `iat`, `nbf`, and `auth_time` are validated using OpenID Token validation rules.

...
The access token type is similar to the ID Token type but with more relax rules which makes it
...
In addition, {es} also validates `nbf` and `auth_time` claims for ID Tokens if these claims present.
...
tokens. ID Tokens are well-defined JWT and they should be always compatible with
...
// This needs to be dropped as well since we support application itself
...
// Be careful with OIDC mentionings

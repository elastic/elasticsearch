...
`metadata`::
(Optional, object) Additional metadata about the user derived from the JWT
used to authenticate. These usable values are visible {es} from the result of
the <<security-api-authenticate-request,/_security/_authenticate>> endpoint.
Reference these value with a `metadata.` prefix. For example
`metadata.jwt_token_principal` and `metadata.jwt_claim_<key>` where `key` is a claim
from the JWT.

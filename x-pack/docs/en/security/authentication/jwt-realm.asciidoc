...
`pkc_jwkset_path`::
The file path to a JSON Web Key (JWK) set containing the key material that the
...
----
<1> Path to a JWK Set, which is a resource for a set of JSON-encoded public keys.
+
[NOTE]
====
...
The JWT realm authorization supports the create or update role mappings API and 
...
realm named `jwt2` is authorization delegation to multiple realms:
...
roles. The following example maps roles in the `native1` realm for the
`principalname1` JWT principal.

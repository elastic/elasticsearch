...
a JWT conforming to OIDC ID Token specification.
...
JWT tokens of both types must contain the following 5 pieces of information.
...
ID Tokens are well-defined JWT and should be always compatible with
a JWT realm of the `id_token` token type. ID Tokens usually represent end-users
which can be of a large number. Therefore, a JWT realm of `id_token` token type
does _not_ enforce the `allowed_subjects` validation.
...
NOTE: Not every access token is a JWT. It can in fact be arbitrary in theory.

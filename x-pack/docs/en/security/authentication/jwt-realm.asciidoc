...
You can also apply the <<run-as-privilege-apply,`run_as` privilege>> to JWT
realm users. If a user's roles in the JWT realm mapping contain the `run_as`
privilege, that user can submit requests on behalf of the specified users. Any
requests from users with this privilege contain an `es_run_as` header in
requests, allowing the request to run as a different user from another realm and
use that user's roles.

For example, the following request creates a user role named `jwt_role1`, which
specifies a `run_as` user named `user123_runas`. Any user with this role can
issue requests as the specified user.
...
maps the `jwt_role1` role to a user with the username `user2` in the `jwt2`
realm. This means that {es} will use the `jwt2` realm to authenticate the user
named `user2`. Because `user2` has a role that includes the `run_as` privilege,
{es} retrieves the role mappings for the `user123_runas` user and uses the roles
for that user to submit requests.
...
----
<1> In {ess}, the realm order starts at `2`. `0` and `1` are reserved in the
realm chain.
...
following secure settings to the {es} keystore. In {ess}, you define settings

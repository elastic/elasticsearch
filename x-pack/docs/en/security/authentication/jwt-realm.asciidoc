...
example HMAC key is in an OIDC HMAC compatible format. The key bytes are in 
...
===== JWT issuer
The following values are for a custom authentication service that issues JWTs
which are external to {es}.

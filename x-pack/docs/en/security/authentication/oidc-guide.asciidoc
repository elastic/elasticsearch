...
op.token_endpoint::
    ...
    {es} will send a request to exchange the code for an ID Token. Setting this is optional when
    using the Implicit Flow. The value for this setting should be provided by your OpenID Connect Provider.
...
In case you use the authorization code flow, {kib} needs to support unsolicited messages that originate from your
OP or a third party (see <<third-party-login>>). In order to do so, we need to explicitly _whitelist_ the

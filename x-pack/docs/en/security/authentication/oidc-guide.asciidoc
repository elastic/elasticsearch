...
claims.principal:: See <<oidc-mapping-claims>>.
claims.groups:: See <<oidc-mapping-claims>>.
...
+
NOTE: You can only map claims with values that are strings, numbers, boolean
values or an array of the aforementioned.

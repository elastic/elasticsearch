...
_jwt_::
A realm that facilitates authentication using JWTs as authentication bearer tokens.
It enables {es} to use OpenID Connect ID Tokens or bespoke JWTs as bearer tokens. See <<jwt-guide>>.

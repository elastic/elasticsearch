...
[NOTE]
====
Some IdPs are only able to send the `groups` list as a comma-separated string.
{es} can't parse this string into an array of groups. However, you can use a
wildcard when <<saml-role-mapping,configuring role mappings>> to map this SAML
attribute to the `attributes.groups` setting in the {es} realm.
====
...
<<saml-es-user-properties,{es} user properties>> for more information.

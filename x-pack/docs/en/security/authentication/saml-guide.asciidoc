...
An SAML realm needs to be created and configured accordingly
...
that gives them the `manage_saml` cluster privilege. The use of the `manage_token`
...
the `saml-service-user` user, using either the name of the SAML realm in the {es} configuration or the value for
...
persisted this way) and redirect the user's browser to the URL that will be contained in the
  `redirect` value. The `id` value should not be disregarded as it is used as a nonce in SAML in
...
defined as a URL which the custom we application handles. Upon receiving this HTTP POST request, the
custom web application will need to parse that and make an HTTP POST request itself to the
`_security/saml/authenticate` API, again - authenticating as the `saml-service-user` user - passing
the Base64 encoded SAML Response that was sent as the body of the request as well as the value for `id` that it had saved in the user's session previously.
+
...
. The response to calling `/_security/saml/authenticate` will only contain the username of the authenticated
user. If you need to get the values for the SAML Attributes for that user that were contained in the SAML
Response, you can call the Authenticate API `/_security/_authenticate/` using the access token as a `Bearer` token
...
. At some point, if necessary, the custom web application can log the user out by using the
  {ref}/security-api-saml-logout.html[SAML Logout API] passing the access token and refresh token as parameters. For example:
+
[source,js]
--------------------------------------------------
...
// TEST[skip:handled in IT]
+
If the SAML realm is configured accordingly and the IdP supports it, this request will trigger a SAML
...
parameter with a URL in the IdP that contains the SAML Logout response and should redirect the user

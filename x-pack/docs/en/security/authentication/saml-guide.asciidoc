...
You can generate the SAML metadata issuing the API request to Elasticsearch and storing metadata as an XML file using tools like `jq`.

Following command, for example, will generate the metadata for the SAML realm `realm1` and save it to `metadata.xml` file:

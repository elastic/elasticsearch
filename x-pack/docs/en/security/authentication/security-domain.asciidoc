...
Users whose realms are no longer in the same domain as the API key's owner user, will not be able to update the API key or perform queries on it that rely on ownership.

...
metadata. For certain resources, such as <<security-api-update-api-key,API keys>>,
{es} re-captures the user's current realm information whenever the user updates the resource.

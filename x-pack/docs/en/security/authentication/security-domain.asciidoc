...
Note that the original creator of an API key will lose ownership, if the creator's realms are no longer in the same domain as those of the current API key owner.

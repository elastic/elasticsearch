...
metadata. For certain resources that allow updates, such as <<security-api-update-api-key,API keys>>, {es} re-captures the user's
username and current realm information on every update.

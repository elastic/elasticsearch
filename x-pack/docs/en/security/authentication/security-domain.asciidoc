...
Users whose realms are no longer in the same domain as the API key's current owner won't be able to update the API key. Likewise, they won't be able to perform queries on the API key that rely on ownership (for example, calls to the <<security-api-get-api-key,get API key API>> with the `owner` parameter set to `true`).

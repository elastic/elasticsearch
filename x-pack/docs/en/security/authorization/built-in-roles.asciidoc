...
write access to `.ml-annotations*` indices.
+
--
NOTE: {ml-cap} administrators also need index privileges for source and
destination indices and roles that grant
...
+
--
NOTE: {ml-cap} users also need index privileges for source and destination
indices and roles that grant
{kibana-ref}/xpack-security-authorization.html[access to {kib}].

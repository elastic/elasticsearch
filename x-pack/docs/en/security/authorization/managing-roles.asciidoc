...
NOTE: Remote indices privileges have no effect for remote clusters that are configured
with the certificate based model.

The remote indices privileges entry has an extra mandatory `clusters` field compared to
an <<roles-indices-priv,indices privileges entry>>. But otherwise the two have identical structure.

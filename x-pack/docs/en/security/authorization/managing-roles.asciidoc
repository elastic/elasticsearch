...
<5> Restricted indices are a special category of indices that are used
    ...
    roles should normally grant privileges over the restricted indices. If
    however, for administrative purposes, you need to create a role with
    privileges covering restricted indices, you must set this field to `true`
    (default  is `false`), and then the `names` field will cover the restricted
    indices as well.

...
<2> The list of application names that may be managed. This list supports
    ...
    `"/app[0-9]*/"`)
<3> The privilege for the ability to update the `access` and `data` of any user profile
<4> The list of names and wildcards to which the update privilege is restricted to

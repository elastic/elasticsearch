...
beta:[]
Privileges to connect to remote clusters configured with the API key based model
...
`cross_cluster_replication`::
beta:[]
Privileges to perform cross-cluster replication for indices located on remote clusters
...
beta:[]
Privileges to perform supporting actions for cross-cluster replication from remote clusters

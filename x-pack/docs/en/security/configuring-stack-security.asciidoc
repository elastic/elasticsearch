...
<<security-minimal-setup,Set up basic security for the Elastic Stack>>
...
{es} and {kib}. TLS on the transport interface is mutual in this
...
You then configure {kib}, {ls}, and Beats to communicate with
...
and securing HTTP traffic, you'll configure connections from {kib}, {ls},
...
Additionally, you'll configure connections from Beats to {ls} over TLS

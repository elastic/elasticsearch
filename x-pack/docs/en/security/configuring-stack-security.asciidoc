...
cluster by setting up passwords for the built-in users.
...
You also configure password authentication for {kib}. Your external HTTP
traffic between {es} and {kib} won't be encrypted, but internode communication
will be secured.

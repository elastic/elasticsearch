...
For cache hashing, salted `sha256` is recommended instead of PBKDF2. Though compliant with FIPS-140-2,
PBKDF2 is, by design, slow and thus not suitable as a cache hashing algorithm. Since the cache
does not store hashed credentials on disk, salted `sha256` provides an adequate security level without
imposing prohibitive performance overhead.
...
of PBKDF2 could undo all performance gain of using a cache.

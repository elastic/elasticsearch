...
For cache hashing, salted `sha256` is recommended instead of `PBKDF2`. Though
compliant with FIPS-140-2, `PBKDF2` is -- by design -- slow, and thus not
generally suitable as a cache hashing algorithm. Cached credentials are never
stored on disk, and salted `sha256` provides an adequate level of security for
in-memory credential hashing, without imposing prohibitive performance overhead.
You _may_ use `PBKDF2`, however you should carefully assess performance impact
first. Depending on your deployment, the overhead of `PBKDF2` could undo most of
the performance gain of using a cache.

You should set the `cache.hash_algo` realm settings to `ssha256`. `ssha256` is
the default value for all `cache.hash_algo` realm settings. See
<<hashing-settings>>.
...
using the algorithm you have selected.

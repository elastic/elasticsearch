...
An {es} cluster is typically comprised of many moving parts. There are the {es}
...
secure {es} clusters and any clients that communicate with it. You can password
protect your data as well as implement more advanced security measures such as
encrypting communications, role-based access control, IP filtering, and
auditing.
...
_authenticate_ users and validate that a user is who they claim to be. For
...
control what data users can access and what tasks they can perform. By enabling the
{es-security-features}, you can _authorize_ users by assigning access
...
data by encrypting communications to, from, and within the cluster. For even	greater protection, you can increase the

...
secure {es} clusters and any clients that communicate with your clusters. 
Password protection and Transport Layer Security (TLS) is configured and enabled
by default when you start {es}, providing confidentiality and integrity
protection to your communications with the Elastic Stack. You can also implement
additional security measures, such as role-based access control, IP filtering,
and auditing.

The {security-features} protect {es} clusters by:

...
While document-level security prevents users from seeing documents that they do not have access to,
it is possible to craft search requests in order to discover aggregate information about the entire
index. A user whose access to an index is restricted via document-level security could still learn
about fields names and terms that only exist in inaccessible documents and count how many inaccessible
documents containing a given term. But no more than that.

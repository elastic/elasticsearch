...
the username, authentication realm type, and (for realms other than file, native or kerberos) realm name.
...
recommended way to configure realms, therefore resource sharing check
does not attempt to detect this inconsistency.
* Realms can be renamed. This can cause inconsistency for resource sharing check
...
Hence resource sharing check does not account for this potential discrepancy.

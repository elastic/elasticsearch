...
the username, authentication realm name and realm type. This logic has a few limitations:

* While realms must be unique on each node, it is not the case across different nodes.
Two different realms can in theory have the same name on different nodes. We do not
...
* The username is computed on the fly based on configurations for some external authentication
providers. It is in theory possible that two distinct users could end up having the
same username. Our recommendation is to avoid this situation in the first place.

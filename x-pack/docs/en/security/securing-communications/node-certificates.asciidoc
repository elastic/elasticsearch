...
+
NOTE: The {ref}/settings.html#config-files-location[{es} configuration
directory] varies depending on your {es} installation.

.. Copy the node certificates into the `certs` directory that you created in the
previous step.

.. Copy the `.p12` keystore into the {es} configuration directory. If this file
does not exist in the location that {es} expects, permission checks will fail
when {es} starts.

...
+
--
Copy the applicable `.p12` file into a directory within the {ref}/settings.html#config-files-location[{es} configuration
directory] on each node, otherwise permissions checks will fail at
the start up. For example, `/home/es/config/certs`. There is no need

...
certificate. Stop one node at a time, replace the node certificate, and then
restart the node.
...
. Stop {es} on the first node in your cluster where you are updating the
keystore.
...
. Restart the node where you updated the keystore. 

. (Optional) Use the <<security-api-ssl,SSL certificate API>> to verify that
{es} loaded the new keystore.

. On each additional node in your cluster, stop {es}, update the keystore,
and restart {es}.

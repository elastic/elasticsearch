...
and your nodes will detect the changes. 
...
If your CA hasn't changed, you can just generate a new node certificate, save it
with the same name as your existing certificate, and replace the existing file.
You'll need to generate new node certificates and keys for both the transport
...
+
    ...
    keystore currently in use. If your 
...
. If you entered a password when creating the node certificate, run the
...
. On every node in your cluster, replace your existing keystore with the new
keystore, ensuring that the file names match. For example,
`elastic-stack-ca.p12`.
+
IMPORTANT: If your keystore is password protected, the password
is stored in the {es} keystore, _and_ the password needs to change, then you
...
. After replacing the keystore on every node in your cluster, restart each node
one at a time.
...
++++
If your CA needs to change, you'll need to generate a new CA certificate and key.
...
. On every node in your cluster, replace your existing keystore with the new
keystore, ensuring that the file names match. For example, `http.p12`.
+
IMPORTANT: If your keystore is password protected, the password
is stored in the {es} keystore, _and_ the password needs to change, then you
must update the `ES_PATH_CONF/elasticsearch.yml` file to use the filename of the

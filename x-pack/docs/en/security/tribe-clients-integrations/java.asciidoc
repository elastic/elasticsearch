...
. <<setup-xpack-client,Configure the {xpack} transport client>>.
...
NOTE: Client authentication is enabled by default. For information about
      disabling client authentication, see <<disabling-client-auth>>.
...
transport protocol. For more information see

...
Identity Provider sent. In such a case, {es} is configured as such
....
xpack.security.authc.realms.saml.<saml-realm-name>.attributes.principal: AttributeName0
....
and this means that {es} expects to find a SAML Attribute with name `AttributeName0` or a `NameID` with appropriate format in the SAML
...
If you are attempting to map a SAML Attribute and it is not part of the list that is printed above, it might mean
...
HTTP-Redirect (urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect). {es} only supports the `HTTP-Redirect` binding for SAML Authentication

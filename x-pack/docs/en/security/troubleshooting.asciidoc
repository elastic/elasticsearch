...
or service login failure on server side or {es} http client. Some of
the common exceptions are listed below with some tips to resolve +
...
When this occurs on http client side, then it may be related to provided
password and if it is wrong.

Keytab is present but it failed to log in as user. Check keytab expiry or
if it does not contain up to date credentials, if so replace it.

Use tools like `klist` or `ktab` to list principals inside the keytab and
validate them.

Kerberos depends on proper hostname resolution so you can check your DNS
setup for missing configurations like incorrect DNS SRV records or incorrect
configuration for KDC servers in `krb5.conf`.
...
synchronization and usually is set to 5 minutes. Check whether time on machines
within the domain are in sync.

--

For further debugging, enable JAAS Kerberos login module debugging logs on
{es} using following settings:
[source,yaml]
----------------
xpack.security.authc.realms.<realm-name>.krb.debug: true
----------------

For detailed information of realm settings,
see {ref}/security-settings.html#ref-kerberos-settings[Kerberos realm settings].

To enable Kerberos logging on JVM you will need to add following JVM system
properties:
...
To configure JVM system properties see {ref}/jvm-options.html[configuring jvm options].

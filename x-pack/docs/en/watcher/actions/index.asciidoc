...
<2> An optional <<conditions,condition>> to restrict action execution
<3> An optional <<transforms,transform>> to transform the payload and prepare the data that should be indexed
...
Like with all other actions, you can use a <<transforms, transform>> to replace

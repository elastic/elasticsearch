...
<2> An optional <<watcher-conditions,condition>> to restrict action execution
<3> An optional <<watcher-transforms,transform>> to transform the payload and prepare the data that should be indexed
...
Like with all other actions, you can use a <<watcher-transforms, transform>> to replace

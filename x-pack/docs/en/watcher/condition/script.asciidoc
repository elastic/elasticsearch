[role="xpack"]
[[watcher-condition-script]]
=== {watcher} script condition
++++
<titleabbrev>Script condition</titleabbrev>
++++

A watch <<watcher-conditions,condition>> that evaluates a script. The default scripting
...
<<watcher-input-search,`search` input>> and uses a `script` condition to check if the

...
A watch <<watcher-trigger-schedule,schedule>> controls how often a watch is triggered.
The watch <<watcher-inputs,input>> gets the data that you want to evaluate.
...
<<watcher-input-http,HTTP input>> that calls the cluster health API:
...
A <<watcher-conditions,condition>> evaluates the data you've loaded into the watch and
...
<<watcher-actions,actions>> define what to do when the watch condition is true--you

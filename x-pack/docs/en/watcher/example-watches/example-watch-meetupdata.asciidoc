...
-------------------------------------------------
// NOTCONSOLE
<1> These index pattens use <<date-math,Date Math>> expressions to match {ls}
indices that contain the meetup data. The second pattern is needed in case the
previous hour crosses days.

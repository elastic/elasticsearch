...
watch, you could use an <<watcher-schedule-interval,interval>> schedule and a
<<watcher-input-search,search>> input. For example, the following Watch searches
...
<1> The <<watcher-condition-compare,compare>> condition lets you easily compare against
...
watch's <<watcher-actions,actions>> define what to do when the watch condition

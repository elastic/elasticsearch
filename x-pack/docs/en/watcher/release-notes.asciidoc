...
.New Features
* Added <<watcher-action-pagerduty,PagerDuty action>>
...
.New Features
* Adds support for <<watcher-inputs-chain,chaining several inputs>>
...
.New Features
* Added new <<watcher-actions-hipchat,HipChat Action>>
* Added new <<watcher-action-slack,Slack Action>>
...
* Added new <<watcher-condition-array-compare,array_compare>>, that can compare an array
...
.New Features
* <<watcher-actions-ack-throttle,Acking and throttling>> are now applied at the action
...
.Enhancements
* The <<watcher-condition-compare,compare condition>> result now includes the value of
...
* Added support for headers in the <<watcher-action-webhook,webhook action>> result
  and the <<watcher-input-http,HTTP input>> result.
* Add plain text response body support for the <<watcher-input-http,HTTP input>>.

.Bug Fixes
* Disallow negative time value settings for <<watcher-actions-ack-throttle,`throttle_period`>>
* Added support for separate keystore and truststore in <<watcher-action-webhook,webhook action>>
  and <<watcher-input-http,HTTP input>>.

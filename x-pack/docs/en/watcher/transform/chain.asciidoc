[role="xpack"]
[[watcher-transform-chain]]
=== {watcher} chain {watcher-transform}
++++
<titleabbrev>Chain {watcher-transform}</titleabbrev>
++++

A <<watcher-transforms,{watcher-transform}>> that executes an ordered list of configured
...
<<watcher-transform-search,`search`>> {watcher-transform} and a
<<watcher-transform-script,`script`>> {watcher-transform}, as shown in the following snippet:

[role="xpack"]
[[watcher-transform-script]]
=== {watcher} script {watcher-transform}
++++
<titleabbrev>Script {watcher-transform}</titleabbrev>
++++

A <<watcher-transforms,{watcher-transform}>> that executes a script on the current
...
TIP:  The `script` {watcher-transform} is often useful when used in combination
      with the <<watcher-transform-search,`search`>> {watcher-transform}, where the
      ...
      <<watcher-transform-chain,`chain`>> {watcher-transform}.

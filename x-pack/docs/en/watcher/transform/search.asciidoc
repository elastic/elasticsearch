...
executes a search over all events indices, matching events with `error` priority:
...
| `request.search_type`                         | no        | query_then_fetch  | The search <<search-request-search-type,type>>.

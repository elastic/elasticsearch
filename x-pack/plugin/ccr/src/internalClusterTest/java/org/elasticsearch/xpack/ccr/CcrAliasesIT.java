...
public class CcrAliasesIT extends CcrIntegTestCase {
    ...
    private CheckedRunnable<Exception> assertShardFollowTask(final int numberOfPrimaryShards) {
        return () -> {
            ...
            final PersistentTasksExtensionMetadata taskMetadata = clusterState.getMetadata().custom(PersistentTasksExtensionMetadata.TYPE);
            ...
            final Collection<PersistentTasksExtensionMetadata.PersistentTask<?>> shardFollowTasks = taskMetadata.findTasks(
                ShardFollowTask.NAME,
                Objects::nonNull
            );
            for (final PersistentTasksExtensionMetadata.PersistentTask<?> shardFollowTask : shardFollowTasks) {

...
public class ShardChangesIT extends ESIntegTestCase {
    ...
    public void testFollowIndex() throws Exception {
        ...
        final int numDocsToDelete = randomIntBetween(2, 64);
        for (int i = 0; i < numDocsToDelete; i++) {
            client().prepareDelete("index1", "doc", Integer.toString(i)).get();
        }

        for (int i = 0; i < numDocsToDelete; i++) {
            assertDocumentDeletedRunnable(i);
        }
    ...
    private CheckedRunnable<Exception> assertDocumentDeletedRunnable(final int value) {
        return () -> {
            final GetResponse getResponse = client().prepareGet("index2", "doc", Integer.toString(value)).get();

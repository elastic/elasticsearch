...
public class FollowingEngineTests extends ESTestCase {
    ...
    private Engine.Result applyOperation(Engine engine, Engine.Operation op, Engine.Operation.Origin origin) throws IOException {
        ...
        if (op instanceof Engine.Index) {
            ...
            result = engine.index(new Engine.Index(index.uid(), index.parsedDoc(), index.seqNo(), index.primaryTerm(),
                index.version(), versionType, origin, index.startTime(), index.getAutoGeneratedIdTimestamp(), index.isRetry()));
        } else if (op instanceof Engine.Delete) {
            Engine.Delete delete = (Engine.Delete) op;
            result = engine.delete(new Engine.Delete(delete.type(), delete.id(), delete.uid(), delete.seqNo(), delete.primaryTerm(),
                delete.version(), versionType, origin, delete.startTime()));
        } else {
            Engine.NoOp noOp = (Engine.NoOp) op;
            result = engine.noOp(new Engine.NoOp(noOp.seqNo(), noOp.primaryTerm(), origin, noOp.startTime(), noOp.reason()));
        }
        return result;
    }
        ...
        try (Store store = createStore(shardId, indexSettings, newDirectory())) {
            ...
            try (FollowingEngine followingEngine = createEngine(store, engineConfig)) {
                ...
                for (Engine.Operation op : operations) {
                    Engine.Result result = applyOperation(followingEngine, op, randomFrom(Engine.Operation.Origin.values()));
                    assertThat(result.getResultType(), equalTo(Engine.Result.Type.SUCCESS));
                }
                // Primary should reject duplicates
                for (Engine.Operation op : operations) {
                    Engine.Result result = applyOperation(followingEngine, op, Engine.Operation.Origin.PRIMARY);

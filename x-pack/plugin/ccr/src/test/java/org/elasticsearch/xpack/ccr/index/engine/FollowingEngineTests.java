...
public class FollowingEngineTests extends ESTestCase {
    ...
    private Engine.Result applyOperation(Engine engine, Engine.Operation op,
        ...
        if (op instanceof Engine.Index) {
            ...
            result = engine.index(new Engine.Index(index.uid(), index.parsedDoc(), index.seqNo(), primaryTerm, index.version(),
                index.versionType(), origin, index.startTime(), index.getAutoGeneratedIdTimestamp(), index.isRetry(),
            ...
            result = engine.delete(new Engine.Delete(delete.type(), delete.id(), delete.uid(), delete.seqNo(), primaryTerm,
                delete.version(), delete.versionType(), origin, delete.startTime(),
                delete.getIfSeqNoMatch(), delete.getIfPrimaryTermMatch()));
        } else {
    ...
    public void testOptimizeSingleDocConcurrently() throws Exception {
        List<Engine.Operation> operations = EngineTestCase.generateSingleDocHistory(
            false, randomFrom(VersionType.values()), false, 2, 10, 500, "id");
        Randomness.shuffle(operations);
        runFollowTest((leader, follower) -> {
            EngineTestCase.concurrentlyApplyOps(operations, leader);
            follower.waitForOpsToComplete(leader.getLocalCheckpoint());
            assertThat(getDocIds(follower, true), equalTo(getDocIds(leader, true)));

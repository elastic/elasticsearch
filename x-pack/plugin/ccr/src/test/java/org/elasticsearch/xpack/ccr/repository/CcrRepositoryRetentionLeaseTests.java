...
public class CcrRepositoryRetentionLeaseTests extends ESTestCase {
    ...
    public void testWhenRetentionLeaseAlreadyExistsWeTryToRenewIt() {
        ...
        doAnswer(invocationOnMock -> {
            ...
            return null;
        }).when(remoteClient).execute(same(RetentionLeaseActions.TransportAddAction.TYPE), addRequestCaptor.capture(), any());
        ...
        doAnswer(invocationOnMock -> {
            ...
            return null;
        }).when(remoteClient).execute(same(RetentionLeaseActions.TransportRenewAction.TYPE), renewRequestCaptor.capture(), any());

        repository.acquireRetentionLeaseOnLeader(followerShardId, retentionLeaseId, leaderShardId, remoteClient);

        verify(remoteClient).execute(
            same(RetentionLeaseActions.TransportAddAction.TYPE),
        ...
        verify(remoteClient).execute(
            same(RetentionLeaseActions.TransportRenewAction.TYPE),
            any(RetentionLeaseActions.RenewRequest.class),
            any()
        );

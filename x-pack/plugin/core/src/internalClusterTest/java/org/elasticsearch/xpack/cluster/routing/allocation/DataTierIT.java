    ...
    public void testOverrideDefaultAllocation() {
        ...
        client().admin().indices().prepareCreate(index)
            ...
            .setSettings(Settings.builder()
                .put(DataTierAllocationDecider.TIER_PREFERENCE, DataTier.DATA_WARM))
            .get();

        Settings idxSettings = client().admin().indices().prepareGetIndex().addIndices(index).get().getSettings().get(index);
        assertThat(idxSettings.get(DataTierAllocationDecider.TIER_PREFERENCE), equalTo(DataTier.DATA_WARM));

...
public class License implements ToXContentObject {
    ...
    public static License fromXContent(XContentParser parser) throws IOException {
        ...
        if (builder.signature != null) {
            ...
            try {
                ...
                throw new ParsingException("malformed signature for license [" + builder.uid + "]", e);
            }
            ...
            if (version == 0) {
                throw new ParsingException("malformed signature for license [" + builder.uid + "]");
    ...
    public static License fromSource(BytesReference bytes, XContentType xContentType) throws IOException {
        if (bytes == null || bytes.length() == 0) {
            throw new ParsingException("failed to parse license - no content provided");
        }
        if (xContentType == null) {
            throw new ParsingException("failed to parse license - no content-type provided");
        }
                ...
                if (parser.nextToken() == XContentParser.Token.FIELD_NAME) {
                    ...
                    if (Fields.LICENSES.equals(currentFieldName)) {
                        ...
                        if (parser.nextToken() == XContentParser.Token.START_ARRAY) {
                            ...
                            throw new ParsingException("failed to parse licenses expected an array of licenses");
                        }
                    ...
                    throw new ParsingException("failed to parse licenses expected field");
                }
            } else {
                throw new ParsingException("failed to parse licenses expected start object");
            }
            return license;
        }
    }
    ...
    private static long parseDate(XContentParser parser, String description, boolean endOfTheDay) throws IOException {
        if (parser.currentToken() == XContentParser.Token.VALUE_NUMBER) {
            ...
            try {
                ...
                throw new ParsingException("invalid " + description + " date format " + parser.text());
            }
        }
    }

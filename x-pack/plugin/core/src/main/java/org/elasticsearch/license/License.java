    ...
    public static License fromXContent(XContentParser parser) throws IOException {
        ...
        if (builder.signature != null) {
            ...
            try {
                ...
                throw new ElasticsearchParseException("malformed signature for license [" + builder.uid + "]", e); // HTTP 400
            }
            ...
            if (version == 0) {
                throw new ElasticsearchParseException("malformed signature for license [" + builder.uid + "]"); // HTTP 400
            } else if (version > VERSION_CURRENT) {
                throw new ElasticsearchParseException("Unknown license version found, please upgrade all nodes to the latest " +
                    "elasticsearch-license plugin"); // HTTP 400
            }

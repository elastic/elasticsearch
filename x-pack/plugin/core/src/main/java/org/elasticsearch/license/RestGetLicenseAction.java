...
import static org.elasticsearch.core.RestApiVersion.V_9;
...
public class RestGetLicenseAction extends BaseRestHandler {
    ...
    public RestChannelConsumer prepareRequest(final RestRequest request, final NodeClient client) throws IOException {
        ...
        if (request.getRestApiVersion() == V_8) {
        ...
        if (request.hasParam("accept_enterprise")) {
            ...
            if (request.paramAsBoolean("accept_enterprise", true) == false && request.getRestApiVersion().matches(onOrAfter(V_9))) {

...
public class StartTrialClusterTask implements ClusterStateTaskListener {
    ...
    private LicensesMetadata execute(
        ...
        if (request.isAcknowledged() == false) {
            ...
            long expiryDate = issueDate + LicenseServiceInterface.NON_BASIC_SELF_GENERATED_LICENSE_DURATION.getMillis();
            ...
            if (License.LicenseType.isEnterprise(request.getType())) {
                specBuilder.maxResourceUnits(LicenseServiceInterface.SELF_GENERATED_LICENSE_MAX_RESOURCE_UNITS);
            } else {
                specBuilder.maxNodes(LicenseServiceInterface.SELF_GENERATED_LICENSE_MAX_NODES);
            }

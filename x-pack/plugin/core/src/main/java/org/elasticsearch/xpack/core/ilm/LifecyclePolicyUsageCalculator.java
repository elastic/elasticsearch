...
public class LifecyclePolicyUsageCalculator {
    ...
    public LifecyclePolicyUsageCalculator(
        ...
        final IndexLifecycleMetadata ilmMetadata = metadata.custom(IndexLifecycleMetadata.TYPE);
        ...
        for (Map.Entry<String, ComposableIndexTemplate> entry : metadata.templatesV2().entrySet()) {
            Settings settings = MetadataIndexTemplateService.resolveSettings(entry.getValue(), metadata.componentTemplates());
        ...
        final var indexTemplates = new ArrayList<>(metadata.templatesV2().entrySet());
        ...
        for (String dataStream : allDataStreams) {
            ...
            String indexTemplate = MetadataIndexTemplateService.findV2TemplateFromSortedList(metadata, indexTemplates, dataStream, false);

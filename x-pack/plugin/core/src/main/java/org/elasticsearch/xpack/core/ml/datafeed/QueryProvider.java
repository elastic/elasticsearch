...
class QueryProvider implements Writeable, ToXContentObject {
    ...
    static QueryProvider fromStream(StreamInput in) throws IOException {
        if (in.getVersion().after(Version.V_6_6_2)) { // Has our bug fix for query/agg providers
    ...
    public void writeTo(StreamOutput out) throws IOException {
        if (out.getVersion().after(Version.V_6_6_2)) { // Has our bug fix for query/agg providers
            out.writeMap(query);

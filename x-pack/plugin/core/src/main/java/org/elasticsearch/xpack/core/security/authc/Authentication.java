...
public final class Authentication implements ToXContentObject {
    ...
    public Authentication maybeRewriteForOlderVersion(TransportVersion olderVersion) {
        ...
        // not possible
        if (isCrossClusterAccess() && olderVersion.before(TransportVersions.V_8_10_X)) {
    ...
    private static void doWriteTo(Subject effectiveSubject, Subject authenticatingSubject, AuthenticationType type, StreamOutput out)
        ...
        if (isCrossClusterAccess && out.getTransportVersion().before(TransportVersions.V_8_10_X)) {
            throw new IllegalArgumentException(
                "versions of Elasticsearch before ["
                    + TransportVersions.V_8_10_X.toReleaseVersion()
    ...
    private static Map<String, Object> maybeRewriteMetadataForApiKeyRoleDescriptors(
        ...
        if (authentication.isAuthenticatedAsApiKey()) {
            ...
            if (authentication.getEffectiveSubject().getTransportVersion().onOrAfter(TransportVersions.V_8_10_X)
                && streamVersion.before(TransportVersions.V_8_10_X)) {

...
/**
 * This class exists for BWC reasons only.
 * From ES version 6.0.0 until 7.2.0, the {@code TokenService} used an encryption based mechanism in the token format.
 * This metadata was used to pass encryption keys between different nodes.
 * From ES version 7.2.0 until 8.8.0, the {@code TokenService} maintained support for these encryption based tokens
 * Since ES 8.8.0, this support has been removed, but the metadata class still exists to allow nodes to join a cluster that
 * has the metadata installed in cluster state.
 */
public final class TokenMetadata extends AbstractNamedDiffable<ClusterState.Custom> implements ClusterState.Custom {
    ...
    /**
     * @deprecated Should only be created for testing purposes. We do not want to install this into new clusters
     */
    @Deprecated

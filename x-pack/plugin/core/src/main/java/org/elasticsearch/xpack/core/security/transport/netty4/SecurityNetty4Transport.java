...
public class SecurityNetty4Transport extends Netty4Transport {
    ...
    protected boolean supportRetry() {
        return coordinator.isDualStackEnabled();
    }

    @Override
    protected Netty4TcpChannel initiateChannel(DiscoveryNode node, boolean isRetry) throws IOException {
        Netty4TcpChannel netty4TcpChannel = super.initiateChannel(node, isRetry);
        if (isRetry) {
            coordinator.registerPlaintextChannel(netty4TcpChannel);
        }
    ...
    protected ChannelHandler getClientChannelInitializer(DiscoveryNode node, boolean isRetry) {
        return new SecurityClientChannelInitializer(node, isRetry);
    }
    ...
    private class SecurityClientChannelInitializer extends ClientChannelInitializer {
        ...
        SecurityClientChannelInitializer(DiscoveryNode node, boolean isRetry) {

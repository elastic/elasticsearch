    ...
    public ActionWrapperResult execute(WatchExecutionContext ctx) {
        ...
        if (condition != null) {
            try {
                ...
                action.logger()
                    .error(() -> format(ROOT, "failed to execute action [%s/%s]. failed to execute condition", ctx.watch().id(), id), e);
                return new ActionWrapperResult(
                    id,
        ...
        if (transform != null) {
            try {
                ...
                action.logger()
                    .error(() -> format(ROOT, "failed to execute action [%s/%s]. failed to transform payload.", ctx.watch().id(), id), e);
        ...
        if (Strings.isEmpty(path)) {
            try {
                ...
                action.logger().error(() -> format(ROOT, "failed to execute action [%s/%s]", ctx.watch().id(), id), e);

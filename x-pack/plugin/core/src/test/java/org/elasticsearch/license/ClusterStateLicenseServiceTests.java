...
import static org.elasticsearch.license.LicenseServiceInterface.LICENSE_EXPIRATION_WARNING_PERIOD;
...
public class ClusterStateLicenseServiceTests extends ESTestCase {
    ...
    /**
     * Tests loading a license when {@link LicenseServiceInterface#ALLOWED_LICENSE_TYPES_SETTING}
     * is on its default value (all license types)
     */
    ...
    /**
     * Tests loading a license when {@link LicenseServiceInterface#ALLOWED_LICENSE_TYPES_SETTING} is set,
    ...
    public void testSuccessfullyRegisterLicenseMatchingTypeRestrictions() throws Exception {
        final List<License.LicenseType> allowed = randomSubsetOf(
            randomIntBetween(1, LicenseServiceInterface.ALLOWABLE_UPLOAD_TYPES.size() - 1),
            LicenseServiceInterface.ALLOWABLE_UPLOAD_TYPES
        );
    ...
    public void testFailToRegisterLicenseNotMatchingTypeRestrictions() throws Exception {
        final List<License.LicenseType> allowed = randomSubsetOf(
            randomIntBetween(1, LicenseServiceInterface.ALLOWABLE_UPLOAD_TYPES.size() - 2),
            LicenseServiceInterface.ALLOWABLE_UPLOAD_TYPES
        );
        final List<String> allowedNames = allowed.stream().map(License.LicenseType::getTypeName).toList();
        final Settings settings = Settings.builder().putList("xpack.license.upload.types", allowedNames).build();
    ...
    public void testLicenseWithOverridenExpiryInPastIsExpired() throws IOException {
        UUID licenseId = UUID.fromString("12345678-abcd-0000-0000-000000000000"); // Special test UUID
        License.LicenseType type = randomFrom(LicenseServiceInterface.ALLOWABLE_UPLOAD_TYPES);
    ...
    public void testStartBasicStartsNewLicenseIfFieldsDifferent() throws Exception {
        ...
        if (future.isDone()) {
            ...
            int maxNodes = randomValueOtherThan(
                LicenseServiceInterface.SELF_GENERATED_LICENSE_MAX_NODES,
                () -> randomIntBetween(1, LicenseServiceInterface.SELF_GENERATED_LICENSE_MAX_NODES)
            );

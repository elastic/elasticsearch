...
public class XPackInfoRequestTests extends ESTestCase {
    ...
    private void assertSerialization(Version version) throws java.io.IOException {
        ...
        assertThat("Serialized request with version [" + version + "] does not match original [categories]",
            read.getCategories(), equalTo(request.getCategories()));
        assertThat("Serialized request with version [" + version + "] does not match original [verbose]",
            read.isVerbose(), equalTo(request.isVerbose()));
    }

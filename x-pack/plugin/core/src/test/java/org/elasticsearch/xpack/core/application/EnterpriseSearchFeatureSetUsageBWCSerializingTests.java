...
public class EnterpriseSearchFeatureSetUsageBWCSerializingTests extends AbstractBWCWireSerializationTestCase<EnterpriseSearchFeatureSetUsage> {
    ...
    protected EnterpriseSearchFeatureSetUsage mutateInstanceForVersion(EnterpriseSearchFeatureSetUsage instance, TransportVersion version) {
        if (version.onOrAfter(QUERY_RULES_TRANSPORT_VERSION)) {
            ...
            return new EnterpriseSearchFeatureSetUsage(true, true, instance.getSearchApplicationsUsage(),
                instance.getAnalyticsCollectionsUsage(), Map.of());
        } else {

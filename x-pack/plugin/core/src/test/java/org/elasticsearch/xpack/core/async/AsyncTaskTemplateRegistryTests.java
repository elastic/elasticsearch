...
import static org.elasticsearch.xpack.core.async.AsyncTaskTemplateRegistry.ASYNC_SEARCH_TEMPLATE_NAME;
import static org.elasticsearch.xpack.core.async.AsyncTaskTemplateRegistry.INDEX_TEMPLATE_VERSION;
...
public class AsyncTaskTemplateRegistryTests extends ESTestCase {
    private AsyncTaskTemplateRegistry registry;
    ...
    public void createRegistryAndClient() {
        ...
        registry = new AsyncTaskTemplateRegistry(Settings.EMPTY, clusterService, threadPool, client, xContentRegistry);
    }
    ...
    public void testThatMissing$MasterNodeDoesNothing() {
        ...
        ClusterChangedEvent event = createClusterChangedEvent(Collections.singletonMap(ASYNC_SEARCH_TEMPLATE_NAME, null), nodes);
        registry.clusterChanged(event);
    }
    ...
    public void testValidate() {
        ...
        assertTrue(registry.validate(createClusterState(Settings.EMPTY,
            Collections.singletonMap(ASYNC_SEARCH_TEMPLATE_NAME, null),
    ...
    private ActionResponse verifyTemplateInstalled(
        ...
        if (action instanceof PutComposableIndexTemplateAction) {
            ...
            assertThat(putRequest.name(), equalTo(ASYNC_SEARCH_TEMPLATE_NAME));
            assertThat(putRequest.indexTemplate().template().settings().get("index.auto_expand_replicas"), equalTo("0-1"));

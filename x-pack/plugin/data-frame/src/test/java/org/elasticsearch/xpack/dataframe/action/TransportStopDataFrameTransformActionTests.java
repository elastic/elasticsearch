...
public class TransportStopDataFrameTransformActionTests extends ESTestCase {
    ...
    public void testFirstNotOKStatus() {
        ...
        assertThat(TransportStopDataFrameTransformAction.firstNotOKStatus(Collections.emptyList(), Collections.emptyList()),
            equalTo(RestStatus.INTERNAL_SERVER_ERROR));

        assertThat(TransportStopDataFrameTransformAction.firstNotOKStatus(taskOperationFailures, Collections.emptyList()),
            equalTo(RestStatus.BAD_REQUEST));
        assertThat(TransportStopDataFrameTransformAction.firstNotOKStatus(taskOperationFailures, nodeFailures),
            equalTo(RestStatus.BAD_REQUEST));
        assertThat(TransportStopDataFrameTransformAction.firstNotOKStatus(taskOperationFailures,
            Collections.singletonList(new ElasticsearchException(new ElasticsearchStatusException("not failure", RestStatus.OK)))),
            equalTo(RestStatus.BAD_REQUEST));

        assertThat(TransportStopDataFrameTransformAction.firstNotOKStatus(
    ...
    public void testBuildException() {
        ...
        ElasticsearchStatusException statusException =
            TransportStopDataFrameTransformAction.buildException(taskOperationFailures, nodeFailures, status);
        ...
        statusException = TransportStopDataFrameTransformAction.buildException(Collections.emptyList(), nodeFailures, status);
        assertThat(statusException.status(), equalTo(status));
        ...
        statusException = TransportStopDataFrameTransformAction.buildException(taskOperationFailures, Collections.emptyList(), status);
        assertThat(statusException.status(), equalTo(status));
        assertThat(statusException.getMessage(), equalTo(taskOperationFailures.get(0).getCause().getMessage()));
        assertThat(statusException.getSuppressed().length, equalTo(0));
    }

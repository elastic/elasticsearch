 ...
 */
public class ComposableIndexTemplateDeprecationChecker implements ResourceDeprecationChecker {

    public static final String NAME = "index_templates";
    private static final List<Function<ComposableIndexTemplate, DeprecationIssue>> CHECKS = List.of(
        ComposableIndexTemplateDeprecationChecker::checkIndexTemplates
    );
    ...
    public Map<String, List<DeprecationIssue>> check(ClusterState clusterState, DeprecationInfoAction.Request request) {
        var templates = clusterState.metadata().templatesV2().entrySet();

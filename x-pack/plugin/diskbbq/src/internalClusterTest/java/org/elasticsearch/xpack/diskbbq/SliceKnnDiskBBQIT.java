...
public class SliceKnnDiskBBQIT extends ESIntegTestCase {
    ...
    public void testKnnSearchWithoutSliceDefaultsToAllSlices() {
        ...
        client().index(
            new IndexRequest("slice-knn-bbq-all").id("1").source("vector", axisVector(64, 0)).routing("s1").setRoutingFromSlice(true)
        ).actionGet();
        client().index(
            new IndexRequest("slice-knn-bbq-all").id("2").source("vector", axisVector(64, 1)).routing("s2").setRoutingFromSlice(true)
        ).actionGet();
        refresh("slice-knn-bbq-all");

        // No _slice provided should default to _all and find documents from all slices
        final KnnSearchBuilder knn = new KnnSearchBuilder("vector", axisVector(64, 1), 10, 10, 100f, null, null);

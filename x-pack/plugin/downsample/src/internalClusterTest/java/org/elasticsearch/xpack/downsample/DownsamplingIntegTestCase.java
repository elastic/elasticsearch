...
public abstract class DownsamplingIntegTestCase extends ESIntegTestCase {
     ...
     */
    public void putTSDBIndexTemplate(String pattern, @Nullable String startTime, @Nullable String endTime, DataStreamLifecycle lifecycle)
        throws IOException {
    ...
    List<String> waitForDataStreamBackingIndices(String dataStreamName, int backingIndexCount) throws Exception {
        List<String> backingIndices = new ArrayList<>();
        assertBusy(() -> {
            var indices = getDataStreamBackingIndexNames(dataStreamName);
            assertThat(indices.size(), equalTo(backingIndexCount));
            backingIndices.addAll(indices);
        });

 ...
 */
public final class AggregateMetricDoubleFieldValueFetcher extends FieldValueFetcher {

    private final AggregateMetricDoubleFieldType aggMetricFieldType;

    private final AggregateMetricDoubleFieldProducer fieldProducer;

    AggregateMetricDoubleFieldValueFetcher(
     ...
     */
    static List<AggregateMetricDoubleFieldValueFetcher> create(
        SearchExecutionContext context,
        AggregateMetricDoubleFieldMapper.AggregateMetricDoubleFieldType aggMetricFieldType,
        DownsampleConfig.SamplingMethod samplingMethod
    ) {
        List<AggregateMetricDoubleFieldValueFetcher> fetchers = new ArrayList<>();
        ...
        for (NumberFieldMapper.NumberFieldType metricSubField : aggMetricFieldType.getMetricFields().values()) {
            if (context.fieldExistsInIndex(metricSubField.name())) {
                ...
                fetchers.add(new AggregateMetricDoubleFieldValueFetcher(metricSubField, aggMetricFieldType, fieldData, samplingMethod));
            }
        }
        return fetchers;
    }

    private AggregateMetricDoubleFieldProducer createFieldProducer(DownsampleConfig.SamplingMethod samplingMethod) {
        ...
        if (aggMetricFieldType.getMetricType() != null) {
            if (samplingMethod != DownsampleConfig.SamplingMethod.LAST_VALUE) {
                ...
                return new AggregateMetricDoubleFieldProducer.Aggregate(aggMetricFieldType.name(), metric);
            } else {
                return new AggregateMetricDoubleFieldProducer.LastValue(aggMetricFieldType.name(), metric, false);
            }
        } else {
            // If a field is not a metric, we downsample it as a label
            return new AggregateMetricDoubleFieldProducer.LastValue(aggMetricFieldType.name(), metric, true);
        }
    }
}

...
public class TransportDownsampleAction extends AcknowledgedTransportMasterNodeAction<DownsampleAction.Request> {
    ...
    protected void masterOperation(
        ...
        final IndicesAccessControl indicesAccessControl = threadPool.getThreadContext()
        ...
        if (canShortCircuit(downsampleIndexName, parentTask, request.getWaitTimeout(), startTime, true, state.metadata(), listener)) {
        ...
        client.admin().indices().getMappings(getMappingsRequest, listener.delegateFailureAndWrap((delegate, getMappingsResponse) -> {
            ...
            try {
                mapping = createDownsampleIndexMapping(request.getDownsampleConfig(), mapperService, sourceIndexMappings);
            } catch (IOException e) {
    ...
    public static String createDownsampleIndexMapping(
        ...
        addMetricFields(sourceIndexMappings, builder);
    ...
    private static void addMetricFields(final Map<String, Object> sourceIndexMappings, final XContentBuilder builder) {

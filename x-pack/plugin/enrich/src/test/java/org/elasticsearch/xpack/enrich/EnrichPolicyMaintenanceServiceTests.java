...
import static org.elasticsearch.xpack.core.enrich.EnrichPolicyDefinition.EXACT_MATCH_TYPE;
...
public class EnrichPolicyMaintenanceServiceTests extends ESSingleNodeTestCase {
    ...
    private EnrichPolicyDefinition randomPolicy() {
        List<String> enrichKeys = new ArrayList<>();
        for (int i = 0; i < randomIntBetween(1, 3); i++) {
            enrichKeys.add(randomAlphaOfLength(10));
        }
        return new EnrichPolicyDefinition(EXACT_MATCH_TYPE, null, List.of(randomAlphaOfLength(10)), randomAlphaOfLength(10), enrichKeys);
    }

    private void addPolicy(String policyName, EnrichPolicyDefinition policyDefinition) throws InterruptedException {
        doSyncronously((clusterService, exceptionConsumer) -> {
            EnrichPolicy policy = new EnrichPolicy(policyName, Version.CURRENT, policyDefinition);
    ...
    private String fakeRunPolicy(String forPolicy) throws IOException {
        String newIndexName = EnrichPolicyDefinition.getBaseName(forPolicy) + "-" + indexNameAutoIncrementingCounter++;
    ...
    private void promoteFakePolicyIndex(String indexName, String forPolicy) {
        String enrichIndexBase = EnrichPolicyDefinition.getBaseName(forPolicy);

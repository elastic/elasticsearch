...
public class RestUpdateConnectorFeaturesAction extends BaseRestHandler {
    ...
    public List<Route> routes() {
        return List.of(new Route(PUT, "/" + EnterpriseSearch.CONNECTOR_API_ENDPOINT + "/{" + CONNECTOR_ID_PARAM + "}/_features"));
    }

    @Override
    protected RestChannelConsumer prepareRequest(RestRequest restRequest, NodeClient client) throws IOException {
        try (XContentParser parser = restRequest.contentParser()) {
            UpdateConnectorFeaturesAction.Request request = UpdateConnectorFeaturesAction.Request.fromXContent(
                parser,

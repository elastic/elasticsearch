...
public class TransportUpdateConnectorLastSyncStatsAction extends HandledTransportAction<
    ...
    @Override
    protected void doExecute(Task task, UpdateConnectorLastSyncStatsAction.Request request, ActionListener<AcknowledgedResponse> listener) {
        connectorIndexService.updateConnectorLastSyncStats(request, listener.map(r -> AcknowledgedResponse.TRUE));
    }
}

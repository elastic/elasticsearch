...
public class TransportUpdateConnectorLastSyncStatsAction extends HandledTransportAction<
    ...
    @Override
    protected void doExecute(Task task, UpdateConnectorLastSyncStatsAction.Request request, ActionListener<ActionResponse.Empty> listener) {
        connectorIndexService.updateConnectorLastSyncStats(request, listener.map(r -> ActionResponse.Empty.INSTANCE));
    }
}

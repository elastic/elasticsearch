...
public class TransportGetEngineActionTests extends ESTestCase {
    public void testWithUnsupportedLicense() {
        ...
        TransportGetEngineAction transportAction = new TransportGetEngineAction(
        ...
        String engineId = randomFrom(new String[] { null, randomAlphaOfLengthBetween(1, 10) });
        GetEngineAction.Request request = new GetEngineAction.Request(engineId);

        final AtomicReference<Throwable> throwableRef = new AtomicReference<>();
        final AtomicReference<GetEngineAction.Response> responseRef = new AtomicReference<>();

        transportAction.doExecute(mock(Task.class), request, new ActionListener<>() {
            @Override
            public void onResponse(GetEngineAction.Response response) {

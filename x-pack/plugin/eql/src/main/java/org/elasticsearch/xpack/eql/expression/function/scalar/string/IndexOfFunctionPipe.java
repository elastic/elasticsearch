...
public class IndexOfFunctionPipe extends Pipe {
    ...
    public IndexOfFunctionPipe(Source source, Expression expression, Pipe input, Pipe substring, Pipe start) {
    ...
    protected IndexOfFunctionPipe replaceChildren(Pipe newInput, Pipe newSubstring, Pipe newStart) {
        return new IndexOfFunctionPipe(source(), expression(), newInput, newSubstring, newStart);
    }
    ...
    protected NodeInfo<IndexOfFunctionPipe> info() {
        return NodeInfo.create(this, IndexOfFunctionPipe::new, expression(), input, substring, start);
    }

    @Override
    public IndexOfFunctionProcessor asProcessor() {
        return new IndexOfFunctionProcessor(input.asProcessor(), substring.asProcessor(), start.asProcessor());
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(input, substring, start);
    }

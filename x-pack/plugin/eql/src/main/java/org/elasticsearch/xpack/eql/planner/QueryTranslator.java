    ...
    public static Query toQuery(Expression e, TranslatorHandler handler) {
        ...
        if (translation != null) {
            if (translation instanceof ScriptQuery) {
                ...
                boolean multiValuedCompatible = false == e.anyMatch(exp -> {
                    if (exp instanceof Literal == false
                        && exp instanceof FieldAttribute == false
                        && exp instanceof MultiValuedOperationCompatible == false) {
                        return true;
                    } else {
                        return false;
                    }});
                if (multiValuedCompatible) {
                    ScriptQuery query = (ScriptQuery) translation;
                    return new NoNullSafetyScriptQuery(query.source(), Scripts.multiValueDocValuesReplacement(query.script()));
                }
            }
            return translation;
        }

...
public class AsyncEqlSearchTaskTests extends ESTestCase {
...
/*
    public void testWaitForInit() throws InterruptedException {
        AsyncEqlSearchTask task = new AsyncEqlSearchTask(0L, "", "", new TaskId("node1", 0), () -> false, TimeValue.timeValueHours(1),
        ...
        for (int i = 0; i < numThreads; i++) {
            Thread thread = new Thread(() -> task.addCompletionListener(new ActionListener<>() {
                @Override
                public void onResponse(AsyncEqlSearchResponse resp) {
                    assertThat(numShards + numSkippedShards, equalTo(resp.getEqlSearchResponse().getTotalShards()));
                    assertThat(numSkippedShards, equalTo(resp.getEqlSearchResponse().getSkippedShards()));
                    assertThat(0, equalTo(resp.getEqlSearchResponse().getFailedShards()));
                    latch.countDown();
                }
    ...
    public void testWaitForCompletion() throws InterruptedException {
        ...
        task.getSearchProgressActionListener().onFinalReduce(shards,
            new TotalHits(0, TotalHits.Relation.GREATER_THAN_OR_EQUAL_TO), null, 0);
        assertCompletionListeners(task, numShards+numSkippedShards, numSkippedShards, numShardFailures, true);
        ((AsyncEqlSearchTask.Listener)task.getProgressListener()).onResponse(
    ...
    private void assertCompletionListeners(AsyncEqlSearchTask task,

...
public class NodeTransformTests extends ESTestCase {
    // Transform Up Tests
    ...
    public void testTransformPropertiesOnly() throws Exception {
        NodeTests.ChildrenAreAProperty tree = createSimpleTree();

        // Sync transformation: change the root mode property only
        NodeTests.Dummy result = tree.transformPropertiesOnly(String.class, s -> s + "_changed");
        NodeTests.ChildrenAreAProperty transformed = (NodeTests.ChildrenAreAProperty) result;
        assertThat(transformed.thing(), equalTo("node_changed"));
        // Children should remain unchanged because transformPropertiesOnly does not traverse
        assertThat(transformed.children().get(0).thing(), equalTo("leaf1"));
        assertThat(transformed.children().get(1).thing(), equalTo("leaf2"));

        // Async variant should yield identical result
        SetOnce<NodeTests.Dummy> asyncResult = new SetOnce<>();
        tree.transformPropertiesOnly(
            String.class,
            (s, l) -> l.onResponse(s + "_changed"),
            ActionListener.wrap(asyncResult::set, ESTestCase::fail)
        );
        assertBusy(() -> assertThat(asyncResult.get(), equalTo(result)));
    }

    public void testTransformPropertiesDown() throws Exception {
        NodeTests.ChildrenAreAProperty tree = createNestedTree();

        // Sync transformation: propagate "_changed" to every "thing" property top-down
        NodeTests.Dummy result = tree.transformPropertiesDown(String.class, s -> s + "_changed");
        // Root, inner, and leaves should all have suffix
        NodeTests.ChildrenAreAProperty outer = (NodeTests.ChildrenAreAProperty) result;
        assertThat(outer.thing(), equalTo("outer_changed"));
        NodeTests.ChildrenAreAProperty inner = (NodeTests.ChildrenAreAProperty) outer.children().get(0);
        assertThat(inner.thing(), equalTo("inner_changed"));
        assertThat(inner.children().get(0).thing(), equalTo("leaf1_changed"));
        assertThat(inner.children().get(1).thing(), equalTo("leaf2_changed"));

        // Async variant should yield identical result
        SetOnce<NodeTests.Dummy> asyncResult = new SetOnce<>();
        tree.transformPropertiesDown(
            String.class,
            (s, l) -> l.onResponse(s + "_changed"),
            ActionListener.wrap(asyncResult::set, ESTestCase::fail)
        );
    ...
    public void testTransformPropertiesUp() throws Exception {
        NodeTests.ChildrenAreAProperty tree = createNestedTree();

        // Sync transformation: propagate "_changed" to every "thing" property bottom-up
        NodeTests.Dummy result = tree.transformPropertiesUp(String.class, s -> s + "_changed");
        NodeTests.ChildrenAreAProperty outer = (NodeTests.ChildrenAreAProperty) result;
        assertThat(outer.thing(), equalTo("outer_changed"));
        NodeTests.ChildrenAreAProperty inner = (NodeTests.ChildrenAreAProperty) outer.children().get(0);
        assertThat(inner.thing(), equalTo("inner_changed"));

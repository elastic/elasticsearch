...
public class CsvFormatReader implements SegmentableFormatReader {
    ...
    /**
     * How a pinned schema binds to ONE file: the raw field index per schema column ({@link #ABSENT_FIELD} where the
     * file lacks it), and the widest row the file may carry — its header's width, or for a headerless file the wider of
     * its first data record (see {@link #leadingColumns}) and the schema.
     *
    ...
    private List<Attribute> readSchema(StorageObject object, Consumer<String> warningSink) throws IOException {
        ...
        try (Closeable abortOnExit = () -> object.abortStream(stream)) {
            CsvLogicalRecordReader recordReader = openLeadingRecords(stream, READER_BUFFER_SIZE);
    ...
    public CloseableIterator<Page> read(StorageObject object, FormatReadContext context) throws IOException {
        ...
        // The read that owns the file's start peeks its columns the way fileHeaderColumns reads them for later splits,
        // so every split binds the same names and bounds rows by the same width.
        List<String> leadingColumns = null;
        if (context.firstSplit() && context.readSchema() != null) {
            try {
                LeadingColumnsPeek peek = peekLeadingColumns(streamAfterBom);
                streamAfterBom = peek.stream();
                leadingColumns = peek.columns();
            } catch (Exception e) {
                try {
                    stream.close();
        ...
        // Raw field index per schema column, or null for the positional contract. Set whenever a schema is pinned;
        // see fieldIndexes.
        ...
        if (readSchema != null) {
            ...
            try {
                ...
                if (context.firstSplit() && options.headerRow()) {
                    // The header was peeked above; the data path still has to step over it.
                    consumeHeaderLine(recordReader);
                }
                headerBinding = bindFileColumns(context.firstSplit() ? leadingColumns : context.fileHeaderColumns(), context, object);
            } catch (Exception e) {
                try {
                    reader.close();
                } catch (IOException suppressed) {
     ...
     * Also finds a headerless file's first data record for {@link #leadingColumns}.
     * Blank/comment classification is the same predicate as {@link #skipLeadingContentRows}.
     */
    ...
    /**
     * Binds a pinned schema to one split's file columns: peeked by the read that owns the file's start, else passed down.
     * A headered file binds each column by header name, and its header's width bounds every row. A headerless file's
     * names are positions, and a row may be as wide as its first record or the schema, whichever is wider: inference
     * sizes a headerless schema by the widest row it samples, so a row that schema reads is not drift. A later split
     * handed no columns fails loudly rather than bind by position.
     *
     * @return the binding, or {@code null} for an empty file — nothing to bind, and nothing to read
     */
    private HeaderBinding bindFileColumns(@Nullable List<String> fileColumns, FormatReadContext context, StorageObject object) {
        List<Attribute> readSchema = context.readSchema();
        if (fileColumns == null) {
            ...
            throw new IllegalStateException(
                options.headerRow()
                    ...
                    : "headerless read of ["
                        + object.path()
                        + "] reached a non-first split without the file's columns; cannot bound its rows by the file's width"
            );
        }
        if (options.headerRow()) {
            return bindToHeaderNames(fileColumns.toArray(new String[0]), readSchema, object);
        }
        return new HeaderBinding(fieldIndexes(readSchema, null, object), Math.max(fileColumns.size(), readSchema.size()));
    }
    ...
    /**
     * The file's own column names via {@link #leadingColumns}, as the read that owns the file's start derives them.
     * <p>
    ...
    public List<String> fileHeaderColumns(StorageObject file) throws IOException {
        ...
        try (Closeable abortOnExit = () -> file.abortStream(stream)) {
            return leadingColumns(openLeadingRecords(stream, READER_BUFFER_SIZE));
        }
    }

    /**
     * Opens a file's leading records, past a byte-order mark and {@code skip_rows}, the one way {@link #readSchema},
     * {@link #fileHeaderColumns} and the first split's column peek all read them, so the three agree on where the header
     * ends: the dialect's quoting, never escape-aware, as schema inference has always read it.
     */
    private CsvLogicalRecordReader openLeadingRecords(InputStream stream, int bufferSize) throws IOException {
        BufferedReader reader = new BufferedReader(new InputStreamReader(stream, options.encoding()), bufferSize);
    ...
    /**
     * The file's column names: a headered file's header names, or for a headerless file {@code col0..col{w-1}}, where
     * {@code w} is the field count of its first data record that tokenizes.
     *
     * @return the names in file order, or {@code null} when the file has no header or data record
     */
    private List<String> leadingColumns(CsvLogicalRecordReader recordReader) throws IOException {
        if (options.headerRow()) {
            String headerLine = consumeHeaderLine(recordReader);
            return headerLine == null ? null : List.of(headerColumnNames(headerLine, splitFieldsForOptions(headerLine, options)));
        }
        String record;
        while ((record = consumeHeaderLine(recordReader)) != null) {
            try {
                return List.of(synthesizeColumnNames(dataRecordWidth(record), options.columnPrefix()));
            } catch (MalformedRowException e) {
                // A row the data path would reject under error_mode does not state the file's width; the next one does.
            }
        }
        return null;
    }

    /** A headerless data record's field count, split the way the data path splits it. */
    private int dataRecordWidth(String record) {
        if (options.multiValueSyntax() == CsvFormatOptions.MultiValueSyntax.BRACKETS && options.delimiter() == ',') {
            return splitCommaDelimiterBracketAwareFields(record, options.quoteChar(), options.escapeChar(), options.trimSpaces()).length;
        }
        return splitRecordFields(record, options, Integer.MAX_VALUE).length;
    }

    /** The front of a file after {@link #peekLeadingColumns}: the stream to read it from, and its columns. */
    private record LeadingColumnsPeek(InputStream stream, @Nullable List<String> columns) {}

    /**
     * Reads the file's columns off the front of {@code stream} without consuming them: every byte the peek pulls is
     * replayed ahead of the rest, so the returned stream starts at the same byte {@code stream} did.
     */
    private LeadingColumnsPeek peekLeadingColumns(InputStream stream) throws IOException {
        RecordingInputStream recording = new RecordingInputStream(stream);
        List<String> columns = leadingColumns(openLeadingRecords(recording, PEEK_BUFFER_SIZE));
        return new LeadingColumnsPeek(new SequenceInputStream(new ByteArrayInputStream(recording.recorded()), stream), columns);
    }

    /** Reader buffer for {@link #peekLeadingColumns}: small, since every char it reads ahead is replayed. */
    private static final int PEEK_BUFFER_SIZE = 8 * 1024;

    /** Passes reads through and keeps a copy of every byte read, for {@link #peekLeadingColumns} to replay. */
    ...
    private class CsvBatchIterator extends BufferingPageIterator {
        ...
        /**
         * Raw field index per pinned-schema position, or {@code null} when no schema is pinned and the reader's own
         * inference (this file's header) is the schema, so position and name coincide. Lets a column read the file
         * column it names rather than the column that happens to sit at its schema position.
         */
        @Nullable
        private final int[] schemaFieldIndex;
        private int[] projectedIdx;
        /**
         * Widest row this schema accepts before it reads as drift. Naming 5 columns of a 105-column file still bounds
         * rows at 105. Positional binding takes {@code schema.size()} (which IS the header's width, inference having
         * derived it from that header); by-name binding takes {@link #bindingMaxRowWidth}. Short rows are not this
         * field's concern — they null-fill.
         */
        private int rowWidthLimit;
        /** The binding's widest row; resolves {@link #rowWidthLimit}. Unused when no schema is pinned. */
        ...
        CsvBatchIterator(
            ...
            this.bindingMaxRowWidth = headerBinding == null ? 0 : headerBinding.maxRowWidth();
        ...
        /**
         * The width guard's message, naming what set the number the guard compared against: the schema under positional
         * binding, the header under a pinned schema over a headered file, and over a headerless file the schema unless
         * the file's first row is wider.
         */
        private String rowTooWideMessage(int actualFields) {
            ...
            // many columns as a headered file still bound the row by the header's width, and should say so.
            if (schemaFieldIndex == null) {
                return "CSV row has [" + actualFields + "] columns but schema defines [" + schemaColumnCount + "] columns";
            }
            if (options.headerRow()) {
                return "CSV row has [" + actualFields + "] columns but the file's header defines [" + rowWidthLimit + "] columns";
            }
            if (rowWidthLimit == schemaColumnCount) {
                return "CSV row has [" + actualFields + "] columns but schema defines [" + rowWidthLimit + "] columns";
            }
            return "CSV row has [" + actualFields + "] columns but the file's first row has [" + rowWidthLimit + "] columns";
        }
        ...
        private void initProjection() {
            ...
            // schema attribute. Without a pinned schema schemaFieldIndex is null, rawFieldIndex() is the
            // identity, and the two arrays are equal.
            int[] schemaPos;

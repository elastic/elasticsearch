...
public class CsvFormatReaderTests extends ESTestCase {
    ...
    // findLastRecordBoundary is what the streaming-parallel coordinator uses to choose chunk cut
    // points. It must skip newlines inside quoted/bracketed cells; otherwise multi-MiB CSV reads
    // with embedded newlines split chunks mid-record and parser threads throw "Unclosed quoted field".
    ...
    public void testFindLastRecordBoundaryAllInsideQuotedField() throws IOException {
        CsvFormatReader reader = new CsvFormatReader(blockFactory);
        // Buffer is entirely the inside of an unterminated quoted cell. There is no record boundary
        // anywhere within it — the caller must grow the buffer to keep reading.
        byte[] data = "\"line one\nline two\nline three\n".getBytes(StandardCharsets.UTF_8);
    ...
    public void testFindLastRecordBoundaryDoubledQuoteEscape() throws IOException {
        CsvFormatReader reader = new CsvFormatReader(blockFactory);
        // RFC 4180 doubled-quote escape inside a quoted field: each {@code ""} is a literal quote, not a
        // close-then-reopen. A scan that flipped quote state on every {@code "} would prematurely exit the
        // quoted region and report a boundary inside the field; verify the final \n is selected instead.
        byte[] data = "\"value with \"\"escaped\"\" quotes\"\nrest\n".getBytes(StandardCharsets.UTF_8);
        int boundary = reader.findLastRecordBoundary(data, data.length);
        assertEquals(data.length - 1, boundary);
        assertEquals('\n', data[boundary]);

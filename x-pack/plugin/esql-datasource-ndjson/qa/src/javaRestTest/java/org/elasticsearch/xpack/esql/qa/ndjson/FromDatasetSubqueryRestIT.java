...
/**
 * Parameterized REST-driven {@code FROM (FROM <dataset> | ...)} IT for the NDJSON format. Runs the
 * same canonical contract as {@code esql-datasource-csv}'s and {@code esql-datasource-parquet}'s
 * suites, but the registered dataset's underlying object is an NDJSON blob (one JSON object per
 * line) and is read through {@link org.elasticsearch.xpack.esql.datasource.ndjson.NdJsonFormatReader}.
 *
 * <p>NDJSON has no typed-header convention (unlike the CSV {@code name:type} prefix); types are
 * inferred from the JSON value primitives. The three-row payload below uses native JSON integers
 * for {@code emp_no} / {@code salary} and native JSON strings for the name fields so the inferred
 * ESQL schema matches {@code (integer, keyword, keyword, integer)} — the same shape every other
 * suite asserts against via {@link AbstractFromDatasetSubqueryRestTestCase#assertEmployeeRow}.
 */
...
public class FromDatasetSubqueryRestIT extends AbstractFromDatasetSubqueryRestTestCase {
    ...
    public void testSubqueryWithFilter() throws Exception {
        ...
        String blobKey = WAREHOUSE + "/standalone/from_dataset_subquery_ndjson_" + backend.name() + ".ndjson";

        backend.uploadBlob(blobKey, sampleNdjsonBytes());
        putDataSource(dataSource, backend.dataSourceType(), backend.dataSourceSettings());
        putDataset(dataset, dataSource, backend.resourceUri(blobKey), Map.of("format", "ndjson"));
    ...
    private static String dataSourceName(String backend) {
        return backend + "_ndjson_ds";
    }

    private static String datasetName(String backend) {
        return backend + "_ndjson_employees";
    }

    /**
     * Three-row employees payload as NDJSON. One JSON object per line, native JSON integers and
     * strings; the NDJSON reader infers the schema from the value types.
     */

...
/**
 * Parameterized REST-driven counterpart to {@code FromDatasetSubqueryIT} for the Parquet format.
 * Collapses the previous per-backend trio ({@code FromDatasetSubqueryS3RestIT},
 * {@code FromDatasetSubqueryGcsRestIT}, {@code FromDatasetSubqueryAzureRestIT}) into a single class
 * that runs the same {@code FROM (FROM <dataset> | WHERE ...)} contract against each of the three
 * production cloud-storage backends (S3, GCS, Azure) via the in-process HTTP fixtures. Payload is
 * the three-row employees Parquet blob produced in-JVM by {@link EmployeesParquetGenerator}.
 */
...
public class FromDatasetSubqueryRestIT extends AbstractFromDatasetSubqueryRestTestCase {
     ...
     * endpoint, register a Parquet dataset on top, then run {@code FROM (FROM <dataset> | WHERE ...)}
    ...
    public void testSubqueryWithFilter() throws Exception {
        ...
        String blobKey = WAREHOUSE + "/standalone/from_dataset_subquery_parquet_" + backend.name() + ".parquet";

        backend.uploadBlob(blobKey, EmployeesParquetGenerator.sampleEmployeesParquetBytes());
        putDataSource(dataSource, backend.dataSourceType(), backend.dataSourceSettings());
        // Format is inferable from the .parquet extension; passing it explicitly mirrors the CSV
        // variant for parity and keeps a single place to flip if the validator's accepted-fields
        // surface ever changes.
        putDataset(dataset, dataSource, backend.resourceUri(blobKey), Map.of("format", "parquet"));
    ...
    private static String dataSourceName(String backend) {
        return backend + "_parquet_ds";
    }

    private static String datasetName(String backend) {
        return backend + "_parquet_employees";
    }
}

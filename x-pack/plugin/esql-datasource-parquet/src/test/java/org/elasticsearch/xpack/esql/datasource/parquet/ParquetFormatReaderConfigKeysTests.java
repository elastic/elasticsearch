...
/** Pins that the Parquet factory claims no per-dataset configuration keys. */
public class ParquetFormatReaderConfigKeysTests extends ESTestCase {
    ...
    public void testInspectClaimsNothingIncludingRemovedKeys() {
        Map<String, Object> config = new HashMap<>();
        for (String key : RemovedParquetDatasetSettings.KEYS) {
            config.put(key, randomBoolean());
        }
        config.put("not_a_parquet_key", true);
        assertThat(new ParquetFormatReaderFactory().inspect(config).consumedKeys(), empty());
    }

    public void testEmptyConfigConsumesNothing() {
        assertThat(new ParquetFormatReaderFactory().inspect(Map.of()).consumedKeys(), empty());
    }

    public void testNullConfigConsumesNothing() {
        assertThat(new ParquetFormatReaderFactory().inspect(null).consumedKeys(), empty());
    }

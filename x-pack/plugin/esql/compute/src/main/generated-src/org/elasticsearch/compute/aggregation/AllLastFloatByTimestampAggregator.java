...
public class AllLastFloatByTimestampAggregator {
    ...
    public static AllLongFloatState initSingle(DriverContext driverContext) {
        return new AllLongFloatState(0, 0);
    }

    private static void first(AllLongFloatState current, long timestamp, float value, boolean v2Seen) {
        current.seen(true);
    ...
    public static void combine(AllLongFloatState current, @Position int position, FloatBlock value, LongBlock timestamp) {
        if (current.seen() == false) {
            // We never observed a value before so we'll take this right in, no questions asked.
            first(current, timestamp.getLong(position), value.getFloat(position), value.isNull(position) == false);
            return;
        }
        ...
        if (ts > current.v1()) {
            ...
            if (value.isNull(position) == false) {
                // non-null value
                current.v2(value.getFloat(position));
                current.v2Seen(true);
            } else {
                // null value
    ...
    public static void combineIntermediate(AllLongFloatState current, long timestamp, float value, boolean seen, boolean v2Seen) {
        if (seen) {
            if (current.seen()) {
    ...
    public static void combine(GroupingState current, int groupId, @Position int position, FloatBlock value, LongBlock timestamp) {
        boolean hasValue = value.isNull(position) == false;
        current.collectValue(groupId, timestamp.getLong(position), value.getFloat(position), hasValue);
    }
    ...
    public static void combineIntermediate(
        ...
        if (valueCount > 0) {
            ...
            for (int i = 0; i < valueCount; i++) {
                current.collectValue(groupId, timestamp, values.getFloat(firstIndex + i), hasValueFlag);
            }
        }
    }
    ...
    public static final class GroupingState extends AbstractArrayState {
        ...
        void collectValue(int groupId, long timestamp, float value, boolean hasVal) {

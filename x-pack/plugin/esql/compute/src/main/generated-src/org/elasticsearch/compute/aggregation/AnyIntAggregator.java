 ...
 */
@Aggregator({ @IntermediateState(name = "observed", type = "BOOLEAN"), @IntermediateState(name = "values", type = "INT_BLOCK") })
@GroupingAggregator({ @IntermediateState(name = "observed", type = "BOOLEAN"), @IntermediateState(name = "values", type = "INT_BLOCK") })
public class AnyIntAggregator {
    ...
    public static void combine(SingleState current, @Position int position, IntBlock values) {
        if (current.observed) {
    ...
    public static void combineIntermediate(SingleState current, boolean observed, IntBlock values) {
    ...
    private static void overrideState(SingleState current, IntBlock values, int position) {
    ...
    public static class SingleState implements AggregatorState {
        ...
        Block intermediateValuesBlockBuilder(DriverContext driverContext) {
            ...
            return driverContext.blockFactory().newIntArrayBlock(vals, 1, new int[] { 0, size }, null, Block.MvOrdering.UNORDERED);
        }
    ...
    public static void combine(GroupingState current, int group, @Position int position, IntBlock values) {
        current.collectValue(group, position, values);
    }

    public static void combineIntermediate(GroupingState current, int group, boolean observed, IntBlock values, int otherPosition) {
        if (observed == false) {
            return;
        }
    ...
    public static final class GroupingState extends AbstractArrayState {
        ...
        void collectValue(int group, int position, IntBlock valuesBlock) {

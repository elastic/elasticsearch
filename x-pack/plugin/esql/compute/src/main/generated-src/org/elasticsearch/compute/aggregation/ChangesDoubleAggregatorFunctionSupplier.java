...
final class ChangesDoubleGroupingAggregatorFunction extends AbstractRateGroupingFunction implements GroupingAggregatorFunction {
    ...
    private void addRawInput(int positionOffset, IntBlock groups, DoubleBlock valuesBlock, LongBlock timestampsBlock) {
        ...
        for (int groupPosition = 0; groupPosition < positionCount; groupPosition++) {
            ...
            double value = valuesBlock.getDouble(valuesBlock.getFirstValueIndex(valuePosition));
    ...
    private void addRawInput(int positionOffset, IntVector groups, DoubleBlock valuesBlock, LongBlock timestampsBlock) {
        int positionCount = groups.getPositionCount();
        for (int groupPosition = 0; groupPosition < positionCount; groupPosition++) {
            int valuePosition = groupPosition + positionOffset;
            if (valuesBlock.isNull(valuePosition) || timestampsBlock.isNull(valuePosition)) {
            ...
            appendRaw(
                ...
                valuesBlock.getDouble(valuesBlock.getFirstValueIndex(valuePosition))
            );
        }
    }

    private void addRawInput(int positionOffset, IntBlock groups, DoubleVector valuesVector, LongVector timestampsVector) {
        int positionCount = groups.getPositionCount();
        for (int groupPosition = 0; groupPosition < positionCount; groupPosition++) {
            if (groups.isNull(groupPosition)) {
                continue;
            ...
            int valuePosition = groupPosition + positionOffset;
            double value = valuesVector.getDouble(valuePosition);
    ...
    private void addRawInput(int positionOffset, IntVector groups, DoubleVector valuesVector, LongVector timestampsVector) {
        int positionCount = groups.getPositionCount();
    ...
    private void appendRawRange(int groupId, int from, int to, DoubleVector valuesVector, LongVector timestampsVector) {
    ...
    private void appendRaw(int groupId, long timestamp, double value) {
    ...
    private void addIntermediatePosition(int groupId, LongBlock timestamps, DoubleBlock values, LongBlock changes, int position) {
    ...
    private void evaluateIntermediate(Block[] blocks, int offset, IntVector selectedInPage) {
        ...
        try (
            ...
            DoubleBlock.Builder values = blockFactory.newDoubleBlockBuilder(positionCount);
    ...
    private void flushGroup(ReducedState state, DoubleRawBuffer buffer, FlushQueue flushQueue) {
        LongBuffer timestamps = buffer.timestamps;
        DoubleBuffer values = buffer.values;
        ...
        if (flushQueue.valueCount == 1) {
            ...
            double value = values.get(flushQueue.top().start);
        ...
        while (flushQueue.size() > 1) {
            ...
            double value = values.get(top.next());
            if (value != previous) {
                changes++;
            }
            previous = value;
    ...
    private final class ReducedState {
        ...
        void appendPoint(long timestamp, double value) {
        ...
        void appendInterval(long lastTs, double lastValue, long firstTs, double firstValue, long changes) {
            compactPointsToInterval();
        ...
        void appendFromBlocks(LongBlock timestamps, DoubleBlock values, LongBlock changes, int position) {
            ...
            if (changeCount == 1 && changes.getLong(firstChange) < 0) {
                ...
                for (int i = 0; i < timestampCount; i++) {
                    appendPoint(timestamps.getLong(firstTimestamp + i), values.getDouble(firstValue + i));
                }
                return;
            }
        ...
        void writeToBlocks(LongBlock.Builder timestamps, DoubleBlock.Builder values, LongBlock.Builder changes) {
            if (intervals.length == 0) {
                sortPoints();
                ...
                for (int point : points) {
                    ...
                    values.appendDouble(pointBuffer.value(point));
                }
    ...
    private static final class DoubleRawBuffer extends RawBuffer {
        ...
        void appendRange(int fromPosition, int toPosition, DoubleVector valuesVector, LongVector timestampsVector) {
    ...
    private static final class PointBuffer implements org.elasticsearch.core.Releasable {
        ...
        int append(long timestamp, double value) {
    ...
    private static final class IntervalBuffer implements org.elasticsearch.core.Releasable {
        ...
        int appendInterval(long lastTs, double lastValue, long firstTs, double firstValue, long changeCount) {
            int id = count();
        ...
        int appendIntervalsFromBlocks(LongBlock ts, DoubleBlock vs, LongBlock cs, int position) {
            ...
            for (int i = 0; i < intervalCount; i++) {
                ...
                values.append(vs.getDouble(valueFirst + 2 * i));
                timestamps.append(ts.getLong(timestampFirst + 2 * i + 1));
                values.append(vs.getDouble(valueFirst + 2 * i + 1));

...
final class ChangesLongGroupingAggregatorFunction extends AbstractRateGroupingFunction implements GroupingAggregatorFunction {
    ...
    private void addRawInput(int positionOffset, IntBlock groups, LongBlock values, LongBlock timestamps) {
        for (int p = 0; p < groups.getPositionCount(); p++) {
            ...
            long value = values.getLong(values.getFirstValueIndex(valuePosition));
    ...
    private void addRawInput(int positionOffset, IntVector groups, LongBlock values, LongBlock timestamps) {
        for (int p = 0; p < groups.getPositionCount(); p++) {
            ...
            getOrInitializeState(groups.getInt(p)).append(
                ...
                values.getLong(values.getFirstValueIndex(valuePosition))
            );
        }
    }
    ...
    private void addRawInput(int positionOffset, IntBlock groups, LongVector values, LongVector timestamps) {
        for (int p = 0; p < groups.getPositionCount(); p++) {
            ...
            int valuePosition = positionOffset + p;
            long value = values.getLong(valuePosition);
    ...
    private void addRawInput(int positionOffset, IntVector groups, LongVector values, LongVector timestamps) {
        for (int p = 0; p < groups.getPositionCount(); p++) {
            int valuePosition = positionOffset + p;
            getOrInitializeState(groups.getInt(p)).append(timestamps.getLong(valuePosition), values.getLong(valuePosition));
        }
    }
    ...
    private void addIntermediatePosition(int groupId, LongBlock timestamps, LongBlock values, int position) {
        ...
        for (int i = 0; i < count; i++) {
            state.append(timestamps.getLong(firstTimestamp + i), values.getLong(firstValue + i));
        }
    }
    ...
    private void evaluateIntermediate(Block[] blocks, int offset, IntVector selectedInPage) {
        ...
        try (
            ...
            LongBlock.Builder values = driverContext.blockFactory().newLongBlockBuilder(positionCount)
        ) {
            for (int p = 0; p < positionCount; p++) {
                ...
                if (state == null) {
                    ...
                    for (int point = state.head; point >= 0; point = points.next(point)) {
                        ...
                        values.appendLong(points.value(point));
                    }
    ...
    private static final class PointBuffer implements org.elasticsearch.core.Releasable {
        ...
        int append(long timestamp, long value, int nextPoint) {

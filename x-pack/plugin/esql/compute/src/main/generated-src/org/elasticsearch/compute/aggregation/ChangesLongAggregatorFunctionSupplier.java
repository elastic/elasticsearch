...
/**
 * {@link AggregatorFunctionSupplier} implementation for PromQL changes over long values.
 * This class is generated. Edit {@code X-ChangesAggregatorFunctionSupplier.java.st} instead.
 */
public final class ChangesLongAggregatorFunctionSupplier implements AggregatorFunctionSupplier {
    @Override
    ...
    public List<IntermediateStateDesc> groupingIntermediateStateDesc() {
        return ChangesLongGroupingAggregatorFunction.intermediateStateDesc();
    }
    ...
    public GroupingAggregatorFunction groupingAggregator(DriverContext driverContext, List<Integer> channels) {
        return new ChangesLongGroupingAggregatorFunction(channels, driverContext);
    }

    @Override
    public String describe() {
        return "changes of longs";
    }
}

final class ChangesLongGroupingAggregatorFunction extends AbstractRateGroupingFunction implements GroupingAggregatorFunction {
    ...
    ChangesLongGroupingAggregatorFunction(List<Integer> channels, DriverContext driverContext) {
    ...
    private void addRawInput(int positionOffset, IntBlock groups, LongBlock valuesBlock, LongBlock timestampsBlock) {
        ...
        for (int groupPosition = 0; groupPosition < positionCount; groupPosition++) {
            ...
            long value = valuesBlock.getLong(valuesBlock.getFirstValueIndex(valuePosition));
    ...
    private void addRawInput(int positionOffset, IntVector groups, LongBlock valuesBlock, LongBlock timestampsBlock) {
        int positionCount = groups.getPositionCount();
        for (int groupPosition = 0; groupPosition < positionCount; groupPosition++) {
            int valuePosition = groupPosition + positionOffset;
            if (valuesBlock.isNull(valuePosition) || timestampsBlock.isNull(valuePosition)) {
            ...
            appendRaw(
                ...
                valuesBlock.getLong(valuesBlock.getFirstValueIndex(valuePosition))
            );
        }
    }

    private void addRawInput(int positionOffset, IntBlock groups, LongVector valuesVector, LongVector timestampsVector) {
        int positionCount = groups.getPositionCount();
        for (int groupPosition = 0; groupPosition < positionCount; groupPosition++) {
            if (groups.isNull(groupPosition)) {
                continue;
            ...
            int valuePosition = groupPosition + positionOffset;
            long value = valuesVector.getLong(valuePosition);
    ...
    private void addRawInput(int positionOffset, IntVector groups, LongVector valuesVector, LongVector timestampsVector) {
        int positionCount = groups.getPositionCount();
    ...
    private void appendRawRange(int groupId, int from, int to, LongVector valuesVector, LongVector timestampsVector) {
    ...
    private void appendRaw(int groupId, long timestamp, long value) {
    ...
    private void addIntermediatePosition(int groupId, LongBlock timestamps, LongBlock values, LongBlock changes, int position) {
    ...
    private void evaluateIntermediate(Block[] blocks, int offset, IntVector selectedInPage) {
        ...
        try (
            ...
            LongBlock.Builder values = blockFactory.newLongBlockBuilder(positionCount);
    ...
    private void flushGroup(ReducedState state, LongRawBuffer buffer, FlushQueue flushQueue) {
        LongBuffer timestamps = buffer.timestamps;
        LongBuffer values = buffer.values;
        ...
        if (flushQueue.valueCount == 1) {
            ...
            long value = values.get(flushQueue.top().start);
        ...
        while (flushQueue.size() > 1) {
            ...
            long value = values.get(top.next());
            if (value != previous) {
                changes++;
            }
            previous = value;
    ...
    private final class ReducedState {
        ...
        void appendPoint(long timestamp, long value) {
        ...
        void appendInterval(long lastTs, long lastValue, long firstTs, long firstValue, long changes) {
            compactPointsToInterval();
        ...
        void appendFromBlocks(LongBlock timestamps, LongBlock values, LongBlock changes, int position) {
            ...
            if (changeCount == 1 && changes.getLong(firstChange) < 0) {
                ...
                for (int i = 0; i < timestampCount; i++) {
                    appendPoint(timestamps.getLong(firstTimestamp + i), values.getLong(firstValue + i));
                }
                return;
            }
        ...
        void writeToBlocks(LongBlock.Builder timestamps, LongBlock.Builder values, LongBlock.Builder changes) {
            if (intervals.length == 0) {
                sortPoints();
                ...
                for (int point : points) {
                    ...
                    values.appendLong(pointBuffer.value(point));
                }
    ...
    private static final class LongRawBuffer extends RawBuffer {
        private final LongBuffer values;

        LongRawBuffer(org.elasticsearch.common.breaker.CircuitBreaker breaker) {
            super(breaker);
        ...
        void appendRange(int fromPosition, int toPosition, LongVector valuesVector, LongVector timestampsVector) {
    ...
    private static final class PointBuffer implements org.elasticsearch.core.Releasable {
        ...
        PointBuffer(org.elasticsearch.common.breaker.CircuitBreaker breaker) {
            ...
            try {
                ...
                values = new LongBuffer(breaker, PAGE_SIZE);
                success = true;
            } finally {
                if (success == false) {
        ...
        int append(long timestamp, long value) {
    ...
    private static final class IntervalBuffer implements org.elasticsearch.core.Releasable {
        ...
        int appendInterval(long lastTs, long lastValue, long firstTs, long firstValue, long changeCount) {
            int id = count();
        ...
        int appendIntervalsFromBlocks(LongBlock ts, LongBlock vs, LongBlock cs, int position) {
            ...
            for (int i = 0; i < intervalCount; i++) {
                ...
                values.append(vs.getLong(valueFirst + 2 * i));
                timestamps.append(ts.getLong(timestampFirst + 2 * i + 1));
                values.append(vs.getLong(valueFirst + 2 * i + 1));

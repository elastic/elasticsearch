...
public class DerivLongGroupingAggregatorFunction implements GroupingAggregatorFunction {

    private static final List<IntermediateStateDesc> INTERMEDIATE_STATE_DESC = List.of(
        new IntermediateStateDesc("sumVal", ElementType.LONG),
        new IntermediateStateDesc("sumTs", ElementType.LONG),
        new IntermediateStateDesc("sumTsVal", ElementType.LONG),
    ...
    public DerivLongGroupingAggregatorFunction(List<Integer> channels, DriverContext driverContext) {
    ...
    public AddInput prepareProcessRawInputPage(SeenGroupIds seenGroupIds, Page page) {
        final LongBlock valueBlock = page.getBlock(channels.get(0));
        final LongBlock timestampBlock = page.getBlock(channels.get(1));
        final LongVector valueVector = valueBlock.asVector();
        return new AddInput() {
            @Override
            ...
            public void add(int positionOffset, IntVector groupIds) {
                for (int groupPosition = 0; groupPosition < groupIds.getPositionCount(); groupPosition++) {
                    ...
                    long vValue = valueVector.getLong(valuesPosition);
            ...
            private void addBlockInput(int positionOffset, IntBlock groupIds, LongBlock timestampBlock, LongVector valueVector) {
                for (int groupPosition = 0; groupPosition < groupIds.getPositionCount(); groupPosition++) {
                    ...
                    for (int g = groupStart; g < groupEnd; g++) {
                        ...
                        for (int v = vStart; v < vEnd; v++) {
                            ...
                            long val = valueVector.getLong(valuePosition);
                            SimpleLinearRegressionWithTimeseries state = states.get(groupId);
                            if (state == null) {
                                state = new SimpleLinearRegressionWithTimeseries();
                                states.set(groupId, state);
    ...
    private void addIntermediateBlockInput(int positionOffset, IntBlock groupIdVector, Page page) {
        LongBlock sumValBlock = page.getBlock(channels.get(0));
        LongBlock sumTsBlock = page.getBlock(channels.get(1));
        LongBlock sumTsValBlock = page.getBlock(channels.get(2));
        ...
        for (int groupPos = 0; groupPos < groupIdVector.getPositionCount(); groupPos++) {
            ...
            for (int g = firstGroup; g < lastGroup; g++) {
                ...
                long sumVal = sumValBlock.getLong(valuePos);
                long sumTsVal = sumTsValBlock.getLong(valuePos);
    ...
    public void evaluateIntermediate(Block[] blocks, int offset, IntVector selected) {
        ...
        try (
            var sumValBuilder = blockFactory.newLongBlockBuilder(positionCount);
            var sumTsBuilder = blockFactory.newLongBlockBuilder(positionCount);
            var sumTsValBuilder = blockFactory.newLongBlockBuilder(positionCount);
            ...
            for (int p = 0; p < positionCount; p++) {
                ...
                if (state == null) {
                    ...
                    sumValBuilder.appendLong((long) state.sumVal);
                    sumTsBuilder.appendLong(state.sumTs);
                    sumTsValBuilder.appendLong((long) state.sumTsVal); // TODO: fix this actually

...
public class IrateIntAggregator {
    ...
    public static final class IntIrateGroupingState implements Releasable, Accountable, GroupingAggregatorState {
        ...
        void append(int groupId, long timestamp, int value) {
            ...
            if (state == null) {
                adjustBreaker(IntIrateState.bytesUsed(1));
                state = new IntIrateState(new long[] { timestamp }, new int[] { value });
                states.set(groupId, state);
            } else {
                if (state.entries() == 1) {
                    adjustBreaker(IntIrateState.bytesUsed(2));
                    state = new IntIrateState(new long[] { state.timestamps[0], timestamp }, new int[] { state.values[0], value });
                    states.set(groupId, state);
                    adjustBreaker(-IntIrateState.bytesUsed(1)); // old state
                }
            }
        }
        ...
        void combine(int groupId, LongBlock timestamps, IntBlock values, int otherPosition) {
            ...
            var state = states.get(groupId);
            if (state == null) {
                adjustBreaker(IntIrateState.bytesUsed(valueCount));
                state = new IntIrateState(valueCount);
                states.set(groupId, state);
                ...
                adjustBreaker(IntIrateState.bytesUsed(state.entries() + valueCount));
                var newState = new IntIrateState(state.entries() + valueCount);
                states.set(groupId, newState);
                merge(state, newState, firstIndex, valueCount, timestamps, values);
                adjustBreaker(-IntIrateState.bytesUsed(state.entries())); // old state
            }
        }

        void merge(IntIrateState curr, IntIrateState dst, int firstIndex, int rightCount, LongBlock timestamps, IntBlock values) {
            int i = 0, j = 0, k = 0;
        ...
        IntIrateState mergeState(IntIrateState s1, IntIrateState s2) {
            adjustBreaker(IntIrateState.bytesUsed(2));
            var dst = new IntIrateState(2);
            int i = 0, j = 0, k = 0;

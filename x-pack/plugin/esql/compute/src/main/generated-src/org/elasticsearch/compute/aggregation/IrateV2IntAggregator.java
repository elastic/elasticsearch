...
public class IrateV2IntAggregator {
    ...
    public static final class IntIrateGroupingState implements Releasable, Accountable, GroupingAggregatorState {
        ...
        void combine(
            ...
            ensureCapacity(groupId);
            append(groupId, timestamps.getLong(firstTs), values.getInt(firstIndex), cumulative);
            if (valueCount > 1) {
                ensureCapacity(groupId);
                append(groupId, timestamps.getLong(firstTs + 1), values.getInt(firstIndex + 1), cumulative);
            }
        }

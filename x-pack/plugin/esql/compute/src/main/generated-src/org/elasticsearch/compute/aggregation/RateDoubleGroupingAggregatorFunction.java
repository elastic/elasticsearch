...
public final class RateDoubleGroupingAggregatorFunction extends AbstractRateGroupingFunction implements GroupingAggregatorFunction {
    ...
    static final class IntervalBuffer implements Releasable {
        ...
        private final DoubleBuffer lastValueBuf;
        private final LongBuffer firstTsBuf;
        private final DoubleBuffer firstValueBuf;

        private int count = 0;

        IntervalBuffer(CircuitBreaker cb) {
            LongBuffer lastTsBuf = null;
            DoubleBuffer lastValueBuf = null;
            LongBuffer firstTsBuf = null;
            DoubleBuffer firstValueBuf = null;
            boolean success = false;
            try {
                lastTsBuf = new LongBuffer(cb, PAGE_SIZE);
                lastValueBuf = new DoubleBuffer(cb, PAGE_SIZE);
                firstTsBuf = new LongBuffer(cb, PAGE_SIZE);
                firstValueBuf = new DoubleBuffer(cb, PAGE_SIZE);
                success = true;
            } finally {
                if (success == false) {
        ...
        int appendIntervalsFromBlocks(LongBlock ts, DoubleBlock vs, int position) {
            ...
            for (int i = 0; i < valueCount; i += 2) {
                ...
                lastValueBuf.set(count, vs.getDouble(vsFirst + i));
                firstTsBuf.set(count, ts.getLong(tsFirst + i + 1));
                firstValueBuf.set(count, vs.getDouble(vsFirst + i + 1));
                count++;
            }
            return firstId;
        }
    ...
    static final class ReducedState {
        ...
        void appendInterval(IntervalBuffer buffer, long lastTs, double lastValue, long firstTs, double firstValue) {
        ...
        void appendIntervalsFromBlocks(IntervalBuffer buffer, LongBlock ts, DoubleBlock vs, int position) {
            assert hasDelta() == false : "cannot append intervals while delta data is pending";

...
public final class RateLongGroupingAggregatorFunction extends AbstractRateGroupingFunction implements GroupingAggregatorFunction {
    ...
    private double computeRate(
        ...
        if (lastTsSec == firstTsSec) {
            // Check for the case where there is only one sample in state, right at the lower boundary of the time bucket towards a
            // non-empty adjacent state.
            // In this case we want to have a result value as the time bucket is not empty, but we already included the increase in the

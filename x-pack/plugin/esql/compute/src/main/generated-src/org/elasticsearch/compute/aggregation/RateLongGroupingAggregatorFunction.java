...
public final class RateLongGroupingAggregatorFunction extends AbstractRateGroupingFunction implements GroupingAggregatorFunction {
            ...
            for (int p = 0; p < positionCount; p++) {
                ...
                if (state != null && state.samples > 0) {
                    ...
                    for (Interval interval : state.intervals) {
                        timestamps.appendLong(interval.lastTs);
                        timestamps.appendLong(interval.firstTs);
                        values.appendLong(interval.lastV);
                        values.appendLong(interval.firstV);
                    }
            ...
            for (int p = 0; p < positionCount; p++) {
                ...
                if (state != null && state.samples > 1 && state.intervals.length > 1) {
                    ...
                    for (int i = 1; i < intervals.length; i++) {
                        ...
                        if (prev.lastV > next.firstV) {
                            state.resets += prev.lastV;
                        }
                    }
                }
            }
    ...
    record Interval(long lastTs, long lastV, long firstTs, long firstV) implements Comparable<Interval> {
        @Override
    ...
    private double computeRateWithoutExtrapolate(ReducedState state, boolean isRateOverTime, double dateFactor) {
        ...
        final long firstTS = state.intervals[state.intervals.length - 1].firstTs;
        final long lastTS = state.intervals[0].lastTs;
        double firstValue = state.intervals[state.intervals.length - 1].firstV;
        double lastValue = state.intervals[0].lastV + state.resets;
        if (isRateOverTime) {
    ...
    private double computeRate(
        ...
        if (previousState == null || previousState.samples == 0) {
            if (state.samples == 1) {
                ...
                firstValue = state.intervals[0].lastV;
            } else {
        ...
        if (nextState == null || nextState.samples == 0) {
            if (state.samples == 1) {
                ...
                lastValue = state.intervals[0].lastV;
            } else {
        ...
        if (lastTsSec == firstTsSec) {
            ...
            if (state.samples == 1) {
                if (previousState != null) {
                    ...
                    assert state.intervals[0].lastTs == firstTsSec * dateFactor : firstTsSec + ":" + state.intervals[0].lastTs;
                    final double startTs = previousState.intervals[0].lastTs / dateFactor;
                    final double delta = deltaBetweenStates(previousState, state, dateFactor);
                    return isRateOverTime ? delta / (firstTsSec - startTs) : delta;
                }
                if (nextState != null) {
                    assert state.intervals[0].lastTs == lastTsSec * dateFactor : lastTsSec + ":" + state.intervals[0].lastTs;
                    final double endTs = nextState.intervals[nextState.intervals.length - 1].firstTs / dateFactor;
    ...
    private static double extrapolateToBoundary(
        ...
        final double startTs = state.intervals[state.intervals.length - 1].firstTs / dateFactor;
        final double startValue = state.intervals[state.intervals.length - 1].firstV;
        final double endTs = state.intervals[0].lastTs / dateFactor;
        final double endValue = state.intervals[0].lastV + state.resets;
    ...
    private static double interpolateBetweenStates(
        ...
        final double startValue = lowerState.intervals[0].lastV;
        final double startTs = lowerState.intervals[0].lastTs / dateFactor;
        final double endValue = upperState.intervals[upperState.intervals.length - 1].firstV;
        final double endTs = upperState.intervals[upperState.intervals.length - 1].firstTs / dateFactor;

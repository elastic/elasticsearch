 ...
 * of this file.
 */
@Aggregator({ @IntermediateState(name = "values", type = "FLOAT_BLOCK"), @IntermediateState(name = "timestamps", type = "LONG_BLOCK") })
@GroupingAggregator
class SparklineFloatAggregator {
    public static SingleState initSingle(BigArrays bigArrays) {
        return new SingleState(bigArrays);
    }

    public static void combine(SingleState state, float value, long timestamp) {
        state.values.add(Tuple.tuple(timestamp, value));
    }

    public static void combineIntermediate(SingleState state, FloatBlock values, LongBlock timestamps) {
        int start = values.getFirstValueIndex(0);
        int end = start + values.getValueCount(0);
        for (int i = start; i < end; i++) {
            combine(state, values.getFloat(i), timestamps.getLong(i));
        }
    }
    ...
    public static void combine(GroupingState state, int groupId, float value, long timestamp) {
        state.addValue(groupId, value, timestamp);
    }

    public static void combineIntermediate(GroupingState state, int groupId, FloatBlock values, LongBlock timestamps, int valuesPosition) {
        int start = values.getFirstValueIndex(valuesPosition);
        int end = start + values.getValueCount(valuesPosition);
        for (int i = start; i < end; i++) {
            combine(state, groupId, values.getFloat(i), timestamps.getLong(i));
        }
    }
    ...
    public static class SingleState implements AggregatorState {
        ...
        public void toIntermediate(Block[] blocks, int offset, DriverContext driverContext) {
            ...
            try (
                FloatBlock.Builder builder = blockFactory.newFloatBlockBuilder(values.size());
                ...
                for (int id = 0; id < values.size(); id++) {
                    builder.appendFloat(values.get(id).v2());
        ...
        Block toFinal(BlockFactory blockFactory) {
            ...
            try (FloatBlock.Builder builder = blockFactory.newFloatBlockBuilder(values.size())) {
                builder.beginPositionEntry();
                for (int id = 0; id < values.size(); id++) {
                    builder.appendFloat(values.get(id).v2());
                }
    ...
    public static class GroupingState implements GroupingAggregatorState {
        ...
        private final Map<Integer, List<Tuple<Long, Float>>> values;
        ...
        public void toIntermediate(Block[] blocks, int offset, IntVector selected, DriverContext driverContext) {
            ...
            try (
                FloatBlock.Builder builder = blockFactory.newFloatBlockBuilder(selected.getPositionCount());
                ...
                for (int s = 0; s < selected.getPositionCount(); s++) {
                    ...
                    builder.beginPositionEntry();
                    timestampsBuilder.beginPositionEntry();
                    List<Tuple<Long, Float>> valuesInGroup = values.get(group);
                    for (int id = 0; id < valuesInGroup.size(); id++) {
                        builder.appendFloat(valuesInGroup.get(id).v2());
        ...
        void addValue(int groupId, float v, long t) {
            ...
            List<Tuple<Long, Float>> valuesInGroup = values.computeIfAbsent(groupId, g -> new ArrayList<>());

 ...
 * of this file.
 */
@Aggregator({ @IntermediateState(name = "values", type = "INT_BLOCK"), @IntermediateState(name = "timestamps", type = "LONG_BLOCK") })
@GroupingAggregator
class SparklineIntAggregator {
    public static SingleState initSingle(BigArrays bigArrays) {
        return new SingleState(bigArrays);
    }

    public static void combine(SingleState state, int value, long timestamp) {
        state.values.add(Tuple.tuple(timestamp, value));
    }

    public static void combineIntermediate(SingleState state, IntBlock values, LongBlock timestamps) {
        int start = values.getFirstValueIndex(0);
        int end = start + values.getValueCount(0);
        for (int i = start; i < end; i++) {
            combine(state, values.getInt(i), timestamps.getLong(i));
        }
    }
    ...
    public static void combine(GroupingState state, int groupId, int value, long timestamp) {
        state.addValue(groupId, value, timestamp);
    }

    public static void combineIntermediate(GroupingState state, int groupId, IntBlock values, LongBlock timestamps, int valuesPosition) {
        int start = values.getFirstValueIndex(valuesPosition);
        int end = start + values.getValueCount(valuesPosition);
        for (int i = start; i < end; i++) {
            combine(state, groupId, values.getInt(i), timestamps.getLong(i));
        }
    }
    ...
    public static class SingleState implements AggregatorState {
        ...
        public void toIntermediate(Block[] blocks, int offset, DriverContext driverContext) {
            ...
            try (
                IntBlock.Builder builder = blockFactory.newIntBlockBuilder(values.size());
                ...
                for (int id = 0; id < values.size(); id++) {
                    builder.appendInt(values.get(id).v2());
        ...
        Block toFinal(BlockFactory blockFactory) {
            ...
            try (IntBlock.Builder builder = blockFactory.newIntBlockBuilder(values.size())) {
                builder.beginPositionEntry();
                for (int id = 0; id < values.size(); id++) {
                    builder.appendInt(values.get(id).v2());
                }
    ...
    public static class GroupingState implements GroupingAggregatorState {
        ...
        private final Map<Integer, List<Tuple<Long, Integer>>> values;
        ...
        public void toIntermediate(Block[] blocks, int offset, IntVector selected, DriverContext driverContext) {
            ...
            try (
                IntBlock.Builder builder = blockFactory.newIntBlockBuilder(selected.getPositionCount());
                ...
                for (int s = 0; s < selected.getPositionCount(); s++) {
                    ...
                    builder.beginPositionEntry();
                    timestampsBuilder.beginPositionEntry();
                    List<Tuple<Long, Integer>> valuesInGroup = values.get(group);
                    for (int id = 0; id < valuesInGroup.size(); id++) {
                        builder.appendInt(valuesInGroup.get(id).v2());
        ...
        void addValue(int groupId, int v, long t) {
            ...
            List<Tuple<Long, Integer>> valuesInGroup = values.computeIfAbsent(groupId, g -> new ArrayList<>());

...
import org.elasticsearch.compute.data.LongBlock;
import org.elasticsearch.compute.data.sort.LongIndirectBucketedSort;
 ...
 * of this file.
 */
@Aggregator({ @IntermediateState(name = "values", type = "LONG_BLOCK"), @IntermediateState(name = "timestamps", type = "LONG_BLOCK") })
@GroupingAggregator
class SparklineLongAggregator {

    private static class TimestampValueComparator implements Comparator<Tuple<Long, Long>> {
        ...
        public int compare(Tuple<Long, Long> lhs, Tuple<Long, Long> rhs) {
            ...
            return order.reverseMul() * Long.compare(lhs.v2(), rhs.v2());
        }
    }
    ...
    public static void combine(SingleState state, long value, long timestamp) {
        state.add(timestamp, value);
    }

    public static void combineIntermediate(SingleState state, LongBlock values, LongBlock timestamps) {
        int start = values.getFirstValueIndex(0);
        int end = start + values.getValueCount(0);
        for (int i = start; i < end; i++) {
            combine(state, values.getLong(i), timestamps.getLong(i));
        }
    }
    ...
    public static void combine(GroupingState state, int groupId, long value, long timestamp) {
        state.add(groupId, timestamp, value);
    }

    public static void combineIntermediate(GroupingState state, int groupId, LongBlock values, LongBlock timestamps, int valuesPosition) {
        int start = values.getFirstValueIndex(valuesPosition);
        int end = start + values.getValueCount(valuesPosition);
        for (int i = start; i < end; i++) {
            combine(state, groupId, values.getLong(i), timestamps.getLong(i));
        }
    }
    ...
    public static class GroupingState implements GroupingAggregatorState {
        private final LongIndirectBucketedSort sort;

        private GroupingState(BigArrays bigArrays, int limit, boolean ascending) {
            Comparator<Tuple<Long, Long>> comparator = Comparator.<Tuple<Long, Long>>comparingLong(Tuple::v1).thenComparing(Tuple::v2);
            this.sort = new LongIndirectBucketedSort(bigArrays, comparator, limit);
        }

        public void add(int groupId, long sortValue, long indirectValue) {
    ...
    public static class SingleState implements AggregatorState {
        ...
        public void add(long sortValue, long indirectValue) {

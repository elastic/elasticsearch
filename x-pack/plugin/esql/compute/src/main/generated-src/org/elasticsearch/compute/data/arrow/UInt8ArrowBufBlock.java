...
public final class UInt8ArrowBufBlock extends AbstractArrowBufBlock<IntVector, IntBlock> implements IntBlock {
     ...
     * fall through to the zero-copy {@link UInt8ArrowBufBlock} path. Multi-valued (List)
    ...
    private static IntBlock tryConstant(ValueVector arrowVector, BlockFactory blockFactory) {
        ...
        ArrowUtils.checkItemSize((org.apache.arrow.vector.FixedWidthVector) arrowVector, Byte.BYTES);
        ...
        if (ArrowBufConstantDetection.isUniform(valueBuffer, rowCount, Byte.BYTES) == false) {
            return null;
        }
        int valueIndex = 0;
        int value = Byte.toUnsignedInt(valueBuffer.getByte(valueIndex));

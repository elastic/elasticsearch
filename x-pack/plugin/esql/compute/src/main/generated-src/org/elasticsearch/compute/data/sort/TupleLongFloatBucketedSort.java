...
/**
 * Aggregates the top N {@code Tuple<Long, Float>} values per bucket.
 * See {@link BucketedSort} for more information.
 * This class is generated. Edit @{code X-BucketedSort.java.st} instead of this file.
 */
public class TupleLongFloatBucketedSort implements Releasable {
     ...
     * </ul>
     */
    private ObjectArray<Tuple<Long, Float>> values;

    public TupleLongFloatBucketedSort(BigArrays bigArrays, SortOrder order, int bucketSize) {
     ...
     * </p>
     */
    public void collect(Tuple<Long, Float> value, int bucket) {
     ...
     */
    public void merge(int groupId, TupleLongFloatBucketedSort other, int otherGroupId) {
        ...
        try (
            ...
            for (int s = 0; s < selected.getPositionCount(); s++) {
                ...
                if (size == 1) {
                    ...
                    builder.appendFloat(values.get(rootIndex).v2());
                    continue;
                }
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    builder.appendFloat(values.get(rootIndex + i).v2());
                }
    ...
    private void setNextGatherOffset(long rootIndex, int offset) {
        values.set(rootIndex, Tuple.<Long, Float>tuple((long) offset, null));
    }
     ...
     */
    private boolean betterThan(Tuple<Long, Float> lhs, Tuple<Long, Float> rhs) {
        int res = Long.compare(lhs.v1(), rhs.v1());
        if (res != 0) {
            return getOrder().reverseMul() * res < 0;
        }
        res = Float.compare(lhs.v2(), rhs.v2());
        return getOrder().reverseMul() * res < 0;
    }
    ...
    private void grow(int bucket) {
        ...
        long newSize = BigArrays.overSize(((long) bucket + 1) * bucketSize, PageCacheRecycler.OBJECT_PAGE_SIZE, Long.BYTES + Float.BYTES);

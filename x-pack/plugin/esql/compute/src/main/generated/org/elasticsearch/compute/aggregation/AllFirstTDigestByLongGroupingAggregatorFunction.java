...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link AllFirstTDigestByLongAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class AllFirstTDigestByLongGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  AllFirstTDigestByLongGroupingAggregatorFunction(List<Integer> channels,
      DriverContext driverContext) {
    this.channels = channels;
    this.state = AllFirstTDigestByLongAggregator.initGrouping(driverContext);
  ...
  public GroupingAggregatorFunction.AddInput prepareProcessRawInputPage(SeenGroupIds seenGroupIds,
    ...
    if (sortKeyVector == null) {
      maybeEnableGroupIdTracking(seenGroupIds, tdigestBlock, sortKeyBlock);
      return new GroupingAggregatorFunction.AddInput() {
        @Override
        public void add(int positionOffset, IntArrayBlock groupIds) {
          addRawInput(positionOffset, groupIds, tdigestBlock, sortKeyBlock);
        }
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int tdigestOffset = tdigestStart; tdigestOffset < tdigestEnd; tdigestOffset++) {
          ...
          for (int sortKeyOffset = sortKeyStart; sortKeyOffset < sortKeyEnd; sortKeyOffset++) {
            ...
            AllFirstTDigestByLongAggregator.combine(state, groupId, tdigestValue, sortKeyValue);
          }
        }
      }
    }
  ...
  public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        int valuesPosition = groupPosition + positionOffset;
        AllFirstTDigestByLongAggregator.combineIntermediate(state, groupId, sortKeys.getLong(valuesPosition), values, seen.getBoolean(valuesPosition), valuesPosition);
      }
    }
  }
  ...
  private void evaluateFinal(Block[] blocks, int offset, IntVector selectedInPage,
      GroupingAggregatorEvaluationContext ctx) {
    blocks[offset] = AllFirstTDigestByLongAggregator.evaluateFinal(state, selectedInPage, ctx);
  }

...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link AllLastTDigestByIntAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class AllLastTDigestByIntGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  AllLastTDigestByIntGroupingAggregatorFunction(List<Integer> channels,
      DriverContext driverContext) {
    this.channels = channels;
    this.state = AllLastTDigestByIntAggregator.initGrouping(driverContext);
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int valueOffset = valueStart; valueOffset < valueEnd; valueOffset++) {
          ...
          for (int sortKeyOffset = sortKeyStart; sortKeyOffset < sortKeyEnd; sortKeyOffset++) {
            ...
            AllLastTDigestByIntAggregator.combine(state, groupId, valueValue, sortKeyValue);
          }
        }
      }
    }
  ...
  public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        int valuesPosition = groupPosition + positionOffset;
        AllLastTDigestByIntAggregator.combineIntermediate(state, groupId, sortKeys.getLong(valuesPosition), values, seen.getBoolean(valuesPosition), valuesPosition);
      }
    }
  }
  ...
  private void evaluateFinal(Block[] blocks, int offset, IntVector selectedInPage,
      GroupingAggregatorEvaluationContext ctx) {
    blocks[offset] = AllLastTDigestByIntAggregator.evaluateFinal(state, selectedInPage, ctx);
  }

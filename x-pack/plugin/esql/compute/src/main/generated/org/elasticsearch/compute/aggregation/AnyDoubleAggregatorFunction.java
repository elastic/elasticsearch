...
/**
 * {@link AggregatorFunction} implementation for {@link AnyDoubleAggregator}.
 * This class is generated. Edit {@code AggregatorImplementer} instead.
 */
public final class AnyDoubleAggregatorFunction implements AggregatorFunction {
  ...
  private final AnyDoubleAggregator.SingleState state;

  private final List<ExpressionEvaluator> inputs;

  AnyDoubleAggregatorFunction(DriverContext driverContext, List<ExpressionEvaluator> inputs) {
  ...
  private void addRawBlock(DoubleBlock valuesBlock) {
    for (int p = 0; p < valuesBlock.getPositionCount(); p++) {
      AnyDoubleAggregator.combine(state, p, valuesBlock);
    }
  }
  ...
  public void addIntermediateInput(Page page) {
    ...
    try (
      ...
      AnyDoubleAggregator.combineIntermediate(state, observed.getBoolean(0), values);
    }
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, DriverContext driverContext) {
    blocks[offset] = AnyDoubleAggregator.evaluateFinal(state, driverContext);
  }

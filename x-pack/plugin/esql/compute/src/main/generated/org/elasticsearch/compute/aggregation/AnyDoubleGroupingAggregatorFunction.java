...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link AnyDoubleAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class AnyDoubleGroupingAggregatorFunction implements GroupingAggregatorFunction {
  private static final List<IntermediateStateDesc> INTERMEDIATE_STATE_DESC = List.of(
      new IntermediateStateDesc("observed", ElementType.BOOLEAN),
      new IntermediateStateDesc("values", ElementType.DOUBLE)  );

  private final AnyDoubleAggregator.GroupingState state;
  ...
  private void addRawInput(int positionOffset, IntArrayBlock groups, DoubleBlock valuesBlock) {
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        AnyDoubleAggregator.combine(state, groupId, valuesPosition, valuesBlock);
      }
    }
  }
  ...
  public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        int valuesPosition = groupPosition + positionOffset;
        AnyDoubleAggregator.combineIntermediate(state, groupId, observed.getBoolean(valuesPosition), values, valuesPosition);
      }
    }
  }
  ...
  private void evaluateFinal(Block[] blocks, int offset, IntVector selectedInPage,
      GroupingAggregatorEvaluationContext ctx) {
    blocks[offset] = AnyDoubleAggregator.evaluateFinal(state, selectedInPage, ctx);
  }

...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link AnyExponentialHistogramAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class AnyExponentialHistogramGroupingAggregatorFunction implements GroupingAggregatorFunction {
  private static final List<IntermediateStateDesc> INTERMEDIATE_STATE_DESC = List.of(
      new IntermediateStateDesc("values", ElementType.EXPONENTIAL_HISTOGRAM),
      new IntermediateStateDesc("seen", ElementType.BOOLEAN)  );

  private final ExponentialHistogramStates.SeenGroupingState state;

  private final List<Integer> channels;

  private final DriverContext driverContext;

  AnyExponentialHistogramGroupingAggregatorFunction(List<Integer> channels,
      DriverContext driverContext) {
    this.channels = channels;
    this.state = AnyExponentialHistogramAggregator.initGrouping(driverContext);
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int valueOffset = valueStart; valueOffset < valueEnd; valueOffset++) {
          ...
          AnyExponentialHistogramAggregator.combine(state, groupId, valueValue);
        }
      }
    }
  }
  ...
  public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        int valuesPosition = groupPosition + positionOffset;
        AnyExponentialHistogramAggregator.combineIntermediate(state, groupId, values.getExponentialHistogram(values.getFirstValueIndex(valuesPosition), valuesScratch), seen.getBoolean(valuesPosition));
      }
    }
  }
  ...
  private void evaluateFinal(Block[] blocks, int offset, IntVector selectedInPage,
      GroupingAggregatorEvaluationContext ctx) {
    blocks[offset] = AnyExponentialHistogramAggregator.evaluateFinal(state, selectedInPage, ctx);
  }

...
/**
 * {@link AggregatorFunction} implementation for {@link FirstBooleanByTimestampAggregator}.
 * This class is generated. Edit {@code AggregatorImplementer} instead.
 */
public final class FirstBooleanByTimestampAggregatorFunction implements AggregatorFunction {
  ...
  private final LongBooleanState state;

  private final List<Integer> channels;

  public FirstBooleanByTimestampAggregatorFunction(DriverContext driverContext,
      List<Integer> channels, LongBooleanState state) {
    this.driverContext = driverContext;
    this.channels = channels;
    this.state = state;
  }

  public static FirstBooleanByTimestampAggregatorFunction create(DriverContext driverContext,
      List<Integer> channels) {
    return new FirstBooleanByTimestampAggregatorFunction(driverContext, channels, FirstBooleanByTimestampAggregator.initSingle(driverContext));
  }
  ...
  private void addRawVector(BooleanVector valueVector, LongVector timestampVector) {
    ...
    while (state.seen() == false && valuesPosition < valueVector.getPositionCount()) {
      ...
      FirstBooleanByTimestampAggregator.first(state, valueValue, timestampValue);
      valuesPosition++;
      state.seen(true);
      break;
    }
    while (valuesPosition < valueVector.getPositionCount()) {
      boolean valueValue = valueVector.getBoolean(valuesPosition);
      long timestampValue = timestampVector.getLong(valuesPosition);
      FirstBooleanByTimestampAggregator.combine(state, valueValue, timestampValue);
      valuesPosition++;
    }
  }
  ...
  private void addRawBlock(BooleanBlock valueBlock, LongBlock timestampBlock) {
  ...
  private void addRawBlock(BooleanBlock valueBlock, LongBlock timestampBlock, BooleanVector mask) {
    for (int p = 0; p < valueBlock.getPositionCount(); p++) {
  ...
  public void addIntermediateInput(Page page) {
    ...
    BooleanVector values = ((BooleanBlock) valuesUncast).asVector();
    ...
    FirstBooleanByTimestampAggregator.combineIntermediate(state, timestamps.getLong(0), values.getBoolean(0), seen.getBoolean(0));
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, DriverContext driverContext) {
    ...
    blocks[offset] = FirstBooleanByTimestampAggregator.evaluateFinal(state, driverContext);
  }

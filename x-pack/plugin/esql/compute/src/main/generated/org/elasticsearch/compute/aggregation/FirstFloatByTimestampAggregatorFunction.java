...
public final class FirstFloatByTimestampAggregatorFunction implements AggregatorFunction {
  ...
  FirstFloatByTimestampAggregatorFunction(DriverContext driverContext, List<Integer> channels) {
    this.driverContext = driverContext;
    this.channels = channels;
    this.state = FirstFloatByTimestampAggregator.initSingle(driverContext);
  }
  ...
  private void addRawVector(FloatVector valueVector, LongVector timestampVector) {
    ...
    while (state.seen() == false && valuesPosition < valueVector.getPositionCount()) {
      ...
      FirstFloatByTimestampAggregator.first(state, valueValue, timestampValue);
      valuesPosition++;
      state.seen(true);
      break;
    }

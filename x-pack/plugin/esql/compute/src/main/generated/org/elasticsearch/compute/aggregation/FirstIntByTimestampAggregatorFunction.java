...
/**
 * {@link AggregatorFunction} implementation for {@link FirstIntByTimestampAggregator}.
 * This class is generated. Edit {@code AggregatorImplementer} instead.
 */
public final class FirstIntByTimestampAggregatorFunction implements AggregatorFunction {
  ...
  FirstIntByTimestampAggregatorFunction(DriverContext driverContext,
  ...
  private void addRawVector(IntVector valueVector, LongVector timestampVector) {
    ...
    while (state.seen() == false && valuesPosition < valueVector.getPositionCount()) {
      ...
      FirstIntByTimestampAggregator.first(state, valueValue, timestampValue);
      valuesPosition++;
      state.seen(true);
      break;
    }
    while (valuesPosition < valueVector.getPositionCount()) {
      int valueValue = valueVector.getInt(valuesPosition);
      long timestampValue = timestampVector.getLong(valuesPosition);
      FirstIntByTimestampAggregator.combine(state, valueValue, timestampValue);
      valuesPosition++;
    }
  }
  ...
  public void addIntermediateInput(Page page) {
    ...
    try (
      ...
      FirstIntByTimestampAggregator.combineIntermediate(state, timestamps.getLong(0), values.getInt(0), seen.getBoolean(0));
    }
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, DriverContext driverContext) {
    ...
    blocks[offset] = FirstIntByTimestampAggregator.evaluateFinal(state, driverContext);
  }

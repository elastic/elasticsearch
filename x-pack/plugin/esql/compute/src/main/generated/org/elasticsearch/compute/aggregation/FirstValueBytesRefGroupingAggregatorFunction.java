...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link FirstValueBytesRefAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class FirstValueBytesRefGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  private final FirstValueBytesRefAggregator.FirstValueLongGroupingState state;

  private final List<Integer> channels;

  private final DriverContext driverContext;

  public FirstValueBytesRefGroupingAggregatorFunction(List<Integer> channels,
      FirstValueBytesRefAggregator.FirstValueLongGroupingState state, DriverContext driverContext) {
    this.channels = channels;
    this.state = state;
    this.driverContext = driverContext;
  }

  public static FirstValueBytesRefGroupingAggregatorFunction create(List<Integer> channels,
      DriverContext driverContext) {
    return new FirstValueBytesRefGroupingAggregatorFunction(channels, FirstValueBytesRefAggregator.initGrouping(driverContext), driverContext);
  }
  ...
  public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = groups.getInt(groupPosition);
      FirstValueBytesRefAggregator.combineIntermediate(state, groupId, value.getBytesRef(groupPosition + positionOffset, scratch), timestamp.getLong(groupPosition + positionOffset), seen.getBoolean(groupPosition + positionOffset));
    }
  }
  ...
  public void addIntermediateRowInput(int groupId, GroupingAggregatorFunction input, int position) {
    ...
    FirstValueBytesRefAggregator.FirstValueLongGroupingState inState = ((FirstValueBytesRefGroupingAggregatorFunction) input).state;
    state.enableGroupIdTracking(new SeenGroupIds.Empty());
    FirstValueBytesRefAggregator.combineStates(state, groupId, inState, position);
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, IntVector selected,
      DriverContext driverContext) {
    blocks[offset] = FirstValueBytesRefAggregator.evaluateFinal(state, selected, driverContext);
  }

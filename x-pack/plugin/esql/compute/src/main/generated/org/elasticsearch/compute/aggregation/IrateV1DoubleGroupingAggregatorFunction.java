...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link IrateV1DoubleAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class IrateV1DoubleGroupingAggregatorFunction implements GroupingAggregatorFunction {
  private static final List<IntermediateStateDesc> INTERMEDIATE_STATE_DESC = List.of(
      new IntermediateStateDesc("timestamps", ElementType.LONG),
      new IntermediateStateDesc("values", ElementType.DOUBLE)  );

  private final IrateV1DoubleAggregator.DoubleIrateGroupingState state;
  ...
  IrateV1DoubleGroupingAggregatorFunction(Warnings warnings, List<Integer> channels,
      DriverContext driverContext, boolean isDateNanos) {
    this.isDateNanos = isDateNanos;
    this.warnings = warnings;
    this.channels = channels;
    this.state = IrateV1DoubleAggregator.initGrouping(driverContext, isDateNanos, warnings);
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int valueOffset = valueStart; valueOffset < valueEnd; valueOffset++) {
          ...
          for (int timestampOffset = timestampStart; timestampOffset < timestampEnd; timestampOffset++) {
            ...
            IrateV1DoubleAggregator.combine(state, groupId, valueValue, timestampValue, valuesPosition, temporalityBlock);
          }
        }
      }
    }
  ...
  public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        int valuesPosition = groupPosition + positionOffset;
        IrateV1DoubleAggregator.combineIntermediate(state, groupId, timestamps, values, valuesPosition);
      }
    }
  }
  ...
  private void evaluateFinal(Block[] blocks, int offset, IntVector selectedInPage,
      GroupingAggregatorEvaluationContext ctx) {
    blocks[offset] = IrateV1DoubleAggregator.evaluateFinal(state, selectedInPage, ctx);
  }

...
/**
 * {@link AggregatorFunction} implementation for {@link LastBytesRefByTimestampAggregator}.
 * This class is generated. Edit {@code AggregatorImplementer} instead.
 */
public final class LastBytesRefByTimestampAggregatorFunction implements AggregatorFunction {
  ...
  LastBytesRefByTimestampAggregatorFunction(DriverContext driverContext,
  ...
  private void addRawVector(BytesRefVector valueVector, LongVector timestampVector) {
    ...
    while (state.seen() == false && valuesPosition < valueVector.getPositionCount()) {
      ...
      LastBytesRefByTimestampAggregator.first(state, valueValue, timestampValue);
      valuesPosition++;
      state.seen(true);
      break;
    }
    while (valuesPosition < valueVector.getPositionCount()) {
      BytesRef valueValue = valueVector.getBytesRef(valuesPosition, valueScratch);
      long timestampValue = timestampVector.getLong(valuesPosition);
      LastBytesRefByTimestampAggregator.combine(state, valueValue, timestampValue);
      valuesPosition++;
    }
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, DriverContext driverContext) {
    ...
    blocks[offset] = LastBytesRefByTimestampAggregator.evaluateFinal(state, driverContext);
  }

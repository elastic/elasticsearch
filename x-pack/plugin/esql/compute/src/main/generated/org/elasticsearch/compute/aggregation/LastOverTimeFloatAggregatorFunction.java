...
/**
 * {@link AggregatorFunction} implementation for {@link LastOverTimeFloatAggregator}.
 * This class is generated. Edit {@code AggregatorImplementer} instead.
 */
public final class LastOverTimeFloatAggregatorFunction implements AggregatorFunction {
  ...
  public LastOverTimeFloatAggregatorFunction(DriverContext driverContext, List<Integer> channels,
  ...
  public static LastOverTimeFloatAggregatorFunction create(DriverContext driverContext,
      List<Integer> channels) {
    return new LastOverTimeFloatAggregatorFunction(driverContext, channels, LastOverTimeFloatAggregator.initSingle(driverContext));
  }
  ...
  private void addRawVector(FloatVector vector, LongVector timestamps) {
    ...
    for (int i = 0; i < vector.getPositionCount(); i++) {
      LastOverTimeFloatAggregator.combine(state, timestamps.getLong(i), vector.getFloat(i));
    }
  }
  ...
  private void addRawBlock(FloatBlock block, LongVector timestamps) {
    for (int p = 0; p < block.getPositionCount(); p++) {
      ...
      for (int i = start; i < end; i++) {
        LastOverTimeFloatAggregator.combine(state, timestamps.getLong(i), block.getFloat(i));
      }
    }
  }
  ...
  public void addIntermediateInput(Page page) {
    ...
    LastOverTimeFloatAggregator.combineIntermediate(state, timestamps.getLong(0), values.getFloat(0), seen.getBoolean(0));
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, DriverContext driverContext) {
    ...
    blocks[offset] = LastOverTimeFloatAggregator.evaluateFinal(state, driverContext);
  }

...
public final class MaxFloatAggregatorFunction implements AggregatorFunction {
  ...
  MaxFloatAggregatorFunction(DriverContext driverContext, List<Integer> channels) {
    this.driverContext = driverContext;
    this.channels = channels;
    this.state = new FloatState(MaxFloatAggregator.init());
  }
  ...
  private void addRawVector(FloatVector vVector) {
    ...
    if (vVector.getClass() == FloatArrowBufVector.class) {
      ...
      float bulkAcc = state.floatValue();
      for (int p = 0; p < bulkCount; p++) {
        float vValue = bulkSegment.getAtIndex(ValueLayout.JAVA_FLOAT_UNALIGNED, p);
        bulkAcc = MaxFloatAggregator.combine(bulkAcc, vValue);
      }
      state.floatValue(bulkAcc);
      state.seen(true);
      return;
    }

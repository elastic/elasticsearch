...
public final class MaxLongGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  public PartitionedHashTable.AggSplitter newSplitter() {
    return new MaxLongAggSplitter();
  }

  @Override
  public void combinePartition(PartitionedHashTable.PartitionedAgg source, int partition,
      int[] dstIds, int offset, int length) {
    final long[] sourceSums = ((MaxLongPartitionedAgg) source).subs[partition];
    int end = offset + length;
    for (int i = offset; i < end; i++) {
      state.set(dstIds[i], MaxLongAggregator.combine(state.getOrDefault(dstIds[i]), sourceSums[i]));
    }
  }
  ...
  private final class MaxLongAggSplitter implements PartitionedHashTable.AggSplitter {
    private long[][] subs;
    ...
    public PartitionedHashTable.PartitionedAgg finish() {
      return new MaxLongPartitionedAgg(subs);
    }
  ...
  static final class MaxLongPartitionedAgg implements PartitionedHashTable.PartitionedAgg {
    final long[][] subs;

    MaxLongPartitionedAgg(long[][] subs) {
      this.subs = subs;
    }

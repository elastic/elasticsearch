...
public final class MaxLongGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  public void addIntermediateInput(Vector groupIdVector, Block block) {
    ...
    LongArrayState tmpState = new LongArrayState(bigArrays, MaxLongAggregator.init());
    blobVector.get(0, tmpState);
    for (int i = 0; i < groupIdVector.getPositionCount(); i++) {
      int groupId = Math.toIntExact(groupIdVector.getLong(i));
      state.set(MaxLongAggregator.combine(state.getOrDefault(groupId), tmpState.get(i)), groupId);
    }
  }

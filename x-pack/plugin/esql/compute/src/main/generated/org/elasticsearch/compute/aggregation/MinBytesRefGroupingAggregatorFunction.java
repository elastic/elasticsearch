...
public final class MinBytesRefGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  private final BytesRefState.GroupingState state;
  ...
  public MinBytesRefGroupingAggregatorFunction(List<Integer> channels,
      BytesRefState.GroupingState state, DriverContext driverContext, BigArrays bigArrays) {
  ...
  public static MinBytesRefGroupingAggregatorFunction create(List<Integer> channels,
    ...
    return new MinBytesRefGroupingAggregatorFunction(channels, MinBytesRefAggregator.initGrouping(bigArrays), driverContext, bigArrays);
  }
  ...
  public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
    ...
    BytesRefVector min = page.<BytesRefBlock>getBlock(channels.get(0)).asVector();
    BytesRef scratch = new BytesRef();
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = Math.toIntExact(groups.getInt(groupPosition));
      MinBytesRefAggregator.combineIntermediate(state, groupId, min.getBytesRef(groupPosition + positionOffset, scratch));
    }
  }
  ...
  public void addIntermediateRowInput(int groupId, GroupingAggregatorFunction input, int position) {
    ...
    BytesRefState.GroupingState inState = ((MinBytesRefGroupingAggregatorFunction) input).state;
    state.enableGroupIdTracking(new SeenGroupIds.Empty());

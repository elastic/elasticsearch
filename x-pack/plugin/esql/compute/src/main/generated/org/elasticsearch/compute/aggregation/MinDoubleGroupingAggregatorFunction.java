...
public final class MinDoubleGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  private void addRawVector(Vector groupIdVector, Vector vector) {
    for (int i = 0; i < vector.getPositionCount(); i++) {
      ...
      state.set(MinDoubleAggregator.combine(state.getOrDefault(groupId), vector.getDouble(i)), groupId);
    }
  }
  ...
  private void addRawBlock(Vector groupIdVector, Block block) {
    for (int i = 0; i < block.getTotalValueCount(); i++) {
      if (block.isNull(i) == false) {
        int groupId = Math.toIntExact(groupIdVector.getLong(i));
        state.set(MinDoubleAggregator.combine(state.getOrDefault(groupId), block.getDouble(i)), groupId);
      }
    }
  }
  ...
  public void addIntermediateInput(Vector groupIdVector, Block block) {
    ...
    DoubleArrayState tmpState = new DoubleArrayState(bigArrays, MinDoubleAggregator.init());
    blobVector.get(0, tmpState);
    for (int i = 0; i < groupIdVector.getPositionCount(); i++) {
      int groupId = Math.toIntExact(groupIdVector.getLong(i));
      state.set(MinDoubleAggregator.combine(state.getOrDefault(groupId), tmpState.get(i)), groupId);
    }
  }

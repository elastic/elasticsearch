...
public final class MinFloatAggregatorFunction implements AggregatorFunction {
  ...
  public MinFloatAggregatorFunction(DriverContext driverContext, List<Integer> channels,
      FloatState state) {
  ...
  public static MinFloatAggregatorFunction create(DriverContext driverContext,
      List<Integer> channels) {
    return new MinFloatAggregatorFunction(driverContext, channels, new FloatState(MinFloatAggregator.init()));
  }
  ...
  private void addRawVector(FloatVector vector) {
    ...
    for (int i = 0; i < vector.getPositionCount(); i++) {
      state.floatValue(MinFloatAggregator.combine(state.floatValue(), vector.getFloat(i)));
    }
  }
  ...
  private void addRawBlock(FloatBlock block) {
    for (int p = 0; p < block.getPositionCount(); p++) {
      ...
      for (int i = start; i < end; i++) {
        state.floatValue(MinFloatAggregator.combine(state.floatValue(), block.getFloat(i)));
      }
    }
  }
  ...
  public void addIntermediateInput(Page page) {
    ...
    if (seen.getBoolean(0)) {
      state.floatValue(MinFloatAggregator.combine(state.floatValue(), min.getFloat(0)));
      state.seen(true);
    }
  }

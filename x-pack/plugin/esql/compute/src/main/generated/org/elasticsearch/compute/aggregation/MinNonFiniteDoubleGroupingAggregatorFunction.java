...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link MinNonFiniteDoubleAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class MinNonFiniteDoubleGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  MinNonFiniteDoubleGroupingAggregatorFunction(List<Integer> channels,
      DriverContext driverContext) {
    this.channels = channels;
    this.state = new DoubleArrayState(driverContext.bigArrays(), driverContext.breaker(), MinNonFiniteDoubleAggregator.init());
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int vOffset = vStart; vOffset < vEnd; vOffset++) {
          ...
          state.set(groupId, MinNonFiniteDoubleAggregator.combine(state.getOrDefault(groupId), vValue));
        }
      }
    }
  }
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        if (seen.getBoolean(valuesPosition)) {
          state.set(groupId, MinNonFiniteDoubleAggregator.combine(state.getOrDefault(groupId), min.getDouble(valuesPosition)));
        }
      }
    }
  }

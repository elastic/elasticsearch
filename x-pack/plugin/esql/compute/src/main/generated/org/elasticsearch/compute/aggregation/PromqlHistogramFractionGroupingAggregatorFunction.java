...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link PromqlHistogramFractionAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class PromqlHistogramFractionGroupingAggregatorFunction implements GroupingAggregatorFunction {
  private static final List<IntermediateStateDesc> INTERMEDIATE_STATE_DESC = List.of(
      new IntermediateStateDesc("buckets", ElementType.DOUBLE)  );

  private final PromqlHistogramStates.Fraction.GroupingState state;
  ...
  PromqlHistogramFractionGroupingAggregatorFunction(Warnings warnings, List<Integer> channels,
      DriverContext driverContext, double lower, double upper) {
    this.lower = lower;
    this.upper = upper;
    this.warnings = warnings;
    this.channels = channels;
    this.state = PromqlHistogramFractionAggregator.initGrouping(driverContext, lower, upper, warnings);
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int countOffset = countStart; countOffset < countEnd; countOffset++) {
          ...
          for (int upperBoundOffset = upperBoundStart; upperBoundOffset < upperBoundEnd; upperBoundOffset++) {
            ...
            PromqlHistogramFractionAggregator.combine(state, groupId, countValue, upperBoundValue);
          }
        }
      }
    }
  ...
  public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
    ...
    PromqlHistogramFractionAggregator.combineIntermediate(state, positionOffset, groups, buckets);
  }
  ...
  private void evaluateFinal(Block[] blocks, int offset, IntVector selectedInPage,
      GroupingAggregatorEvaluationContext ctx) {
    blocks[offset] = PromqlHistogramFractionAggregator.evaluateFinal(state, selectedInPage, ctx);
  }

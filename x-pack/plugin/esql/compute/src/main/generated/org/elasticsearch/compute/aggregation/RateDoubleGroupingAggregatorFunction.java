...
public final class RateDoubleGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  public RateDoubleGroupingAggregatorFunction(List<Integer> channels,
      RateStates.GroupingState state, BigArrays bigArrays) {
  ...
  public static RateDoubleGroupingAggregatorFunction create(List<Integer> channels,
      BigArrays bigArrays) {
    return new RateDoubleGroupingAggregatorFunction(channels, RateDoubleAggregator.initGrouping(bigArrays), bigArrays);
  }
  ...
  private void addRawInput(int positionOffset, IntVector groups, DoubleBlock values) {
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int v = valuesStart; v < valuesEnd; v++) {
        RateDoubleAggregator.combine(state, groupId, values.getDouble(v));
      }
    }
  }
  ...
  private void addRawInput(int positionOffset, IntVector groups, DoubleVector values) {
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = Math.toIntExact(groups.getInt(groupPosition));
      RateDoubleAggregator.combine(state, groupId, values.getDouble(groupPosition + positionOffset));
    }
  }
  ...
  public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = Math.toIntExact(groups.getInt(groupPosition));
      RateDoubleAggregator.combineIntermediate(state, groupId, rate.getBytesRef(groupPosition + positionOffset, scratch));
    }
  }
  ...
  public void addIntermediateRowInput(int groupId, GroupingAggregatorFunction input, int position) {
    ...
    RateStates.GroupingState inState = ((RateDoubleGroupingAggregatorFunction) input).state;
    state.enableGroupIdTracking(new SeenGroupIds.Empty());
  ...
  public void evaluateFinal(Block[] blocks, int offset, IntVector selected) {
    blocks[offset] = RateDoubleAggregator.evaluateFinal(state, selected);
  }

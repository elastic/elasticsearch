...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link SparklineFloatAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class SparklineFloatGroupingAggregatorFunction implements GroupingAggregatorFunction {
  private static final List<IntermediateStateDesc> INTERMEDIATE_STATE_DESC = List.of(
      new IntermediateStateDesc("values", ElementType.FLOAT),
      new IntermediateStateDesc("timestamps", ElementType.LONG)  );

  private final SparklineFloatAggregator.GroupingState state;
  ...
  public SparklineFloatGroupingAggregatorFunction(List<Integer> channels,
      SparklineFloatAggregator.GroupingState state, DriverContext driverContext, int limit,
      boolean ascending) {
  ...
  public static SparklineFloatGroupingAggregatorFunction create(List<Integer> channels,
      DriverContext driverContext, int limit, boolean ascending) {
    return new SparklineFloatGroupingAggregatorFunction(channels, SparklineFloatAggregator.initGrouping(driverContext.bigArrays(), limit, ascending), driverContext, limit, ascending);
  }
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int valueOffset = valueStart; valueOffset < valueEnd; valueOffset++) {
          ...
          for (int timestampOffset = timestampStart; timestampOffset < timestampEnd; timestampOffset++) {
            ...
            SparklineFloatAggregator.combine(state, groupId, valueValue, timestampValue);
          }
        }
      }
    }
  ...
  public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
    ...
    assert values.getPositionCount() == timestamps.getPositionCount();
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      if (groups.isNull(groupPosition)) {
        continue;
      }
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        int valuesPosition = groupPosition + positionOffset;
        SparklineFloatAggregator.combineIntermediate(state, groupId, values, timestamps, valuesPosition);
      }
    }
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, IntVector selected,
    ...
    blocks[offset] = SparklineFloatAggregator.evaluateFinal(state, selected, ctx);
  }

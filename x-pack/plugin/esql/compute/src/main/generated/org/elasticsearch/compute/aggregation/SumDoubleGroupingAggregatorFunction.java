...
public final class SumDoubleGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  private void addRawVector(LongVector groupIdVector, DoubleVector vector) {
    for (int position = 0; position < vector.getPositionCount(); position++) {
      ...
      state.set(SumDoubleAggregator.combine(state.getOrDefault(groupId), vector.getDouble(position)), groupId);
    }
  }
  ...
  private void addRawBlock(LongVector groupIdVector, DoubleBlock block) {
    for (int offset = 0; offset < block.getTotalValueCount(); offset++) {
      if (block.isNull(offset) == false) {
        ...
        state.set(SumDoubleAggregator.combine(state.getOrDefault(groupId), block.getDouble(offset)), groupId);
      }
    }
  }

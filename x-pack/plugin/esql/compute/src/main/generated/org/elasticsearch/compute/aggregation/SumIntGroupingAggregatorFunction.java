...
public final class SumIntGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  public PartitionedHashTable.AggSplitter newSplitter() {
    return new SumIntAggSplitter();
  }

  @Override
  public void combinePartition(PartitionedHashTable.PartitionedAgg source, int partition,
      int[] dstIds, int offset, int length) {
    final long[] sourceSums = ((SumIntPartitionedAgg) source).subs[partition];
    int end = offset + length;
    for (int i = offset; i < end; i++) {
      state.set(dstIds[i], SumIntAggregator.combine(state.getOrDefault(dstIds[i]), sourceSums[i]));
    }
  }
  ...
  private final class SumIntAggSplitter implements PartitionedHashTable.AggSplitter {
    private long[][] subs;
    ...
    public PartitionedHashTable.PartitionedAgg finish() {
      return new SumIntPartitionedAgg(subs);
    }
  ...
  static final class SumIntPartitionedAgg implements PartitionedHashTable.PartitionedAgg {
    final long[][] subs;

    SumIntPartitionedAgg(long[][] subs) {
      this.subs = subs;
    }

...
public final class TopBytesRefFloatGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  public TopBytesRefFloatGroupingAggregatorFunction(List<Integer> channels,
      TopBytesRefFloatAggregator.GroupingState state, DriverContext driverContext, int limit,
      boolean ascending) {
  ...
  public static TopBytesRefFloatGroupingAggregatorFunction create(List<Integer> channels,
      DriverContext driverContext, int limit, boolean ascending) {
    return new TopBytesRefFloatGroupingAggregatorFunction(channels, TopBytesRefFloatAggregator.initGrouping(driverContext.bigArrays(), limit, ascending), driverContext, limit, ascending);
  }
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int vOffset = vStart; vOffset < vEnd; vOffset++) {
          ...
          for (int extraOffset = extraStart; extraOffset < extraEnd; extraOffset++) {
            ...
            TopBytesRefFloatAggregator.combine(state, groupId, vValue, extraValue);
          }
        }
      }
    }
  ...
  public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        int valuesPosition = groupPosition + positionOffset;
        TopBytesRefFloatAggregator.combineIntermediate(state, groupId, top, extra, valuesPosition);
      }
    }
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, IntVector selected,
    ...
    blocks[offset] = TopBytesRefFloatAggregator.evaluateFinal(state, selected, ctx);
  }

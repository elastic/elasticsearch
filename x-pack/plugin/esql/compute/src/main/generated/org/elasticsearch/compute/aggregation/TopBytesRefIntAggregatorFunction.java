...
public final class TopBytesRefIntAggregatorFunction implements AggregatorFunction {
  ...
  public TopBytesRefIntAggregatorFunction(DriverContext driverContext, List<Integer> channels,
      TopBytesRefIntAggregator.SingleState state, int limit, boolean ascending) {
  ...
  public static TopBytesRefIntAggregatorFunction create(DriverContext driverContext,
      List<Integer> channels, int limit, boolean ascending) {
    return new TopBytesRefIntAggregatorFunction(driverContext, channels, TopBytesRefIntAggregator.initSingle(driverContext.bigArrays(), limit, ascending), limit, ascending);
  }
  ...
  private void addRawVector(BytesRefVector vVector, IntVector extraVector) {
    ...
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
      ...
      TopBytesRefIntAggregator.combine(state, vValue, extraValue);
    }
  }

  private void addRawVector(BytesRefVector vVector, IntVector extraVector, BooleanVector mask) {
    BytesRef vScratch = new BytesRef();
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
  ...
  private void addRawBlock(BytesRefBlock vBlock, IntBlock extraBlock) {
    BytesRef vScratch = new BytesRef();
  ...
  private void addRawBlock(BytesRefBlock vBlock, IntBlock extraBlock, BooleanVector mask) {
    BytesRef vScratch = new BytesRef();
    for (int p = 0; p < vBlock.getPositionCount(); p++) {
  ...
  public void addIntermediateInput(Page page) {
    ...
    TopBytesRefIntAggregator.combineIntermediate(state, top, extra);
  }

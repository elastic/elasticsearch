...
public final class TopBytesRefIntGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  public TopBytesRefIntGroupingAggregatorFunction(List<Integer> channels,
      TopBytesRefIntAggregator.GroupingState state, DriverContext driverContext, int limit,
      boolean ascending) {
  ...
  public static TopBytesRefIntGroupingAggregatorFunction create(List<Integer> channels,
      DriverContext driverContext, int limit, boolean ascending) {
    return new TopBytesRefIntGroupingAggregatorFunction(channels, TopBytesRefIntAggregator.initGrouping(driverContext.bigArrays(), limit, ascending), driverContext, limit, ascending);
  }
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int vOffset = vStart; vOffset < vEnd; vOffset++) {
          ...
          for (int extraOffset = extraStart; extraOffset < extraEnd; extraOffset++) {
            ...
            TopBytesRefIntAggregator.combine(state, groupId, vValue, extraValue);
          }
        }
      }
    }
  ...
  public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        int valuesPosition = groupPosition + positionOffset;
        TopBytesRefIntAggregator.combineIntermediate(state, groupId, top, extra, valuesPosition);
      }
    }
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, IntVector selected,
    ...
    blocks[offset] = TopBytesRefIntAggregator.evaluateFinal(state, selected, ctx);
  }

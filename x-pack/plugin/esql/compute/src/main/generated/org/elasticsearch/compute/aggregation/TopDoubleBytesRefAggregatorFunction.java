...
public final class TopDoubleBytesRefAggregatorFunction implements AggregatorFunction {
  ...
  public TopDoubleBytesRefAggregatorFunction(DriverContext driverContext, List<Integer> channels,
      TopDoubleBytesRefAggregator.SingleState state, int limit, boolean ascending) {
  ...
  public static TopDoubleBytesRefAggregatorFunction create(DriverContext driverContext,
      List<Integer> channels, int limit, boolean ascending) {
    return new TopDoubleBytesRefAggregatorFunction(driverContext, channels, TopDoubleBytesRefAggregator.initSingle(driverContext.bigArrays(), limit, ascending), limit, ascending);
  }
  ...
  private void addRawVector(DoubleVector vVector, BytesRefVector extraVector) {
    ...
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
      ...
      TopDoubleBytesRefAggregator.combine(state, vValue, extraValue);
    }
  }

  private void addRawVector(DoubleVector vVector, BytesRefVector extraVector, BooleanVector mask) {
    BytesRef extraScratch = new BytesRef();
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
  ...
  private void addRawBlock(DoubleBlock vBlock, BytesRefBlock extraBlock) {
    BytesRef extraScratch = new BytesRef();
  ...
  private void addRawBlock(DoubleBlock vBlock, BytesRefBlock extraBlock, BooleanVector mask) {
    BytesRef extraScratch = new BytesRef();
    for (int p = 0; p < vBlock.getPositionCount(); p++) {
  ...
  public void addIntermediateInput(Page page) {
    ...
    TopDoubleBytesRefAggregator.combineIntermediate(state, top, extra);
  }

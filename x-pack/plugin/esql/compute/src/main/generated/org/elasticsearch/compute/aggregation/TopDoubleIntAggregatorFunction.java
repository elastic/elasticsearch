...
public final class TopDoubleIntAggregatorFunction implements AggregatorFunction {
  ...
  TopDoubleIntAggregatorFunction(DriverContext driverContext, List<Integer> channels, int limit,
      boolean ascending) {
    this.limit = limit;
    this.ascending = ascending;
    this.driverContext = driverContext;
    this.channels = channels;
    this.state = TopDoubleIntAggregator.initSingle(driverContext.bigArrays(), limit, ascending);
  }
  ...
  private void addRawVectorGeneric(DoubleVector vVector, IntVector outputValueVector) {
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
      double vValue = vVector.getDouble(valuesPosition);
      int outputValueValue = outputValueVector.getInt(valuesPosition);
      TopDoubleIntAggregator.combine(state, vValue, outputValueValue);
  ...
  private void addRawVectorDoubleArrayVector(DoubleArrayVector vVector, IntVector outputValueVector,
      BooleanVector mask) {
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {

...
public final class TopDoubleLongAggregatorFunction implements AggregatorFunction {
  ...
  TopDoubleLongAggregatorFunction(DriverContext driverContext, List<Integer> channels, int limit,
      boolean ascending) {
    this.limit = limit;
    this.ascending = ascending;
    this.driverContext = driverContext;
    this.channels = channels;
    this.state = TopDoubleLongAggregator.initSingle(driverContext.bigArrays(), limit, ascending);
  }
  ...
  private void addRawVectorGeneric(DoubleVector vVector, LongVector outputValueVector) {
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
      double vValue = vVector.getDouble(valuesPosition);
      long outputValueValue = outputValueVector.getLong(valuesPosition);
      TopDoubleLongAggregator.combine(state, vValue, outputValueValue);

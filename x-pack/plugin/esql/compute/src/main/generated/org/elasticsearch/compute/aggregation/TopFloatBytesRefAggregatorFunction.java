...
public final class TopFloatBytesRefAggregatorFunction implements AggregatorFunction {
  ...
  public TopFloatBytesRefAggregatorFunction(DriverContext driverContext, List<Integer> channels,
      TopFloatBytesRefAggregator.SingleState state, int limit, boolean ascending) {
  ...
  public static TopFloatBytesRefAggregatorFunction create(DriverContext driverContext,
      List<Integer> channels, int limit, boolean ascending) {
    return new TopFloatBytesRefAggregatorFunction(driverContext, channels, TopFloatBytesRefAggregator.initSingle(driverContext.bigArrays(), limit, ascending), limit, ascending);
  }
  ...
  private void addRawVector(FloatVector vVector, BytesRefVector extraVector) {
    ...
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
      ...
      TopFloatBytesRefAggregator.combine(state, vValue, extraValue);
    }
  }

  private void addRawVector(FloatVector vVector, BytesRefVector extraVector, BooleanVector mask) {
    BytesRef extraScratch = new BytesRef();
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
  ...
  private void addRawBlock(FloatBlock vBlock, BytesRefBlock extraBlock) {
    BytesRef extraScratch = new BytesRef();
  ...
  private void addRawBlock(FloatBlock vBlock, BytesRefBlock extraBlock, BooleanVector mask) {
    BytesRef extraScratch = new BytesRef();
    for (int p = 0; p < vBlock.getPositionCount(); p++) {
  ...
  public void addIntermediateInput(Page page) {
    ...
    TopFloatBytesRefAggregator.combineIntermediate(state, top, extra);
  }

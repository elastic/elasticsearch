...
public final class TopFloatLongAggregatorFunction implements AggregatorFunction {
  ...
  TopFloatLongAggregatorFunction(DriverContext driverContext, List<Integer> channels, int limit,
      boolean ascending) {
    this.limit = limit;
    this.ascending = ascending;
    this.driverContext = driverContext;
    this.channels = channels;
    this.state = TopFloatLongAggregator.initSingle(driverContext.bigArrays(), limit, ascending);
  }
  ...
  private void addRawVectorGeneric(FloatVector vVector, LongVector outputValueVector) {
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
      float vValue = vVector.getFloat(valuesPosition);
      long outputValueValue = outputValueVector.getLong(valuesPosition);
      TopFloatLongAggregator.combine(state, vValue, outputValueValue);
  ...
  private void addRawVectorFloatArrayVector(FloatArrayVector vVector, LongVector outputValueVector,
      BooleanVector mask) {
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {

...
public final class TopIntIntAggregatorFunction implements AggregatorFunction {
  ...
  TopIntIntAggregatorFunction(DriverContext driverContext, List<Integer> channels, int limit,
      boolean ascending) {
    this.limit = limit;
    this.ascending = ascending;
    this.driverContext = driverContext;
    this.channels = channels;
    this.state = TopIntIntAggregator.initSingle(driverContext.bigArrays(), limit, ascending);
  }
  ...
  private void addRawVectorIntArrayVector(IntArrayVector vVector, IntVector outputValueVector) {
  ...
  private void addRawVectorGeneric(IntVector vVector, IntVector outputValueVector) {
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
      int vValue = vVector.getInt(valuesPosition);
      int outputValueValue = outputValueVector.getInt(valuesPosition);
      TopIntIntAggregator.combine(state, vValue, outputValueValue);
  ...
  private void addRawVectorIntArrowBufVector(IntArrowBufVector vVector, IntVector outputValueVector,
      BooleanVector mask) {
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
      if (mask.getBoolean(valuesPosition) == false) {
        continue;
  ...
  private void addRawVectorInt16ArrowBufVector(Int16ArrowBufVector vVector,
      IntVector outputValueVector, BooleanVector mask) {
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
      if (mask.getBoolean(valuesPosition) == false) {
        continue;
      }
  ...
  private void addRawVectorConstantIntVector(ConstantIntVector vVector, IntVector outputValueVector,
      BooleanVector mask) {
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
      if (mask.getBoolean(valuesPosition) == false) {
        continue;

...
public final class TopIntLongAggregatorFunction implements AggregatorFunction {
  ...
  TopIntLongAggregatorFunction(DriverContext driverContext, List<Integer> channels, int limit,
      boolean ascending) {
    this.limit = limit;
    this.ascending = ascending;
    this.driverContext = driverContext;
    this.channels = channels;
    this.state = TopIntLongAggregator.initSingle(driverContext.bigArrays(), limit, ascending);
  }
  ...
  private void addRawVectorIntArrayVector(IntArrayVector vVector, LongVector outputValueVector) {
  ...
  private void addRawVectorGeneric(IntVector vVector, LongVector outputValueVector) {
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
      int vValue = vVector.getInt(valuesPosition);
      long outputValueValue = outputValueVector.getLong(valuesPosition);
      TopIntLongAggregator.combine(state, vValue, outputValueValue);
  ...
  private void addRawVectorIntArrowBufVector(IntArrowBufVector vVector,
      LongVector outputValueVector, BooleanVector mask) {
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
      if (mask.getBoolean(valuesPosition) == false) {
        continue;
      }

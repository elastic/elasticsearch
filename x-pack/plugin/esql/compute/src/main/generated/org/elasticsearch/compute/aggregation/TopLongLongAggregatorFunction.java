...
public final class TopLongLongAggregatorFunction implements AggregatorFunction {
  ...
  TopLongLongAggregatorFunction(DriverContext driverContext, List<Integer> channels, int limit,
      boolean ascending) {
    this.limit = limit;
  ...
  private void addRawVector(LongVector vVector, LongVector outputValueVector) {
    if (vVector instanceof LongArrayVector specialized) {
      addRawVectorLongArrayVector(specialized, outputValueVector);
      return;
    }
    if (vVector instanceof LongArrowBufVector specialized) {
      addRawVectorLongArrowBufVector(specialized, outputValueVector);
      return;
    }
    if (vVector instanceof ConstantLongVector specialized) {
      addRawVectorConstantLongVector(specialized, outputValueVector);
      return;
    }
    addRawVectorGeneric(vVector, outputValueVector);
  }

  private void addRawVectorLongArrayVector(LongArrayVector vVector, LongVector outputValueVector) {
  ...
  private void addRawVectorGeneric(LongVector vVector, LongVector outputValueVector) {
    for (int valuesPosition = 0; valuesPosition < vVector.getPositionCount(); valuesPosition++) {
      long vValue = vVector.getLong(valuesPosition);
      long outputValueValue = outputValueVector.getLong(valuesPosition);
      TopLongLongAggregator.combine(state, vValue, outputValueValue);
  ...
  private void addRawVector(LongVector vVector, LongVector outputValueVector, BooleanVector mask) {
    if (vVector instanceof LongArrayVector specialized) {
      addRawVectorLongArrayVector(specialized, outputValueVector, mask);
      return;
    }
    if (vVector instanceof LongArrowBufVector specialized) {
      addRawVectorLongArrowBufVector(specialized, outputValueVector, mask);
      return;
    }
    if (vVector instanceof ConstantLongVector specialized) {
      addRawVectorConstantLongVector(specialized, outputValueVector, mask);
      return;
    }

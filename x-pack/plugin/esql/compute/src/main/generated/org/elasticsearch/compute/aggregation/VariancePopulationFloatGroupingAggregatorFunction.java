...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link VariancePopulationFloatAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class VariancePopulationFloatGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  public VariancePopulationFloatGroupingAggregatorFunction(List<Integer> channels,
  ...
  public static VariancePopulationFloatGroupingAggregatorFunction create(List<Integer> channels,
      DriverContext driverContext) {
    return new VariancePopulationFloatGroupingAggregatorFunction(channels, VariancePopulationFloatAggregator.initGrouping(driverContext.bigArrays()), driverContext);
  }
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int v = valuesStart; v < valuesEnd; v++) {
          VariancePopulationFloatAggregator.combine(state, groupId, values.getFloat(v));
        }
      }
    }
  }
  ...
  private void addRawInput(int positionOffset, IntArrayBlock groups, FloatVector values) {
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        VariancePopulationFloatAggregator.combine(state, groupId, values.getFloat(groupPosition + positionOffset));
      }
    }
  }
  ...
  public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = groups.getInt(groupPosition);
      VariancePopulationFloatAggregator.combineIntermediate(state, groupId, mean.getDouble(groupPosition + positionOffset), m2.getDouble(groupPosition + positionOffset), count.getLong(groupPosition + positionOffset));
    }
  }
  ...
  public void addIntermediateRowInput(int groupId, GroupingAggregatorFunction input, int position) {
    ...
    StdDevStates.GroupingState inState = ((VariancePopulationFloatGroupingAggregatorFunction) input).state;
    state.enableGroupIdTracking(new SeenGroupIds.Empty());
    VariancePopulationFloatAggregator.combineStates(state, groupId, inState, position);
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, IntVector selected,
    ...
    blocks[offset] = VariancePopulationFloatAggregator.evaluateFinal(state, selected, evaluatorContext.driverContext());
  }

...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link VarianceSampleDoubleAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class VarianceSampleDoubleGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  public VarianceSampleDoubleGroupingAggregatorFunction(List<Integer> channels,
  ...
  public static VarianceSampleDoubleGroupingAggregatorFunction create(List<Integer> channels,
      DriverContext driverContext) {
    return new VarianceSampleDoubleGroupingAggregatorFunction(channels, VarianceSampleDoubleAggregator.initGrouping(driverContext.bigArrays()), driverContext);
  }
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int v = valuesStart; v < valuesEnd; v++) {
          VarianceSampleDoubleAggregator.combine(state, groupId, values.getDouble(v));
        }
      }
    }
  }
  ...
  private void addRawInput(int positionOffset, IntArrayBlock groups, DoubleVector values) {
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        VarianceSampleDoubleAggregator.combine(state, groupId, values.getDouble(groupPosition + positionOffset));
      }
    }
  }
  ...
  public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = groups.getInt(groupPosition);
      VarianceSampleDoubleAggregator.combineIntermediate(state, groupId, mean.getDouble(groupPosition + positionOffset), m2.getDouble(groupPosition + positionOffset), count.getLong(groupPosition + positionOffset));
    }
  }
  ...
  public void addIntermediateRowInput(int groupId, GroupingAggregatorFunction input, int position) {
    ...
    StdDevStates.GroupingState inState = ((VarianceSampleDoubleGroupingAggregatorFunction) input).state;
    state.enableGroupIdTracking(new SeenGroupIds.Empty());
    VarianceSampleDoubleAggregator.combineStates(state, groupId, inState, position);
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, IntVector selected,
    ...
    blocks[offset] = VarianceSampleDoubleAggregator.evaluateFinal(state, selected, evaluatorContext.driverContext());
  }

...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link VarianceSampleFloatAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class VarianceSampleFloatGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  public VarianceSampleFloatGroupingAggregatorFunction(List<Integer> channels,
  ...
  public static VarianceSampleFloatGroupingAggregatorFunction create(List<Integer> channels,
      DriverContext driverContext) {
    return new VarianceSampleFloatGroupingAggregatorFunction(channels, VarianceSampleFloatAggregator.initGrouping(driverContext.bigArrays()), driverContext);
  }
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int v = valuesStart; v < valuesEnd; v++) {
          VarianceSampleFloatAggregator.combine(state, groupId, values.getFloat(v));
        }
      }
    }
  }
  ...
  private void addRawInput(int positionOffset, IntArrayBlock groups, FloatVector values) {
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        VarianceSampleFloatAggregator.combine(state, groupId, values.getFloat(groupPosition + positionOffset));
      }
    }
  }
  ...
  public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = groups.getInt(groupPosition);
      VarianceSampleFloatAggregator.combineIntermediate(state, groupId, mean.getDouble(groupPosition + positionOffset), m2.getDouble(groupPosition + positionOffset), count.getLong(groupPosition + positionOffset));
    }
  }
  ...
  public void addIntermediateRowInput(int groupId, GroupingAggregatorFunction input, int position) {
    ...
    StdDevStates.GroupingState inState = ((VarianceSampleFloatGroupingAggregatorFunction) input).state;
    state.enableGroupIdTracking(new SeenGroupIds.Empty());
    VarianceSampleFloatAggregator.combineStates(state, groupId, inState, position);
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, IntVector selected,
    ...
    blocks[offset] = VarianceSampleFloatAggregator.evaluateFinal(state, selected, evaluatorContext.driverContext());
  }

...
/**
 * {@link AggregatorFunction} implementation for {@link VarianceSampleIntAggregator}.
 * This class is generated. Edit {@code AggregatorImplementer} instead.
 */
public final class VarianceSampleIntAggregatorFunction implements AggregatorFunction {
  ...
  public VarianceSampleIntAggregatorFunction(DriverContext driverContext, List<Integer> channels,
  ...
  public static VarianceSampleIntAggregatorFunction create(DriverContext driverContext,
      List<Integer> channels) {
    return new VarianceSampleIntAggregatorFunction(driverContext, channels, VarianceSampleIntAggregator.initSingle());
  }
  ...
  private void addRawVector(IntVector vector) {
    for (int i = 0; i < vector.getPositionCount(); i++) {
      VarianceSampleIntAggregator.combine(state, vector.getInt(i));
    }
  }
  ...
  private void addRawBlock(IntBlock block) {
    for (int p = 0; p < block.getPositionCount(); p++) {
      ...
      for (int i = start; i < end; i++) {
        VarianceSampleIntAggregator.combine(state, block.getInt(i));
      }
    }
  }
  ...
  public void addIntermediateInput(Page page) {
    ...
    VarianceSampleIntAggregator.combineIntermediate(state, mean.getDouble(0), m2.getDouble(0), count.getLong(0));
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, DriverContext driverContext) {
    blocks[offset] = VarianceSampleIntAggregator.evaluateFinal(state, driverContext);
  }

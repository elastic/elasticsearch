...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link VarianceSampleLongAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class VarianceSampleLongGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  public VarianceSampleLongGroupingAggregatorFunction(List<Integer> channels,
  ...
  public static VarianceSampleLongGroupingAggregatorFunction create(List<Integer> channels,
      DriverContext driverContext) {
    return new VarianceSampleLongGroupingAggregatorFunction(channels, VarianceSampleLongAggregator.initGrouping(driverContext.bigArrays()), driverContext);
  }
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        ...
        for (int v = valuesStart; v < valuesEnd; v++) {
          VarianceSampleLongAggregator.combine(state, groupId, values.getLong(v));
        }
      }
    }
  }
  ...
  private void addRawInput(int positionOffset, IntArrayBlock groups, LongVector values) {
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int g = groupStart; g < groupEnd; g++) {
        int groupId = groups.getInt(g);
        VarianceSampleLongAggregator.combine(state, groupId, values.getLong(groupPosition + positionOffset));
      }
    }
  }
  ...
  public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = groups.getInt(groupPosition);
      VarianceSampleLongAggregator.combineIntermediate(state, groupId, mean.getDouble(groupPosition + positionOffset), m2.getDouble(groupPosition + positionOffset), count.getLong(groupPosition + positionOffset));
    }
  }
  ...
  public void addIntermediateRowInput(int groupId, GroupingAggregatorFunction input, int position) {
    ...
    StdDevStates.GroupingState inState = ((VarianceSampleLongGroupingAggregatorFunction) input).state;
    state.enableGroupIdTracking(new SeenGroupIds.Empty());
    VarianceSampleLongAggregator.combineStates(state, groupId, inState, position);
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, IntVector selected,
    ...
    blocks[offset] = VarianceSampleLongAggregator.evaluateFinal(state, selected, evaluatorContext.driverContext());
  }

...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link SpatialStExtentCartesianPointDocValuesAggregator}.
...
public final class SpatialStExtentCartesianPointDocValuesGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  public static SpatialStExtentCartesianPointDocValuesGroupingAggregatorFunction create(
    ...
    return new SpatialStExtentCartesianPointDocValuesGroupingAggregatorFunction(channels, SpatialStExtentCartesianPointDocValuesAggregator.initGrouping(driverContext.bigArrays()), driverContext);
  }
  ...
  private void addRawInput(int positionOffset, IntVector groups, LongBlock values) {
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int v = valuesStart; v < valuesEnd; v++) {
        SpatialStExtentCartesianPointDocValuesAggregator.combine(state, groupId, values.getLong(v));
      }
    }
  }
  ...
  private void addRawInput(int positionOffset, IntVector groups, LongVector values) {
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = groups.getInt(groupPosition);
      SpatialStExtentCartesianPointDocValuesAggregator.combine(state, groupId, values.getLong(groupPosition + positionOffset));
    }
  }
  ...
  public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = groups.getInt(groupPosition);
      SpatialStExtentCartesianPointDocValuesAggregator.combineIntermediate(state, groupId, xVal.getDouble(groupPosition + positionOffset), xDel.getDouble(groupPosition + positionOffset), yVal.getDouble(groupPosition + positionOffset), yDel.getDouble(groupPosition + positionOffset));
    }
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, IntVector selected,
    ...
    blocks[offset] = SpatialStExtentCartesianPointDocValuesAggregator.evaluateFinal(state, selected, driverContext);
  }

...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link SpatialStExtentCartesianPointSourceValuesAggregator}.
...
public final class SpatialStExtentCartesianPointSourceValuesGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  private void addRawInput(int positionOffset, IntVector groups, BytesRefBlock values) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int v = valuesStart; v < valuesEnd; v++) {
        SpatialStExtentCartesianPointSourceValuesAggregator.combine(state, groupId, values.getBytesRef(v, scratch));
      }
    }
  }
  ...
  private void addRawInput(int positionOffset, IntVector groups, BytesRefVector values) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = groups.getInt(groupPosition);
      SpatialStExtentCartesianPointSourceValuesAggregator.combine(state, groupId, values.getBytesRef(groupPosition + positionOffset, scratch));
    }
  }
  ...
  public void evaluateFinal(Block[] blocks, int offset, IntVector selected,
    ...
    blocks[offset] = SpatialStExtentCartesianPointSourceValuesAggregator.evaluateFinal(state, selected, driverContext);
  }

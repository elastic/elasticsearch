...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link SpatialStExtentGeoPointDocValuesAggregator}.
...
public final class SpatialStExtentGeoPointDocValuesGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  private void addRawInput(int positionOffset, IntVector groups, LongBlock values) {
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      ...
      for (int v = valuesStart; v < valuesEnd; v++) {
        SpatialStExtentGeoPointDocValuesAggregator.combine(state, groupId, values.getLong(v));
      }
    }
  }
  ...
  private void addRawInput(int positionOffset, IntVector groups, LongVector values) {
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = groups.getInt(groupPosition);
      SpatialStExtentGeoPointDocValuesAggregator.combine(state, groupId, values.getLong(groupPosition + positionOffset));
    }
  }
  ...
  public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = groups.getInt(groupPosition);
      SpatialStExtentGeoPointDocValuesAggregator.combineIntermediate(state, groupId, minX.getInt(groupPosition + positionOffset), maxX.getInt(groupPosition + positionOffset), maxY.getInt(groupPosition + positionOffset), minY.getInt(groupPosition + positionOffset));
    }
  }
  ...
  public void addIntermediateRowInput(int groupId, GroupingAggregatorFunction input, int position) {
    ...
    StExtentGroupingState inState = ((SpatialStExtentGeoPointDocValuesGroupingAggregatorFunction) input).state;
    state.enableGroupIdTracking(new SeenGroupIds.Empty());
  ...
  public void evaluateFinal(Block[] blocks, int offset, IntVector selected,
    ...
    blocks[offset] = SpatialStExtentGeoPointDocValuesAggregator.evaluateFinal(state, selected, driverContext);
  }

...
public final class SpatialStExtentGeoPointSourceValuesGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  public static SpatialStExtentGeoPointSourceValuesGroupingAggregatorFunction create(
    ...
    return new SpatialStExtentGeoPointSourceValuesGroupingAggregatorFunction(channels, SpatialStExtentGeoPointSourceValuesAggregator.initGrouping(driverContext.bigArrays()), driverContext);
  }
  ...
  public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = groups.getInt(groupPosition);
      SpatialStExtentGeoPointSourceValuesAggregator.combineIntermediate(state, groupId, xVal.getDouble(groupPosition + positionOffset), xDel.getDouble(groupPosition + positionOffset), yVal.getDouble(groupPosition + positionOffset), yDel.getDouble(groupPosition + positionOffset));
    }
  }

...
public final class SpatialStExtentGeoPointSourceValuesGroupingAggregatorFunction implements GroupingAggregatorFunction {
  ...
  public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
    ...
    for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
      int groupId = groups.getInt(groupPosition);
      SpatialStExtentGeoPointSourceValuesAggregator.combineIntermediate(state, groupId, minX.getInt(groupPosition + positionOffset), maxX.getInt(groupPosition + positionOffset), maxY.getInt(groupPosition + positionOffset), minY.getInt(groupPosition + positionOffset));
    }
  }
  ...
  public void addIntermediateRowInput(int groupId, GroupingAggregatorFunction input, int position) {
    ...
    StExtentGroupingState inState = ((SpatialStExtentGeoPointSourceValuesGroupingAggregatorFunction) input).state;
    state.enableGroupIdTracking(new SeenGroupIds.Empty());

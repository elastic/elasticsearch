...
public class DerivGroupingAggregatorFunction implements GroupingAggregatorFunction {
    ...
    public DerivGroupingAggregatorFunction(List<Integer> channels, DriverContext driverContext) {
    ...
    public AddInput prepareProcessRawInputPage(SeenGroupIds seenGroupIds, Page page) {
        final DoubleBlock valueBlock = page.getBlock(channels.get(0));
        final LongBlock timestampBlock = page.getBlock(channels.get(1));
        final DoubleVector valueVector = valueBlock.asVector();
        return new AddInput() {
            @Override
            ...
            public void add(int positionOffset, IntVector groupIds) {
                for (int groupPosition = 0; groupPosition < groupIds.getPositionCount(); groupPosition++) {
                    ...
                    double vValue = valueVector.getDouble(valuesPosition);
            ...
            private void addBlockInput(int positionOffset, IntBlock groupIds, LongBlock timestampBlock, DoubleVector valueVector) {
                for (int groupPosition = 0; groupPosition < groupIds.getPositionCount(); groupPosition++) {
                    ...
                    for (int g = groupStart; g < groupEnd; g++) {
                        ...
                        for (int v = vStart; v < vEnd; v++) {
                            ...
                            double val = valueVector.getDouble(valuePosition);
                            SimpleLinearRegressionWithTimeseries state = states.get(groupId);
                            if (state == null) {
                                state = new SimpleLinearRegressionWithTimeseries();
                                states.set(groupId, state);
    ...
    private void addIntermediateBlockInput(int positionOffset, IntBlock groupIdVector, Page page) {
        DoubleBlock sumValBlock = page.getBlock(channels.get(0));
        LongBlock sumTsBlock = page.getBlock(channels.get(1));
        DoubleBlock sumTsValBlock = page.getBlock(channels.get(2));
        ...
        for (int groupPos = 0; groupPos < groupIdVector.getPositionCount(); groupPos++) {
            ...
            for (int g = firstGroup; g < lastGroup; g++) {
                ...
                double sumVal = sumValBlock.getDouble(valuePos);
                double sumTsVal = sumTsValBlock.getDouble(valuePos);
    ...
    public void evaluateIntermediate(Block[] blocks, int offset, IntVector selected) {
        ...
        try (
            var sumValBuilder = blockFactory.newDoubleBlockBuilder(positionCount);
            var sumTsBuilder = blockFactory.newLongBlockBuilder(positionCount);
            var sumTsValBuilder = blockFactory.newDoubleBlockBuilder(positionCount);
            ...
            for (int p = 0; p < positionCount; p++) {
                ...
                if (state == null) {
                    ...
                    sumValBuilder.appendDouble(state.sumVal);
                    sumTsBuilder.appendLong(state.sumTs);
                    sumTsValBuilder.appendDouble(state.sumTsVal);

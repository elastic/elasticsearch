 ...
 */
public final class LastValueDoubleAggregatorFunction implements AggregatorFunction {
    ...
    public LastValueDoubleAggregatorFunction(DriverContext driverContext, List<Integer> channels, LastValueStates.SingleState state) {
        this.driverContext = driverContext;
        this.channels = channels;
        this.state = state;
    }

    public static LastValueDoubleAggregatorFunction create(DriverContext driverContext, List<Integer> channels) {
        return new LastValueDoubleAggregatorFunction(driverContext, channels, LastValueIntAggregator.initSingle());
    }
    ...
    private void addRawVector(DoubleVector vector, LongVector timestamps) {
        for (int i = 0; i < vector.getPositionCount(); i++) {
            LastValueDoubleAggregator.combine(state, vector.getDouble(i), timestamps.getLong(i));
        }
    }
    ...
    private void addRawBlock(DoubleBlock block, LongBlock timestamps) {
        for (int p = 0; p < block.getPositionCount(); p++) {
            ...
            for (int i = start; i < end; i++) {
                LastValueDoubleAggregator.combine(state, block.getDouble(i), timestamps.getLong(i));
            }
        }
    }
    ...
    public void addIntermediateInput(Page page) {
        ...
        LastValueDoubleAggregator.combineIntermediate(state, LastValue.getBytesRef(0, scratch));
    }
    ...
    public void evaluateFinal(Block[] blocks, int offset, DriverContext driverContext) {
        blocks[offset] = LastValueDoubleAggregator.evaluateFinal(state, driverContext);
    }

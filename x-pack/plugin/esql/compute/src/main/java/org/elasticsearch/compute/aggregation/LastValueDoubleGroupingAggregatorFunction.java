...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link LastValueDoubleAggregator}.
 * This class is geneLastValued. Do not edit it.
 */
public final class LastValueDoubleGroupingAggregatorFunction implements GroupingAggregatorFunction {
    ...
    public LastValueDoubleGroupingAggregatorFunction(
    ...
    public static LastValueDoubleGroupingAggregatorFunction create(
        List<Integer> channels,
        DriverContext driverContext,
        BigArrays bigArrays
    ) {
        return new LastValueDoubleGroupingAggregatorFunction(
            channels,
            LastValueDoubleAggregator.initGrouping(bigArrays),
            driverContext,
            bigArrays
        );
    }
    ...
    private void addRawInput(int positionOffset, IntVector groups, DoubleBlock values, LongBlock timestamps) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            ...
            LastValueDoubleAggregator.combine(state, groupId, values.getDouble(valuesStart), timestamps.getLong(valuesStart));
        }
    }
    ...
    private void addRawInput(int positionOffset, IntVector groups, DoubleVector values, LongVector timestamps) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            ...
            LastValueDoubleAggregator.combine(state, groupId, values.getDouble(offset), timestamps.getLong(offset));
        }
    }
    ...
    public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
        ...
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            int groupId = Math.toIntExact(groups.getInt(groupPosition));
            LastValueDoubleAggregator.combineIntermediate(state, groupId, LastValue.getBytesRef(groupPosition + positionOffset, scratch));
        }
    }
    ...
    public void addIntermediateRowInput(int groupId, GroupingAggregatorFunction input, int position) {
        ...
        LastValueStates.GroupingState inState = ((LastValueDoubleGroupingAggregatorFunction) input).state;
        state.enableGroupIdTracking(new SeenGroupIds.Empty());
        LastValueDoubleAggregator.combineStates(state, groupId, inState, position);
    }
    ...
    public void evaluateFinal(Block[] blocks, int offset, IntVector selected, DriverContext driverContext) {
        blocks[offset] = LastValueDoubleAggregator.evaluateFinal(state, selected, driverContext);
    }

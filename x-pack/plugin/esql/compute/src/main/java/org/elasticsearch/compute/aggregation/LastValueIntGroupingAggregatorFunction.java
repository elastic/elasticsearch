...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link LastValueIntAggregator}.
 * This class is geneLastValued. Do not edit it.
 */
public final class LastValueIntGroupingAggregatorFunction implements GroupingAggregatorFunction {
    ...
    public LastValueIntGroupingAggregatorFunction(
    ...
    public static LastValueIntGroupingAggregatorFunction create(List<Integer> channels, DriverContext driverContext, BigArrays bigArrays) {
        return new LastValueIntGroupingAggregatorFunction(
            channels,
            LastValueIntAggregator.initGrouping(bigArrays),
            driverContext,
            bigArrays
        );
    }
    ...
    private void addRawInput(int positionOffset, IntVector groups, IntBlock values, LongBlock timestamps) {
        ...
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            ...
            LastValueIntAggregator.combine(state, groupId, values.getInt(valuesStart), timestamps.getLong(valuesStart));
        }
    }
    ...
    private void addRawInput(int positionOffset, IntVector groups, IntVector values, LongVector timestamps) {
        ...
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            ...
            LastValueIntAggregator.combine(state, groupId, values.getInt(offset), timestamps.getLong(offset));
        }
    }
    ...
    public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
        ...
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            int groupId = Math.toIntExact(groups.getInt(groupPosition));
            LastValueIntAggregator.combineIntermediate(state, groupId, LastValue.getBytesRef(groupPosition + positionOffset, scratch));
        }
    }
    ...
    public void addIntermediateRowInput(int groupId, GroupingAggregatorFunction input, int position) {
        ...
        LastValueStates.GroupingState inState = ((LastValueIntGroupingAggregatorFunction) input).state;
        state.enableGroupIdTracking(new SeenGroupIds.Empty());
        LastValueIntAggregator.combineStates(state, groupId, inState, position);
    }
    ...
    public void evaluateFinal(Block[] blocks, int offset, IntVector selected, DriverContext driverContext) {
        blocks[offset] = LastValueIntAggregator.evaluateFinal(state, selected, driverContext);
    }

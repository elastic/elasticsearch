...
@Aggregator({ @IntermediateState(name = "max", type = "BYTES_REF") })
@GroupingAggregator
class MaxBytesRefAggregator {

    public static BytesRefState.SingleState initSingle() {
        return new BytesRefState.SingleState(BytesRefState.Operations.MAX);
    }
    ...
    public static BytesRefState.GroupingState initGrouping(BigArrays bigArrays) {
        return new BytesRefState.GroupingState(bigArrays, BytesRefState.Operations.MAX);
    }

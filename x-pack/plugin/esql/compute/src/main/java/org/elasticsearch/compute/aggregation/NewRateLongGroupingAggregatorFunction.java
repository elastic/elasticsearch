...
public final class NewRateLongGroupingAggregatorFunction implements GroupingAggregatorFunction {
    ...
    public static final class FunctionSupplier implements AggregatorFunctionSupplier {
        // Overriding constructor to support isNewRateOverTime flag
        private final boolean isNewRateOverTime;

        public FunctionSupplier(boolean isNewRateOverTime, boolean isDateNanos) {
            this.isNewRateOverTime = isNewRateOverTime;
            if (isDateNanos) {
        ...
        @Override
        public NewRateLongGroupingAggregatorFunction groupingAggregator(DriverContext driverContext, List<Integer> channels) {
            return new NewRateLongGroupingAggregatorFunction(channels, driverContext, isNewRateOverTime);
        }
    ...
    public NewRateLongGroupingAggregatorFunction(List<Integer> channels, DriverContext driverContext, boolean isNewRateOverTime) {
        ...
        try (var rates = blockFactory.newDoubleBlockBuilder(positionCount)) {
            for (int p = 0; p < positionCount; p++) {
                ...
                if (evalContext instanceof TimeSeriesGroupingAggregatorEvaluationContext tsContext) {
                    ...
                    rate = computeNewRateWithoutExtrapolate(state, isNewRateOverTime);
                }
    ...
    private static double computeNewRateWithoutExtrapolate(ReducedState state, boolean isNewRateOverTime) {
     ...
     */
    private static double extrapolateNewRate(ReducedState state, long rangeStart, long rangeEnd, boolean isNewRateOverTime) {
        assert state.samples >= 2 : "rate requires at least two samples; got " + state.samples;
        final long firstTS = state.intervals[state.intervals.length - 1].t2;
        final long lastTS = state.intervals[0].t1;
        double firstValue = state.intervals[state.intervals.length - 1].v2;

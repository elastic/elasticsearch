...
import org.elasticsearch.compute.aggregation.oldrate.OldRateDoubleAggregator;
...
public final class RateDoubleGroupingAggregatorFunction implements GroupingAggregatorFunction {
    ...
    private final OldRateDoubleAggregator.DoubleRateGroupingState oldState;
    private final OldRateDoubleGroupingAggregatorFunction oldRate;
    ...
    public RateDoubleGroupingAggregatorFunction(List<Integer> channels, DriverContext driverContext) {
        ...
        this.oldState = new OldRateDoubleAggregator.DoubleRateGroupingState(driverContext.bigArrays(), driverContext.breaker());
        this.oldRate = new OldRateDoubleGroupingAggregatorFunction(channels, oldState, driverContext);
    }
    ...
    private void addRawInput(int positionOffset, IntBlock groups, DoubleBlock valueBlock, LongVector timestampVector) {
        ...
        for (int p = 0; p < positionCount; p++) {
            ...
            for (int g = groupStart; g < groupEnd; g++) {
                ...
                buffer.appendOneValue(timestamp, valueBlock.getDouble(valueStart));
            }
        }
    }
    ...
    private void addRawInput(int positionOffset, IntVector groups, DoubleBlock valueBlock, LongVector timestampVector) {
        if (groups.isConstant()) {
            ...
            for (int p = 0; p < groups.getPositionCount(); p++) {
                ...
                assert valueBlock.getValueCount(valuePosition) == 1 : "expected single-valued block " + valueBlock;
                buffer.appendOneValue(timestampVector.getLong(valuePosition), valueBlock.getDouble(valuePosition));
            }
        } else {
            int lastGroup = -1;
            Buffer buffer = null;
            for (int p = 0; p < groups.getPositionCount(); p++) {
                ...
                buffer.appendOneValue(timestamp, valueBlock.getDouble(valuePosition));
            }
        }
    }
    ...
    private void addRawInput(int positionOffset, IntVector groups, DoubleVector valueVector, LongVector timestampVector) {
        ...
        if (groups.isConstant()) {
            ...
            for (int p = 0; p < positionCount; p++) {
                int valuePosition = positionOffset + p;
                state.appendOneValue(timestampVector.getLong(valuePosition), valueVector.getDouble(valuePosition));
            }
        } else {
            int lastGroup = -1;
            Buffer buffer = null;
            for (int p = 0; p < positionCount; p++) {
                ...
                buffer.appendOneValue(timestamp, valueVector.getDouble(valuePosition));
            }
        }
    }
    ...
    void flushBufferToOldRate(Buffer buffer, int groupId) {
        if (buffer.totalCount == 1) {
            try (
                ...
                var vs = driverContext.blockFactory().newConstantDoubleVector(buffer.values.get(0), 1)
            ) {
        ...
        try (
            ...
            var vBuilder = driverContext.blockFactory().newDoubleBlockBuilder(2)
        ) {
            ...
            vBuilder.appendDouble(lastValue);
            vBuilder.appendDouble(buffer.values.get(position));
    ...
    static final class Buffer implements Releasable {
        ...
        Buffer(BigArrays bigArrays, int initialSize) {
            ...
            this.values = bigArrays.newDoubleArray(initialSize, false);
            this.sliceOffsets = EMPTY_SLICES;
        }

        void appendOneValue(long timestamp, double value) {

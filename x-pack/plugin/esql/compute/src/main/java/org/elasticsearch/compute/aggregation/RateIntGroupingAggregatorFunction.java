...
public final class RateIntGroupingAggregatorFunction implements GroupingAggregatorFunction {
    ...
    private final OldRateIntAggregator.IntRateGroupingState oldState;
    private final OldRateIntGroupingAggregatorFunction oldRate;
    ...
    public RateIntGroupingAggregatorFunction(List<Integer> channels, DriverContext driverContext) {
        ...
        this.oldState = new OldRateIntAggregator.IntRateGroupingState(driverContext.bigArrays(), driverContext.breaker());
        this.oldRate = new OldRateIntGroupingAggregatorFunction(channels, oldState, driverContext);
    }
    ...
    private void addRawInput(int positionOffset, IntBlock groups, IntBlock valueBlock, LongVector timestampVector) {
        ...
        for (int p = 0; p < positionCount; p++) {
            ...
            for (int g = groupStart; g < groupEnd; g++) {
                ...
                buffer.appendOneValue(timestamp, valueBlock.getInt(valueStart));
            }
        }
    }
    ...
    private void addRawInput(int positionOffset, IntVector groups, IntBlock valueBlock, LongVector timestampVector) {
        if (groups.isConstant()) {
            ...
            for (int p = 0; p < groups.getPositionCount(); p++) {
                ...
                assert valueBlock.getValueCount(valuePosition) == 1 : "expected single-valued block " + valueBlock;
                buffer.appendOneValue(timestampVector.getLong(valuePosition), valueBlock.getInt(valuePosition));
            }
        } else {
            int lastGroup = -1;
            Buffer buffer = null;
            for (int p = 0; p < groups.getPositionCount(); p++) {
                ...
                buffer.appendOneValue(timestamp, valueBlock.getInt(valuePosition));
            }
        }
    }
    ...
    private void addRawInput(int positionOffset, IntVector groups, IntVector valueVector, LongVector timestampVector) {
        ...
        if (groups.isConstant()) {
            ...
            for (int p = 0; p < positionCount; p++) {
                int valuePosition = positionOffset + p;
                state.appendOneValue(timestampVector.getLong(valuePosition), valueVector.getInt(valuePosition));
            }
        } else {
            int lastGroup = -1;
            Buffer buffer = null;
            for (int p = 0; p < positionCount; p++) {
                ...
                buffer.appendOneValue(timestamp, valueVector.getInt(valuePosition));
            }
        }
    }
    ...
    void flushBufferToOldRate(Buffer buffer, int groupId) {
        if (buffer.totalCount == 1) {
            try (
                ...
                var vs = driverContext.blockFactory().newConstantIntVector(buffer.values.get(0), 1)
            ) {
        ...
        try (
            ...
            var vBuilder = driverContext.blockFactory().newIntBlockBuilder(2)
        ) {
            ...
            vBuilder.appendInt(lastValue);
            vBuilder.appendInt(buffer.values.get(position));
    ...
    static final class Buffer implements Releasable {
        ...
        Buffer(BigArrays bigArrays, int initialSize) {
            ...
            this.values = bigArrays.newIntArray(initialSize, false);
            this.sliceOffsets = EMPTY_SLICES;
        }

        void appendOneValue(long timestamp, int value) {

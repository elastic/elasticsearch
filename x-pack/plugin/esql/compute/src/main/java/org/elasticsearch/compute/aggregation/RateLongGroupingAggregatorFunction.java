...
public final class RateLongGroupingAggregatorFunction implements GroupingAggregatorFunction {
    ...
    public static RateLongGroupingAggregatorFunction create(List<Integer> channels, DriverContext driverContext, BigArrays bigArrays) {
    ...
    private void addRawInput(int positionOffset, IntVector groups, LongBlock values, LongBlock timestamps) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            ...
            for (int v = valuesStart; v < valuesEnd; v++) {
                RateLongAggregator.combine(state, groupId, values.getLong(v), timestamps.getLong(v));
            }
        }
    }
    ...
    private void addRawInput(int positionOffset, IntVector groups, LongVector values, LongVector timestamps) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            ...
            RateLongAggregator.combine(state, groupId, values.getLong(offset), timestamps.getLong(offset));
        }
    }
    ...
    public void addIntermediateInput(int positionOffset, IntVector groups, Page page) {
        ...
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            int groupId = Math.toIntExact(groups.getInt(groupPosition));
            RateLongAggregator.combineIntermediate(state, groupId, rate.getBytesRef(groupPosition + positionOffset, scratch));
        }
    }
    ...
    public void addIntermediateRowInput(int groupId, GroupingAggregatorFunction input, int position) {
        ...
        RateStates.GroupingState inState = ((RateLongGroupingAggregatorFunction) input).state;
        state.enableGroupIdTracking(new SeenGroupIds.Empty());
    ...
    public void evaluateFinal(Block[] blocks, int offset, IntVector selected, DriverContext driverContext) {
        BytesRefVector tsids = ((BytesRefBlock) blocks[1]).asVector();
        blocks[offset] = RateLongAggregator.evaluateFinal(state, selected, tsids, driverContext);
    }

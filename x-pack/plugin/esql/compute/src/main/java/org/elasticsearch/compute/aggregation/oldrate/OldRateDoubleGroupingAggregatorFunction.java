 ...
 * 2.0.
 */
package org.elasticsearch.compute.aggregation.oldrate;
...
public final class OldRateDoubleGroupingAggregatorFunction implements GroupingAggregatorFunction {
    ...
    private final OldRateDoubleAggregator.DoubleRateGroupingState state;

    private final List<Integer> channels;

    private final DriverContext driverContext;

    public OldRateDoubleGroupingAggregatorFunction(
        List<Integer> channels,
        OldRateDoubleAggregator.DoubleRateGroupingState state,
    ...
    public static OldRateDoubleGroupingAggregatorFunction create(List<Integer> channels, DriverContext driverContext) {
        return new OldRateDoubleGroupingAggregatorFunction(channels, OldRateDoubleAggregator.initGrouping(driverContext), driverContext);
    }
    ...
    private void addRawInput(int positionOffset, IntArrayBlock groups, DoubleBlock valueBlock, LongBlock timestampBlock) {
            ...
            for (int g = groupStart; g < groupEnd; g++) {
                ...
                for (int valueOffset = valueStart; valueOffset < valueEnd; valueOffset++) {
                    ...
                    for (int timestampOffset = timestampStart; timestampOffset < timestampEnd; timestampOffset++) {
                        ...
                        OldRateDoubleAggregator.combine(state, groupId, valueValue, timestampValue);
                    }
                }
            }
        }
    ...
    private void addRawInput(int positionOffset, IntArrayBlock groups, DoubleVector valueVector, LongVector timestampVector) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            if (groups.isNull(groupPosition)) {
                continue;
            }
    ...
    public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
        ...
        assert timestamps.getPositionCount() == values.getPositionCount()
            && timestamps.getPositionCount() == sampleCounts.getPositionCount()
            && timestamps.getPositionCount() == resets.getPositionCount();
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            if (groups.isNull(groupPosition)) {
                continue;
            }
            ...
            for (int g = groupStart; g < groupEnd; g++) {
                int groupId = groups.getInt(g);
                int valuesPosition = groupPosition + positionOffset;
                OldRateDoubleAggregator.combineIntermediate(
                    state,
                    groupId,
                    timestamps,
                    values,
                    sampleCounts.getInt(valuesPosition),
                    resets.getDouble(valuesPosition),
                    valuesPosition
                );
            }
        }
    ...
    private void addRawInput(int positionOffset, IntBigArrayBlock groups, DoubleBlock valueBlock, LongBlock timestampBlock) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            if (groups.isNull(groupPosition)) {
                continue;
            }
    ...
    private void addRawInput(int positionOffset, IntBigArrayBlock groups, DoubleVector valueVector, LongVector timestampVector) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            if (groups.isNull(groupPosition)) {
                continue;
            }
    ...
    private void addRawInput(int positionOffset, IntVector groups, DoubleBlock valueBlock, LongBlock timestampBlock) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            int valuesPosition = groupPosition + positionOffset;
            if (valueBlock.isNull(valuesPosition)) {
                continue;
    ...
    private void maybeEnableGroupIdTracking(SeenGroupIds seenGroupIds, DoubleBlock valueBlock, LongBlock timestampBlock) {
    ...
    public void evaluateFinal(Block[] blocks, int offset, IntVector selected, GroupingAggregatorEvaluationContext ctx) {
        blocks[offset] = OldRateDoubleAggregator.evaluateFinal(state, selected, ctx);
    }

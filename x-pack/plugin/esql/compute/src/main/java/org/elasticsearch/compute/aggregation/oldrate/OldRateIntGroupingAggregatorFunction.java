...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link OldRateIntAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class OldRateIntGroupingAggregatorFunction implements GroupingAggregatorFunction {
    ...
    private final OldRateIntAggregator.IntRateGroupingState state;

    private final List<Integer> channels;

    private final DriverContext driverContext;

    public OldRateIntGroupingAggregatorFunction(
        List<Integer> channels,
        OldRateIntAggregator.IntRateGroupingState state,
    ...
    public static OldRateIntGroupingAggregatorFunction create(List<Integer> channels, DriverContext driverContext) {
        return new OldRateIntGroupingAggregatorFunction(channels, OldRateIntAggregator.initGrouping(driverContext), driverContext);
    }
    ...
    private void addRawInput(int positionOffset, IntArrayBlock groups, IntBlock valueBlock, LongBlock timestampBlock) {
            ...
            for (int g = groupStart; g < groupEnd; g++) {
                ...
                for (int valueOffset = valueStart; valueOffset < valueEnd; valueOffset++) {
                    ...
                    for (int timestampOffset = timestampStart; timestampOffset < timestampEnd; timestampOffset++) {
                        ...
                        OldRateIntAggregator.combine(state, groupId, valueValue, timestampValue);
                    }
                }
            }
        }
    ...
    private void addRawInput(int positionOffset, IntArrayBlock groups, IntVector valueVector, LongVector timestampVector) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            if (groups.isNull(groupPosition)) {
                continue;
            }
    ...
    public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
        ...
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            ...
            for (int g = groupStart; g < groupEnd; g++) {
                int groupId = groups.getInt(g);
                int valuesPosition = groupPosition + positionOffset;
                OldRateIntAggregator.combineIntermediate(
                    state,
                    groupId,
                    timestamps,
                    values,
    ...
    private void addRawInput(int positionOffset, IntBigArrayBlock groups, IntBlock valueBlock, LongBlock timestampBlock) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            if (groups.isNull(groupPosition)) {
                continue;
            }
    ...
    private void addRawInput(int positionOffset, IntBigArrayBlock groups, IntVector valueVector, LongVector timestampVector) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            if (groups.isNull(groupPosition)) {
                continue;
            }
    ...
    private void addRawInput(int positionOffset, IntVector groups, IntBlock valueBlock, LongBlock timestampBlock) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            int valuesPosition = groupPosition + positionOffset;
            if (valueBlock.isNull(valuesPosition)) {
                continue;
    ...
    private void maybeEnableGroupIdTracking(SeenGroupIds seenGroupIds, IntBlock valueBlock, LongBlock timestampBlock) {
    ...
    public void evaluateFinal(Block[] blocks, int offset, IntVector selected, GroupingAggregatorEvaluationContext ctx) {
        blocks[offset] = OldRateIntAggregator.evaluateFinal(state, selected, ctx);
    }

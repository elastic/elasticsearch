...
/**
 * {@link GroupingAggregatorFunction} implementation for {@link OldRateLongAggregator}.
 * This class is generated. Edit {@code GroupingAggregatorImplementer} instead.
 */
public final class OldRateLongGroupingAggregatorFunction implements GroupingAggregatorFunction {
    ...
    private final OldRateLongAggregator.LongRateGroupingState state;

    private final List<Integer> channels;

    private final DriverContext driverContext;

    public OldRateLongGroupingAggregatorFunction(
        List<Integer> channels,
        OldRateLongAggregator.LongRateGroupingState state,
    ...
    public static OldRateLongGroupingAggregatorFunction create(List<Integer> channels, DriverContext driverContext) {
        return new OldRateLongGroupingAggregatorFunction(channels, OldRateLongAggregator.initGrouping(driverContext), driverContext);
    }
            ...
            for (int g = groupStart; g < groupEnd; g++) {
                ...
                for (int valueOffset = valueStart; valueOffset < valueEnd; valueOffset++) {
                    ...
                    for (int timestampOffset = timestampStart; timestampOffset < timestampEnd; timestampOffset++) {
                        ...
                        OldRateLongAggregator.combine(state, groupId, valueValue, timestampValue);
                    }
                }
            }
        }
    ...
    private void addRawInput(int positionOffset, IntArrayBlock groups, LongVector valueVector, LongVector timestampVector) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            if (groups.isNull(groupPosition)) {
                continue;
            }
    ...
    public void addIntermediateInput(int positionOffset, IntArrayBlock groups, Page page) {
        ...
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            ...
            for (int g = groupStart; g < groupEnd; g++) {
                int groupId = groups.getInt(g);
                int valuesPosition = groupPosition + positionOffset;
                OldRateLongAggregator.combineIntermediate(
                    state,
                    groupId,
                    timestamps,
                    values,
    ...
    private void addRawInput(int positionOffset, IntBigArrayBlock groups, LongVector valueVector, LongVector timestampVector) {
        for (int groupPosition = 0; groupPosition < groups.getPositionCount(); groupPosition++) {
            if (groups.isNull(groupPosition)) {
                continue;
            }
    ...
    public void evaluateFinal(Block[] blocks, int offset, IntVector selected, GroupingAggregatorEvaluationContext ctx) {
        blocks[offset] = OldRateLongAggregator.evaluateFinal(state, selected, ctx);
    }

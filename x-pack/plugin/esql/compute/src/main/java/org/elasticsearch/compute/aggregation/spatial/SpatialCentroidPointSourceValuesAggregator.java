 ...
 * <p>
 * The per-document coordinates are quantized through the {@link CoordinateEncoder} to match
 * the precision of doc-values storage, ensuring consistent results regardless of whether the
 * points are loaded from source or doc-values.
 */
@Aggregator(
    {
...
class SpatialCentroidPointSourceValuesAggregator extends CentroidPointAggregator {
    public static void combine(CentroidState current, BytesRef wkb) {
        ...
        double x = current.encoder.decodeX(current.encoder.encodeX(current.encoder.normalizeX(point.getX())));
        double y = current.encoder.decodeY(current.encoder.encodeY(current.encoder.normalizeY(point.getY())));
        current.add(x, y);
    }
    ...
    public static void combine(GroupingCentroidState current, int groupId, BytesRef wkb) {
        ...
        double x = current.encoder.decodeX(current.encoder.encodeX(current.encoder.normalizeX(point.getX())));
        double y = current.encoder.decodeY(current.encoder.encodeY(current.encoder.normalizeY(point.getY())));
        current.add(x, 0d, y, 0d, 1, groupId);
    }
}

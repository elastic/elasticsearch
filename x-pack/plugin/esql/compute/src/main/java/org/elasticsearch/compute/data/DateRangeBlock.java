...
public final class DateRangeBlock extends AbstractNonThreadSafeRefCounted implements Block {
    private final LongBlock fromBlock;
    private final LongBlock toBlock;

    public DateRangeBlock(LongBlock fromBlock, LongBlock toBlock) {
    ...
    public Pair<Long, Long> getDateRange(int position) {
        return Pair.of(fromBlock.getLong(position), toBlock.getLong(position));
    }
    ...
    public DateRangeBlock filter(int... positions) {
        ...
        try {
            ...
            result = new DateRangeBlock(newFromBlock, newToBlock);
            return result;
        } finally {
            if (result == null) {
    ...
    public static Block readFrom(StreamInput in) throws IOException {
        ...
        try {
            ...
            DateRangeBlock result = new DateRangeBlock(from, to);
            success = true;
            return result;
        } finally {
            if (success == false) {
    ...
    public boolean equals(Object obj) {
        ...
        DateRangeBlock that = (DateRangeBlock) obj;
        return fromBlock.equals(that.fromBlock) && toBlock.equals(that.toBlock);
    }

    @Override
    public int hashCode() {
        int result = fromBlock.hashCode();
        result = 31 * result + toBlock.hashCode();
        return result;
    }
}

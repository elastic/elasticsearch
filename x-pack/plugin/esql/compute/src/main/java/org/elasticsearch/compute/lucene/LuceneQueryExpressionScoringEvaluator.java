...
public class LuceneQueryExpressionScoringEvaluator implements EvalOperator.ExpressionEvaluator {

    /**
     * Score to use for false results. We could use zero, but that would be ambiguous with a zero boosting applied to some queries.
     */
    ...
    public LuceneQueryExpressionScoringEvaluator(BlockFactory blockFactory, ShardConfig[] shards) {
    ...
    private class SegmentState {
        ...
        DoubleVector scoreDense(int min, int max) throws IOException {
            int length = max - min + 1;
            if (noMatch) {
                return blockFactory.newConstantDoubleVector(SCORE_FOR_FALSE, length);
            }
        ...
        DoubleVector scoreSparse(IntVector docs) throws IOException {
            ...
            if (noMatch) {
                return blockFactory.newConstantDoubleVector(SCORE_FOR_FALSE, docs.getPositionCount());
            }
            try (DoubleVector.Builder builder = blockFactory.newDoubleVectorFixedBuilder(docs.getPositionCount())) {
                for (int i = 0; i < docs.getPositionCount(); i++) {
        ...
        private void scoreSingleDocWithScorer(DoubleVector.Builder builder, int doc) throws IOException {
            if (scorer.iterator().docID() == doc) {
                ...
                builder.appendDouble(scorer.iterator().advance(doc) == doc ? scorer.score() : SCORE_FOR_FALSE);
            }
        }
    }
    ...
    public static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
        ...
        public EvalOperator.ExpressionEvaluator get(DriverContext context) {
            return new LuceneQueryExpressionScoringEvaluator(context.blockFactory(), shardConfigs);
        }
    }
}

 ...
 */
public final class LuceneTimeSeriesSourceOperator extends LuceneOperator {
    /**
    ...
    public static final class Factory implements LuceneOperator.Factory {
        ...
        public String describe() {
            return "LuceneTimeSeriesSourceOperator[maxPageSize = " + maxPageSize + ", limit = " + limit + "]]";
        }
    }
    ...
    private Page collect() {
        ...
        try {
            if (perShardCollector == null || perShardCollector.shardIndex != scorer.shardIndex()) {
                ...
                perShardCollector = new PerShardCollector(scorer.shardIndex(), scorer.searchContext(), limit);
            }
    ...
    static final class PerShardCollector {
        ...
        PerShardCollector(int shardIndex, SearchContext searchContext, int limit) throws IOException {
            ...
            this.topFieldCollector = TopFieldCollector.create(
                sortAndFormats.get().sort,
                Math.min(searchContext.searcher().getIndexReader().maxDoc(), limit),
                0
            );
        }

...
/**
 * Exchange request for cross-cluster search that carries index context via {@link IndicesRequest.Replaceable}.
 * The original query indices are preserved separately in {@link #originalQueryIndices()} and used by the
 * receiving cluster to validate the exchange sink lookup.
 */
public final class CcsExchangeRequest extends AbstractTransportRequest implements IndicesRequest.Replaceable {
    ...
    public boolean equals(Object obj) {
        return obj == this
            || (obj instanceof CcsExchangeRequest that
                && sourcesFinished == that.sourcesFinished
                && exchangeId.equals(that.exchangeId)
                && Arrays.equals(indices, that.indices)
                && Arrays.equals(originalQueryIndices, that.originalQueryIndices)
                && indicesOptions.equals(that.indicesOptions));
    }

    @Override
    public int hashCode() {
        return Objects.hash(exchangeId, sourcesFinished, Arrays.hashCode(indices), Arrays.hashCode(originalQueryIndices), indicesOptions);
    }

...
public class AllFirstBooleanByLongAggregatorFunctionTests extends AggregatorFunctionTestCase {
    @Override
    protected SourceOperator simpleInput(BlockFactory blockFactory, int size) {
        ...
        return new ListRowsBlockSourceOperator(
            blockFactory,
            List.of(ElementType.BOOLEAN, ElementType.LONG),
            IntStream.range(0, size).mapToObj(l -> List.of(randomBoolean(), tsgen.gen())).toList()
        );
    }

    @Override
    protected AggregatorFunctionSupplier aggregatorFunction() {
        return new AllFirstBooleanByLongAggregatorFunctionSupplier();
    }

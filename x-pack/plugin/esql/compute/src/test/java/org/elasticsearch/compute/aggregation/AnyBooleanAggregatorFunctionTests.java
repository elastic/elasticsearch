...
public class AnyBooleanAggregatorFunctionTests extends AggregatorFunctionTestCase {
    @Override
    ...
    protected SourceOperator simpleInput(BlockFactory blockFactory, int size) {
        return new ListRowsBlockSourceOperator(
            blockFactory,
            List.of(ElementType.BOOLEAN),
            IntStream.range(0, size).mapToObj(l -> List.of(randomBoolean())).toList()
        );
    }

    @Override
    protected AggregatorFunctionSupplier aggregatorFunction() {
        return new AnyBooleanAggregatorFunctionSupplier();
    }
    ...
    public void assertSimpleOutput(List<Page> input, Block result) {
        ...
        FirstLastAggregatorTestingUtils.GroundTruthFirstLastAggregator.check(expected, actual);
    }
}

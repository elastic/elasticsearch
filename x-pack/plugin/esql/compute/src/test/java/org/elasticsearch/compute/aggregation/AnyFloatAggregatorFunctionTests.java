...
public class AnyFloatAggregatorFunctionTests extends AggregatorFunctionTestCase {
    @Override
    ...
    protected SourceOperator simpleInput(BlockFactory blockFactory, int size) {
        return new ListRowsBlockSourceOperator(
            blockFactory,
            List.of(ElementType.FLOAT),
            IntStream.range(0, size).mapToObj(l -> List.of(randomFloat())).toList()
        );
    }

    @Override
    protected AggregatorFunctionSupplier aggregatorFunction() {
        return new AnyFloatAggregatorFunctionSupplier();
    }

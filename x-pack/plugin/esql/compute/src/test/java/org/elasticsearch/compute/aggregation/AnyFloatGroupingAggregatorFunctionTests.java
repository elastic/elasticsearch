...
public class AnyFloatGroupingAggregatorFunctionTests extends GroupingAggregatorFunctionTestCase {
    @Override
    protected SourceOperator simpleInput(BlockFactory blockFactory, int size) {
        return new ListRowsBlockSourceOperator(
            blockFactory,
            List.of(ElementType.LONG, ElementType.FLOAT),
            IntStream.range(0, size).mapToObj(l -> List.of(randomLongBetween(0, 4), randomFloat())).toList()
        );
    }

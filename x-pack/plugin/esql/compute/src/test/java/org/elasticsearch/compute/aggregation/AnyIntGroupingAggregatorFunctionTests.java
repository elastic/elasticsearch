...
public class AnyIntGroupingAggregatorFunctionTests extends GroupingAggregatorFunctionTestCase {
    @Override
    protected SourceOperator simpleInput(BlockFactory blockFactory, int size) {
        return new ListRowsBlockSourceOperator(
            blockFactory,
            List.of(ElementType.LONG, ElementType.INT),
            IntStream.range(0, size).mapToObj(l -> List.of(randomLongBetween(0, 4), randomInt())).toList()
        );
    }
    ...
    protected AggregatorFunctionSupplier aggregatorFunction() {
        return new AnyIntAggregatorFunctionSupplier();
    }

    @Override
    protected String expectedDescriptionOfAggregator() {
        return "any_int_aggregator";
    }

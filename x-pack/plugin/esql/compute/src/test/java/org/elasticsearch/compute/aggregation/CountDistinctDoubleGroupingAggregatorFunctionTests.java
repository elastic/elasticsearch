...
public class CountDistinctDoubleGroupingAggregatorFunctionTests extends GroupingAggregatorFunctionTestCase {
    ...
    protected GroupingAggregatorFunction.Factory aggregatorFunction() {
        return GroupingAggregatorFunction.COUNT_DISTINCT_DOUBLES;
    }
    ...
    protected void assertSimpleGroup(List<Page> input, Block result, int position, long group) {
        ...
        long expected = input.stream().flatMapToDouble(b -> IntStream.range(0, b.getPositionCount()).filter(p -> {
        ...
        }).mapToDouble(p -> ((DoubleBlock) b.getBlock(valueIndex)).getDouble(p))).distinct().count();

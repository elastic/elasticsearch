...
public class LossySumDoubleGroupingAggregatorFunctionTests extends GroupingAggregatorFunctionTestCase {
    @Override
    ...
    protected void assertSimpleGroup(List<Page> input, Block result, int position, Long group) {
        double sum = input.stream().flatMapToDouble(p -> allDoubles(p, group)).sum();
        // Won't precisely match in distributed case but will be close
        assertThat(((DoubleBlock) result).getDouble(position), closeTo(sum, 0.01));
    }
}

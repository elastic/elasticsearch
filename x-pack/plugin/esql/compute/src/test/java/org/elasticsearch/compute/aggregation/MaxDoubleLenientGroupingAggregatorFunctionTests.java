...
/**
 * Grouping tests for the PromQL-only lenient {@code max} aggregator, {@link MaxDoubleLenientAggregator}.
 * <p>
 ...
 * </p>
 */
public class MaxDoubleLenientGroupingAggregatorFunctionTests extends GroupingAggregatorFunctionTestCase {
    ...
    protected void assertSimpleGroup(List<Page> input, Block result, int position, Long group) {
        ...
        assertEquals(lenientMax(values), ((DoubleBlock) result).getDouble(position), 0.0);
    }

    public void testAllNaNProducesNaN() {
        assertSingleGroupMax(List.of(Double.NaN, Double.NaN, Double.NaN), Double.NaN);
    }

    public void testNaNSkippedWhenFinitePresent() {
        assertSingleGroupMax(List.of(Double.NaN, 3.0, Double.NaN, 1.0), 3.0);
    }

    public void testAllNegativeInfinity() {
        assertSingleGroupMax(List.of(Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY), Double.NEGATIVE_INFINITY);
    }

    public void testPositiveInfinityDominates() {
        assertSingleGroupMax(List.of(-2.0, Double.POSITIVE_INFINITY, Double.NaN, 100.0), Double.POSITIVE_INFINITY);
    }

    public void testMixedInfinities() {
        assertSingleGroupMax(List.of(Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, Double.NaN), Double.POSITIVE_INFINITY);
    }

    /**
     * Feeds all values into a single group ({@code 0}) and asserts the aggregated value for that group.
     */
    private void assertSingleGroupMax(List<Double> values, double expected) {
     ...
     */
    private static double lenientMax(double[] values) {

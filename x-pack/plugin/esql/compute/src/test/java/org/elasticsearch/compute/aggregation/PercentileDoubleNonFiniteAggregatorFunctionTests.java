 ...
 * tallied and ranked as {@code NaN < -Inf < finite < +Inf} instead of being rejected by the t-digest.
 * <p>
...
public class PercentileDoubleNonFiniteAggregatorFunctionTests extends AggregatorFunctionTestCase {
    ...
    protected String expectedToStringOfSimpleAggregator() {
        ...
        return "PercentileDoubleAggregatorFunction[channels=[0]]";
    }

    @Override
    protected SourceOperator simpleInput(BlockFactory blockFactory, int size) {
        return new SequenceDoubleBlockSourceOperator(blockFactory, LongStream.range(0, size).mapToDouble(l -> randomMaybeNonFinite()));
    }

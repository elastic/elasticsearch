...
public class PercentileFloatGroupingAggregatorFunctionTests extends GroupingAggregatorFunctionTestCase {
    ...
    protected void assertSimpleGroup(List<Page> input, Block result, int position, Long group) {
        try (TDigestState td = TDigestState.create(newLimitedBreaker(ByteSizeValue.ofMb(100)), QuantileStates.DEFAULT_COMPRESSION)) {
            input.stream().flatMap(p -> allFloats(p, group)).forEach(td::add);
            if (td.size() > 0) {

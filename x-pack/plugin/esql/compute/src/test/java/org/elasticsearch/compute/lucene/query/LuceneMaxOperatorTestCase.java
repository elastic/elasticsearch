...
public abstract class LuceneMaxOperatorTestCase extends LuceneMinMaxOperatorTestCase {
    ...
    private void testMax(Supplier<DriverContext> contexts, int size, int limit) {
        ...
        testMax(contexts, factory, numberTypeTest, size, limit);
    }

    private void testMax(Supplier<DriverContext> contexts, LuceneMaxFactory factory, NumberTypeTest numberTypeTest, int size, int limit) {
    ...
    public void testSkipperFastPathNoDocByDocIteration() throws IOException {
        int size = between(1_000, 20_000);
        NumberTypeTest numberTypeTest = getNumberTypeTest();
        LuceneMaxFactory factory = simpleWithSkipperCheck(numberTypeTest, size);
        testMax(this::driverContext, factory, numberTypeTest, size, Integer.MAX_VALUE);
    }

    private LuceneMaxFactory simpleWithSkipperCheck(NumberTypeTest numberTypeTest, int numDocs) throws IOException {
        try (
            RandomIndexWriter writer = new RandomIndexWriter(
                random(),
                directory,

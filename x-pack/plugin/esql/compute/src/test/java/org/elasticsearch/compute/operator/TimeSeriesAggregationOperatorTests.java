...
public class TimeSeriesAggregationOperatorTests extends ComputeTestCase {
    ...
    public void testRangeForStartAlignedBucketKey() {
        ...
        long roundedBucketTsMillis = DateFieldMapper.Resolution.MILLISECONDS.roundDownToMillis(bucketTs);
        assertEquals(bucketTs, roundedBucketTsMillis);
        long roundedBucketTsMillis1 = DateFieldMapper.Resolution.MILLISECONDS.roundDownToMillis(bucketTs);
        assertEquals(Instant.parse("2024-01-01T12:20:00Z").toEpochMilli(), timeBucket.nextRoundingValue(roundedBucketTsMillis1));
    }
    ...
    public void testRangeForEndAlignedBucketKey() {
        ...
        long roundedBucketTsMillis = DateFieldMapper.Resolution.MILLISECONDS.roundDownToMillis(bucketTs);
        long nextBucket = timeBucket.nextRoundingValue(roundedBucketTsMillis);
        assertEquals(Instant.parse("2024-01-01T12:00:00Z").toEpochMilli(), roundedBucketTsMillis - (nextBucket - roundedBucketTsMillis));
        long roundedBucketTsMillis1 = DateFieldMapper.Resolution.MILLISECONDS.roundDownToMillis(bucketTs);

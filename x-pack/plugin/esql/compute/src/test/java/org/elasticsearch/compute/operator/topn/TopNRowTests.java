...
public class TopNRowTests extends ESTestCase {
    ...
    public void testRamBytesUsedEmpty() {
        TopNOperator.Row row = new TopNOperator.Row(breaker, sortOrders(), 0);
        assertThat(row.ramBytesUsed(), equalTo(expectedRamBytesUsed(row)));
    }

    public void testRamBytesUsedSmall() {
        TopNOperator.Row row = new TopNOperator.Row(new NoopCircuitBreaker(CircuitBreaker.REQUEST), sortOrders(), 0);
    ...
    public void testRamBytesUsedPreAllocated() {
        TopNOperator.Row row = new TopNOperator.Row(new NoopCircuitBreaker(CircuitBreaker.REQUEST), sortOrders(), 1024);
        assertThat(row.ramBytesUsed(), equalTo(expectedRamBytesUsed(row)));
    }

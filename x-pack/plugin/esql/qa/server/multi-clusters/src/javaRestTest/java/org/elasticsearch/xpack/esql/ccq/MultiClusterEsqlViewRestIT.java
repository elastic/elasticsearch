...
public class MultiClusterEsqlViewRestIT extends ESRestTestCase {
    ...
    public void testLocalView() throws IOException {
        assumeTrue("views not supported", hasEsqlCapability(client(), Cap.VIEWS_CRUD_AS_INDEX_ACTIONS));
    ...
    public void testRemoteView() throws IOException {
        assumeTrue("views not supported", hasEsqlCapability(client(), Cap.VIEWS_CRUD_AS_INDEX_ACTIONS));
        assumeTrue("views not supported on remote cluster", hasEsqlCapability(remoteClusterClient(), Cap.VIEWS_CRUD_AS_INDEX_ACTIONS));
        final boolean viewsNotDiscoverableOnRemotes = hasEsqlCapability(client(), Cap.VIEWS_NOT_DISCOVERABLE_ON_REMOTES);
        ...
        // existing view matched using concrete expression
        if (viewsNotDiscoverableOnRemotes) {
            expectResponseException(
                400,
                containsString("Unknown index [remote_cluster:view]"),
        ...
        // existing view matched by pattern expression
        if (viewsNotDiscoverableOnRemotes) {
            // skipped when resolving by patterns
            assertOK(runEsqlQuery(client(), "FROM " + REMOTE_CLUSTER_NAME + ":view*"));
        } else {
            expectResponseException(
                400,
                containsString("ES|QL queries with remote views are not supported"),
    ...
    private boolean hasEsqlCapability(RestClient client, Cap cap) throws IOException {
        return clusterHasCapability(client, "POST", "/_query", List.of(), List.of(cap.capabilityName())).orElse(false);
    }

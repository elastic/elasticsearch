 ...
 *                    MATCH, MATCH_PHRASE, and the {@code :} operator can be pushed down to Lucene on index-mapped
 *                    fields, or run as a row-by-row runtime search on keyword/text expressions. Runtime options
 *                    are allowed only on TEXT.

...
public class CCSAsyncEsqlQueryActionIT extends ESTestCase /*AbstractPausableIntegTestCase*/ {
    ...
    protected Collection<Class<? extends Plugin>> nodePlugins() {
        ...
        ArrayList<Class<? extends Plugin>> actions = new ArrayList<>();
        actions.add(EsqlPlugin.class);  // MP TODO: should be able to remove this once I use the MultiClusterTestCase parent class?
        actions.add(EsqlAsyncActionIT.LocalStateEsqlAsync.class); // allows the async_search DELETE action
        actions.add(InternalExchangePlugin.class);   // MP TODO: might be able to remove once we have MultiClusters parent class?

        actions.add(PausableFieldPlugin.class);
    ...
    public void testBasicAsyncExecution() throws Exception {
        try (var initialResponse = sendAsyncQuery()) {
            System.err.println("init Response: " + Strings.toString(initialResponse));
            ...
            if (true) {
                ...
                try (var responseWithTimeout = future.get()) {
                    ...
                    System.err.println("responseWithTimeout: " + Strings.toString(responseWithTimeout));
                }
            }
            ...
            final int numPermitsReleased = numberOfDocs();
            System.err.println("releasing permits: " + numPermitsReleased);
            // release the permits to allow the query to proceed
            scriptPermits.release(numPermitsReleased);
            ...
            try (var finalResponse = future.get()) {
                ...
                System.err.println("Final Response: " + Strings.toString(finalResponse));
            }
            ...
            try (var finalResponse = again.get()) {
                ...
                assertThat(finalResponse.columns(), equalTo(List.of(new ColumnInfoImpl("sum(pause_me)", "long"))));
                assertThat(getValuesList(finalResponse).size(), equalTo(1));
                System.err.println("Stored Response: " + Strings.toString(finalResponse));
            }
    ...
    // MP TODO: might be able to remove once we have MultiClusters parent class?

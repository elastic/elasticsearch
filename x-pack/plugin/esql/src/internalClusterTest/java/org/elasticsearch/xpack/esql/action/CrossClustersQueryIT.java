    ...
    public void testSearchesWhereMissingIndicesAreSpecified() {
...

        {
            ...
            if (skipUnavailable == false) {
                ...
                IndexNotFoundException exception = expectThrows(IndexNotFoundException.class, () -> runQuery(query));
        ...
        // since the remote cluster has a valid index expression, the missing local index is ignored and local cluster is marked as SKIPPED
        try (EsqlQueryResponse resp = runQuery("from nomatch,*:logs-* | stats count(*) by tag | sort tag | keep tag")) {
            List<List<Object>> values = getValuesList(resp);
            assertThat(values, hasSize(1));
...

        {
            ...
            if (skipUnavailable == false) {
                ...
                // when multiple wildcard non-matching index expressions are specified on the remote,
                // the remote should be SKIPPED with an indication that zero shards were searched and a failure message
                try (EsqlQueryResponse resp = runQuery(query)) {
                    List<List<Object>> values = getValuesList(resp);
                    assertThat(values, hasSize(1));
                    assertThat(values.get(0), equalTo(List.of("local")));

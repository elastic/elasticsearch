...
/**
 * Centralizes the fetch-after-TopN integration contract so the concrete fixtures exercise identical behavior under different cluster
 * topologies.
 */
public abstract class EsqlFetchTopNTestCase extends AbstractEsqlIntegTestCase {
    ...
    public void setupIndex() {
        indexName = "fetch_topn_" + getTestName().toLowerCase(Locale.ROOT).replaceAll("[^a-z0-9]+", "_");
        client().admin()
            .indices()
    ...
    public void testBasicFetchTopN() {
        try (
    ...
    public void testFetchTopNWithoutKeepPreservesOutput() {
        ...
        try (EsqlQueryResponse response = runQuery("FROM " + regressionIndex + " | SORT unique_sort + 1 DESC | LIMIT 3", true)) {
            ...
            assertFetchRows(response, 3);
            assertFieldLoadedBeforeFetch(response, "unique_sort");
            assertFieldNotLoadedBeforeFetch(response, "payload");
        }
    }

    public void testInitialComputePreservesQueryWarnings() {
        String warningIndex = indexName + "_query_warning";
        client().admin()
            .indices()
            .prepareCreate(warningIndex)
            .setSettings(indexSettings(4, 0))
            .setMapping("unique_sort", "type=long", "filter_value", "type=keyword", "payload", "type=keyword")
            .get();

        client().prepareBulk()
            .add(prepareIndex(warningIndex).setSource("unique_sort", 0, "filter_value", "match", "payload", "payload-0"))
            .add(prepareIndex(warningIndex).setSource("unique_sort", 1, "filter_value", List.of("match", "other"), "payload", "payload-1"))
            .add(prepareIndex(warningIndex).setSource("unique_sort", 2, "filter_value", "match", "payload", "payload-2"))
            .setRefreshPolicy(WriteRequest.RefreshPolicy.IMMEDIATE)
            .get();

        ResponseListener listener = new ResponseListener(client().threadPool());
        client().execute(
            EsqlQueryAction.INSTANCE,
            queryRequest(
                "FROM " + warningIndex + " | WHERE filter_value == \"match\" | SORT unique_sort DESC | LIMIT 2 | KEEP payload",
                true
            ),
            listener
        );
        try (EsqlQueryResponse response = listener.actionGet(1, TimeUnit.MINUTES)) {
            assertThat(EsqlTestUtils.getValuesList(response), equalTo(List.of(List.of("payload-2"), List.of("payload-0"))));
            assertFetchRows(response, 2);
        }
        assertThat(listener.warnings, hasItem(containsString("single-value function encountered multi-value")));
    }

    public void testMultipleSortKeysFetchTopN() {
        try (
            EsqlQueryResponse response = runQuery(
    ...
    public void testBinaryFetchHandlesSurviveTopN() {
        ...
        try (
            ...
            assertFetchRows(response, 300);
            assertFieldLoadedBeforeFetch(response, "unique_sort");
            assertFieldNotLoadedBeforeFetch(response, "payload");
        }
    }

    public void testFetchTopNDisabledWithoutPragma() {
        try (
            EsqlQueryResponse response = runQuery(
                "FROM " + indexName + " | SORT unique_sort + 1 DESC | LIMIT 5 | KEEP unique_sort, payload, category",
                false
            ...
            assertThat(fetchStatuses(response), empty());
            assertFieldLoadedBeforeFetch(response, "unique_sort");
    ...
    public void testUserFieldNamedLikeFetchHandle() {
        try (
            EsqlQueryResponse response = runQuery(
                "FROM " + indexName + " | SORT unique_sort + 1 DESC | LIMIT 3 | KEEP `_fetch_handle`, payload",
                true
            )
        ) {
            assertThat(
            ...
            assertFetchRows(response, 3);
            assertFieldLoadedBeforeFetch(response, "unique_sort");
            assertFieldNotLoadedBeforeFetch(response, "_fetch_handle");
            assertFieldNotLoadedBeforeFetch(response, "payload");
        }
    }

    public void testMultipleTopNsRunSingleFetchAtFirstExchangeTopN() {
        try (
            EsqlQueryResponse response = runQuery(
                "FROM "
                    + indexName
            ...
            assertThat(fetchStatuses(response), hasSize(1));
            assertThat(fetchRowsEmitted(response), equalTo(20L));
            assertFieldLoadedBeforeFetch(response, "unique_sort");
    ...
    public void testNoFetchWhenNoDeferredFields() {
        try (
    ...
    public void testFetchSourceOnlyField() {
        try (
            EsqlQueryResponse response = runQuery(
    ...
    public void testExpressionAfterTopNUsesFetchedField() {
        try (
            EsqlQueryResponse response = runQuery(
                "FROM "
                    + indexName
    ...
    public void testExpressionBeforeTopNIsNotFetched() {
        try (
            EsqlQueryResponse response = runQuery(
                "FROM "
                    + indexName
    ...
    public void testNoFetchAfterAggregation() {
        try (
            EsqlQueryResponse response = runQuery(
    ...
    public void testPotentiallyUnmappedFieldDoesNotUseFetch() {
    ...
    public void testUnionTypedFieldDoesNotUseFetch() {
    ...
    private EsqlQueryResponse runQuery(String query, boolean fetchTopN) {
        return client().execute(EsqlQueryAction.INSTANCE, queryRequest(query, fetchTopN)).actionGet(1, TimeUnit.MINUTES);
    }

    private static EsqlQueryRequest queryRequest(String query, boolean fetchTopN) {
        ...
        if (fetchTopN) {
            pragmas.put(QueryPragmas.FETCH_TOPN.getKey(), true);
        }
        return syncEsqlQueryRequest(query).acceptedPragmaRisks(true).pragmas(new QueryPragmas(pragmas.build())).profile(true);
    }

    private static void assertFetchRows(EsqlQueryResponse response, int rowsEmitted) {
        List<FetchOperator.Status> statuses = fetchStatuses(response);
        assertThat(statuses, not(empty()));
        assertThat(fetchRowsEmitted(response), equalTo((long) rowsEmitted));
        assertThat(statuses.stream().mapToLong(FetchOperator.Status::batchesSent).sum(), greaterThan(0L));
        assertThat(statuses.stream().mapToInt(FetchOperator.Status::exchangesOpened).sum(), greaterThan(0));
    }

    private static long fetchRowsEmitted(EsqlQueryResponse response) {
        return fetchStatuses(response).stream().mapToLong(FetchOperator.Status::rowsEmitted).sum();
    }

    private static List<FetchOperator.Status> fetchStatuses(EsqlQueryResponse response) {
    ...
    private static class ResponseListener extends PlainActionFuture<EsqlQueryResponse> {
        ...
        private ResponseListener(ThreadPool threadPool) {
        ...
        public void onResponse(EsqlQueryResponse response) {
            response.mustIncRef();
            warnings = List.copyOf(threadPool.getThreadContext().getResponseHeaders().getOrDefault("Warning", List.of()));
            if (set(response) == false) {
                response.decRef();
            }
        }
    }

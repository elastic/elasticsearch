...
public class EsqlSearchActionIT extends AbstractEsqlIntegTestCase {
    ...
    public void testFilterRangeQuery() {
        ...
        try (var resp = run(query)) {
            ...
            assertThat(resp.columns().stream().map(ColumnInfoImpl::type).map(DataType::toString).toList(), contains("INTEGER", "FLOAT"));
            // values
    ...
    public void testRankRangeQueryDropScore() {
        ...
        try (var resp = run(query)) {
            ...
            assertThat(resp.columns().stream().map(ColumnInfoImpl::type).map(DataType::toString).toList(), contains("INTEGER", "TEXT"));
            // values
            List<List<Object>> values = getValuesList(resp);
    ...
    public void testWithMultipleRank() {
        var query = """
            SEARCH test [
              | WHERE id > 2
              | RANK MATCH(content, "quick brown dog")
              | RANK MATCH(content, "brown fox")
              | RANK MATCH(content, "brown dog")
              ]
            | KEEP id, _score, content
            """;   // default rank sort is score descending

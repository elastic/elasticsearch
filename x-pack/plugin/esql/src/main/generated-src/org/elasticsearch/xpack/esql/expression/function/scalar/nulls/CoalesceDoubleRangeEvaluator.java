 ...
 */
abstract sealed class CoalesceDoubleRangeEvaluator implements ExpressionEvaluator permits // checkstyle hack
    CoalesceDoubleRangeEvaluator.CoalesceDoubleRangeEagerEvaluator, // checkstyle hack
    CoalesceDoubleRangeEvaluator.CoalesceDoubleRangeLazyEvaluator {

    private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(CoalesceDoubleRangeEvaluator.class);
        ...
        if (childEvaluators.stream().allMatch(ExpressionEvaluator.Factory::eagerEvalSafeInLazy)) {
            return new ExpressionEvaluator.Factory() {
                ...
                public ExpressionEvaluator get(DriverContext context) {
                    return new CoalesceDoubleRangeEagerEvaluator(
                ...
                public String toString() {
                    return "CoalesceDoubleRangeEagerEvaluator[values=" + childEvaluators + ']';
                }
            };
        }
        return new ExpressionEvaluator.Factory() {
            @Override
            public ExpressionEvaluator get(DriverContext context) {
                return new CoalesceDoubleRangeLazyEvaluator(
                    // comment to make spotless happy about line breaks
                    context,
                    childEvaluators.stream().map(x -> x.get(context)).toList()
                );
            ...
            @Override
            public String toString() {
                return "CoalesceDoubleRangeLazyEvaluator[values=" + childEvaluators + ']';
            }
        };
    }
    ...
    protected CoalesceDoubleRangeEvaluator(DriverContext driverContext, List<ExpressionEvaluator> evaluators) {
        this.driverContext = driverContext;
        this.evaluators = evaluators;
    }

    @Override
    public final DoubleRangeBlock eval(Page page) {
     ...
     * </ul>
     */
    private DoubleRangeBlock entireBlock(Page page) {
        int lastFullBlockIdx = 0;
        while (true) {
            DoubleRangeBlock lastFullBlock = (DoubleRangeBlock) evaluators.get(lastFullBlockIdx++).eval(page);
     ...
     * </p>
     */
    protected abstract DoubleRangeBlock perPosition(Page page, DoubleRangeBlock lastFullBlock, int firstToEvaluate);
     ...
     * {@link CoalesceDoubleRangeLazyEvaluator} but will include spurious warnings if any of the
     ...
     */
    static final class CoalesceDoubleRangeEagerEvaluator extends CoalesceDoubleRangeEvaluator {
        CoalesceDoubleRangeEagerEvaluator(DriverContext driverContext, List<ExpressionEvaluator> evaluators) {
            super(driverContext, evaluators);
        }

        @Override
        protected DoubleRangeBlock perPosition(Page page, DoubleRangeBlock lastFullBlock, int firstToEvaluate) {
            int positionCount = page.getPositionCount();
            DoubleRangeBlock[] flatten = new DoubleRangeBlock[evaluators.size() - firstToEvaluate + 1];
            try {
                flatten[0] = lastFullBlock;
                for (int f = 1; f < flatten.length; f++) {
                    flatten[f] = (DoubleRangeBlock) evaluators.get(firstToEvaluate + f - 1).eval(page);
                }
                try (
                    DoubleRangeBlock.Builder result = driverContext.blockFactory() //
                        .newDoubleRangeBlockBuilder(positionCount)
                ) {
                    position: for (int p = 0; p < positionCount; p++) {
                        for (DoubleRangeBlock f : flatten) {
                            if (false == f.isNull(p)) {
                                result.copyFrom(f, p);
                                continue position;
                            }
     ...
     * </ul>
     */
    static final class CoalesceDoubleRangeLazyEvaluator extends CoalesceDoubleRangeEvaluator {
        CoalesceDoubleRangeLazyEvaluator(DriverContext driverContext, List<ExpressionEvaluator> evaluators) {
            super(driverContext, evaluators);
        }
                ...
                position: for (int p = 0; p < positionCount; p++) {
                    ...
                    try (Releasable ignored = limited::releaseBlocks) {
                        for (int e = firstToEvaluate; e < evaluators.size(); e++) {
                            try (DoubleRangeBlock block = (DoubleRangeBlock) evaluators.get(e).eval(limited)) {

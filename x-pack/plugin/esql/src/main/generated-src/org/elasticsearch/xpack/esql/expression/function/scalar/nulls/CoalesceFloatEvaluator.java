 ...
 */
abstract sealed class CoalesceFloatEvaluator implements EvalOperator.ExpressionEvaluator permits
    CoalesceFloatEvaluator.CoalesceFloatEagerEvaluator, //
    CoalesceFloatEvaluator.CoalesceFloatLazyEvaluator {

    private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(CoalesceFloatEvaluator.class);
        ...
        if (childEvaluators.stream().allMatch(ExpressionEvaluator.Factory::eagerEvalSafeInLazy)) {
            return new ExpressionEvaluator.Factory() {
                ...
                public ExpressionEvaluator get(DriverContext context) {
                    return new CoalesceFloatEagerEvaluator(
                ...
                public String toString() {
                    return "CoalesceFloatEagerEvaluator[values=" + childEvaluators + ']';
                }
            };
        }
        return new ExpressionEvaluator.Factory() {
            @Override
            public ExpressionEvaluator get(DriverContext context) {
                return new CoalesceFloatLazyEvaluator(
                    // comment to make spotless happy about line breaks
                    context,
                    childEvaluators.stream().map(x -> x.get(context)).toList()
                );
            ...
            @Override
            public String toString() {
                return "CoalesceFloatLazyEvaluator[values=" + childEvaluators + ']';
            }
        };
    }
    ...
    protected CoalesceFloatEvaluator(DriverContext driverContext, List<EvalOperator.ExpressionEvaluator> evaluators) {
        this.driverContext = driverContext;
        this.evaluators = evaluators;
    }

    @Override
    public final FloatBlock eval(Page page) {
     ...
     * </ul>
     */
    private FloatBlock entireBlock(Page page) {
        int lastFullBlockIdx = 0;
        while (true) {
            FloatBlock lastFullBlock = (FloatBlock) evaluators.get(lastFullBlockIdx++).eval(page);
     ...
     * </p>
     */
    protected abstract FloatBlock perPosition(Page page, FloatBlock lastFullBlock, int firstToEvaluate);
     ...
     * {@link CoalesceFloatLazyEvaluator} but will include spurious warnings if any of the
     ...
     */
    static final class CoalesceFloatEagerEvaluator extends CoalesceFloatEvaluator {
        CoalesceFloatEagerEvaluator(DriverContext driverContext, List<EvalOperator.ExpressionEvaluator> evaluators) {
            super(driverContext, evaluators);
        }

        @Override
        protected FloatBlock perPosition(Page page, FloatBlock lastFullBlock, int firstToEvaluate) {
            int positionCount = page.getPositionCount();
            FloatBlock[] flatten = new FloatBlock[evaluators.size() - firstToEvaluate + 1];
            try {
                flatten[0] = lastFullBlock;
                for (int f = 1; f < flatten.length; f++) {
                    flatten[f] = (FloatBlock) evaluators.get(firstToEvaluate + f - 1).eval(page);
                }
                try (
                    FloatBlock.Builder result = driverContext.blockFactory() //
                        .newFloatBlockBuilder(positionCount)
                ) {
                    position: for (int p = 0; p < positionCount; p++) {
                        for (FloatBlock f : flatten) {
                            if (false == f.isNull(p)) {
                                result.copyFrom(f, p);
                                continue position;
                            }
     ...
     * </ul>
     */
    static final class CoalesceFloatLazyEvaluator extends CoalesceFloatEvaluator {
        CoalesceFloatLazyEvaluator(DriverContext driverContext, List<EvalOperator.ExpressionEvaluator> evaluators) {
            super(driverContext, evaluators);
        }
                ...
                position: for (int p = 0; p < positionCount; p++) {
                    ...
                    try (Releasable ignored = limited::releaseBlocks) {
                        for (int e = firstToEvaluate; e < evaluators.size(); e++) {
                            try (FloatBlock block = (FloatBlock) evaluators.get(e).eval(limited)) {

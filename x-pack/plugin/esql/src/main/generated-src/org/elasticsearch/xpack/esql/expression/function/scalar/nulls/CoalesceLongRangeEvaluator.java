 ...
 */
abstract sealed class CoalesceLongRangeEvaluator implements ExpressionEvaluator permits // checkstyle hack
    CoalesceLongRangeEvaluator.CoalesceLongRangeEagerEvaluator, // checkstyle hack
    CoalesceLongRangeEvaluator.CoalesceLongRangeLazyEvaluator {

    private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(CoalesceLongRangeEvaluator.class);
        ...
        if (childEvaluators.stream().allMatch(ExpressionEvaluator.Factory::eagerEvalSafeInLazy)) {
            return new ExpressionEvaluator.Factory() {
                ...
                public ExpressionEvaluator get(DriverContext context) {
                    return new CoalesceLongRangeEagerEvaluator(
                ...
                public String toString() {
                    return "CoalesceLongRangeEagerEvaluator[values=" + childEvaluators + ']';
                }
            };
        }
        return new ExpressionEvaluator.Factory() {
            @Override
            public ExpressionEvaluator get(DriverContext context) {
                return new CoalesceLongRangeLazyEvaluator(
                    // comment to make spotless happy about line breaks
                    context,
                    childEvaluators.stream().map(x -> x.get(context)).toList()
                );
            ...
            @Override
            public String toString() {
                return "CoalesceLongRangeLazyEvaluator[values=" + childEvaluators + ']';
            }
        };
    }
    ...
    protected CoalesceLongRangeEvaluator(DriverContext driverContext, List<ExpressionEvaluator> evaluators) {
        this.driverContext = driverContext;
        this.evaluators = evaluators;
    }

    @Override
    public final LongRangeBlock eval(Page page) {
     ...
     * </ul>
     */
    private LongRangeBlock entireBlock(Page page) {
        int lastFullBlockIdx = 0;
        while (true) {
            LongRangeBlock lastFullBlock = (LongRangeBlock) evaluators.get(lastFullBlockIdx++).eval(page);
     ...
     * </p>
     */
    protected abstract LongRangeBlock perPosition(Page page, LongRangeBlock lastFullBlock, int firstToEvaluate);
     ...
     * {@link CoalesceLongRangeLazyEvaluator} but will include spurious warnings if any of the
     ...
     */
    static final class CoalesceLongRangeEagerEvaluator extends CoalesceLongRangeEvaluator {
        CoalesceLongRangeEagerEvaluator(DriverContext driverContext, List<ExpressionEvaluator> evaluators) {
            super(driverContext, evaluators);
        }

        @Override
        protected LongRangeBlock perPosition(Page page, LongRangeBlock lastFullBlock, int firstToEvaluate) {
            int positionCount = page.getPositionCount();
            LongRangeBlock[] flatten = new LongRangeBlock[evaluators.size() - firstToEvaluate + 1];
            try {
                flatten[0] = lastFullBlock;
                for (int f = 1; f < flatten.length; f++) {
                    flatten[f] = (LongRangeBlock) evaluators.get(firstToEvaluate + f - 1).eval(page);
                }
                try (
                    LongRangeBlock.Builder result = driverContext.blockFactory() //
                        .newLongRangeBlockBuilder(positionCount)
                ) {
                    position: for (int p = 0; p < positionCount; p++) {
                        for (LongRangeBlock f : flatten) {
                            if (false == f.isNull(p)) {
                                result.copyFrom(f, p);
                                continue position;
                            }
     ...
     * </ul>
     */
    static final class CoalesceLongRangeLazyEvaluator extends CoalesceLongRangeEvaluator {
        CoalesceLongRangeLazyEvaluator(DriverContext driverContext, List<ExpressionEvaluator> evaluators) {
            super(driverContext, evaluators);
        }
                ...
                position: for (int p = 0; p < positionCount; p++) {
                    ...
                    try (Releasable ignored = limited::releaseBlocks) {
                        for (int e = firstToEvaluate; e < evaluators.size(); e++) {
                            try (LongRangeBlock block = (LongRangeBlock) evaluators.get(e).eval(limited)) {

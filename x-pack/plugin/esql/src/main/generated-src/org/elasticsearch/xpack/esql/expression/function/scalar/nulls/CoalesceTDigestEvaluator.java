 ...
 */
abstract sealed class CoalesceTDigestEvaluator implements EvalOperator.ExpressionEvaluator permits
    CoalesceTDigestEvaluator.CoalesceTDigestEagerEvaluator, //
    CoalesceTDigestEvaluator.CoalesceTDigestLazyEvaluator {

    private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(CoalesceTDigestEvaluator.class);
        ...
        if (childEvaluators.stream().allMatch(ExpressionEvaluator.Factory::eagerEvalSafeInLazy)) {
            return new ExpressionEvaluator.Factory() {
                ...
                public ExpressionEvaluator get(DriverContext context) {
                    return new CoalesceTDigestEagerEvaluator(
                ...
                public String toString() {
                    return "CoalesceTDigestEagerEvaluator[values=" + childEvaluators + ']';
                }
            };
        }
        return new ExpressionEvaluator.Factory() {
            @Override
            public ExpressionEvaluator get(DriverContext context) {
                return new CoalesceTDigestLazyEvaluator(
                    // comment to make spotless happy about line breaks
                    context,
                    childEvaluators.stream().map(x -> x.get(context)).toList()
                );
            ...
            @Override
            public String toString() {
                return "CoalesceTDigestLazyEvaluator[values=" + childEvaluators + ']';
            }
        };
    }
    ...
    protected CoalesceTDigestEvaluator(DriverContext driverContext, List<EvalOperator.ExpressionEvaluator> evaluators) {
        this.driverContext = driverContext;
        this.evaluators = evaluators;
    }

    @Override
    public final TDigestBlock eval(Page page) {
     ...
     * </ul>
     */
    private TDigestBlock entireBlock(Page page) {
        int lastFullBlockIdx = 0;
        while (true) {
            TDigestBlock lastFullBlock = (TDigestBlock) evaluators.get(lastFullBlockIdx++).eval(page);
     ...
     * </p>
     */
    protected abstract TDigestBlock perPosition(Page page, TDigestBlock lastFullBlock, int firstToEvaluate);
     ...
     * {@link CoalesceTDigestLazyEvaluator} but will include spurious warnings if any of the
     ...
     */
    static final class CoalesceTDigestEagerEvaluator extends CoalesceTDigestEvaluator {
        CoalesceTDigestEagerEvaluator(DriverContext driverContext, List<EvalOperator.ExpressionEvaluator> evaluators) {
            super(driverContext, evaluators);
        }

        @Override
        protected TDigestBlock perPosition(Page page, TDigestBlock lastFullBlock, int firstToEvaluate) {
            int positionCount = page.getPositionCount();
            TDigestBlock[] flatten = new TDigestBlock[evaluators.size() - firstToEvaluate + 1];
            try {
                flatten[0] = lastFullBlock;
                for (int f = 1; f < flatten.length; f++) {
                    flatten[f] = (TDigestBlock) evaluators.get(firstToEvaluate + f - 1).eval(page);
                }
                try (
                    TDigestBlock.Builder result = driverContext.blockFactory() //
                        .newTDigestBlockBuilder(positionCount)
                ) {
                    position: for (int p = 0; p < positionCount; p++) {
                        for (TDigestBlock f : flatten) {
                            if (false == f.isNull(p)) {
                                result.copyFrom(f, p);
                                continue position;
                            }
     ...
     * </ul>
     */
    static final class CoalesceTDigestLazyEvaluator extends CoalesceTDigestEvaluator {
        CoalesceTDigestLazyEvaluator(DriverContext driverContext, List<EvalOperator.ExpressionEvaluator> evaluators) {
            super(driverContext, evaluators);
        }
                ...
                position: for (int p = 0; p < positionCount; p++) {
                    ...
                    try (Releasable ignored = limited::releaseBlocks) {
                        for (int e = firstToEvaluate; e < evaluators.size(); e++) {
                            try (TDigestBlock block = (TDigestBlock) evaluators.get(e).eval(limited)) {

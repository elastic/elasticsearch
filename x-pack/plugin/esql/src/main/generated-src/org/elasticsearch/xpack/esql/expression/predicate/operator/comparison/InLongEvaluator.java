 ...
 */
public class InLongEvaluator implements ExpressionEvaluator {
    ...
    public InLongEvaluator(Source source, ExpressionEvaluator lhs, ExpressionEvaluator[] rhs, DriverContext driverContext) {
        this.source = source;
        this.lhs = lhs;
        this.rhs = rhs;
    ...
    private BooleanBlock eval(int positionCount, LongVector lhsVector, LongVector[] rhsVectors) {
        try (BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
            ...
            for (int p = 0; p < positionCount; p++) {
                ...
                result.appendBoolean(In.process(null, null, lhsVector.getLong(p), rhsValues));
            }
            return result.build();
        }
    }

...
public final class DecayDatetimeEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public DoubleBlock eval(int positionCount, LongVector valueVector, LongVector originVector,
    ...
    try(DoubleBlock.Builder result = driverContext.blockFactory().newDoubleBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        try {
          result.appendDouble(Decay.processDatetime(valueVector.getLong(p), originVector.getLong(p), scaleVector.getLong(p), offsetVector.getLong(p), decayVector.getDouble(p), functionTypeVector.getBytesRef(p, functionTypeScratch)));
        } catch (InvalidArgumentException | IllegalArgumentException e) {
          warnings().registerException(e);
          result.appendNull();
        }

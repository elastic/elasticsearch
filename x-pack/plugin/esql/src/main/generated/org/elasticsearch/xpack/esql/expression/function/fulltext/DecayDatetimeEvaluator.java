...
public final class DecayDatetimeEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public DecayDatetimeEvaluator(Source source, EvalOperator.ExpressionEvaluator value,
  ...
  public DoubleVector eval(int positionCount, LongVector valueVector, LongVector originVector,
    ...
    try(DoubleVector.FixedBuilder result = driverContext.blockFactory().newDoubleVectorFixedBuilder(positionCount)) {
      ...
      BytesRef functionTypeScratch = new BytesRef();
      position: for (int p = 0; p < positionCount; p++) {
        result.appendDouble(p, Decay.processDatetime(valueVector.getLong(p), originVector.getLong(p), scaleVector.getBytesRef(p, scaleScratch), offsetVector.getBytesRef(p, offsetScratch), decayVector.getDouble(p), functionTypeVector.getBytesRef(p, functionTypeScratch)));
      }
      return result.build();
    }
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public DecayDatetimeEvaluator get(DriverContext context) {
      return new DecayDatetimeEvaluator(source, value.get(context), origin.get(context), scale.get(context), offset.get(context), decay.get(context), functionType.get(context), context);
    }

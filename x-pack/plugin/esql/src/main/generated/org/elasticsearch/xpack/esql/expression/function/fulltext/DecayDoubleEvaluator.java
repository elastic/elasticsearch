...
public final class DecayDoubleEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public DecayDoubleEvaluator(Source source, EvalOperator.ExpressionEvaluator value,
  ...
  public Block eval(Page page) {
    try (DoubleBlock valueBlock = (DoubleBlock) value.eval(page)) {
      try (DoubleBlock originBlock = (DoubleBlock) origin.eval(page)) {
          ...
          try (DoubleBlock offsetBlock = (DoubleBlock) offset.eval(page)) {
            try (DoubleBlock decayBlock = (DoubleBlock) decay.eval(page)) {
              try (BytesRefBlock functionTypeBlock = (BytesRefBlock) functionType.eval(page)) {
                ...
                DoubleVector originVector = originBlock.asVector();
  ...
  public DoubleBlock eval(int positionCount, DoubleBlock valueBlock, DoubleBlock originBlock,
    ...
    try(DoubleBlock.Builder result = driverContext.blockFactory().newDoubleBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendDouble(Decay.process(valueBlock.getDouble(valueBlock.getFirstValueIndex(p)), originBlock.getDouble(originBlock.getFirstValueIndex(p)), scaleBlock.getDouble(scaleBlock.getFirstValueIndex(p)), offsetBlock.getDouble(offsetBlock.getFirstValueIndex(p)), decayBlock.getDouble(decayBlock.getFirstValueIndex(p)), functionTypeBlock.getBytesRef(functionTypeBlock.getFirstValueIndex(p), functionTypeScratch)));
      }
      return result.build();
    }
  }

  public DoubleVector eval(int positionCount, DoubleVector valueVector, DoubleVector originVector,
      DoubleVector scaleVector, DoubleVector offsetVector, DoubleVector decayVector,
      BytesRefVector functionTypeVector) {
    try(DoubleVector.FixedBuilder result = driverContext.blockFactory().newDoubleVectorFixedBuilder(positionCount)) {
      BytesRef functionTypeScratch = new BytesRef();
      position: for (int p = 0; p < positionCount; p++) {
        result.appendDouble(p, Decay.process(valueVector.getDouble(p), originVector.getDouble(p), scaleVector.getDouble(p), offsetVector.getDouble(p), decayVector.getDouble(p), functionTypeVector.getBytesRef(p, functionTypeScratch)));
      }
      return result.build();
    }
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public DecayDoubleEvaluator get(DriverContext context) {
      return new DecayDoubleEvaluator(source, value.get(context), origin.get(context), scale.get(context), offset.get(context), decay.get(context), functionType.get(context), context);
    }

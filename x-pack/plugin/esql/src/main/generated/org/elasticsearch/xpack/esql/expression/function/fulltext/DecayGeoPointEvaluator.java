...
public final class DecayGeoPointEvaluator implements EvalOperator.ExpressionEvaluator {
          ...
          try (BytesRefBlock offsetBlock = (BytesRefBlock) offset.eval(page)) {
            try (DoubleBlock decayBlock = (DoubleBlock) decay.eval(page)) {
              try (BytesRefBlock functionTypeBlock = (BytesRefBlock) functionType.eval(page)) {
                ...
                BytesRefVector originVector = originBlock.asVector();
  ...
  public DoubleVector eval(int positionCount, BytesRefVector valueVector,
    ...
    try(DoubleVector.FixedBuilder result = driverContext.blockFactory().newDoubleVectorFixedBuilder(positionCount)) {
      ...
      BytesRef functionTypeScratch = new BytesRef();
      position: for (int p = 0; p < positionCount; p++) {
        result.appendDouble(p, Decay.process(valueVector.getBytesRef(p, valueScratch), originVector.getBytesRef(p, originScratch), scaleVector.getBytesRef(p, scaleScratch), offsetVector.getBytesRef(p, offsetScratch), decayVector.getDouble(p), functionTypeVector.getBytesRef(p, functionTypeScratch)));
      }
      return result.build();
    }
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public DecayGeoPointEvaluator get(DriverContext context) {
      return new DecayGeoPointEvaluator(source, value.get(context), origin.get(context), scale.get(context), offset.get(context), decay.get(context), functionType.get(context), context);
    }

 ...
 */
public final class DecayLongOffsetLongEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public DecayLongOffsetLongEvaluator(Source source, EvalOperator.ExpressionEvaluator value,
  ...
  public DoubleVector eval(int positionCount, LongVector valueVector, LongVector originVector,
    ...
    try(DoubleVector.FixedBuilder result = driverContext.blockFactory().newDoubleVectorFixedBuilder(positionCount)) {
      BytesRef functionTypeScratch = new BytesRef();
      position: for (int p = 0; p < positionCount; p++) {
        result.appendDouble(p, Decay.process(valueVector.getLong(p), originVector.getLong(p), scaleVector.getLong(p), offsetVector.getLong(p), decayVector.getDouble(p), functionTypeVector.getBytesRef(p, functionTypeScratch)));
      }
      return result.build();
    }
  }
  ...
  public String toString() {
    return "DecayLongOffsetLongEvaluator[" + "value=" + value + ", origin=" + origin + ", scale=" + scale + ", offset=" + offset + ", decay=" + decay + ", functionType=" + functionType + "]";
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    @Override
    public DecayLongOffsetLongEvaluator get(DriverContext context) {
      return new DecayLongOffsetLongEvaluator(source, value.get(context), origin.get(context), scale.get(context), offset.get(context), decay.get(context), functionType.get(context), context);
    }

...
public final class RuntimeSearchScoreLuceneQueryEvaluator implements ExpressionEvaluator {
  ...
  public DoubleBlock eval(int positionCount, BytesRefBlock fieldBlockBlock) {
    try(DoubleBlock.Builder result = driverContext.blockFactory().newDoubleBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        result.appendDouble(RuntimeSearch.scoreLuceneQuery(p, fieldBlockBlock, this.analyzer, this.query, this.memoryIndex, this.scratch));
      }
      return result.build();
    }
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    public RuntimeSearchScoreLuceneQueryEvaluator get(DriverContext context) {
      return new RuntimeSearchScoreLuceneQueryEvaluator(source, fieldBlock.get(context), analyzer, query, memoryIndex.apply(context), scratch.apply(context), context);
    }

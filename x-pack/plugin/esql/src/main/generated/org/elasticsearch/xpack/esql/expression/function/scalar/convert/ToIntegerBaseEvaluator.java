...
public final class ToIntegerBaseEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public IntBlock eval(int positionCount, BytesRefBlock stringBlock, IntBlock baseBlock) {
    try(IntBlock.Builder result = driverContext.blockFactory().newIntBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          result.appendInt(ToIntegerBase.process(string, base));
  ...
  public IntBlock eval(int positionCount, BytesRefVector stringVector, IntVector baseVector) {
    try(IntBlock.Builder result = driverContext.blockFactory().newIntBlockBuilder(positionCount)) {
      BytesRef stringScratch = new BytesRef();
      position: for (int p = 0; p < positionCount; p++) {
  ...
  public String toString() {
    return "ToIntegerBaseEvaluator[" + "string=" + string + ", base=" + base + "]";
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    @Override
    public ToIntegerBaseEvaluator get(DriverContext context) {
      return new ToIntegerBaseEvaluator(source, string.get(context), base.get(context), context);
    }

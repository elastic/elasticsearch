 ...
 */
public final class ToRangeDoubleEvaluator implements ExpressionEvaluator {
  private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(ToRangeDoubleEvaluator.class);
  ...
  public ToRangeDoubleEvaluator(Source source, ExpressionEvaluator from, ExpressionEvaluator to,
  ...
  public Block eval(Page page) {
    try (DoubleBlock fromBlock = (DoubleBlock) from.eval(page)) {
      try (DoubleBlock toBlock = (DoubleBlock) to.eval(page)) {
  ...
  public DoubleRangeBlock eval(int positionCount, DoubleBlock fromBlock, DoubleBlock toBlock) {
    try(DoubleRangeBlock.Builder result = driverContext.blockFactory().newDoubleRangeBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        double from = fromBlock.getDouble(fromBlock.getFirstValueIndex(p));
        double to = toBlock.getDouble(toBlock.getFirstValueIndex(p));
        try {
          result.appendDoubleRange(ToRange.process(from, to));
  ...
  public String toString() {
    return "ToRangeDoubleEvaluator[" + "from=" + from + ", to=" + to + "]";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    @Override
    public ToRangeDoubleEvaluator get(DriverContext context) {
      return new ToRangeDoubleEvaluator(source, from.get(context), to.get(context), context);
    }

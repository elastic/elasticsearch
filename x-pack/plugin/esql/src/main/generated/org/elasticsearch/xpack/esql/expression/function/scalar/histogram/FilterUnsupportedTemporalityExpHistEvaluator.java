 ...
 */
public final class FilterUnsupportedTemporalityExpHistEvaluator implements ExpressionEvaluator {
  private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(FilterUnsupportedTemporalityExpHistEvaluator.class);
  ...
  public FilterUnsupportedTemporalityExpHistEvaluator(Source source, ExpressionEvaluator histogram,
  ...
  public ExponentialHistogramBlock eval(int positionCount, ExponentialHistogramBlock histogramBlock,
    ...
    try(ExponentialHistogramBlock.Builder result = driverContext.blockFactory().newExponentialHistogramBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        try {
          FilterUnsupportedTemporality.processExpHist(result, p, histogramBlock, temporalityBlock, this.accessor);
  ...
  public String toString() {
    return "FilterUnsupportedTemporalityExpHistEvaluator[" + "histogram=" + histogram + ", temporality=" + temporality + "]";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    @Override
    public FilterUnsupportedTemporalityExpHistEvaluator get(DriverContext context) {
      return new FilterUnsupportedTemporalityExpHistEvaluator(source, histogram.get(context), temporality.get(context), accessor.apply(context), context);
    }

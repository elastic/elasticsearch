 ...
 */
public final class CbrtUnsignedLongEvaluator implements ExpressionEvaluator {
  ...
  public CbrtUnsignedLongEvaluator(Source source, ExpressionEvaluator val,
  ...
  public DoubleBlock eval(int positionCount, LongBlock valBlock) {
    try(DoubleBlock.Builder result = driverContext.blockFactory().newDoubleBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendDouble(Cbrt.processUnsignedLong(val));
      }
      return result.build();
    }
  }
  ...
  public DoubleVector eval(int positionCount, LongVector valVector) {
    try(DoubleVector.FixedBuilder result = driverContext.blockFactory().newDoubleVectorFixedBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendDouble(p, Cbrt.processUnsignedLong(val));
      }
      return result.build();
    }
  }

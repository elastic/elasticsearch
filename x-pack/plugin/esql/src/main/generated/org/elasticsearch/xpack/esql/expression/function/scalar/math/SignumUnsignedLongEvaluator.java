...
/**
 * {@link ExpressionEvaluator} implementation for {@link Signum}.
 * This class is generated. Edit {@code EvaluatorImplementer} instead.
 */
public final class SignumUnsignedLongEvaluator implements ExpressionEvaluator {
  ...
  public SignumUnsignedLongEvaluator(Source source, ExpressionEvaluator val,
  ...
  public DoubleBlock eval(int positionCount, LongBlock valBlock) {
    try(DoubleBlock.Builder result = driverContext.blockFactory().newDoubleBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendDouble(Signum.processUnsignedLong(val));
      }
      return result.build();
    }
  }
  ...
  public DoubleVector eval(int positionCount, LongVector valVector) {
    try(DoubleVector.FixedBuilder result = driverContext.blockFactory().newDoubleVectorFixedBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendDouble(p, Signum.processUnsignedLong(val));
      }
      return result.build();
    }
  }

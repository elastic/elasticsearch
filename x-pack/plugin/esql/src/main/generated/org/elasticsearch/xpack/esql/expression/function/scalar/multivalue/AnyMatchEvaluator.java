...
/**
 * {@link ExpressionEvaluator} implementation for {@link AnyMatch}.
 * This class is generated. Edit {@code LambdaEvaluatorImplementer} instead.
 */
public final class AnyMatchEvaluator implements ExpressionEvaluator {
  private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(AnyMatchEvaluator.class);
  ...
  public AnyMatchEvaluator(ExpressionEvaluator field, ExpressionEvaluator lambda,
    ...
    try {
      ...
      try (BooleanBlock lambdaBlock = (BooleanBlock) lambda.eval(innerPage); BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(fieldBlock.getPositionCount())) {
        for (int p = 0; p < fieldBlock.getPositionCount(); p++) {
          AnyMatch.process(result, lambdaBlock, p, p + 1);
        }
        return result.build();
      }
    } finally {
    ...
    try {
      ...
      try (BooleanBlock lambdaBlock = (BooleanBlock) lambda.eval(innerPage); BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(fieldBlock.getPositionCount())) {
        ...
        for (int p = 0; p < fieldBlock.getPositionCount(); p++) {
          ...
          AnyMatch.process(result, lambdaBlock, row, row + valueCount);
          row += valueCount;
        }
        return result.build();
      }
  ...
  public String toString() {
    return "AnyMatchEvaluator[field=" + field + ", lambda=" + lambda + "]";
  }
  ...
  public static class Factory implements ExpressionEvaluator.Factory {
    ...
    @Override
    public AnyMatchEvaluator get(DriverContext context) {
      return new AnyMatchEvaluator(field.get(context), lambda.get(context), outerChannels, context);
    }

 ...
 */
public final class MvFilterBooleanEvaluator implements ExpressionEvaluator {
  private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(MvFilterBooleanEvaluator.class);
  ...
  public MvFilterBooleanEvaluator(ExpressionEvaluator field, ExpressionEvaluator lambda,
  ...
  private Block evalNotExpanded(Page page, Block fieldBlock) {
    ...
    try {
      ...
      try (BooleanBlock lambdaBlock = (BooleanBlock) lambda.eval(innerPage); BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(fieldBlock.getPositionCount())) {
        for (int p = 0; p < fieldBlock.getPositionCount(); p++) {
          MvFilter.process(result, (BooleanBlock) fieldBlock, lambdaBlock, p, p + 1);
        }
        return result.build();
      }
    } finally {
    ...
    try {
      ...
      try (BooleanBlock lambdaBlock = (BooleanBlock) lambda.eval(innerPage); BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(fieldBlock.getPositionCount())) {
        ...
        for (int p = 0; p < fieldBlock.getPositionCount(); p++) {
          ...
          MvFilter.process(result, (BooleanBlock) inner[outerChannels.length], lambdaBlock, row, row + valueCount);
          row += valueCount;
        }
        return result.build();
      }
  ...
  public String toString() {
    return "MvFilterBooleanEvaluator[field=" + field + ", lambda=" + lambda + "]";
  }
  ...
  public static class Factory implements ExpressionEvaluator.Factory {
    ...
    @Override
    public MvFilterBooleanEvaluator get(DriverContext context) {
      return new MvFilterBooleanEvaluator(field.get(context), lambda.get(context), outerChannels, context);
    }

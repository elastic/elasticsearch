 ...
 */
public final class MvFilterBytesRefEvaluator implements ExpressionEvaluator {
  private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(MvFilterBytesRefEvaluator.class);
  ...
  public MvFilterBytesRefEvaluator(ExpressionEvaluator field, ExpressionEvaluator lambda,
  ...
  private Block evalNotExpanded(Page page, Block fieldBlock) {
    ...
    try {
      ...
      try (BooleanBlock lambdaBlock = (BooleanBlock) lambda.eval(innerPage); BytesRefBlock.Builder result = driverContext.blockFactory().newBytesRefBlockBuilder(fieldBlock.getPositionCount())) {
        for (int p = 0; p < fieldBlock.getPositionCount(); p++) {
          MvFilter.process(result, (BytesRefBlock) fieldBlock, lambdaBlock, p, p + 1);
        }
        return result.build();
      }
    } finally {
    ...
    try {
      ...
      try (BooleanBlock lambdaBlock = (BooleanBlock) lambda.eval(innerPage); BytesRefBlock.Builder result = driverContext.blockFactory().newBytesRefBlockBuilder(fieldBlock.getPositionCount())) {
        ...
        for (int p = 0; p < fieldBlock.getPositionCount(); p++) {
          ...
          MvFilter.process(result, (BytesRefBlock) inner[outerChannels.length], lambdaBlock, row, row + valueCount);
          row += valueCount;
        }
        return result.build();
      }
  ...
  public String toString() {
    return "MvFilterBytesRefEvaluator[field=" + field + ", lambda=" + lambda + "]";
  }
  ...
  public static class Factory implements ExpressionEvaluator.Factory {
    ...
    @Override
    public MvFilterBytesRefEvaluator get(DriverContext context) {
      return new MvFilterBytesRefEvaluator(field.get(context), lambda.get(context), outerChannels, context);
    }

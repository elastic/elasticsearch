 ...
 */
public final class MvFilterDoubleEvaluator implements ExpressionEvaluator {
  private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(MvFilterDoubleEvaluator.class);
  ...
  public MvFilterDoubleEvaluator(ExpressionEvaluator field, ExpressionEvaluator lambda,
  ...
  private Block evalNotExpanded(Page page, Block fieldBlock) {
    ...
    try {
      ...
      try (BooleanBlock lambdaBlock = (BooleanBlock) lambda.eval(innerPage); DoubleBlock.Builder result = driverContext.blockFactory().newDoubleBlockBuilder(fieldBlock.getPositionCount())) {
        for (int p = 0; p < fieldBlock.getPositionCount(); p++) {
          MvFilter.process(result, (DoubleBlock) fieldBlock, lambdaBlock, p, p + 1);
        }
        return result.build();
      }
    } finally {
    ...
    try {
      ...
      try (BooleanBlock lambdaBlock = (BooleanBlock) lambda.eval(innerPage); DoubleBlock.Builder result = driverContext.blockFactory().newDoubleBlockBuilder(fieldBlock.getPositionCount())) {
        ...
        for (int p = 0; p < fieldBlock.getPositionCount(); p++) {
          ...
          MvFilter.process(result, (DoubleBlock) inner[outerChannels.length], lambdaBlock, row, row + valueCount);
          row += valueCount;
        }
        return result.build();
      }
  ...
  public String toString() {
    return "MvFilterDoubleEvaluator[field=" + field + ", lambda=" + lambda + "]";
  }
  ...
  public static class Factory implements ExpressionEvaluator.Factory {
    ...
    @Override
    public MvFilterDoubleEvaluator get(DriverContext context) {
      return new MvFilterDoubleEvaluator(field.get(context), lambda.get(context), outerChannels, context);
    }

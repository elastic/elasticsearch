...
/**
 * {@link ExpressionEvaluator} implementation for {@link MvFilter}.
 * This class is generated. Edit {@code LambdaEvaluatorImplementer} instead.
 */
public final class MvFilterIntEvaluator implements ExpressionEvaluator {
  private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(MvFilterIntEvaluator.class);
  ...
  public MvFilterIntEvaluator(ExpressionEvaluator field, ExpressionEvaluator lambda,
  ...
  private Block evalNotExpanded(Page page, Block fieldBlock) {
    ...
    try {
      ...
      try (BooleanBlock lambdaBlock = (BooleanBlock) lambda.eval(innerPage); IntBlock.Builder result = driverContext.blockFactory().newIntBlockBuilder(fieldBlock.getPositionCount())) {
        for (int p = 0; p < fieldBlock.getPositionCount(); p++) {
          MvFilter.process(result, (IntBlock) fieldBlock, lambdaBlock, p, p + 1);
        }
        return result.build();
      }
    } finally {
    ...
    try {
      ...
      try (BooleanBlock lambdaBlock = (BooleanBlock) lambda.eval(innerPage); IntBlock.Builder result = driverContext.blockFactory().newIntBlockBuilder(fieldBlock.getPositionCount())) {
        ...
        for (int p = 0; p < fieldBlock.getPositionCount(); p++) {
          ...
          MvFilter.process(result, (IntBlock) inner[outerChannels.length], lambdaBlock, row, row + valueCount);
          row += valueCount;
        }
        return result.build();
      }
  ...
  public String toString() {
    return "MvFilterIntEvaluator[field=" + field + ", lambda=" + lambda + "]";
  }
  ...
  public static class Factory implements ExpressionEvaluator.Factory {
    ...
    @Override
    public MvFilterIntEvaluator get(DriverContext context) {
      return new MvFilterIntEvaluator(field.get(context), lambda.get(context), outerChannels, context);
    }

 ...
 */
public final class MvFilterLongEvaluator implements ExpressionEvaluator {
  private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(MvFilterLongEvaluator.class);
  ...
  public MvFilterLongEvaluator(ExpressionEvaluator field, ExpressionEvaluator lambda,
  ...
  private Block evalNotExpanded(Page page, Block fieldBlock) {
    ...
    try {
      ...
      try (BooleanBlock lambdaBlock = (BooleanBlock) lambda.eval(innerPage); LongBlock.Builder result = driverContext.blockFactory().newLongBlockBuilder(fieldBlock.getPositionCount())) {
        for (int p = 0; p < fieldBlock.getPositionCount(); p++) {
          MvFilter.process(result, (LongBlock) fieldBlock, lambdaBlock, p, p + 1);
        }
        return result.build();
      }
    } finally {
    ...
    try {
      ...
      try (BooleanBlock lambdaBlock = (BooleanBlock) lambda.eval(innerPage); LongBlock.Builder result = driverContext.blockFactory().newLongBlockBuilder(fieldBlock.getPositionCount())) {
        ...
        for (int p = 0; p < fieldBlock.getPositionCount(); p++) {
          ...
          MvFilter.process(result, (LongBlock) inner[outerChannels.length], lambdaBlock, row, row + valueCount);
          row += valueCount;
        }
        return result.build();
      }
  ...
  public String toString() {
    return "MvFilterLongEvaluator[field=" + field + ", lambda=" + lambda + "]";
  }
  ...
  public static class Factory implements ExpressionEvaluator.Factory {
    ...
    @Override
    public MvFilterLongEvaluator get(DriverContext context) {
      return new MvFilterLongEvaluator(field.get(context), lambda.get(context), outerChannels, context);
    }

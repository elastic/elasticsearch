 ...
 */
public final class SpatialContainsCartesianPointDocValuesAndConstantEvaluator implements ExpressionEvaluator {
  ...
  public BooleanBlock eval(int positionCount, LongBlock leftBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          SpatialContains.processCartesianPointDocValuesAndConstant(result, p, leftBlock, this.right);
  ...
  public String toString() {
    return "SpatialContainsCartesianPointDocValuesAndConstantEvaluator[" + "left=" + left + ", right=" + right + "]";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    public SpatialContainsCartesianPointDocValuesAndConstantEvaluator get(DriverContext context) {
      return new SpatialContainsCartesianPointDocValuesAndConstantEvaluator(source, left.get(context), right, context);
    }

      ...
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          ...
          for (int leftValueBlockIndex = leftValueBlockFirst; leftValueBlockIndex < leftValueBlockFirst + leftValueBlockCount; leftValueBlockIndex++) {
            multiValuesCombiner.add(SpatialContains.processCartesianSourceAndConstant(leftValueBlock.getBytesRef(leftValueBlockIndex, leftValueScratch), this.rightValue));
          }
  ...
  public BooleanBlock eval(int positionCount, BytesRefVector leftValueVector) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        try {
          multiValuesCombiner.initialize();
          multiValuesCombiner.add(SpatialContains.processCartesianSourceAndConstant(leftValueVector.getBytesRef(p, leftValueScratch), this.rightValue));
          result.appendBoolean(multiValuesCombiner.result());
        } catch (IllegalArgumentException | IOException e) {
          warnings.registerException(e);
          result.appendNull();

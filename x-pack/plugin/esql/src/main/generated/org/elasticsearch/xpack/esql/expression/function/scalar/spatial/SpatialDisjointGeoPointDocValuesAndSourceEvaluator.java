 ...
 */
public final class SpatialDisjointGeoPointDocValuesAndSourceEvaluator implements ExpressionEvaluator {
  ...
  public SpatialDisjointGeoPointDocValuesAndSourceEvaluator(Source source, ExpressionEvaluator left,
  ...
  public BooleanBlock eval(int positionCount, LongBlock leftBlock, BytesRefBlock rightBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          SpatialDisjoint.processGeoPointDocValuesAndSource(result, p, leftBlock, rightBlock);
  ...
  public String toString() {
    return "SpatialDisjointGeoPointDocValuesAndSourceEvaluator[" + "left=" + left + ", right=" + right + "]";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    public SpatialDisjointGeoPointDocValuesAndSourceEvaluator get(DriverContext context) {
      return new SpatialDisjointGeoPointDocValuesAndSourceEvaluator(source, left.get(context), right.get(context), context);
    }

 ...
 */
public final class SpatialIntersectsCartesianPointDocValuesAndConstantEvaluator implements ExpressionEvaluator {
  ...
  public BooleanBlock eval(int positionCount, LongBlock leftBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          SpatialIntersects.processCartesianPointDocValuesAndConstant(result, p, leftBlock, this.right);
  ...
  public String toString() {
    return "SpatialIntersectsCartesianPointDocValuesAndConstantEvaluator[" + "left=" + left + ", right=" + right + "]";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    public SpatialIntersectsCartesianPointDocValuesAndConstantEvaluator get(DriverContext context) {
      return new SpatialIntersectsCartesianPointDocValuesAndConstantEvaluator(source, left.get(context), right, context);
    }

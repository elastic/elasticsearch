 ...
 */
public final class SpatialIntersectsGeoSourceAndConstantEvaluator implements ExpressionEvaluator {
  ...
  public SpatialIntersectsGeoSourceAndConstantEvaluator(Source source, ExpressionEvaluator left,
  ...
  public BooleanBlock eval(int positionCount, BytesRefBlock leftBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          SpatialIntersects.processGeoSourceAndConstant(result, p, leftBlock, this.right);
  ...
  public String toString() {
    return "SpatialIntersectsGeoSourceAndConstantEvaluator[" + "left=" + left + ", right=" + right + "]";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    public SpatialIntersectsGeoSourceAndConstantEvaluator get(DriverContext context) {
      return new SpatialIntersectsGeoSourceAndConstantEvaluator(source, left.get(context), right, context);
    }

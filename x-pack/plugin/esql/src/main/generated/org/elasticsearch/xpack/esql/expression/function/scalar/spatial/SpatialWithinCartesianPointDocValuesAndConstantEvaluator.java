 ...
 */
public final class SpatialWithinCartesianPointDocValuesAndConstantEvaluator implements ExpressionEvaluator {
  ...
  public BooleanBlock eval(int positionCount, LongBlock leftBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          SpatialWithin.processCartesianPointDocValuesAndConstant(result, p, leftBlock, this.right);
  ...
  public String toString() {
    return "SpatialWithinCartesianPointDocValuesAndConstantEvaluator[" + "left=" + left + ", right=" + right + "]";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    public SpatialWithinCartesianPointDocValuesAndConstantEvaluator get(DriverContext context) {
      return new SpatialWithinCartesianPointDocValuesAndConstantEvaluator(source, left.get(context), right, context);
    }

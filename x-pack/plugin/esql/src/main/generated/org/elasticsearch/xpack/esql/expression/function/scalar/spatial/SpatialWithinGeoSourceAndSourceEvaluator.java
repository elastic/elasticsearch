 ...
 */
public final class SpatialWithinGeoSourceAndSourceEvaluator implements ExpressionEvaluator {
  ...
  public SpatialWithinGeoSourceAndSourceEvaluator(Source source, ExpressionEvaluator left,
  ...
  public BooleanBlock eval(int positionCount, BytesRefBlock leftBlock, BytesRefBlock rightBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          SpatialWithin.processGeoSourceAndSource(result, p, leftBlock, rightBlock);
  ...
  public String toString() {
    return "SpatialWithinGeoSourceAndSourceEvaluator[" + "left=" + left + ", right=" + right + "]";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    public SpatialWithinGeoSourceAndSourceEvaluator get(DriverContext context) {
      return new SpatialWithinGeoSourceAndSourceEvaluator(source, left.get(context), right.get(context), context);
    }

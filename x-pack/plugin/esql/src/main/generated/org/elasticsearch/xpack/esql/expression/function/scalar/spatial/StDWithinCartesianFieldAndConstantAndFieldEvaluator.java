...
public final class StDWithinCartesianFieldAndConstantAndFieldEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public BooleanBlock eval(int positionCount, BytesRefBlock leftValueBlock,
    ...
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          result.appendBoolean(StDWithin.processCartesianFieldAndConstantAndField(leftValueBlock.getBytesRef(leftValueBlock.getFirstValueIndex(p), leftValueScratch), rightValue, distanceBlock.getDouble(distanceBlock.getFirstValueIndex(p))));
  ...
  public BooleanBlock eval(int positionCount, BytesRefVector leftValueVector,
    ...
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        try {
          result.appendBoolean(StDWithin.processCartesianFieldAndConstantAndField(leftValueVector.getBytesRef(p, leftValueScratch), rightValue, distanceVector.getDouble(p)));
        } catch (IllegalArgumentException | IOException e) {
          warnings.registerException(e);
          result.appendNull();
        }
  ...
  public String toString() {
    return "StDWithinCartesianFieldAndConstantAndFieldEvaluator[" + "leftValue=" + leftValue + ", rightValue=" + rightValue + ", distance=" + distance + "]";
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public StDWithinCartesianFieldAndConstantAndFieldEvaluator get(DriverContext context) {
      return new StDWithinCartesianFieldAndConstantAndFieldEvaluator(source, leftValue.get(context), rightValue, distance.get(context), context);
    }

 ...
 */
public final class StDWithinCartesianPointDocValuesAndConstantAndFieldEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public StDWithinCartesianPointDocValuesAndConstantAndFieldEvaluator(Source source,
  ...
  public BooleanBlock eval(int positionCount, LongBlock leftValueBlock, DoubleBlock distanceBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          result.appendBoolean(StDWithin.processCartesianPointDocValuesAndConstantAndField(leftValueBlock.getLong(leftValueBlock.getFirstValueIndex(p)), rightValue, distanceBlock.getDouble(distanceBlock.getFirstValueIndex(p))));
  ...
  public BooleanBlock eval(int positionCount, LongVector leftValueVector,
    ...
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        try {
          result.appendBoolean(StDWithin.processCartesianPointDocValuesAndConstantAndField(leftValueVector.getLong(p), rightValue, distanceVector.getDouble(p)));
        } catch (IllegalArgumentException e) {
          warnings.registerException(e);
          result.appendNull();
        }
  ...
  public String toString() {
    return "StDWithinCartesianPointDocValuesAndConstantAndFieldEvaluator[" + "leftValue=" + leftValue + ", rightValue=" + rightValue + ", distance=" + distance + "]";
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    @Override
    public StDWithinCartesianPointDocValuesAndConstantAndFieldEvaluator get(DriverContext context) {
      return new StDWithinCartesianPointDocValuesAndConstantAndFieldEvaluator(source, leftValue.get(context), rightValue, distance.get(context), context);
    }

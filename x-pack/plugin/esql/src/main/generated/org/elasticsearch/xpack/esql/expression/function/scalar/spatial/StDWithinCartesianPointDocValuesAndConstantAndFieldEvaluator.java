...
public final class StDWithinCartesianPointDocValuesAndConstantAndFieldEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public BooleanBlock eval(int positionCount, LongBlock leftValueBlock, DoubleBlock argValueBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          result.appendBoolean(StDWithin.processCartesianPointDocValuesAndConstantAndField(leftValueBlock.getLong(leftValueBlock.getFirstValueIndex(p)), rightValue, argValueBlock.getDouble(argValueBlock.getFirstValueIndex(p))));
  ...
  public BooleanBlock eval(int positionCount, LongVector leftValueVector,
    ...
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        try {
          result.appendBoolean(StDWithin.processCartesianPointDocValuesAndConstantAndField(leftValueVector.getLong(p), rightValue, argValueVector.getDouble(p)));
        } catch (IllegalArgumentException e) {
          warnings.registerException(e);
          result.appendNull();
        }
  ...
  public String toString() {
    return "StDWithinCartesianPointDocValuesAndConstantAndFieldEvaluator[" + "leftValue=" + leftValue + ", rightValue=" + rightValue + ", argValue=" + argValue + "]";
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public StDWithinCartesianPointDocValuesAndConstantAndFieldEvaluator get(DriverContext context) {
      return new StDWithinCartesianPointDocValuesAndConstantAndFieldEvaluator(source, leftValue.get(context), rightValue, argValue.get(context), context);
    }

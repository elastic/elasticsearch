...
public final class StDWithinGeoFieldAndFieldAndFieldEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public BooleanBlock eval(int positionCount, BytesRefBlock leftValueBlock,
    ...
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          result.appendBoolean(StDWithin.processGeoFieldAndFieldAndField(leftValueBlock.getBytesRef(leftValueBlock.getFirstValueIndex(p), leftValueScratch), rightValueBlock.getBytesRef(rightValueBlock.getFirstValueIndex(p), rightValueScratch), argValueBlock.getDouble(argValueBlock.getFirstValueIndex(p))));
  ...
  public BooleanBlock eval(int positionCount, BytesRefVector leftValueVector,
    ...
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        try {
          result.appendBoolean(StDWithin.processGeoFieldAndFieldAndField(leftValueVector.getBytesRef(p, leftValueScratch), rightValueVector.getBytesRef(p, rightValueScratch), argValueVector.getDouble(p)));
        } catch (IllegalArgumentException | IOException e) {
          warnings.registerException(e);
          result.appendNull();
        }
  ...
  public String toString() {
    return "StDWithinGeoFieldAndFieldAndFieldEvaluator[" + "leftValue=" + leftValue + ", rightValue=" + rightValue + ", argValue=" + argValue + "]";
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public StDWithinGeoFieldAndFieldAndFieldEvaluator get(DriverContext context) {
      return new StDWithinGeoFieldAndFieldAndFieldEvaluator(source, leftValue.get(context), rightValue.get(context), argValue.get(context), context);
    }

...
public final class StDistanceCartesianSourceAndConstantEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public DoubleBlock eval(int positionCount, BytesRefBlock leftValueBlock) {
    try(DoubleBlock.Builder result = driverContext.blockFactory().newDoubleBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          result.appendDouble(StDistance.processCartesianSourceAndConstant(leftValueBlock.getBytesRef(leftValueBlock.getFirstValueIndex(p), leftValueScratch), rightValue));
  ...
  public DoubleBlock eval(int positionCount, BytesRefVector leftValueVector) {
    try(DoubleBlock.Builder result = driverContext.blockFactory().newDoubleBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        try {
          result.appendDouble(StDistance.processCartesianSourceAndConstant(leftValueVector.getBytesRef(p, leftValueScratch), rightValue));
        } catch (IllegalArgumentException | IOException e) {
          warnings.registerException(e);
          result.appendNull();
        }

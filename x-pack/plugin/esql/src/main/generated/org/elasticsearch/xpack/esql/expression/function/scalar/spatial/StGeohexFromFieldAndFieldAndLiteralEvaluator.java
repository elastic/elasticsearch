...
public final class StGeohexFromFieldAndFieldAndLiteralEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public BytesRefBlock eval(int positionCount, BytesRefBlock inBlock, IntBlock precisionBlock) {
    try(BytesRefBlock.Builder result = driverContext.blockFactory().newBytesRefBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          StGeohex.fromFieldAndFieldAndLiteral(result, p, inBlock, precisionBlock.getInt(precisionBlock.getFirstValueIndex(p)), this.bounds);
  ...
  public String toString() {
    return "StGeohexFromFieldAndFieldAndLiteralEvaluator[" + "in=" + in + ", precision=" + precision + ", bounds=" + bounds + "]";
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public StGeohexFromFieldAndFieldAndLiteralEvaluator get(DriverContext context) {
      return new StGeohexFromFieldAndFieldAndLiteralEvaluator(source, in.get(context), precision.get(context), bounds, context);
    }

...
public final class StGeohexFromFieldDocValuesAndFieldAndLiteralEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public LongBlock eval(int positionCount, LongBlock encodedBlock, IntBlock precisionBlock) {
    try(LongBlock.Builder result = driverContext.blockFactory().newLongBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          StGeohex.fromFieldDocValuesAndFieldAndLiteral(result, p, encodedBlock, precisionBlock.getInt(precisionBlock.getFirstValueIndex(p)), this.bounds);
  ...
  public String toString() {
    return "StGeohexFromFieldDocValuesAndFieldAndLiteralEvaluator[" + "encoded=" + encoded + ", precision=" + precision + ", bounds=" + bounds + "]";
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public StGeohexFromFieldDocValuesAndFieldAndLiteralEvaluator get(DriverContext context) {
      return new StGeohexFromFieldDocValuesAndFieldAndLiteralEvaluator(source, encoded.get(context), precision.get(context), bounds, context);
    }

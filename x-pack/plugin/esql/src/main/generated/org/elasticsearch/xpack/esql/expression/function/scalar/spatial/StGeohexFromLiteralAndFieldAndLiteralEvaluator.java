...
public final class StGeohexFromLiteralAndFieldAndLiteralEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public LongBlock eval(int positionCount, IntBlock precisionBlock) {
    try(LongBlock.Builder result = driverContext.blockFactory().newLongBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          result.appendLong(StGeohex.fromLiteralAndFieldAndLiteral(this.in, precisionBlock.getInt(precisionBlock.getFirstValueIndex(p)), this.bounds));
  ...
  public LongBlock eval(int positionCount, IntVector precisionVector) {
    try(LongBlock.Builder result = driverContext.blockFactory().newLongBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        try {
          result.appendLong(StGeohex.fromLiteralAndFieldAndLiteral(this.in, precisionVector.getInt(p), this.bounds));
        } catch (IllegalArgumentException e) {
          warnings().registerException(e);
          result.appendNull();
        }

...
public final class StGeohexFromLiteralAndFieldAndLiteralEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public BytesRefBlock eval(int positionCount, IntBlock precisionBlock) {
    try(BytesRefBlock.Builder result = driverContext.blockFactory().newBytesRefBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          result.appendBytesRef(StGeohex.fromLiteralAndFieldAndLiteral(this.in, precisionBlock.getInt(precisionBlock.getFirstValueIndex(p)), this.bounds));
  ...
  public BytesRefBlock eval(int positionCount, IntVector precisionVector) {
    try(BytesRefBlock.Builder result = driverContext.blockFactory().newBytesRefBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        try {
          result.appendBytesRef(StGeohex.fromLiteralAndFieldAndLiteral(this.in, precisionVector.getInt(p), this.bounds));
        } catch (IllegalArgumentException e) {
          warnings().registerException(e);
          result.appendNull();
        }
  ...
  public String toString() {
    return "StGeohexFromLiteralAndFieldAndLiteralEvaluator[" + "in=" + in + ", precision=" + precision + ", bounds=" + bounds + "]";
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public StGeohexFromLiteralAndFieldAndLiteralEvaluator get(DriverContext context) {
      return new StGeohexFromLiteralAndFieldAndLiteralEvaluator(source, in, precision.get(context), bounds, context);
    }

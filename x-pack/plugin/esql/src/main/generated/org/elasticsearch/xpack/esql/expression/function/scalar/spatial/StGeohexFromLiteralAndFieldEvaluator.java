...
public final class StGeohexFromLiteralAndFieldEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public BytesRefBlock eval(int positionCount, IntBlock precisionBlock) {
    try(BytesRefBlock.Builder result = driverContext.blockFactory().newBytesRefBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          result.appendBytesRef(StGeohex.fromLiteralAndField(this.in, precisionBlock.getInt(precisionBlock.getFirstValueIndex(p))));
  ...
  public BytesRefBlock eval(int positionCount, IntVector precisionVector) {
    try(BytesRefBlock.Builder result = driverContext.blockFactory().newBytesRefBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        try {
          result.appendBytesRef(StGeohex.fromLiteralAndField(this.in, precisionVector.getInt(p)));
        } catch (IllegalArgumentException e) {
          warnings().registerException(e);
          result.appendNull();
        }

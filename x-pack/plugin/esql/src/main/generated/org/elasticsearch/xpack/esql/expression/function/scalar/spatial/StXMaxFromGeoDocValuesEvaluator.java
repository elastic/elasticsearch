...
public final class StXMaxFromGeoDocValuesEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public DoubleBlock eval(int positionCount, LongBlock encodedBlockBlock) {
    try(DoubleBlock.Builder result = driverContext.blockFactory().newDoubleBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          StXMax.fromGeoDocValues(result, p, encodedBlockBlock, this.resultsBuilder);
  ...
  public String toString() {
    return "StXMaxFromGeoDocValuesEvaluator[" + "encodedBlock=" + encodedBlock + "]";
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    @Override
    public StXMaxFromGeoDocValuesEvaluator get(DriverContext context) {
      return new StXMaxFromGeoDocValuesEvaluator(source, encodedBlock.get(context), resultsBuilder.apply(context), context);
    }

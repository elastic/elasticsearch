...
public final class StYMaxFromCartesianWKBEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public DoubleBlock eval(int positionCount, BytesRefBlock wkbBlockBlock) {
    try(DoubleBlock.Builder result = driverContext.blockFactory().newDoubleBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          StYMax.fromCartesianWKB(result, p, wkbBlockBlock, this.resultsBuilder);
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public StYMaxFromCartesianWKBEvaluator get(DriverContext context) {
      return new StYMaxFromCartesianWKBEvaluator(source, wkbBlock.get(context), resultsBuilder.apply(context), context);
    }

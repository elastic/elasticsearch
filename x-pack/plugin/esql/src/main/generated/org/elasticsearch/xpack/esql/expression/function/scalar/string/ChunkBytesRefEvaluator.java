...
public final class ChunkBytesRefEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public ChunkBytesRefEvaluator(Source source, EvalOperator.ExpressionEvaluator str, int numChunks,
      int chunkSize, DriverContext driverContext) {
    this.source = source;
    this.str = str;
  ...
  public BytesRefBlock eval(int positionCount, BytesRefBlock strBlock) {
    try(BytesRefBlock.Builder result = driverContext.blockFactory().newBytesRefBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        ...
        Chunk.process(result, str, this.numChunks, this.chunkSize);
      }
      return result.build();
    }
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public Factory(Source source, EvalOperator.ExpressionEvaluator.Factory str, int numChunks,
        int chunkSize) {
      this.source = source;
      this.str = str;
      this.numChunks = numChunks;
    ...
    public ChunkBytesRefEvaluator get(DriverContext context) {
      return new ChunkBytesRefEvaluator(source, str.get(context), numChunks, chunkSize, context);
    }

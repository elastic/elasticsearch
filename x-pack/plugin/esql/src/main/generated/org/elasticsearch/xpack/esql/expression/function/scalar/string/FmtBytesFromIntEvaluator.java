 ...
 */
public final class FmtBytesFromIntEvaluator implements ExpressionEvaluator {
  private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(FmtBytesFromIntEvaluator.class);
  ...
  public FmtBytesFromIntEvaluator(Source source, ExpressionEvaluator bytes,
  ...
  public BytesRefBlock eval(int positionCount, IntBlock bytesBlock) {
    try(BytesRefBlock.Builder result = driverContext.blockFactory().newBytesRefBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendBytesRef(FmtBytes.processInt(bytes));
      }
      return result.build();
    }
  }
  ...
  public String toString() {
    return "FmtBytesFromIntEvaluator[" + "bytes=" + bytes + "]";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    @Override
    public FmtBytesFromIntEvaluator get(DriverContext context) {
      return new FmtBytesFromIntEvaluator(source, bytes.get(context), context);
    }

 ...
 */
public final class DivIntsByConstantEvaluator implements ExpressionEvaluator {
  ...
  public DivIntsByConstantEvaluator(Source source, ExpressionEvaluator lhs, int rhs,
  ...
  public IntBlock eval(int positionCount, IntBlock lhsBlock) {
    try(IntBlock.Builder result = driverContext.blockFactory().newIntBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendInt(Div.processIntsByConstant(lhs, this.rhs));
      }
      return result.build();
    }
  }
  ...
  public IntVector eval(int positionCount, IntVector lhsVector) {
    try(IntVector.FixedBuilder result = driverContext.blockFactory().newIntVectorFixedBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendInt(p, Div.processIntsByConstant(lhs, this.rhs));
      }
      return result.build();
    }
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    public DivIntsByConstantEvaluator get(DriverContext context) {
      return new DivIntsByConstantEvaluator(source, lhs.get(context), rhs, context);
    }

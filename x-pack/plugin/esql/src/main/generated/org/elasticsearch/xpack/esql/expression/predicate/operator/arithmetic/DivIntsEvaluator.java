 ...
 */
public final class DivIntsEvaluator implements ExpressionEvaluator {
  ...
  public DivIntsEvaluator(Source source, ExpressionEvaluator lhs, ExpressionEvaluator rhs,
  ...
  public IntBlock eval(int positionCount, IntBlock lhsBlock, IntBlock rhsBlock) {
    try(IntBlock.Builder result = driverContext.blockFactory().newIntBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        try {
          result.appendInt(Div.processInts(lhs, rhs));
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    @Override
    public DivIntsEvaluator get(DriverContext context) {
      return new DivIntsEvaluator(source, lhs.get(context), rhs.get(context), context);
    }

...
public abstract class DivLongsByConstantEvaluator implements ExpressionEvaluator {
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    public DivLongsByConstantEvaluator get(DriverContext context) {
      Optional<Class<? extends DivLongsByConstantEvaluator>> constantSpecializedClassOpt = ConstantMethodResultSpecializer.SHARED.specializeLong(DivLongsByConstantEvaluator.class, "rhs", this.rhs);
      if (constantSpecializedClassOpt.isPresent()) {
        Class<? extends DivLongsByConstantEvaluator> constantSpecializedClass = constantSpecializedClassOpt.get();
        try {
          return (DivLongsByConstantEvaluator) constantSpecializedClass.getConstructors()[0].newInstance(source, lhs.get(context), context);
        } catch (InstantiationException | IllegalAccessException | InvocationTargetException e) {
          throw new IllegalStateException("failed to construct specialized evaluator for DivLongsByConstantEvaluator", e);
        }
      }

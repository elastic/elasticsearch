 ...
 */
public abstract class DivNonFiniteDoublesByConstantEvaluator implements ExpressionEvaluator {
  private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(DivNonFiniteDoublesByConstantEvaluator.class);
  ...
  public DivNonFiniteDoublesByConstantEvaluator(Source source, ExpressionEvaluator lhs,
  ...
  public DoubleBlock eval(int positionCount, DoubleBlock lhsBlock) {
    try(DoubleBlock.Builder result = driverContext.blockFactory().newDoubleBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendDouble(Div.processNonFiniteDoublesByConstant(lhs, rhs()));
      }
      return result.build();
    }
  }
  ...
  public DoubleVector eval(int positionCount, DoubleVector lhsVector) {
    try(DoubleVector.FixedBuilder result = driverContext.blockFactory().newDoubleVectorFixedBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendDouble(p, Div.processNonFiniteDoublesByConstant(lhs, rhs()));
      }
      return result.build();
    }
  }
  ...
  public String toString() {
    return "DivNonFiniteDoublesByConstantEvaluator[" + "lhs=" + lhs + ", rhs=" + rhs() + "]" + " (" + pathLabel() + ")";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    @Override
    public DivNonFiniteDoublesByConstantEvaluator get(DriverContext context) {
      Optional<Class<? extends DivNonFiniteDoublesByConstantEvaluator>> constantSpecializedClassOpt = ConstantMethodResultSpecializer.SHARED.specializeDouble(DivNonFiniteDoublesByConstantEvaluator.class, "rhs", this.rhs);
      if (constantSpecializedClassOpt.isPresent()) {
        Class<? extends DivNonFiniteDoublesByConstantEvaluator> constantSpecializedClass = constantSpecializedClassOpt.get();
        try {
          return (DivNonFiniteDoublesByConstantEvaluator) constantSpecializedClass.getConstructors()[0].newInstance(source, lhs.get(context), context);
        } catch (InstantiationException | IllegalAccessException | InvocationTargetException e) {
          throw new IllegalStateException("failed to construct specialized evaluator for DivNonFiniteDoublesByConstantEvaluator", e);
        }
      }
    ...
    @Override
    public String toString() {
      return "DivNonFiniteDoublesByConstantEvaluator[" + "lhs=" + lhs + ", rhs=" + rhs + "]";
    }
  }
   ...
   */
  public static final class Standard extends DivNonFiniteDoublesByConstantEvaluator {
    private final double rhs;

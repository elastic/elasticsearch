...
public abstract class ModIntsByConstantEvaluator implements ExpressionEvaluator {
  ...
  public String toString() {
    return "ModIntsByConstantEvaluator[" + "lhs=" + lhs + ", rhs=" + rhs() + "]" + " (" + pathLabel() + ")";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    public ModIntsByConstantEvaluator get(DriverContext context) {
      Optional<Class<? extends ModIntsByConstantEvaluator>> constantSpecializedClassOpt = ConstantMethodResultSpecializer.SHARED.specializeInt(ModIntsByConstantEvaluator.class, "rhs", this.rhs);
      if (constantSpecializedClassOpt.isPresent()) {
        Class<? extends ModIntsByConstantEvaluator> constantSpecializedClass = constantSpecializedClassOpt.get();
        try {
          return (ModIntsByConstantEvaluator) constantSpecializedClass.getConstructors()[0].newInstance(source, lhs.get(context), context);

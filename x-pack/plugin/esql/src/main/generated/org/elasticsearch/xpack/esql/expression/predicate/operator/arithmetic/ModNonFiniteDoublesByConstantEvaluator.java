 ...
 */
public abstract class ModNonFiniteDoublesByConstantEvaluator implements ExpressionEvaluator {
  private static final long BASE_RAM_BYTES_USED = RamUsageEstimator.shallowSizeOfInstance(ModNonFiniteDoublesByConstantEvaluator.class);
  ...
  public ModNonFiniteDoublesByConstantEvaluator(Source source, ExpressionEvaluator lhs,
  ...
  public DoubleBlock eval(int positionCount, DoubleBlock lhsBlock) {
    try(DoubleBlock.Builder result = driverContext.blockFactory().newDoubleBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendDouble(Mod.processNonFiniteDoublesByConstant(lhs, rhs()));
      }
      return result.build();
    }
  }

  public DoubleVector eval(int positionCount, DoubleVector lhsVector) {
    try(DoubleVector.FixedBuilder result = driverContext.blockFactory().newDoubleVectorFixedBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        double lhs = lhsVector.getDouble(p);
        result.appendDouble(p, Mod.processNonFiniteDoublesByConstant(lhs, rhs()));
      }
      return result.build();
    }
  }
  ...
  public String toString() {
    return "ModNonFiniteDoublesByConstantEvaluator[" + "lhs=" + lhs + ", rhs=" + rhs() + "]" + " (" + pathLabel() + ")";
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    @Override
    public ModNonFiniteDoublesByConstantEvaluator get(DriverContext context) {
      Optional<Class<? extends ModNonFiniteDoublesByConstantEvaluator>> constantSpecializedClassOpt = ConstantMethodResultSpecializer.SHARED.specializeDouble(ModNonFiniteDoublesByConstantEvaluator.class, "rhs", this.rhs);
      if (constantSpecializedClassOpt.isPresent()) {
        Class<? extends ModNonFiniteDoublesByConstantEvaluator> constantSpecializedClass = constantSpecializedClassOpt.get();
        try {
          return (ModNonFiniteDoublesByConstantEvaluator) constantSpecializedClass.getConstructors()[0].newInstance(source, lhs.get(context), context);
        } catch (InstantiationException | IllegalAccessException | InvocationTargetException e) {
          throw new IllegalStateException("failed to construct specialized evaluator for ModNonFiniteDoublesByConstantEvaluator", e);
        }
      }
    ...
    @Override
    public String toString() {
      return "ModNonFiniteDoublesByConstantEvaluator[" + "lhs=" + lhs + ", rhs=" + rhs + "]";
    }
  }
   ...
   */
  public static final class Standard extends ModNonFiniteDoublesByConstantEvaluator {
    private final double rhs;

...
public final class EqualsDoublesEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public BooleanVector eval(int positionCount, DoubleVector lhsVector, DoubleVector rhsVector) {
    try(BooleanVector.FixedBuilder result = driverContext.blockFactory().newBooleanVectorFixedBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        result.appendBoolean(p, Equals.processDoubles(lhsVector.getDouble(p), rhsVector.getDouble(p)));
      }
      return result.build();
    }
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public EqualsDoublesEvaluator get(DriverContext context) {
      return new EqualsDoublesEvaluator(source, lhs.get(context), rhs.get(context), context);
    }

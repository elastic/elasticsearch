...
public final class EqualsDoublesEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public EqualsDoublesEvaluator(Source source, EvalOperator.ExpressionEvaluator lhs,
  ...
  public BooleanBlock eval(int positionCount, DoubleBlock lhsBlock, DoubleBlock rhsBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendBoolean(Equals.processDoubles(lhsBlock.getDouble(lhsBlock.getFirstValueIndex(p)), rhsBlock.getDouble(rhsBlock.getFirstValueIndex(p))));
      }
      return result.build();
    }
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    @Override
    public EqualsDoublesEvaluator get(DriverContext context, boolean collectWarnings) {
      return new EqualsDoublesEvaluator(source, lhs.get(context, collectWarnings), rhs.get(context, collectWarnings), context, collectWarnings ? new Warnings(source) : Warnings.NOOP_WARNINGS);
    }

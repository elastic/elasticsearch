    ...
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        ...
        for (int i = 0; i < rhsBlocks.length; i++) {
          ...
          rhsValues[i] = rhsBlocks[i].isNull(p) ? null : rhsBlocks[i].getBytesRef(o, rhsScratch[i]);
        }
        In.process(result, lhsBlock.getBytesRef(lhsBlock.getFirstValueIndex(p), lhsScratch), rhsValues);
      }
      return result.build();
    }
  }
  ...
  public BooleanBlock eval(int positionCount, BytesRefVector lhsVector,
    ...
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      ...
      position: for (int p = 0; p < positionCount; p++) {
        ...
        In.process(result, lhsVector.getBytesRef(p, lhsScratch), rhsValues);
      }
      return result.build();
    }
  }

...
public final class InsensitiveEqualsConstantEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public Block eval(Page page) {
    try (BytesRefBlock lhsBlock = (BytesRefBlock) lhs.eval(page)) {
      var lhsOrdinalBlock = lhsBlock.asOrdinals();
      if (lhsOrdinalBlock != null) {
        return evalOrdinals(page.getPositionCount(), lhsOrdinalBlock);
      }
  ...
  public BooleanBlock evalOrdinals(int positionCount, OrdinalBytesRefBlock lhsBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      var lhsVector = lhsBlock.getDictionaryVector();
      var ordinalPositions = lhsBlock.getOrdinalsBlock();
      try(var dictResult = eval(ordinalPositions.getPositionCount(), lhsVector)) {

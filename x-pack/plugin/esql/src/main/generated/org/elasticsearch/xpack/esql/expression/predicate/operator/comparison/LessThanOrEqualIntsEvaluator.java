 ...
 */
public final class LessThanOrEqualIntsEvaluator implements ExpressionEvaluator {
  ...
  public LessThanOrEqualIntsEvaluator(Source source, ExpressionEvaluator lhs,
  ...
  public BooleanBlock eval(int positionCount, IntBlock lhsBlock, IntBlock rhsBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendBoolean(LessThanOrEqual.processInts(lhs, rhs));
      }
      return result.build();
    }
  }
  ...
  public BooleanVector eval(int positionCount, IntVector lhsVector, IntVector rhsVector) {
    try(BooleanVector.FixedBuilder result = driverContext.blockFactory().newBooleanVectorFixedBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendBoolean(p, LessThanOrEqual.processInts(lhs, rhs));
      }
      return result.build();
    }
  }
  ...
  static class Factory implements ExpressionEvaluator.Factory {
    ...
    public LessThanOrEqualIntsEvaluator get(DriverContext context) {
      return new LessThanOrEqualIntsEvaluator(source, lhs.get(context), rhs.get(context), context);
    }

...
public final class EqualsLongsEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public EqualsLongsEvaluator(Source source, EvalOperator.ExpressionEvaluator lhs,
  ...
  public BooleanBlock eval(int positionCount, LongBlock lhsBlock, LongBlock rhsBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendBoolean(Equals.processLongs(lhsBlock.getLong(lhsBlock.getFirstValueIndex(p)), rhsBlock.getLong(rhsBlock.getFirstValueIndex(p))));
      }
      return result.build();
    }
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public EqualsLongsEvaluator get(DriverContext context) {
      return new EqualsLongsEvaluator(source, lhs.get(context), rhs.get(context), context);
    }

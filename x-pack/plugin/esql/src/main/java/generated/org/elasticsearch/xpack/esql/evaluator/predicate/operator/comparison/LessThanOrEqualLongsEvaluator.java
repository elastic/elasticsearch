...
public final class LessThanOrEqualLongsEvaluator implements EvalOperator.ExpressionEvaluator {
  ...
  public LessThanOrEqualLongsEvaluator(Source source, EvalOperator.ExpressionEvaluator lhs,
  ...
  public BooleanBlock eval(int positionCount, LongBlock lhsBlock, LongBlock rhsBlock) {
    try(BooleanBlock.Builder result = driverContext.blockFactory().newBooleanBlockBuilder(positionCount)) {
      position: for (int p = 0; p < positionCount; p++) {
        ...
        result.appendBoolean(LessThanOrEqual.processLongs(lhsBlock.getLong(lhsBlock.getFirstValueIndex(p)), rhsBlock.getLong(rhsBlock.getFirstValueIndex(p))));
      }
      return result.build();
    }
  }
  ...
  static class Factory implements EvalOperator.ExpressionEvaluator.Factory {
    ...
    public LessThanOrEqualLongsEvaluator get(DriverContext context) {
      return new LessThanOrEqualLongsEvaluator(source, lhs.get(context), rhs.get(context), context);
    }

 ...
 */
public final class IntersectsStringStringEvaluator implements EvalOperator.ExpressionEvaluator {
  private final EvalOperator.ExpressionEvaluator leftValue;

  private final EvalOperator.ExpressionEvaluator rightValue;

  public IntersectsStringStringEvaluator(EvalOperator.ExpressionEvaluator leftValue,
  ...
  public Block eval(Page page) {
    ...
    BytesRefBlock leftValueBlock = (BytesRefBlock) leftValueUncastBlock;
  ...
  public BooleanBlock eval(int positionCount, BytesRefBlock leftValueBlock,
    ...
    position: for (int p = 0; p < positionCount; p++) {
      ...
      result.appendBoolean(Intersects.process(leftValueBlock.getBytesRef(leftValueBlock.getFirstValueIndex(p), leftValueScratch), rightValueBlock.getBytesRef(rightValueBlock.getFirstValueIndex(p), rightValueScratch)));
    }
    return result.build();
  }

  public BooleanVector eval(int positionCount, BytesRefVector leftValueVector,
      BytesRefVector rightValueVector) {
    BooleanVector.Builder result = BooleanVector.newVectorBuilder(positionCount);
    BytesRef leftValueScratch = new BytesRef();
    BytesRef rightValueScratch = new BytesRef();
    position: for (int p = 0; p < positionCount; p++) {
      result.appendBoolean(Intersects.process(leftValueVector.getBytesRef(p, leftValueScratch), rightValueVector.getBytesRef(p, rightValueScratch)));
    }
    return result.build();
  }
  ...
  public String toString() {
    return "IntersectsStringStringEvaluator[" + "leftValue=" + leftValue + ", rightValue=" + rightValue + "]";
  }
}

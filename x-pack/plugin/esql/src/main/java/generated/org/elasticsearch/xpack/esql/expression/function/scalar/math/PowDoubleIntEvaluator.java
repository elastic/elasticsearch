 ...
 */
public final class PowDoubleIntEvaluator implements EvalOperator.ExpressionEvaluator {
  private final EvalOperator.ExpressionEvaluator base;

  private final EvalOperator.ExpressionEvaluator exponent;

  public PowDoubleIntEvaluator(EvalOperator.ExpressionEvaluator base,
  ...
  static Double fold(Expression base, Expression exponent) {
    ...
    return Pow.process(((Number) baseVal).doubleValue(), ((Number) exponentVal).intValue());
  }
  ...
  public DoubleBlock eval(int positionCount, DoubleBlock baseBlock, IntBlock exponentBlock) {
    ...
    position: for (int p = 0; p < positionCount; p++) {
      ...
      result.appendDouble(Pow.process(baseBlock.getDouble(baseBlock.getFirstValueIndex(p)), exponentBlock.getInt(exponentBlock.getFirstValueIndex(p))));
    }
    return result.build();
  }

  public DoubleVector eval(int positionCount, DoubleVector baseVector, IntVector exponentVector) {
    DoubleVector.Builder result = DoubleVector.newVectorBuilder(positionCount);
    position: for (int p = 0; p < positionCount; p++) {
      result.appendDouble(Pow.process(baseVector.getDouble(p), exponentVector.getInt(p)));
    }
    return result.build();
  }
  ...
  public String toString() {
    return "PowDoubleIntEvaluator[" + "base=" + base + ", exponent=" + exponent + "]";
  }
}

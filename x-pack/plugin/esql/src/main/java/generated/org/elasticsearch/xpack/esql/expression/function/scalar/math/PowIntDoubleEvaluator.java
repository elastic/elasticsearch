 ...
 */
public final class PowIntDoubleEvaluator implements EvalOperator.ExpressionEvaluator {
  private final EvalOperator.ExpressionEvaluator base;

  private final EvalOperator.ExpressionEvaluator exponent;

  public PowIntDoubleEvaluator(EvalOperator.ExpressionEvaluator base,
  ...
  static Double fold(Expression base, Expression exponent) {
    ...
    return Pow.process(((Number) baseVal).intValue(), ((Number) exponentVal).doubleValue());
  }
  ...
  public DoubleBlock eval(int positionCount, IntBlock baseBlock, DoubleBlock exponentBlock) {
    ...
    position: for (int p = 0; p < positionCount; p++) {
      ...
      result.appendDouble(Pow.process(baseBlock.getInt(baseBlock.getFirstValueIndex(p)), exponentBlock.getDouble(exponentBlock.getFirstValueIndex(p))));
    }
    return result.build();
  }

  public DoubleVector eval(int positionCount, IntVector baseVector, DoubleVector exponentVector) {
    DoubleVector.Builder result = DoubleVector.newVectorBuilder(positionCount);
    position: for (int p = 0; p < positionCount; p++) {
      result.appendDouble(Pow.process(baseVector.getInt(p), exponentVector.getDouble(p)));
    }
    return result.build();
  }
  ...
  public String toString() {
    return "PowIntDoubleEvaluator[" + "base=" + base + ", exponent=" + exponent + "]";
  }
}

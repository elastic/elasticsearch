 ...
 */
public final class PowIntLongEvaluator implements EvalOperator.ExpressionEvaluator {
  private final EvalOperator.ExpressionEvaluator base;

  private final EvalOperator.ExpressionEvaluator exponent;

  public PowIntLongEvaluator(EvalOperator.ExpressionEvaluator base,
  ...
  static Long fold(Expression base, Expression exponent) {
    ...
    return Pow.process(((Number) baseVal).intValue(), ((Number) exponentVal).longValue());
  }
  ...
  public LongBlock eval(int positionCount, IntBlock baseBlock, LongBlock exponentBlock) {
    ...
    position: for (int p = 0; p < positionCount; p++) {
      ...
      result.appendLong(Pow.process(baseBlock.getInt(baseBlock.getFirstValueIndex(p)), exponentBlock.getLong(exponentBlock.getFirstValueIndex(p))));
    }
    return result.build();
  }

  public LongVector eval(int positionCount, IntVector baseVector, LongVector exponentVector) {
    LongVector.Builder result = LongVector.newVectorBuilder(positionCount);
    position: for (int p = 0; p < positionCount; p++) {
      result.appendLong(Pow.process(baseVector.getInt(p), exponentVector.getLong(p)));
    }
    return result.build();
  }
  ...
  public String toString() {
    return "PowIntLongEvaluator[" + "base=" + base + ", exponent=" + exponent + "]";
  }
}

 ...
 */
public final class ReplacePassthroughEvaluator implements EvalOperator.ExpressionEvaluator {
  private final EvalOperator.ExpressionEvaluator str;

  public ReplacePassthroughEvaluator(EvalOperator.ExpressionEvaluator str) {
    this.str = str;
  }
  ...
  public BytesRefBlock eval(int positionCount, BytesRefBlock strBlock) {
    ...
    position: for (int p = 0; p < positionCount; p++) {
      ...
      result.appendBytesRef(Replace.process(strBlock.getBytesRef(strBlock.getFirstValueIndex(p), strScratch)));
    }
    return result.build();
  }
  ...
  public BytesRefVector eval(int positionCount, BytesRefVector strVector) {
    ...
    BytesRef strScratch = new BytesRef();
    position: for (int p = 0; p < positionCount; p++) {
      result.appendBytesRef(Replace.process(strVector.getBytesRef(p, strScratch)));
    }
    return result.build();
  }
  ...
  public String toString() {
    return "ReplacePassthroughEvaluator[" + "str=" + str + "]";
  }
}

...
public class ResolveHighlight extends AnalyzerRule<Highlight> {
    ...
    protected LogicalPlan rule(Highlight highlight) {
        ...
        if (query == null) {
            HighlightSupport.ImplicitQuery implicitQuery = HighlightSupport.collectImplicitQuery(highlight.child(), highlight.source());
            query = implicitQuery.query();
            implicit = query != null;
            options = withDerivedAnalyzer(highlight.source(), options, implicitQuery.analyzerName());
        } else if (implicit == false) {
            // Implicit queries already synthesized an analyzer on the pass that derived them.
            options = withDerivedAnalyzer(highlight.source(), options, HighlightSupport.uniformAnalyzerOf(query));
        }
        ...
        if (query == highlight.query() && fields == highlight.fields() && Objects.equals(options, highlight.options())) {
            return highlight;
        }
        return highlight.withResolved(query, implicit, fields, generated).withOptions(options);
    }

...
/**
 * A Connector is the top-level SPI for external data sources.
 ...
 * </pre>
 *
 * <p>The expression after {@code :} is connector-specific and opaque to ES|QL.
 ...
 *       an external source reference. Returns the connector's logical plan node with schema.</li>
 *   <li><b>Logical Optimization</b> ({@link #optimizationRules}) - Connector-provided rules run
 *       as a separate pass after the main {@code LogicalPlanOptimizer}. Rules pattern-match on
 *       standard ES|QL plan nodes (Filter, Limit, etc.) above connector plan leaves and fold
 ...
 * "pushdown types", connectors provide their own tree-rewriting rules that fold operations
...
public interface Connector {
    ...
    /**
     * Declare what this connector supports for execution planning.
     *
    ...
    /**
     * Return optimization rules that push ES|QL operations into this connector's plan nodes.
     *
     * <p><b>Called by:</b> {@link ConnectorOptimizer}, which runs as a separate pass after
     * the main {@code LogicalPlanOptimizer} completes. This ensures connector rules see a
     ...
     * (e.g., {@code Filter}, {@code Limit}) whose child is a connector plan node, and
     * rewrites the tree by folding the operation into the connector plan.
     *
     * <p><b>Identity guard:</b> Rules MUST verify that the child connector plan belongs
    ...
    /**
     * Run all connector-provided optimization rules on a plan.
     *
     * <p><b>Called by:</b> {@code LogicalPlanOptimizer} after the main optimization pass.
     * This is internal wiring, not part of the connector SPI.
     *
     * <p>Walks the plan tree, collects {@link #optimizationRules()} from each connector
     * found in the tree, and runs them as a single batch. No-op if the plan contains
     * no connector plan nodes.
     *
     * @param plan The plan already processed by the main optimizer
     * @return The plan with connector optimizations applied
     */
    static LogicalPlan applyOptimizationRules(LogicalPlan plan) {
        return ConnectorOptimizer.optimize(plan);
    }
     ...
     *
     * <p><b>Called by:</b> {@code Mapper} when it visits the connector's logical plan node
     * during logical-to-physical plan conversion.
     *
     * <p>Default implementation creates a {@link ConnectorExec} wrapping the
     * connector plan. The {@link ConnectorExec} carries the fully-optimized plan
     * (with any pushed-down filters, limits, SQL, etc.) through physical planning
     * and into execution.
     *
     * <p>Connectors can override this if they need a custom physical plan node,
     * but the default is sufficient for most cases.
     *
     * @param plan The ConnectorPlan with all pushed operations
     * @param context Physical planning context (coordinator-only flag, etc.)
     * @return Physical plan node for this source
     */
    default PhysicalPlan createPhysicalPlan(ConnectorPlan plan, PhysicalPlanningContext context) {
        return new ConnectorExec(plan.source(), plan);
    }
     ...
     * for connectors where {@link #capabilities()}.{@link ConnectorCapabilities#distributed()}
     * is true.
     *
     * <p>Default returns a single partition for coordinator-only execution.
     *
     * @param plan The connector plan to partition
     ...
     */
    default List<ConnectorPartition> planPartitions(ConnectorPlan plan, DistributionHints hints) {
     ...
     */
    SourceOperator.SourceOperatorFactory createSourceOperator(ConnectorPartition partition, ExecutionContext context);
     ...
     * Connectors use the {@link #blockFactory()} to allocate Blocks and Vectors
    ...
    interface ExecutionContext {
         ...
         *
         * <p>Advanced use only — most connectors only need {@link #blockFactory()}.

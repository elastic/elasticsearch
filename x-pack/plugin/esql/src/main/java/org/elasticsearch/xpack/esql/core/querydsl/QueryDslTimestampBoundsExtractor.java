...
public final class QueryDslTimestampBoundsExtractor {
    private static final DateMathParser DEFAULT_DATE_TIME_PARSER = DateFieldMapper.DEFAULT_DATE_TIME_FORMATTER.toDateMathParser();
    private static final DateMathParser DEFAULT_DATE_TIME_NANOS_PARSER = DateFieldMapper.DEFAULT_DATE_TIME_NANOS_FORMATTER
        .toDateMathParser();
    ...
    private static Instant parseInstant(
        ...
        try {
            if (format != null) {
                return parseInstant(DateFormatter.forPattern(format).toDateMathParser(), stringValue, zone, nowSupplier);
            }
            return parseInstant(DEFAULT_DATE_TIME_NANOS_PARSER, stringValue, zone, nowSupplier);
        } catch (RuntimeException e) {
            if (format == null) {
                try {
                    return parseInstant(DEFAULT_DATE_TIME_PARSER, stringValue, zone, nowSupplier);
    ...
    private static Instant parseInstant(DateMathParser parser, String value, @Nullable ZoneId zone, LongSupplier nowSupplier) {

 ...
 *       an external source reference. Asynchronously resolves schema and calls back with the
 *       data source's logical plan node.</li>
...
public interface DataSource {
     ...
     * </ul>
     *
     * <p><b>Async contract:</b> Resolution may involve I/O (reading schema from remote storage,
     * connecting to a database, querying a catalog). Implementations MUST perform blocking I/O
     * on the executor provided by {@link ResolutionContext#executor()}, then call
     * {@link ActionListener#onResponse} with the resolved plan or {@link ActionListener#onFailure}
     * with any error.
     *
     * @param source The unresolved source with type, configuration, settings, and expression
     * @param context Resolution context providing cluster state, configuration, and executor
     * @param listener Callback for the resolved plan or failure
     */
    void resolve(DataSourceDescriptor source, ResolutionContext context, ActionListener<DataSourcePlan> listener);
     ...
     *
     * <p>Base classes (e.g., {@link org.elasticsearch.xpack.esql.datasource.lakehouse.spi.LakehouseDataSource})
     * provide default rules. Subclasses can override to add, remove, or replace rules.
     *
     ...
     * See {@link org.elasticsearch.xpack.esql.datasource.lakehouse.spi.LakehouseDataSource} for an example.
     *
     ...
     *
     * <p>Provides an {@link #executor()} for offloading blocking I/O (S3 HEAD requests,
     * JDBC connections, catalog API calls) off the transport thread.
     */
    interface ResolutionContext {
        /**
         * Executor for blocking I/O during resolution.
         *
         * <p>Data sources MUST run any blocking operations (network calls, file I/O)
         * on this executor rather than blocking the calling thread.
         */
        Executor executor();
    }

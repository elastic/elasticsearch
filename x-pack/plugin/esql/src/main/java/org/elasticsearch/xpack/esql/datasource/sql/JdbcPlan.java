 ...
 *
 * <p>This is an example data source implementation showing how a SQL-style
 * data source creates its plan node. Operations are accumulated as SQL fragments
 * that are combined into a single query for the database to execute.
 *
 * <h2>Lifecycle</h2>
 * <ol>
 *   <li>{@link JdbcDataSource#resolve} connects via JDBC, reads table schema, creates this node</li>
 *   <li>DataSource optimization rules push filter/limit/orderBy/aggregation via {@code with*()} methods</li>
...
public class JdbcPlan extends SqlPlan {
    ...
    public void writeTo(StreamOutput out) throws IOException {
        throw new UnsupportedOperationException("Serialization not yet implemented for JDBC data source");
    }
    ...
    public SqlPlan withFilter(Expression filter) {
        return new JdbcPlan(source(), dataSource, location, output, tableName, builtSql, filter, limit, orderBy, aggregation);
    }
    ...
    protected NodeInfo<JdbcPlan> info() {
        return NodeInfo.create(this, JdbcPlan::new, dataSource, location, output, tableName, builtSql, filter, limit, orderBy, aggregation);
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(dataSource, location, output, tableName, builtSql, filter, limit, orderBy, aggregation);
    }

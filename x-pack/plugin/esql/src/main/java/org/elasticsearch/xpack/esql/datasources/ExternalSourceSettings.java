...
public final class ExternalSourceSettings {
    ...
    /**
     * How many keys schema discovery may visit before it stops listing, when the listing it is building will be
     * used for nothing but a schema. What a schema costs should depend on the dataset's shape, not on how many
     * files carry it: a declared mapping is the schema outright, and {@code first_file_wins} takes it from one
     * file, so neither learns anything from the ten-thousandth key. {@code union_by_name} and {@code strict}
     * read every file by contract and are never bounded.
     * <p>
     * Default: 1,000 — one {@code ListObjectsV2} page, so a schema answer costs one request against a bucket.
     * Partition columns come from the paths visited, so a dataset whose outermost partition values are not all
     * present within this many keys in listing order surfaces the columns it can see; raise this for such a
     * layout. Bounded listings are never cached and never reach a query that reads rows, so raising it slows
     * schema discovery and changes nothing else.
     */
    public static final Setting<Integer> SCHEMA_DISCOVERY_MAX_KEYS = Setting.intSetting(
        "esql.external.schema_discovery_max_keys",
        1_000,
        1,
        10_000_000,
        Setting.Property.NodeScope,

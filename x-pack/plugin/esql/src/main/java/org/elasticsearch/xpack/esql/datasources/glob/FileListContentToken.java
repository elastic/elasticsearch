 ...
 * token is a pure function of the file SET — the same files listed in a different order produce the
 * same token, and any add/remove/mtime/size change produces a different one. The file count is mixed
 * into the final avalanche so that pathological cancellations across files still perturb the result.
 * <p>
 * The token is an identity for cache keying (correct-or-miss dataset-level derived state), not a
 ...
 */
final class FileListContentToken {

    private FileListContentToken() {}

 ...
 * <p>
 * {@code fileSet} bounds the files the expansion returns — what split discovery reads and what the listing
 * cache may hold. {@code partitionSample} bounds the paths partition detection folds over to decide the
 ...
 */
public record ListingExtents(int fileSet, int partitionSample) {
    ...
    public ListingExtents {
        if (fileSet < 1) {
            throw new IllegalArgumentException("fileSet extent must be positive, got [" + fileSet + "]");
        }
        if (partitionSample < 1) {
            throw new IllegalArgumentException("partitionSample extent must be positive, got [" + partitionSample + "]");
        }
    }

    public boolean boundsFileSet() {
        return fileSet != Integer.MAX_VALUE;
    }
     ...
     * subset of it. A read-only view of {@code files}, not a copy.
     * <p>
     * Sampled only where the file-set extent already bounded the listing. A full listing sampled here would ship
     * partition metadata covering a prefix of its own files — types from part of the dataset, and no partition
     * values at all for the files past the sample — and nothing downstream could tell.
     */
    public List<StorageEntry> partitionSampleOf(List<StorageEntry> files) {
        return files.size() <= partitionSample ? files : files.subList(0, partitionSample);
    }
}

 ...
 * - Directory listing via ListObjectsV2
 * - Metadata retrieval via HeadObject
 * - Configurable credentials and endpoint
 */
public final class S3StorageProvider implements StorageProvider {
    ...
    /**
     * Builds an S3Client from configuration.
     */
    private static S3Client buildS3Client(S3Configuration config) {
        ...
        if (config != null && config.region() != null) {
            ...
            // Use default region (from environment or AWS config)
            builder.region(Region.US_EAST_1);
        }

        // Configure endpoint override (for S3-compatible services like MinIO)
        if (config != null && config.endpoint() != null) {
            builder.endpointOverride(URI.create(config.endpoint()));
            // For custom endpoints, enable path-style access
    ...
    /**
     * Validates that the path uses an S3 scheme.
     */
    ...
    /**
     * Extracts the S3 key from the path.
    ...
    /**
     * Returns the S3Client used by this provider.
    ...
    private static final class S3StorageIterator implements StorageIterator {
        ...
        @Override
        public void close() throws IOException {
            // No resources to close for S3 iterator
        }

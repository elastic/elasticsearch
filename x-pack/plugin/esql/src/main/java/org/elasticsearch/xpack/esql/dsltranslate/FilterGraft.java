...
/**
 * Generic mechanism for grafting a Query DSL {@code filter} onto a logical plan as an ordinary {@link Filter}.
 *
 ...
 * Binding is therefore relative to the graft point, so the installed filter behaves exactly like a user-written
 * {@code WHERE} placed there. Callers decide <em>where</em> (the predicate); this class decides nothing about datasets,
 * indices, or views.
 *
 * <p>It is meant to run on the analyzed plan <em>before</em> optimization, so the grafted {@link Filter} enters the
 ...
 */
public final class FilterGraft {

    private FilterGraft() {}

    /**
     * Grafts {@code filter} above every node of {@code plan} that {@code target} accepts, binding it against that node's
     * output schema. Returns the rewritten plan, marked analyzed so the pre-optimizer accepts the fresh {@link Filter}
     * nodes and the spine rebuilt above them.
     *
     * @param target      selects the nodes to graft above (e.g. {@code node -> node instanceof ExternalRelation} for a
     ...
     * @param onDropped   invoked once per clause the translator could not apply at a given graft node, so the caller can
     *                    surface it (e.g. a response-header warning); the node is the one the clause was dropped for.
     */
    public static LogicalPlan graftFilterAt(
        LogicalPlan plan,
        ...
        LogicalPlan grafted = plan.transformUp(LogicalPlan.class, node -> {
            ...
            // A wholly-unsupported filter folds to TRUE (a no-op); leave the node unwrapped rather than graft it.

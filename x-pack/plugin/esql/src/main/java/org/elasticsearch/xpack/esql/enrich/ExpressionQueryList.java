...
public class ExpressionQueryList implements LookupEnrichQueryGenerator {
    ...
    private ExpressionQueryList(
        ...
        // Build pre-join filters using the context from planning (this is safe as filters are static)
    ...
    public Query getQuery(int position, Page inputPage, SearchExecutionContext searchExecutionContext) {
        ...
        // Build queries dynamically using the current SearchExecutionContext to avoid stale IndexReader references
        for (QueryBuilder filterBuilder : lucenePushableFilterBuilders) {
            try {
                Query preJoinFilter = filterBuilder.toQuery(searchExecutionContext);

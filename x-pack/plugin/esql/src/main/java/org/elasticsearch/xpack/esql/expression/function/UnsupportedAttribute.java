...
public final class UnsupportedAttribute extends FieldAttribute implements Unresolvable {
    ...
    private UnsupportedAttribute(StreamInput in) throws IOException {
        this(
            ...
            in.readString(),
            in.getTransportVersion().onOrAfter(TransportVersions.ESQL_ATTRIBUTE_CACHED_SERIALIZATION_8_15)
                ? EsField.readFrom(in)
    ...
    public void writeTo(StreamOutput out) throws IOException {
        if (((PlanStreamOutput) out).writeAttributeCacheHeader(this)) {
            ...
            if (out.getTransportVersion().onOrAfter(TransportVersions.ESQL_ATTRIBUTE_CACHED_SERIALIZATION_8_15)) {
                field().writeTo(out);
            } else {

...
public class Increase extends TimeSeriesAggregateFunction implements OptionalArgument, ToAggregator, TemporalityAware {
    ...
    public Increase(Source source, Expression field, Expression window, Expression timestamp) {
        this(source, field, window, timestamp, null);
    }
    ...
    private static Increase readFrom(StreamInput in) throws IOException {
        return readFrom(
            in,
            (source, field, filter, window, parameters) -> new Increase(
                source,
                field,
                filter,
                window,

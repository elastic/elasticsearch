...
public class TStep extends GroupingFunction.EvaluatableGroupingFunction
    ...
    public boolean needsTimestampBounds() {
        ...
        // (Batch 2 "Resolution"), so a string literal like "1 hour" still has KEYWORD type
        // here. Accept foldable KEYWORD as a proxy for "will become TIME_DURATION after
        // casting", so bounds are injected before postAnalysisVerification runs.
        // Integer (bucket-count) steps intentionally stay excluded: they require explicit
        // from/to bounds and must not silently inherit them from the request filter.

...
public class RemoteFetchHandleFunction extends EsqlScalarFunction {
    ...
    /*
     * Node/retained-session routing intentionally participates in expression identity through info(), so equivalent _doc
     * expressions for different retained sessions cannot be deduplicated together. The MVP remote-fetch planner
     * creates at most one handle expression per query; future multi-fetch planning should preserve this distinction.
     */
    ...
    private static final class Evaluator implements ExpressionEvaluator {
        ...
        public Block eval(Page page) {
            try (
                ...
                if (docVector == null) {
                    throw new IllegalStateException("remote fetch handle requires single-valued _doc entries");
                }

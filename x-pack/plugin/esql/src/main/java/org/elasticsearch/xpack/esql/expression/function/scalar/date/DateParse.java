...
public class DateParse extends ScalarFunction implements OptionalArgument, Mappable {
    ...
    static final DateFormatter DEFAULT_FORMATTER = toFormatter(new BytesRef("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"), UTC);
    ...
    public Object fold() {
        if (format == null) {
            return DateParseEvaluator.fold(field, DEFAULT_FORMATTER);
        }
        return DateParseEvaluator.fold(field, format, UTC);
    }

    // evaluators cannot be autogenerated (yet) here, because this method could result in an exception that has to be handled
    static long process(BytesRef val, DateFormatter formatter) throws DateTimeParseException {
        String dateString = val.utf8ToString();
        return formatter.parseMillis(dateString);
    }

    static long process(BytesRef val, BytesRef formatter, ZoneId zoneId) throws DateTimeParseException {

...
public class DayName extends EsqlScalarFunction implements ConfigurationFunction {
    ...
    public EvalOperator.ExpressionEvaluator.Factory toEvaluator(ToEvaluator toEvaluator) {
        ...
        if (field().dataType() == DataType.DATE_NANOS) {
            return new DayNameNanosEvaluator.Factory(source(), fieldEvaluator, zoneId, locale);
        }
        return new DayNameMillisEvaluator.Factory(source(), fieldEvaluator, zoneId, locale);
    }

    @Override
    public Expression replaceChildren(List<Expression> newChildren) {
        return new DayName(source(), newChildren.get(0));
    }

    @Override
    protected NodeInfo<? extends Expression> info() {
        return NodeInfo.create(this, DayName::new, field);
    }

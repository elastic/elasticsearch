...
/**
 * Evaluator for RangeMax(date_range) -> date.
 * Returns the maximum (end) value of a date_range.
 */
public class RangeMaxEvaluator implements EvalOperator.ExpressionEvaluator {
    ...
    public RangeMaxEvaluator(Source source, EvalOperator.ExpressionEvaluator range, DriverContext driverContext) {
        this.source = source;
        this.range = range;
    ...
    private Block eval(Block rangeBlock) {
        ...
        try (LongBlock.Builder result = driverContext.blockFactory().newLongBlockBuilder(positionCount)) {
            for (int p = 0; p < positionCount; p++) {
                ...
                // Get the maximum (end) value from the range
                long maxValue = toBlock.getLong(toBlock.getFirstValueIndex(p));

...
public class MvContains extends BinaryScalarFunction implements EvaluatorMapper {
    ...
    protected TypeResolution resolveType() {
        ...
        TypeResolution resolution = isRepresentableExceptCountersAndAggregateMetricDouble(left(), sourceText(), FIRST);
        if (resolution.unresolved()) {
            return resolution;
        }
        if (left().dataType() == DataType.NULL) {
            return isRepresentableExceptCountersAndAggregateMetricDouble(right(), sourceText(), SECOND);
        }

...
public class MvContains extends BinaryScalarFunction implements EvaluatorMapper {
    ...
    protected TypeResolution resolveType() {
        ...
        TypeResolution resolution = isRepresentableExceptCountersAndDenseVector(left(), sourceText(), FIRST);
        if (resolution.unresolved()) {
            return resolution;
        }
        if (left().dataType() == DataType.NULL) {
            return isRepresentableExceptCountersAndDenseVector(right(), sourceText(), SECOND);
        }

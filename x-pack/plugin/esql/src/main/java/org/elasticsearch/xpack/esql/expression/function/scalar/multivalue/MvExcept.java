 ...
 *
 * a = ["a","b","c"] b = ["b"] MV_EXCEPT(a,b) => ["a","c"]
 */
public class MvExcept extends BinaryScalarFunction implements EvaluatorMapper {
    public static final NamedWriteableRegistry.Entry ENTRY = new NamedWriteableRegistry.Entry(Expression.class, "MvExcept", MvExcept::new);
    ...
    @FunctionInfo(
        ...
        preview = true,
        examples = { @Example(file = "mv_except", tag = "mv_except") },
    ...
    private MvExcept(StreamInput in) throws IOException {
        super(in);
    }
    ...
    protected BinaryScalarFunction replaceChildren(Expression newLeft, Expression newRight) {
        return new MvExcept(source(), newLeft, newRight);
    }

    @Override
    protected NodeInfo<? extends Expression> info() {
        return NodeInfo.create(this, MvExcept::new, left(), right());
    }
    ...
    public EvalOperator.ExpressionEvaluator.Factory toEvaluator(EvaluatorMapper.ToEvaluator toEvaluator) {
        ...
        if (field1Type != ElementType.NULL && field2Type != ElementType.NULL && field1Type != field2Type) {
            throw new EsqlIllegalArgumentException(
                "Incompatible data types for MvExcept, field1 type({}) value({}) and field2 type({}) value({}) don't match.",
                field1Type,
                left(),
                field2Type,
                right()
        ...
        return switch (field1Type) {
            case BOOLEAN -> new MvExceptBooleanEvaluator.Factory(source(), toEvaluator.apply(left()), toEvaluator.apply(right()));
            case BYTES_REF -> new MvExceptBytesRefEvaluator.Factory(source(), toEvaluator.apply(left()), toEvaluator.apply(right()));
            case INT -> new MvExceptIntEvaluator.Factory(source(), toEvaluator.apply(left()), toEvaluator.apply(right()));
            case LONG -> new MvExceptLongEvaluator.Factory(source(), toEvaluator.apply(left()), toEvaluator.apply(right()));
            case DOUBLE -> new MvExceptDoubleEvaluator.Factory(source(), toEvaluator.apply(left()), toEvaluator.apply(right()));

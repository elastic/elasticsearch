...
public class MvLess extends MvCompare {
    ...
    @FunctionInfo(
        returnType = "boolean",
        briefSummary = "Checks whether a multivalue field has any value less than a bound.",
        ...
        examples = {
            ...
            @Example(description = "Strings are compared by their UTF-8 bytes:", file = "mv_compare", tag = "mv_less_keyword") },
        preview = true,
    ...
    public MvLess(
        ...
        super(source, field, bound, options, false);
    }

    public MvLess(Source source, Expression field, Expression bound) {
        this(source, field, bound, null);
    }

    private MvLess(StreamInput in) throws IOException {
        super(in, false);
    }
    ...
    public Expression replaceChildren(List<Expression> newChildren) {
        return new MvLess(source(), newChildren.get(0), newChildren.get(1), newChildren.size() > 2 ? newChildren.get(2) : null);
    }

    @Override
    protected NodeInfo<? extends Expression> info() {
        return NodeInfo.create(this, MvLess::new, field(), bound(), options());
    }
}

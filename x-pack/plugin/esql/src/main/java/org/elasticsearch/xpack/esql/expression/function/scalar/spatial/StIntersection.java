...
public class StIntersection extends BinarySpatialGeometryFunction implements AnyNullIsNull {
    public static final NamedWriteableRegistry.Entry ENTRY = new NamedWriteableRegistry.Entry(
        Expression.class,
        "StIntersection",
        StIntersection::new
    );
    public static final FunctionDefinition DEFINITION = FunctionDefinition.def(StIntersection.class)
        .binary(StIntersection::new)
    ...
    @FunctionInfo(
        ...
        examples = @Example(file = "spatial-jts", tag = "st_intersection"),
        depthOffset = 1
    )
    ...
    private StIntersection(Source source, Expression left, Expression right, boolean leftDocValues, boolean rightDocValues) {
        super(source, left, right, leftDocValues, rightDocValues);
    }

    private StIntersection(StreamInput in) throws IOException {
        this(Source.readFrom((PlanStreamInput) in), in.readNamedWriteable(Expression.class), in.readNamedWriteable(Expression.class));
    }

    @Override
    public Expression replaceChildren(List<Expression> newChildren) {
        return new StIntersection(source(), newChildren.get(0), newChildren.get(1));
    }

    @Override
    protected NodeInfo<? extends Expression> info() {
        return NodeInfo.create(this, StIntersection::new, left, right);
    }
    ...
    public BinarySpatialGeometryFunction withDocValues(boolean foundLeft, boolean foundRight) {
        return new StIntersection(source(), left, right, leftDocValues || foundLeft, rightDocValues || foundRight);
    }
    ...
    public ExpressionEvaluator.Factory toEvaluator(ToEvaluator toEvaluator) {
        ...
        if (leftDocValues && rightDocValues) {
            if (DataType.isSpatialGeo(left.dataType())) {
                return new StIntersectionBothGeoPointDocValuesEvaluator.Factory(source(), leftEval, rightEval);
            } else {
                return new StIntersectionBothCartesianPointDocValuesEvaluator.Factory(source(), leftEval, rightEval);
            }
        } else if (leftDocValues && left.dataType() == DataType.GEO_POINT) {
            return new StIntersectionSourceAndGeoPointDocValuesEvaluator.Factory(source(), rightEval, leftEval);
        } else if (leftDocValues && left.dataType() == DataType.CARTESIAN_POINT) {
            return new StIntersectionSourceAndCartesianPointDocValuesEvaluator.Factory(source(), rightEval, leftEval);
        } else if (rightDocValues && right.dataType() == DataType.GEO_POINT) {
            return new StIntersectionSourceAndGeoPointDocValuesEvaluator.Factory(source(), leftEval, rightEval);
        } else if (rightDocValues && right.dataType() == DataType.CARTESIAN_POINT) {
            return new StIntersectionSourceAndCartesianPointDocValuesEvaluator.Factory(source(), leftEval, rightEval);
        } else {
            return new StIntersectionSourceAndSourceEvaluator.Factory(source(), leftEval, rightEval);
        }
    }

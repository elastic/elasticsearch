...
public class StSymDifference extends BinarySpatialGeometryFunction implements AnyNullIsNull {
    public static final NamedWriteableRegistry.Entry ENTRY = new NamedWriteableRegistry.Entry(
        Expression.class,
        "StSymDifference",
        StSymDifference::new
    );
    public static final FunctionDefinition DEFINITION = FunctionDefinition.def(StSymDifference.class)
        .binary(StSymDifference::new)
    ...
    @FunctionInfo(
        ...
        examples = @Example(file = "spatial-jts", tag = "st_symdifference"),
        depthOffset = 1
    )
    ...
    private StSymDifference(Source source, Expression left, Expression right, boolean leftDocValues, boolean rightDocValues) {
        super(source, left, right, leftDocValues, rightDocValues);
    }

    private StSymDifference(StreamInput in) throws IOException {
        this(Source.readFrom((PlanStreamInput) in), in.readNamedWriteable(Expression.class), in.readNamedWriteable(Expression.class));
    }

    @Override
    public Expression replaceChildren(List<Expression> newChildren) {
        return new StSymDifference(source(), newChildren.get(0), newChildren.get(1));
    }

    @Override
    protected NodeInfo<? extends Expression> info() {
        return NodeInfo.create(this, StSymDifference::new, left, right);
    }
    ...
    public BinarySpatialGeometryFunction withDocValues(boolean foundLeft, boolean foundRight) {
        return new StSymDifference(source(), left, right, leftDocValues || foundLeft, rightDocValues || foundRight);
    }
    ...
    public ExpressionEvaluator.Factory toEvaluator(ToEvaluator toEvaluator) {
        ...
        if (leftDocValues && rightDocValues) {
            if (DataType.isSpatialGeo(left.dataType())) {
                return new StSymDifferenceBothGeoPointDocValuesEvaluator.Factory(source(), leftEval, rightEval);
            } else {
                return new StSymDifferenceBothCartesianPointDocValuesEvaluator.Factory(source(), leftEval, rightEval);
            }
        } else if (leftDocValues && left.dataType() == DataType.GEO_POINT) {
            return new StSymDifferenceSourceAndGeoPointDocValuesEvaluator.Factory(source(), rightEval, leftEval);
        } else if (leftDocValues && left.dataType() == DataType.CARTESIAN_POINT) {
            return new StSymDifferenceSourceAndCartesianPointDocValuesEvaluator.Factory(source(), rightEval, leftEval);
        } else if (rightDocValues && right.dataType() == DataType.GEO_POINT) {
            return new StSymDifferenceSourceAndGeoPointDocValuesEvaluator.Factory(source(), leftEval, rightEval);
        } else if (rightDocValues && right.dataType() == DataType.CARTESIAN_POINT) {
            return new StSymDifferenceSourceAndCartesianPointDocValuesEvaluator.Factory(source(), leftEval, rightEval);
        } else {
            return new StSymDifferenceSourceAndSourceEvaluator.Factory(source(), leftEval, rightEval);
        }
    }

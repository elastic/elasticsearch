...
public class StXMax extends SpatialUnaryDocValuesFunction {
    ...
    public Expression replaceChildren(List<Expression> newChildren) {
        return new StXMax(source(), newChildren.getFirst());
    }
    ...
    private static void fromWellKnownBinary(
        ...
        if (pointVisitor.isValid()) {
            results.appendDouble(visitor.getResult().getMaxX());
            return;
        }
    ...
    private static void fromDocValues(
        ...
        // It is possible to just use Math.max here, but using the visitor supports wrapping longitudes correctly
        for (int i = 0; i < valueCount; i++) {
            long encoded = encodedBlock.getLong(firstValueIndex + i);
            pointVisitor.visitPoint(spatialCoordinateType.decodeX(encoded), 0);
        }
        if (pointVisitor.isValid()) {
            results.appendDouble(pointVisitor.getResult().getMaxX());
            return;
        }
        throw new IllegalArgumentException("Cannot determine envelope of geometry");
    }

...
/**
 * Abstract class to serve as base for computing scalar functions over vectors, such as magnitude.
 */
public abstract class VectorScalarFunction extends UnaryScalarFunction implements EvaluatorMapper, VectorFunction {

    protected VectorScalarFunction(Source source, Expression field) {
        super(source, field);
    }

    protected VectorScalarFunction(StreamInput in) throws IOException {
        super(in);
    }
    ...
    /**
     * Returns the concrete vector scalar function.
     */
    protected abstract ScalarEvaluatorFunction getScalarFunction();

    @Override
    public final EvalOperator.ExpressionEvaluator.Factory toEvaluator(EvaluatorMapper.ToEvaluator toEvaluator) {
        return new ScalarEvaluatorFactory(toEvaluator.apply(field()), getScalarFunction(), getClass().getSimpleName() + "Evaluator");
    }

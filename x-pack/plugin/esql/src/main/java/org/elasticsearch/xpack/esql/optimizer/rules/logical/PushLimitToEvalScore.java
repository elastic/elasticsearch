...
public class PushLimitToEvalScore extends OptimizerRules.ParameterizedOptimizerRule<Limit, LogicalOptimizerContext> {
    ...
    public LogicalPlan rule(Limit limit, LogicalOptimizerContext ctx) {
        ...
        return limit.transformDown(plan -> {
            ...
            if (plan instanceof Eval eval) {
                if (shouldExecuteOnDataNode(eval) && (false == hasDirectLimitChild(eval))) {
    ...
    private static boolean shouldExecuteOnDataNode(Eval eval) {
        return eval.fields().stream().anyMatch(x -> x.child() instanceof ExecutesOn.Data);
    }

    private static boolean hasDirectLimitChild(Eval eval) {
        return eval.child() instanceof Limit;
    }
}

...
public class ProjectAwayColumns extends Rule<PhysicalPlan, PhysicalPlan> {
    ...
    private PhysicalPlan apply(PhysicalPlan plan, boolean isUnionBranch) {
        ...
        return plan.transformDown(currentPlanNode -> {
            ...
            if (currentPlanNode instanceof MergeExec mergeExec) {
                ...
                if (changed) {
                    // Preserve the original MergeExec output (which uses the union's NameIds) unless it is
            ...
            if (currentPlanNode instanceof ExchangeExec exec) {
                ...
                if (child instanceof FragmentExec fragmentExec) {
                    ...
                    // The only exception is when we are dealing with a union branch, because we might be dealing with a combination
                    ...
                    if ((logicalFragment instanceof Aggregate == false
                        ...
                        if (output.isEmpty() && isUnionBranch == false) {

...
public final class DocVectorConsumers {
    ...
    /** Runtime-search full-text functions do not require {@code _doc} because they evaluate each row directly. */
    public static boolean consumesDocVector(PhysicalPlan plan) {
        ...
        plan.forEachExpression(Expression.class, e -> {
            ...
            if (e instanceof Score || (e instanceof FullTextFunction ftf && ftf.isRuntimeSearch() == false)) {
                found.set(true);
            }
        });
        return found.get();

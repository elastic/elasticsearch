...
public abstract class ExpressionBuilder extends IdentifierBuilder {
    ...
    public Object visitNumericArrayLiteral(EsqlBaseParser.NumericArrayLiteralContext ctx) {
        ...
        if (numbers.stream().anyMatch(l -> l.dataType() == DataTypes.UNSIGNED_LONG)) {
            return new Literal(
                ...
                mapNumbers(
                    numbers,
                    (no, dt) -> dt == DataTypes.UNSIGNED_LONG ? no.longValue() : bigIntegerToUnsignedLong(BigInteger.valueOf(no.longValue()))
                ),
    ...
    private List<Object> mapNumbers(List<Literal> numbers, BiFunction<Number, DataTypes, Object> map) {
    ...
    private Object visitArrayLiteral(ParserRuleContext ctx, List<? extends ParserRuleContext> contexts, DataTypes dataType) {
        Source source = source(ctx);
    ...
    public Expression visitInlineCast(EsqlBaseParser.InlineCastContext ctx) {
        ...
        DataTypes dataType = typedParsing(this, ctx.dataType(), DataTypes.class);
    ...
    @Override
    public DataTypes visitToDataType(EsqlBaseParser.ToDataTypeContext ctx) {
        String typeName = visitIdentifier(ctx.identifier());
        DataTypes dataType = EsqlDataTypes.fromNameOrAlias(typeName);
    ...
    public Object visitInputParam(EsqlBaseParser.InputParamContext ctx) {
        ...
        DataTypes dataType = EsqlDataTypes.fromTypeName(param.type);
        Source source = source(ctx);
        if (dataType == null) {

...
public class Mapper {
    ...
    private PhysicalPlan mapUnary(UnaryPlan unary) {
        ...
        if (unary instanceof Completion completion) {
            ...
            return new CompletionExec(completion.source(), mappedChild, completion.target(), completion.prompt(), completion.inferenceId());
        }
    ...
    public static boolean isPipelineBreaker(LogicalPlan p) {
        return p instanceof Aggregate || p instanceof TopN || p instanceof Limit || p instanceof OrderBy || p instanceof Completion;
    }

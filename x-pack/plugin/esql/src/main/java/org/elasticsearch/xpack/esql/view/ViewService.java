    ...
    public LogicalPlan replaceViews(LogicalPlan plan, PlanTelemetry telemetry) {
        ...
        plan = plan.transformUp(Fork.class, fork -> {
            ...
            for (int i = 0; i < subplans.size(); i++) {
                ...
                LogicalPlan subplan = resolveViewsSubPlan(subplans.get(i), views, innerSeen, telemetry);
        ...
        return resolveViewsSubPlan(plan, views, seen, telemetry);
    }

    private LogicalPlan resolveViewsSubPlan(LogicalPlan plan, ViewMetadata views, List<String> seen, PlanTelemetry telemetry) {
        while (true) {

...
public class GlobDiscoveryLocalTests extends ESTestCase {
    ...
    private static class TestLocalStorageProvider implements StorageProvider {
            ...
            if (recursive) {
                Files.walkFileTree(dirPath, new SimpleFileVisitor<>() {
                    ...
                    public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) {
                        if (attrs.isRegularFile() && isTestDataFile(file)) {
                ...
                try (DirectoryStream<Path> stream = Files.newDirectoryStream(dirPath)) {
                    for (Path entry : stream) {
                        ...
                        if (attrs.isRegularFile() && isTestDataFile(entry)) {
        ...
        /** Filter out ESTestCase temp dir artifacts (e.g., extra0 files) that have no extension. */

...
/**
 * Tests for SingleFileExternalSourceOperatorFactory.
 *
 * This demonstrates the integration of StorageProvider and FormatReader
 * to create source operators for external data.
 */
public class SingleFileExternalSourceOperatorFactoryTests extends ESTestCase {
    ...
    public void testCreateOperatorWithMockedStorageAndFormat() throws IOException {
        ...
        SingleFileExternalSourceOperatorFactory factory = new SingleFileExternalSourceOperatorFactory(
            storageProvider,
            formatReader,
            path,
            attributes,
    ...
    public void testFactoryValidation() {
        ...
        expectThrows(
            ...
            () -> new SingleFileExternalSourceOperatorFactory(null, formatReader, path, attributes, 1000)
        );

        // Test null format reader
        expectThrows(
            IllegalArgumentException.class,
            () -> new SingleFileExternalSourceOperatorFactory(storageProvider, null, path, attributes, 1000)
        );

        // Test null path
        expectThrows(
            IllegalArgumentException.class,
            () -> new SingleFileExternalSourceOperatorFactory(storageProvider, formatReader, null, attributes, 1000)
        );

        // Test null attributes
        expectThrows(
            IllegalArgumentException.class,
            () -> new SingleFileExternalSourceOperatorFactory(storageProvider, formatReader, path, null, 1000)
        );

        // Test invalid batch size
        expectThrows(
            IllegalArgumentException.class,
            () -> new SingleFileExternalSourceOperatorFactory(storageProvider, formatReader, path, attributes, 0)
        );

        expectThrows(
            IllegalArgumentException.class,
            () -> new SingleFileExternalSourceOperatorFactory(storageProvider, formatReader, path, attributes, -1)
        );
    }

 ...
 */
public record TestCaseSupplier(String name, List<DataTypes> types, Supplier<TestCase> supplier)
    implements
     ...
     */
    public TestCaseSupplier(List<DataTypes> types, Supplier<TestCase> supplier) {
        this(nameFromTypes(types), types, supplier);
    }

    public static String nameFromTypes(List<DataTypes> types) {
    ...
    public static List<TestCaseSupplier> stringCases(
        ...
        BiFunction<DataTypes, DataTypes, String> evaluatorToString,
        List<String> warnings,
        DataTypes expectedType
    ) {
        List<TypedDataSupplier> lhsSuppliers = new ArrayList<>();
        List<TypedDataSupplier> rhsSuppliers = new ArrayList<>();
        List<TestCaseSupplier> suppliers = new ArrayList<>();
        for (DataTypes type : AbstractConvertFunction.STRING_TYPES) {
    ...
    public static void casesCrossProduct(
        ...
        List<TypedDataSupplier> lhsSuppliers,
        List<TypedDataSupplier> rhsSuppliers,
        BiFunction<DataTypes, DataTypes, Matcher<String>> evaluatorToString,
    ...
    public record NumericTypeTestConfigs<T>(
        ...
        public NumericTypeTestConfig<T> get(DataTypes type) {
    ...
    public static DataTypes widen(DataTypes lhs, DataTypes rhs) {
        if (lhs == rhs) {
            return lhs;
        }
    ...
    public static List<TypedDataSupplier> getSuppliersForNumericType(DataTypes type, Number min, Number max, boolean includeZero) {
        if (type == DataTypes.INTEGER) {
    ...
    public static List<TestCaseSupplier> forBinaryComparisonWithWidening(
        ...
        for (DataTypes lhsType : numericTypes) {
            for (DataTypes rhsType : numericTypes) {
                DataTypes expected = widen(lhsType, rhsType);
                NumericTypeTestConfig<Boolean> expectedTypeStuff = typeStuff.get(expected);
                BiFunction<DataTypes, DataTypes, String> evaluatorToString = (lhs, rhs) -> expectedTypeStuff.evaluatorName()
                    + "["
                    + lhsName
                    + "="
    ...
    public static List<TypedDataSupplier> stringCases(DataTypes type) {
    ...
    public static String getCastEvaluator(String original, DataTypes current, DataTypes target) {
        if (current == target) {
            return original;
        }
    ...
    private static String castToLongEvaluator(String original, DataTypes current) {
    ...
    private static String castToUnsignedLongEvaluator(String original, DataTypes current) {
        if (current == DataTypes.UNSIGNED_LONG) {
            return original;
        }
        if (current == DataTypes.INTEGER) {
    ...
    private static String castToDoubleEvaluator(String original, DataTypes current) {
        if (current == DataTypes.DOUBLE) {
            return original;
        }
        if (current == DataTypes.INTEGER) {
    ...
    public static class TestCase {
         ...
         */
        private final DataTypes expectedType;
        /**
        ...
        public TestCase(List<TypedData> data, String evaluatorToString, DataTypes expectedType, Matcher<Object> matcher) {
            this(data, equalTo(evaluatorToString), expectedType, matcher);
        }

        public TestCase(List<TypedData> data, Matcher<String> evaluatorToString, DataTypes expectedType, Matcher<Object> matcher) {
     ...
     */
    public record TypedDataSupplier(String name, Supplier<Object> supplier, DataTypes type, boolean forceLiteral) {

        public TypedDataSupplier(String name, Supplier<Object> supplier, DataTypes type) {
    ...
    public static class TypedData {
         ...
         */
        private TypedData(Object data, DataTypes type, String name, boolean forceLiteral) {
         ...
         * @param name a name for the value, used for generating test case names
         */
        public TypedData(Object data, DataTypes type, String name) {

...
public class StdVarTests extends AbstractAggregationTestCase {
    public StdVarTests(@Name("TestCase") Supplier<TestCaseSupplier.TestCase> testCaseSupplier) {
    ...
    public static Iterable<Object[]> parameters() {
        ...
        ).flatMap(List::stream).map(StdVarTests::makeSupplier).collect(Collectors.toCollection(() -> suppliers));
    ...
    protected Expression build(Source source, List<Expression> args) {
        return new StdVar(source, args.get(0));
    }
    ...
    private static TestCaseSupplier makeSupplier(TestCaseSupplier.TypedDataSupplier fieldSupplier) {
        return new TestCaseSupplier(List.of(fieldSupplier.type()), () -> {
            ...
            WelfordAlgorithm welfordAlgorithm = new WelfordAlgorithm(false);

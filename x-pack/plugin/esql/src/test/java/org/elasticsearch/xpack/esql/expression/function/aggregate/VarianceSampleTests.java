...
public class VarianceSampleTests extends AbstractAggregationTestCase {
    public VarianceSampleTests(@Name("TestCase") Supplier<TestCaseSupplier.TestCase> testCaseSupplier) {
    ...
    public static Iterable<Object[]> parameters() {
        ...
        ).flatMap(List::stream).map(VarianceSampleTests::makeSupplier).collect(Collectors.toCollection(() -> suppliers));
    ...
    protected Expression build(Source source, List<Expression> args) {
        return new VarianceSample(source, args.get(0));
    }
    ...
    private static TestCaseSupplier makeSupplier(TestCaseSupplier.TypedDataSupplier fieldSupplier) {
        return new TestCaseSupplier(List.of(fieldSupplier.type()), () -> {
            ...
            var result = welfordAlgorithm.evaluateSampleVariance();
            var expected = Double.isInfinite(result) ? null : result;
            return new TestCaseSupplier.TestCase(
                List.of(fieldTypedData),
                "VarianceSample[field=Attribute[channel=0]]",
                DataType.DOUBLE,
                equalTo(expected)
            );
        });

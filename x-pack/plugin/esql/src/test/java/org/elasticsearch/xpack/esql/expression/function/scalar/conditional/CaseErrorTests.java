...
public class CaseErrorTests extends ErrorsForCasesWithoutExamplesTestCase {
    ...
    protected Matcher<String> expectedTypeErrorMatcher(List<Set<DataType>> validPerPosition, List<DataType> signature) {
        ...
        if (mainType == DataType.DATE_RANGE) {
            return typeErrorMessage(signature, 1, "any but date_range");
        }
        for (int i = 2; i < signature.size(); i++) {
            if (i % 2 == 0 && i != signature.size() - 1) {
                ...
                if (signature.get(i) == DataType.DATE_RANGE) {
                    return typeErrorMessage(signature, i, "any but date_range");
                }
            }
        }

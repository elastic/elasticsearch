...
public class ClampTests extends AbstractScalarFunctionTestCase {
        ...
        for (DataType numericType : DataType.types().stream().filter(DataType::isNumeric).toList()) {
            ...
            suppliers.add(
                new TestCaseSupplier(
                    ...
                    () -> new TestCaseSupplier.TestCase(
                        List.of(
                            new TestCaseSupplier.TypedData(1, numericType, "a"),
                            new TestCaseSupplier.TypedData(2, numericType, "b"),
                            new TestCaseSupplier.TypedData(3, numericType, "c")
                        ),
                        String.format(
                            Locale.ROOT,
    ...
    @Override
    protected Clamp build(Source source, List<Expression> args) {

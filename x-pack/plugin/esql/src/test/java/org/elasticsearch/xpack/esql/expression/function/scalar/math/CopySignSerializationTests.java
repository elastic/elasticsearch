...
public class CopySignSerializationTests extends AbstractExpressionSerializationTests<CopySign> {
    ...
    protected CopySign mutateInstance(CopySign instance) throws IOException {
        ...
        Expression magnitude = instance.children().get(0);
        Expression sign = instance.children().get(1);
        if (randomBoolean()) {
            magnitude = randomValueOtherThan(magnitude, AbstractUnaryScalarSerializationTests::randomChild);
        } else {
            sign = randomValueOtherThan(sign, AbstractUnaryScalarSerializationTests::randomChild);
        }
        return new CopySign(source, magnitude, sign);
    }
}

...
public class MvGreaterTests extends AbstractScalarFunctionTestCase {
    public MvGreaterTests(@Name("TestCase") Supplier<TestCaseSupplier.TestCase> testCaseSupplier) {
        this.testCase = testCaseSupplier.get();
    }

    private static String channels(boolean includeBound) {
        return "[field=Attribute[channel=0], bound=Attribute[channel=1], greater=true, includeBound=" + includeBound + "]";
    }

    // The default (2-argument) form resolves includeBound to false.
    ...
    public static Iterable<Object[]> parameters() {
        ...
        addInt(suppliers, List.of(1, 5, 10), 4, true); // 5 and 10 clear 4
        addInt(suppliers, List.of(0, 100), 40, true); // 100 clears 40
        addInt(suppliers, List.of(4), 4, false); // strict: equal does not match
        addInt(suppliers, List.of(3), 4, false); // below the bound
        ...
        addIntOpt(withNulls, List.of(4), 4, true, true); // inclusive: equal matches
        addIntOpt(withNulls, List.of(4), 4, false, false); // exclusive: equal does not
        addIntOpt(withNulls, List.of(5), 4, false, true); // exclusive: above matches
        addLongOpt(withNulls, DataType.LONG, List.of(5L), 5L, true, true);
        addLongOpt(withNulls, DataType.LONG, List.of(5L), 5L, false, false);
        addLongOpt(withNulls, DataType.DATETIME, List.of(9L), 5L, false, true);
        ...
        addBytesRefOpt(withNulls, "m", "b", false, true);
    ...
    protected Expression build(Source source, List<Expression> args) {
        return new MvGreater(source, args.get(0), args.get(1), args.size() > 2 ? args.get(2) : null);
    }

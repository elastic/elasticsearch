...
public class MvLastTests extends AbstractMultivalueFunctionTestCase {
    ...
    public static Iterable<Object[]> parameters() {
        ...
        geoPoints(cases, "mv_last", "MvLast", EsqlDataTypes.GEO_POINT, (size, values) -> equalTo(values.reduce((f, s) -> s).getAsLong()));
        cartesianPoints(
            cases,
            "mv_last",
            "MvLast",
            EsqlDataTypes.CARTESIAN_POINT,
            (size, values) -> equalTo(values.reduce((f, s) -> s).getAsLong())
        );

...
public class SpatialContainsTests extends AbstractFunctionTestCase {
                ...
                if (typeCompatible(leftType, rightType)) {
                    ...
                    suppliers.add(
                        TestCaseSupplier.testCaseSupplier(
                            leftDataSupplier,
                            rightDataSupplier,
                            SpatialContainsTests::spatialEvaluatorString,
                            DataTypes.BOOLEAN,
    ...
    private static TestCaseSupplier.TypedDataSupplier testCaseSupplier(DataType dataType) {
        return switch (dataType.esType()) {
            ...
            default -> throw new IllegalArgumentException("Unsupported datatype for ST_CONTAINS: " + dataType);
        };
    }
    ...
    private static Object expected(Object left, DataType leftType, Object right, DataType rightType) {
        ...
        try {
            return spatialContains.geometryRelatesGeometry(leftWKB, rightWKB);
        } catch (IOException e) {
    ...
    private static SpatialRelatesFunction.SpatialRelations spatialRelations(
        ...
        if (isSpatialGeo(leftType) || isSpatialGeo(rightType)) {
            return SpatialContains.GEO;
        } else if (isSpatial(leftType) || isSpatial(rightType)) {
            return SpatialContains.CARTESIAN;
        } else {
    ...
    private static String spatialEvaluatorString(DataType leftType, DataType rightType) {
        ...
        return "SpatialContains" + crsType + "SourceAndSourceEvaluator[leftValue=Attribute[channel=0], rightValue=Attribute[channel=1]]";
    }

...
public class StSimplifyPreserveTopologyTests extends AbstractScalarFunctionTestCase {
    ...
    public static Iterable<Object[]> parameters() {
        ...
        addTestCaseSuppliers(suppliers, GEO_POINT, StSimplifyTests.testCaseSupplier(GEO_POINT));
        addTestCaseSuppliers(suppliers, CARTESIAN_POINT, StSimplifyTests.testCaseSupplier(CARTESIAN_POINT));
        addTestCaseSuppliers(suppliers, GEO_SHAPE, StSimplifyTests.testCaseSupplier(GEO_SHAPE));
        addTestCaseSuppliers(suppliers, CARTESIAN_SHAPE, StSimplifyTests.testCaseSupplier(CARTESIAN_SHAPE));
        ...
        for (DataType toleranceType : new DataType[] { DataType.DOUBLE, DataType.FLOAT, DataType.LONG, DataType.INTEGER }) {
            ...
            suppliers.add(new TestCaseSupplier(testName, List.of(spatialType, toleranceType), () -> {
                ...
                String evaluatorName =
                    "StSimplifyPreserveTopologyNonFoldableGeometryAndFoldableToleranceEvaluator[geometry=Attribute[channel=0], tolerance="
                        + tolerance
                        + "]";
    ...
    private static BytesRef valueOf(BytesRef wkb, double tolerance) {
        ...
        try {
            ...
            org.locationtech.jts.geom.Geometry simplifiedGeometry = TopologyPreservingSimplifier.simplify(jtsGeometry, tolerance);

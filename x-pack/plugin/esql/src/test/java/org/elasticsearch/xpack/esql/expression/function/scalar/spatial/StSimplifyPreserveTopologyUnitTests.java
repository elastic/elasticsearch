...
public class StSimplifyPreserveTopologyUnitTests extends ESTestCase {
    ...
    public void testTopologyPreserved() {
        // A bowtie polygon — ST_SIMPLIFY would change the topology, but ST_SIMPLIFYPRESERVETOPOLOGY should keep it valid
        var wkb = process("POLYGON((0 0, 2 2, 0 2, 2 0, 0 0))", 0.0);
        var result = UNSPECIFIED.wkbToWkt(wkb);
        // TopologyPreservingSimplifier preserves the topology
    ...
    public void testSimplificationPreservesValidity() {
        // A polygon that ST_SIMPLIFY might make empty but topology-preserving version keeps valid
        var wkb = process("POLYGON((0 0, 1 0.1, 2 0, 2 2, 1 1.9, 0 2, 0 0))", 0.2);
        var result = UNSPECIFIED.wkbToWkt(wkb);
        assertEquals("POLYGON ((0.0 0.0, 2.0 0.0, 2.0 2.0, 0.0 2.0, 0.0 0.0))", result);
    }
}

...
public class ConcatTests extends AbstractFunctionTestCase {
    ...
    public static Iterable<Object[]> parameters() {
        ...
        Set<DataTypes> supported = Set.of(DataTypes.NULL, DataTypes.KEYWORD, DataTypes.TEXT);
        List<Set<DataTypes>> supportedPerPosition = List.of(supported, supported);
        for (DataTypes lhs : DataTypes.types()) {
            if (lhs == DataTypes.NULL || EsqlDataTypes.isRepresentable(lhs) == false) {
                continue;
            }
            for (DataTypes rhs : DataTypes.types()) {
    ...
    private static TestCaseSupplier supplier(String name, DataTypes type, int length, Supplier<String> valueSupplier) {
    ...
    private static void add(List<TestCaseSupplier> suppliers, String name, int length, Supplier<String> valueSupplier) {
        Map<Integer, List<List<DataTypes>>> permutations = new HashMap<Integer, List<List<DataTypes>>>();
        List<DataTypes> supportedDataTypes = List.of(DataTypes.KEYWORD, DataTypes.TEXT);
        permutations.put(0, List.of(List.of(DataTypes.KEYWORD), List.of(DataTypes.TEXT)));
        for (int v = 0; v < length - 1; v++) {
            List<List<DataTypes>> current = permutations.get(v);
            List<List<DataTypes>> next = new ArrayList<>();
            for (int i = 0; i < current.size(); i++) {
                for (int j = 0; j < supportedDataTypes.size(); j++) {
                    List<DataTypes> n = new ArrayList<>(current.get(i));
        ...
        for (List<DataTypes> types : permutations.get(length - 1)) {

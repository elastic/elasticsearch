...
public class LeftTests extends AbstractScalarFunctionTestCase {
    ...
    public static Iterable<Object[]> parameters() {
        ...
        suppliers.add(new TestCaseSupplier("ascii as text input", List.of(DataType.TEXT, DataType.INTEGER), () -> {
            String text = randomAlphaOfLengthBetween(1, 64);
            int length = between(1, text.length());
            return new TestCaseSupplier.TestCase(
                List.of(
    ...
    private String process(String str, int length) {
        try (
            EvalOperator.ExpressionEvaluator eval = evaluator(
                new Left(Source.EMPTY, field("str", DataType.KEYWORD), new Literal(Source.EMPTY, length, DataType.INTEGER))
            ).get(driverContext());

...
public class LogicalPlanOptimizerTests extends AbstractLogicalPlanOptimizerTests {
    ...
    public void testInlineStatsOverSubqueryUnusedOutput() {
        assumeTrue("Requires subquery in FROM command support", EsqlCapabilities.Cap.SUBQUERY_IN_FROM_COMMAND.isEnabled());
        assumeTrue(
            "Requires the fix to inline stats pruning for subqueries",
            EsqlCapabilities.Cap.SUBQUERY_IN_FROM_COMMAND_INLINE_STATS_PRUNING.isEnabled()
        );
        var query = """
            FROM test, (FROM test)
            | EVAL x = emp_no
    ...
    public void testInlineStatsOverForkUnusedOutput() {
        assumeTrue(
            "Requires the fix to inline stats pruning over Fork",
            EsqlCapabilities.Cap.SUBQUERY_IN_FROM_COMMAND_INLINE_STATS_PRUNING.isEnabled()
        );
        var query = """
            from employees

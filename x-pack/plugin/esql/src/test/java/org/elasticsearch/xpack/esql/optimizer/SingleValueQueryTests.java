...
public class SingleValueQueryTests extends MapperServiceTestCase {
    interface Setup {
        ...
        void assertStats(SingleValueQuery.Builder builder, QueryBuilder rewritten);
    }
    ...
    public void testMatchNone() throws IOException {
        testCase(new SingleValueQuery.Builder(new MatchNoneQueryBuilder(), "foo", new SingleValueQuery.Stats()), (fieldValues, count) -> {
        ...
        try (Directory d = newDirectory(); RandomIndexWriter iw = new RandomIndexWriter(random(), d)) {
            ...
            try (IndexReader reader = iw.getReader()) {
                ...
                if (rewritten instanceof MatchNoneQueryBuilder) {
                    ...
                    setup.assertStats(builder, rewritten);
                }
    ...
    private record StandardSetup(String fieldType, boolean multivaluedField) implements Setup {
        ...
        @Override
        public void assertStats(SingleValueQuery.Builder builder, QueryBuilder rewritten) {
            assertThat(builder.stats().missingField(), equalTo(0));
            switch (fieldType) {
                case "long", "integer", "short", "byte", "double", "float" -> {
                    ...
                    if (multivaluedField) {
                        ...
                        if (builder.next() instanceof MatchPhraseQueryBuilder) {
                            assertThat(builder.stats().numericMultiNoApprox(), equalTo(0));

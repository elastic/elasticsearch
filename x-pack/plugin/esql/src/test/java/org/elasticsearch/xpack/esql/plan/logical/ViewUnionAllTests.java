...
public class ViewUnionAllTests extends ESTestCase {
    ...
    /**
     * Completes the type-preservation set alongside {@link #testReplaceChildrenPreservesType}, {@link #testReplaceSubPlansPreservesType}
     * and {@link #testReplaceSubPlansAndOutputPreservesType}. {@code refreshOutput} was the one plan-returning method without an override,
     * so it returned a plain {@link UnionAll} and dropped the named-subqueries map - which also let a nested view union slip past
     * {@code UnionAll.checkNestedUnionAlls}, since that check only rejects nested unions that are still {@link ViewUnionAll}.
     */

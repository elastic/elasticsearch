...
public class TimeSeriesLifecycleActionsIT extends ESRestTestCase {
    ...
    public void testFullPolicy() throws Exception {
        ...
        // create policy
        TimeSeriesRestDriver.createFullPolicy(client(), policy, TimeValue.ZERO);
    ...
    public void testMoveToAllocateStep() throws Exception {
        ...
        // create policy
        TimeSeriesRestDriver.createFullPolicy(client(), policy, TimeValue.timeValueHours(10));
        // update policy on index
        updatePolicy(originalIndex, policy);

        // move to a step
        Request moveToStepRequest = new Request("POST", "_ilm/move/" + originalIndex);
        assertBusy(() -> assertTrue(TimeSeriesRestDriver.getStepKeyForIndex(client(), originalIndex).equals(new StepKey("new", "complete", "complete"))));
    ...
    public void testMoveToRolloverStep() throws Exception {
        ...
        index(client(), originalIndex, "_id", "foo", "bar");
        logger.info(TimeSeriesRestDriver.getStepKeyForIndex(client(), originalIndex));
        moveToStepRequest.setJsonEntity("{\n" +
            "  \"current_step\": {\n" +
            "    \"phase\": \"new\",\n" +
            "    \"action\": \"complete\",\n" +
    ...
    public void testRetryFailedDeleteAction() throws Exception {
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "delete", new DeleteAction());
        ...
        assertBusy(() -> assertThat((Integer) TimeSeriesRestDriver.explainIndex(client(), index).get(FAILED_STEP_RETRY_COUNT_FIELD), greaterThanOrEqualTo(1)),
            30, TimeUnit.SECONDS);
    ...
    public void testRetryFreezeDeleteAction() throws Exception {
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "cold", new FreezeAction());
        ...
        assertBusy(() -> assertThat((Integer) TimeSeriesRestDriver.explainIndex(client(), index).get(FAILED_STEP_RETRY_COUNT_FIELD), greaterThanOrEqualTo(1)),
            30, TimeUnit.SECONDS);
        assertFalse(TimeSeriesRestDriver.getOnlyIndexSettings(client(), index).containsKey("index.frozen"));

        Request request = new Request("PUT", index + "/_settings");
        request.setJsonEntity("{\"index.blocks.read_only\":false}");
        assertOK(client().performRequest(request));

        assertBusy(() -> assertThat(TimeSeriesRestDriver.getOnlyIndexSettings(client(), index).get("index.frozen"), equalTo("true")));
    }
    ...
    public void testRetryFailedShrinkAction() throws Exception {
        ...
        createIndexWithSettings(index, Settings.builder().put(IndexMetadata.SETTING_NUMBER_OF_SHARDS, numShards)
            .put(IndexMetadata.SETTING_NUMBER_OF_REPLICAS, 0));
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "warm", new ShrinkAction(numShards + randomIntBetween(1, numShards)));
        ...
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "warm", new ShrinkAction(expectedFinalShards));
        updatePolicy(index, policy);
        ...
        assertBusy(() -> {
            Map<String, Object> settings = TimeSeriesRestDriver.getOnlyIndexSettings(client(), shrunkenIndex);
        ...
        expectThrows(ResponseException.class, () -> TimeSeriesRestDriver.indexDocument(client(), index));
    }
    ...
    public void testRolloverAction() throws Exception {
        ...
        // create policy
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "hot", new RolloverAction(null, null, 1L));
        // update policy on index
        updatePolicy(originalIndex, policy);
        // index document {"foo": "bar"} to trigger rollover
        index(client(), originalIndex, "_id", "foo", "bar");
        ...
        assertBusy(() -> assertEquals("true",
            TimeSeriesRestDriver.getOnlyIndexSettings(client(), originalIndex).get(LifecycleSettings.LIFECYCLE_INDEXING_COMPLETE)));
    }
    ...
    public void testRolloverActionWithIndexingComplete() throws Exception {
        ...
        // index document {"foo": "bar"} to trigger rollover
        index(client(), originalIndex, "_id", "foo", "bar");
        assertBusy(() -> assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), originalIndex), equalTo(PhaseCompleteStep.finalStep("hot").getKey())));
        assertBusy(() -> assertTrue(indexExists(originalIndex)));
    ...
    public void testAllocateOnlyAllocation() throws Exception {
        ...
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, endPhase, allocateAction);
        updatePolicy(index, policy);
        assertBusy(() -> {
            assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index), equalTo(PhaseCompleteStep.finalStep(endPhase).getKey()));
        });
        ensureGreen(index);
    }
    ...
    public void testAllocateActionOnlyReplicas() throws Exception {
        ...
        updatePolicy(index, policy);
        assertBusy(() -> {
            Map<String, Object> settings = TimeSeriesRestDriver.getOnlyIndexSettings(client(), index);
            assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index), equalTo(PhaseCompleteStep.finalStep(endPhase).getKey()));
    ...
    public void testWaitForSnapshot() throws Exception {
        ...
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "delete", new WaitForSnapshotAction(slmPolicy));
        updatePolicy(index, policy);
        assertBusy( () -> {
            Map<String, Object> indexILMState = TimeSeriesRestDriver.explainIndex(client(), index);
        ...
        assertBusy(() -> assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index).getAction(), equalTo("complete")), slmPolicy);
    }
    ...
    public void testDeleteOnlyShouldNotMakeIndexReadonly() throws Exception {
        createIndexWithSettings(index, Settings.builder().put(IndexMetadata.SETTING_NUMBER_OF_SHARDS, 1)
            .put(IndexMetadata.SETTING_NUMBER_OF_REPLICAS, 0));
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "delete", new DeleteAction(), TimeValue.timeValueHours(1));
        updatePolicy(index, policy);
        assertBusy(() -> {
            assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index).getAction(), equalTo("complete"));
            Map<String, Object> settings = TimeSeriesRestDriver.getOnlyIndexSettings(client(), index);
            assertThat(settings.get(IndexMetadata.INDEX_BLOCKS_WRITE_SETTING.getKey()), not("true"));
        });
        TimeSeriesRestDriver.indexDocument(client(), index);
    }
    ...
    public void testDeleteDuringSnapshot() throws Exception {
        ...
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "delete", new DeleteAction(), TimeValue.timeValueMillis(0));
    ...
    public void testReadOnly() throws Exception {
        createIndexWithSettings(index, Settings.builder().put(IndexMetadata.SETTING_NUMBER_OF_SHARDS, 1)
            .put(IndexMetadata.SETTING_NUMBER_OF_REPLICAS, 0));
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "warm", new ReadOnlyAction());
        updatePolicy(index, policy);
        assertBusy(() -> {
            Map<String, Object> settings = TimeSeriesRestDriver.getOnlyIndexSettings(client(), index);
            assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index), equalTo(PhaseCompleteStep.finalStep("warm").getKey()));
            assertThat(settings.get(IndexMetadata.INDEX_BLOCKS_WRITE_SETTING.getKey()), equalTo("true"));
        });
    }
    ...
    public void forceMergeActionWithCodec(String codec) throws Exception {
        ...
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "warm", new ForceMergeAction(1, codec));
        updatePolicy(index, policy);
    ...
    public void testShrinkSameShards() throws Exception {
        ...
        createIndexWithSettings(index, Settings.builder().put(IndexMetadata.SETTING_NUMBER_OF_SHARDS, numberOfShards)
            .put(IndexMetadata.SETTING_NUMBER_OF_REPLICAS, 0));
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "warm", new ShrinkAction(numberOfShards));
        updatePolicy(index, policy);
        assertBusy(() -> {
            assertTrue(indexExists(index));
    ...
    public void testShrinkDuringSnapshot() throws Exception {
        ...
        assertOK(client().performRequest(request));
        // create delete policy
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "warm", new ShrinkAction(1), TimeValue.timeValueMillis(0));
        // create index without policy
        createIndexWithSettings(index, Settings.builder()
        ...
        assertBusy(() -> {
            ...
            Map<String, Object> settings = TimeSeriesRestDriver.getOnlyIndexSettings(client(), shrunkenIndex);
            assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), shrunkenIndex), equalTo(PhaseCompleteStep.finalStep("warm").getKey()));
    ...
    public void testSetSingleNodeAllocationRetriesUntilItSucceeds() throws Exception {
       ...
       // unallocate all index shards
        ...
        assertTrue("ILM did not start retrying the set-single-node-allocation step", waitUntil(() -> {
            try {
                Map<String, Object> explainIndexResponse = TimeSeriesRestDriver.explainIndex(client(), index);
    ...
    public void testFreezeAction() throws Exception {
        ...
        assertBusy(() -> {
            Map<String, Object> settings = TimeSeriesRestDriver.getOnlyIndexSettings(client(), index);
            assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index), equalTo(PhaseCompleteStep.finalStep("cold").getKey()));
            assertThat(settings.get(IndexMetadata.INDEX_BLOCKS_WRITE_SETTING.getKey()), equalTo("true"));
    ...
    public void testFreezeDuringSnapshot() throws Exception {
        ...
        assertOK(client().performRequest(request));
        // create delete policy
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "cold", new FreezeAction(), TimeValue.timeValueMillis(0));
        // create index without policy
        createIndexWithSettings(index, Settings.builder().put(IndexMetadata.SETTING_NUMBER_OF_SHARDS, 1)
            .put(IndexMetadata.SETTING_NUMBER_OF_REPLICAS, 0));
        // index document so snapshot actually does something
    ...
    public void testSetPriority() throws Exception {
        ...
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "warm", new SetPriorityAction(priority));
        updatePolicy(index, policy);
        assertBusy(() -> {
            Map<String, Object> settings = TimeSeriesRestDriver.getOnlyIndexSettings(client(), index);
            assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index), equalTo(PhaseCompleteStep.finalStep("warm").getKey()));
    ...
    public void testSetNullPriority() throws Exception {
        createIndexWithSettings(index, Settings.builder().put(IndexMetadata.SETTING_NUMBER_OF_SHARDS, 1)
            .put(IndexMetadata.SETTING_NUMBER_OF_REPLICAS, 0).put(IndexMetadata.INDEX_PRIORITY_SETTING.getKey(), 100));
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "warm", new SetPriorityAction((Integer) null));
        updatePolicy(index, policy);
        assertBusy(() -> {
            Map<String, Object> settings = TimeSeriesRestDriver.getOnlyIndexSettings(client(), index);
            assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index), equalTo(PhaseCompleteStep.finalStep("warm").getKey()));
    ...
    public void testInvalidPolicyNames() {
        ...
        ex = expectThrows(ResponseException.class, () -> TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "delete", new DeleteAction()));
    ...
    public void testDeletePolicyInUse() throws IOException {
        ...
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "delete", new DeleteAction(), TimeValue.timeValueHours(12));
        String originalPolicy = policy;
        String otherPolicy = randomValueOtherThan(policy, () -> randomAlphaOfLength(5));
        policy = otherPolicy;
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "delete", new DeleteAction(), TimeValue.timeValueHours(13));
    ...
    public void testRemoveAndReaddPolicy() throws Exception {
        ...
        assertBusy(() -> assertTrue((boolean) TimeSeriesRestDriver.explainIndex(client(), originalIndex).getOrDefault("managed", false)));
    ...
    public void testMoveToInjectedStep() throws Exception {
        String shrunkenIndex = ShrinkAction.SHRUNKEN_INDEX_PREFIX + index;
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "warm", new ShrinkAction(1), TimeValue.timeValueHours(12));
        ...
        assertBusy(() -> assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index), equalTo(new StepKey("new", "complete", "complete"))));
    ...
    public void testMoveToStepRereadsPolicy() throws Exception {
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "hot", new RolloverAction(null, TimeValue.timeValueHours(1), null), TimeValue.ZERO);
        ...
        assertBusy(() -> assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), "test-1"),
            equalTo(new StepKey("hot", "rollover", "check-rollover-ready"))));

        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "hot", new RolloverAction(null, TimeValue.timeValueSeconds(1), null), TimeValue.ZERO);
    ...
    public void testExplainFilters() throws Exception {
        ...
        assertBusy(() -> {
            Map<String, Map<String, Object>> explainResponse = TimeSeriesRestDriver.explain(client(), index + "*", false, false);
            ...
            Map<String, Map<String, Object>> onlyManagedResponse = TimeSeriesRestDriver.explain(client(), index + "*", false, true);
            ...
            Map<String, Map<String, Object>> onlyErrorsResponse = TimeSeriesRestDriver.explain(client(), index + "*", true, true);
    ...
    public void testExplainIndexContainsAutomaticRetriesInformation() throws Exception {
        ...
        assertBusy(() -> {
            Map<String, Object> explainIndex = TimeSeriesRestDriver.explainIndex(client(), index);
    ...
    public void testILMRolloverRetriesOnReadOnlyBlock() throws Exception {
        ...
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "hot", new RolloverAction(null, TimeValue.timeValueSeconds(1), null));
        ...
        assertBusy(() -> assertThat((Integer) TimeSeriesRestDriver.explainIndex(client(), firstIndex).get(FAILED_STEP_RETRY_COUNT_FIELD),
        ...
        assertBusy(() -> assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), firstIndex), equalTo(PhaseCompleteStep.finalStep("hot").getKey())));
    }
    ...
    public void testILMRolloverOnManuallyRolledIndex() throws Exception {
        ...
        // Set up a policy with rollover
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "hot", new RolloverAction(null, null, 2L));
        ...
        // Manual rollover
        TimeSeriesRestDriver.rolloverMaxOneDocCondition(client(), alias);
        assertBusy(() -> assertTrue(indexExists(secondIndex)));
        ...
        assertBusy(() -> assertTrue((boolean) TimeSeriesRestDriver.explainIndex(client(), secondIndex).getOrDefault("managed", true)));
        ...
        assertBusy(() -> assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), secondIndex), equalTo(PhaseCompleteStep.finalStep("hot").getKey())));
    ...
    public void testRolloverStepRetriesUntilRolledOverIndexIsDeleted() throws Exception {
        ...
        assertBusy(() -> assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index), equalTo(PhaseCompleteStep.finalStep("hot").getKey())));
    }
    ...
    public void testUpdateRolloverLifecycleDateStepRetriesWhenRolloverInfoIsMissing() throws Exception {
        ...
        assertBusy(() -> assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index).getName(), Matchers.is(WaitForRolloverReadyStep.NAME)));
    ...
    public void testWaitForActiveShardsStep() throws Exception {
        ...
        assertBusy(() -> assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), originalIndex).getName(), equalTo(WaitForActiveShardsStep.NAME)));
    ...
    public void testHistoryIsWrittenWithSuccess() throws Exception {
        ...
        assertBusy(() -> assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index + "-1"), equalTo(PhaseCompleteStep.finalStep("hot").getKey())));
    ...
    public void testHistoryIsWrittenWithFailure() throws Exception {
        ...
        assertBusy(() -> assertThat((Integer) TimeSeriesRestDriver.explainIndex(client(), index + "-1").get("failed_step_retry_count"),
            greaterThanOrEqualTo(1)));
    ...
    public void testRetryableInitializationStep() throws Exception {
        ...
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "hot", new SetPriorityAction(1));
        ...
        assertBusy(() -> {
            Map<String, Object> explainResp = TimeSeriesRestDriver.explainIndex(client(), index);
    ...
    public void testRefreshablePhaseJson() throws Exception {
        ...
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "hot", new RolloverAction(null, null, 100L));
        Request createIndexTemplate = new Request("PUT", "_template/rolling_indexes");
        createIndexTemplate.setJsonEntity("{" +
            "\"index_patterns\": [\""+ index + "-*\"], \n" +
            "  \"settings\": {\n" +
        ...
        assertBusy(() -> assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index + "-1").getName(), equalTo(WaitForRolloverReadyStep.NAME)));
        ...
        assertBusy(() -> assertThat(TimeSeriesRestDriver.getStepKeyForIndex(client(), index + "-1").getName(), equalTo(PhaseCompleteStep.NAME)));
    }

    public void testHaltAtEndOfPhase() throws Exception {
        String index = "halt-index";

        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "hot", new SetPriorityAction(100));
    ...
    public void testSearchableSnapshotAction() throws Exception {
        String snapshotRepo = randomAlphaOfLengthBetween(4, 10);
        TimeSeriesRestDriver.createSnapshotRepo(client(), snapshotRepo, randomBoolean());
        TimeSeriesRestDriver.createNewSingletonPolicy(client(), policy, "cold", new SearchableSnapshotAction(snapshotRepo));
    ...
    private void assertHistoryIsPresent(String policyName, String indexName, boolean success,
        ...
        Step.StepKey stepKey = TimeSeriesRestDriver.getStepKeyForIndex(client(), "ilm-history-2-000001");
    ...
    private String getFailedStepForIndex(String indexName) throws IOException {
        Map<String, Object> indexResponse = TimeSeriesRestDriver.explainIndex(client(), indexName);
    ...
    private void assertBusy(CheckedRunnable<Exception> runnable, String slmPolicy) throws Exception {
        assertBusy(() -> {
            try {
                ...
                throw new AssertionError("Index:" + TimeSeriesRestDriver.explainIndex(client(), index) + "\nSLM:" + slm, e);
            }
        });
    }
}

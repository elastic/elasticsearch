...
public class ExecuteStepsUpdateTask extends IndexLifecycleRunner.IndexLifecycleClusterStateUpdateTask {
    ...
    public ClusterState execute(final ClusterState currentState) throws IOException {
        ...
        Step registeredCurrentStep = IndexLifecycleRunner.getCurrentStep(currentState, policyStepsRegistry, policy, indexMetadata);
        if (currentStep.equals(registeredCurrentStep)) {
            ...
            while (currentStep instanceof ClusterStateActionStep || currentStep instanceof ClusterStateWaitStep) {
                ...
                currentStep = policyStepsRegistry.getStep(currentState, indexMetadata, currentStep.getNextStepKey());
            }
            return state;
        } else {

...
public class IndexLifecycleService
    ...
    void onMaster(ClusterState clusterState) {
        ...
        final IndexLifecycleMetadata currentMetadata = clusterState.metadata().projectCustom(IndexLifecycleMetadata.TYPE);
    ...
    public void applyClusterState(ClusterChangedEvent event) {
        if (event.localNodeMaster()) { // only act if we are master, otherwise
            ...
            final IndexLifecycleMetadata ilmMetadata = event.state().metadata().projectCustom(IndexLifecycleMetadata.TYPE);
            ...
            if (ilmMetadata != null
                && (event.previousState().nodes().isLocalNodeElectedMaster() == false
                    || ilmMetadata != event.previousState().metadata().projectCustom(IndexLifecycleMetadata.TYPE))) {
    ...
    void triggerPolicies(ClusterState clusterState, boolean fromClusterStateChange) {
        IndexLifecycleMetadata currentMetadata = clusterState.metadata().projectCustom(IndexLifecycleMetadata.TYPE);

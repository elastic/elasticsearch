        ...
        try (XContentBuilder builder = XContentFactory.jsonBuilder()) {
            ...
            threadPool.executor(ThreadPool.Names.GENERIC).execute(() -> {
                try {
                    ...
                    logger.error(
                        () -> format(ROOT, "failed add ILM history item to queue for index [%s]: [%s]", ILM_HISTORY_DATA_STREAM, item),
                        e
                    );
                }
            });
            ...
            logger.error(
                () -> format(ROOT, "failed to queue ILM history item in index [%s]: [%s]", ILM_HISTORY_DATA_STREAM, item),
                exception
            );
        }
    }

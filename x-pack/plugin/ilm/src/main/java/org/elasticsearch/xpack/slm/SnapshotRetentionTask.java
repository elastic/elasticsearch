...
public class SnapshotRetentionTask implements SchedulerEngine.Listener {
    ...
    static String getPolicyId(SnapshotInfo snapshotInfo) {
        return Optional.ofNullable(snapshotInfo.userMetadata())
            .filter(meta -> meta.get(SnapshotsService.POLICY_ID_METADATA_FIELD) != null)
            .filter(meta -> meta.get(SnapshotsService.POLICY_ID_METADATA_FIELD) instanceof String)
            .map(meta -> (String) meta.get(SnapshotsService.POLICY_ID_METADATA_FIELD))

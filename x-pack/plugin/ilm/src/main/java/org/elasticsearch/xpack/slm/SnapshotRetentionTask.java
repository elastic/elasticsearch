            ...
            try {
                ...
                deleteSnapshot(policyId, repo, snapshotId, slmStats, ActionListener.runAfter(ActionListener.wrap(acknowledgedResponse -> {
                    ...
                    try {
                        ...
                        logger.error(
                            () -> format(ROOT, "failed to record snapshot deletion failure for snapshot lifecycle policy [%s]", policyId),
                            ex
                        );
                    } finally {
    ...
    void deleteSnapshot(
        ...
        client.admin()
            ...
            .execute(ActionListener.wrap(acknowledgedResponse -> {
                ...
                try {
                    logger.warn(() -> format(ROOT, "[%s] failed to delete snapshot [%s] for retention", repo, snapshot), e);

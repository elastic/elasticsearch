...
public class TransportPutInferenceModelAction extends TransportMasterNodeAction<
    ...
    private void checkInferenceIndexMappingVersion(ClusterState state, ActionListener<Boolean> listener) {
        var descriptor = systemIndices.findMatchingDescriptor(InferenceIndex.INDEX_NAME);

        final IndexMetadata indexMetadata = state.metadata().index(InferenceIndex.INDEX_NAME);
        if (indexMetadata == null) { // index does not exist
            if (state.getMinTransportVersion().onOrAfter(TransportVersions.ML_INFERENCE_CHUNKING_SETTINGS)) {
        ...
        if (SystemIndexMappingUpdateService.checkIndexMappingUpToDate(descriptor, indexMetadata) == false) {

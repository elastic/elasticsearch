    ...
    public EmbeddingRequestChunker(
        ...
        for (int inputIndex = 0; inputIndex < inputs.size(); inputIndex++) {
            ...
            for (int chunkIndex = 0; chunkIndex < chunks.size(); chunkIndex++) {
                ...
                int lastStart = resultOffsetStarts.size() - 1;
                int lastEnd = resultOffsetEnds.size() - 1;
                if (resultOffsetStarts.get(lastStart).size() <= targetChunkIndex) {
                    resultOffsetStarts.get(lastStart).add(chunks.get(chunkIndex).start());
                    resultOffsetEnds.get(lastEnd).add(chunks.get(chunkIndex).end());
                } else {
                    resultOffsetEnds.get(lastEnd).set(targetChunkIndex, chunks.get(chunkIndex).end());
                }

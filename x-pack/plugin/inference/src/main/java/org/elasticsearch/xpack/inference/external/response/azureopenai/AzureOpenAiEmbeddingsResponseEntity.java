...
import static org.elasticsearch.xpack.inference.external.response.XContentUtils.*;

public class AzureOpenAiEmbeddingsResponseEntity {
    private static final String FAILED_TO_FIND_FIELD_TEMPLATE = "Failed to find required field [%s] in Azure OpenAI embeddings response";
    ...
    public static TextEmbeddingResults fromResponse(Request request, HttpResult response) throws IOException {
        ...
        try (XContentParser jsonParser = XContentFactory.xContent(XContentType.JSON).createParser(parserConfig, response.body())) {
            ...
            List<TextEmbeddingResults.Embedding> embeddingList = XContentParserUtils.parseList(
                jsonParser,
                AzureOpenAiEmbeddingsResponseEntity::parseEmbeddingObject
            );
    ...
    private static TextEmbeddingResults.Embedding parseEmbeddingObject(XContentParser parser) throws IOException {
        ...
        List<Float> embeddingValues = XContentParserUtils.parseList(parser, AzureOpenAiEmbeddingsResponseEntity::parseEmbeddingList);
    ...
    private AzureOpenAiEmbeddingsResponseEntity() {}
}

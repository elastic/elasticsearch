...
public class AzureAiStudioRerankResponseEntity extends BaseResponseEntity {
    /**
     * Parses the AzureAI json response.
    ...
    protected InferenceServiceResults fromResponse(Request request, HttpResult response) throws IOException {
        ...
        try (XContentParser jsonParser = XContentFactory.xContent(XContentType.JSON).createParser(parserConfig, response.body())) {
            ...
            AzureAiStudioRerankRequest azureAiStudioRerankRequest = (AzureAiStudioRerankRequest) request;

            var rankedDocStreamTopN = azureAiStudioRerankRequest.getTopN() == null
                ? rankedDocsByRelevanceStream
                : rankedDocsByRelevanceStream.limit(azureAiStudioRerankRequest.getTopN());
    ...
    private static List<RankedDocsResults.RankedDoc> doParse(XContentParser parser) throws IOException {
        return parseList(parser, (listParser, index) -> {
            var parsedRankedDoc = AzureAiStudioRerankResponseEntity.RankedDocEntry.parse(parser);
    ...
    private record RankedDocEntry(Integer index, Float score, @Nullable String text) {
        ...
        private static final ConstructingObjectParser<AzureAiStudioRerankResponseEntity.RankedDocEntry, Void> PARSER =
            new ConstructingObjectParser<>(
                "azure_ai_studio_rerank_response",
                true,
                args -> new AzureAiStudioRerankResponseEntity.RankedDocEntry((int) args[0], (float) args[1], (String) args[2])
            );
        ...
        public static AzureAiStudioRerankResponseEntity.RankedDocEntry parse(XContentParser parser) {

...
import org.elasticsearch.xpack.inference.services.groq.completion.GroqCompletionResponseHandler;
...
/**
 * Creates executable actions for Groq inference requests, handling completions.
 * This class implements the {@link GroqActionVisitor} interface to provide specific action creation methods.
 */
public class GroqActionCreator implements GroqActionVisitor {

    private static final String FAILED_TO_SEND_REQUEST_ERROR_MESSAGE = "Failed to send Groq %s request from inference entity id [%s]";
    private static final String COMPLETION_ERROR_PREFIX = "Groq completions";
    private static final String USER_ROLE = "user";

    private static final ResponseHandler COMPLETION_HANDLER = new GroqCompletionResponseHandler(
        "Groq completion",
    ...
    /**
     * Constructs a new {@link GroqActionCreator} with the specified sender and service components.
     *
    ...
    @Override
    public ExecutableAction create(GroqChatCompletionModel model) {
        var manager = new GenericRequestManager<>(
            serviceComponents.threadPool(),
            model,
            COMPLETION_HANDLER,
            inputs -> new GroqChatCompletionRequest(new UnifiedChatInput(inputs, USER_ROLE), model),

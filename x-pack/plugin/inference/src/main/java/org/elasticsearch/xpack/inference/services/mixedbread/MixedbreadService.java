...
public class MixedbreadService extends SenderService {
     ...
     * For Mixedbread use a conservatively small max batch size as it is
     * unknown how the model is deployed
     */
    static final int EMBEDDING_MAX_BATCH_SIZE = 20;
    private static final EnumSet<TaskType> SUPPORTED_TASK_TYPES = EnumSet.of(TEXT_EMBEDDING);

    /**
     * Constructor for creating a MixedbreadService with specified HTTP request sender factory and service components.
     *
     ...
     */
    public MixedbreadService(HttpRequestSender.Factory factory, ServiceComponents serviceComponents) {
    ...
    protected void doInfer(
        ...
        if (model instanceof MixedbreadEmbeddingsModel mixedbreadEmbeddingsModel) {
            mixedbreadEmbeddingsModel.accept(actionCreator).execute(inputs, timeout, listener);
        } else if (model instanceof MixedbreadRerankModel mixedbreadRerankModel) {
            mixedbreadRerankModel.accept(actionCreator, taskSettings).execute(inputs, timeout, listener);
        } else {
    ...
    /**
     * Creates a MixedbreadModel based on the provided parameters.
     *
     ...
     * @return a new instance of MixedbreadModel based on the provided parameters
     */
    ...
    public Model updateModelWithEmbeddingDetails(Model model, int embeddingSize) {
        if (model instanceof MixedbreadEmbeddingsModel embeddingsModel) {
            ...
            return new MixedbreadEmbeddingsModel(embeddingsModel, updatedServiceSettings);
        } else {
    ...
    protected void doChunkedInfer(
        ...
        var MixedbreadModel = (MixedbreadEmbeddingsModel) model;
        var actionCreator = new MixedbreadActionCreator(getSender(), getServiceComponents());

        List<EmbeddingRequestChunker.BatchRequestAndListener> batchedRequests = new EmbeddingRequestChunker<>(
            inputs.getInputs(),
            EMBEDDING_MAX_BATCH_SIZE,
            MixedbreadModel.getConfigurations().getChunkingSettings()
        ).batchRequestsWithListeners(listener);

        for (var request : batchedRequests) {
            var action = MixedbreadModel.accept(actionCreator);
    ...
    public Set<TaskType> supportedStreamingTasks() {
        return null; // EnumSet.of(COMPLETION, CHAT_COMPLETION);
    }
    ...
    private MixedbreadModel createModelFromPersistent(
    ...
    /**
     * Configuration class for the Mixedbread inference service.
            ...
            () -> {
                ...
                configurationMap.put(
                    ...
                    new SettingsConfiguration.Builder(SUPPORTED_TASK_TYPES).setDescription(
                        "Refer to the Mixedbread models documentation for the list of available models."
                    )
                        .setLabel("Model")
                        .setRequired(true)
                        .setSensitive(false)

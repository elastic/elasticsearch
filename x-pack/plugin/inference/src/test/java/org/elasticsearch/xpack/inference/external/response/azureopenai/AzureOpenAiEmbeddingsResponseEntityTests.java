...
public class AzureOpenAiEmbeddingsResponseEntityTests extends ESTestCase {
    public void testFromResponse_CreatesResultsForASingleItem() throws IOException {
        ...
        TextEmbeddingResults parsedResults = AzureOpenAiEmbeddingsResponseEntity.fromResponse(
            mock(Request.class),
    ...
    public void testFromResponse_FailsWhenDataFieldIsNotPresent() {
        ...
        var thrownException = expectThrows(
            ...
            () -> AzureOpenAiEmbeddingsResponseEntity.fromResponse(
                mock(Request.class),
                new HttpResult(mock(HttpResponse.class), responseJson.getBytes(StandardCharsets.UTF_8))
            )
        );

        assertThat(thrownException.getMessage(), is("Failed to find required field [data] in Azure OpenAI embeddings response"));
    }
    ...
    public void testFromResponse_FailsWhenEmbeddingsDoesNotExist() {
        ...
        assertThat(thrownException.getMessage(), is("Failed to find required field [embedding] in Azure OpenAI embeddings response"));
    }

...
public class LegacySemanticTextFieldTests extends AbstractXContentTestCase<LegacySemanticTextField> {
    private static final String NAME = "field";

    @Override
    protected Predicate<String> getRandomFieldsExcludeFilter() {
        return n -> n.endsWith(LegacySemanticTextField.CHUNKED_EMBEDDINGS_FIELD);
    }

    @Override
    protected void assertEqualInstances(LegacySemanticTextField expectedInstance, LegacySemanticTextField newInstance) {
        ...
        LegacySemanticTextField.ModelSettings modelSettings = newInstance.inference().modelSettings();
    ...
    @Override
    protected LegacySemanticTextField createTestInstance() {
        ...
        try { // try catch required for override
            ...
            fail("Failed to create random LegacySemanticTextField instance");
        }
        return null;
    }

    @Override
    protected LegacySemanticTextField doParseInstance(XContentParser parser) throws IOException {
        return LegacySemanticTextField.parse(parser, new Tuple<>(NAME, parser.contentType()));
    }
    ...
    public void testModelSettingsValidation() {
        NullPointerException npe = expectThrows(NullPointerException.class, () -> {
            new LegacySemanticTextField.ModelSettings(null, 10, SimilarityMeasure.COSINE, DenseVectorFieldMapper.ElementType.FLOAT);
        });
        assertThat(npe.getMessage(), equalTo("task type must not be null"));

        IllegalArgumentException ex = expectThrows(IllegalArgumentException.class, () -> {
            new LegacySemanticTextField.ModelSettings(
                TaskType.COMPLETION,
                10,
        ...
        ex = expectThrows(IllegalArgumentException.class, () -> {
            new LegacySemanticTextField.ModelSettings(TaskType.SPARSE_EMBEDDING, 10, null, null);
        });
        assertThat(ex.getMessage(), containsString("[dimensions] is not allowed"));

        ex = expectThrows(IllegalArgumentException.class, () -> {
            new LegacySemanticTextField.ModelSettings(TaskType.SPARSE_EMBEDDING, null, SimilarityMeasure.COSINE, null);
        });
        assertThat(ex.getMessage(), containsString("[similarity] is not allowed"));

        ex = expectThrows(IllegalArgumentException.class, () -> {
            new LegacySemanticTextField.ModelSettings(TaskType.SPARSE_EMBEDDING, null, null, DenseVectorFieldMapper.ElementType.FLOAT);
        });
        ...
        ex = expectThrows(IllegalArgumentException.class, () -> {
            new LegacySemanticTextField.ModelSettings(TaskType.TEXT_EMBEDDING, 10, null, DenseVectorFieldMapper.ElementType.FLOAT);
        });
        assertThat(ex.getMessage(), containsString("required [similarity] field is missing"));

        ex = expectThrows(IllegalArgumentException.class, () -> {
            new LegacySemanticTextField.ModelSettings(TaskType.TEXT_EMBEDDING, 10, SimilarityMeasure.COSINE, null);
        });
    ...
    public static LegacySemanticTextField randomSemanticText(String fieldName, Model model, List<String> inputs, XContentType contentType)
        throws IOException {
    ...
    public static LegacySemanticTextField semanticTextFieldFromChunkedInferenceResults(
        String fieldName,
        Model model,
        List<String> inputs,
        ...
        return new LegacySemanticTextField(
            ...
            new LegacySemanticTextField.InferenceResult(
                ...
                LegacySemanticTextField.toSemanticTextFieldChunks(List.of(results), contentType)
            ),
            contentType
        );
    }
    ...
    public static ChunkedInferenceServiceResults toChunkedResult(LegacySemanticTextField field) throws IOException {
        switch (field.inference().modelSettings().taskType()) {
            case SPARSE_EMBEDDING -> {
                ...
                for (var chunk : field.inference().chunks()) {
                    ...
                    // TODO
                    chunks.add(new MlChunkedTextExpansionResults.ChunkedResult(null, tokens));
                }
                return new InferenceChunkedSparseEmbeddingResults(chunks);
            }
            case TEXT_EMBEDDING -> {

...
public class Ai21ActionCreatorTests extends ESTestCase {
    ...
    public void testExecute_ReturnsSuccessfulResponse_ForChatCompletionAction() throws IOException {
        ...
        try (var sender = createSender(senderFactory)) {
            ...
            assertThat(result.asMap(), is(buildExpectationCompletion(List.of("I'm doing great, thanks for asking!"))));
    ...
    public void testSend_FailsFromInvalidResponseFormat_ForChatCompletionAction() throws IOException {
        ...
        try (var sender = createSender(senderFactory)) {
            ...
            assertThat(
                ...
                is("Failed to send AI21 completion request from inference entity id " + "[id]. Cause: Required [choices]")
            );
    ...
    private PlainActionFuture<InferenceServiceResults> createChatCompletionFuture(Sender sender, ServiceComponents threadPool) {
        var model = Ai21ChatCompletionModelTests.createCompletionModel("secret", "model");
        model.setURI(getUrl(webServer));
        var actionCreator = new Ai21ActionCreator(sender, threadPool);

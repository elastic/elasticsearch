...
public class ElasticInferenceServiceTests extends ESTestCase {
    ...
    //TODO: test parse request config -> throws if extra key exists in task settings map (should be empty for now; the task settings, not the test :))

    //TODO: test parse request config -> throws if extra key exists in secret settings map (should be empty for now; the secret settings, not the test :))

    //TODO: test parse persisted config with secrets (we don't have secrets for now (empty secrets), but will have them later, so useful to already test)  -> creates sparse embeddings model
    ...
    //TODO: test parse persisted config with secrets -> does not throw if extra key exists in task settings map (should be empty for now; the task settings, not the test :))

    //TODO: test parse persisted config with secrets -> does not throw if extra key exists in secret settings map (should be empty for now; the secret settings, not the test :))

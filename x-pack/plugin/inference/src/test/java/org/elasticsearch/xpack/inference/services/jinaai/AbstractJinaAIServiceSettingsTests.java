...
/**
 * Base test case for the JinaAI service-settings classes. Holds the assertions for the fields common to every JinaAI task (model
 * identity and rate limiting) so they are exercised once for each task type instead of being duplicated in every concrete settings
 * test. Task-specific tests live in the concrete subclasses.
 */
public abstract class AbstractJinaAIServiceSettingsTests<T extends ServiceSettings> extends AbstractBWCWireSerializationTestCase<T> {

    protected static final String TEST_MODEL_ID = "test-model";
    protected static final String INITIAL_TEST_MODEL_ID = "initial-model";

    protected static final int TEST_RATE_LIMIT = 500;
    protected static final int INITIAL_TEST_RATE_LIMIT = 100;
    protected static final int DEFAULT_RATE_LIMIT = 2000;
    ...
    /**
     * The task-specific immutable fields an update request must reject, in addition to the common {@code model_id} field. Subclasses
     * override this when they declare additional immutable fields.
     */
    ...
    public void testFromMap_OnlyMandatoryFields_UsesDefaultValues_Success() {
        ...
        assertThat(serviceSettings, is(createServiceSettings(TEST_MODEL_ID, new RateLimitSettings(DEFAULT_RATE_LIMIT))));
    }

    public void testFromMap_EmptyRateLimitObject_UsesDefaultValue() {
        var map = buildCommonServiceSettingsMap(TEST_MODEL_ID, null);
    ...
    public void testFromMap_NoModelId_ThrowsException() {
        var thrownException = expectThrows(
            ...
            () -> fromMap(buildCommonServiceSettingsMap(null, TEST_RATE_LIMIT), randomFrom(ConfigurationParseContext.values()))
        );
    ...
    public void testUpdateServiceSettings_RateLimit_IsUpdated() {
        ...
        assertThat(updatedServiceSettings, is(createServiceSettings(INITIAL_TEST_MODEL_ID, new RateLimitSettings(TEST_RATE_LIMIT))));
    }
    ...
    public void testUpdateServiceSettings_EmptyRateLimitObject_RevertsToDefault() {
        ...
        assertThat(updatedServiceSettings, is(createServiceSettings(INITIAL_TEST_MODEL_ID, new RateLimitSettings(DEFAULT_RATE_LIMIT))));
    }
    ...
    public void testUpdateServiceSettings_GivenImmutableFields_ThrowsException() {
        var serviceSettings = createServiceSettings(INITIAL_TEST_MODEL_ID, new RateLimitSettings(INITIAL_TEST_RATE_LIMIT));

        var immutableFields = new ArrayList<>(List.of(ServiceFields.MODEL_ID));

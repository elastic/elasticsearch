...
import static org.elasticsearch.xpack.inference.services.validation.SimpleDocumentExtractionServiceIntegrationValidator.TEST_PDF_BASE64_INPUT;
...
public class SimpleDocumentExtractionServiceIntegrationValidatorTests extends ESTestCase {

    private static final TimeValue TIMEOUT = TimeValue.ONE_MINUTE;

    private static final DocumentExtractionRequest EXPECTED_REQUEST = new DocumentExtractionRequest(
        List.of(TEST_PDF_BASE64_INPUT),
        Map.of()
    );
    ...
    private SimpleDocumentExtractionServiceIntegrationValidator underTest;

    @Before
    public void setup() {
        openMocks(this);

        underTest = new SimpleDocumentExtractionServiceIntegrationValidator();
    ...
    public void testValidate_ServiceThrowsException() {
        doThrow(ElasticsearchStatusException.class).when(mockInferenceService)
            .documentExtractionInfer(eq(mockModel), eq(EXPECTED_REQUEST), eq(TIMEOUT), any());
    ...
    public void testValidate_CallsListenerOnFailure_WhenServiceThrowsException() {
        ...
        doAnswer(ans -> {
            ...
            responseListener.onFailure(returnedException);
            return null;
        }).when(mockInferenceService).documentExtractionInfer(eq(mockModel), eq(EXPECTED_REQUEST), eq(TIMEOUT), any());
    ...
    private void verifyCallToService() {
        verify(mockInferenceService).documentExtractionInfer(eq(mockModel), eq(EXPECTED_REQUEST), eq(TIMEOUT), any());

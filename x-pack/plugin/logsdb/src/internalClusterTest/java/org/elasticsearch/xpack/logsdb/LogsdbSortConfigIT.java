...
public class LogsdbSortConfigIT extends ESSingleNodeTestCase {
    ...
    private List<String> indexDocuments(String dataStreamName, List<DocWithId> docs) {
        ...
        return Arrays.stream(bulkResponse.getItems()).map(item -> item.getId()).toList();
    }
    ...
    private void assertSettings(Index backingIndex, Consumer<Settings> settingsTest) {
        final GetSettingsResponse getSettingsResponse = indicesAdmin().getSettings(
            new GetSettingsRequest(TEST_REQUEST_TIMEOUT).indices(backingIndex.getName()).includeDefaults(false)
        ).actionGet();
        final Settings settings = getSettingsResponse.getIndexToSettings().get(backingIndex.getName());
        assertNotNull(settings);
        settingsTest.accept(settings);
    }

...
public final class PatternTextCompositeValues extends BinaryDocValues {
    ...
    static PatternTextCompositeValues from(LeafReader leafReader, PatternTextFieldType fieldType) throws IOException {
        ...
        if (rawDocValues != null) {
            var counts = leafReader.getNumericDocValues(
                fieldType.storedNamed() + MultiValuedBinaryDocValuesField.SeparateCount.COUNT_FIELD_SUFFIX
            );
            rawBinaryDocValues = MultiValuedSortedBinaryDocValues.from(rawDocValues, counts);
        }

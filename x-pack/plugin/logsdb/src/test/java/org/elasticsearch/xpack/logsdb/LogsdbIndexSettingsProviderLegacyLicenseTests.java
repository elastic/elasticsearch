...
public class LogsdbIndexSettingsProviderLegacyLicenseTests extends ESTestCase {
    ...
    public void testGetAdditionalIndexSettingsDefault() {
        ...
        if (PatternedTextFieldMapper.PATTERNED_TEXT_MAPPER.isEnabled()) {
            expectedBuilder.put(PatternedTextFieldMapper.PATTERNED_TEXT_FALLBACK_SETTING.getKey(), true);
        }
    ...
    public void testGetAdditionalIndexSettingsApm() throws IOException {
        ...
        provider.provideAdditionalMetadata(indexName, dataStreamName, null, null, null, settings, List.of(), builder, (k, v) -> {});
        var result = builder.build();
        Settings expectedAdditionalSettings = PatternedTextFieldMapper.PATTERNED_TEXT_MAPPER.isEnabled()
            ? Settings.builder().put(PatternedTextFieldMapper.PATTERNED_TEXT_FALLBACK_SETTING.getKey(), true).build()
            : Settings.EMPTY;

...
public class PatternTextDocValuesTests extends ESTestCase {
    ...
    private static List<Message> makeRandomMessages(int numDocs, boolean includeStoredAndRaw) {
        List<Message> messages = new ArrayList<>();
        for (int i = 0; i < numDocs; i++) {
            // if arg is present, it's at the beginning
            Storage storage = includeStoredAndRaw ? randomFrom(Storage.values()) : randomFrom(Storage.DOC_VALUE, Storage.EMPTY);

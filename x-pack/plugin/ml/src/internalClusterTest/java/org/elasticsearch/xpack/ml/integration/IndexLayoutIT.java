...
public class IndexLayoutIT extends BaseMlIntegTestCase {
    ...
    public void testCrudOnTwoJobsInSharedIndex() throws Exception {
        ...
        assertBusy(() -> {
            GetJobsStatsAction.Response statsResponse =
                client().execute(GetJobsStatsAction.INSTANCE, new GetJobsStatsAction.Request(jobId2)).actionGet();
            assertEquals(statsResponse.getResponse().results().get(0).getState(), JobState.OPENED);
        });
        ...
        assertThat(client
            ...
            .indices()
            .prepareGetIndex()
            .addIndices(AnomalyDetectorsIndex.jobStateIndexPattern()).get().indices(), arrayContaining(".ml-state-000001"));
        assertThat(client
            .admin()
            .indices()
            ...
            .get(".ml-state-000001")
            .get(0)
            .alias(), equalTo(".ml-state-write"));
        assertThat(client
            .admin()
            .indices()
            .prepareGetIndex()
            .addIndices(AnomalyDetectorsIndex.jobResultsAliasedName(jobId)).get().indices().length, equalTo(1));
        assertThat(client
            .admin()
            .indices()
            .prepareGetIndex()
            .addIndices(AnomalyDetectorsIndex.jobResultsAliasedName(jobId2)).get().indices().length, equalTo(1));
    }
    ...
    public void testForceCloseDoesNotCreateState() throws Exception {
        ...
        client().execute(PutJobAction.INSTANCE,  new PutJobAction.Request(createJob(jobId, ByteSizeValue.ofMb(2))
            .setDataDescription(new DataDescription.Builder()))).get();
        client().execute(OpenJobAction.INSTANCE, new OpenJobAction.Request(jobId)).get();
        assertBusy(() -> {
            GetJobsStatsAction.Response statsResponse =
                client().execute(GetJobsStatsAction.INSTANCE, new GetJobsStatsAction.Request(jobId)).actionGet();
        ...
        client().execute(PutDatafeedAction.INSTANCE,
            new PutDatafeedAction.Request(createDatafeed(jobId + "-datafeed", jobId, Collections.singletonList("data")))).get();

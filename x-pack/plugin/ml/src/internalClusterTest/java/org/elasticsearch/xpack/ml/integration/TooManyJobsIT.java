...
public class TooManyJobsIT extends BaseMlIntegTestCase {
    ...
    public void testCloseFailedJob() throws Exception {
        ...
        List<PersistentTasksMetadataSection.PersistentTask<?>> tasks = findTasks(state, MlTasks.JOB_TASK_NAME);
        assertEquals(1, tasks.size());
        // now just double check that the first job is still opened:
        PersistentTasksMetadataSection.PersistentTask<?> task = tasks.get(0);
            ...
            try {
                ...
                assertBusy(() -> {
                    for (Client client : clients()) {
                        PersistentTasksMetadataSection tasks = client.admin()
                            .cluster()
                            .prepareState()
                            .get()
                            .getState()

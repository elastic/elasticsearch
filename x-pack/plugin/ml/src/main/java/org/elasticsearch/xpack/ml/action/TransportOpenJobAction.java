...
public class TransportOpenJobAction extends TransportMasterNodeAction<OpenJobAction.Request, NodeAcknowledgedResponse> {
    ...
    protected void masterOperation(
        ...
        if (MachineLearningField.ML_API_FEATURE.check(licenseState)) {
            ...
            ActionListener<PersistentTasksExtensionMetadata.PersistentTask<OpenJobAction.JobParams>> waitForJobToStart =
                new ActionListener<>() {
                    @Override
                    public void onResponse(PersistentTasksExtensionMetadata.PersistentTask<OpenJobAction.JobParams> task) {
    ...
    private void waitForJobStarted(String taskId, OpenJobAction.JobParams jobParams, ActionListener<NodeAcknowledgedResponse> listener) {
        ...
        persistentTasksService.waitForPersistentTaskCondition(
            ...
            new PersistentTasksService.WaitForPersistentTaskListener<OpenJobAction.JobParams>() {
                @Override
                public void onResponse(PersistentTasksExtensionMetadata.PersistentTask<OpenJobAction.JobParams> persistentTask) {
     ...
     */
    private static class JobPredicate implements Predicate<PersistentTasksExtensionMetadata.PersistentTask<?>> {

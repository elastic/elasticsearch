...
public class TransportStartDatafeedAction extends TransportMasterNodeAction<StartDatafeedAction.Request, NodeAcknowledgedResponse> {
    ...
    private static class DatafeedPredicate implements Predicate<PersistentTasksCustomMetadata.PersistentTask<?>> {
        ...
        public boolean test(PersistentTasksCustomMetadata.PersistentTask<?> persistentTask) {
            ...
            if (assignment != null) {
                // This means we are awaiting the datafeed's job to be assigned or relocated
                if (assignment.getReason() == PersistentTasksCustomMetadata.Assignment.Reason.AWAITING_LAZY_ASSIGNMENT) {
                    return true;
                }
                if (assignment.getReason() != PersistentTasksCustomMetadata.Assignment.Reason.TASK_CREATED

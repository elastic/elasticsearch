...
public class TransportStopTrainedModelDeploymentAction extends TransportTasksAction<
    ...
    protected void doExecute(
        ...
        logger.debug(() -> format(ROOT, "[%s] Received request to undeploy%s", request.getId(), request.isForce() ? " (force)" : ""));
        ...
        ActionListener<StopTrainedModelDeploymentAction.Response> finalListener = ActionListener.wrap(r -> {
            ...
            trainedModelAssignmentClusterService.removeModelAssignment(
                ...
                ActionListener.wrap(deleted -> listener.onResponse(r), deletionFailed -> {
                    logger.error(
                        () -> format(ROOT, "[%s] failed to delete model assignment after nodes unallocated the deployment", modelId),
                        deletionFailed
                    );
                    listener.onFailure(

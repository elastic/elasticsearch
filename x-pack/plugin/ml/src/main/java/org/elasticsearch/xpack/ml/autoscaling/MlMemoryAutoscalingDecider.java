...
class MlMemoryAutoscalingDecider {
    ...
    private long maxMemoryBytes(MlAutoscalingContext mlContext) {
        long maxMemoryBytes = Math.max(
            mlContext.anomalyDetectionTasks.stream()
                .filter(PersistentTasksMetadataSection.PersistentTask::isAssigned)
     ...
     * - If > 1 "batch" ml tasks are running on the same node, we sum their resources.
     */
    Optional<NativeMemoryCapacity> calculateFutureAvailableCapacity(PersistentTasksMetadataSection tasks, List<NodeLoad> nodeLoads) {
        final List<PersistentTasksMetadataSection.PersistentTask<StartDatafeedAction.DatafeedParams>> jobsWithLookbackDatafeeds =
            datafeedTasks(tasks).stream().filter(t -> t.getParams().getEndTime() != null && t.getExecutorNode() != null).toList();
        final List<PersistentTasksMetadataSection.PersistentTask<?>> assignedAnalyticsJobs = MlAutoscalingContext.dataframeAnalyticsTasks(
            tasks
        ...
        for (PersistentTasksMetadataSection.PersistentTask<StartDatafeedAction.DatafeedParams> lookbackOnlyDf : jobsWithLookbackDatafeeds) {
        ...
        for (PersistentTasksMetadataSection.PersistentTask<?> task : assignedAnalyticsJobs) {
    ...
    private static Collection<PersistentTasksMetadataSection.PersistentTask<StartDatafeedAction.DatafeedParams>> datafeedTasks(
        ...
        return tasksCustomMetadata.findTasks(MlTasks.DATAFEED_TASK_NAME, Predicates.always())
            .stream()
            .map(p -> (PersistentTasksMetadataSection.PersistentTask<StartDatafeedAction.DatafeedParams>) p)
            .toList();
    }
    ...
    private Long getAnalyticsMemoryRequirement(PersistentTasksMetadataSection.PersistentTask<?> task) {
    ...
    private Long getAnomalyMemoryRequirement(PersistentTasksMetadataSection.PersistentTask<?> task) {

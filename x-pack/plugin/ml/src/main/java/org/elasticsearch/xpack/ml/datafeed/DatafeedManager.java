    ...
    public void updateDatafeed(
        ...
        Runnable doUpdate = () ->
            ...
            ElasticsearchMappings.addDocMappingIfMissing(
                ...
                ActionListener.wrap(
                    bool -> useSecondaryAuthIfAvailable(securityContext, () -> {
        ...
        // If the user changed the jobId (deprecated), we need to delete the existing timing stats
                ...
                ActionListener.wrap(
                    datafeedConfigBuilder -> {
                        ...
                        if (jobId.equals(request.getUpdate().getJobId())) {
                            ...
                            jobDataDeleter.deleteDatafeedTimingStats(ActionListener.wrap(b -> doUpdate.run(), listener::onFailure));
                        }
                    },
                    listener::onFailure));

        // We need to validate this, even though updating the job_id for a datafeed is deprecated
        if (request.getUpdate().getJobId() != null) {
            checkJobDoesNotHaveADifferentDatafeed(
                request.getUpdate().getJobId(),
                datafeedId,
                ActionListener.wrap(deleteTimingStatsAndUpdateConsumer, listener::onFailure)
            );
        } else {
            doUpdate.run();
        }

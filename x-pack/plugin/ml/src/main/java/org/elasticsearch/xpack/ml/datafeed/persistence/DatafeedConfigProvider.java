...
public class DatafeedConfigProvider {
    ...
    /**
     * The same logic as {@link #expandDatafeedIds(String, boolean, PersistentTasksMetadataSection, boolean, TaskId, ActionListener)} but
     * the full datafeed configuration is returned.
     *
     * See {@link #expandDatafeedIds(String, boolean, PersistentTasksMetadataSection, boolean, TaskId, ActionListener)}
     *
     * @param expression the expression to resolve
     * @param allowNoMatch if {@code false}, an error is thrown when no name matches the {@code expression}.
     *                     This only applies to wild card expressions, if {@code expression} is not a
    ...
    static Collection<String> matchingDatafeedIdsWithTasks(String[] datafeedIdPatterns, PersistentTasksMetadataSection tasksMetadata) {

...
public class DataFrameAnalyticsManager {
    ...
    public void execute(DataFrameAnalyticsTask task, ClusterState clusterState, TimeValue masterNodeTimeout) {
        ...
        ActionListener<DataFrameAnalyticsConfig> configListener = ActionListener.wrap(config -> {
            ...
            IndexMetadata destIndex = clusterState.getMetadata().projectMetadata.index(config.getDest().getIndex());
            if (destIndex != null) {
                MappingMetadata destIndexMapping = clusterState.getMetadata().projectMetadata.index(config.getDest().getIndex()).mapping();

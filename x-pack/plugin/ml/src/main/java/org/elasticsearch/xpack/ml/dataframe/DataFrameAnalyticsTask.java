...
public class DataFrameAnalyticsTask extends LicensedAllocatedPersistentTask implements StartDataFrameAnalyticsAction.TaskMatcher {
    ...
    public void stop(String reason, TimeValue timeout) {
        ...
        LOGGER.debug(() -> Message.createParameterizedMessage("[{}] Stopping task due to reason [{}]", getParams().getId(), reason));
        ...
        ActionListener<Void> stepProgressListener = ActionListener.wrap(aVoid -> cachedCurrentStep.cancel(reason, timeout), e -> {
            LOGGER.error(
                    Message.createParameterizedMessage("[{}] Error updating progress for step [{}]", taskParams.getId(), cachedCurrentStep.name()),
                e
            );
    ...
    public void setFailed(Exception error) {
        if (analyticsManager.isNodeShuttingDown()) {
            LOGGER.warn(
                    Message.createParameterizedMessage("[{}] *Not* setting task to failed because the node is being shutdown", taskParams.getId()),
                error
            );
            return;
        }
        persistProgress(client, taskParams.getId(), () -> {
            LOGGER.error(Message.createParameterizedMessage("[{}] Setting task to failed", taskParams.getId()), error);
    ...
    void persistProgress(Client clientToUse, String jobId, Runnable runnable) {
        ...
        ActionListener<IndexResponse> indexProgressDocListener = ActionListener.wrap(indexResponse -> {
            ...
            LOGGER.error(Message.createParameterizedMessage("[{}] cannot persist progress as an error occurred while indexing", jobId), indexError);
            runnable.run();
        });
        ...
        ActionListener<SearchResponse> searchFormerProgressDocListener = ActionListener.wrap(searchResponse -> {
            ...
            if (searchResponse.getHits().getHits().length > 0) {
                ...
                try {
                    ...
                    LOGGER.warn(Message.createParameterizedMessage("[{}] failed to parse previously stored progress", jobId), ex);
                }
            }
            ...
            try (XContentBuilder jsonBuilder = JsonXContent.contentBuilder()) {
                LOGGER.debug(() -> Message.createParameterizedMessage("[{}] Persisting progress is: {}", jobId, progress));

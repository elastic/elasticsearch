...
public class TrainedModelAssignmentNodeService implements ClusterStateListener {
        ...
        updateStoredState(
            ...
            ActionListener.wrap(success -> stopDeploymentAsync(task, "task locally canceled", notifyDeploymentOfStopped), e -> {
                if (ExceptionsHelper.unwrapCause(e) instanceof ResourceNotFoundException) {
                    logger.debug(
                        () -> format(ROOT, "[%s] failed to set routing state to stopping as assignment already removed", task.getModelId()),
                        e
                    );
                } else {
    ...
    void prepareModelToLoad(StartTrainedModelDeploymentAction.TaskParams taskParams) {
        logger.debug(() -> format(ROOT, "[%s] preparing to load model with task params: %s", taskParams.getModelId(), taskParams));
    ...
    private void updateStoredState(
        ...
        trainedModelAssignmentService.updateModelAssignmentState(
            ...
            ActionListener.wrap(success -> {
                logger.debug(() -> format(ROOT, "[%s] model is [%s] and master notified", modelId, routingStateAndReason.getState()));
                listener.onResponse(AcknowledgedResponse.TRUE);
            }, error -> {
                logger.warn(
                    () -> format(ROOT, "[%s] model is [%s] but failed to notify master", modelId, routingStateAndReason.getState()),
                    error
                );

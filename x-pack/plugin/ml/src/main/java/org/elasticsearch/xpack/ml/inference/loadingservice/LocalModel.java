...
public class LocalModel implements Model {
    ...
    public void infer(Map<String, Object> fields, InferenceConfig config, ActionListener<InferenceResults> listener) {
        try {
            if (fieldNames.stream().noneMatch(
                f -> {
                    Object fieldValue = MapHelper.dig(f.getFieldName(), fields);
                    if (fieldValue == null) {
                        return false;
                    }
                    ...
                    if (f.getModelFieldType() == null) {
                        return true;
                    }
                    return f.getModelFieldType().supportsJavaObjectType(fieldValue);
                })
            ) {

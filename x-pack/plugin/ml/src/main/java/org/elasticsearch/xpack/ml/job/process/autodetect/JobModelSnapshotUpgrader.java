...
public final class JobModelSnapshotUpgrader {
        ...
        process = autodetectProcessFactory.createAutodetectProcess(
            ...
            (reason) -> {
                ...
                try {
                    ...
                    logger.error(() -> format(ROOT, "[%s] [%s] failed to delete temporary files snapshot upgrade", jobId, snapshotId), e);
                }
            }
        );
    ...
    void setTaskToFailed(String reason, ActionListener<PersistentTask<?>> listener) {
        ...
        task.updatePersistentTaskState(taskState, ActionListener.wrap(listener::onResponse, f -> {
            logger.warn(() -> format(ROOT, "[%s] [%s] failed to set task to failed", task.getJobId(), task.getSnapshotId()), f);
    ...
    public synchronized void killProcess(String reason) {
        if (process != null) {
            try {
                ...
                logger.error(() -> format(ROOT, "[%s] failed to kill upgrade process for model snapshot [%s]", jobId, snapshotId), e);
            }
        } else {
    ...
    private class Executor {
        ...
        FlushAcknowledgement waitFlushToCompletion(String flushId) throws Exception {
            logger.debug(() -> format(ROOT, "[%s] [%s] waiting for flush [%s]", jobId, snapshotId, flushId));
            ...
            logger.debug(() -> format(ROOT, "[%s] [%s] flush completed [%s]", jobId, snapshotId, flushId));
        ...
        void restoreState() {
            try {
                ...
                logger.error(() -> format(ROOT, "[%s] [%s] failed to write old state", jobId, snapshotId), e);
                setTaskToFailed(
            ...
            submitOperation(() -> {
                ...
                Runnable nextStep;
                if (e != null) {
                    logger.error(() -> format(ROOT, "[%s] [%s] failed to flush after writing old state", jobId, snapshotId), e);
        ...
        private void requestStateWrite() {
            task.updatePersistentTaskState(
                ...
                ActionListener.wrap(readingNewState -> {
                    ...
                    logger.error(() -> format(ROOT, "[%s] [%s] failed to update snapshot upgrader task to started", jobId, snapshotId), f);
                    shutdown(
        ...
        void shutdown(Exception e) {
            ...
            Future<?> future = autodetectWorkerExecutor.submit(() -> {
                try {
                    ...
                    logger.warn(() -> format(ROOT, "[%s] [%s] failed to shutdown process", jobId, snapshotId), exc);
                } finally {
                    onFinish.accept(e);
                }

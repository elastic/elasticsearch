    ...
    public void process() {
        ...
        try {
            ...
            try {
                ...
                LOGGER.warn(() -> format(ROOT, "[%s] [%s] Error persisting model snapshot upgrade results", jobId, snapshotId), e);
            }
        } catch (Exception e) {
            failed = true;
            ...
            if (processKilled) {
                ...
                LOGGER.error(() -> format(ROOT, "[%s] [%s] error parsing model snapshot upgrade output", jobId, snapshotId), e);
            }
        } finally {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                try {
                    ...
                    LOGGER.warn(() -> format(ROOT, "[%s] [%s] Error processing model snapshot upgrade result", jobId, snapshotId), e);
                }
            }
        } finally {

...
public class AnalyticsResultProcessorTests extends ESTestCase {
    ...
    public void testProcess_GivenInferenceModelFailedToStore() {
        ...
        givenProcessResults(Arrays.asList(new AnalyticsResult(null, null, null, null, null, null, modelSizeInfo, null)));
        AnalyticsResultProcessor resultProcessor = createResultProcessor();
        ...
        assertThat(auditCaptor.getValue(),
            containsString("Error processing results; error storing trained model metadata with id [" + JOB_ID));
        Mockito.verifyNoMoreInteractions(auditor);

        assertThat(resultProcessor.getFailure(),
            startsWith("error processing results; error storing trained model metadata with id [" + JOB_ID));
        assertThat(statsHolder.getProgressTracker().getWritingResultsProgressPercent(), equalTo(0));
    }

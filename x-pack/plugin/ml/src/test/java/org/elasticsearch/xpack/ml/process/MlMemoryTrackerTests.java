...
public class MlMemoryTrackerTests extends ESTestCase {
    ...
    public void testRefreshAll() {
        ...
        Map<String, PersistentTasksMetadataSection.PersistentTask<?>> tasks = new HashMap<>();

        int numAnomalyDetectorJobTasks = randomIntBetween(2, 5);
        for (int i = 1; i <= numAnomalyDetectorJobTasks; ++i) {
            String jobId = "job" + i;
            PersistentTasksMetadataSection.PersistentTask<?> task = makeTestAnomalyDetectorTask(jobId);
        ...
        for (int i = numAnomalyDetectorJobTasks; i < numAnomalyDetectorJobTasks + numSnapshotUpgradeTasks; ++i) {
            ...
            PersistentTasksMetadataSection.PersistentTask<?> task = makeTestSnapshotUpgradeTask(jobId, snapshotId);
            tasks.put(task.getId(), task);
        }
        ...
        for (int i = 1; i <= numDataFrameAnalyticsTasks; ++i) {
            ...
            allIds.add(id);
            PersistentTasksMetadataSection.PersistentTask<?> task = makeTestDataFrameAnalyticsTask(id, false);
            tasks.put(task.getId(), task);
        }

        PersistentTasksMetadataSection persistentTasks = new PersistentTasksMetadataSection(
    ...
    private PersistentTasksMetadataSection.PersistentTask<OpenJobAction.JobParams> makeTestAnomalyDetectorTask(String jobId) {
        return new PersistentTasksMetadataSection.PersistentTask<>(
            MlTasks.jobTaskId(jobId),
            MlTasks.JOB_TASK_NAME,
            new OpenJobAction.JobParams(jobId),
            0,
            PersistentTasksMetadataSection.INITIAL_ASSIGNMENT
        );
    }

    private PersistentTasksMetadataSection.PersistentTask<SnapshotUpgradeTaskParams> makeTestSnapshotUpgradeTask(
        String jobId,
        String snapshotId
    ) {
        return new PersistentTasksMetadataSection.PersistentTask<>(
    ...
    private PersistentTasksMetadataSection.PersistentTask<StartDataFrameAnalyticsAction.TaskParams> makeTestDataFrameAnalyticsTask(
        String id,

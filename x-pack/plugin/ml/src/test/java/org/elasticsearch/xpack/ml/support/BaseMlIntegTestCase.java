...
public abstract class BaseMlIntegTestCase extends ESIntegTestCase {
    ...
    protected void assertRecentLastTaskStateChangeTime(String taskId, Duration howRecent, String queryNode) {
        ...
        PersistentTasksExtensionMetadata tasks = csResponse.getState().getMetadata().custom(PersistentTasksExtensionMetadata.TYPE);
        assertNotNull(tasks);
        PersistentTasksExtensionMetadata.PersistentTask<?> task = tasks.getTask(taskId);
        assertNotNull(task);

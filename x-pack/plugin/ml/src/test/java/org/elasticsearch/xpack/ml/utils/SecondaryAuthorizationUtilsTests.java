...
public class SecondaryAuthorizationUtilsTests extends ESTestCase {
    ...
    public void testGetSecurityHeadersPreferringSecondary_emptyHeaders() {
    ...
    public void testGetSecurityHeadersPreferringSecondary_withSecondaryAuth() {
        ThreadContext threadContext = new ThreadContext(Settings.EMPTY);
        ThreadPool threadPool = mock(ThreadPool.class);
        when(threadPool.getThreadContext()).thenReturn(threadContext);
        SecurityContext securityContext = new SecurityContext(Settings.EMPTY, threadContext);
        ...
        // When secondary auth is present, it should be preferred and only auth key should be returned
        assertThat(filteredHeaders.keySet(), contains(AUTH_KEY));
    }

    public void testGetSecurityHeadersPreferringSecondary_withOnlyPrimaryAuth() {
        ThreadContext threadContext = new ThreadContext(Settings.EMPTY);
        ThreadPool threadPool = mock(ThreadPool.class);
        when(threadPool.getThreadContext()).thenReturn(threadContext);
        SecurityContext securityContext = new SecurityContext(Settings.EMPTY, threadContext);

        threadContext.setHeaders(Tuple.tuple(Map.of(AUTH_KEY, JOHN_HEADER, NOT_AN_AUTH_KEY, NOT_AN_AUTH_HEADER), Map.of()));
        Map<String, String> filteredHeaders = SecondaryAuthorizationUtils.getSecurityHeadersPreferringSecondary(
            threadPool,
            securityContext,
            ClusterState.EMPTY_STATE
        );
        // When no secondary auth, primary auth should be used
        assertThat(filteredHeaders.keySet(), contains(AUTH_KEY));
        assertThat(filteredHeaders.get(AUTH_KEY), is(equalTo(JOHN_HEADER)));
    }

    public void testUseSecondaryAuthIfAvailable_nullSecurityContext() {
    ...
    public void testUseSecondaryAuthIfAvailable_noSecondaryAuth() {
        AtomicInteger counter = new AtomicInteger(0);
        ThreadContext threadContext = new ThreadContext(Settings.EMPTY);
        SecurityContext securityContext = new SecurityContext(Settings.EMPTY, threadContext);
    ...
    public void testUseSecondaryAuthIfAvailable_withSecondaryAuth() {
        AtomicInteger counter = new AtomicInteger(0);
        ThreadContext threadContext = new ThreadContext(Settings.EMPTY);
        SecurityContext securityContext = new SecurityContext(Settings.EMPTY, threadContext);
        ...
        SecondaryAuthorizationUtils.useSecondaryAuthIfAvailable(securityContext, () -> {
            ...
            // Inside the runnable, the secondary auth should be in effect
        ...
        assertThat(counter.get(), is(equalTo(1)));
        // After the runnable, headers should be restored
        assertThat(

...
public class HttpExporterResourceTests extends AbstractPublishableHttpResourceTestCase {
    ...
    public void testWatchCheckBlocksAfterSuccessfulWatcherCheck() {
        ...
        if (firstSucceeds) {
            ...
            if (validLicense) {
                ...
                // last check fails implies that N - 2 publishes succeeded!
                whenPerformRequestAsyncWith(client, new RequestMatcher(is("GET"), startsWith("/watcher/watch/")),
                                            first, otherResponses, exception);
            ...
            // deleting watches
                ...
                whenPerformRequestAsyncWith(client, new RequestMatcher(is("DELETE"), startsWith("/watcher/watch/")),
            ...
            whenPerformRequestAsyncWith(client, new RequestMatcher(is(method), startsWith("/watcher/watch/")), exception);
        }
    ...
    public void testWatchPublishBlocksAfterSuccessfulWatcherCheck() {
        ...
        if (firstSucceeds) {
            ...
            // previous publishes must have succeeded
            whenPerformRequestAsyncWith(client, new RequestMatcher(is("PUT"), startsWith("/watcher/watch/")),
                                        firstSuccess, otherResponses, exception);
            ...
            whenPerformRequestAsyncWith(client, new RequestMatcher(is("PUT"), startsWith("/watcher/watch/")), exception);
        }
    ...
    private Response successfulGetWatchResponse(final String watchId) {
        ...
        return response("GET", "/watcher/watch/" + watchId, successfulCheckStatus(), goodEntity);
    }
    private Response unsuccessfulGetWatchResponse(final String watchId) {
        if (randomBoolean()) {
            ...
            return response("GET", "/watcher/watch/" + watchId, successfulCheckStatus(), badEntity);
        }
    ...
    private void whenGetWatches(final int successful, final int unsuccessful) {
        ...
        whenPerformRequestAsyncWith(client, new RequestMatcher(is("GET"), startsWith("/watcher/watch/")), gets);
    }
    ...
    private void whenSuccessfulPutWatches(final int successful) {
        ...
        // empty is possible if they all exist
        whenPerformRequestAsyncWith(client, new RequestMatcher(is("PUT"), startsWith("/watcher/watch/")), successfulPuts);
    }
    ...
    private void whenSuccessfulDeleteWatches(final int successful) {
        ...
        // empty is possible if they all exist
        whenPerformRequestAsyncWith(client, new RequestMatcher(is("DELETE"), startsWith("/watcher/watch/")), successfulDeletes);
    }
    ...
    private void verifyDeleteWatches(final int called) {
        verify(client, times(called))
            .performRequestAsync(argThat(new RequestMatcher(is("DELETE"), startsWith("/watcher/watch/"))),
                                 any(ResponseListener.class));
    }

    private void verifyGetWatches(final int called) {
        verify(client, times(called))
            .performRequestAsync(argThat(new RequestMatcher(is("GET"), startsWith("/watcher/watch/"))), any(ResponseListener.class));
    }

    private void verifyPutWatches(final int called) {
        verify(client, times(called))
            .performRequestAsync(argThat(new RequestMatcher(is("PUT"), startsWith("/watcher/watch/"))), any(ResponseListener.class));
    }

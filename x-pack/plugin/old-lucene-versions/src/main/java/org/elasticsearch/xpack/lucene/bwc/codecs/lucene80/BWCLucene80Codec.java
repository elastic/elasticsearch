...
/**
 * Archive wrapper for Lucene 8.0 segments (Elasticsearch 7.0.0–7.5.2).
 *
 * <p>Used only when an Elasticsearch 6.x archive index was later written in 7.x, so some segments
 * still name {@code Lucene80Codec}. {@link BWCCodec} remaps that class to this wrapper. Indices
 * created in 7.x are opened read-only by the server {@code Lucene80} codec, not by this class.
 */

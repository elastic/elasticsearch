...
public class PrometheusInstantQueryRestIT extends AbstractPrometheusRestIT {
    ...
    /**
     * Verifies that an instant selector query returns resultType "vector".
     */
    public void testInstantQueryResultTypeVector() throws Exception {
        ingestTestData("test_vector_type");

        Request request = new Request("GET", "/_prometheus/api/v1/query");
        request.addParameter("query", "test_vector_type{job=\"test_job\"}");
        request.addParameter("time", "2026-01-01T00:05:00Z");
        addReadAuth(request);
    ...
    /**
     * Verifies that a scalar literal query returns resultType "scalar".
     */
    public void testInstantQueryResultTypeScalar() throws Exception {
        Request request = new Request("GET", "/_prometheus/api/v1/query");
    ...
    /**
     * Verifies that a scalar arithmetic expression returns resultType "scalar".
     */
    public void testInstantQueryResultTypeScalarArithmetic() throws Exception {
        Request request = new Request("GET", "/_prometheus/api/v1/query");
        request.addParameter("query", "1 + 2 * 3");
        request.addParameter("time", "2026-01-01T00:05:00Z");
        addReadAuth(request);
    ...
    /**
     * Verifies that an aggregation function query returns resultType "vector".
     * Aggregation functions like sum() return instant vectors.
     */
    public void testInstantQueryResultTypeAggregation() throws Exception {
        ingestTestData("test_agg_type");

        Request request = new Request("GET", "/_prometheus/api/v1/query");
        request.addParameter("query", "sum(test_agg_type)");
        request.addParameter("time", "2026-01-01T00:05:00Z");
        addReadAuth(request);

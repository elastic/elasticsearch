...
public class PrometheusQueryRangeRestIT extends AbstractPrometheusRestIT {
    ...
    /**
     * Verifies that an instant selector in range query returns resultType "matrix".
     * In range queries, instant vectors become matrices (one value per step).
     */
    public void testQueryRangeResultTypeMatrix() throws Exception {
        ingestTestData("test_matrix_type");

        Request request = new Request("GET", "/_prometheus/api/v1/query_range");
        request.addParameter("query", "test_matrix_type{job=\"test_job\"}");
        request.addParameter("start", "2026-01-01T00:00:00Z");
        request.addParameter("end", "2026-01-01T00:05:00Z");
        request.addParameter("step", "60s");
        addReadAuth(request);
    ...
    /**
     * Verifies that a scalar literal in range query returns resultType "scalar".
     */
    public void testQueryRangeResultTypeScalar() throws Exception {
        Request request = new Request("GET", "/_prometheus/api/v1/query_range");
    ...
    /**
     * Verifies that a scalar arithmetic expression in range query returns resultType "scalar".
     */
    public void testQueryRangeResultTypeScalarArithmetic() throws Exception {
        Request request = new Request("GET", "/_prometheus/api/v1/query_range");
        request.addParameter("query", "10 + 5 * 2");
        request.addParameter("start", "2026-01-01T00:00:00Z");
        request.addParameter("end", "2026-01-01T00:05:00Z");
        request.addParameter("step", "60s");
        addReadAuth(request);
    ...
    /**
     * Verifies that an aggregation function in range query returns resultType "matrix".
     * Aggregation functions return instant vectors, which become matrices in range queries.
     */
    public void testQueryRangeResultTypeAggregation() throws Exception {
        ingestTestData("test_agg_range");

        Request request = new Request("GET", "/_prometheus/api/v1/query_range");
        request.addParameter("query", "sum(test_agg_range)");
        request.addParameter("start", "2026-01-01T00:00:00Z");
        request.addParameter("end", "2026-01-01T00:05:00Z");
        request.addParameter("step", "60s");
        addReadAuth(request);

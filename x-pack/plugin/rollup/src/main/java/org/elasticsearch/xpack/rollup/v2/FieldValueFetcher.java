...
class FieldValueFetcher {
    ...
    LeafFieldData.Leaf getLeaf(LeafReaderContext context) {
        final LeafFieldData.Leaf delegate = fieldData.load(context).getLeafValueFetcher(DocValueFormat.RAW);
        return new LeafFieldData.Leaf() {
            @Override

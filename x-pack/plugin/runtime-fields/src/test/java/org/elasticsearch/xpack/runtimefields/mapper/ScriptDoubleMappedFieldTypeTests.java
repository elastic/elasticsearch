...
public class ScriptDoubleMappedFieldTypeTests extends AbstractNonTextScriptMappedFieldTypeTestCase {
    ...
    private static ScriptDoubleMappedFieldType build(Script script) throws IOException {
        ScriptPlugin scriptPlugin = new ScriptPlugin() {
            ...
            public ScriptEngine getScriptEngine(Settings settings, Collection<ScriptContext<?>> contexts) {
                return new ScriptEngine() {
                    ...
                    private DoubleScript.Factory factory(String code) {
                        switch (code) {
                            case "read_foo":
                                return (fieldName, params, lookup) -> (ctx) -> new DoubleScript(fieldName, params, lookup, ctx) {
                                    @Override
                                    public void execute() {
        ...
        try (ScriptService scriptService = new ScriptService(Settings.EMPTY, scriptModule.engines, scriptModule.contexts)) {
            DoubleScript.Factory factory = scriptService.compile(script, DoubleScript.CONTEXT);

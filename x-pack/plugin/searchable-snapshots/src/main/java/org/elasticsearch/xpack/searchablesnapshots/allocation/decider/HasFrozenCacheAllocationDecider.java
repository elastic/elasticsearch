...
public class HasFrozenCacheAllocationDecider extends AllocationDecider {
    ...
    private static final Decision STILL_FETCHING_WHILE_SIMULATION = Decision.single(
        Decision.Type.NO,
        NAME,
        "value of [" + SHARED_CACHE_SIZE_SETTING.getKey() + "] on this node is not known yet while running shard allocation simulation"
    );
    ...
    public Decision canAllocate(ShardRouting shardRouting, RoutingNode node, RoutingAllocation allocation) {
        return canAllocateToNode(allocation.metadata().getIndexSafe(shardRouting.index()), node.node(), allocation);
    }
    ...
    private Decision canAllocateToNode(IndexMetadata indexMetadata, DiscoveryNode node, RoutingAllocation allocation) {
        if (indexMetadata.isPartialSearchableSnapshot() == false) {
            return Decision.ALWAYS;
        }
        return switch (frozenCacheService.getNodeState(node)) {
            case HAS_CACHE -> HAS_FROZEN_CACHE;
            case NO_CACHE -> NO_FROZEN_CACHE;
            case FAILED -> UNKNOWN_FROZEN_CACHE;
            default -> allocation.isSimulating() ? STILL_FETCHING_WHILE_SIMULATION : STILL_FETCHING;
        };
    }

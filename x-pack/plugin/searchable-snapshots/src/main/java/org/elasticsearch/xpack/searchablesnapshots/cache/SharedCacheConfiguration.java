...
public final class SharedCacheConfiguration {
    ...
    public static final int TINY_REGION_SIZE = Math.toIntExact(ByteSizeValue.ofKb(4).getBytes());
    public static final int SMALL_REGION_SIZE = Math.toIntExact(ByteSizeValue.ofKb(64).getBytes());
    ...
    public SharedCacheConfiguration(Settings settings) {
        ...
        if (cacheSize > this.largeRegionSize) {
            ...
            if (largeRegionSize % TINY_REGION_SIZE != 0) {
                throw new IllegalArgumentException("Large region size must be a multiple of 4kb");
            }
    ...
    public long totalSize() {
        return (long) TINY_REGION_SIZE * numTinyRegions + (long) SMALL_REGION_SIZE * numSmallRegions + largeRegionSize * numLargeRegions;
    }

...
public class AutoConfigInitialNode extends EnvironmentAwareCommand {
    ...
    public static final String AUTO_CONFIG_ALT_CN = "Elasticsearch security auto-configuration HTTP CA";
    ...
    private boolean isInitialNodeAutoConfigured(Settings settings) {
        return XPackSettings.SECURITY_ENABLED.get(settings) &&
                XPackSettings.ENROLLMENT_ENABLED.get(settings) &&
                isInitialClusterNode(settings) &&
                ...
                usesTransportTLSWithAutoGeneratedCerts(settings);
    }
    ...
    protected void execute(Terminal terminal, OptionSet options, Environment env) throws Exception {
        ...
        // the node looks like it is already configured in an automatic fashion, but the node did not start
        // at the point when this was started
        // return exit code 0 here to signal that credential generation should be attempted
        if (isInitialNodeAutoConfigured(env.settings())) {
            return;
        }
        ...
        assert isInitialNodeAutoConfigured(env.settings());
    }
    ...
    private GeneralNames getSubjectAltNames() throws IOException {
        ...
        generalNameSet.add(CertGenUtils.createCommonName(AUTO_CONFIG_ALT_CN));
    ...
    private boolean usesTransportTLSWithAutoGeneratedCerts(Settings settings) {
        return settings.get("xpack.security.transport.ssl.enabled").equals("true") &&
                ...
                Files.isReadable(Paths.get(settings.get("xpack.security.transport.ssl.keystore.path"))) &&
                settings.get("xpack.security.transport.ssl.truststore.path")
                        .equals(settings.get("xpack.security.transport.ssl.keystore.path"));
    }

    ...
    public void testRemoteAccessForCrossClusterSearch() throws Exception {
        ...
        // Query cluster
        {
            ...
            assertThat(
                ...
                containsString(
                    "action [indices:data/read/search] is unauthorized for remote user [remote_search_user] "
                        + "with assigned roles [remote_search] authenticated by API key id ["
                        + remoteAccessApiKeyId
                        + "] of user [test_user] for remote access on indices [index2]"
                )
            );
            ...
            assertThat(
                ...
                containsString(
                    "action [indices:data/read/search] is unauthorized for remote user [remote_search_user] "
                        + "with assigned roles [remote_search] authenticated by API key id ["
                        + remoteAccessApiKeyId
                        + "] of user [test_user] for remote access on indices [prefixed_index]"
                )
            );

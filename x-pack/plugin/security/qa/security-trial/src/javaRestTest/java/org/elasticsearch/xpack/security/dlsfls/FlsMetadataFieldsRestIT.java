...
public class FlsMetadataFieldsRestIT extends SecurityOnTrialLicenseRestTestCase {
    ...
    public void testSearchFunctionalityRelyingOnMetadataFieldsWorksUnderFls() throws IOException {
        ...
        request.setOptions(RequestOptions.DEFAULT.toBuilder().addHeader("Authorization", authzHeader(FLS_FILE_ROLE_USER)));
        final Map<String, Object> response = responseAsMap(client().performRequest(request));
        final Map<String, Object> hit = firstHit(response);
        assertThat(hit.get("_seq_no"), equalTo(0));
        assertThat(hit.get("_primary_term"), equalTo(1));
        assertThat(hit.get("_version"), equalTo(1));

        // terms aggregations on the _index metadata field work with FLS enabled
        final Request aggRequest = new Request("GET", INDEX_NAME + "/_search");
        aggRequest.setJsonEntity("""
            {
              "size": 0,
              "aggs": { "indices": { "terms": { "field": "_index" } } }
            }""");
        aggRequest.setOptions(RequestOptions.DEFAULT.toBuilder().addHeader("Authorization", authzHeader(FLS_FILE_ROLE_USER)));
        final Map<String, Object> aggResponse = responseAsMap(client().performRequest(aggRequest));
        assertThat(ObjectPath.evaluate(aggResponse, "aggregations.indices.buckets.0.key"), equalTo(INDEX_NAME));
        assertThat(ObjectPath.evaluate(aggResponse, "aggregations.indices.buckets.0.doc_count"), equalTo(1));
    }
    ...
    public void testUnderscoredUserFieldsAreNotAccessibleUnderFls() throws IOException {
        ...
        request.setJsonEntity("""
            {
              "query": { "match_all": {} },
              "fields": [ "_field3", "hidden_field", "field1" ]
            }""");
        request.setOptions(RequestOptions.DEFAULT.toBuilder().addHeader("Authorization", authzHeader(FLS_FILE_ROLE_USER)));
        final Map<String, Object> response = responseAsMap(client().performRequest(request));
    ...
    private static String authzHeader(String user) {
    ...
    private void assertHitCount(String user, String queryBody, int expectedHits) throws IOException {
        ...
        final Map<String, Object> response = responseAsMap(client().performRequest(request));
        assertThat(ObjectPath.evaluate(response, "hits.total.value"), equalTo(expectedHits));
    }

    @SuppressWarnings("unchecked")
    private static Map<String, Object> firstHit(Map<String, Object> searchResponse) throws IOException {
        final List<Map<String, Object>> hits = (List<Map<String, Object>>) ObjectPath.evaluate(searchResponse, "hits.hits");

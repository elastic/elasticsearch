...
public class SecurityClearScrollTests extends SecurityIntegTestCase {
    ...
    public void indexRandomDocuments() {
        ...
        for (int i = 0; i < randomIntBetween(10, 50); i++) {
            bulkRequestBuilder.add(
                client().prepareIndex("index", "type", String.valueOf(i)).setSource("{ \"foo\" : \"bar\" }", XContentType.JSON)
            );
        }
    ...
    public void testThatClearingAllScrollIdsWorks() throws Exception {
        ...
        ClearScrollResponse clearScrollResponse = internalCluster().transportClient()
    ...
    public void testThatClearingAllScrollIdsRequirePermissions() throws Exception {
        ...
        assertRequestBuilderThrows(
            internalCluster().transportClient().filterWithHeader(headers).prepareClearScroll().addScrollId("_all"),

...
public class ProfileDomainIntegTests extends AbstractProfileIntegTestCase {
    ...
    public void testConcurrentCreationOfNewProfiles() throws InterruptedException {
        ...
        if (existingCollision) {
            ...
            final UpdateRequest updateRequest = client().prepareUpdate(SECURITY_PROFILE_ALIAS, "profile_" + existingUid).setDoc(String.format(java.util.Locale.ROOT, """
                {
                  "user_profile": {
                    "user": { "username": "%s" }
                  }
                }
                """, "not-" + username), XContentType.JSON).setRefreshPolicy(WriteRequest.RefreshPolicy.IMMEDIATE).request();
    ...
    public void testDifferentiator() {
        ...
        for (int i = 0; i < differentiatorLimit; i++) {
            ...
            final UpdateRequest updateRequest = client().prepareUpdate(SECURITY_PROFILE_ALIAS, "profile_" + currentUid).setDoc(String.format(java.util.Locale.ROOT, """
                {
                  "user_profile": {
                    "user": { "username": "%s" }
                  }

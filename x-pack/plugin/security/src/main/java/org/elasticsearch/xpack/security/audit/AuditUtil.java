...
public class AuditUtil {
     ...
     * <p>
     * If {@code maxBytes > 0} and the rendered JSON length exceeds
     * that limit, an {@link ElasticsearchStatusException} with status 413 is thrown so the caller
     * can reject the request before it is written to the audit log. If the request has no XContent type 

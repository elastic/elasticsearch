 ...
 *
 * <p>This replaces the previously used {@link org.apache.logging.log4j.message.StringMapMessage}. That class keeps its fields in a
 * sorted-by-key structure, so every field added during construction paid for a binary search to find its insertion point, and the
 * backing array was grown/copied as fields were added. Audit entries are assembled on the transport thread, where that per-field
 * overhead matters, yet the sorted ordering bought us nothing: an entry is written exactly once and never queried by key.
 *
 * <p>Instead, the set of audit fields is fixed and known ahead of time (it mirrors the {@code appender.audit_rolling.layout.pattern}
 * in {@code log4j2.properties}). We capture that as a static {@link #AUDIT_FORMAT}: an ordered array of {@link LogField}s plus a
 * name&rarr;slot index. Each {@link #with(String, Object)} is then a single map lookup and an array store — O(1), no sorting and no
 * resizing — and field values are held <em>as references</em> rather than being eagerly rendered to JSON.
 *
 * <p>All JSON rendering is deferred to {@link #formatTo(StringBuilder)}, which the {@code %m} pattern converter calls on the appender
 * thread (and, once the audit logger moves to an async appender, off the transport thread entirely). The rendered string is memoized
 * because the entry is immutable once handed to the logger, so repeated formatting (e.g. multiple appenders) does no extra work.
 */

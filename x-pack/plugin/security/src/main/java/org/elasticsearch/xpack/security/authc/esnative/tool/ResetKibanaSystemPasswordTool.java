...
public class ResetKibanaSystemPasswordTool extends BaseRunAsSuperuserCommand {
    ...
    public ResetKibanaSystemPasswordTool() {
        this(CommandLineHttpClient::new, environment -> KeyStoreWrapper.load(environment.configFile()));
    }

    public static void main(String[] args) throws Exception {
        exit(new ResetKibanaSystemPasswordTool().main(args, Terminal.DEFAULT));
    }

    protected ResetKibanaSystemPasswordTool(
        Function<Environment, CommandLineHttpClient> clientFunction,
        CheckedFunction<Environment, KeyStoreWrapper, Exception> keyStoreFunction) {
        super(clientFunction, keyStoreFunction, "Resets the password of the kibana_system built-in user");
    ...
    protected void executeCommand(Terminal terminal, OptionSet options, Environment env, String username, SecureString password)
        throws Exception {
        final SecureString kibanaSystemPassword;
        if (options.has(interactive)) {
            if (options.has(batch) == false) {
                terminal.println("This tool will reset the password of the [kibana_system] user.");
            ...
            kibanaSystemPassword = promptForPassword(terminal);
        } else {
            if (options.has(batch) == false) {
            ...
            kibanaSystemPassword = new SecureString(generatePassword(20));
        }
        try {
            final CommandLineHttpClient client = clientFunction.apply(env);
            final URL changePasswordUrl = createURL(new URL(client.getDefaultURL()), "_security/user/kibana_system/_password", "?pretty");
            final HttpResponse httpResponse = client.execute(
                "POST",
                changePasswordUrl,
                username,
                password,
                () -> requestBodySupplier(kibanaSystemPassword),
            ...
            if (httpResponse.getHttpStatus() != HttpURLConnection.HTTP_OK) {
                throw new UserException(ExitCodes.TEMP_FAILURE,
                    "Failed to reset password for the kibana_system user. Unexpected http status [" + responseStatus + "]");
            } else {
                if (options.has(interactive)) {
                    terminal.println("Password for the kibana_system user successfully reset.");
                } else {
                    terminal.println("Password for the kibana_system user successfully reset.");
                    terminal.print(Terminal.Verbosity.NORMAL,"New value: ");
                    terminal.println(Terminal.Verbosity.SILENT, kibanaSystemPassword.toString());
                }
            }
        } catch (Exception e) {
            throw new UserException(ExitCodes.TEMP_FAILURE, "Failed to reset password for the kibana_system user", e);
        } finally {
            kibanaSystemPassword.close();
        }
    }

    private SecureString promptForPassword(Terminal terminal) {
        while (true) {
            SecureString password1 = new SecureString(terminal.readSecret("Enter password for [kibana_system]: "));
            ...
            try (SecureString password2 = new SecureString(terminal.readSecret("Re-enter password for [kibana_system]: "))) {

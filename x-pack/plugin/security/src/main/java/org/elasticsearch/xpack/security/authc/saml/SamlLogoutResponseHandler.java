...
public class SamlLogoutResponseHandler extends SamlRequestHandler {
    ...
    public SamlLogoutResponseResponse buildResponse(byte[] payload, Collection<String> allowedSamlRequestIds) {
        ...
        if (LOGOUT_RESPONSE_TAG_NAME.equals(root.getLocalName()) && SAML_NAMESPACE.equals(root.getNamespaceURI())) {
            ...
            if (StatusCode.SUCCESS.equals(status.getStatusCode().getValue()) == false) {
            ...
            return new SamlLogoutResponseResponse();
        } else {
    ...
    private void checkInResponseTo(StatusResponseType response, Collection<String> allowedSamlRequestIds) {
    ...
    private void checkResponseDestination(StatusResponseType response, String spConfiguredUrl) {

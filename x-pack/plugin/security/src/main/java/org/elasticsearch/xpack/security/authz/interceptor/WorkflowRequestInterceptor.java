    ...
    public void intercept(
        ...
        if (role != null && role.hasWorkflowsPermission()) {
            ...
            if (restEndpoint != null) {
                ...
                logger.info(
                    "workflows: access to endpoint ["
                        + restEndpoint
                        + "] is ["
                        + (isWorkflowEndpointAllowed ? "allowed" : "not allowed")
                        + "]"
                );
                ...
                logger.info(
                    "workflows: access not allowed because request not originating from rest handler and user has restriction to workflows "
                        + role.workflows().workflows()
                        + ""
                );

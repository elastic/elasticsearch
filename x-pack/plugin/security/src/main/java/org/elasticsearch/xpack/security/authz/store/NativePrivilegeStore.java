        ...
        if (frozenSecurityIndex.exists() == false) {
            ...
            securityIndexManager.checkIndexVersionThenExecute(listener::onFailure, () -> {
                ...
                try (ThreadContext.StoredContext ignore = stashWithOrigin(client.threadPool().getThreadContext(), SECURITY_ORIGIN)) {
                    SearchRequest request = client.prepareSearch(SECURITY_ALIAS_NAME)
        ...
        if (frozenSecurityIndex.isAvailable() == false) {
            ...
            securityIndexManager.checkIndexVersionThenExecute(listener::onFailure,
                () -> executeAsyncWithOrigin(client.threadPool().getThreadContext(), SECURITY_ORIGIN,
                    client.prepareGet(SECURITY_ALIAS_NAME, "doc", toDocId(application, name)).request(),
    ...
    private void innerPutPrivilege(ApplicationPrivilegeDescriptor privilege, WriteRequest.RefreshPolicy refreshPolicy,
        ...
        try {
            ...
            ClientHelper.executeAsyncWithOrigin(client.threadPool().getThreadContext(), SECURITY_ORIGIN,
                client.prepareIndex(SECURITY_ALIAS_NAME, "doc", toDocId(privilege.getApplication(), name))
        ...
        if (frozenSecurityIndex.exists() == false) {
            ...
            securityIndexManager.checkIndexVersionThenExecute(listener::onFailure, () -> {
                ...
                for (String name : names) {
                    ClientHelper.executeAsyncWithOrigin(client.threadPool().getThreadContext(), SECURITY_ORIGIN,
                        client.prepareDelete(SECURITY_ALIAS_NAME, "doc", toDocId(application, name))
                            .setRefreshPolicy(refreshPolicy)

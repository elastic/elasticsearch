...
public class ApiKeyFieldNameTranslators {
    ...
    public static QueryBuilder translateQueryFields(QueryBuilder qb, @Nullable Consumer<String> visitor) {
        ...
        if (qb instanceof final BoolQueryBuilder query) {
            ...
            query.must().stream().map(q -> translateQueryFields(q, fieldNameVisitor)).forEach(newQuery::must);
            query.should().stream().map(q -> translateQueryFields(q, fieldNameVisitor)).forEach(newQuery::should);
            query.mustNot().stream().map(q -> translateQueryFields(q, fieldNameVisitor)).forEach(newQuery::mustNot);
            query.filter().stream().map(q -> translateQueryFields(q, fieldNameVisitor)).forEach(newQuery::filter);
            return newQuery;

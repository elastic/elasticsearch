...
public class CreateDocsIndexPrivilegeTests extends AbstractPrivilegeTestCase {
    ...
    private static final String CREATE_DOCS_USER = "create_docs_user";
    ...
    public void testAppendOnlyUserCanIndexNewDocumentsWithAutoGeneratedId() throws IOException {
        assertAccessIsAllowed(CREATE_DOCS_USER, "POST", "/" + INDEX_NAME + "/_doc", "{ \"foo\" : \"bar\" }");
    }

    public void testAppendOnlyUserCanIndexNewDocumentsWithExternalIdAndOpTypeIsCreate() throws IOException {
        assertAccessIsAllowed(CREATE_DOCS_USER, "PUT", "/" + INDEX_NAME + "/_doc/2?op_type=create", "{ \"foo\" : \"bar\" }");
    }

    public void testAppendOnlyUserIsDeniedToIndexNewDocumentsWithExternalIdAndOpTypeIsIndex() throws IOException {
        assertAccessIsDenied(CREATE_DOCS_USER, "PUT", "/" + INDEX_NAME + "/_doc/3", "{ \"foo\" : \"bar\" }");
    }

    public void testAppendOnlyUserIsDeniedToIndexUpdatesToExistingDocument() throws IOException {
        assertAccessIsDenied(CREATE_DOCS_USER, "POST", "/" + INDEX_NAME + "/_doc/1/_update", "{ \"doc\" : { \"foo\" : \"baz\" } }");
        assertAccessIsDenied(CREATE_DOCS_USER, "PUT", "/" + INDEX_NAME + "/_doc/1", "{ \"foo\" : \"baz\" }");
    }

    public void testAppendOnlyUserCanIndexNewDocumentsWithAutoGeneratedIdUsingBulkApi() throws IOException {
        assertAccessIsAllowed(CREATE_DOCS_USER, randomFrom("PUT", "POST"),
    ...
    public void testAppendOnlyUserIsDeniedToIndexNewDocumentsWithExternalIdAndOpTypeIsIndexUsingBulkApi() throws IOException {
        assertBodyHasAccessIsDenied(CREATE_DOCS_USER, randomFrom("PUT", "POST"),

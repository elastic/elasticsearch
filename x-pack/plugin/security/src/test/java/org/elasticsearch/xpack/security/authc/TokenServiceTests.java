...
import static org.elasticsearch.xpack.security.authc.TokenService.hashTokenString;
...
import static org.mockito.Mockito.when;
 ...
 public class TokenServiceTests extends ESTestCase {
    ...
    public void testInvalidateRefreshToken() throws Exception {
        ...
        final String userTokenSecret = UUIDs.randomBase64UUID(random());
        final TokenService.RefreshToken refreshToken = newRefreshToken(tokenService, userTokenSecret);
        ...
        final UserToken userToken = buildUserToken(tokenService, userTokenSecret, authentication, Map.of());
        mockGetTokenByRefreshToken(refreshToken, userToken, null);
    ...
    public void testInvalidateRefreshTokenThatIsAlreadyInvalidated() throws Exception {
        ...
        final String clientRefreshToken = tokenFuture.get().getRefreshToken();
        assertNotNull(accessToken);
        mockGetTokenByRefreshToken(
            refreshToken,
            buildUserToken(tokenService, userTokenSecret, authentication, Map.of()),
    ...
    public void testTokenExpiry() throws Exception {
        ...
        Authentication authentication = AuthenticationTestHelper.builder()
            ...
            .realmRef(new RealmRef("native_realm", "native", "node1"))
            .build(false);
        final String userTokenSecret = UUIDs.randomBase64UUID();
        UserToken userToken = buildUserToken(tokenService, userTokenSecret, authentication, Collections.emptyMap());
        mockGetTokenFromId(userToken, null, false);
        ...
        try (ThreadContext.StoredContext ignore = requestContext.newStoredContextPreservingResponseHeaders()) {
            ...
            final SecureString bearerToken = Authenticator.extractBearerTokenFromHeader(requestContext);
            tokenService.tryAuthenticateToken(bearerToken, future);
            final UserToken token = future.get();
            assertNotNull(token);
            assertAuthentication(authentication, token.getAuthentication());
        }
    ...
    public void testGetAuthenticationWorksWithExpiredUserToken() throws Exception {
        ...
        Authentication authentication = AuthenticationTestHelper.builder()
            ...
            .build(false);
        final String userTokenSecret = UUIDs.randomBase64UUID();
        final UserToken expired = buildUserToken(
            tokenService,
            userTokenSecret,
            authentication,
            Collections.emptyMap(),
        ...
        mockGetTokenFromId(expired, null, false);
        final String accessToken = tokenService.prependVersionAndEncodeAccessToken(
            tokenService.getTokenVersionCompatibility(),
            userTokenSecret
        );
    ...
    public void testSupersedingTokenEncryption() throws Exception {
        ...
        final TokenService.RefreshToken refreshToken = newRefreshToken(tokenService, hashTokenString(newAccessToken));
        ...
        String encryptedTokens = tokenService.encryptSupersedingTokens(newAccessToken, newRefreshToken, refreshToken.payload(), iv, salt);
        ...
        tokenService.decryptAndReturnSupersedingTokens(refreshToken, refreshTokenStatus, securityTokensIndex, authentication, tokenFuture);
    ...
    public void testCannotValidateTokenIfLicenseDoesNotAllowTokens() throws Exception {
        ...
        Authentication authentication = AuthenticationTestHelper.builder()
            ...
            .build(false);
        final String userTokenId = UUIDs.randomBase64UUID();
        final UserToken token = buildUserToken(tokenService, userTokenId, authentication, Collections.emptyMap());
        mockGetTokenFromId(token, null, false);
        final String accessToken = tokenService.prependVersionAndEncodeAccessToken(
            tokenService.getTokenVersionCompatibility(),
            userTokenId
        );
    ...
    protected static UserToken buildUserToken(
        ...
        return buildUserToken(tokenService, userTokenSecret, authentication, metadata, tokenService.getExpirationTime());
    }

    private static UserToken buildUserToken(
        TokenService tokenService,
        String userTokenSecret,
        Authentication authentication,
        Map<String, Object> metadata,
        ...
        final String possiblyHashedUserTokenId;
        if (tokenVersion.onOrAfter(TokenService.VERSION_ACCESS_TOKENS_AS_UUIDS)) {
            possiblyHashedUserTokenId = TokenService.hashTokenString(userTokenSecret);
        } else {
            possiblyHashedUserTokenId = userTokenSecret;
        }

        final Authentication tokenAuth = authentication.token().maybeRewriteForOlderVersion(tokenVersion);
        return new UserToken(possiblyHashedUserTokenId, tokenVersion, tokenAuth, expirationTime, metadata);
    }

    private void mockGetTokenFromId(
        UserToken userToken,
        @Nullable Tuple<TokenService.RefreshToken, RefreshTokenStatus> refreshTokenTuple,
        boolean isExpired
    ) {
        doAnswer(invocationOnMock -> {
            GetRequest request = (GetRequest) invocationOnMock.getArguments()[0];
            ...
            if (userToken.getId().equals(request.id().replace("token_", ""))) {
                ...
                final Authentication clientAuthentication = AuthenticationTests.randomServiceAccountAuthentication();
                final BytesReference source;
                if (refreshTokenTuple == null) {
                    source = buildDocumentSource(userToken, null, null, clientAuthentication);
                } else {
                    source = buildDocumentSource(userToken, refreshTokenTuple.v1(), refreshTokenTuple.v2(), clientAuthentication);
                }
                Map<String, Object> sourceMap = XContentHelper.convertToMap(source, false, XContentType.JSON).v2();
                when(response.getSource()).thenReturn(sourceMap);
    ...
    private void mockGetTokenByRefreshToken(
        ...
        if (transportVersion.onOrAfter(TransportVersion.V_8_8_0)) {
            ...
            mockGetTokenFromId(userToken, new Tuple<>(refreshToken, refreshTokenStatus), false);
        } else {
            mockSearchForTokenByRefreshToken(refreshToken, userToken, refreshTokenStatus);
        }
    }

    private void mockSearchForTokenByRefreshToken(
        TokenService.RefreshToken refreshToken,
        UserToken userToken,
        ...
        doAnswer(invocationOnMock -> {
            ...
            assertThat(refreshFilter.value(), is(refreshToken.id()));
            ...
            BytesReference source = buildDocumentSource(userToken, refreshToken, refreshTokenStatus, clientAuthentication);
    ...
    private static BytesReference buildDocumentSource(
        UserToken userToken,
        TokenService.RefreshToken refreshToken,
        RefreshTokenStatus refreshTokenStatus,
        ...
        BytesReference source = TokenService.createTokenDocument(
            userToken,
            refreshToken == null ? null : refreshToken.id(),

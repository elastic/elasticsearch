...
public class FieldPermissionsTests extends ESTestCase {
    ...
    public void testParseFieldPermissions() throws Exception {
        ...
        ElasticsearchParseException e = expectThrows(
            ...
            () -> RoleDescriptor.parser().parse("test", new BytesArray(exceptWithoutGrant), XContentType.JSON)
        );
        assertThat(
        ...
        e = expectThrows(
            ElasticsearchParseException.class,
            () -> RoleDescriptor.parser().parse("test", new BytesArray(grantNull), XContentType.JSON)
        );
        assertThat(
            e.getDetailedMessage(),
        ...
        e = expectThrows(
            ElasticsearchParseException.class,
            () -> RoleDescriptor.parser().parse("test", new BytesArray(exceptNull), XContentType.JSON)
        );
        assertThat(
            e.getDetailedMessage(),
        ...
        e = expectThrows(
            ElasticsearchParseException.class,
            () -> RoleDescriptor.parser().parse("test", new BytesArray(exceptGrantNull), XContentType.JSON)
        );
        assertThat(
            e.getDetailedMessage(),
            containsString("failed to parse indices privileges " + "for role [test]. grant must not be null.")
        ...
        e = expectThrows(
            ElasticsearchParseException.class,
            () -> RoleDescriptor.parser().parse("test", new BytesArray(bothFieldsMissing), XContentType.JSON)
        );
        assertThat(
            e.getDetailedMessage(),
    ...
    public void testBWCFieldPermissions() throws Exception {
        ...
        RoleDescriptor rd = RoleDescriptor.parser().allow2xFormat(true).parse("test", new BytesArray(q), XContentType.JSON);
        assertArrayEquals(rd.getIndicesPrivileges()[0].getGrantedFields(), new String[] { "f1", "f2" });
        assertNull(rd.getIndicesPrivileges()[0].getDeniedFields());
        ...
        rd = RoleDescriptor.parser().allow2xFormat(true).parse("test", new BytesArray(q), XContentType.JSON);
        assertArrayEquals(rd.getIndicesPrivileges()[0].getGrantedFields(), new String[] {});
        assertNull(rd.getIndicesPrivileges()[0].getDeniedFields());

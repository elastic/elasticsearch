...
public class PermissionTests extends ESTestCase {
    ...
    public void testAllowedIndicesMatcherForMappingUpdates() throws Exception {
        for (String mappingUpdateActionName : List.of(PutMappingAction.NAME, AutoPutMappingAction.NAME)) {
            ...
            IndicesPermission.IndexMatcherPredicate indexPredicate = permission.indices().allowedIndicesMatcher(mappingUpdateActionName);
    ...
    public void testAllowedIndicesMatcherActionCaching() throws Exception {
        IndicesPermission.IndexMatcherPredicate matcher1 = permission.indices().allowedIndicesMatcher(GetAction.NAME);
        IndicesPermission.IndexMatcherPredicate matcher2 = permission.indices().allowedIndicesMatcher(GetAction.NAME);
    ...
    // "baz_*foo", "/fool.*bar/"
    private void testAllowedIndicesMatcher(IndicesPermission.IndexMatcherPredicate indicesMatcher) {

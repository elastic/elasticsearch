...
public class CrossClusterAccessTransportInterceptorTests extends AbstractServerTransportInterceptorTests {
    ...
    public void testSendForUserManagedServiceAccountSucceedsWhenRemoteClusterSupportsThem() throws Exception {
        doTestSendWithCrossClusterAccessHeaders(
            false,
            randomAlphaOfLengthBetween(5, 30),
            mock(TransportRequest.class),
            AuthenticationTestHelper.builder().userManagedServiceAccount("foo/bar", "role1").build()
        );
    }
    ...
    private static TransportVersion versionBeforeUserManagedServiceAccounts() {
        return TransportVersionUtils.getPreviousVersion(ServiceAccountInfo.USER_MANAGED_SERVICE_ACCOUNT_INFO);
    }

    /**
     * Resolves to a remote cluster for which no credentials are configured, which is how certificate based remote cluster
     * security presents itself to the interceptor.
     */
    private static
        Function<Transport.Connection, Optional<RemoteConnectionManager.RemoteClusterAliasWithCredentials>>
        mockRemoteClusterWithoutCredentialsResolver(String remoteClusterAlias) {
    ...
    private SendOutcome sendToRemoteCluster(
        Authentication authentication,
        Function<Transport.Connection, Optional<RemoteConnectionManager.RemoteClusterAliasWithCredentials>> credentialsResolver,
        TransportVersion connectionTransportVersion,
        String action
    ) throws Exception {
        authentication.writeToContext(threadContext);
        final AuthorizationService authzService = mock(AuthorizationService.class);
        ...
        final AtomicBoolean senderCalled = new AtomicBoolean(false);
        final TransportInterceptor.AsyncSender sender = interceptor.interceptSender(new TransportInterceptor.AsyncSender() {
            @Override
            public <T extends TransportResponse> void sendRequest(
                Transport.Connection connection,
                ...
                assertTrue("sender called more than once", senderCalled.compareAndSet(false, true));
            }
        });

...
/**
 * Tests that inbound messages on the RCS transport port are properly validated (because this port can be exposed
 * without TLS authentication):
 *  - that request header size limits are enforced (connection dropped)
 *  - that responses are not permitted (the RCS port is only used for requests)
 */
public final class SecurityNetty4TransportHeaderValidationTests extends ESTestCase {
    ...
    public void testThatResponsesAreNotAcceptableForTheRemoteClusterPort() throws Exception {
        int messageLength = maxHeaderSize + 10;
        long requestId = randomLongBetween(1L, 1000L);
        // int headerSize = maxHeaderSize - 10;
        try (
            ReleasableBytesStreamOutput out = new ReleasableBytesStreamOutput(
                messageLength + TcpHeader.BYTES_REQUIRED_FOR_MESSAGE_SIZE,
                bigarrays
            ...
            TestResponse testResponse = new TestResponse("testThatResponsesAreNotAcceptableForTheRemoteClusterPort");
            // assembleDummyResponse(out, messageLength, requestId, headerSize);

            try (Socket socket = new MockSocket(remoteIngressHost, remoteIngressPort)) {
                testResponse.writeTo(out);
                socket.getOutputStream().write(out.bytes().array());
                socket.getOutputStream().flush();
    ...
    private void assembleDummyResponse(BytesStreamOutput out, int messageLength, long requestId, int variableHeaderSize)
        throws IOException {
        ...
        out.writeByte((byte) 1); // status byte (response)

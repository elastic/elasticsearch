...
/**
 * Contributes the {@link SlmEncryptedDataHandler} to the project encryption key rotation coordinator.
 * Discovered by the encryption plugin via the {@code META-INF/services} SPI entry for
 * {@link EncryptedDataHandlerProvider}.
 */
public final class SlmEncryptedDataHandlerProvider implements EncryptedDataHandlerProvider {

    @Override
    public Collection<EncryptedDataHandler<?>> getHandlers() {
        return List.of(new SlmEncryptedDataHandler());
    }
}

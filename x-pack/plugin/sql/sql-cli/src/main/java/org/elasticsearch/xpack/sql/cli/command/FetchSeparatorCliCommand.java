...
public class FetchSeparatorCliCommand extends AbstractCliCommand {
    ...
    protected boolean doHandle(CliTerminal terminal, CliSession cliSession, Matcher m, String line) {
        cliSession.getConfiguration().setFetchSeparator(m.group(1));
        terminal.line().text("fetch separator set to \"").em(cliSession.getConfiguration().getFetchSeparator()).text("\"").end();
        return true;
    }
}

 ...
 *
 * The {@link org.elasticsearch.xpack.sql.action.compute.data.Page} class is the batched columnar representation of data
 * that's passed around in the compute engine. Pages are immutable and thread-safe.
 * The {@link org.elasticsearch.xpack.sql.action.compute.operator.Operator} interface is the low-level building block that consumes,
 * transforms and produces data in the compute engine.
 * Each {@link org.elasticsearch.xpack.sql.action.compute.operator.Driver} operates in single-threaded fashion on a simple chain of
 ...
 * (see {@link org.elasticsearch.xpack.sql.action.compute.operator.exchange.ExchangeSinkOperator} and
 * {@link org.elasticsearch.xpack.sql.action.compute.operator.exchange.ExchangeSourceOperator}).
 * Exchanges can be thought of as simple FIFO queues of pages
 * (see {@link org.elasticsearch.xpack.sql.action.compute.operator.exchange.ExchangeSource}).
 ...
 * They allow multi-plexing via an {@link org.elasticsearch.xpack.sql.action.compute.operator.exchange.Exchanger}, broadcasting one

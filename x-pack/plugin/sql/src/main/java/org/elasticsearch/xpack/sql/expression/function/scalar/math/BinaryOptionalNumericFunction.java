...
public abstract class BinaryOptionalNumericFunction extends ScalarFunction {
    ...
    public BinaryOptionalNumericFunction(Location location, Expression left, Expression right) {
        super(location, right != null ? Arrays.asList(left, right) : Arrays.asList(left));
        this.left = left;
        this.right = right;
    }
    ...
    protected TypeResolution resolveType() {
        ...
        TypeResolution resolution = isNumeric(left, functionName(), ParamOrdinal.FIRST);
        if (resolution.unresolved()) {
            return resolution;

        }

        return right == null ? TypeResolution.TYPE_RESOLVED : isInteger(right, functionName(), ParamOrdinal.SECOND);
    }

    @Override
    protected Pipe makePipe() {
        return new BinaryOptionalMathPipe(location(), this,

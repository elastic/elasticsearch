...
public class Replace extends ScalarFunction {
    ...
    protected TypeResolution resolveType() {
        ...
        TypeResolution sourceResolution = isStringAndExact(source, functionName(), ParamOrdinal.FIRST);
        if (sourceResolution.unresolved()) {
            return sourceResolution;
        }

        TypeResolution patternResolution = isStringAndExact(pattern, functionName(), ParamOrdinal.SECOND);
        if (patternResolution.unresolved()) {
            return patternResolution;
        }

        return isStringAndExact(replacement, functionName(), ParamOrdinal.THIRD);
    }

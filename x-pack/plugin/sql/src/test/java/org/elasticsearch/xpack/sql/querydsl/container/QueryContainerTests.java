...
public class QueryContainerTests extends ESTestCase {
    ...
    public void testRewriteToContainNestedFieldNoQuery() {
        Query expected = new NestedQuery(source, path, singletonMap(name, hasDocValues), new MatchAll(source));
        assertEquals(expected, QueryContainer.rewriteToContainNestedField(null, source, path, name, hasDocValues));
    }
    ...
    public void testRewriteToContainsNestedFieldWhenContainsNestedField() {
        ...
        assertSame(original, QueryContainer.rewriteToContainNestedField(original, source, path, name, randomBoolean()));
    }

...
public class StatelessShardsAvailabilityHealthIndicatorServiceTests extends ESTestCase {
    ...
    /**
     * Pre-handoff reshard targets keep {@link RecoverySource.Type#RESHARD_SPLIT} across failed recoveries (e.g. source relocates and
     * cancels start-split). Those failures must stay provisional/GREEN rather than RED while the source still serves the data.
     */
    public void testHealthWhileReshardSplitTargetShardsInactiveAfterFailedAllocation() {
        final var projectId = randomProjectIdOrDefault();
        final var indexMetadata = IndexMetadata.builder(INDEX_NAME)
            .settings(Settings.builder().put(IndexMetadata.SETTING_VERSION_CREATED, IndexVersion.current()).build())
            .numberOfShards(2)
        ...
        final var targetShardId = new ShardId(index.getName(), index.getUUID(), 1);
        final var unassignedAt = new TimeValue(System.currentTimeMillis() - TimeValue.timeValueMinutes(5).millis(), TimeUnit.MILLISECONDS);
        final var failedAllocationInfo = unassignedInfo(

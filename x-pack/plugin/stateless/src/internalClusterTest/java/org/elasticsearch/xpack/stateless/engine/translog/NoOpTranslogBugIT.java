...
/// Proves that [org.elasticsearch.common.util.MockPageCacheRecycler] detects the translog page leak
/// introduced when [org.elasticsearch.action.bulk.BulkPrimaryExecutionContext] skips setting
/// `locationToSync` for no-op FAILURE results (the bug fixed in elastic/elasticsearch#147367).
///
/// Requires [NodeMocksPlugin] so that [org.elasticsearch.common.util.MockBigArrays] is wired in,
/// which in turn wraps the node's [org.elasticsearch.common.util.PageCacheRecycler] with
/// [org.elasticsearch.common.util.MockPageCacheRecycler]. Pages allocated by the translog buffer
/// (via [org.elasticsearch.transport.BytesRefRecycler]) are tracked in `ACQUIRED_PAGES` and must be
/// released before [org.elasticsearch.test.ESTestCase#after] or [org.elasticsearch.test.ESIntegTestCase#afterClass]
/// completes.
public class NoOpTranslogBugIT extends AbstractStatelessPluginIntegTestCase {

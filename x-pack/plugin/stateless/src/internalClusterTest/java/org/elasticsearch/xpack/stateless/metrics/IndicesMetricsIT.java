...
public class IndicesMetricsIT extends AbstractStatelessIntegTestCase {
    ...
    public void testIndicesMetrics() throws Exception {
        ...
        final TestTelemetryPlugin indexTelemetry = internalCluster().getInstance(PluginsService.class, indexNode)
        ...
        // search and fetch
        final TestTelemetryPlugin searchTelemetry = internalCluster().getInstance(PluginsService.class, searchNode)
            .filterPlugins(TestTelemetryPlugin.class)
            .findFirst()
            .orElseThrow();
        searchTelemetry.resetMeter();
        indicesService = internalCluster().getInstance(IndicesService.class, searchNode);
        SearchResponse resp = client().prepareSearch("standard*").setSize(1000).get();
        ElasticsearchAssertions.assertHitCount(resp, numStandardDocs);
        resp.decRef();
        ...
        client().prepareSearch("time*").setSize(1000).get().decRef();
        ...
        client().prepareSearch("logs*").setSize(1000).get().decRef();

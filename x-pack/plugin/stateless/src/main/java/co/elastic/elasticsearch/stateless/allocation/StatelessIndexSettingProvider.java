...
public class StatelessIndexSettingProvider implements IndexSettingProvider {
    ...
    public static final Setting<Integer> DEFAULT_NUMBER_OF_SHARDS_FOR_REGULAR_INDICES_SETTING = Setting.intSetting(
        "indices.regular_indices.number_of_shards",
        3,
        1,
        5,
        Property.NodeScope,
    ...
    public void initialize(ClusterService clusterService, IndexNameExpressionResolver indexNameExpressionResolver) {
        clusterService.getClusterSettings()
            .initializeAndWatch(
                ...
                newValue -> defaultNumberOfShardsForRegularIndices = newValue
            );
    ...
    public Settings getAdditionalIndexSettings(
        ...
        assert defaultNumberOfShardsForRegularIndices != 0 || systemNamePredicate == null : "object is not initialized properly";
        ...
        if (dataStreamName == null
            ...
            logger.debug(
                () -> Strings.format(
                    "apply stateless number_of_shards default of [%s] to index [%s]",
                    defaultNumberOfShardsForRegularIndices,
                    indexName
                )
            );
            settings.put(INDEX_NUMBER_OF_SHARDS_SETTING.getKey(), defaultNumberOfShardsForRegularIndices);
        }

...
public class IndexEngineDeletionPolicy extends ElasticsearchIndexDeletionPolicy {
    ...
    private void processCommits(List<? extends IndexCommit> commits, CheckedConsumer<List<? extends IndexCommit>, IOException> consumer)
        ...
        if (wrappedCommits.isEmpty() == false) {
            ...
            var latest = acquireIndexCommit(false);
            boolean success = false;
            try {
                var additionalFiles = new HashSet<>(latest.getFileNames());
                if (previous != null) {
                    assert previous.getGeneration() <= latest.getGeneration();
                    additionalFiles.removeAll(previous.getFileNames());
                }
                this.lastCommit = latest.getIndexCommit();
                if (commitsListener != null && (previous == null || previous.getGeneration() != latest.getIndexCommit().getGeneration())) {
                    commitsListener.onNewCommit(new Engine.IndexCommitRef(latest, () -> releaseIndexCommit(latest)), additionalFiles);
                    success = true;
                }
            } finally {
                if (success == false) {
                    releaseIndexCommit(latest);
                }
            }
        }
    ...
    public synchronized SoftDeleteIndexCommit acquireIndexCommit(boolean acquiringSafeCommit) {
        return commitsRefs.incRef(delegate.acquireIndexCommit(acquiringSafeCommit));
    }
    ...
    public boolean hasAcquiredIndexCommitsForTesting() {
        return delegate.hasAcquiredIndexCommitsForTesting();
    }

...
public class TranslogReplicatorReader implements Translog.Snapshot {
    ...
    public TranslogReplicatorReader(
        ...
        // TODO: Change replay to return an iterator of Record instead of Operation
    ...
    private Iterator<Translog.Operation> getOperationIterator(String name, StreamInput streamInput, CompoundTranslogHeader translogHeader)
        ...
        if (metadata.containsKey(shardId) && metadata.get(shardId).operations().totalOps() != 0) {
            ...
            if (toSeqNo >= operations.minSeqNo() && fromSeqNo <= operations.maxSeqNo()) {
                ...
                // numOps counts logical operations, not records: a batch record contributes every operation it holds
                ...
                // If there are batch records, the loop below can consume fewer records than numOps.
                long opsRead = 0;
                while (opsRead < numOps) {
                    Translog.Record record = Translog.readRecord(checksumStreamInput);
                    if (record instanceof Translog.Operation operation) {
                        opsRead++;
                ...
                assert checksumStreamInput.available() == 0
                    : "shard region has " + checksumStreamInput.available() + " unread bytes after " + opsRead + " operations";

                filesWithShardOperations++;
                operationsRead += opsRead;
    ...
    private void updateOperationMetrics(Translog.Operation operation) {

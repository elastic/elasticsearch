...
public class SearchLoadProbeTests extends ESTestCase {
    ...
    public void testCalculateSearchLoadForExecutor() {
        ...
        assertThat(calculateSearchLoadForExecutor(0.0, 0.0, 0, maxTimeToClearQueue, randomQueueContribution), closeTo(0.0, 1e-3));
        // When there is nothing in the queue, we'd still want to keep up with average load
        assertThat(
            calculateSearchLoadForExecutor(1.0, timeValueMillis(100).nanos(), 0, maxTimeToClearQueue, randomQueueContribution),
            closeTo(1.0, 1e-3)
        );
        ...
        assertThat(
            calculateSearchLoadForExecutor(1.0, timeValueMillis(100).nanos(), 1, maxTimeToClearQueue, randomDoubleBetween(0.01, 100, true)),
            closeTo(1.01, 1e-3)
        );
        assertThat(
            calculateSearchLoadForExecutor(
        ...
        assertThat(calculateSearchLoadForExecutor(2.0, timeValueMillis(100).nanos(), 1000, maxTimeToClearQueue, 4.00), closeTo(6.0, 1e-3));
    }
    ...
    public void testGetSearchLoad() {
        ...
        statsPerExecutor.put(Names.SEARCH, new SearchExecutorStats(3.0, timeValueMillis(200).nanos(), 0, between(1, 10)));
        statsPerExecutor.put(Names.SEARCH_COORDINATION, new SearchExecutorStats(2.0, timeValueMillis(70).nanos(), 0, between(1, 10)));
        assertThat(searchLoadProbe.getSearchLoad(), closeTo(5.0, 1e-3));
        ...
        statsPerExecutor.put(Names.SEARCH, new SearchExecutorStats(3.0, timeValueMillis(200).nanos(), queueSize, 1));
        statsPerExecutor.put(Names.SEARCH_COORDINATION, new SearchExecutorStats(2.0, timeValueMillis(70).nanos(), 0, 1));
        var expectedExtraThreads = queueEmpty ? 0.0 : 1.0;
        assertThat(searchLoadProbe.getSearchLoad(), closeTo(5.0 + expectedExtraThreads, 1e-3));
    }
}

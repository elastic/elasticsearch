...
public class IndexEngineTests extends AbstractEngineTestCase {
    ...
    public void testDeleteUnownedDocumentsSetsReshardCleanupUUID() throws Exception {
        ...
        try (var engine = newIndexEngine(indexConfig(settings))) {
            ...
            assertNull(engine.getReshardCleanupUUID());
            assertNull(engine.getLastCommittedSegmentInfos().getUserData().get(Engine.RESHARD_CLEANUP_UUID_KEY));
            ...
            String firstUuid = engine.getReshardCleanupUUID();
            assertNotNull(firstUuid);
            assertThat(engine.getLastCommittedSegmentInfos().getUserData().get(Engine.RESHARD_CLEANUP_UUID_KEY), equalTo(firstUuid));

            engine.deleteUnownedDocuments(new ShardSplittingQuery(metadata, 0, false));
            engine.flush(true, true);

            String secondUuid = engine.getReshardCleanupUUID();
            assertNotNull(secondUuid);
            assertThat(secondUuid, not(equalTo(firstUuid)));
            assertThat(engine.getLastCommittedSegmentInfos().getUserData().get(Engine.RESHARD_CLEANUP_UUID_KEY), equalTo(secondUuid));
        }
    }
}

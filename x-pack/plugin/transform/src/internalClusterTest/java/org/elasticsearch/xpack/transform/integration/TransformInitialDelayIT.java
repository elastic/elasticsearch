...
public class TransformInitialDelayIT extends TransformSingleNodeTestCase {
    ...
    /**
     * With {@code _start?initial_delay=0s} the first checkpoint processes data up to "now", so an already-present (backfilled)
     * document is summarized into the destination index immediately, despite a conservative 60s steady-state delay. Without the
     * override the document would only appear after it ages past the 60s watermark.
     */
    public void testInitialDelayProcessesBackfillImmediately() throws Exception {
        ...
        // initial_delay is a one-time _start parameter, not part of the persisted config.
        client().execute(
            StartTransformAction.INSTANCE,
            new StartTransformAction.Request(transformId, null, TimeValue.ZERO, TimeValue.THIRTY_SECONDS)

...
class TransformClusterStateListener implements ClusterStateListener {
    ...
    private static void createAuditAliasForDataFrameBWC(ClusterState state, Client client, final ActionListener<Boolean> finalListener) {
        ...
        if (state.getMetaData().getIndexSpaceLookup().containsKey(TransformInternalIndexConstants.AUDIT_INDEX_DEPRECATED) == false) {
            finalListener.onResponse(false);
            return;
        }

        if (state.getMetaData().getIndexSpaceLookup().get(TransformInternalIndexConstants.AUDIT_INDEX_DEPRECATED).getIndices().stream()

            ...
            for (int shardIndex = 0; shardIndex < numShards; shardIndex++) {
                ...
                for (int replica = 0; replica < numShardCopies; replica++) {
                    ...
                    if (inconsistentReplica == replica) {
                        ...
                        shardStats.add(
                            new ShardStats(shardRouting, new ShardPath(false, path, path, shardId), stats, null, invalidSeqNoStats, null, fieldUsageStats)
                        );
                    } else {
                        shardStats.add(
                            new ShardStats(shardRouting, new ShardPath(false, path, path, shardId), stats, null, validSeqNoStats, null, fieldUsageStats)
                        );
                    }
                }
            }

...
public class TransformPersistentTasksExecutorTests extends ESTestCase {
    ...
    public void testNodeVersionAssignment() {
        ...
        assertThat(
            executor.getAssignment(new TransformTaskParams("new-task-id", Version.CURRENT, null, true), cs.nodes().copyToCollection(), cs)
                .getExecutorNode(),
            equalTo("current-data-node-with-1-tasks")
        );
        assertThat(
            executor.getAssignment(new TransformTaskParams("new-task-id", Version.CURRENT, null, false), cs.nodes().copyToCollection(), cs)
                .getExecutorNode(),
            equalTo("current-data-node-with-0-tasks-transform-remote-disabled")
        );
        assertThat(
            executor.getAssignment(
                new TransformTaskParams("new-old-task-id", Version.V_7_7_0, null, true),
                cs.nodes().copyToCollection(),
                cs
            ).getExecutorNode(),

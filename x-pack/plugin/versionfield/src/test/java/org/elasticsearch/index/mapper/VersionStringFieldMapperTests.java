...
public class VersionStringFieldMapperTests extends ESSingleNodeTestCase {
    ...
    public void testQueries() throws Exception {
        ...
        assertEquals(0, response.getHits().getTotalHits().value);
        response = client().prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", ("1.3.0+build1234567"))).get();
        assertEquals(1, response.getHits().getTotalHits().value);
        ...
        response = client().prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("0.1.0").to("2.1.0-alpha")).get();
        assertEquals(3, response.getHits().getTotalHits().value);

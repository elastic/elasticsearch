...
public class BootStrapTests extends AbstractWatcherIntegrationTestCase {
    ...
    public void testLoadMalformedWatchRecord() throws Exception {
        ...
        client().prepareIndex().setIndex(HistoryStoreField.DATA_STREAM_NAME).setId(wid.value())
    ...
    public void testWatchRecordSavedTwice() throws Exception {
        ...
        for (int i = 0; i < numRecords; i++) {
            ...
            bulkRequestBuilder.add(client().prepareIndex().setIndex(HistoryStoreField.DATA_STREAM_NAME).setId(watchRecord.id().value())
        ...
        assertBusy(() -> {
            ...
            refresh();
            SearchResponse searchResponse = client().prepareSearch(HistoryStoreField.DATA_STREAM_NAME).setSize(numRecords).get();

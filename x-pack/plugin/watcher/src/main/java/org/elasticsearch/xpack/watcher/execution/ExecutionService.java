    ...
    private void forcePutHistory(WatchRecord watchRecord) {
        ...
        try {
            try (
                ...
                IndexRequest request = new IndexRequest(index).id(watchRecord.id().value())
                    .source(builder)
                ...
                try (
                    ...
                    IndexRequest request = new IndexRequest(index).id(watchRecord.id().value()).source(xContentBuilder.value(watchRecord));
                    client.index(request).get(30, TimeUnit.SECONDS);
                }

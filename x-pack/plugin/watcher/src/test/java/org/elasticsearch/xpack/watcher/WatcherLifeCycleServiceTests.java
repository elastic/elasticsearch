...
public class WatcherLifeCycleServiceTests extends ESTestCase {
    ...
    public void testStartWithStateNotRecoveredBlock() {
        DiscoveryNodes.Builder nodes = new DiscoveryNodes.Builder().add(
            TestDiscoveryNode.builder("id1").roles(new HashSet<>(DiscoveryNodeRole.roles())).build()
    ...
    public void testNonDataNode() {
        ...
        DiscoveryNode node1 = TestDiscoveryNode.builder("node_1")
            .roles(new HashSet<>(asList(randomFrom(DiscoveryNodeRole.INGEST_ROLE, DiscoveryNodeRole.MASTER_ROLE))))
            .build();

        DiscoveryNode node2 = TestDiscoveryNode.builder("node_2").roles(new HashSet<>(asList(DiscoveryNodeRole.DATA_ROLE))).build();

        DiscoveryNode node3 = TestDiscoveryNode.builder("node_3").roles(new HashSet<>(asList(DiscoveryNodeRole.DATA_ROLE))).build();
        ...
        ClusterState state = ClusterState.builder(new ClusterName("my-cluster"))
            .nodes(
                new DiscoveryNodes.Builder().masterNodeId("node_1")
                    .localNodeId("node_1")
                    .add(TestDiscoveryNode.builder("node_1").roles(new HashSet<>(roles)).build())
            )
            .build();

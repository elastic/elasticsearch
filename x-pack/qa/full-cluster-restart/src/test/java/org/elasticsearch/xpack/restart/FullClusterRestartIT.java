...
public class FullClusterRestartIT extends ESRestTestCase {
     ...
     * Tests that a RollUp job created on a old cluster is correctly restarted after the upgrade.
     */
    public void testRollupIndexIncompatibilityAfterRestart() throws Exception {
        assumeTrue("Rollup 6.3 index not compatible with 6.4+",
            oldClusterVersion.onOrAfter(Version.V_6_3_0) && oldClusterVersion.before(Version.V_6_4_0));
        ...
        if (runningAgainstOldCluster) {
            ...
            Map<String, Object> createRollupJobResponse = toMap(client().performRequest(createRollupJobRequest2));
            assertThat(createRollupJobResponse.get("acknowledged"), equalTo(Boolean.TRUE));

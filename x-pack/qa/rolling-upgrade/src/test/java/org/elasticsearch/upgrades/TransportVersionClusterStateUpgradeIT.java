...
public class TransportVersionClusterStateUpgradeIT extends AbstractUpgradeTestCase {
    ...
    public void testReadsInferredTransportVersions() throws IOException {
        ...
        assumeTrue("Only runs on the upgraded cluster", CLUSTER_TYPE == ClusterType.MIXED);
        ...
        Map<?, ?> tvs = ((List<?>) localResponse.get("transport_versions")).stream()
            .map(o -> (Map<?, ?>) o)
            .collect(Collectors.toMap(m -> m.get("node_id"), m -> m.get("transport_version")));

        // any node that is >=8.8.0 should use an inferred version
        // TODO
    }
    ...
    public void testCompletesRealTransportVersions() throws IOException {
        ...
        Map<?, ?> tvs = ((List<?>) response.get("transport_versions")).stream()
            .map(o -> (Map<?, ?>) o)
            .collect(Collectors.toMap(m -> m.get("node_id"), m -> m.get("transport_version")));

        assertThat(tvs.values(), everyItem(equalTo(TransportVersion.V_8_8_0.toString())));
    }
}

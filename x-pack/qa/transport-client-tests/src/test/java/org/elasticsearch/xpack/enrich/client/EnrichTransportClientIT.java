...
public class EnrichTransportClientIT extends ESXPackSmokeClientTestCase {
    ...
    private static void assertEqualPolicies(EnrichPolicy expectedInstance, EnrichPolicy newInstance) {
        ...
        if (newInstance.getQuery() != null) {
            ...
            assertThat(newInstance.getQuery().getQueryAsMap(), Matchers.equalTo(expectedInstance.getQuery().getQueryAsMap()));
        } else {
            assertThat(expectedInstance.getQuery(), nullValue());
        }
        assertThat(newInstance.getIndices(), Matchers.equalTo(expectedInstance.getIndices()));
        assertThat(newInstance.getMatchField(), Matchers.equalTo(expectedInstance.getMatchField()));
        assertThat(newInstance.getEnrichFields(), Matchers.equalTo(expectedInstance.getEnrichFields()));
    }

...
public class StatelessDistributionDescriptor implements DistributionDescriptor {
    ...
    public Path getDistributionDir() {
        ...
        Path statelessDir = defaultDir.getParent().resolve("elasticsearch");
        if (Files.exists(statelessDir) == false) {
            throw new IllegalStateException("Could not locate elasticsearch distribution. Tried: " + defaultDir + " and " + statelessDir);
        }
        return statelessDir;
    }
